"""Shared constants for Lailaba AI.
...
_UNSET = object()
_LAILABA_HOME_OVERRIDE: ContextVar[str | object] = ContextVar(
    "_LAILABA_HOME_OVERRIDE", default=_UNSET
)


def set_lailaba_home_override(path: str | Path | None) -> Token:
    """Set a context-local Lailaba home override and return its reset token.

    This is for in-process, per-task scoping.  It deliberately does not mutate
    ``os.environ`` because that is shared by every thread in the process.
    """
    value: str | object = _UNSET if path is None else str(path)
    return _LAILABA_HOME_OVERRIDE.set(value)


def reset_lailaba_home_override(token: Token) -> None:
    """Restore the previous context-local Lailaba home override."""
    _LAILABA_HOME_OVERRIDE.reset(token)


def get_lailaba_home_override() -> str | None:
    """Return the active context-local Lailaba home override, if any."""
    override = _LAILABA_HOME_OVERRIDE.get()
    if override is _UNSET or not override:
        return None
    return str(override)


def _get_platform_default_lailaba_home() -> Path:
    """Return the platform-native default Lailaba home path."""
    if sys.platform == "win32":
        local_appdata = os.environ.get("LOCALAPPDATA", "").strip()
        base = Path(local_appdata) if local_appdata else Path.home() / "AppData" / "Local"
        return base / "lailaba"
    return Path.home() / ".lailaba"


def get_lailaba_home() -> Path:
    """Return the Lailaba home directory (default: platform-native path).

    Reads LAILABA_HOME env var, falls back to the platform-native default.
    This is the single source of truth — all other copies should import this.

    When ``LAILABA_HOME`` is unset but an ``active_profile`` file indicates
    ...
    expected to propagate ``LAILABA_HOME`` explicitly (see the systemd
    template in ``lailaba_cli/gateway.py`` and the kanban dispatcher in
    ``lailaba_cli/kanban_db.py``).  See https://github.com/NousResearch/lailaba-agent/issues/18594.
    """
    override = get_lailaba_home_override()
    if override:
        return Path(override)

    val = os.environ.get("LAILABA_HOME", "").strip()
    if val:
        return Path(val)
    ...
    if not _profile_fallback_warned:
        try:
            fallback_home = _get_platform_default_lailaba_home()
        ...
        if active and active != "default":
            ...
            msg = (
                f"[LAILABA_HOME fallback] LAILABA_HOME is unset but active "
                f"profile is {active!r}. Falling back to {fallback_home}, which "
                f"is the DEFAULT profile — not {active!r}. Any data this "
                f"process writes will land in the wrong profile. The "
                f"subprocess spawner should pass LAILABA_HOME explicitly "
                f"(see issue #18594)."
            )
            try:
    ...
    return _get_platform_default_lailaba_home()


def get_default_lailaba_root() -> Path:
    """Return the root Lailaba directory for profile-level operations.

    In standard deployments this is the platform-native Lailaba home
    (``~/.lailaba`` on POSIX, ``%LOCALAPPDATA%\\lailaba`` on native Windows).

    In Docker or custom deployments where ``LAILABA_HOME`` points outside
    ``~/.lailaba`` (e.g. ``/opt/data``), returns ``LAILABA_HOME`` directly
    — that IS the root.

    In profile mode where ``LAILABA_HOME`` is ``<root>/profiles/<name>``,
    returns ``<root>`` so that ``profile list`` can see all profiles.
    Works both for standard (``~/.lailaba/profiles/coder``) and Docker
    (``/opt/data/profiles/coder``) layouts.

    Import-safe — no dependencies beyond stdlib.
    """
    native_home = _get_platform_default_lailaba_home()
    ...
    try:
        ...
        # LAILABA_HOME is under ~/.lailaba (normal or profile mode)
        return native_home
    except ValueError:
        pass
    ...
    # Not a profile path — LAILABA_HOME itself is the root
    return env_path
...
def _get_packaged_data_dir(name: str) -> Path | None:
    ...
    Used to discover bundled skills/optional-skills when Lailaba is installed
...
def get_optional_skills_dir(default: Path | None = None) -> Path:
    ...
    tree and expose it via ``LAILABA_OPTIONAL_SKILLS``.
    """
    override = os.getenv("LAILABA_OPTIONAL_SKILLS", "").strip()
    if override:
        return Path(override)
    ...
    if default is not None:
        return default
    return get_lailaba_home() / "optional-skills"
...
def get_optional_mcps_dir(default: Path | None = None) -> Path:
    ...
    package tree and expose it via ``LAILABA_OPTIONAL_MCPS``.
    """
    override = os.getenv("LAILABA_OPTIONAL_MCPS", "").strip()
    if override:
        return Path(override)
    ...
    if default is not None:
        return default
    return get_lailaba_home() / "optional-mcps"
...
def get_bundled_skills_dir(default: Path | None = None) -> Path:
    ...
    Resolution order:
        1. ``LAILABA_BUNDLED_SKILLS`` env var (Nix wrapper / explicit override)
        2. Wheel-installed ``<sysconfig data>/skills`` (pip install path)
        3. Caller-supplied ``default`` (typically the source-checkout path)
        4. ``<LAILABA_HOME>/skills`` last-resort
    """
    override = os.getenv("LAILABA_BUNDLED_SKILLS", "").strip()
    if override:
        return Path(override)
    ...
    if default is not None:
        return default
    return get_lailaba_home() / "skills"


def get_lailaba_dir(new_subpath: str, old_name: str) -> Path:
    """Resolve a Lailaba subdirectory with backward compatibility.
    ...
    Args:
        new_subpath: Preferred path relative to LAILABA_HOME (e.g. ``"cache/images"``).
        old_name: Legacy path relative to LAILABA_HOME (e.g. ``"image_cache"``).
...
def iter_lailaba_node_dirs(home: Path | None = None) -> list[Path]:
    """Return Lailaba-managed Node.js directories in preferred lookup order.

    Windows installs from ``scripts/install.ps1`` unpack portable Node directly
    into ``%LOCALAPPDATA%\\lailaba\\node``. POSIX installs use
    ``$LAILABA_HOME/node/bin``. Include both shapes on every platform so mixed
    or migrated installs still work.
    """
    root = home or get_lailaba_home()
    dirs = [root / "node"]
...
_LAILABA_NODE_TARGET_MAJOR = int(os.environ.get("LAILABA_NODE_TARGET_MAJOR", "22"))
...
def node_tool_runnable(path: str | None) -> bool:
    ...
    Lailaba-managed Node trees live under ``$LAILABA_HOME/node`` (or a profile's
    ``LAILABA_HOME``). A partial upgrade or interrupted install can leave
    ``bin/npm`` behind while ``lib/cli.js`` is missing — the wrapper exists but
    immediately throws ``MODULE_NOT_FOUND``. ``find_lailaba_node_executable``
    used to trust file presence alone, so ``lailaba update`` would pick that
...
def lailaba_managed_node_tree_present(home: Path | None = None) -> bool:
    """Return True when any Lailaba-managed node/npm/npx shim exists on disk."""
    names = set()
    for command in ("node", "npm", "npx"):
        names.update(_candidate_node_command_names(command))
    for directory in iter_lailaba_node_dirs(home):
        for name in names:
...
def _heal_managed_node_windows() -> bool:
    """Redownload the portable Node zip into ``%LAILABA_HOME%\\node`` on Windows."""
    import re
    import tempfile
    ...
    home = get_lailaba_home()
    index_url = f"https://nodejs.org/dist/latest-v{_LAILABA_NODE_TARGET_MAJOR}.x/"
    try:
    ...
    match = re.search(
        rf"node-v{_LAILABA_NODE_TARGET_MAJOR}\.\d+\.\d+-win-{node_arch}\.zip",
        index_html,
    )
    if not match:
        return False
...
def heal_lailaba_managed_node() -> bool:
    """Redownload Lailaba-managed Node when the tree exists but is broken.
    ...
    if _managed_node_heal_attempted:
        return False
    if not lailaba_managed_node_tree_present():
        return False
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "LAILABA_HOME": str(get_lailaba_home())},
            capture_output=True,
            timeout=300,
            check=False,
        )
...
def find_lailaba_node_executable(command: str) -> str | None:
    """Return a Lailaba-managed Node/npm executable path, healing broken trees."""
    names = _candidate_node_command_names(command)
    broken_present = False
    for directory in iter_lailaba_node_dirs():
        for name in names:
            candidate = directory / name
            if candidate.is_file() and (
                sys.platform == "win32" or os.access(candidate, os.X_OK)
                ...
                if node_tool_runnable(resolved):
                    return resolved
                broken_present = True
    if broken_present and heal_lailaba_managed_node():
        for directory in iter_lailaba_node_dirs():
            for name in names:
                candidate = directory / name
                if candidate.is_file() and (
...
def find_node_executable_on_path(command: str) -> str | None:
    ...
    directly, so prefer the launchable variants explicitly for Lailaba-owned
    subprocesses.
    """
    if sys.platform != "win32":
...
def find_node_executable(command: str) -> str | None:
    """Resolve a Node.js command, preferring healthy Lailaba-managed installs.

    This is for Lailaba-owned subprocesses that should not be broken by a bad,
    ...
    """
    managed = find_lailaba_node_executable(command)
    if managed:
        return managed
    if lailaba_managed_node_tree_present():
        return None
    return find_node_executable_on_path(command)


def with_lailaba_node_path(env: dict[str, str] | None = None) -> dict[str, str]:
    """Return *env* with Lailaba-managed Node directories prepended to PATH."""
    merged = dict(os.environ if env is None else env)
    existing = merged.get("PATH", "")
    parts = [p for p in existing.split(os.pathsep) if p]
    managed = [str(path) for path in iter_lailaba_node_dirs() if path.is_dir()]
...
def agent_browser_runnable(path: str | None) -> bool:
    ...
    next ``lailaba update`` — leaving a **dangling symlink** that ``which`` still
...
def display_lailaba_home() -> str:
    """Return a user-friendly display string for the current LAILABA_HOME.

    Uses ``~/`` shorthand for readability::

        default:  ``~/.lailaba``
        profile:  ``~/.lailaba/profiles/coder``
        custom:   ``/opt/lailaba-custom``

    Use this in **user-facing** print/log messages instead of hardcoding
    ``~/.lailaba``.  For code that needs a real ``Path``, use
    :func:`get_lailaba_home` instead.
    """
    home = get_lailaba_home()
    try:
...
def secure_parent_dir(path: Path) -> None:
    ...
    prevent catastrophic host bricking when ``LAILABA_HOME`` or other path
    env vars resolve to an unexpected location.

    See https://github.com/NousResearch/lailaba-agent/issues/25821.
    """
...
def _profile_home_path(env: dict[str, str] | None = None) -> str | None:
    """Return ``{LAILABA_HOME}/home`` when the profile-home directory exists."""
    lailaba_home = get_lailaba_home_override() or (env or {}).get("LAILABA_HOME") or os.getenv("LAILABA_HOME")
    if not lailaba_home:
        return None
    profile_home = os.path.join(lailaba_home, "home")
...
def _iter_real_home_candidates(env: dict[str, str] | None = None) -> list[str]:
    ...
    env = env or {}
    candidates: list[str] = []
    explicit = str(env.get("LAILABA_REAL_HOME") or os.getenv("LAILABA_REAL_HOME", "")).strip()
    if explicit:
...
def get_real_home(env: dict[str, str] | None = None) -> str:
    """Return the OS user's real home directory, avoiding Lailaba profile HOME.

    ``LAILABA_HOME`` scopes Lailaba state. ``HOME`` is reserved for the OS/user
    account and the many external CLIs that store credentials under ``~``.
    If a parent process is already running with ``HOME={LAILABA_HOME}/home``,
...
def get_subprocess_home(env: dict[str, str] | None = None) -> str | None:
    ...
    * ``auto`` (default): host installs keep the real user HOME; containers use
      ``{LAILABA_HOME}/home`` for persistent state. If a host parent already has
      HOME pointed at the profile home, repair subprocesses back to real HOME.
    * ``real``: always prefer the real OS-user HOME.
    * ``profile``: use ``{LAILABA_HOME}/home`` when it exists, preserving the
...
def apply_subprocess_home_env(env: dict[str, str]) -> None:
    """Apply Lailaba' subprocess HOME contract to *env* in-place."""
    real_home = get_real_home(env)
    if real_home:
        env["LAILABA_REAL_HOME"] = real_home
...
def is_container() -> bool:
    ...
    See: NousResearch/lailaba-agent#47111
    """
...
def get_config_path() -> Path:
    """Return the path to ``config.yaml`` under LAILABA_HOME.

    Replaces the ``get_lailaba_home() / "config.yaml"`` pattern repeated
    in 7+ files (skill_utils.py, lailaba_logging.py, lailaba_time.py, etc.).
    """
    return get_lailaba_home() / "config.yaml"


def get_skills_dir() -> Path:
    """Return the path to the skills directory under LAILABA_HOME."""
    return get_lailaba_home() / "skills"



def get_env_path() -> Path:
    """Return the path to the ``.env`` file under LAILABA_HOME."""
...
def apply_ipv4_preference(force: bool = False) -> None:
    ...
    if getattr(socket.getaddrinfo, "_lailaba_ipv4_patched", False):
        return
    ...
    _ipv4_getaddrinfo._lailaba_ipv4_patched = True  # type: ignore[attr-defined]
