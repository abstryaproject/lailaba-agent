    ...
    mcp_servers:
      my_server:
        ...
        oauth:                                  # all fields optional
          ...
          client_name: "My Custom Client"       # default: "Lailaba AI"
"""
...
from lailaba_constants import secure_parent_dir
...
_USER_SKIPPED_SENTINEL = "__lailaba_user_skipped__"
...
def _get_token_dir() -> Path:
    ...
    Uses LAILABA_HOME so each profile gets its own OAuth tokens.
    Layout: ``LAILABA_HOME/mcp-tokens/``
    """
    try:
        from lailaba_constants import get_lailaba_home
        base = Path(get_lailaba_home())
    except ImportError:
        base = Path(os.environ.get("LAILABA_HOME", str(Path.home() / ".lailaba")))
...
class HermesTokenStorage:
    ...
    File layout::
        ...
        LAILABA_HOME/mcp-tokens/<server_name>.json         -- tokens
        LAILABA_HOME/mcp-tokens/<server_name>.client.json   -- client info
        LAILABA_HOME/mcp-tokens/<server_name>.meta.json     -- oauth server metadata
    """
    ...
    async def get_tokens(self) -> "OAuthToken | None":
        ...
        if data is None:
            return None
        # Lailaba records an absolute wall-clock ``expires_at`` alongside the
    ...
    class _Handler(BaseHTTPRequestHandler):
        def do_GET(self) -> None:  # noqa: N802
            ...
            body = (
                ...
                "<p>You can close this tab and return to Lailaba.</p></body></html>"
            ) if code else (
...
async def _redirect_handler(authorization_url: str) -> None:
    ...
    if _oauth_port and (os.getenv("SSH_CLIENT") or os.getenv("SSH_TTY")):
        print(
            ...
            f"\n"
            f"  See: https://lailaba-agent.nousresearch.com/docs/guides/oauth-over-ssh\n",
            file=sys.stderr,
        )
...
def _paste_callback_reader(result: dict) -> None:
    ...
    if line.lower() in _SKIP_TOKENS:
        ...
        print(
            "  OAuth skipped. Run `lailaba mcp login <server>` later to "
...
def _build_client_metadata(cfg: dict) -> "OAuthClientMetadata":
    ...
    client_name = cfg.get("client_name", "Lailaba AI")
    scope = cfg.get("scope")
...
def build_oauth_auth(
    ...
    if not _is_interactive() and not storage.has_cached_tokens():
        raise OAuthNonInteractiveError(
            ...
            f"`lailaba mcp login {server_name}` interactively first to complete "
