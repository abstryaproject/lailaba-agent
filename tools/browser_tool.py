...
from lailaba_constants import agent_browser_runnable, get_lailaba_home
from utils import env_int, is_truthy_value
from lailaba_cli.config import DEFAULT_CONFIG, cfg_get
...
# means a compromised transitive dependency could read every Lailaba secret
...
def _build_browser_env() -> dict:
    ...
    Strips Lailaba-managed secrets (provider keys, gateway tokens, GitHub auth,
    infra secrets) then re-adds only the browser-backend keys the worker needs.
    The ``lailaba_subprocess_env`` import is deferred to keep ``browser_tool``
    ...
    env = lailaba_subprocess_env(inherit_credentials=False)
...
def _browser_candidate_path_dirs() -> list[str]:
    ...
    lailaba_node_bin = str(lailaba_home / "node" / "bin")
    lailaba_node_root = str(lailaba_home / "node")
    lailaba_nm_bin = str(lailaba_home / "node_modules" / ".bin")
    return [lailaba_node_bin, lailaba_node_root, lailaba_nm_bin, *list(_discover_homebrew_node_dirs()), *_SANE_PATH_DIRS]
...
# plugins drop in under ``~/.lailaba/plugins/browser/<vendor>/``.
...
def _get_cloud_provider() -> Optional[CloudBrowserProvider]:
    ...
    browser plugins (``~/.lailaba/plugins/browser/<vendor>/``) participate
...
def _lightpanda_fallback_reason(engine: str, command: str, result: Dict[str, Any]) -> Optional[str]:
    ...
    the fallback result so CLI/TUI/gateway users can see when Lailaba silently
...
def _run_chrome_fallback_command(
    ...
    def _run_tmp(cmd: str, cmd_args: List[str]) -> Dict[str, Any]:
        ...
        try:
            ...
            #   in the Lailaba CLI where sys.stdout and sys.stderr both report
...
def _socket_safe_tmpdir() -> str:
    ...
    append ``agent-browser-lailaba_…`` the resulting socket path exceeds the
...
def _emergency_cleanup_all_sessions():
    ...
    crashed lailaba processes — this way every clean lailaba exit sweeps
    ...
    # Sweep orphans from other crashed lailaba processes.  Safe even if we
    # never used the browser — uses owner_pid liveness to avoid reaping
    # daemons owned by other live lailaba processes.
    try:
...
def _write_owner_pid(socket_dir: str, session_name: str) -> None:
    """Record the current lailaba PID as the owner of a browser socket dir.

    Written atomically to ``<socket_dir>/<session_name>.owner_pid`` so the
    orphan reaper can distinguish daemons owned by a live lailaba process
...
def _reap_orphaned_browser_sessions():
    ...
    daemons whose owning lailaba process is no longer alive.

    Ownership detection priority:
      1. ``<session>.owner_pid`` file (written by current code) — if the
         referenced lailaba PID is alive, leave the daemon alone regardless
         of whether it's in *this* process's ``_active_sessions``.  This is
         cross-process safe: two concurrent lailaba instances won't reap each
         other's daemons.
    ...
    socket_dirs += glob.glob(os.path.join(tmpdir, "agent-browser-lailaba_*"))
    ...
    for socket_dir in socket_dirs:
        ...
        if owner_alive is True:
            # Owner is alive — this session belongs to a live lailaba process.
            continue
...
def _find_agent_browser() -> str:
    ...
    Checks in order: current PATH, Homebrew/common bin dirs, Lailaba-managed
    ...
    # node_modules binary, which disappears on the next ``lailaba update`` and
    ...
    # Build an extended search PATH including Lailaba-managed Node, macOS
    ...
    try:
        ...
        if ensure_dependency("browser"):
            candidates = [
                ...
                shutil.which("agent-browser", path=str(get_lailaba_home() / "node_modules" / ".bin")),
                shutil.which("agent-browser", path=str(get_lailaba_home() / "node" / "bin")),
                shutil.which("agent-browser", path=str(get_lailaba_home() / "node")),
            ]
...
def _run_browser_command(
    ...
    try:
        ...
        os.makedirs(task_socket_dir, mode=0o700, exist_ok=True)
        # Record this lailaba PID as the session owner (cross-process safe
...
def _maybe_start_recording(task_id: str):
    ...
    try:
        ...
        recordings_dir = lailaba_home / "browser_recordings"
...
def browser_vision(question: str, annotate: bool = False, task_id: Optional[str] = None) -> Union[str, Dict[str, Any]]:
    ...
    conversation so the model can inspect it on the next turn; otherwise Lailaba
    ...
    from lailaba_constants import get_lailaba_dir
    screenshots_dir = get_lailaba_dir("cache/screenshots", "browser_screenshots")
        ...
        try:
            ...
            if _requires_real_termux_browser_install(browser_cmd):
                ...
                if _running_in_docker():
                    ...
                    print("       docker pull ghcr.io/nousresearch/lailaba-agent:latest")
                else:
