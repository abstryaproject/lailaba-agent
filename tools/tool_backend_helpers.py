...
def nous_tool_gateway_unavailable_message(
    ...
    *,
    ...
    try:
        ...
        pass
    return (
        f"{capability} is unavailable. Run `lailaba model` to refresh your "
...
def fal_key_is_configured() -> bool:
    ...
    Consults both ``os.environ`` and ``~/.lailaba/.env`` (via
    ``lailaba_cli.config.get_env_value`` when available) so tool-side
