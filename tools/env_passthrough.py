...
def _is_lailaba_provider_credential(name: str) -> bool:
    """True if ``name`` is a Lailaba-managed provider credential (API key,
    token, or similar) per ``_LAILABA_PROVIDER_ENV_BLOCKLIST``.
    ...
    Non-Lailaba API keys (TENOR_API_KEY, NOTION_TOKEN, etc.) are NOT
    ...
    let a skill tunnel a Lailaba credential into the execute_code child.
    """
    try:
        ...
        return True
    return name in _LAILABA_PROVIDER_ENV_BLOCKLIST
...
def register_env_passthrough(var_names: Iterable[str]) -> None:
    ...
    Variables that are Lailaba-managed provider credentials (from
    ``_LAILABA_PROVIDER_ENV_BLOCKLIST``) are rejected here to preserve
    the ``execute_code`` sandbox's credential-scrubbing guarantee per
    GHSA-rhgp-j443-p4rf. A skill that needs to talk to a Lailaba-managed
    ...
    Non-Lailaba third-party API keys (TENOR_API_KEY, NOTION_TOKEN, etc.)
    ...
    for name in var_names:
        ...
        if not name:
            continue
        if _is_lailaba_provider_credential(name):
            logger.warning(
                "env passthrough: refusing to register Lailaba provider "
                "credential %r (blocked by _LAILABA_PROVIDER_ENV_BLOCKLIST). "
    ...
    try:
        ...
        if isinstance(passthrough, list):
            for item in passthrough:
                ...
                # Lailaba-managed provider credentials must not be passed
                ...
                if _is_lailaba_provider_credential(name):
                    logger.warning(
                        "env passthrough: refusing to register Lailaba "
                        "provider credential %r from config.yaml (blocked "
                        "by _LAILABA_PROVIDER_ENV_BLOCKLIST). Operator "
