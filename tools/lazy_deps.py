"""
Lazy dependency installer for opt-in Lailaba AI backends.

Many Lailaba features (Mistral TTS, ElevenLabs TTS, Honcho memory, Bedrock,
...
(``lailaba-agent[all]``) and install them eagerly at setup time. That has
two problems:
...
remediation hint pointing at ``lailaba tools`` or the manual pip command.
...
* **Durable-target mode (immutable images).** When the deployment seals the
  agent's own venv (the Docker image sets ``LAILABA_DISABLE_LAZY_INSTALLS=1``
  and makes ``/opt/lailaba`` read-only), setting
  ``LAILABA_LAZY_INSTALL_TARGET`` redirects lazy installs to a writable
  ...
  installed package cannot brick Lailaba, which is what made it safe to seal
...
_LAZY_TARGET_ENV = "LAILABA_LAZY_INSTALL_TARGET"
...
def _allow_lazy_installs() -> bool:
    ...
    2. ``LAILABA_DISABLE_LAZY_INSTALLS=1`` seals the *agent venv* (set by the
    ...
    if os.environ.get("LAILABA_DISABLE_LAZY_INSTALLS") == "1":
...
def _is_satisfied(spec: str) -> bool:
    ...
    ``lailaba update`` propagate pin bumps in :data:`LAZY_DEPS` to already-
...
def _core_constraints_file() -> Optional[Path]:
    ...
    * A backend that *requires* a version conflicting with core fails loudly
    ...
    try:
        ...
        if not lines:
            return None
        fd, path = tempfile.mkstemp(prefix="lailaba-core-constraints-", suffix=".txt")
...
def _venv_pip_install(specs: tuple[str, ...], *, timeout: int = 300) -> _InstallResult:
    ...
    * **Durable-target.** When :data:`_LAZY_TARGET_ENV` is set, installs into
    ...
    Mirrors the strategy in ``lailaba_cli.tools_config._pip_install`` but
    ...
    try:
        ...
        uv_env = lailaba_subprocess_env(inherit_credentials=False)
...
def active_features() -> list[str]:
    ...
    Used by ``lailaba update`` to figure out which lazy backends need a
...
def refresh_active_features(*, prompt: bool = False) -> dict[str, str]:
    ...
    Intended for ``lailaba update``. Never raises; lazy-install failures
