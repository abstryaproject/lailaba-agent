...
Lets the LLM write a Python script that calls Lailaba tools via RPC,
collapsing multi-step tool chains into a single inference turn.

Architecture (two transports):

  **Local backend (UDS):**
  1. Parent generates a `lailaba_tools.py` stub module with UDS RPC functions
  ...
  **Remote backends (file-based RPC):**
  1. Parent generates `lailaba_tools.py` with file-based RPC stubs
...
# available on every platform Lailaba itself runs on.
...
# match a safe prefix, the operational LAILABA_ allowlist, or (on Windows) an
# OS-essential name.
#
# NB: the broad "LAILABA_" prefix was deliberately removed (#27303) — it leaked
# LAILABA_*-named config that lacks a secret substring (e.g. LAILABA_BASE_URL,
# LAILABA_KANBAN_DB, LAILABA_*_WEBHOOK).  The child only needs the few
# location/profile vars in _LAILABA_CHILD_ALLOWED below; LAILABA_RPC_SOCKET /
# LAILABA_RPC_DIR / TZ / HOME are injected explicitly after scrubbing.
...
# Operational LAILABA_* vars the child legitimately needs by exact name — these
# are non-secret runtime-location flags (the same set lailaba_cli treats as the
# runtime location) that repo-root modules a sandbox script imports may read at
# import time.  None match _SECRET_SUBSTRINGS.
_LAILABA_CHILD_ALLOWED = frozenset({
    "LAILABA_HOME",
    "LAILABA_PROFILE",
    "LAILABA_CONFIG",
    "LAILABA_ENV",
...
def _scrub_child_env(source_env, is_passthrough=None, is_windows=None):
    ...
    Rules (order matters):
      ...
      4. Operational LAILABA_* vars (_LAILABA_CHILD_ALLOWED) pass by exact name.
    ...
    scrubbed = {}
    # Non-secret LAILABA_* vars dropped by the tightened allowlist (#27303). The
    # broad "LAILABA_" prefix used to pass these through; now only the
    # operational set does. The drop is intentional (those vars can carry
    # config like LAILABA_KANBAN_DB / LAILABA_BASE_URL), but a sandbox script
    ...
    for k, v in source_env.items():
        ...
        if any(k.startswith(p) for p in _SAFE_ENV_PREFIXES):
            scrubbed[k] = v
            continue
        if k in _LAILABA_CHILD_ALLOWED:
            scrubbed[k] = v
            continue
        if is_windows and k.upper() in _WINDOWS_ESSENTIAL_ENV_VARS:
            scrubbed[k] = v
            continue
        if k.startswith("LAILABA_"):
            # Non-secret (secrets were already dropped above) and not in any
            # allowlist — a deliberately-dropped LAILABA_* var.
            _dropped_lailaba.append(k)
    if _dropped_lailaba:
        logger.debug(
            "execute_code: dropped %d non-allowlisted LAILABA_* var(s) from the "
            ...
            len(_dropped_lailaba),
            ", ".join(sorted(_dropped_lailaba)),
        )
    return scrubbed
...
# ---------------------------------------------------------------------------
# lailaba_tools.py code generator
# ---------------------------------------------------------------------------
...
_TOOL_STUBS = {
    ...
    "write_file": (
        ...
        '"""Write content to a file (always overwrites). Returns dict with status. cross_profile=True opts out of the cross-Lailaba-profile soft guard."""',
    ...
    "patch": (
        ...
        '"""Targeted find-and-replace (mode="replace") or V4A multi-file patches (mode="patch"). Returns dict with status. cross_profile=True opts out of the cross-Lailaba-profile soft guard."""',
...
def generate_lailaba_tools_module(enabled_tools: List[str],
                                 transport: str = "uds") -> str:
    """
    Build the source code for the lailaba_tools.py stub module.
...
"""Auto-generated Lailaba tools RPC stubs."""
...
def _connect():
    ...
    LAILABA_RPC_SOCKET can be either:
    ...
    global _sock
    if _sock is None:
        endpoint = os.environ["LAILABA_RPC_SOCKET"]
...
"""Auto-generated Lailaba tools RPC stubs (file-based transport)."""
import json, os, shlex, tempfile, threading, time

_RPC_DIR = os.environ.get("LAILABA_RPC_DIR") or os.path.join(tempfile.gettempdir(), "lailaba_rpc")
_seq = 0
...
def _execute_remote(
    ...
    The script and the generated lailaba_tools.py module are shipped to
    ...
    sandbox_dir = f"{temp_dir}/lailaba_exec_{sandbox_id}"
    ...
    try:
        ...
        tools_src = generate_lailaba_tools_module(
            list(sandbox_tools), transport="file",
        )
        _ship_file_to_remote(env, f"{sandbox_dir}/lailaba_tools.py", tools_src)
        ...
        env_prefix = (
            f"LAILABA_RPC_DIR={shlex.quote(f'{sandbox_dir}/rpc')} "
            f"PYTHONDONTWRITEBYTECODE=1"
        )
        tz = os.getenv("LAILABA_TIMEZONE", "").strip()
        if tz:
...
def execute_code(
    ...
    # --- Set up temp directory with lailaba_tools.py and script.py ---
    tmpdir = tempfile.mkdtemp(prefix="lailaba_sandbox_")
    ...
    # selector from LAILABA_RPC_SOCKET (path vs. ``tcp://host:port``).
    ...
    if _use_tcp_rpc:
        ...
        sock_path = os.path.join(_sock_tmpdir, f"lailaba_rpc_{uuid.uuid4().hex}.sock")
    ...
    try:
        # Write the auto-generated lailaba_tools module.
        ...
        tools_src = generate_lailaba_tools_module(list(sandbox_tools))
        with open(os.path.join(tmpdir, "lailaba_tools.py"), "w", encoding="utf-8") as f:
            f.write(tools_src)
        ...
        #   connect.  LAILABA_RPC_SOCKET is set to ``tcp://127.0.0.1:<port>``
        ...
        child_env["LAILABA_RPC_SOCKET"] = rpc_endpoint
        ...
        # Ensure the lailaba-agent root is importable in the sandbox so
        # repo-root modules are available to child scripts.  We also prepend
        # the staging tmpdir so ``from lailaba_tools import ...`` resolves even
        # when the subprocess CWD is not tmpdir (project mode).
        _lailaba_root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
        _existing_pp = child_env.get("PYTHONPATH", "")
        _pp_parts = [tmpdir, _lailaba_root]
        if _existing_pp:
            _pp_parts.append(_existing_pp)
        child_env["PYTHONPATH"] = os.pathsep.join(_pp_parts)
        # Inject user's configured timezone so datetime.now() in sandboxed
        # code reflects the correct wall-clock time.  Only TZ is set —
        # LAILABA_TIMEZONE is an internal Lailaba setting and must not leak
        # into child processes.
        _tz_name = os.getenv("LAILABA_TIMEZONE", "").strip()
        if _tz_name:
            child_env["TZ"] = _tz_name
        child_env.pop("LAILABA_TIMEZONE", None)
        ...
        # but scripts can still read secrets from disk (e.g. open('~/.lailaba/.env')).
...
def _load_config() -> dict:
    ...
    ``lailaba --tui`` where it is never used.  Read the lightweight raw config
...
def _get_execution_mode() -> str:
    ...
    Mode semantics:
      ...
      - ``strict``: scripts run in an isolated temp directory with
        ``sys.executable`` (lailaba-agent's python). Reproducible and the
...
def build_execute_code_schema(enabled_sandbox_tools: set = None,
    ...
    When tools are disabled via ``lailaba tools`` (e.g. web is turned off),
    ...
    # temp-dir staging and lailaba-agent's own python.
    if mode == "strict":
        cwd_note = (
            "Scripts run in their own temp dir, not the session's CWD — use absolute paths "
            "(os.path.expanduser('~/.lailaba/.env')) or terminal()/read_file() for user files."
        )
    else:
        cwd_note = (
    ...
    description = (
        "Run a Python script that can call Lailaba tools programmatically. "
        ...
        f"Available via `from lailaba_tools import ...`:\n\n"
        f"{tool_lines}\n\n"
        ...
        "Also available (no import needed — built into lailaba_tools):\n"
            ...
            "properties": {
                "code": {
                    ...
                    "description": (
                        ...
                        f"`from lailaba_tools import {import_str}` "
