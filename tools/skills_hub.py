#!/usr/bin/env python3
"""
Skills Hub — Source adapters and hub state management for the Lailaba Skills Hub.
...
Used by lailaba_cli/skills_hub.py for CLI commands and the /skills slash command.
"""
...
class GitHubSource(SkillSource):
    ...
    def inspect(self, identifier: str) -> Optional[SkillMeta]:
        ...
        if isinstance(metadata, dict):
            lailaba_meta = metadata.get("lailaba", {})
            if isinstance(lailaba_meta, dict):
                tags = lailaba_meta.get("tags", [])
        if not tags:
...
class UrlSource(SkillSource):
    ...
    def inspect(self, identifier: str) -> Optional[SkillMeta]:
        ...
        if isinstance(metadata, dict):
            lailaba_meta = metadata.get("lailaba", {})
            if isinstance(lailaba_meta, dict):
                raw_tags = lailaba_meta.get("tags", [])
...
class OptionalSkillSource(SkillSource):
    ...
    ~/.lailaba/skills/ during setup.  They are discoverable via the Skills Hub
    ...
    def _scan_all(self) -> List[SkillMeta]:
        ...
        for skill_md in sorted(self._optional_dir.rglob("SKILL.md")):
            ...
            if isinstance(meta_block, dict):
                lailaba_meta = meta_block.get("lailaba", {})
                if isinstance(lailaba_meta, dict):
                    tags = lailaba_meta.get("tags", [])
...
# ---------------------------------------------------------------------------
# Lailaba centralized index source
# ---------------------------------------------------------------------------

LAILABA_INDEX_URL = "https://lailaba-agent.nousresearch.com/docs/api/skills-index.json"
LAILABA_INDEX_CACHE_FILE = INDEX_CACHE_DIR / "lailaba-index.json"
LAILABA_INDEX_TTL = 6 * 3600  # 6 hours


def _load_lailaba_index() -> Optional[dict]:
    """Fetch the centralized skills index, with local cache.

    The index is a JSON file hosted on the docs site, rebuilt daily by CI.
    We cache it locally for LAILABA_INDEX_TTL seconds to avoid repeated
    downloads within a session.
    """
    # Check local cache
    if LAILABA_INDEX_CACHE_FILE.exists():
        try:
            age = time.time() - LAILABA_INDEX_CACHE_FILE.stat().st_mtime
            if age < LAILABA_INDEX_TTL:
                return json.loads(LAILABA_INDEX_CACHE_FILE.read_text())
        except (OSError, json.JSONDecodeError):
            pass

    # Fetch from docs site
    try:
        resp = httpx.get(LAILABA_INDEX_URL, timeout=15, follow_redirects=True)
        if resp.status_code != 200:
            logger.debug("Lailaba index fetch returned %d", resp.status_code)
            return _load_stale_index_cache()
        data = resp.json()
    except (httpx.HTTPError, json.JSONDecodeError) as e:
        logger.debug("Lailaba index fetch failed: %s", e)
        return _load_stale_index_cache()
    ...
    # Cache locally
    try:
        LAILABA_INDEX_CACHE_FILE.parent.mkdir(parents=True, exist_ok=True)
        LAILABA_INDEX_CACHE_FILE.write_text(json.dumps(data))
    except OSError:
        pass
...
class HermesIndexSource(SkillSource):
    """Skill source backed by the centralized Lailaba Skills Index.
    ...
    def _ensure_loaded(self) -> dict:
        if not self._loaded:
            self._index = _load_lailaba_index()
            self._loaded = True
    ...
    def fetch(self, identifier: str) -> Optional[SkillBundle]:
        ...
        if resolved:
            ...
            if bundle:
                bundle.source = entry.get("source", "lailaba-index")
    ...
    def _to_meta(entry: dict) -> SkillMeta:
        return SkillMeta(
            name=entry.get("name", ""),
            description=entry.get("description", ""),
            source=entry.get("source", "lailaba-index"),
            identifier=entry.get("identifier", ""),
...
def parallel_search_sources(
    ...
    *on_source_done* is an optional callback ``(source_id, count) -> None``
    ...
    if _effective_filter == "all":
        for src in sources:
            if (src.source_id() == "lailaba-index"
