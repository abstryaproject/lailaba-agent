...
def has_xai_credentials() -> bool:
    ...
    hot-paint paths (``lailaba tools`` repaint, tool-registration scans,
    ...
    2. ``~/.lailaba/auth.json`` has a non-empty ``providers.xai-oauth.tokens.access_token``
    ...
    try:
        ...
        auth_path = get_lailaba_home() / "auth.json"
...
def get_env_value(name: str, default=None):
    """Read ``name`` from ``~/.lailaba/.env`` first, then ``os.environ``.

    Wraps :func:`lailaba_cli.config.get_env_value` so tests can patch
    ``tools.xai_http.get_env_value`` to inject dotenv-only secrets into the
    xAI credential resolver.
    """
    try:
        from lailaba_cli.config import get_env_value as _lailaba_get_env_value

        value = _lailaba_get_env_value(name)
...
def lailaba_xai_user_agent() -> str:
    """Return a stable Lailaba-specific User-Agent for xAI HTTP calls."""
    try:
        from lailaba_cli import __version__
    except Exception:
        __version__ = "unknown"
    return f"Lailaba-Agent/{__version__}"


def resolve_xai_http_credentials(*, force_refresh: bool = False) -> Dict[str, str]:
    """Resolve bearer credentials for direct xAI HTTP endpoints.

    Prefers Lailaba-managed xAI OAuth credentials when available, then falls back
    to ``XAI_API_KEY`` resolved via ``lailaba_cli.config.get_env_value`` so keys
    stored in ``~/.lailaba/.env`` (the standard Lailaba location) are honored —
