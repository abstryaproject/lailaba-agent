...
_YOLO_MODE_FROZEN: bool = is_truthy_value(os.getenv("LAILABA_YOLO_MODE", ""))
...
def get_current_session_key(default: str = "default") -> str:
    ...
    return get_session_env("LAILABA_SESSION_KEY", default)
...
def _get_session_platform() -> str:
    ...
    try:
        ...
        return get_session_env("LAILABA_SESSION_PLATFORM", "") or ""
    except Exception:
        return os.getenv("LAILABA_SESSION_PLATFORM", "") or ""


def _is_gateway_approval_context() -> bool:
    """True when this call is inside a gateway/API session.

    Legacy gateway integrations set LAILABA_GATEWAY_SESSION in process env.
    Newer concurrent gateway paths bind LAILABA_SESSION_PLATFORM via
    ...
    from a gateway platform (cron binds LAILABA_SESSION_PLATFORM via
    ...
    """
    if env_var_enabled("LAILABA_CRON_SESSION"):
        return False
    if env_var_enabled("LAILABA_GATEWAY_SESSION"):
        return True
    return bool(_get_session_platform())

# Sensitive write targets that should trigger approval even when referenced
# via shell expansions like $HOME or $LAILABA_HOME, or by the resolved absolute
# active profile home path such as /home/lailaba/.lailaba/config.yaml. The
# resolved-absolute form is folded into the ~/.lailaba/ patterns at detection
...
# go stale when LAILABA_HOME is set after this module is imported, e.g. under the
# hermetic test conftest or any deferred-profile-resolution path).
_SSH_SENSITIVE_PATH = r'(?:~|\$home|\$\{home\})/\.ssh(?:/|$)'
_LAILABA_ENV_PATH = (
    r'(?:~\/\.lailaba/|'
    r'(?:\$home|\$\{home\})/\.lailaba/|'
    r'(?:\$lailaba_home|\$\{lailaba_home\})/)'
    r'\.env\b'
)
# ~/.lailaba/config.yaml IS the security policy: approvals.mode, yolo, and the
...
# theater. Mirrors _LAILABA_ENV_PATH; matches the LAILABA_HOME override form as
# well as ~/.lailaba/.
_LAILABA_CONFIG_PATH = (
    r'(?:~\/\.lailaba/|'
    r'(?:\$home|\$\{home\})/\.lailaba/|'
    r'(?:\$lailaba_home|\$\{lailaba_home\})/)'
    r'config\.yaml\b'
...
_SENSITIVE_WRITE_TARGET = (
    ...
    rf'{_LAILABA_ENV_PATH}|'
    rf'{_LAILABA_CONFIG_PATH}|'
    rf'{_SHELL_RC_FILES}|'
...
DANGEROUS_PATTERNS = [
    ...
    (r'\bhermes\s+gateway\s+(stop|restart)\b', "stop/restart lailaba gateway (kills running agents)"),
    (r'\bhermes\s+update\b', "lailaba update (restarts gateway, kills running agents)"),
    # Docker container lifecycle — any user with docker.sock mounted (a common
    # Docker Compose pattern) gives the agent the ability to restart/stop/kill
    # containers without approval.  These are agent-initiated lifecycle operations
    # that should always require user consent, just like `lailaba gateway restart`
    ...
    (r'gateway\s+run\b.*(&\s*$|&\s*;|\bdisown\b|\bsetsid\b)', "start gateway outside systemd (use 'systemctl --user restart lailaba-gateway')"),
    (r'\bnohup\b.*gateway\s+run\b', "start gateway outside systemd (use 'systemctl --user restart lailaba-gateway')"),
    # Self-termination protection: prevent agent from killing its own process
    (r'\b(pkill|killall)\b.*\b(lailaba|gateway|cli\.py)\b', "kill lailaba/gateway process (self-termination)"),
    # Self-termination via kill + command substitution (pgrep/pidof).
    # The name-based pattern above catches `pkill lailaba` but not
    # `kill -9 $(pgrep -f lailaba)` because the substitution is opaque
    ...
    # the `lailaba gateway stop|restart` pattern above by driving launchd
    # directly against the service label (commonly `ai.lailaba.gateway`).
    # Catch the operations that stop, restart, or unload it.
    (r'\blaunchctl\s+(stop|kickstart|bootout|unload|kill|disable|remove)\b.*\b(lailaba|ai\.lailaba)\b', "stop/restart lailaba launchd service (kills running agents)"),
    # File copy/move/edit into sensitive system paths (/etc/ and macOS
    # /private/etc/ mirror).
    (rf'\b(cp|mv|install)\b.*\s{_SYSTEM_CONFIG_PATH}', "copy/move file into system config path"),
    (rf'\b(cp|mv|install)\b.*\s["\']?{_PROJECT_SENSITIVE_WRITE_TARGET}["\']?{_COMMAND_TAIL}', "overwrite project env/config file"),
    # cp/mv/install OVERWRITING a sensitive credential/SSH/shell-rc/Lailaba file.
    # The tee/redirection patterns above already gate _SENSITIVE_WRITE_TARGET
    # (~/.ssh/*, ~/.netrc/.pgpass/.npmrc/.pypirc, shell rc files,
    # ~/.lailaba/config.yaml/.env), but cp/mv/install was only paired for /etc and
    ...
    # In-place edit of a Lailaba-managed security file (~/.lailaba/config.yaml or
    ...
    (rf'\bsed\s+-[^\s]*i.*(?:{_LAILABA_CONFIG_PATH}|{_LAILABA_ENV_PATH})', "in-place edit of Lailaba config/env"),
    (rf'\bsed\s+--in-place\b.*(?:{_LAILABA_CONFIG_PATH}|{_LAILABA_ENV_PATH})', "in-place edit of Lailaba config/env (long flag)"),
    ...
    (rf'\b(?:perl|ruby)\b.*(?:^|\s)-[^\s]*i\b.*(?:{_LAILABA_CONFIG_PATH}|{_LAILABA_ENV_PATH})', "in-place edit of Lailaba config/env (perl/ruby)"),
...
def _normalize_command_for_detection(command: str) -> str:
    ...
    # ~/ and ~/.lailaba/ forms so static user-sensitive patterns catch
    ...
    # it tracks HOME / LAILABA_HOME even when those are set after this module is
    ...
    #
    # Fold the (more specific) Lailaba home first: on Windows it nests under the
    # user home (C:\Users\alice\AppData\...\lailaba), so folding the user home
    # first would eat the prefix the Lailaba-home fold needs.
    command = _rewrite_resolved_lailaba_home(command)
...
def _home_prefix_fold_regex(path: str):
    ...
    components below the root — so a stray HOME / LAILABA_HOME such as ``/``,
...
def _fold_home_prefixes(command: str, paths, replacement: str) -> str:
    ...
    *replacement* has no trailing separator (``~`` / ``~/.lailaba``); the matched
...
def _rewrite_resolved_lailaba_home(command: str) -> str:
    """Rewrite the resolved absolute Lailaba home prefix to ``~/.lailaba/``.

    Resolves the active ``LAILABA_HOME`` at call time (and its symlink-resolved
    form) and folds an occurrence of ``<home>/`` in *command* into
    ``~/.lailaba/`` so the static ``_LAILABA_CONFIG_PATH`` / ``_LAILABA_ENV_PATH``
    patterns match. In Docker and gateway deployments the agent often references
    the resolved absolute path directly (e.g. ``sed -i ...
    /home/lailaba/.lailaba/config.yaml``) rather than ``~``, ``$HOME``, or
    ``$LAILABA_HOME``. Matches both POSIX and Windows separators. No-op when the
    path can't be resolved or doesn't appear.
    """
    try:
        from lailaba_constants import get_lailaba_home
        home = get_lailaba_home().expanduser()
        candidates = [
            str(home),
        ...
        return command
    return _fold_home_prefixes(command, candidates, "~/.lailaba")
...
def prompt_dangerous_approval(command: str, description: str,
    ...
    os.environ["LAILABA_SPINNER_PAUSE"] = "1"
    try:
        ...
        if "LAILABA_SPINNER_PAUSE" in os.environ:
            del os.environ["LAILABA_SPINNER_PAUSE"]
        print()
        sys.stdout.flush()
...
def check_dangerous_command(command: str, env_type: str,
    ...
    is_cli = env_var_enabled("LAILABA_INTERACTIVE")
    ...
    if not is_cli and not is_gateway:
        ...
        logger.warning(
            ...
            "(pattern: %s): %s — set LAILABA_INTERACTIVE or LAILABA_GATEWAY_SESSION to require approval.",
            description, command[:200],
        )
        return {"approved": True, "message": None}

    if is_gateway or env_var_enabled("LAILABA_EXEC_ASK"):
...
def check_all_command_guards(command: str, env_type: str,
    ...
    is_cli = env_var_enabled("LAILABA_INTERACTIVE")
    is_gateway = _is_gateway_approval_context()
    is_ask = env_var_enabled("LAILABA_EXEC_ASK")
