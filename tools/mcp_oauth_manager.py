...
`mcp_tool.py`, and `lailaba_cli/mcp_config.py`. This module is the ONLY place
...
def _make_lailaba_provider_class() -> Optional[type]:
    ...
    class HermesMCPOAuthProvider(OAuthClientProvider):
        ...
        def __init__(
            ...
            **kwargs: Any,
            ...
            self._lailaba_server_name = server_name
            ...
            self._lailaba_preregistered = preregistered
                ...
                for url in build_protected_resource_metadata_discovery_urls(
                    ...
                    try:
                        ...
                        logger.debug(
                            ...
                            self._lailaba_server_name, url, exc,
                        )
                        continue
                ...
                for url in build_oauth_authorization_server_metadata_discovery_urls(
                    ...
                    if asm:
                        ...
                        logger.debug(
                            ...
                            "token_endpoint=%s",
                            self._lailaba_server_name, asm.token_endpoint,
                        )
                        break
        ...
        async def _maybe_flag_poisoned_client(self, response: Any) -> None:
            ...
            and is handled by ``lailaba mcp reauth``.
            ...
            back to ``lailaba mcp reauth``.
            """
            try:
                if self._lailaba_preregistered:
                    return
...
_LAILABA_PROVIDER_CLS: Optional[type] = _make_lailaba_provider_class()
...
class MCPOAuthManager:
    ...
    def _build_provider(
        ...
        Returns None if the MCP SDK's OAuth support is unavailable.
        """
        if _LAILABA_PROVIDER_CLS is None:
            logger.warning(
        ...
        if not _is_interactive() and not storage.has_cached_tokens():
            raise OAuthNonInteractiveError(
                ...
                "cached tokens found. Run `lailaba mcp login "
    ...
    def remove(self, server_name: str) -> None:
        ...
        Called by ``lailaba mcp remove <name>`` and (indirectly) by
        ``lailaba mcp login <name>`` during forced re-auth.
        """
        with self._entries_lock:
