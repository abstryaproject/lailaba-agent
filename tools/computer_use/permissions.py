...
something different on each:
  ...
  * macOS — explicit TCC grants (Accessibility + Screen Recording). cua-driver
    ...
    the installed ``CuaDriver.app``), NOT Lailaba — so no Lailaba entitlement is
...
``lailaba computer-use permissions`` CLI, and ``/api/tools/computer-use/status``.
"""
...
def _child_env() -> Dict[str, str]:
    """cua-driver child env honoring the Lailaba telemetry opt-in policy."""
    try:
