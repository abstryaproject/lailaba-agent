...
Copies bundled skills from the repo's skills/ directory into ~/.lailaba/skills/
...
The manifest lives at ~/.lailaba/skills/.bundled_manifest.
"""
...
from lailaba_constants import get_bundled_skills_dir, get_lailaba_home, get_optional_skills_dir
...
# Marker file written by `lailaba profile create --no-skills` (named profiles)
# and by the installer's `--no-skills` flag (the default ~/.lailaba profile).
# When present in LAILABA_HOME, sync_skills() is a no-op so neither the
# installer, `lailaba update`, nor a direct sync re-injects bundled skills.
# Delete the file to opt back in. Mirrors
# lailaba_cli.profiles.NO_BUNDLED_SKILLS_MARKER (kept as a literal here to
...
def _get_bundled_dir() -> Path:
    ...
    Checks LAILABA_BUNDLED_SKILLS env var first (set by Nix wrapper),
...
def _read_suppressed_names() -> set:
    ...
    to reading ``~/.lailaba/skills/.curator_suppressed`` directly if that import
...
def _compute_relative_dest(skill_dir: Path, bundled_dir: Path) -> Path:
    ...
    e.g., bundled/skills/mlops/axolotl -> ~/.lailaba/skills/mlops/axolotl
    """
...
def sync_skills(quiet: bool = False) -> dict:
    """
    Sync bundled skills into ~/.lailaba/skills/ using the manifest.

    Returns:
        dict with keys: copied (list), updated (list), skipped (int),
                        user_modified (list), cleaned (list), total_bundled (int)
    """
    # Opt-out: a profile (named or the default ~/.lailaba) that wrote the
    ...
    if (LAILABA_HOME / NO_BUNDLED_SKILLS_MARKER).exists():
        if not quiet:
    ...
    for skill_name, skill_src in bundled_skills:
        ...
        # (~/.lailaba/skills/.curator_suppressed) is written when the curator
        # archives a bundled skill with curator.prune_builtins enabled. Without
        # this skip, every `lailaba update` would resurrect a skill the user
                ...
                if dest.exists():
                    ...
                    if _dir_hash(dest) == bundled_hash:
                        ...
                        print(
                            ...
                            f"was kept. Run `lailaba skills reset {skill_name}` "
...
def _rmtree_writable(path: Path) -> None:
    ...
    # ``LAILABA_HOME/skills/`` to prevent the catastrophic wipe of
    # ``~/.lailaba/`` (``.env``, ``MEMORY.md``, ``kanban.db``, custom
    ...
    # ``LAILABA_HOME``. Require a strict-child relationship so both escape
...
def reset_bundled_skill(name: str, restore: bool = False) -> dict:
    ...
    if not in_manifest and not is_bundled:
        return {
            ...
            "message": (
                ...
                f"(Hub-installed skills use `lailaba skills uninstall`.)"
            ),
            "synced": None,
        }
    ...
    if restore and deleted_user_copy:
        ...
        message = (
            f"Cleared manifest entry for '{name}'. Future `lailaba update` runs "
...
def _is_tracked_user_modification(origin_hash: str, user_hash: str) -> bool:
    """Whether an on-disk skill counts as a user modification ``lailaba update`` keeps.
...
def list_user_modified_bundled_skills() -> List[dict]:
    """Return the bundled skills that ``lailaba update`` keeps because the user
    edited them locally.
...
def diff_bundled_skill(name: str) -> dict:
    ...
    edits or ``lailaba skills reset`` back to upstream.
    ...
    if bundled_src is None:
        return {
            ...
            "message": (
                ...
                f"diff against). Hub-installed skills use `lailaba skills inspect`."
            ),
        }
...
def set_bundled_skills_opt_out(enabled: bool) -> dict:
    ...
    When ``enabled`` is True, writes LAILABA_HOME/.no-bundled-skills so the
    installer, ``lailaba update``, and any direct sync stop seeding bundled
    skills. When False, removes the marker so seeding resumes on the next
    sync. This is the on-disk-state half of ``lailaba skills opt-out`` /
    ...
    Returns:
        dict with keys: ok (bool), changed (bool), marker (str path),
                        message (str).
    """
    marker = LAILABA_HOME / NO_BUNDLED_SKILLS_MARKER
    ...
    try:
        if enabled:
            ...
            changed = existed
            message = (
                "Opted back in. The next `lailaba update` (or `lailaba skills "
...
def is_bundled_skills_opt_out() -> bool:
    ...
    return (LAILABA_HOME / NO_BUNDLED_SKILLS_MARKER).exists()
...
if __name__ == "__main__":
    print("Syncing bundled skills into ~/.lailaba/skills/ ...")
