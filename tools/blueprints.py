...
`lailaba skills publish` for sharing. No new source type, no new store, no new
...
@dataclass
class BlueprintSpec:
    """Parsed ``metadata.lailaba.blueprint`` automation spec for a skill."""
...
def parse_blueprint(skill_md_text: str) -> Optional[BlueprintSpec]:
    ...
    A skill is a blueprint iff ``metadata.lailaba.blueprint`` is a mapping containing
    ...
    lailaba = meta.get("lailaba") if isinstance(meta, dict) else None
    blueprint = lailaba.get("blueprint") if isinstance(lailaba, dict) else None
    if blueprint is None:
        return None
    if not isinstance(blueprint, dict):
        raise BlueprintError("metadata.lailaba.blueprint must be a mapping")
...
def export_blueprint(job: Dict[str, Any], body: str, *, blueprint_name: Optional[str] = None) -> str:
    ...
    and emit a SKILL.md (with a ``metadata.lailaba.blueprint`` block) they can hand
    to ``lailaba skills publish`` to share. ``body`` is the plain-language
