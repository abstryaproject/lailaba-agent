...
# Lailaba-internal env vars that should NOT leak into terminal subprocesses.
_LAILABA_PROVIDER_ENV_FORCE_PREFIX = "_LAILABA_FORCE_"

# Lailaba-managed AWS *inference* credentials for ``auth_type="aws_sdk"``
# providers (Bedrock).  Scoped DELIBERATELY NARROW: this lists only the
# Bedrock-specific bearer token, which is a Lailaba inference secret exactly
...
def _build_provider_env_blocklist() -> frozenset:
    ...
    try:
        from lailaba_cli.config import OPTIONAL_ENV_VARS
    ...
    blocked.update({
        ...
        "LAILABA_DASHBOARD_SESSION_TOKEN",
...
_LAILABA_PROVIDER_ENV_BLOCKLIST = _build_provider_env_blocklist()
...
# project's dependencies into the Lailaba venv path instead of the project's own
# ``.venv`` — silently clobbering the Lailaba environment (e.g. a project pinned
# to a different Python version overwrites it and breaks the gateway). The
# Lailaba venv stays reachable via PATH (its bin dir is first), so stripping
# these markers is safe and only prevents the cross-project clobber (#23473).
_ACTIVE_VENV_MARKER_VARS = ("VIRTUAL_ENV", "CONDA_PREFIX")


def _inject_context_lailaba_home(env: dict) -> None:
    """Bridge the context-local Lailaba home override into subprocess env."""
    try:
        from lailaba_constants import get_lailaba_home_override

        value = get_lailaba_home_override()
        if value:
            env["LAILABA_HOME"] = value
    except Exception:
        pass


def _sanitize_subprocess_env(base_env: dict | None, extra_env: dict | None = None) -> dict:
    """Filter Lailaba-managed secrets from a subprocess environment."""
    try:
    ...
    for key, value in (base_env or {}).items():
        if key.startswith(_LAILABA_PROVIDER_ENV_FORCE_PREFIX):
            continue
        if key not in _LAILABA_PROVIDER_ENV_BLOCKLIST or _is_passthrough(key):
            sanitized[key] = value

    for key, value in (extra_env or {}).items():
        if key.startswith(_LAILABA_PROVIDER_ENV_FORCE_PREFIX):
            real_key = key[len(_LAILABA_PROVIDER_ENV_FORCE_PREFIX):]
            sanitized[real_key] = value
        elif key not in _LAILABA_PROVIDER_ENV_BLOCKLIST or _is_passthrough(key):
            sanitized[key] = value

    _inject_context_lailaba_home(sanitized)

    from lailaba_constants import apply_subprocess_home_env
...
# legitimate child Lailaba spawns needs them, and they are the highest-value
...
# narrow subset of _LAILABA_PROVIDER_ENV_BLOCKLIST; provider keys are handled by
# the conditional Tier-2 strip in lailaba_subprocess_env().
...
def lailaba_subprocess_env(*, inherit_credentials: bool = False) -> dict[str, str]:
    ...
    single source of truth (``_LAILABA_PROVIDER_ENV_BLOCKLIST``).  The terminal
    ...
    * **Tier 1 (always):** ``_ALWAYS_STRIP_KEYS`` — gateway bot tokens, GitHub
      ...
      ``inherit_credentials``.  No child Lailaba spawns legitimately needs them.
    * **Tier 2 (conditional):** the rest of ``_LAILABA_PROVIDER_ENV_BLOCKLIST``
    ...
    if not inherit_credentials:
        ...
        for key in _LAILABA_PROVIDER_ENV_BLOCKLIST:
            env.pop(key, None)

    # Windows UTF-8 safety for spawned processes (#31420).
    env.setdefault("PYTHONUTF8", "1")

    _inject_context_lailaba_home(env)
    from lailaba_constants import apply_subprocess_home_env
...
def _find_bash() -> str:
    ...
    custom = os.environ.get("LAILABA_GIT_BASH_PATH")
    ...
    #   PortableGit: %LOCALAPPDATA%\lailaba\git\bin\bash.exe   (primary)
    #   MinGit:      %LOCALAPPDATA%\lailaba\git\usr\bin\bash.exe (legacy/32-bit fallback)
    _local_appdata = os.environ.get("LOCALAPPDATA", "")
    _lailaba_portable_git = os.path.join(_local_appdata, "lailaba", "git") if _local_appdata else ""
    if _lailaba_portable_git:
        for candidate in (
            os.path.join(_lailaba_portable_git, "bin", "bash.exe"),        # PortableGit (primary)
            os.path.join(_lailaba_portable_git, "usr", "bin", "bash.exe"), # MinGit fallback
        ):
    ...
    raise RuntimeError(
        "Git Bash not found. Lailaba AI requires Git for Windows on Windows.\n"
        "Install it from: https://git-scm.com/download/win\n"
        "Or set LAILABA_GIT_BASH_PATH to your bash.exe location."
    )
...
# Cached directory containing the ``lailaba`` console-script.
# ``_SENTINEL`` distinguishes "not resolved yet" from a resolved ``None``.
_SENTINEL = object()
_LAILABA_BIN_DIR: "str | None | object" = _SENTINEL


def _resolve_lailaba_bin_dir() -> str | None:
    """Return the directory holding the ``lailaba`` console-script, or None.
    ...
    lailaba install dir (``~/.local/bin``, the venv ``bin``/``Scripts``, pipx,
    nix) is absent from that PATH, so plugins shelling out to bare ``lailaba``
    via the terminal tool hit ``command not found`` (exit 127) even though
    ``lailaba`` works fine in the user's own interactive terminal.

    We resolve the install dir once (it never changes within a process) and
    prepend-if-missing it to the subshell PATH so bare ``lailaba`` resolves
    regardless of how the gateway was started.

    Resolution order (cheap, no heavy imports):
      1. ``shutil.which("lailaba")`` — normal PATH-installed shim.
      2. The directory of ``sys.argv[0]`` when it's an absolute path to a
         real ``lailaba`` executable (covers nix-store / venv wrappers).
      3. The directory of ``sys.executable`` — the running interpreter's
         venv ``bin``/``Scripts`` is where its console-scripts live.
    """
    global _LAILABA_BIN_DIR
    if _LAILABA_BIN_DIR is not _SENTINEL:
        return _LAILABA_BIN_DIR  # type: ignore[return-value]

    candidate: str | None = None

    which = shutil.which("lailaba")
    if which:
    ...
    if candidate is None:
        ...
        if (
            os.path.isabs(argv0)
            and (base == "lailaba" or base.startswith("lailaba."))
    ...
    if candidate is None:
        ...
        if exe_dir:
            shim = "lailaba.exe" if _IS_WINDOWS else "lailaba"
    ...
    _LAILABA_BIN_DIR = candidate
    return candidate


def _prepend_lailaba_bin_dir(existing_path: str) -> str:
    """Prepend the lailaba install dir to ``existing_path`` if it's missing.
    ...
    """
    bin_dir = _resolve_lailaba_bin_dir()
    if not bin_dir:
...
def _make_run_env(env: dict) -> dict:
    ...
    for k, v in merged.items():
        if k.startswith(_LAILABA_PROVIDER_ENV_FORCE_PREFIX):
            real_key = k[len(_LAILABA_PROVIDER_ENV_FORCE_PREFIX):]
            run_env[real_key] = v
        elif k not in _LAILABA_PROVIDER_ENV_BLOCKLIST or _is_passthrough(k):
            run_env[k] = v
    path_key = _path_env_key(run_env)
    if path_key is not None:
        new_path = _append_missing_sane_path_entries(run_env.get(path_key, ""))
        # Ensure the lailaba install dir is reachable so plugins can shell out
        # to bare ``lailaba`` via the terminal tool even when the gateway was
        # launched without it on PATH (systemd, service managers, cron, etc.).
        run_env[path_key] = _prepend_lailaba_bin_dir(new_path)

    _inject_context_lailaba_home(run_env)
...
def _resolve_shell_init_files() -> list[str]:
    ...
    an explicit list — once they have, Lailaba trusts them.
    """
...
class LocalEnvironment(BaseEnvironment):
    ...
    def get_temp_dir(self) -> str:
        ...
        **Windows:** hardcoded ``/tmp`` is wrong in two ways — native Python
        ...
        ``LAILABA_HOME`` instead — single-word path, guaranteed to exist, same
        string resolves in both Git Bash and native Python.
        """
        if _IS_WINDOWS:
            # Derive a Windows-safe temp dir under LAILABA_HOME.  Using
            ...
            try:
                ...
                cache_dir = get_lailaba_home() / "cache" / "terminal"
            except Exception:
                cache_dir = Path(tempfile.gettempdir()) / "lailaba_terminal"
    ...
    def _run_bash(self, cmd_string: str, *, login: bool = False,
        ...
        if not _IS_WINDOWS:
            try:
                proc._lailaba_pgid = os.getpgid(proc.pid)
        ...
        try:
            if _IS_WINDOWS:
                ...
                try:
                    ...
                    pgid = getattr(proc, "_lailaba_pgid", None)
                    if pgid is None:
                        raise
