...
from tools.environments.local import _LAILABA_PROVIDER_ENV_BLOCKLIST
...
def _load_lailaba_env_vars() -> dict[str, str]:
    """Load ~/.lailaba/.env values without failing Docker command execution."""
    try:
...
def _get_active_profile_name() -> str:
    """Return the active Lailaba profile name, or ``"default"`` on any error.
...
def reap_orphan_containers(
    *,
    ...
    """Remove stale lailaba-tagged containers left behind by prior processes.

    Targets containers that match all of:

    * ``label=lailaba-agent=1`` (created by this codebase)
    * ``status=exited`` (running containers are NEVER reaped — they may
      belong to a sibling Lailaba process whose reuse path will pick them
      up; killing them would crash the sibling mid-command)
    * (optional) ``label=lailaba-profile=<profile_filter>`` (sweep only the
      caller's profile by default; a lailaba process in profile A must not
    ...
    * ``State.FinishedAt`` older than *max_age_seconds* ago (so a sibling
    ...
    even with the cleanup-fix in the prior commit, a hard-killed Lailaba
    process leaves its container behind permanently because there's no
    subsequent Lailaba process scheduled to reuse that exact (task, profile)
    pair.
    """
    docker = docker_exe or find_docker() or "docker"
    filters = ["--filter", "label=lailaba-agent=1", "--filter", "status=exited"]
    if profile_filter:
        filters.extend(["--filter", f"label=lailaba-profile={_sanitize_label_value(profile_filter)}"])
...
def find_docker() -> Optional[str]:
    ...
    Resolution order:
    1. ``LAILABA_DOCKER_BINARY`` env var — explicit override (e.g. ``/usr/bin/podman``)
    ...
    override = os.getenv("LAILABA_DOCKER_BINARY")
    if override and os.path.isfile(override) and os.access(override, os.X_OK):
        _docker_executable = override
        logger.info("Using LAILABA_DOCKER_BINARY override: %s", override)
        return override
...
#   SETUID/SETGID - the image's init drops from root to the 'lailaba'
...
def _image_uses_init_entrypoint(docker_exe: str, image: str) -> bool:
    ...
    ``lailaba-agent:latest``) already provide their own PID-1 init and execute
...
class DockerEnvironment(BaseEnvironment):
    ...
    def __init__(
        ...
        # (TERMINAL_SANDBOX_DIR, default ~/.lailaba/sandboxes/). Non-persistent
        ...
        # s6-overlay images (e.g. lailaba-agent:latest) already use /init as PID 1
        ...
        container_name = f"lailaba-{uuid.uuid4().hex[:8]}"
        # Labels make lailaba-created containers identifiable to:
        #   * the orphan reaper (`lailaba-agent=1` for the global sweep filter)
        #   * future cross-process reuse (`lailaba-task-id`, `lailaba-profile`)
        #   * operators running `docker ps --filter label=lailaba-agent=1`
        # Values are limited to the safe character set defined by
        # _sanitize_label_value(); the active Lailaba profile is captured at
        ...
        label_args = [
            "--label", "lailaba-agent=1",
            "--label", f"lailaba-task-id={task_label}",
            "--label", f"lailaba-profile={profile_name}",
        ]
        ...
        self._labels = {
            "lailaba-agent": "1",
            "lailaba-task-id": task_label,
            "lailaba-profile": profile_name,
        }

        # Cross-process container reuse (issue #20561 — docs claim "ONE long-lived
        # container shared across sessions").  If a prior Lailaba process
    ...
    def _build_init_env_args(self) -> list[str]:
        ...
        # win over the generic Lailaba secret blocklist. Only implicit passthrough
        # keys are filtered.
        forward_keys = explicit_forward_keys | (passthrough_keys - _LAILABA_PROVIDER_ENV_BLOCKLIST)
        lailaba_env = _load_lailaba_env_vars() if forward_keys else {}
        for key in sorted(forward_keys):
            value = os.getenv(key)
            if not value:
                value = lailaba_env.get(key)
            if value:
                exec_env[key] = value
    ...
    def _recreate_container(self) -> bool:
        ...
        task_label = self._labels.get("lailaba-task-id", "")
        profile_label = self._labels.get("lailaba-profile", "")
        ...
        if not self._container_id:
            ...
            try:
                ...
                new_name = f"lailaba-{_uuid.uuid4().hex[:8]}"
    ...
    def _find_reusable_container(self, task_label: str, profile_label: str) -> Optional[tuple[str, str]]:
        ...
        matches containers that happened to be named ``lailaba-*`` but were
        ...
        try:
            result = subprocess.run(
                [
                    ...
                    "--filter", "label=lailaba-agent=1",
                    "--filter", f"label=lailaba-task-id={task_label}",
                    "--filter", f"label=lailaba-profile={profile_label}",
                    "--format", "{{.ID}}\t{{.State}}",
                ],
                capture_output=True,
                text=True,
        ...
        # container) but can happen if a previous Lailaba process crashed
    ...
    def cleanup(self, *, force_remove: bool = False):
        ...
        container shared across sessions" and stopping it on every Lailaba exit
        breaks that promise:
        ...
        * The user-visible difference between "ONE long-lived container" and
        ...
        Lailaba process touches a labeled container for ``2 × lifetime_seconds``
        it gets ``docker rm -f``'d at the next Lailaba startup. That covers the
        ...
        # outlives Lailaba processes, processes inside it stay alive, and
        ...
        import threading
        t = threading.Thread(target=_do_cleanup, daemon=True, name=f"lailaba-cleanup-{log_id}")
        t.start()
    ...
    def wait_for_cleanup(self, timeout: float = 30.0) -> bool:
        ...
        before the Python process exits — without this, ``lailaba /quit``
