...
it is automatically downloaded from GitHub releases to $LAILABA_HOME/bin/tirith.
...
def _get_lailaba_home() -> str:
    """Return the Lailaba home directory, respecting LAILABA_HOME env var."""
    return str(get_lailaba_home())


def _failure_marker_path() -> str:
    """Return the path to the install-failure marker file."""
    return os.path.join(_get_lailaba_home(), ".tirith-install-failed")
...
def _lailaba_bin_dir() -> str:
    """Return $LAILABA_HOME/bin, creating it if needed."""
    d = os.path.join(_get_lailaba_home(), "bin")
...
def _install_tirith(*, log_failures: bool = True) -> tuple[str | None, str]:
    """Download and install tirith to $LAILABA_HOME/bin/tirith.
    ...
    try:
        ...
        dest = os.path.join(_lailaba_bin_dir(), "tirith")
        try:
...
def _resolve_tirith_path(configured_path: str) -> str:
    ...
    2. $LAILABA_HOME/bin/tirith (previously auto-installed)
    3. Auto-install from GitHub releases → $LAILABA_HOME/bin/tirith
    ...
    lailaba_bin = os.path.join(_lailaba_bin_dir(), "tirith")
    if os.path.isfile(lailaba_bin) and os.access(lailaba_bin, os.X_OK):
        _resolved_path = lailaba_bin
        _install_failure_reason = ""
        _clear_install_failed()
        return lailaba_bin
