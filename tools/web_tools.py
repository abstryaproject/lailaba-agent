...
Backend is selected during ``lailaba tools`` setup (web.backend in config.yaml).
When available, Lailaba can route Firecrawl calls through a Nous-hosted tool-gateway
...
def _env_value(name: str) -> str:
    """Resolve ``name`` via Lailaba config-aware env, falling back to process env.

    Mirrors the SearXNG provider's ``_searxng_url()`` so that values set
    through Lailaba' config/.env layer (``lailaba config set``, ``lailaba tools``)
...
def _load_web_config() -> dict:
    """Load the ``web:`` section from ~/.lailaba/config.yaml."""
    try:
...
def _get_backend() -> str:
    ...
    Reads ``web.backend`` from config.yaml (set by ``lailaba tools``).
...
def _is_backend_available(backend: str) -> bool:
    ...
    if backend == "xai":
        ...
        # runs on every web_search dispatch + every `lailaba tools` repaint.
        try:
        ...
        if provider is None:
            response_data = {
                ...
                "error": (
                    ...
                    "Run `lailaba tools` to set one up."
                ),
            }
        else:
            logger.info(
