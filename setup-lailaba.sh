...
# 5. Symlinks the 'lailaba' CLI command into a user-facing bin dir
...
echo ""
echo -e "${CYAN}⚕ Lailaba AI Setup${NC}"
echo ""
...
if is_termux; then
    ...
    if [ -n "$UV_CMD" ]; then
        ...
        _uv_log="$(mktemp 2>/dev/null || echo "/tmp/lailaba-uv-install.$$.log")"
...
# ============================================================================
# PATH setup — symlink lailaba into a user-facing bin dir
# ============================================================================

echo -e "${CYAN}→${NC} Setting up lailaba command..."

LAILABA_BIN="$SCRIPT_DIR/venv/bin/lailaba"
COMMAND_LINK_DIR="$(get_command_link_dir)"
COMMAND_LINK_DISPLAY_DIR="$(get_command_link_display_dir)"
mkdir -p "$COMMAND_LINK_DIR"
ln -sf "$LAILABA_BIN" "$COMMAND_LINK_DIR/lailaba"
echo -e "${GREEN}✓${NC} Symlinked lailaba → $COMMAND_LINK_DISPLAY_DIR/lailaba"
...
# ============================================================================
# Seed bundled skills into ~/.lailaba/skills/
# ============================================================================

LAILABA_SKILLS_DIR="${LAILABA_HOME:-$HOME/.lailaba}/skills"
mkdir -p "$LAILABA_SKILLS_DIR"

echo ""
echo "Syncing bundled skills to ~/.lailaba/skills/ ..."
if "$SCRIPT_DIR/venv/bin/python" "$SCRIPT_DIR/tools/skills_sync.py" 2>/dev/null; then
    echo -e "${GREEN}✓${NC} Skills synced"
else
    # Fallback: copy if sync script fails (missing deps, etc.)
    if [ -d "$SCRIPT_DIR/skills" ]; then
        cp -rn "$SCRIPT_DIR/skills/"* "$LAILABA_SKILLS_DIR/" 2>/dev/null || true
        echo -e "${GREEN}✓${NC} Skills copied"
    fi
fi

# ============================================================================
# Hausa voice layer (TTS -> OGG/Opus voice bubble, STT via Groq)
# ============================================================================

echo -e "${CYAN}→${NC} Setting up Hausa voice layer..."

# 1) ffmpeg (needed to transcode gTTS mp3 -> Opus ogg for voice bubbles)
if ! command -v ffmpeg >/dev/null 2>&1; then
    if is_termux; then
        echo -e "${CYAN}→${NC} Installing ffmpeg (Termux pkg)..."
        pkg install -y ffmpeg >/dev/null 2>&1 && echo -e "${GREEN}✓${NC} ffmpeg installed" \
            || echo -e "${YELLOW}⚠${NC} pkg install ffmpeg failed — install manually: pkg install ffmpeg"
    elif command -v apt-get >/dev/null 2>&1; then
        sudo apt-get update -y >/dev/null 2>&1 && sudo apt-get install -y ffmpeg >/dev/null 2>&1 \
            && echo -e "${GREEN}✓${NC} ffmpeg installed" \
            || echo -e "${YELLOW}⚠${NC} apt install ffmpeg failed"
    else
        echo -e "${YELLOW}⚠${NC} No supported pkg mgr for ffmpeg; install it yourself (voice bubbles need it)"
    fi
else
    echo -e "${GREEN}✓${NC} ffmpeg present"
fi

# 2) gTTS (Hausa TTS, free, no API key)
echo -e "${CYAN}→${NC} Installing gTTS (Hausa text-to-speech)..."
"$SETUP_PYTHON" -m pip install -q gtts 2>/dev/null \
    && echo -e "${GREEN}✓${NC} gTTS installed" \
    || echo -e "${YELLOW}⚠${NC} gTTS install failed: $SETUP_PYTHON -m pip install gtts"

# 3) hausa_tts.py -> ~/bin/ (used by config.yaml tts.providers.hausa.command)
HAUSA_TTS_SRC="$SCRIPT_DIR/scripts/hausa_tts.py"
if [ -f "$HAUSA_TTS_SRC" ]; then
    mkdir -p "$HOME/bin"
    cp "$HAUSA_TTS_SRC" "$HOME/bin/hausa_tts.py"
    chmod +x "$HOME/bin/hausa_tts.py"
    echo -e "${GREEN}✓${NC} Copied hausa_tts.py → ~/bin/hausa_tts.py"
else
    echo -e "${YELLOW}⚠${NC} scripts/hausa_tts.py not found; Hausa TTS will not work"
fi

# 4) Config + voice-mode templates into ~/.lailaba/
LAILABA_HOME_DIR="${LAILABA_HOME:-$HOME/.lailaba}"
mkdir -p "$LAILABA_HOME_DIR"
if [ -f "$SCRIPT_DIR/config.yaml.example" ] && [ ! -f "$LAILABA_HOME_DIR/config.yaml" ]; then
    cp "$SCRIPT_DIR/config.yaml.example" "$LAILABA_HOME_DIR/config.yaml"
    echo -e "${GREEN}✓${NC} Created ~/.lailaba/config.yaml from template (edit tts/stt + .env keys)"
fi
if [ -f "$SCRIPT_DIR/gateway_voice_mode.json.example" ] && [ ! -f "$LAILABA_HOME_DIR/gateway_voice_mode.json" ]; then
    cp "$SCRIPT_DIR/gateway_voice_mode.json.example" "$LAILABA_HOME_DIR/gateway_voice_mode.json"
    echo -e "${GREEN}✓${NC} Created ~/.lailaba/gateway_voice_mode.json (set your Telegram chat id)"
fi
echo -e "${CYAN}→${NC} Hausa voice layer ready. Enable voice-only for a chat:"
echo -e "    ${GREEN}echo '{\"telegram:<CHAT_ID>\": \"voice_only\"}' > ~/.lailaba/gateway_voice_mode.json${NC}"
...
echo "Other commands:"
echo "  lailaba status        # Check configuration"
if is_termux; then
    echo "  lailaba gateway       # Run gateway in foreground"
else
    echo "  lailaba gateway install # Install gateway service (messaging + cron)"
fi
echo "  lailaba cron list     # View scheduled jobs"
echo "  lailaba doctor        # Diagnose issues"
echo ""
...
if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
    ...
    "$SCRIPT_DIR/venv/bin/python" -m lailaba_cli.main setup
fi
