#!/usr/bin/env bash
# Canonical test runner for lailaba-agent. Run this instead of calling
...
#   * Proper venv activation (probes .venv, venv, then ~/.lailaba/...)
#
# Usage:
...
# ── Activate venv ───────────────────────────────────────────────────────────
VENV=""
for candidate in "$REPO_ROOT/.venv" "$REPO_ROOT/venv" "$HOME/.lailaba/lailaba-agent/venv"; do
...
if [ -f "$HOME/.lailaba/pytest_live_guard.py" ]; then
  EXTRA_PYTHONPATH="$HOME/.lailaba"
...
exec env -i \
  ...
  ${LAILABA_RUN_SLOW_PET_TESTS:+LAILABA_RUN_SLOW_PET_TESTS="$LAILABA_RUN_SLOW_PET_TESTS"} \
