...
# Bootstrap Open WebUI against Lailaba AI's OpenAI-compatible API server.
#
# Idempotent by design:
# - ensures ~/.lailaba/.env has API server settings
# - installs Open WebUI into ~/.local/open-webui-venv
# - writes a reusable launcher at ~/.local/bin/start-open-webui-lailaba.sh
...
#   OPEN_WEBUI_NAME='Johnny Lailaba'
...
#   LAILABA_API_PORT=8642
#   LAILABA_API_HOST=127.0.0.1
#   LAILABA_API_MODEL_NAME='Lailaba AI'

OPEN_WEBUI_PORT="${OPEN_WEBUI_PORT:-8080}"
OPEN_WEBUI_HOST="${OPEN_WEBUI_HOST:-127.0.0.1}"
OPEN_WEBUI_NAME="${OPEN_WEBUI_NAME:-Lailaba AI WebUI}"
...
LAILABA_ENV_FILE="${LAILABA_ENV_FILE:-$HOME/.lailaba/.env}"
LAILABA_API_PORT="${LAILABA_API_PORT:-8642}"
LAILABA_API_HOST="${LAILABA_API_HOST:-127.0.0.1}"
LAILABA_API_CONNECT_HOST="${LAILABA_API_CONNECT_HOST:-127.0.0.1}"
LAILABA_API_MODEL_NAME="${LAILABA_API_MODEL_NAME:-Lailaba AI}"
LAILABA_API_BASE_URL="http://${LAILABA_API_CONNECT_HOST}:${LAILABA_API_PORT}/v1"
LAUNCHER_PATH="$HOME/.local/bin/start-open-webui-lailaba.sh"
LOG_DIR="$HOME/.lailaba/logs"
...
write_launcher() {
  ...
  quoted_base_url="$(shell_quote "$LAILABA_API_BASE_URL")"
...
ensure_env_permissions() {
  chmod 600 "$LAILABA_ENV_FILE" 2>/dev/null || true
}

install_launchd_service() {
  local plist="$HOME/Library/LaunchAgents/ai.openwebui.lailaba.plist"
...
<dict>
  <key>Label</key>
  <string>ai.openwebui.lailaba</string>
...
EOF
  ...
  launchctl enable "gui/$(id -u)/ai.openwebui.lailaba"
  launchctl kickstart -k "gui/$(id -u)/ai.openwebui.lailaba"
}

install_systemd_user_service() {
  require_cmd systemctl
  local unit_dir="$HOME/.config/systemd/user"
  local unit="$unit_dir/openwebui-lailaba.service"
  mkdir -p "$unit_dir"
  cat > "$unit" <<EOF
[Unit]
Description=Open WebUI connected to Lailaba AI
After=default.target

[Service]
Type=simple
ExecStart=/bin/bash %h/.local/bin/start-open-webui-lailaba.sh
Restart=always
RestartSec=3
WorkingDirectory=%h
StandardOutput=append:%h/.lailaba/logs/openwebui.log
StandardError=append:%h/.lailaba/logs/openwebui.error.log

[Install]
WantedBy=default.target
EOF
  systemctl --user daemon-reload
  systemctl --user enable --now openwebui-lailaba.service
}
...
main() {
  ...
  local api_key
  api_key="$(get_env_value API_SERVER_KEY "$LAILABA_ENV_FILE")"
  if [[ -z "$api_key" ]]; then
    api_key="$(generate_secret)"
  fi

  log 'Ensuring Lailaba API server is configured...'
  upsert_env API_SERVER_ENABLED true "$LAILABA_ENV_FILE"
  upsert_env API_SERVER_HOST "$LAILABA_API_HOST" "$LAILABA_ENV_FILE"
  upsert_env API_SERVER_PORT "$LAILABA_API_PORT" "$LAILABA_ENV_FILE"
  upsert_env API_SERVER_MODEL_NAME "$LAILABA_API_MODEL_NAME" "$LAILABA_ENV_FILE"
  upsert_env API_SERVER_KEY "$api_key" "$LAILABA_ENV_FILE"
  ensure_env_permissions

  log 'Restarting Lailaba gateway so API server settings take effect...'
  lailaba gateway restart >/dev/null 2>&1 || true
  sleep 4
  if ! curl -fsS "http://${LAILABA_API_CONNECT_HOST}:${LAILABA_API_PORT}/health" >/dev/null; then
    log 'Lailaba API server did not answer on the first check. Trying to start gateway in the background...'
    nohup lailaba gateway run >/dev/null 2>&1 &
    sleep 6
  fi
  curl -fsS "http://${LAILABA_API_CONNECT_HOST}:${LAILABA_API_PORT}/health" >/dev/null
  ...
  log "Lailaba API endpoint: ${LAILABA_API_BASE_URL}"
