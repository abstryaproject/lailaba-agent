 ...
 *
 * Usage:
 *   node bridge.js --port 3000 --session ~/.lailaba/whatsapp/session
 */
...
const SESSION_DIR = getArg('session', path.join(process.env.HOME || '~', '.lailaba', 'whatsapp', 'session'));
// Cache directories: the Python gateway passes the profile-aware paths via
// env (LAILABA_HOME-aware, new cache/ layout).  Fall back to the legacy
// hardcoded locations for bridges launched outside the gateway.
const IMAGE_CACHE_DIR = process.env.LAILABA_IMAGE_CACHE_DIR
  || path.join(process.env.HOME || '~', '.lailaba', 'image_cache');
const DOCUMENT_CACHE_DIR = process.env.LAILABA_DOCUMENT_CACHE_DIR
  || path.join(process.env.HOME || '~', '.lailaba', 'document_cache');
const AUDIO_CACHE_DIR = process.env.LAILABA_AUDIO_CACHE_DIR
  || path.join(process.env.HOME || '~', '.lailaba', 'audio_cache');
...
// `lailaba update` updates bridge.js on disk but a long-lived bridge process
...
const DEFAULT_REPLY_PREFIX = '⚕ *Lailaba AI*\n────────────\n';
...
async function startSocket() {
  ...
  sock = makeWASocket({
    ...
    browser: ['Lailaba AI', 'Chrome', '120.0'],
  ...
  sock.ev.on('messages.upsert', async ({ messages, type }) => {
    ...
    for (const msg of messages) {
      ...
      // Ignore Lailaba' own reply messages in self-chat mode to avoid loops.
  ...
  try {
    ...
    switch (type) {
      ...
      case 'audio': {
        ...
        let tmpPath = null;
        if (needsConversion) {
          tmpPath = path.join(tmpdir(), `lailaba_voice_${randomBytes(6).toString('hex')}.ogg`);
          try {
            execSync(
