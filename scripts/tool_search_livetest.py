#!/usr/bin/env python3
"""Live test harness for Lailaba AI's Tool Search feature.
...
# Force-isolate the test environment BEFORE any lailaba imports.
ORIGINAL_HOME = os.environ.get("LAILABA_HOME")
ORIGINAL_AUTH = Path.home() / ".lailaba" / "auth.json"
...
def setup_isolated_home(enabled: bool) -> Path:
    """Create a fresh ~/.lailaba/ for one test, copying minimal credentials.

    Also reads OPENROUTER_API_KEY from the user's real ``~/.lailaba/.env`` so
    the agent can authenticate against OpenRouter inside the isolated home.
    """
    home_dir = Path(tempfile.mkdtemp(prefix="lailaba_ts_live_"))
    lailaba_home = home_dir / ".lailaba"
    lailaba_home.mkdir(parents=True)

    if ORIGINAL_AUTH.exists():
        shutil.copy(ORIGINAL_AUTH, lailaba_home / "auth.json")

    # Copy .env so OPENROUTER_API_KEY (or others) are visible to the agent
    # running inside the isolated home.
    real_env_file = Path.home() / ".lailaba" / ".env"
    if real_env_file.exists():
        shutil.copy(real_env_file, lailaba_home / ".env")
        ...
        load_lailaba_dotenv(lailaba_home=str(Path.home() / ".lailaba"))
    ...
    (lailaba_home / "config.yaml").write_text(_yaml_dump(cfg), encoding="utf-8")
    return lailaba_home
...
def reset_module_state():
    """Drop cached modules so the new LAILABA_HOME takes effect."""
    keys = [k for k in sys.modules.keys()
            if k.startswith(("tools.", "model_tools", "toolsets",
                             "lailaba_cli", "agent.", "run_agent"))]
    for k in keys:
        del sys.modules[k]
...
def run_one_scenario(scenario: Dict[str, Any], enabled: bool, out_dir: Path) -> Dict[str, Any]:
    ...
    os.environ["LAILABA_HOME"] = str(home)
...
def main():
    ...
    # Restore original LAILABA_HOME
    if ORIGINAL_HOME is not None:
        os.environ["LAILABA_HOME"] = ORIGINAL_HOME
    else:
