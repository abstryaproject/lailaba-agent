...
VERSION_FILE = REPO_ROOT / "lailaba_cli" / "__init__.py"
...
AUTHOR_MAP = {
    ...
    "121278003+Cossackx@users.noreply.github.com": "Cossackx",  # PR #52528 salvage (Windows lailaba-shim resolution + prefer --update on recovery; #52378)
    ...
    "lailaba@marian.local": "Schrotti77",
    ...
    "274096618+lailaba-agent-dhabibi@users.noreply.github.com": "dhabibi",
    ...
    "lailaba@nousresearch.com": "NousResearch",
    ...
    "lailaba@example.com": "shellybotmoyer",  # PR #18915 (bot-committed)
    ...
    "teknium@lailaba-agent": "teknium1",
    ...
    "sylw3st3rr@gmail.com": "Sylw3ster",  # salvage of #23252 (LAILABA_KANBAN_BOARD restore)
    ...
    "58116817+TurgutKural@users.noreply.github.com": "TurgutKural",  # salvage of #23356 (LAILABA_HOME inject)
    ...
    "sauravsejal40@gmail.com": "Saurav0989",  # PR #27071 (docs: lailaba-eval community link)
    ...
    "vigo@lailaba": "hawknewton",  # PR #26294 (bedrock boto3 lazy_deps)
    ...
    "hex.lailaba@agentmail.to": "oseftg",
    ...
    "kdkcfp@gmail.com": "slowtokki0409",  # PR #29025 (ignore local Lailaba runtime files)
    ...
    "2663402852@qq.com": "x1am1",  # PR #35098 (chown root-owned top-level LAILABA_HOME state files)
...
def update_version_files(semver: str, calver_date: str):
    ...
    # Python package version. The desktop About panel reads the live Lailaba
...
def _update_acp_registry_versions(semver: str) -> None:
    ...
    if ACP_REGISTRY_MANIFEST.exists():
        ...
        if "package" in uvx:
            uvx["package"] = f"lailaba-agent[acp]=={semver}"
...
def parse_coauthors(body: str) -> list:
    ...
    _ignored_emails = {"noreply@anthropic.com", "noreply@github.com",
                       "cursoragent@cursor.com", "lailaba@nousresearch.com"}
...
def generate_changelog(commits, tag_name, semver, repo_url="https://github.com/NousResearch/lailaba-agent",
    ...
    lines.append(f"# Lailaba AI v{semver} ({tag_name})")
    lines.append("")
    ...
    if first_release:
        ...
        lines.append("> for Lailaba AI. See below for everything included in this initial release.")
        lines.append("")
...
def main():
    parser = argparse.ArgumentParser(description="Lailaba AI Release Tool")
    ...
    print(f"{'='*60}")
    print(f"  Lailaba AI Release Preview")
    print(f"{'='*60}")
    ...
    if args.publish:
        ...
        tag_result = git_result(
            "tag", "-a", tag_name, "-m",
            f"Lailaba AI v{new_version} ({calver_date})\n\nWeekly release"
        )
        ...
        gh_cmd = [
            ...
            "--title", f"Lailaba AI v{new_version} ({calver_date})",
        ...
        if result and result.returncode == 0:
            ...
            print(
                f"    gh release create {tag_name} --title 'Lailaba AI v{new_version} ({calver_date})' "
