# ============================================================================
# Lailaba AI Installer for Windows
# ============================================================================
# Installation script for Windows (PowerShell).
# Uses uv for fast Python provisioning and package management.
#
# Usage:
#   iex (irm https://lailaba-agent.nousresearch.com/install.ps1)
#
...
param(
    ...
    [string]$Commit = "",
    [string]$Tag = "",
    [string]$HermesHome = $(if ($env:LAILABA_HOME) { $env:LAILABA_HOME } else { "$env:LOCALAPPDATA\lailaba" }),
    [string]$InstallDir = $(if ($env:LAILABA_HOME) { "$env:LAILABA_HOME\lailaba-agent" } else { "$env:LOCALAPPDATA\lailaba\lailaba-agent" }),
    ...
    # builds apps/desktop into a launchable Lailaba.exe.
    #
    # Why opt-in:
    #   * Lailaba-Setup.exe (the signed Tauri bootstrap installer) passes
    ...
    #     from inside an already-launched Lailaba.exe; if THAT recursively
    #     built apps/desktop it would try to overwrite the live Lailaba.exe
    ...
    #     `lailaba desktop` already builds on demand.
...
$RepoUrlSsh = "git@github.com:NousResearch/lailaba-agent.git"
$RepoUrlHttps = "https://github.com/NousResearch/lailaba-agent.git"
$PythonVersion = "3.11"
...
function Write-Banner {
    ...
    Write-Host "|             * Lailaba AI Installer                    |" -ForegroundColor Magenta
    Write-Host "+---------------------------------------------------------+" -ForegroundColor Magenta
...
function Install-Uv {
    # Lailaba owns its own uv at $HermesHome\bin\uv.exe.  Always install there —
    # no PATH probing, no conda guards, no multi-location resolution chains.
    # The runtime update path (lailaba_cli/managed_uv.py) looks in the same
    # place, so install.ps1 and `lailaba update` stay in sync.
...
function Resolve-UvCmd {
    ...
    # sibling process and LAILABA_HOME wasn't propagated).
...
function Resolve-AvailablePythonVersion {
    ...
    # ``$script:PythonVersion = $fallbackVer`` mutation.  Under Lailaba-Setup.exe
...
function Install-Git {
    ...
    which Lailaba uses to run shell commands.
    ...
    package manager):
      ...
      2. Download **PortableGit** from the official git-for-windows GitHub
         ...
         ``%LOCALAPPDATA%\lailaba\git`` -- never touches system Git, never
    ...
    **Why PortableGit, not MinGit:**  MinGit is the minimal-automation
    ...
    Lailaba needs ``bash.exe`` to run shell commands.  PortableGit is the
    ...
    blocked).  Owning the Lailaba copy of Git ourselves is predictable and
    recoverable: if it ever breaks, ``Remove-Item %LOCALAPPDATA%\lailaba\git``
    and re-running this installer fully recovers.

    After install we locate ``bash.exe`` and persist the path in
    ``LAILABA_GIT_BASH_PATH`` (User scope) so Lailaba can find it in a fresh
    ...
    try {
        ...
        if ($arch -eq "32-bit-mingit") {
            Write-Warn "32-bit Windows detected -- PortableGit is 64-bit only.  Installing MinGit 32-bit as a last resort; bash-dependent Lailaba features (terminal tool, agent-browser) will not work on this machine."
        ...
        Write-Info "then re-run this installer.  Lailaba needs Git Bash on Windows to run"
...
function Set-GitBashEnvVar {
    ...
    ``LAILABA_GIT_BASH_PATH`` (User env scope) so Lailaba can find it even before
    ...
    foreach ($candidate in $candidates) {
        if ($candidate -and (Test-Path $candidate)) {
            [Environment]::SetEnvironmentVariable("LAILABA_GIT_BASH_PATH", $candidate, "User")
            $env:LAILABA_GIT_BASH_PATH = $candidate
            Write-Info "Set LAILABA_GIT_BASH_PATH=$candidate"
            return
        }
    }

    Write-Warn "Could not locate bash.exe -- Lailaba may not find Git Bash."
    Write-Info "If needed, set LAILABA_GIT_BASH_PATH manually to your bash.exe path."
}
...
function Test-Node {
    ...
    if ((Test-Path $managedNode) -and (Test-NodeVersionOk (& $managedNode --version))) {
        ...
        $env:Path = "$HermesHome\node;$env:Path"
        Write-Success "Node.js $version found (Lailaba-managed)"
        $script:HasNode = $true
        return $true
    }

    Write-Info "Installing Lailaba-managed Node.js $NodeVersion LTS..."
    ...
    try {
        ...
        if ($zipName) {
            ...
            $tmpDir = "$env:TEMP\lailaba-node-extract"
...
function Install-SystemPackages {
    ...
    if ($hasWinget) {
        ...
        foreach ($pkg in $wingetPkgs) {
            $log = "$env:TEMP\lailaba-winget-$($pkg -replace '[^A-Za-z0-9]','_')-$(Get-Random).log"
            $pkgLogs[$pkg] = $log
...
function Install-Repository {
        ...
        if ($repoValid) {
            ...
            try {
                ...
                if (-not [string]::IsNullOrWhiteSpace(($statusOut -join "`n"))) {
                    ...
                    # the index conflict state is dropped. Mirrors the `lailaba
                    # update` path (#4735).
                    ...
                    $stashName = "lailaba-install-autostash-" + (Get-Date -Format "yyyyMMdd-HHmmss")
                ...
                if ($autostashRef) {
                    ...
                    if ($restoreNow) {
                        ...
                        if ($LASTEXITCODE -eq 0) {
                            ...
                            Write-Warn "Review git diff / git status if Lailaba behaves unexpectedly."
                        } else {
            ...
            try {
                ...
                Write-Info "terminals, running lailaba processes) and try again."
                throw
            }
        }
    }
    ...
    if (-not $didUpdate) {
        ...
        if (-not $cloneSuccess) {
            ...
            try {
                ...
                if ($Commit) {
                    $zipUrl = "https://github.com/NousResearch/lailaba-agent/archive/$Commit.zip"
                    $zipLabel = $Commit
                } elseif ($Tag) {
                    $zipUrl = "https://github.com/NousResearch/lailaba-agent/archive/refs/tags/$Tag.zip"
                    $zipLabel = $Tag
                } else {
                    $zipUrl = "https://github.com/NousResearch/lailaba-agent/archive/refs/heads/$Branch.zip"
                    $zipLabel = $Branch
                }
                $zipPath = "$env:TEMP\lailaba-agent-$zipLabel.zip"
                $extractPath = "$env:TEMP\lailaba-agent-extract"
    ...
    # next `lailaba update` checkout aborts on a "dirty" tree the user never
...
function Install-Venv {
    ...
    # Re-resolve the interpreter before creating the venv.  Under Lailaba-Setup.exe
    ...
    if (Test-Path "venv") {
        ...
        # speedups.pyd) are loaded as DLLs by any running lailaba process.
        ...
        if ($env:OS -eq "Windows_NT") {
            $myPid = $PID
            Write-Info "Stopping any running lailaba processes before recreating venv..."
            # The launcher CLI (lailaba.exe) plus its child tree.
            & taskkill /F /T /IM lailaba.exe /FI "PID ne $myPid" 2>$null | Out-Null
            # taskkill /IM lailaba.exe is NOT enough: the gateway/agent that a
            # scheduled task or watchdog autostarts runs as
            # `pythonw.exe -m lailaba_cli.main gateway run` straight out of
            # venv\Scripts\, so its image name is python/pythonw, not lailaba.exe.
            ...
            # user (see lailaba_cli/gateway_windows.py), so the installer always
...
function Install-Dependencies {
    ...
    if (Test-Path "uv.lock") {
        ...
        # empty and producing the broken state where `lailaba.exe` exists
...
"@ 2>$null
    ...
    if (-not $installed) {
        throw "Failed to install lailaba-agent package even with no extras. Inspect the uv pip install output above."
    }
    ...
    # isn't set, leaving venv\ empty and lailaba.exe broken with
    ...
    # here, not 30 seconds later when the user runs `lailaba`.
    if (-not $NoVenv) {
    ...
    # users hit and lazy-import errors from `lailaba dashboard` are confusing.
    ...
    if (Test-Path $pythonExe) {
        ...
        $ErrorActionPreference = $prevEAP
        if (-not $webOk) {
            Write-Warn "fastapi/uvicorn not importable -- `lailaba dashboard` will not work."
            Write-Info "Attempting targeted install of [web] extra as last resort..."
            & $UvCmd pip install -e ".[web]"
            if ($LASTEXITCODE -eq 0) {
                Write-Success "[web] extra installed; `lailaba dashboard` should now work."
            } else {
...
function Set-PathVariable {
    Write-Info "Setting up lailaba command..."
    ...
    # Add the venv Scripts dir to user PATH so lailaba is globally available
    # On Windows, the lailaba.exe in venv\Scripts\ has the venv Python baked in
    ...
    # Set LAILABA_HOME so the Python code finds config/data in the right place.
    # Only needed on Windows where we install to %LOCALAPPDATA%\lailaba instead
    # of the Unix default ~/.lailaba
    $currentHermesHome = [Environment]::GetEnvironmentVariable("LAILABA_HOME", "User")
    if (-not $currentHermesHome -or $currentHermesHome -ne $HermesHome) {
        [Environment]::SetEnvironmentVariable("LAILABA_HOME", $HermesHome, "User")
        Write-Success "Set LAILABA_HOME=$HermesHome"
    }
    $env:LAILABA_HOME = $HermesHome
    
    # Update current session
    $env:Path = "$hermesBin;$env:Path"
    
    Write-Success "lailaba command ready"
}

function Write-BootstrapMarker {
    # Writes $InstallDir\.lailaba-bootstrap-complete which tells the Lailaba
    ...
    # Lailaba-Setup.exe) or fall back to whatever git resolves in the
    # checkout. The desktop validates schemaVersion + pinnedCommit
    # length but doesn't enforce that HEAD matches the pin (users
    # update via `lailaba update` which moves HEAD legitimately).
    ...
    $markerPath = Join-Path $InstallDir ".lailaba-bootstrap-complete"
    $marker = [ordered]@{
        schemaVersion = 1
...
function Copy-ConfigTemplates {
    ...
    # Create the LAILABA_HOME directory structure ($HermesHome, default %LOCALAPPDATA%\lailaba)
    ...
    # (the default PS5 behaviour), and Lailaba's prompt-injection scanner
    ...
    if (-not (Test-Path $soulPath)) {
        # MUST match DEFAULT_SOUL_MD in lailaba_cli/default_soul.py. The runtime
...
function Install-NodeDeps {
    if (-not $HasNode) {
        # Cross-process driver mode (Lailaba-Setup.exe runs each -Stage NAME
    ...
    if (-not $npmCmd) {
        ...
        Write-Info "Open a new PowerShell window and re-run 'lailaba setup tools' later."
        return
    }
    $npmExe = $npmCmd.Source
    if ($npmExe -like "*.ps1") {
    ...
    if (Test-Path "$InstallDir\package.json") {
        ...
        $browserLog = "$env:TEMP\lailaba-npm-browser-$(Get-Random).log"
        ...
        if ($browserNpmOk) {
            ...
            if (-not $npxExe) {
                ...
                $pwLog = "$env:TEMP\lailaba-playwright-install-$(Get-Random).log"
                Push-Location $InstallDir
    ...
    if (Test-Path "$tuiDir\package.json") {
        ...
        $tuiLog = "$env:TEMP\lailaba-npm-tui-$(Get-Random).log"
...
# the final `electron` -> `Lailaba` rename dies with ENOENT and every re-run
...
function Install-Desktop {
    # Build apps/desktop into a launchable Lailaba.exe. Only called from
    ...
    #
    # The Tauri bootstrap installer's launch_lailaba_desktop command
    # resolves apps/desktop/release/win-unpacked/Lailaba.exe directly,
    ...
    # enforces the build floor (^20.19 || >=22.12) and prepends the Lailaba-managed
    ...
    # The --dir mode produces an unpacked Lailaba.exe in
    ...
    # are NOT trying to work around). The Lailaba icon + product name are
    # stamped onto Lailaba.exe by our own rcedit step (Set-DesktopExeIdentity)
    ...
    $buildLog = "$env:TEMP\lailaba-desktop-build-$(Get-Random).log"
    ...
    $exeCandidates = @(
        "$desktopDir\release\win-unpacked\Lailaba.exe",
        "$desktopDir\release\win-arm64-unpacked\Lailaba.exe"
    )
    $found = $false
    $desktopExe = $null
    ...
    if (-not $found) {
        throw "Desktop build completed but no Lailaba.exe was found under $desktopDir\release\*-unpacked\"
    }

    # 3b. The Lailaba icon + identity are stamped onto Lailaba.exe by the
    ...
    #    Lailaba.exe. We deliberately do NOT point them at `lailaba desktop`: that
...
function New-DesktopShortcuts {
    ...
    try {
        ...
        $targets = @(
            (Join-Path ([Environment]::GetFolderPath('Programs')) 'Lailaba.lnk'),
            (Join-Path ([Environment]::GetFolderPath('Desktop')) 'Lailaba.lnk')
        )
        ...
        # with the Lailaba icon, but without this the shortcut can keep drawing
...
function Install-PlatformSdks {
    ...
    # ~/.lailaba/.env are importable.  Two problems this solves:
    #
...
function Invoke-SetupWizard {
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Skipping setup wizard (non-interactive). Configure via the GUI or 'lailaba setup'."
        return
    }
    ...
    if (-not $NoVenv) {
        & ".\venv\Scripts\python.exe" -m lailaba_cli.main setup
    } else {
        python -m lailaba_cli.main setup
    }
...
function Start-GatewayIfConfigured {
    ...
    $hermesCmd = "$InstallDir\venv\Scripts\lailaba.exe"
    ...
    if ($whatsappEnabled -and -not (Test-Path $whatsappSession)) {
        ...
        Write-Info "Running 'lailaba whatsapp' to pair via QR code..."
        Write-Host ""
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Start the gateway later with: lailaba gateway"
        return
    }
    ...
    if ($response -eq "" -or $response -match "^[Yy]") {
        ...
        try {
            ...
            Write-Warn "Failed to start gateway. Run manually: lailaba gateway"
        }
    } else {
        Write-Info "Skipped. Start the gateway later with: lailaba gateway"
    }
}
...
function Write-Completion {
    ...
    Write-Host "$HermesHome\lailaba-agent\"
    Write-Host ""
    ...
    Write-Host ""
    Write-Host "   lailaba              " -NoNewline -ForegroundColor Green
    Write-Host "Start chatting"
    Write-Host "   lailaba setup        " -NoNewline -ForegroundColor Green
    Write-Host "Configure API keys & settings"
    Write-Host "   lailaba config       " -NoNewline -ForegroundColor Green
    Write-Host "View/edit configuration"
    Write-Host "   lailaba config edit  " -NoNewline -ForegroundColor Green
    Write-Host "Open config in editor"
    Write-Host "   lailaba gateway      " -NoNewline -ForegroundColor Green
    Write-Host "Start messaging gateway (Telegram, Discord, etc.)"
    Write-Host "   lailaba update       " -NoNewline -ForegroundColor Green
...
$InstallStages = @(
    ...
    @{ Name = "repository";       Title = "Cloning Lailaba repository";            Category = "install";      NeedsUserInput = $false; Worker = "Stage-Repository" }
...
if ($IncludeDesktop) {
    ...
    # (Lailaba-Setup.exe), never via the irm|iex CLI one-liner.
    $InstallStages += @{ Name = "desktop"; Title = "Building desktop app"; Category = "install"; NeedsUserInput = $false; Worker = "Stage-Desktop" }
}
$InstallStages += @(
    @{ Name = "path";             Title = "Adding Lailaba to PATH";                Category = "finalize";     NeedsUserInput = $false; Worker = "Stage-Path" }
...
try {
    ...
    Write-Host "  Invoke-WebRequest -Uri 'https://lailaba-agent.nousresearch.com/install.ps1' -OutFile install.ps1" -ForegroundColor Yellow
