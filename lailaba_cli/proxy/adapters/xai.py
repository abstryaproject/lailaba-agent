...
# xAI's public API is OpenAI-compatible for the endpoints Lailaba commonly
# uses. The Responses endpoint is included because Lailaba' native xAI runtime
...
class XAIGrokAdapter(UpstreamAdapter):
    """Proxy upstream for xAI Grok via Lailaba-managed OAuth credentials."""

    auth_hint = "lailaba auth add xai-oauth --type oauth"
        ...
        with self._lock:
            ...
            if pool is None or not pool.has_credentials():
                raise RuntimeError(
                    ...
                    "`lailaba auth add xai-oauth --type oauth` first."
                )
            ...
            if entry is None:
                raise RuntimeError(
                    ...
                    "`lailaba auth reset xai-oauth` or re-authenticate with "
                    "`lailaba auth add xai-oauth --type oauth`."
                )
    ...
    def _credential_from_entry(self, entry: PooledCredential) -> UpstreamCredential:
        ...
        if not bearer:
            raise RuntimeError(
                ...
                "Re-authenticate with `lailaba auth add xai-oauth --type oauth`."
            )
