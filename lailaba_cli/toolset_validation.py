...
the tool registry or launching Lailaba (mirrors the decoupled-helper pattern used
elsewhere in the CLI).

Motivated by #38798: a config migration silently rewrote the valid toolset name
``lailaba-cli`` to the non-existent ``lailaba``. ``resolve_toolset('lailaba')``
...
def validate_platform_toolsets(
    ...
    1. A toolset name that ``is_valid_toolset`` rejects — usually a corrupted or
       renamed entry. When ``lailaba-<platform>`` would have been valid (the exact
       #38798 shape, where ``cli`` held ``lailaba`` instead of ``lailaba-cli``),
    ...
    if valid_count == 0:
        warnings.append(
            ...
            "have no tools. Run `lailaba tools` to reconfigure."
        )
    return warnings
