"""``lailaba slack ...`` CLI subcommands.

Today only ``lailaba slack manifest`` is implemented — it generates the
...
Typical workflow::
    ...
    $ lailaba slack manifest > slack-manifest.json
    # or:
...
def _build_full_manifest(
    ...
    it stays in sync with the rest of Lailaba. Other manifest sections
    (display info, OAuth scopes, socket mode) are set to sensible defaults
    for a Lailaba deployment — users can tweak them in the Slack UI after
    pasting.
    ...
    ``/new``, etc. work inline.
    """
    from lailaba_cli.commands import slack_app_manifest
    ...
    if include_assistant:
        features["assistant_view"] = {
            "assistant_description": "Chat with Lailaba in threads and DMs.",
        }
    ...
    return {
        ...
        "display_information": {
            "name": bot_name[:35],
            "description": (bot_description or "Your Lailaba agent on Slack")[:140],
...
def slack_manifest_command(args) -> int:
    ...
    Flags (all parsed in ``lailaba_cli/main.py``):
      --write [PATH]  Write to file instead of stdout (default path:
                      ``$LAILABA_HOME/slack-manifest.json``)
      --name NAME     Override the bot display name (default: "Lailaba")
    ...
    """
    name = getattr(args, "name", None) or "Lailaba"
    description = getattr(args, "description", None) or "Your Lailaba agent on Slack"
    ...
    if write_target is not None:
        if isinstance(write_target, bool) and write_target:
            ...
            try:
                ...
                target = Path(os.environ.get("LAILABA_HOME") or str(Path.home() / ".lailaba")) / "slack-manifest.json"
        else:
        ...
        print(
            "\nNext steps:\n"
            "  1. Open https://api.slack.com/apps and pick your Lailaba app\n"
