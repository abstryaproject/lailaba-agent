"""
Lailaba AI Uninstaller.

Provides options for:
- Full uninstall: Remove everything including configs and data
- Keep data: Remove code but keep ~/.lailaba/ (configs, sessions, logs)
"""
...
def remove_path_from_shell_configs():
    """Remove Lailaba PATH entries from shell configuration files."""
    ...
    for config_path in configs:
        try:
            ...
            # Remove lines containing lailaba-agent or lailaba PATH entries
            new_lines = []
            skip_next = False
            
            for line in content.split('\n'):
                # Skip the "# Lailaba AI" comment and following line
                if '# Lailaba AI' in line or '# lailaba-agent' in line:
                    skip_next = True
                    continue
                if skip_next and ('lailaba' in line.lower() and 'PATH' in line):
                    skip_next = False
                    continue
                skip_next = False
                
                # Remove any PATH line containing lailaba
                if 'lailaba' in line.lower() and ('PATH=' in line or 'path=' in line.lower()):
                    continue
...
def remove_wrapper_script():
    """Remove the lailaba wrapper script if it exists."""
    wrapper_paths = [
        Path.home() / ".local" / "bin" / "lailaba",
        Path("/usr/local/bin/lailaba"),
    ]
    ...
    for wrapper in wrapper_paths:
        if wrapper.exists():
            try:
                # Check if it's our wrapper (contains lailaba_cli reference)
                content = wrapper.read_text()
                if 'lailaba_cli' in content or 'lailaba-agent' in content:
                    wrapper.unlink()
...
def remove_node_symlinks(lailaba_home: Path) -> list:
    ...
    symlinks node/npm/npx into the same directory as the ``lailaba`` command:
    ...
    the FHS fix).  Only symlinks that resolve into this Lailaba home's ``node``
    directory are removed — links the user has repointed elsewhere (nvm, fnm,
    etc.) are left untouched.
    """
    node_dir = (lailaba_home / "node").resolve()
    removed = []
...
def uninstall_gateway_service():
    ...
    - All platforms: standalone ``lailaba gateway run`` processes
    ...
    try:
        from lailaba_cli.gateway import kill_gateway_processes, find_gateway_pids
    ...
    # 3. macOS: uninstall launchd plist
        ...
        try:
            from lailaba_cli.gateway import get_launchd_plist_path
...
#
#   1. Sets User-scope env vars ``LAILABA_HOME`` and ``LAILABA_GIT_BASH_PATH``
...
#      like ``%LOCALAPPDATA%\lailaba\git\cmd``, ``%LOCALAPPDATA%\lailaba\git\bin``,
#      ``%LOCALAPPDATA%\lailaba\git\usr\bin``, ``%LOCALAPPDATA%\lailaba\node``.
#      Again not in any rc file — only accessible via the registry or the
#      .NET [Environment] API.
#   3. Downloads PortableGit to ``%LOCALAPPDATA%\lailaba\git\`` and Node to
#      ``%LOCALAPPDATA%\lailaba\node\`` as user-scoped, isolated copies.
#      These are ~200MB combined and serve no purpose after uninstall.
#   4. On the ``lailaba dashboard`` + gateway paths, drops files into
#      ``%LOCALAPPDATA%\lailaba\gateway-service\`` and sometimes
...
def _lailaba_path_markers(lailaba_home: Path) -> list[str]:
    """Path-entry substrings that identify Lailaba-owned User-PATH entries."""
    root = str(lailaba_home).rstrip("\\/")
    # Match on prefix so sub-entries (git\cmd, git\bin, git\usr\bin, node, etc.)
    # all get swept.  Also match the bare lailaba-agent install dir.
    markers = [root + "\\lailaba-agent", root + "\\git", root + "\\node", root + "\\venv"]
    # Also match if LAILABA_HOME was customised to somewhere else — find-and-nuke
    # any entry whose path component contains "lailaba".  We don't want to catch
    # unrelated entries like "chermes-foo" or "ephermeral", so we look for
    # backslash-lailaba as a word-ish boundary.
    return markers


def remove_path_from_windows_registry(lailaba_home: Path) -> list[str]:
    """Strip Lailaba-owned entries from User-scope PATH in the registry.
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, key_path, 0,
            ...
            markers = _lailaba_path_markers(lailaba_home)
            kept: list[str] = []
...
def remove_lailaba_env_vars_windows() -> list[str]:
    """Delete LAILABA_HOME and LAILABA_GIT_BASH_PATH from User-scope env vars."""
    try:
        import winreg
    except ImportError:
        return []
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, "Environment", 0,
                            winreg.KEY_READ | winreg.KEY_WRITE) as key:
            for name in ("LAILABA_HOME", "LAILABA_GIT_BASH_PATH"):
                try:
...
def _is_default_lailaba_home(lailaba_home: Path) -> bool:
    """Return True when ``lailaba_home`` points at the default (non-profile) root."""
    try:
        from lailaba_constants import get_default_lailaba_root
        return lailaba_home.resolve() == get_default_lailaba_root().resolve()
    except Exception:
        return False
...
def _uninstall_profile(profile) -> None:
    ...
    remove its alias wrapper, and wipe its LAILABA_HOME directory.

    We shell out to ``lailaba -p <name> gateway stop|uninstall`` because
    service names, unit paths, and plist paths are all derived from the
    current LAILABA_HOME and can't be easily switched in-process.
    """
    import sys as _sys
    name = profile.name
    ...
    #    Use `python -m lailaba_cli.main` so we don't depend on a `lailaba`
    #    wrapper that may be half-removed mid-uninstall.
    lailaba_invocation = [_sys.executable, "-m", "lailaba_cli.main", "--profile", name]
    for subcmd in ("stop", "uninstall"):
        try:
            subprocess.run(
                lailaba_invocation + ["gateway", subcmd],
                capture_output=True,
                text=True,
                timeout=60,
                check=False,
    ...
    # 3. Wipe the profile's LAILABA_HOME directory.
    try:
...
def run_gui_uninstall(args):
    ...
    Mirrors ``lailaba uninstall --gui``. Removes the desktop app's built
    artifacts, the packaged app bundle (best-effort), and the Electron
    userData dir — nothing under ``$LAILABA_HOME`` config/sessions/.env, and
    never the Python agent or its venv.
    """
    from lailaba_cli.gui_uninstall import (
        agent_is_installed,
        gui_install_summary,
        uninstall_gui,
    )
    ...
    summary = gui_install_summary(lailaba_home)
    skip_confirm = bool(getattr(args, "yes", False))

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│         ⚕ Lailaba Chat GUI Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()

    if not summary["gui_installed"]:
        print("No Lailaba Chat GUI installation was found.")
        print(f"  Checked: {lailaba_home}, and the standard app locations for this OS.")
        return

    print(color("This removes the Chat GUI only. The Lailaba agent stays installed.", Colors.CYAN))
    print()
    ...
    print()
    if agent_is_installed(lailaba_home):
        print(color("Kept intact:", Colors.GREEN, Colors.BOLD))
        print(f"  • The Lailaba agent at {lailaba_home / 'lailaba-agent'}")
        print(f"  • Your config, sessions, and secrets under {lailaba_home}")
        print()
    ...
    print(color("└─────────────────────────────────────────────────────────┘", Colors.GREEN, Colors.BOLD))
    print()
    print("The Lailaba agent is still installed. Run 'lailaba' to use the CLI,")
    print("or 'lailaba uninstall' to remove the agent too.")
    print()
...
def run_uninstall(args):
    ...
    Options:
    - Full uninstall: removes code + ~/.lailaba/ (configs, data, logs)
    - Keep data: removes code but keeps ~/.lailaba/ for future reinstall
    """
    project_root = get_project_root()
    lailaba_home = get_lailaba_home()

    # Detect named profiles when uninstalling from the default root —
    # offer to clean them up too instead of leaving zombie LAILABA_HOMEs
    # and systemd units behind.
    is_default_profile = _is_default_lailaba_home(lailaba_home)
    named_profiles = _discover_named_profiles() if is_default_profile else []

    # Non-interactive fast path (``--yes``): no prompts. ``--full`` selects a
    # full wipe (code + ~/.lailaba data); otherwise keep-data. Named profiles
    ...
    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│            ⚕ Lailaba AI Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()
    ...
    print(f"  Config:  {lailaba_home / 'config.yaml'}")
    print(f"  Secrets: {lailaba_home / '.env'}")
    print(f"  Data:    {lailaba_home / 'cron/'}, {lailaba_home / 'sessions/'}, {lailaba_home / 'logs/'}")
    print()
    ...
    # their alias wrappers, and wiping their LAILABA_HOME dirs. Otherwise
    ...
    print()
    if full_uninstall:
        print(color("⚠️  WARNING: This will permanently delete ALL Lailaba data!", Colors.RED, Colors.BOLD))
        ...
        print("This will remove the Lailaba code but keep your configuration and data.")
...
def _perform_uninstall(
    *,
    ...
    ``lailaba`` wrapper + node symlinks → remove the desktop Chat GUI artifacts →
    delete the code checkout → (Windows) remove PortableGit/Node → optionally
    wipe ``$LAILABA_HOME`` data and named profiles on full uninstall.
    """
    print()
    ...
    if _is_windows():
        ...
        # Expand %LOCALAPPDATA% etc. in lailaba_home so the marker matching is
        # against fully resolved paths — installer writes literal strings
        # like C:\Users\<u>\AppData\Local\lailaba\git\cmd, not %LOCALAPPDATA%.
        removed_path_entries = remove_path_from_windows_registry(Path(os.path.expandvars(str(lailaba_home))))
        if removed_path_entries:
            for entry in removed_path_entries:
                log_success(f"Removed from User PATH: {entry}")
        else:
            log_info("No Lailaba-owned PATH entries in User environment")

        log_info("Removing LAILABA_HOME / LAILABA_GIT_BASH_PATH User env vars...")
        removed_env = remove_lailaba_env_vars_windows()
        if removed_env:
            for name in removed_env:
                log_success(f"Removed User env var: {name}")
        else:
            log_info("No Lailaba-set User env vars to remove")
    
    # 3. Remove wrapper script
    log_info("Removing lailaba command...")
    ...
    #     (only when they still point into this Lailaba home's node dir, so we
    #     never clobber an existing nvm / user-managed Node).
    log_info("Removing Lailaba-managed node/npm/npx symlinks...")
    removed_node_links = remove_node_symlinks(lailaba_home)
    if removed_node_links:
        for link in removed_node_links:
            log_success(f"Removed {link}")
    else:
        log_info("No Lailaba-managed node/npm/npx symlinks found")
    ...
    #     step-5 rmtree(lailaba_home) would sweep the in-tree artifacts anyway,
    #     but the packaged app + Electron userData live OUTSIDE LAILABA_HOME and
    #     must be cleaned explicitly here.
    log_info("Removing desktop Chat GUI artifacts...")
    try:
        from lailaba_cli.gui_uninstall import uninstall_gui
        gui_removed = uninstall_gui(lailaba_home)
        if not gui_removed:
    ...
    try:
        if project_root.exists():
            # If the install is inside ~/.lailaba/, just remove the lailaba-agent subdir
            if lailaba_home in project_root.parents or project_root.parent == lailaba_home:
    ...
    #     under LAILABA_HOME but they're install tooling, not config — safe to
    #     remove even in "keep data" mode.  If we're doing a full uninstall
    #     the step-5 rmtree(lailaba_home) would sweep them anyway; calling
    ...
    if _is_windows():
        ...
        removed_artifacts = remove_portable_tooling_windows(lailaba_home)
    ...
    # 5. Optionally remove ~/.lailaba/ data directory (and named profiles)
    if full_uninstall:
        # 5a. Stop and remove each named profile's gateway service and
        #     alias wrapper. The profile LAILABA_HOME dirs live under
        ...
        try:
            if lailaba_home.exists():
                shutil.rmtree(lailaba_home)
                log_success(f"Removed {lailaba_home}")
        except Exception as e:
            log_warn(f"Could not fully remove {lailaba_home}: {e}")
            log_info("You may need to manually remove it")
    else:
        log_info(f"Keeping configuration and data in {lailaba_home}")
    ...
    if not full_uninstall:
        ...
        print(f"  {lailaba_home}/")
        print()
        print("To reinstall later with your existing settings:")
        if _is_windows():
            print(color("  iex (irm https://lailaba-agent.nousresearch.com/install.ps1)", Colors.DIM))
        else:
            print(color("  curl -fsSL https://lailaba-agent.nousresearch.com/install.sh | bash", Colors.DIM))
        print()
    ...
    print()
    print("Thank you for using Lailaba AI! ⚕")
    print()
...
def main(argv=None) -> int:
    """Module entrypoint: ``python -m lailaba_cli.uninstall --mode <gui|lite|full>``.
    ...
    so ``import lailaba_cli`` resolves from source while the venv is torn down.

    This module imports only stdlib + ``lailaba_constants`` + ``lailaba_cli.colors``
    (and lazily ``lailaba_cli.gui_uninstall``), so it runs fine under a bare
    system Python with no site-packages from the venv.
    """
    import argparse

    parser = argparse.ArgumentParser(prog="python -m lailaba_cli.uninstall")
    parser.add_argument(
        "--mode",
