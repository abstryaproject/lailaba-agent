"""
Lailaba AI — Web UI server.

Provides a FastAPI backend serving the Vite/React frontend and REST API
endpoints for managing configuration, environment variables, and sessions.

Usage:
    python -m lailaba_cli.main web          # Start on http://127.0.0.1:9119
    python -m lailaba_cli.main web --port 8080
"""
...
from lailaba_cli._subprocess_compat import windows_detach_flags, windows_hide_flags
...
from lailaba_cli.memory_providers import (
    MemoryProvider,
    ProviderField,
    get_memory_provider,
)
...
try:
    ...
    # running `lailaba dashboard` needs fastapi+uvicorn; lazy install keeps
...
WEB_DIST = Path(os.environ["LAILABA_WEB_DIST"]) if "LAILABA_WEB_DIST" in os.environ else Path(__file__).parent / "web_dist"
...
def _start_desktop_cron_ticker(stop_event: "threading.Event", interval: int = 60) -> None:
    ...
    The scheduler tick loop normally lives in ``lailaba gateway run`` — but the
    desktop app spawns a ``lailaba dashboard`` backend, not a gateway, so a cron
    ...
    real gateway on the same LAILABA_HOME — whichever process grabs the lock
    first wins the tick.
    """
...
def _warm_gateway_module() -> None:
    try:
        import lailaba_cli.gateway  # noqa: F401
    except Exception:
        pass
...
def _resolve_restart_drain_timeout() -> float:
    try:
        from lailaba_cli.gateway import _get_restart_drain_timeout
...
async def _lifespan(app: "FastAPI"):
    ...
    # Fire lailaba_cli.gateway import into a background thread so the event
    # loop is not blocked and LAILABA_DASHBOARD_READY fires without delay.
    ...
    # Desktop-spawned backends (LAILABA_DESKTOP=1) fire cron jobs themselves,
    # since the app has no gateway running the scheduler. Server `lailaba
    # dashboard` is unaffected — it relies on its own gateway.
    cron_stop: "threading.Event | None" = None
    cron_thread: "threading.Thread | None" = None
    if os.getenv("LAILABA_DESKTOP") == "1":
...
app = FastAPI(title="Lailaba AI", version=__version__, lifespan=_lifespan)

# Memory-provider OAuth connect routes live in the memory layer, not here.
from lailaba_cli.memory_oauth import router as _memory_oauth_router  # noqa: E402
...
# LAILABA_DASHBOARD_SESSION_TOKEN so its main process can authenticate the
...
# ---------------------------------------------------------------------------
_SESSION_TOKEN = os.environ.get("LAILABA_DASHBOARD_SESSION_TOKEN") or secrets.token_urlsafe(32)
_SESSION_HEADER_NAME = "X-Lailaba-Session-Token"
...
#
# This list is defined in ``lailaba_cli.dashboard_auth.public_paths`` so the
...
# ---------------------------------------------------------------------------
from lailaba_cli.dashboard_auth.public_paths import (
...
def _require_token(request: Request) -> None:
    ...
    * **Loopback / ``--insecure`` mode** (``auth_required`` False): the
      ...
      back via ``X-Lailaba-Session-Token`` (or the legacy ``Bearer`` header).
      Validate it here.
...
def should_require_auth(host: str, allow_public: bool = False) -> bool:
    ...
    unauthenticated-public-dashboard hole behind the June 2026 ``lailaba-0day``
...
async def _dashboard_auth_gate(request: Request, call_next):
    from lailaba_cli.dashboard_auth.middleware import gated_auth_middleware
...
async def _token_auth_seam(request: Request, call_next):
    ...
    through untouched.
    """
    from lailaba_cli.dashboard_auth.token_auth import token_auth_middleware
...
_SCHEMA_OVERRIDES: Dict[str, Dict[str, Any]] = {
    ...
    "updates.non_interactive_local_changes": {
        "type": "select",
        "description": (
            "When the chat app / gateway updates Lailaba (no terminal prompt), "
...
class ModelAssignment(BaseModel):
    ...
    # ``lailaba model`` custom flow collects. Honored only on the main slot for
...
def _normalize_main_model_assignment(provider: str, model: str) -> tuple[str, str]:
    ...
    - The "Change" picker sends a real Lailaba provider slug — fine.
    ...
    1. Vendor-name → Lailaba-provider mapping: when the provider string is
       not a known Lailaba provider/alias (e.g. ``moonshotai``, ``x-ai`` is
    ...
    """
    from lailaba_cli.models import _KNOWN_PROVIDER_NAMES, normalize_provider
    from lailaba_cli.model_normalize import normalize_model_for_provider
    ...
    if canonical not in _KNOWN_PROVIDER_NAMES and "/" in model_in:
        ...
        try:
            ...
            cur_provider = ""
        from lailaba_cli.models import _AGGREGATOR_PROVIDERS
...
_MANAGED_FILES_ROOT_ENV = "LAILABA_DASHBOARD_FILES_ROOT"
...
def _default_lailaba_root_is_opt_data() -> bool:
    raw = os.environ.get("LAILABA_HOME", "").strip()
    if not raw:
        return False
    try:
        from lailaba_constants import get_default_lailaba_root

        root = get_default_lailaba_root().expanduser().resolve(strict=False)
    except (OSError, RuntimeError):
...
def _dashboard_local_update_managed_externally() -> bool:
    """Return true when the dashboard should not offer ``lailaba update``.
    ...
    a container — e.g. the lailaba-webui image sharing the Lailaba source tree),
    the dashboard's ``lailaba update`` button is the correct update path and
    ...
    """
    if _default_lailaba_root_is_opt_data():
        return True
    try:
        from lailaba_constants import is_container
...
def _managed_files_policy(request: Request, *, create_root: bool = True) -> ManagedFilesPolicy:
    ...
    # to /opt/data only when the installation's Lailaba root is actually /opt/data
    # (the container/hosted layout) or when LAILABA_DASHBOARD_FILES_ROOT is set.
    if _default_lailaba_root_is_opt_data():
...
async def fs_write_text(payload: FsWriteText):
    ...
    Mirrors the local Electron ``lailaba:fs:writeText`` hardening: the path is
    ...
    tmp = target.with_name(f".{target.name}.lailaba-tmp-{os.getpid()}")
    try:
...
async def get_status(profile: Optional[str] = None):
    ...
    # across that await. Status only resolves get_lailaba_home() at call time
    ...
    try:
        ...
        # Windows install the first import of lailaba_cli.gateway blocks the
        ...
        # and which providers are registered so ``lailaba status`` and the
        # SPA's StatusPage can show "OAuth gate ON via Nous Research" or
        # "loopback only — no auth gate" with no extra round trips.
        auth_required = bool(getattr(app.state, "auth_required", False))
        auth_providers: list[str] = []
        try:
            from lailaba_cli.dashboard_auth import list_providers as _list_providers
        ...
        status = {
            ...
            "can_update_lailaba": not _dashboard_local_update_managed_externally(),
            "gateway_running": gateway_running,
            "gateway_state": gateway_state,
        ...
        if not auth_required:
            status.update({
                "lailaba_home": str(get_lailaba_home()),
...
async def get_system_stats():
    ...
    and non-sensitive (no env values, no paths beyond the lailaba home root).
    """
    ...
    try:
        ...
        try:
            du = psutil.disk_usage(str(get_lailaba_home()))
            info["disk"] = {
                "total": du.total,
                "used": du.used,
                "free": du.free,
...
async def run_curator():
    ...
    try:
        proc = _spawn_lailaba_action(["curator", "run"], "curator-run")
    except Exception as exc:
...
async def get_portal_status():
    ...
    auth: Dict[str, Any] = {}
    try:
        from lailaba_cli.auth import get_nous_auth_status
    ...
    features = []
    try:
        from lailaba_cli.nous_subscription import get_nous_subscription_features
...
async def run_prompt_size():
    try:
        proc = _spawn_lailaba_action(["prompt-size"], "prompt-size")
    except Exception as exc:
...
async def run_dump():
    try:
        proc = _spawn_lailaba_action(["dump"], "dump")
    except Exception as exc:
        raise HTTPException(status_code=500, detail=f"Failed: {exc}")
...
async def run_config_migrate():
    try:
        proc = _spawn_lailaba_action(["config", "migrate"], "config-migrate")
    except Exception as exc:
        raise HTTPException(status_code=500, detail=f"Failed: {exc}")
...
async def run_debug_share_endpoint(body: DebugShareRequest | None = None):
    ...
    *synchronous*: the whole point of ``debug share`` is the set of shareable
    ...
    """
    from lailaba_cli.debug import build_debug_share
...
# streamed to a per-action log file under ``~/.lailaba/logs/<action>.log`` so
# the dashboard can tail them back to the user.
# ---------------------------------------------------------------------------

_ACTION_LOG_DIR: Path = get_lailaba_home() / "logs"
...
_ACTION_LOG_FILES: Dict[str, str] = {
    ...
    "lailaba-update": "lailaba-update.log",
...
def _spawn_lailaba_action(subcommand: List[str], name: str) -> subprocess.Popen:
    """Spawn ``lailaba <subcommand>`` detached and record the Popen handle.

    Uses the running interpreter's ``lailaba_cli.main`` module so the action
    ...
    cmd = [_dashboard_spawn_executable(), "-m", "lailaba_cli.main", *subcommand]
    ...
    popen_kwargs: Dict[str, Any] = {
        ...
        "env": {**os.environ, "LAILABA_NONINTERACTIVE": "1"},
    }
    if sys.platform == "win32":
...
def _gateway_display_command(profile: Optional[str], verb: str) -> str:
    return " ".join(["lailaba", *_gateway_subcommand(profile, verb)])
...
def _spawn_gateway_restart(profile: Optional[str] = None) -> Tuple[subprocess.Popen, bool]:
    """Spawn ``lailaba gateway restart``, reusing an in-flight restart.
    ...
    concurrent ``lailaba gateway restart`` children race each other on the
    ...
    return _spawn_lailaba_action(subcommand, "gateway-restart"), False
...
async def restart_gateway(profile: Optional[str] = None):
    """Kick off a ``lailaba gateway restart`` in the background."""
    try:
...
async def gateway_drain(request: Request):
    ...
    active (no ``LAILABA_DASHBOARD_DRAIN_SECRET``), the route is NOT a token
...
@app.post("/api/lailaba/update")
async def update_lailaba():
    """Kick off ``lailaba update`` in the background."""
    if _dashboard_local_update_managed_externally():
        message = (
            "Lailaba updates are managed outside this dashboard in "
            "containerized environments. The built-in local updater is "
            "disabled here."
        )
        _record_completed_action("lailaba-update", message, exit_code=1)
        return {
            "ok": False,
            "pid": None,
            "name": "lailaba-update",
    ...
    try:
        proc = _spawn_lailaba_action(["update"], "lailaba-update")
    except Exception as exc:
        _log.exception("Failed to spawn lailaba update")
...
@app.get("/api/lailaba/update/check")
async def check_lailaba_update(force: bool = False):
    """Report whether a Lailaba update is available, without applying it.

    Powers the dashboard's "check before you update" flow: the System page
    shows the commit-behind count and asks the user to confirm before
    ``POST /api/lailaba/update`` actually runs ``lailaba update``.

    Returns:
        install_method: 'git' | 'pip' | 'docker' | 'nixos' | 'homebrew' | ...
        current_version: installed Lailaba version string
    ...
    try:
        ...
        if force:
            try:
                (get_lailaba_home() / ".update_check").unlink()
            except OSError:
                pass
...
async def speak_text(payload: TTSSpeakRequest):
    ...
    configured in ``~/.lailaba/config.yaml`` under ``tts.``.
    """
...
async def get_profiles_sessions(
    ...
    from lailaba_state import SessionDB
    from lailaba_cli import profiles as profiles_mod
...
async def search_sessions(q: str = "", limit: int = 20, profile: Optional[str] = None):
    ...
    try:
        ...
        try:
            ...
            # logs, or another Lailaba surface. FTS can't find those unless the
...
def _normalize_config_for_web(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    Lailaba supports ``model`` as either a bare string (``"anthropic/claude-sonnet-4"``)
...
def _memory_provider_config_path(provider: MemoryProvider) -> Path:
    return get_lailaba_home() / provider.name / "config.json"
...
# in lailaba_cli/config.py — listed here for deterministic ordering in the UI.
...
def get_model_options(profile: Optional[str] = None, refresh: bool = False):
    ...
    """
    try:
        from lailaba_cli.inventory import build_models_payload, load_picker_context
        ...
        # onboarding picker) sees the SAME full provider universe `lailaba model`
...
def get_recommended_default_model(provider: str = ""):
    ...
    Mirrors the model-curation `lailaba model` does so GUI onboarding lands on a
    ...
    if slug == "nous":
        try:
            ...
            from lailaba_cli.auth import get_provider_auth_state
...
def get_moa_models(profile: Optional[str] = None):
    ...
    try:
        from lailaba_cli.moa_config import normalize_moa_config
...
async def set_model_assignment(body: ModelAssignment, profile: Optional[str] = None):
    ...
    Writes to ``~/.lailaba/config.yaml`` — applies to **new** sessions only.
    ...
    try:
        ...
        if model and not body.confirm_expensive_model:
            try:
                from lailaba_cli.model_cost_guard import expensive_model_warning
...
def _apply_model_assignment_sync(
    ...
    if scope == "main":
        ...
        # post-model-selection behaviour (lailaba_cli/main.py
        ...
        if provider.strip().lower() == "nous":
            try:
                from lailaba_cli.nous_subscription import apply_nous_managed_defaults
                from lailaba_cli.tools_config import _get_platform_tools
        ...
        # endpoint, mirroring the ``lailaba model`` custom flow
        ...
        if provider.strip().lower() in {"custom", "local"} and base_url:
            try:
                from lailaba_cli.main import _auto_provider_name, _save_custom_provider
...
def _catalog_provider_env_metadata() -> dict:
    ...
    (i.e. the ``lailaba model`` universe). This is what lets the desktop Keys tab
    ...
    """
    try:
        from lailaba_cli.provider_catalog import provider_catalog
    except Exception:
        return {}
    ...
    try:
        from lailaba_cli.config import OPTIONAL_ENV_VARS as _OPT
    except Exception:
        _OPT = {}
    _non_provider_keys = {
    ...
    for d in provider_catalog():
        ...
        # Keys tab (otherwise Bedrock — a `lailaba model` provider — would be
...
async def get_env_vars(profile: Optional[str] = None):
    ...
    def _row(var_name: str, info: dict) -> dict:
        ...
        return {
            ...
            # CLI `lailaba model` picker uses (not desktop-only prefix guesses).
...
_PLATFORM_OVERRIDES: dict[str, dict[str, Any]] = {
    "telegram": {
        "name": "Telegram",
        "description": "Run Lailaba from Telegram DMs, groups, and topics.",
    ...
    "discord": {
        "name": "Discord",
        "description": "Connect Lailaba to Discord DMs, channels, and threads.",
    ...
    "slack": {
        "name": "Slack",
        "description": "Use Lailaba from Slack via Socket Mode. Add allowed Slack member IDs so connected bots can respond.",
    ...
    "mattermost": {
        "name": "Mattermost",
        "description": "Connect Lailaba to Mattermost channels and direct messages.",
    ...
    "matrix": {
        "name": "Matrix",
        "description": "Use Lailaba in Matrix rooms and direct messages.",
    ...
    "whatsapp": {
        "name": "WhatsApp",
        "description": "Use Lailaba through the bundled WhatsApp bridge with QR-based auth.",
    ...
    "homeassistant": {
        "name": "Home Assistant",
        "description": "Control your smart home from Lailaba via Home Assistant.",
    ...
    "email": {
        "name": "Email",
        "description": "Talk to Lailaba through an IMAP/SMTP mailbox.",
        "docs_url": "https://lailaba-agent.nousresearch.com/docs/user-guide/messaging/",
        "env_vars": (
            "EMAIL_ADDRESS",
            "EMAIL_PASSWORD",
            "EMAIL_IMAP_HOST",
    ...
    "dingtalk": {
        "name": "DingTalk",
        "description": "Connect Lailaba to DingTalk groups (钉钉).",
    ...
    "feishu": {
        "name": "Feishu / Lark",
        "description": "Use Lailaba inside Feishu / Lark.",
    ...
    "google_chat": {
        "name": "Google Chat",
        "description": "Connect Lailaba to Google Chat via Cloud Pub/Sub.",
        "docs_url": "https://lailaba-agent.nousresearch.com/docs/user-guide/messaging/google_chat",
    },
    "wecom": {
    ...
    "weixin": {
        ...
        "docs_url": "https://lailaba-agent.nousresearch.com/docs/user-guide/messaging/weixin/",
        "env_vars": ("WEIXIN_ACCOUNT_ID", "WEIXIN_TOKEN", "WEIXIN_BASE_URL"),
        "required_env": ("WEIXIN_ACCOUNT_ID", "WEIXIN_TOKEN"),
    },
    "bluebubbles": {
        "name": "BlueBubbles (iMessage)",
        "description": "Use Lailaba through iMessage via a BlueBubbles server.",
    ...
    "qqbot": {
        "name": "QQ Bot",
        "description": "Connect Lailaba to a QQ Bot from the QQ Open Platform.",
    ...
    "teams": {
        "docs_url": "https://lailaba-agent.nousresearch.com/docs/user-guide/messaging/teams",
    },
    "yuanbao": {
        "name": "Yuanbao (元宝)",
        "description": "Connect Lailaba to Tencent Yuanbao.",
        "docs_url": "",
        "required_env": (),
    },
    "api_server": {
        "name": "API server",
        "description": "Expose Lailaba as an OpenAI-compatible HTTP API for tools like Open WebUI.",
        "docs_url": "https://lailaba-agent.nousresearch.com/docs/user-guide/messaging/",
        "env_vars": (
            "API_SERVER_ENABLED",
            "API_SERVER_KEY",
    ...
    "webhook": {
        ...
        "docs_url": "https://lailaba-agent.nousresearch.com/docs/user-guide/messaging/webhooks/",
...
_MESSAGING_ENV_FALLBACKS: dict[str, dict[str, Any]] = {
    ...
    "WEIXIN_ACCOUNT_ID": {
        "description": "iLink Bot account ID obtained through QR login in lailaba gateway setup",
        "prompt": "iLink Bot account ID",
    },
    "WEIXIN_TOKEN": {
        "description": "iLink Bot token obtained through QR login in lailaba gateway setup",
...
_TELEGRAM_ONBOARDING_DEFAULT_URL = "https://setup.lailaba-agent.nousresearch.com"
...
async def start_telegram_onboarding(body: TelegramOnboardingStart):
    bot_name = (body.bot_name or "Lailaba AI").strip() or "Lailaba AI"
...
def _restart_gateway_after_telegram_onboarding(profile: Optional[str] = None) -> dict[str, Any]:
    ...
    saved token waits on a separate dashboard restart click, Lailaba appears
...
async def get_messaging_platforms(profile: Optional[str] = None):
    ...
    # all resolve against the requested profile's LAILABA_HOME.
...
# the canonical ``lailaba auth add <provider>`` command so the dashboard
...
def _anthropic_oauth_status() -> Dict[str, Any]:
    ...
    1. ``~/.lailaba/.anthropic_oauth.json`` — Lailaba-managed PKCE flow (what
    ...
    2. ``ANTHROPIC_API_KEY`` → ``ANTHROPIC_TOKEN`` → ``CLAUDE_CODE_OAUTH_TOKEN``
       ...
       env during ``load_lailaba_dotenv()``, so the same check covers them)
    ...
    try:
        from agent.anthropic_adapter import (
            read_lailaba_oauth_credentials,
            _LAILABA_OAUTH_FILE,
        )
    except ImportError:
        read_lailaba_oauth_credentials = None  # type: ignore
        _LAILABA_OAUTH_FILE = None  # type: ignore

    lailaba_creds = None
    if read_lailaba_oauth_credentials:
        try:
            lailaba_creds = read_lailaba_oauth_credentials()
        except Exception:
            lailaba_creds = None
    if lailaba_creds and lailaba_creds.get("accessToken"):
        return {
            "logged_in": True,
            "source": "lailaba_pkce",
            "source_label": f"Lailaba PKCE ({_LAILABA_OAUTH_FILE})",
            "token_preview": _truncate_token(lailaba_creds.get("accessToken")),
            "expires_at": lailaba_creds.get("expiresAt"),
            "has_refresh_token": bool(lailaba_creds.get("refreshToken")),
        }
    ...
    try:
        ...
        pass
    try:
        from lailaba_cli.config import get_env_value
    except ImportError:
...
def _claude_code_only_status() -> Dict[str, Any]:
    ...
    Claude Code subscription tokens are actively flowing into Lailaba even
    when they also have a separate Lailaba-managed PKCE login.
    """
    try:
...
def _copilot_acp_status() -> Dict[str, Any]:
    ...
    Lailaba never claims a login state it can't verify.
    """
    return {
        "logged_in": False,
        "source": "copilot_cli",
...
_OAUTH_PROVIDER_CATALOG: tuple[Dict[str, Any], ...] = (
    {
        ...
        "name": "Nous Portal",
        "flow": "device_code",
        "cli_command": "lailaba auth add nous",
    ...
    },
    {
        ...
        "flow": "device_code",
        "cli_command": "lailaba auth add openai-codex",
    ...
    },
    {
        ...
        "flow": "external",
        "cli_command": "lailaba auth add qwen-oauth",
    ...
    },
    {
        ...
        "flow": "device_code",
        "cli_command": "lailaba auth add minimax-oauth",
    ...
    },
    {
        ...
        "flow": "loopback",
        "cli_command": "lailaba auth add xai-oauth",
        "docs_url": "https://lailaba-agent.nousresearch.com/docs/guides/xai-grok-oauth",
    ...
    # usage credits on top of a Claude Max plan — see disclaimer in name).
    {
        ...
        "flow": "pkce",
        "cli_command": "lailaba auth add anthropic",
...
def _resolve_provider_status(provider_id: str, status_fn) -> Dict[str, Any]:
    ...
    try:
        from lailaba_cli import auth as hauth
        if provider_id == "nous":
...
def _oauth_provider_disconnect_command(provider: Dict[str, Any]) -> Optional[str]:
    ...
    External providers store their credentials outside Lailaba, so the disconnect
    ...
    user sees exactly what executes, and Lailaba then stops resolving the token.
...
def _oauth_provider_disconnect_hint(provider: Dict[str, Any], status: Dict[str, Any]) -> Optional[str]:
    ...
    if provider.get("flow") == "external":
        if _oauth_provider_disconnect_command(provider):
            ...
            return "Managed outside Lailaba — run the disconnect command to remove it."
...
def _build_oauth_catalog() -> list[Dict[str, Any]]:
    ...
    MEMBERSHIP is the union of:
      ...
      2. every accounts-tab provider in the unified ``provider_catalog()`` (the
         ``lailaba model`` universe) — so any OAuth/external provider added as a
    ...
    #    in lockstep with the `lailaba model` universe (zero-edit for new plugins).
    try:
        from lailaba_cli.provider_catalog import provider_catalog
        for d in provider_catalog():
            ...
            rows.append({
                ...
                "name": d.label,
                "flow": "external",
                "cli_command": f"lailaba auth add {d.slug}",
...
async def list_oauth_providers(profile: Optional[str] = None):
    ...
    Response shape (per provider):
        ...
        status:
          ...
          source           short slug ("lailaba_pkce", "claude_code", ...)
    ...
    sync with the `lailaba model` picker; _OAUTH_OVERRIDES supplies per-provider
...
async def disconnect_oauth_provider(
    ...
    with _profile_scope(profile):
        ...
        # Anthropic clears only the Lailaba-managed PKCE file and auth-store entry.
        ...
        if provider_id == "anthropic":
            cleared = False
            try:
                from agent.anthropic_adapter import _LAILABA_OAUTH_FILE
                if _LAILABA_OAUTH_FILE.exists():
                    _LAILABA_OAUTH_FILE.unlink()
                    cleared = True
            except Exception:
                pass
            # Also clear the credential pool entry if present.
            try:
                from lailaba_cli.auth import clear_provider_auth
        ...
        try:
            from lailaba_cli.auth import clear_provider_auth, invalidate_nous_auth_status_cache
...
#          → persists to ~/.lailaba/.anthropic_oauth.json AND credential pool
...
# Guarded so lailaba web still starts if anthropic_adapter is unavailable;
...
def _save_anthropic_oauth_creds(access_token: str, refresh_token: str, expires_at_ms: int) -> None:
    """Persist Anthropic PKCE creds to both Lailaba file AND credential pool.

    Mirrors what auth_commands.add_command does so the dashboard flow leaves
    the system in the same state as ``lailaba auth add anthropic``.
    """
    from agent.anthropic_adapter import _LAILABA_OAUTH_FILE
    payload = {
    ...
    _LAILABA_OAUTH_FILE.parent.mkdir(parents=True, exist_ok=True)
    tmp_path = _LAILABA_OAUTH_FILE.with_name(
        f"{_LAILABA_OAUTH_FILE.name}.tmp.{os.getpid()}.{secrets.token_hex(8)}"
    )
    try:
        ...
        os.replace(tmp_path, _LAILABA_OAUTH_FILE)
        try:
            _LAILABA_OAUTH_FILE.chmod(stat.S_IRUSR | stat.S_IWUSR)
        except OSError:
            pass
    finally:
        try:
    ...
    for _endpoint in _ANTHROPIC_OAUTH_TOKEN_URLS:
        req = urllib.request.Request(
            ...
            headers={
                ...
                "User-Agent": "lailaba-dashboard/1.0",
            },
            method="POST",
        )
        try:
...
async def _start_device_code_flow(
    ...
    if provider_id == "nous":
        ...
        portal_base_url = (
            os.getenv("LAILABA_PORTAL_BASE_URL")
...
# the tokens exactly like `lailaba auth add xai-oauth`.
...
def _xai_loopback_worker(session_id: str) -> None:
    ...
    try:
        ...
        base_url = hauth._xai_validate_inference_base_url(
            os.getenv("LAILABA_XAI_BASE_URL", "").strip().rstrip("/")
...
def _add_xai_oauth_pool_entry(
    ...
    """Mirror `lailaba auth add xai-oauth`'s credential-pool insert.
...
def _nous_poller(session_id: str) -> None:
    ...
    try:
        ...
        with _profile_scope(_oauth_session_profile(session_id)):
            ...
            from lailaba_cli.auth import persist_nous_credentials
...
def _minimax_poller(session_id: str) -> None:
    ...
    ``lailaba auth add minimax-oauth``.
    """
    from lailaba_cli.auth import (
        _minimax_poll_token,
...
def _codex_full_login_worker(session_id: str) -> None:
    ...
    try:
        ...
        from lailaba_cli.auth import _save_codex_tokens
...
async def get_session_stats(profile: Optional[str] = None):
    """Session-store statistics for the Sessions page (mirrors `lailaba sessions stats`).
...
async def prune_sessions_endpoint(body: SessionPrune):
    """Delete ended sessions older than N days (mirrors `lailaba sessions prune`)."""
    if body.older_than_days < 1:
        raise HTTPException(status_code=400, detail="older_than_days must be >= 1")
    profile_home = _cron_profile_home(body.profile)[1] if body.profile else get_lailaba_home()
    db = _open_session_db_for_profile(body.profile)
    try:
...
async def get_logs(
    ...
    from lailaba_cli.logs import _read_tail, LOG_FILES

    log_name = LOG_FILES.get(file)
    if not log_name:
        raise HTTPException(status_code=400, detail=f"Unknown log file: {file}")
    log_path = get_lailaba_home() / "logs" / log_name
    if not log_path.exists():
        return {"file": file, "lines": []}

    try:
        from lailaba_logging import COMPONENT_PREFIXES
    except ImportError:
        COMPONENT_PREFIXES = {}
...
def _cron_profile_home(profile: Optional[str]) -> Tuple[str, Path]:
    """Resolve a profile query value to (profile_name, LAILABA_HOME)."""
    from lailaba_cli import profiles as profiles_mod
...
def _annotate_cron_job(job: Dict[str, Any], profile: str, home: Path) -> Dict[str, Any]:
    ...
    annotated["lailaba_home"] = str(home)
...
def _call_cron_for_profile(target_profile: Optional[str], func_name: str, *args, **kwargs):
    ...
    from the process LAILABA_HOME at import time. The dashboard is a single
    ...
    with _CRON_PROFILE_LOCK:
        ...
        from lailaba_constants import (
            reset_lailaba_home_override,
            set_lailaba_home_override,
        )

        old_cron_dir = cron_jobs.CRON_DIR
        old_jobs_file = cron_jobs.JOBS_FILE
        old_output_dir = cron_jobs.OUTPUT_DIR
        token = set_lailaba_home_override(str(home))
        ...
        try:
            ...
            reset_lailaba_home_override(token)
...
#
# Wraps the same config data layer the CLI uses (lailaba_cli.mcp_config), so
# servers managed here show up under `lailaba mcp list` and vice versa.  Secrets
...
async def list_mcp_servers(profile: Optional[str] = None):
    from lailaba_cli.mcp_config import _get_mcp_servers
...
async def add_mcp_server(body: MCPServerCreate, profile: Optional[str] = None):
    from lailaba_cli.mcp_config import _get_mcp_servers, _save_mcp_server
...
async def remove_mcp_server(name: str, profile: Optional[str] = None):
    from lailaba_cli.mcp_config import _remove_mcp_server
...
async def test_mcp_server(name: str, profile: Optional[str] = None):
    ...
    from lailaba_cli.mcp_config import _get_mcp_servers, _probe_single_server
    ...
    def _probe_scoped():
        ...
        # LAILABA_HOME override (see mcp_tool._wrap_with_home_override), so
...
async def install_mcp_catalog_entry(body: MCPCatalogInstall, profile: Optional[str] = None):
    ...
    # The -p subprocess rebinds LAILABA_HOME-derived paths in the child.
    if entry.install is not None:
        try:
            proc = _spawn_lailaba_action(
...
#
# Wraps the same JSON store the CLI uses (lailaba_cli.webhook); the webhook
...
async def list_webhooks():
    import lailaba_cli.webhook as wh
...
# spawn the real `lailaba gateway <verb>` so behaviour matches the CLI exactly.
...
async def start_gateway(profile: Optional[str] = None):
    try:
        proc = _spawn_lailaba_action(_gateway_subcommand(profile, "start"), "gateway-start")
    except HTTPException:
        raise
    except Exception as exc:
...
async def stop_gateway(profile: Optional[str] = None):
    try:
        proc = _spawn_lailaba_action(_gateway_subcommand(profile, "stop"), "gateway-stop")
    except HTTPException:
        raise
    except Exception as exc:
...
async def list_credential_pool():
    ...
    from lailaba_cli.auth import read_credential_pool
...
# `lailaba memory setup`).  The dashboard covers the common admin actions:
...
async def set_memory_provider(body: MemoryProviderSelect):
    ...
    if provider:
        ...
        if provider not in valid:
            raise HTTPException(
                status_code=400,
                detail=f"Unknown memory provider '{provider}'. Run `lailaba memory setup` to configure a new one.",
            )
...
async def run_doctor():
    try:
        proc = _spawn_lailaba_action(["doctor"], "doctor")
    except Exception as exc:
...
async def run_security_audit():
    try:
        proc = _spawn_lailaba_action(["security", "audit"], "security-audit")
    except Exception as exc:
...
async def run_backup(body: BackupRequest):
    ...
    try:
        proc = _spawn_lailaba_action(args, "backup")
    except Exception as exc:
...
class ImportRequest(BaseModel):
    archive: str
    # Pass --force to `lailaba import`. The spawned action runs with
...
async def run_import(body: ImportRequest):
    ...
    if body.force:
        args.append("--force")
    try:
        proc = _spawn_lailaba_action(args, "import")
    except Exception as exc:
...
async def list_hooks():
    ...
    form can offer them.
    """
    from lailaba_cli.config import load_config as _load_config
    from agent import shell_hooks

    try:
        from lailaba_cli.plugins import VALID_HOOKS
...
async def list_checkpoints():
    ...
    # Checkpoints live under <lailaba_home>/checkpoints/.  Surface a count +
    ...
    cp_dir = get_lailaba_home() / "checkpoints"
    sessions = []
    total_bytes = 0
    if cp_dir.is_dir():
...
async def prune_checkpoints():
    try:
        proc = _spawn_lailaba_action(["checkpoints", "prune"], "checkpoints-prune")
    except Exception as exc:
...
def _profile_cli_args(profile: Optional[str]) -> List[str]:
    ...
    Hub install/uninstall/update run in a fresh ``lailaba`` subprocess, and
...
async def preview_skill_hub(identifier: str = ""):
    ...
    def _run():
        from lailaba_cli.skills_hub import _resolve_source_meta_and_bundle
        from tools.skills_hub import create_source_router
...
class ProfileCreate(BaseModel):
    ...
    # via a subprocess scoped to the profile (`lailaba -p <name> skills install`)
    # because skills_hub.SKILLS_DIR is import-time-bound and the LAILABA_HOME
...
def _fallback_profile_dicts(profiles_mod) -> List[Dict[str, Any]]:
    ...
    default_home = profiles_mod._get_default_lailaba_home()
...
def _profile_setup_command(name: str) -> str:
    ...
    return "lailaba setup" if name == "default" else f"{name} setup"
...
def _write_profile_model(profile_dir: Path, provider: str, model: str) -> None:
    ...
    context-local LAILABA_HOME override so the write lands in the target
    ...
    """
    from lailaba_constants import set_lailaba_home_override, reset_lailaba_home_override

    token = set_lailaba_home_override(str(profile_dir))
    try:
        provider, model = _normalize_main_model_assignment(provider, model)
        cfg = load_config()
...
def _write_profile_mcp_servers(profile_dir: Path, servers: List["MCPServerCreate"]) -> int:
    ...
    Scopes ``load_config``/``save_config`` to ``profile_dir`` via the
    context-local LAILABA_HOME override (same mechanism as
    ...
    """
    from lailaba_constants import set_lailaba_home_override, reset_lailaba_home_override
    from lailaba_cli.mcp_security import validate_mcp_server_entry
...
def _disable_unselected_skills(profile_dir: Path, keep: List[str]) -> int:
    ...
    install.) Scoped to the profile via the LAILABA_HOME override. Returns the
    number of skills newly disabled.
    """
    from lailaba_constants import set_lailaba_home_override, reset_lailaba_home_override
    from lailaba_cli.skills_config import get_disabled_skills, save_disabled_skills
...
async def create_profile_endpoint(body: ProfileCreate):
    ...
    # profile's LAILABA_HOME at import). Returns PIDs for the UI to poll.
...
async def get_active_profile_endpoint():
    ...
    ``active`` is the sticky default written by ``lailaba profile use`` —
    the profile new CLI invocations pick up. ``current`` is the profile
    the running dashboard/gateway is scoped to (derived from LAILABA_HOME).
    """
    from lailaba_cli import profiles as profiles_mod
    try:
...
async def set_active_profile_endpoint(body: ProfileActiveUpdate):
    """Set the sticky active profile (mirrors ``lailaba profile use``).
...
async def update_profile_model_endpoint(name: str, body: ProfileModelUpdate):
    ...
    to the named profile via the LAILABA_HOME override.
    """
    profile_dir = _resolve_profile_dir(name)
    provider = (body.provider or "").strip()
    model = (body.model or "").strip()
...
async def describe_profile_auto_endpoint(name: str, body: ProfileDescribeAuto):
    ...
    (``auxiliary.profile_describer``). Mirrors ``lailaba profile describe
    <name> --auto``.
    ...
    _resolve_profile_dir(name)
    try:
        from lailaba_cli import profile_describer
...
def _profile_scope(profile: Optional[str]):
    ...
    1. ``load_config``/``save_config`` resolve ``get_lailaba_home()`` at call
       time — the context-local override from ``set_lailaba_home_override``
    ...
    retargeted to the *current* ``get_lailaba_home()`` so writes land in the
    live home even when the import-time binding is stale (e.g. the process
    imported the modules before a LAILABA_HOME override, or under test
    isolation).
    """
    requested = (profile or "").strip()
...
def _config_profile_scope(profile: Optional[str]):
    ...
    ``set_lailaba_home_override`` contextvar — it does NOT swap the
    ...
    which is all endpoints that resolve ``get_lailaba_home()`` at call time
...
async def get_skills(profile: Optional[str] = None):
    ...
    from lailaba_cli.skills_config import get_disabled_skills
    with _profile_scope(profile):
        config = load_config()
...
async def get_toolsets(profile: Optional[str] = None):
    from lailaba_cli.tools_config import (
...
async def toggle_toolset(name: str, body: ToolsetToggle, profile: Optional[str] = None):
    ...
    helper the CLI ``lailaba tools`` picker uses, so the GUI and CLI stay in
...
async def get_toolset_config(name: str, profile: Optional[str] = None):
    ...
    Surfaces the same provider rows the CLI ``lailaba tools`` picker shows
...
async def select_toolset_provider(
    ...
    core extracted from the CLI configurator — so the GUI and ``lailaba tools``
...
async def save_toolset_env(name: str, body: ToolsetEnvUpdate, profile: Optional[str] = None):
    ...
    Writes each ``key: value`` to ``~/.lailaba/.env`` via ``save_env_value`` —
    the same store ``lailaba tools`` writes when it prompts for keys. Keys are
...
async def run_toolset_post_setup(
    ...
    ``lailaba tools post-setup <key>`` and the frontend tails the log via
    ...
    ``profile`` spawns the hook as ``lailaba -p <profile> tools post-setup``.
    ...
    write per-profile state must see the same LAILABA_HOME the rest of the
...
# (which attach to cua-driver's OWN identity, com.trycua.driver — not Lailaba,
...
async def grant_computer_use_permissions(profile: Optional[str] = None):
    """Spawn ``lailaba computer-use permissions grant`` as a background action.
...
#
# The endpoint spawns the same ``lailaba --tui`` binary the CLI uses, behind
...
if sys.platform.startswith("win"):
    try:
        from lailaba_cli.win_pty_bridge import WinPtyBridge as PtyBridge, PtyUnavailableError
    ...
    try:
        from lailaba_cli.pty_bridge import PtyBridge, PtyUnavailableError
        _PTY_BRIDGE_AVAILABLE = True
...
def _ws_auth_reason(ws: "WebSocket") -> tuple[Optional[str], str]:
    ...
    * ``?internal=<process-credential>`` — the process-lifetime internal
    ...
    if auth_required:
        ...
        from lailaba_cli.dashboard_auth.audit import AuditEvent, audit_log
        from lailaba_cli.dashboard_auth.ws_tickets import (
            TicketInvalid,
...
def _resolve_chat_argv(
    ...
    Default: whatever ``lailaba --tui`` would run.  Tests monkeypatch this
    function to inject a tiny fake command (``cat``, ``sh -c 'printf …'``)
    so nothing has to build Node or the TUI bundle.

    Session resume is propagated via the ``LAILABA_TUI_RESUME`` env var —
    matching what ``lailaba_cli.main._launch_tui`` does for the CLI path.
    Appending ``--resume <id>`` to argv doesn't work because ``ui-tui`` does
    not parse its argv.

    ``LAILABA_TUI_GATEWAY_URL`` is injected so the PTY child can attach to
    this process's in-memory ``tui_gateway`` instance instead of spawning
    its own Python gateway subprocess.

    `sidecar_url` (when set) is forwarded as ``LAILABA_TUI_SIDECAR_URL`` so
    the spawned ``tui_gateway.entry`` can mirror dispatcher emits to the
    dashboard's ``/api/pub`` endpoint (see :func:`pub_ws`).

    `active_session_file` (when set) is forwarded as
    ``LAILABA_TUI_ACTIVE_SESSION_FILE``. The TUI writes the current session id
    ...
    ``LAILABA_HOME`` at the profile dir in the child env. Every spawned
    process (the TUI and the ``tui_gateway.entry`` it launches) resolves
    ``get_lailaba_home()`` from that env var at its own import, so the child
    binds the profile's config, skills, memory, and state.db from the start
    — the same propagation ``lailaba -p <name>`` performs. The in-process
    ``LAILABA_TUI_GATEWAY_URL`` attach is SKIPPED for scoped chats: the
    ...
    """
    from lailaba_cli.main import PROJECT_ROOT, _make_tui_argv
    ...
    env.setdefault("LAILABA_TUI_DISABLE_MOUSE", "1")
    env.setdefault("LAILABA_TUI_INLINE", "1")
    env["LAILABA_TUI_DASHBOARD"] = "1"

    if profile_dir is not None:
        env["LAILABA_HOME"] = str(profile_dir)

    if resume:
        latest_resume, _latest_path = _session_latest_descendant(resume)
        if latest_resume:
            resume = latest_resume
        env["LAILABA_TUI_RESUME"] = resume

    if sidecar_url:
        env["LAILABA_TUI_SIDECAR_URL"] = sidecar_url
    ...
    # inherits the profile LAILABA_HOME set above.
    if profile_dir is None:
        if gateway_ws_url := _build_gateway_ws_url():
            env["LAILABA_TUI_GATEWAY_URL"] = gateway_ws_url
...
def _build_gateway_ws_url() -> Optional[str]:
    ...
    if getattr(app.state, "auth_required", False):
        from lailaba_cli.dashboard_auth.ws_tickets import internal_ws_credential
...
def _active_session_file_for_channel(app: "FastAPI", channel: str) -> Path:
    ...
    fd, raw_path = tempfile.mkstemp(prefix="lailaba-pty-active-", suffix=".json")
    os.close(fd)
    path = Path(raw_path)
    files[channel] = path
    return path
...
async def pty_ws(ws: WebSocket) -> None:
    ...
    if not _PTY_BRIDGE_AVAILABLE:
        await ws.send_text(
            ...
            "\x1b[33mInstall Lailaba inside WSL2 to use the dashboard's /chat "
...
# LAILABA_TUI_SIDECAR_URL set in /api/pty's PTY env) and writes every
...
def _normalise_prefix(raw: Optional[str]) -> str:
    ...
    Thin re-export of :func:`lailaba_cli.dashboard_auth.prefix.normalise_prefix`
    ...
    """
    from lailaba_cli.dashboard_auth.prefix import normalise_prefix
...
def mount_spa(application: FastAPI):
    ...
    ``mission-control.tilos.com/lailaba/*`` -> local Caddy -> :9119), the
    proxy injects ``X-Forwarded-Prefix: /lailaba`` on every request. We
    rewrite the served ``index.html`` so absolute asset URLs (``/assets/...``)
    and the SPA's runtime ``__LAILABA_BASE_PATH__`` honour that prefix
    ...
    def _serve_index(prefix: str = ""):
        ...
        ``prefix`` is the normalised ``X-Forwarded-Prefix`` (e.g. ``/lailaba``)
        ...
        ``__LAILABA_AUTH_REQUIRED__`` flag lets the SPA pick the right
        ...
        if gated:
            bootstrap_script = (
                f"<script>"
                f"window.__LAILABA_DASHBOARD_EMBEDDED_CHAT__={chat_js};"
                f'window.__LAILABA_BASE_PATH__="{prefix}";'
                f"window.__LAILABA_AUTH_REQUIRED__={gated_js};"
                f"</script>"
            )
        else:
            bootstrap_script = (
                f'<script>window.__LAILABA_SESSION_TOKEN__="{_SESSION_TOKEN}";'
                f"window.__LAILABA_DASHBOARD_EMBEDDED_CHAT__={chat_js};"
                f'window.__LAILABA_BASE_PATH__="{prefix}";'
                f"window.__LAILABA_AUTH_REQUIRED__={gated_js};"
                f"</script>"
    ...
    # under ``/lailaba`` they'd hit ``mission-control.tilos.com/fonts/...``
    # (the MC Pages app), not the Lailaba backend. Intercept CSS asset
...
_BUILTIN_DASHBOARD_THEMES = [
    {"name": "default",       "label": "Lailaba Teal",         "description": "Classic dark teal — the canonical Lailaba look"},
    {"name": "default-large", "label": "Lailaba Teal (Large)", "description": "Lailaba Teal with bigger fonts and roomier spacing"},
...
def _normalise_theme_definition(data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    # YAML in ~/.lailaba/, same trust level as the config file itself.
...
def _discover_user_themes() -> list:
    """Scan ~/.lailaba/dashboard-themes/*.yaml for user-created themes.

    Returns a list of fully-normalised theme definitions ready to ship
    to the frontend, so the client can apply them without a secondary
    round-trip or a built-in stub.
    """
    themes_dir = get_lailaba_home() / "dashboard-themes"
...
async def get_dashboard_themes():
    ...
    from `~/.lailaba/dashboard-themes/*.yaml` ship with their full
...
def _discover_dashboard_plugins() -> list:
    ...
    Checks three plugin sources (same as lailaba_cli.plugins):
    1. User plugins:    ~/.lailaba/plugins/<name>/dashboard/manifest.json
    2. Bundled plugins: <repo>/plugins/<name>/dashboard/manifest.json  (memory/, etc.)
    3. Project plugins: ./.lailaba/plugins/  (only if LAILABA_ENABLE_PROJECT_PLUGINS)
    """
    plugins = []
    seen_names: set = set()

    from lailaba_cli.plugins import get_bundled_plugins_dir
    bundled_root = get_bundled_plugins_dir()
    search_dirs = [
        (get_lailaba_home() / "plugins", "user"),
    ...
    # ``lailaba_cli/plugins.py`` and the documented user contract.
    if env_var_enabled("LAILABA_ENABLE_PROJECT_PLUGINS"):
        search_dirs.append((Path.cwd() / ".lailaba" / "plugins", "project"))
...
def _merged_plugins_hub() -> Dict[str, Any]:
    ...
    from lailaba_cli.plugins_cmd import (
    ...
    plugins_root_resolved = (get_lailaba_home() / "plugins").resolve()
    rows: List[Dict[str, Any]] = []
            ...
            try:
                ...
                for tname in provides_tools:
                    ...
                    if entry and entry.check_fn and not entry.check_fn():
                        auth_required = True
                        auth_command = f"lailaba auth {name}"
                        break
            except Exception:
                pass
...
async def post_agent_plugin_install(request: Request, body: _AgentPluginInstallBody):
    _require_token(request)
    from lailaba_cli.plugins_cmd import dashboard_install_plugin
...
async def post_agent_plugin_enable(request: Request, name: str):
    ...
    from lailaba_cli.plugins_cmd import dashboard_set_agent_plugin_enabled
...
async def post_agent_plugin_update(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from lailaba_cli.plugins_cmd import dashboard_update_user_plugin
...
async def delete_agent_plugin(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from lailaba_cli.plugins_cmd import dashboard_remove_user_plugin
...
def _mount_plugin_api_routes():
    ...
    Project plugins (``./.lailaba/plugins/``) ship with the CWD and are
    ...
    for plugin in _get_dashboard_plugins():
        ...
        if plugin.get("source") == "project":
            _log.warning(
                ...
                "~/.lailaba/plugins/ if you trust it)",
        ...
        if not api_path.exists():
            ...
            continue
        try:
            module_name = f"lailaba_dashboard_plugin_{plugin['name']}"
...
from lailaba_cli.dashboard_auth.routes import router as _dashboard_auth_router  # noqa: E402
...
def _write_dashboard_ready_file(actual_port: int) -> None:
    ...
    so Electron passes ``LAILABA_DESKTOP_READY_FILE`` and waits for this JSON.
    Normal CLI/dashboard launches still use the stdout READY line below.
    """
    target = os.environ.get("LAILABA_DESKTOP_READY_FILE")
    if not target:
        return
...
def start_server(
    ...
    try:
        from lailaba_cli.nous_auth_keepalive import start_nous_auth_keepalive
    ...
    # the lailaba-0day MCP-persistence campaign abused unauthenticated public
    ...
    if app.state.auth_required:
        ...
        if not list_providers():
            ...
            # to register (e.g. missing LAILABA_DASHBOARD_OAUTH_CLIENT_ID).
            # Each provider plugin that ships with Lailaba AI exposes a
            ...
            _fix_hint = (
                ...
                "  • OAuth: run `lailaba dashboard register` (Nous Portal) or "
    ...
    # LAILABA_DASHBOARD_READY, open the browser, *then* serve.
    #
    ...
    async def _serve():
        ...
        with server.capture_signals():
            ...
            print(f"LAILABA_DASHBOARD_READY port={actual_port}", flush=True)
            print(f"  Lailaba Web UI → http://{host}:{actual_port}")
