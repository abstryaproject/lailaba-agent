"""``lailaba logs`` — view and filter Lailaba log files.
...
Usage examples::
    ...
    lailaba logs                    # last 50 lines of agent.log
    lailaba logs -f                 # follow agent.log in real time
    lailaba logs errors             # last 50 lines of errors.log
    lailaba logs gateway -n 100    # last 100 lines of gateway.log
    lailaba logs gui -f            # follow gui.log (dashboard/pty/ws)
    lailaba logs desktop -f        # follow desktop.log (Electron app boot/backend)
    lailaba logs --level WARNING    # only WARNING+ lines
    lailaba logs --session abc123   # filter by session ID substring
    lailaba logs --component tools  # only tool-related lines
    lailaba logs --since 1h         # lines from the last hour
    lailaba logs --since 30m -f     # follow, starting 30 min ago
"""
...
def tail_log(
    ...
    *,
    ...
    log_path = get_lailaba_home() / "logs" / filename
    if not log_path.exists():
        print(f"Log file not found: {log_path}")
        print(f"(Logs are created when Lailaba runs — try 'lailaba chat' first)")
        sys.exit(1)
    ...
    if follow:
        print(f"--- {display_lailaba_home()}/logs/{filename}{filter_desc} (Ctrl+C to stop) ---")
    else:
        print(f"--- {display_lailaba_home()}/logs/{filename}{filter_desc} (last {num_lines}) ---")
...
def list_logs() -> None:
    ...
    if not log_dir.exists():
        print(f"No logs directory at {display_lailaba_home()}/logs/")
        return

    print(f"Log files in {display_lailaba_home()}/logs/:\n")
    found = False
    ...
    if not found:
        print("  (no log files yet — run 'lailaba chat' to generate logs)")
