"""
Backup and import commands for lailaba CLI.

`lailaba backup` creates a zip archive of the entire ~/.lailaba/ directory
(excluding the lailaba-agent repo and transient files).

`lailaba import` restores from a backup zip, overlaying onto the current
LAILABA_HOME root.
"""
...
from lailaba_constants import get_default_lailaba_root, get_lailaba_home, display_lailaba_home
...
# ``lailaba-agent`` is special-cased to root level only in ``_should_exclude``
# so that skill directories like ``skills/autonomous-ai-agents/lailaba-agent/``
...
# LAILABA_HOME gets walked file-by-file, ballooning a backup to hundreds of
...
_EXCLUDED_DIRS = {
    "lailaba-agent",     # the codebase repo — re-clone instead
    ...
    # Python dependency trees (plugin / MCP-server venvs under LAILABA_HOME) —
...
# File names that ``lailaba import`` must never overwrite, matched by basename so
...
# Reserved archive subtree for provider state that lives OUTSIDE LAILABA_HOME
...
def _collect_memory_provider_external_paths() -> List[Path]:
    ...
    outside LAILABA_HOME, resolved from config only (no network, no init).
...
def _should_exclude(rel_path: Path) -> bool:
    """Return True if *rel_path* (relative to lailaba root) should be skipped."""
    parts = rel_path.parts

    for part in parts:
        if part not in _EXCLUDED_DIRS:
            continue
        # ``lailaba-agent`` only matches at the root level (first component).
        # Nested directories with the same name — e.g.
        # ``skills/autonomous-ai-agents/lailaba-agent/`` — must be preserved.
        if part == "lailaba-agent" and part != parts[0]:
            continue
        return True
...
def _should_skip_backup_file(abs_path: Path, rel_path: Path, out_path: Path) -> bool:
    ...
    # write can copy data from outside LAILABA_HOME.
...
def run_backup(args) -> None:
    """Create a zip backup of the Lailaba home directory."""
    lailaba_root = get_default_lailaba_root()

    if not lailaba_root.is_dir():
        print(f"Error: Lailaba home directory not found at {lailaba_root}")
        sys.exit(1)
    ...
    if args.output:
        ...
        if out_path.is_dir():
            ...
            out_path = out_path / f"lailaba-backup-{stamp}.zip"
    else:
        stamp = datetime.now().strftime("%Y-%m-%d-%H%M%S")
        out_path = Path.home() / f"lailaba-backup-{stamp}.zip"
    ...
    # Collect files
    print(f"Scanning {display_lailaba_home()} ...")
    files_to_add: list[tuple[Path, Path]] = []  # (absolute, relative)
    skipped_dirs = set()

    for dirpath, dirnames, filenames in os.walk(lailaba_root, followlinks=False):
        dp = Path(dirpath)
        rel_dir = dp.relative_to(lailaba_root)

        # Prune excluded directories in-place so os.walk doesn't descend
        # ``lailaba-agent`` is only pruned at the root level; nested dirs
        # with the same name (e.g. in skills/) must be preserved.
        is_root = rel_dir == Path(".")
        orig_dirnames = dirnames[:]
        dirnames[:] = [
            d for d in dirnames
            if d not in _EXCLUDED_DIRS or (d == "lailaba-agent" and not is_root)
        ]
        ...
        for fname in filenames:
            fpath = dp / fname
            rel = fpath.relative_to(lailaba_root)
    ...
    # outside LAILABA_HOME, so the walk above never sees it. Ask the active
    ...
    if external_to_add:
        print(
            ...
            f"stored outside {display_lailaba_home()}."
        )
    ...
    print(f"\nRestore with: lailaba import {out_path.name}")
...
def _validate_backup_zip(zf: zipfile.ZipFile) -> tuple[bool, str]:
    """Check that a zip looks like a Lailaba backup.
    ...
    # Look for telltale files that a lailaba home would have
    ...
    if not found:
        return False, (
            "zip does not appear to be a Lailaba backup "
...
def _detect_prefix(zf: zipfile.ZipFile) -> str:
    ...
    Some tools zip as `.lailaba/config.yaml` instead of `config.yaml`.
    ...
    if len(first_parts) == 1:
        ...
        # Only strip if it looks like a lailaba dir name
        if prefix in {".lailaba", "lailaba"}:
            return prefix + "/"
...
def run_import(args) -> None:
    ...
    with zipfile.ZipFile(zip_path, "r") as zf:
        ...
        print(f"Target: {display_lailaba_home()}")

        if prefix:
            print(f"Detected archive prefix: {prefix!r} (will be stripped)")

        # Check for existing installation
        has_config = (lailaba_root / "config.yaml").exists()
        has_env = (lailaba_root / ".env").exists()

        if (has_config or has_env) and not args.force:
            print()
            print("Warning: Target directory already has Lailaba configuration.")
        ...
        for member in members:
            ...
            # location (e.g. ~/.honcho/config.json), NOT under LAILABA_HOME.
            ...
            target = lailaba_root / rel

            # Security: reject absolute paths and traversals
            try:
                target.resolve().relative_to(lailaba_root.resolve())
            except ValueError:
        ...
        if restored_external:
            print(
                ...
                f"their original location(s) outside {display_lailaba_home()}."
            )
        ...
        profiles_dir = lailaba_root / "profiles"
        restored_profiles = []
        if profiles_dir.is_dir():
            try:
                ...
                # lailaba_cli.profiles might not be available (fresh install)
                if any(profiles_dir.iterdir()):
                    print(f"\n  Profiles detected but aliases could not be created.")
                    print(f"  Run: lailaba profile list  (after installing lailaba)")

        # Guidance
        print()
        if not (lailaba_root / "lailaba-agent").is_dir():
            print("Note: The lailaba-agent codebase was not included in the backup.")
            print("  If this is a fresh install, run: lailaba update")

        if restored_profiles:
            gw_profiles = [n for n, _ in restored_profiles]
            print("\nTo re-enable gateway services for profiles:")
            for pname in gw_profiles:
                print(f"  lailaba -p {pname} gateway install")

        print("Done. Your Lailaba configuration has been restored.")


# ---------------------------------------------------------------------------
# Quick state snapshots (used by /snapshot slash command and lailaba backup --quick)
# ---------------------------------------------------------------------------

# Critical state files to include in quick snapshots (relative to LAILABA_HOME).
...
# — `lailaba update` snapshots this set before pulling so approved-user lists
...
_QUICK_STATE_FILES = (
    ...
    # are at $LAILABA_HOME/<name> for the default/root profile; on non-root
    # profiles the real path is outside LAILABA_HOME and the entry is silently
...
def _quick_snapshot_root(lailaba_home: Optional[Path] = None) -> Path:
...
def list_quick_snapshots(
    ...
    root = _quick_snapshot_root(lailaba_home)
    if not root.exists():
        return []
...
# Relative path of the cron job database inside LAILABA_HOME. Kept in sync with
...
def restore_cron_jobs_if_emptied(
    ...
    """Safety net for silent cron-job loss across ``lailaba update``.
    ...
    Args:
        ...
        lailaba_home: Override for the Lailaba home directory (tests).
...
def prune_quick_snapshots(
    ...
    return _prune_quick_snapshots(_quick_snapshot_root(lailaba_home), keep=keep)


def run_quick_backup(args) -> None:
    """CLI entry point for lailaba backup --quick."""
    label = getattr(args, "label", None)
    snap_id = create_quick_snapshot(label=label)
    if snap_id:
        print(f"State snapshot created: {snap_id}")
        snaps = list_quick_snapshots()
        print(f"  {len(snaps)} snapshot(s) stored in {display_lailaba_home()}/state-snapshots/")
...
def _write_full_zip_backup(out_path: Path, lailaba_root: Path) -> Optional[Path]:
    """Write a full zip snapshot of ``lailaba_root`` to ``out_path``.
...
def _pre_update_backup_dir(lailaba_home: Optional[Path] = None) -> Path:
    home = lailaba_home or get_lailaba_home()
...
def create_pre_update_backup(
    ...
    """Create a full zip backup of LAILABA_HOME under ``backups/``.

    Mirrors :func:`run_backup` (same exclusion rules, same SQLite safe-copy)
    but writes to ``<LAILABA_HOME>/backups/pre-update-<timestamp>.zip`` and
    auto-prunes old pre-update backups.

    Returns the path to the created zip, or ``None`` if no files were
    found or the backup could not be created.  Never raises — the caller
    (``lailaba update``) should continue even if the backup fails.
    """
    lailaba_root = lailaba_home or get_default_lailaba_root()
    if not lailaba_root.is_dir():
        return None

    backup_dir = _pre_update_backup_dir(lailaba_root)
    try:
    ...
    result = _write_full_zip_backup(out_path, lailaba_root)
    if result is None:
        return None
...
# ---------------------------------------------------------------------------
# Pre-migration auto-backup (used by `lailaba claw migrate`)
# ---------------------------------------------------------------------------
...
def create_pre_migration_backup(
    ...
    """Create a full zip backup of LAILABA_HOME under ``backups/`` before a
    ``lailaba claw migrate`` apply.

    Shares implementation with :func:`create_pre_update_backup` via
    ``_write_full_zip_backup`` — same exclusions, same SQLite safe-copy,
    restorable with ``lailaba import <archive>``.  Writes to
    ``<LAILABA_HOME>/backups/pre-migration-<timestamp>.zip`` and auto-prunes
    ...
    # Reuses the shared backups/ directory so `lailaba import` and the
