"""
Dump command for lailaba CLI.

Outputs a compact, plain-text summary of the user's Lailaba setup
...
from lailaba_cli.config import get_lailaba_home, get_env_path, get_project_root, load_config
...
def _get_git_commit(project_root: Path) -> str:
    ...
    to ``<project_root>/.lailaba_build_sha`` by the Dockerfile's
    ``LAILABA_GIT_SHA`` build-arg (see ``lailaba_cli/build_info.py``).
...
def _redact(value: str) -> str:
    ...
    ``lailaba dump`` formats empty values as blank, not as ``"(not set)"``).
    """
...
def _gateway_status() -> str:
    ...
    try:
        from lailaba_cli.gateway import get_gateway_runtime_snapshot
...
def _count_skills(lailaba_home: Path) -> int:
...
def _cron_summary(lailaba_home: Path) -> str:
    """Return cron jobs summary."""
    jobs_file = lailaba_home / "cron" / "jobs.json"
...
def run_dump(args):
    ...
    lines.append(f"lailaba_home:      {display_lailaba_home()}")
    ...
    for env_var, label in api_keys:
        ...
        # A credential added via `lailaba auth add openrouter` lives in the
        # credential pool, not as an env var — surface it so the dump doesn't
        # misleadingly read "not set" while `lailaba auth list` shows it (#42130).
    ...
    toolsets = config.get("toolsets", ["lailaba-cli"])
    ...
    lines.append(f"  cron_jobs:          {_cron_summary(lailaba_home)}")
    lines.append(f"  skills:             {_count_skills(lailaba_home)}")
