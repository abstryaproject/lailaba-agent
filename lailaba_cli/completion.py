"""Shell completion script generation for lailaba CLI.
...
def generate_bash(parser: argparse.ArgumentParser) -> str:
    ...
    for cmd in sorted(tree["subcommands"]):
        ...
        if cmd == "profile" and info["subcommands"]:
            ...
            cases.append(
                ...
                f"                    COMPREPLY=($(compgen -W \"$(_lailaba_profiles)\" -- \"$cur\"))\n"
                f"                    return\n"
                f"                    ;;\n"
                f"            esac\n"
                f"            ;;"
    ...
    return f"""# Lailaba AI bash completion
# Add to ~/.bashrc:
#   eval "$(lailaba completion bash)"

_lailaba_profiles() {{
    local profiles_dir="$HOME/.lailaba/profiles"
...
_lailaba_completion() {{
    ...
    if [[ "$prev" == "-p" || "$prev" == "--profile" ]]; then
        COMPREPLY=($(compgen -W "$(_lailaba_profiles)" -- "$cur"))
        return
    fi
...
complete -F _lailaba_completion lailaba
"""
...
def generate_zsh(parser: argparse.ArgumentParser) -> str:
    ...
    return f"""#compdef lailaba
# Lailaba AI zsh completion
# Add to ~/.zshrc:
...
_lailaba_profiles() {{
    local -a profiles
    profiles=(default)
    if [[ -d "$HOME/.lailaba/profiles" ]]; then
        profiles+=($HOME/.lailaba/profiles/*(N/:t))
    fi
...
_lailaba() {{
    ...
    _arguments -C \\
        ...
        '(-)'{{-p,--profile}}'[Profile name]:profile:_lailaba_profiles' \\
        '1:command:->commands' \\
        '*::arg:->args'
...
{top_cmds_str}
            ...
            _describe 'lailaba command' subcmds
            ;;
        args)
            case ${{line[1]}} in
{sub_cases_str}
...
def generate_fish(parser: argparse.ArgumentParser) -> str:
    ...
    lines: list[str] = [
        "# Lailaba AI fish completion",
        "# Add to your config:",
        "#   lailaba completion fish | source",
        "",
        "# Helper: list available profiles",
        "function __lailaba_profiles",
        "    echo default",
        "    if test -d $HOME/.lailaba/profiles",
        "        for d in $HOME/.lailaba/profiles/*/",
        "            basename $d",
        "        end",
        "    end",
        "end",
        ...
        "complete -c lailaba -f",
        "",
        "# Complete profile names after -p / --profile",
        "complete -c lailaba -f -s p -l profile"
        " -d 'Profile name' -xa '(__lailaba_profiles)'",
        "",
        ...
        if cmd == "profile":
            for action in sorted(profile_name_actions):
                lines.append(
                    ...
                    f"-a '(__lailaba_profiles)' -d 'Profile name'"
                )
