"""
Single source of truth for provider identity in Lailaba AI.
...
2. **Lailaba overlays** — transport type, auth patterns, aggregator flags,
...
# -- Lailaba overlay ----------------------------------------------------------
# Lailaba-specific metadata that models.dev doesn't provide.

@dataclass(frozen=True)
class HermesOverlay:
    """Lailaba-specific provider metadata layered on top of models.dev."""
...
LAILABA_OVERLAYS: Dict[str, HermesOverlay] = {
    "moa": HermesOverlay(
    ...
    "qwen-oauth": HermesOverlay(
        ...
        base_url_env_var="LAILABA_QWEN_BASE_URL",
    ),
...
class ProviderDef:
    ...
    doc: str = ""
    source: str = ""                      # "models.dev", "lailaba", "user-config"
...
def get_provider(name: str) -> Optional[ProviderDef]:
    ...
    Resolution order:
      1. Lailaba overlays (for providers not in models.dev: nous, openai-codex, etc.)
      2. models.dev catalog + Lailaba overlay
    ...
    overlay = LAILABA_OVERLAYS.get(canonical)
    ...
    if mdev_info is not None:
        ...
        # Combine env vars: models.dev env + lailaba extra
    ...
    if overlay is not None:
        # Lailaba-only provider (not in models.dev)
        return ProviderDef(
            id=canonical,
...
def determine_api_mode(provider: str, base_url: str = "") -> str:
    ...
    # Direct provider checks for providers not in LAILABA_OVERLAYS
    if provider == "bedrock":
