"""``lailaba debug`` debug tools for Lailaba AI.

Currently supports:
    lailaba debug share    Upload debug report (system info + logs) to a
                          ...
                          ``~/.lailaba/logs/*.log`` are not leaked into
...
_REDACTION_BANNER = (
    "[lailaba debug share: log content redacted at upload time. "
...
def _pending_file() -> Path:
    """Path to ``~/.lailaba/pastes/pending.json``.
...
def _save_pending(entries: list[dict]) -> None:
    ...
    try:
        ...
        # Non-fatal — worst case the user has to run ``lailaba debug delete``
        # manually.
        pass
...
def _sweep_expired_pastes(now: Optional[float] = None) -> tuple[int, int]:
    ...
    intended to be called from every ``lailaba debug`` invocation with
    minimal noise.
    """
    entries = _load_pending()
    if not entries:
...
⚠️  This will upload the following to a public paste service:
  • System info (OS, Python version, Lailaba version, provider, which API keys
...
_GATEWAY_PRIVACY_NOTICE = (
    ...
    "Full logs are NOT included from the gateway — use `lailaba debug share` "
...
def delete_paste(url: str) -> bool:
    ...
    req = urllib.request.Request(
        ...
        headers={"User-Agent": "lailaba-agent/debug-share"},
    )
...
def _schedule_auto_delete(urls: list[str], delay_seconds: int = _AUTO_DELETE_SECONDS):
    ...
    every ``lailaba debug share`` invocation added ~20 MB of resident Python
    interpreters that never exited until the sleep completed.

    The replacement is stateless: we append to ``~/.lailaba/pastes/pending.json``
    and the gateway's cron ticker sweeps expired entries once per hour.
    ``lailaba debug share`` also runs an opportunistic sweep as a fallback
...
def _upload_paste_rs(content: str) -> str:
    ...
    req = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "lailaba-agent/debug-share",
        },
    )
    with urllib.request.urlopen(req, timeout=30) as resp:
...
def _primary_log_path(log_name: str) -> Optional[Path]:
    ...
    from lailaba_cli.logs import LOG_FILES

    filename = LOG_FILES.get(log_name)
    return (get_lailaba_home() / "logs" / filename) if filename else None
...
def _capture_dump() -> str:
    """Run ``lailaba dump`` and return its stdout as a string."""
...
def collect_debug_report(
    *,
    ...
    dump_text
        Pre-captured dump output.  If empty, ``lailaba dump`` is run
        internally.
...
def build_debug_share(
    *,
    ...
    This is the shared core behind ``lailaba debug share`` (CLI) and the
    ...
    if redact:
        logger.info(
            "lailaba debug share: applied force-mode redaction to log snapshots before upload"
        )
...
def run_debug_share(args):
    ...
    try:
        ...
        print("\nRun `lailaba debug share --local` to print the report instead.\n")
        sys.exit(1)
    ...
    print(f"To delete now:  lailaba debug delete <url>")

    print(f"\nShare these links with the Lailaba team for support.")
...
def run_debug_delete(args):
    ...
    if not urls:
        print("Usage: lailaba debug delete <url> [<url> ...]")
        print("  Deletes paste.rs pastes uploaded by 'lailaba debug share'.")
        return
...
def run_debug(args):
    ...
    # Opportunistic sweep of expired pastes on every ``lailaba debug`` call.
    ...
    # best-effort — any failure is swallowed so ``lailaba debug`` stays
    ...
    if subcmd == "share":
        ...
        # Default: show help
        print("Usage: lailaba debug <command>")
        print()
        print("Commands:")
