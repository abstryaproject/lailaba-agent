"""
Cron subcommand for lailaba CLI.
...
_GATEWAY_LIFECYCLE_PATTERNS = re.compile(
    r"(?i)"
    r"(lailaba\s+gateway\s+(restart|stop|start))"
    r"|(launchctl\s+(kickstart|unload|load|stop|restart)\s+.*lailaba)"
    r"|(systemctl\s+(-\S+\s+)*(restart|stop|start)\s+.*lailaba)"
    r"|(p?kill\s+.*lailaba.*gateway)"
)
...
def _warn_if_gateway_not_running() -> None:
    ...
    print(color("     Start it with: lailaba gateway install", Colors.DIM))
    print(color("                    sudo lailaba gateway install --system  # Linux servers", Colors.DIM))
    print(color("     Check status:  lailaba cron status", Colors.DIM))
...
def cron_list(show_all: bool = False):
    ...
    if not jobs:
        ...
        print(color("Create one with 'lailaba cron create ...' or the /cron command in chat.", Colors.DIM))
        return
...
def cron_status():
    ...
    if pids:
        ...
        if hb_age is not None and hb_age > STALE_AFTER:
            ...
            print("  Cron jobs may NOT be firing. Restart: lailaba gateway restart")
        ...
        print("    lailaba gateway install    # Install as a user service")
        print("    sudo lailaba gateway install --system  # Linux servers: boot-time system service")
        print("    lailaba gateway            # Or run in foreground")
...
def cron_create(args):
    ...
    if _contains_gateway_lifecycle_command(combined):
        print(color(
            ...
            "Use `lailaba gateway restart` from a shell outside the gateway.",
            Colors.RED,
        ))
        return 1
...
def cron_command(args):
    ...
    print("Usage: lailaba cron [list|create|edit|pause|resume|run|remove|status|tick]")
    sys.exit(1)
