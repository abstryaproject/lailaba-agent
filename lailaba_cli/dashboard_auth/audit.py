...
Profile-aware location: ``$LAILABA_HOME/logs/dashboard-auth.log``.
...
from ``lailaba_constants`` or other lailaba_cli modules — so it can be
...
def _resolve_log_path() -> Path:
    """``$LAILABA_HOME/logs/dashboard-auth.log`` with the standard fallback.

    Mirrors ``lailaba_constants.get_lailaba_home`` semantics: env var wins,
    else ``~/.lailaba``. A local copy avoids an import cycle with the
    middleware which lives below ``lailaba_cli``.
    """
    home = os.environ.get("LAILABA_HOME") or str(Path.home() / ".lailaba")
