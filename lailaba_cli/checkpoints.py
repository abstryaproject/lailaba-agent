"""`lailaba checkpoints` CLI subcommand.

Gives users direct visibility and control over the filesystem checkpoint
store at ``~/.lailaba/checkpoints/``.  Actions:

    lailaba checkpoints               # same as `status`
    lailaba checkpoints status        # total size, project count, breakdown
    lailaba checkpoints list          # per-project checkpoint counts + workdir
    lailaba checkpoints prune [opts]  # force a sweep (ignores the 24h marker)
    lailaba checkpoints clear [-f]    # nuke the entire base (asks first)
    lailaba checkpoints clear-legacy  # delete just the legacy-* archives

Examples::

    lailaba checkpoints
    lailaba checkpoints prune --retention-days 3 --max-size-mb 200
    lailaba checkpoints clear -f
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if legacy:
        ...
        print()
        print("Clear with: lailaba checkpoints clear-legacy")
    return 0
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire subcommands onto the ``lailaba checkpoints`` parser."""
    parser.set_defaults(func=cmd_status)  # bare `lailaba checkpoints` → status
