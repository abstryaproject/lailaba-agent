"""lailaba webhook — manage dynamic webhook subscriptions from the CLI.
...
Subscriptions persist to ~/.lailaba/webhook_subscriptions.json and are
...
def _subscriptions_path() -> Path:
    return _lailaba_home() / _SUBSCRIPTIONS_FILENAME
...
def _setup_hint() -> str:
  ...
  Then start the gateway: lailaba gateway run
"""
...
def webhook_command(args):
    """Entry point for 'lailaba webhook' subcommand."""
    sub = getattr(args, "webhook_action", None)

    if not sub:
        print("Usage: lailaba webhook {subscribe|list|remove|test}")
        print("Run 'lailaba webhook --help' for details.")
        return
...
def _cmd_subscribe(args):
    ...
    print(f"  The gateway must be running to receive events (lailaba gateway run).\n")
...
def _cmd_list(args):
    ...
    if not subs:
        ...
        print("  Create one with: lailaba webhook subscribe <name>")
        return
...
def _cmd_test(args):
    ...
    payload = args.payload or '{"test": true, "event_type": "test", "message": "Hello from lailaba webhook test"}'
    ...
    try:
        ...
        print(f"  Error: {e}")
        print("  Is the gateway running? (lailaba gateway run)")
