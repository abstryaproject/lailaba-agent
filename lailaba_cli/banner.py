...
from lailaba_cli import __version__ as VERSION, __release_date__ as RELEASE_DATE
...
# (e.g. nix-built lailaba — no local git history to count against).
UPDATE_AVAILABLE_NO_COUNT = -1

_UPSTREAM_REPO_URL = "https://github.com/NousResearch/lailaba-agent.git"
_OFFICIAL_REPO_CANONICAL = "github.com/nousresearch/lailaba-agent"
...
def _fetch_pypi_latest(package: str = "lailaba-agent") -> Optional[str]:
...
def check_for_updates() -> Optional[int]:
    """Check whether a Lailaba update is available.

    Two paths: if ``LAILABA_REVISION`` is set (nix builds embed it), compare
    ...
    cache_file = lailaba_home / ".update_check"
    embedded_rev = os.environ.get("LAILABA_REVISION") or None

    # Docker images have no working tree to count commits against — the
    # published image excludes `.git` (see .dockerignore) and sets no
    # LAILABA_REVISION (that's nix-only). Without this guard the checks below
    ...
    # and `lailaba update` correctly refuses to run in-place inside the
    # container anyway. The dashboard's REST `/api/lailaba/update/check`
    ...
    if embedded_rev:
        ...
        # $LAILABA_HOME/lailaba-agent/ may be a stale copy from --clone-all;
        # Path(__file__) always resolves to the actual installed checkout.
        repo_dir = Path(__file__).parent.parent.resolve()
        if not (repo_dir / ".git").exists():
            repo_dir = lailaba_home / "lailaba-agent"
        if not (repo_dir / ".git").exists():
...
def _resolve_repo_dir() -> Optional[Path]:
    """Return the active Lailaba git checkout, or None if this isn't a git install.

    Prefers the running code's location over the profile-scoped path
    because ``$LAILABA_HOME/lailaba-agent/`` may be a stale copy carried
    over by ``--clone-all``.
    """
    repo_dir = Path(__file__).parent.parent.resolve()
    if not (repo_dir / ".git").exists():
...
def get_git_banner_state(repo_dir: Optional[Path] = None) -> Optional[dict]:
    ...
    ``lailaba_cli/build_info.py``) and return it as a frozen
    ...
    if repo_dir is None:
        ...
        try:
            from lailaba_cli.build_info import get_build_sha
...
_RELEASE_URL_BASE = "https://github.com/NousResearch/lailaba-agent/releases/tag"
...
def get_latest_release_tag(repo_dir: Optional[Path] = None) -> Optional[tuple]:
    ...
    Lailaba checkout. Cached per-process. Release URL always points at the
    canonical NousResearch/lailaba-agent repo (forks don't get a link).
    """
...
def format_banner_version_label() -> str:
    ...
    base = f"Lailaba AI v{VERSION} ({RELEASE_DATE})"
...
def build_welcome_banner(console: "Console", model: str, cwd: str,
    ...
    try:
        ...
        _hero = _bskin.banner_hero if hasattr(_bskin, 'banner_hero') and _bskin.banner_hero else LAILABA_CADUCEUS
    except Exception:
        _bskin = None
        _hero = LAILABA_CADUCEUS
    left_lines = ["", _hero, ""]
    ...
    if os.getenv("LAILABA_YOLO_MODE"):
    ...
    try:
        from lailaba_cli.codex_runtime_switch import get_current_runtime
    ...
    try:
        ...
        if behind is not None and behind != 0:
            from lailaba_cli.config import get_managed_update_command, recommended_update_command
            if behind > 0:
                ...
                # UPDATE_AVAILABLE_NO_COUNT: nix-built lailaba; we know an update
    ...
    # Pip-install warning — `pip install lailaba-agent` is not the supported
    ...
    if term_width >= 95:
        _logo = _bskin.banner_logo if _bskin and hasattr(_bskin, 'banner_logo') and _bskin.banner_logo else LAILABA_AGENT_LOGO
        console.print(_logo)
        console.print()
