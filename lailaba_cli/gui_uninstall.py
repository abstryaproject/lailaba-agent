"""
Lailaba Desktop (Chat GUI) uninstaller.
...
the Python agent or the user's config/data:
  ...
  1. Source-built GUI (``lailaba desktop`` / ``lailaba gui``)
     Built inside the agent checkout under ``$LAILABA_HOME/lailaba-agent/``:
       ...
       - ``$LAILABA_HOME/desktop-build-stamp.json`` (the build freshness stamp)
  ...
  2. Packaged distributable (DMG / NSIS / AppImage / deb / rpm)
     ...
     own bundled Electron + a per-user Electron ``userData`` directory:
       - macOS:   ``/Applications/Lailaba.app`` or ``~/Applications/Lailaba.app``
       - Windows: ``%LOCALAPPDATA%\\Programs\\Lailaba`` (NSIS per-user)
...
the app name ("Lailaba"), separate from ``$LAILABA_HOME``:
  - macOS:   ``~/Library/Application Support/Lailaba``
  - Windows: ``%APPDATA%\\Lailaba``
  - Linux:   ``$XDG_CONFIG_HOME/Lailaba`` (default ``~/.config/Lailaba``)
...
``lailaba uninstall --gui`` (and friends) without paying for the full CLI.
"""
...
def _agent_root(lailaba_home: Path) -> Path:
    """The agent checkout root — same layout install.sh / install.ps1 use."""
    return lailaba_home / "lailaba-agent"


def desktop_userdata_dir() -> Path:
    """Return the Electron ``userData`` directory for the desktop app.

    Mirrors Electron's ``app.getPath('userData')`` for an app named "Lailaba"
    ...
    if sys.platform == "darwin":
        return home / "Library" / "Application Support" / "Lailaba"
...
def source_built_gui_artifacts(lailaba_home: Path) -> "list[Path]":
    """GUI build artifacts produced by ``lailaba desktop`` inside the checkout.

    These are removable on a GUI uninstall without harming the agent: the
    Python agent runs from ``lailaba-agent/`` source + ``venv/`` and never
    needs the Electron build output or node_modules.
    """
    agent_root = _agent_root(lailaba_home)
    ...
    return [
        ...
        lailaba_home / "desktop-build-stamp.json",
    ]
...
def packaged_gui_app_paths() -> "list[Path]":
    ...
    electron-builder output locations for the "Lailaba" product.
    """
    home = Path.home()
    paths: list[Path] = []
    if sys.platform == "darwin":
        paths += [
            Path("/Applications/Lailaba.app"),
            home / "Applications" / "Lailaba.app",
        ]
    elif sys.platform == "win32":
        local = os.environ.get("LOCALAPPDATA")
        local_base = Path(local) if local else (home / "AppData" / "Local")
        paths += [
            # NSIS per-user install (perMachine=false → Programs\Lailaba).
            local_base / "Programs" / "Lailaba",
            # Older / alternate layout some builds used.
            local_base / "lailaba-desktop",
        ]
        program_files = os.environ.get("ProgramFiles")
        if program_files:
            # NSIS per-machine fallback (needs admin to remove).
            paths.append(Path(program_files) / "Lailaba")
    else:
        ...
        paths += [
            data_base / "applications" / "lailaba.desktop",
            data_base / "applications" / "Lailaba.desktop",
        ]
    return paths


def agent_is_installed(lailaba_home: Path) -> bool:
    """Return True when a usable Python agent install exists under LAILABA_HOME.
    ...
    if (agent_root / "lailaba_cli").is_dir():
        return True
...
def gui_is_installed(lailaba_home: Path) -> bool:
    """Return True when any desktop GUI artifact exists (built or packaged)."""
    for p in source_built_gui_artifacts(lailaba_home):
        if p.exists():
            return True
...
def gui_install_summary(lailaba_home: "Path | None" = None) -> dict:
    ...
    """
    home: Path = lailaba_home if lailaba_home is not None else get_lailaba_home()
...
def uninstall_gui(lailaba_home: "Path | None" = None, *, remove_userdata: bool = True) -> "list[Path]":
    ...
    Never touches ``lailaba-agent/lailaba_cli`` (agent source), ``venv/``, or any
    config / sessions / .env under ``$LAILABA_HOME``.
    ...
    if sys.platform.startswith("linux"):
        log_info(
            ...
            "with your package manager (e.g. 'sudo apt remove lailaba' or "
            "'sudo dnf remove lailaba'). AppImage builds are a single file you "
