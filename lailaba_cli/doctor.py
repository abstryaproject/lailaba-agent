"""
Doctor command for lailaba CLI.

Diagnoses issues with Lailaba AI setup.
"""
...
from lailaba_cli.config import get_project_root, get_lailaba_home, get_env_path
from lailaba_cli.env_loader import load_lailaba_dotenv
from lailaba_constants import display_lailaba_home
from lailaba_constants import agent_browser_runnable

PROJECT_ROOT = get_project_root()
LAILABA_HOME = get_lailaba_home()
_DHH = display_lailaba_home()  # user-facing display path (e.g. ~/.lailaba or ~/.lailaba/profiles/coder)

# Load environment variables from ~/.lailaba/.env so API key checks work
_env_path = get_env_path()
load_lailaba_dotenv(lailaba_home=_env_path.parent, project_env=PROJECT_ROOT / ".env")

from lailaba_cli.colors import Colors, color
from lailaba_cli.models import _LAILABA_USER_AGENT
from lailaba_constants import OPENROUTER_MODELS_URL
...
from lailaba_constants import is_termux as _is_termux
...
def _has_provider_env_config(content: str) -> bool:
    """Return True when ~/.lailaba/.env contains provider auth/base URL settings."""
...
def _doctor_tool_availability_detail(toolset: str) -> str:
    ...
    if toolset == "kanban" and not os.environ.get("LAILABA_KANBAN_TASK"):
...
def _has_healthy_oauth_fallback_for_apikey_provider(provider_label: str) -> bool:
    ...
    if normalized == "minimax":
        try:
            from lailaba_cli.auth import get_minimax_oauth_auth_status
...
def _check_version_consistency(issues: list[str]) -> None:
    """Verify pyproject.toml version matches lailaba_cli.__version__.

    A git conflict resolution (reset/merge) can revert one file without the
    other, leaving ``lailaba --version`` reporting a stale version while
    ...
    """
    try:
        from lailaba_cli import __version__ as init_version
    except Exception:
        return
    ...
    if pyproject_version == init_version:
        ...
        _fail_and_issue(
            ...
            f"(pyproject.toml {pyproject_version} != lailaba_cli/__init__.py {init_version})",
            "Re-sync version files (e.g. run 'lailaba update', or set "
            "lailaba_cli/__init__.py __version__ to match pyproject.toml)",
            issues,
        )
...
def _check_s6_supervision(issues: list[str]) -> None:
    ...
    Reports:
      - Whether the main-lailaba and dashboard static services are up
    ...
    for static in ("main-lailaba", "dashboard"):
    ...
    if not profiles:
        check_info("No per-profile gateways registered yet — create one with `lailaba profile create <name>`")
        return
...
def _build_apikey_providers_list() -> list:
    ...
    try:
        ...
        try:
            from lailaba_cli.providers import normalize_provider as _normalize_provider
...
def managed_scope_check() -> None:
    ...
    resolved from the LAILABA_MANAGED_DIR override (rather than the system
    ...
    if os.environ.get("LAILABA_MANAGED_DIR", "").strip():
        check_info(f"managed dir set via LAILABA_MANAGED_DIR={managed_dir}")
...
def run_doctor(args):
    ...
    # checks (like cronjob management) should see the same context as `lailaba`.
    os.environ.setdefault("LAILABA_INTERACTIVE", "1")

    # Handle `lailaba doctor --ack <id>` as a fast path. Persist the ack and
    ...
    if ack_target:
        ...
        if ack_advisory(ack_target):
            ...
            print(color(
                ...
                f"Check ~/.lailaba/config.yaml is writable.",
                Colors.RED,
            ))
            sys.exit(1)
        return
    ...
    print(color("│                 🩺 Lailaba Doctor                        │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
        ...
        if fresh_hits:
            for hit in fresh_hits:
                ...
                manual_issues.append(
                    ...
                    f"`lailaba doctor --ack {hit.advisory.id}`."
                )
    ...
    # Detect drift between pyproject.toml and lailaba_cli/__init__.py versions
    ...
    managed_scope_check()
    # Check ~/.lailaba/.env (primary location for user config)
    env_path = LAILABA_HOME / '.env'
    if env_path.exists():
        ...
        if _has_provider_env_config(content):
            ...
            issues.append("Run 'lailaba setup' to configure API keys")
    else:
        ...
        if fallback_env.exists():
            ...
            if should_fix:
                ...
                check_info("Run 'lailaba setup' to configure API keys")
                fixed_count += 1
            else:
                check_info("Run 'lailaba setup' to create one")
                issues.append("Run 'lailaba setup' to create .env")
    
    # Check ~/.lailaba/config.yaml (primary) or project cli-config.yaml (fallback)
    config_path = LAILABA_HOME / 'config.yaml'
    if config_path.exists():
        ...
        try:
            ...
            try:
                ...
                pass
            try:
                from lailaba_cli.config import get_compatible_custom_providers as _compatible_custom_providers
                from lailaba_cli.providers import (
                ...
                if catalog_provider is None or (
                    ...
                    _fail_and_issue(
                        ...
                        (
                            ...
                            f"Fix: run 'lailaba config set model.provider <valid_provider>'"
                        ),
                        issues,
                    )
            ...
            if runtime_provider and runtime_provider not in ("auto", "custom"):
                try:
                    if runtime_provider == "openrouter":
                        ...
                        from lailaba_cli.auth import PROVIDER_REGISTRY, get_auth_status
                    ...
                    if not configured:
                        _fail_and_issue(
                            ...
                            "(check ~/.lailaba/.env or run 'lailaba setup')",
                            (
                                f"No credentials found for provider '{runtime_provider}'. "
                                f"Run 'lailaba setup' or set the provider's API key in {_DHH}/.env, "
                                f"or switch providers with 'lailaba config set model.provider <name>'"
                            ),
                            issues,
                        )
                except Exception:
        ...
        if fallback_config.exists():
            ...
            if should_fix:
                ...
                if example_config.exists():
                    ...
                    from lailaba_cli.config import DEFAULT_CONFIG, save_config
    ...
    if config_path.exists():
        try:
            from lailaba_cli.config import check_config_version, migrate_config
            ...
            if current_ver < latest_ver:
                ...
                if should_fix:
                    try:
                        ...
                        issues.append("Run 'lailaba setup' to migrate config")
                else:
                    issues.append("Run 'lailaba doctor --fix' or 'lailaba setup' to migrate config")
            else:
        ...
        try:
            ...
            if stale_root_keys:
                ...
                if should_fix:
                    ...
                    issues.append("Stale root-level provider/base_url in config.yaml — run 'lailaba doctor --fix'")
        except Exception:
            pass

        # Detect stale LAILABA_MAX_ITERATIONS ghost in .env shadowing
        ...
        # bridge normally derives LAILABA_MAX_ITERATIONS from agent.max_turns
        ...
        try:
            import yaml
            from lailaba_cli.config import load_env, remove_env_value
            with open(config_path, encoding="utf-8") as f:
                raw_config = yaml.safe_load(f) or {}
            ...
            env_ghost = load_env().get("LAILABA_MAX_ITERATIONS")
            drift = (
            ...
            if drift:
                check_warn(
                    f"LAILABA_MAX_ITERATIONS={env_ghost} in .env shadows "
                    f"agent.max_turns={cfg_max_turns} in config.yaml",
                    "(stale ghost from an earlier `lailaba setup` run)",
                )
                if should_fix:
                    if remove_env_value("LAILABA_MAX_ITERATIONS"):
                        check_ok(
                            "Removed stale LAILABA_MAX_ITERATIONS from .env "
                            f"(config.yaml agent.max_turns={cfg_max_turns} is now authoritative)"
                        )
                        fixed_count += 1
                    else:
                        check_warn("Could not remove LAILABA_MAX_ITERATIONS from .env")
                        manual_issues.append(
                            "Manually delete the LAILABA_MAX_ITERATIONS line from "
                            f"{_DHH}/.env — config.yaml agent.max_turns is authoritative."
                        )
                else:
                    issues.append(
                        "Stale LAILABA_MAX_ITERATIONS in .env shadows config.yaml — "
                        "run 'lailaba doctor --fix'"
                    )
        except Exception:
            pass
    ...
    try:
        ...
        if codex_status.get("logged_in"):
            ...
            # Native OAuth uses Lailaba' own device-code flow — the Codex CLI is
    ...
    for subdir_name in expected_subdirs:
        subdir_path = lailaba_home / subdir_name
    ...
    soul_path = lailaba_home / "SOUL.md"
    if soul_path.exists():
        ...
        check_warn(f"{_DHH}/SOUL.md not found", "(create it to give Lailaba a custom personality)")
        if should_fix:
            soul_path.parent.mkdir(parents=True, exist_ok=True)
            soul_path.write_text(
                "# Lailaba AI Persona\n\n"
                "<!-- Edit this file to customize how Lailaba communicates. -->\n\n"
                "You are Lailaba, a helpful AI assistant.\n",
                encoding="utf-8",
            )
    ...
    memories_dir = lailaba_home / "memories"
    ...
    state_db_path = lailaba_home / "state.db"
    if state_db_path.exists():
        try:
            ...
            from lailaba_state import _db_opens_cleanly, repair_state_db_schema
            ...
            if _write_reason is not None:
                ...
                if should_fix:
                    ...
                    issues.append(
                        "state.db FTS write corruption — run 'lailaba doctor --fix' "
                        "(or 'lailaba sessions repair') to rebuild the FTS index"
                    )
        except Exception as e:
            from lailaba_state import is_malformed_db_error, repair_state_db_schema
            ...
            if is_malformed_db_error(e):
                ...
                if should_fix:
                    ...
                    issues.append(
                        "state.db schema malformed — run 'lailaba doctor --fix' "
                        "(or 'lailaba sessions repair') to recover hidden sessions"
                    )
            else:
    ...
    wal_path = lailaba_home / "state.db-wal"
    if wal_path.exists():
        try:
            ...
            if wal_size > 50 * 1024 * 1024:  # 50 MB
                ...
                if should_fix:
                    ...
                    issues.append("Large WAL file — run 'lailaba doctor --fix' to checkpoint")
    ...
    if sys.platform != "win32":
        ...
        for _venv_name in ("venv", ".venv"):
            _candidate = PROJECT_ROOT / _venv_name / "bin" / "lailaba"
        ...
        _cmd_link = _cmd_link_dir / "lailaba"

        if _venv_bin is None:
            check_warn(
                "Venv entry point not found",
                "(lailaba not in venv/bin/ or .venv/bin/ — reinstall with pip install -e '.[all]')"
            )
            manual_issues.append(
            ...
            if _cmd_link.is_symlink():
                ...
                if _target == _expected:
                    check_ok(f"{_cmd_link_display}/lailaba → correct target")
                else:
                    check_warn(
                        f"{_cmd_link_display}/lailaba points to wrong target",
                        f"(→ {_target}, expected → {_expected})"
                    )
                    if should_fix:
                        _cmd_link.unlink()
                        _cmd_link.symlink_to(_venv_bin)
                        check_ok(f"Fixed symlink: {_cmd_link_display}/lailaba → {_venv_bin}")
                        fixed_count += 1
                    else:
                        issues.append(f"Broken symlink at {_cmd_link_display}/lailaba — run 'lailaba doctor --fix'")
            elif _cmd_link.exists():
                # It's a regular file, not a symlink — possibly a wrapper script
                check_ok(f"{_cmd_link_display}/lailaba exists (non-symlink)")
            else:
                check_fail(
                    f"{_cmd_link_display}/lailaba not found",
                    "(lailaba command may not work outside the venv)"
                )
                if should_fix:
                    _cmd_link_dir.mkdir(parents=True, exist_ok=True)
                    _cmd_link.symlink_to(_venv_bin)
                    check_ok(f"Created symlink: {_cmd_link_display}/lailaba → {_venv_bin}")
                    fixed_count += 1
                    ...
                    issues.append(f"Missing {_cmd_link_display}/lailaba symlink — run 'lailaba doctor --fix'")
    ...
    try:
        from lailaba_constants import is_container as _is_container
    ...
    if _safe_which("node"):
        ...
        if agent_browser_path.exists():
            ...
            # `lailaba update` wiped node_modules (issue #48521).
            check_warn(
        ...
        if agent_browser_ok and not _is_termux():
            try:
                ...
                # to eagerly load in every `lailaba doctor` invocation.
    ...
    if _npm_bin:
        ...
        # The WhatsApp bridge may live under a writable LAILABA_HOME mirror
            ...
            try:
                ...
                if total == 0:
                    ...
                    if audit_extra and audit_extra[0] == "--workspace":
                        ...
                        # tree — in that case it is an npm bug, not a Lailaba one.
                        check_info(
            ...
            if r.status_code == 402:
                return _ConnectivityResult(
                    ...
                    ["OpenRouter account has insufficient credits. "
                     "Fix: run 'lailaba config set model.provider <provider>' "
    ...
    def _probe_apikey_provider(pname, env_vars, default_url, base_env,
        ...
        try:
            ...
            headers = {
                ...
                "User-Agent": _LAILABA_USER_AGENT,
            }
            ...
            # GOOGLE_API_KEY/GEMINI_API_KEY always shows red in ``lailaba doctor``.
    ...
    # never the legitimate source for `lailaba doctor`.
    ...
    try:
        ...
        if api_disabled:
            issues.append("Run 'lailaba setup' to configure missing API keys for full tool access")
    except Exception as e:
        check_warn("Could not check tool availability", f"({e})")
    
    _section("Skills Hub")
    hub_dir = LAILABA_HOME / "skills" / ".hub"
    if hub_dir.exists():
        ...
        check_warn("Skills Hub directory not initialized", "(run: lailaba skills list)")
    ...
    try:
        import yaml as _yaml
        _mem_cfg_path = LAILABA_HOME / "config.yaml"
    ...
    if not _active_memory_provider:
        ...
        try:
            ...
            if not _honcho_cfg_path.exists():
                ...
                if hcfg.api_key or hcfg.base_url:
                    ...
                    check_warn("Honcho config not found", "run: lailaba memory setup")
            elif not hcfg.enabled:
                check_info(f"Honcho disabled (set enabled: true in {_honcho_cfg_path} to activate)")
            elif not (hcfg.api_key or hcfg.base_url):
                _fail_and_issue(
                    "Honcho API key or base URL not set",
                    "run: lailaba memory setup",
                    "No Honcho API key — run 'lailaba memory setup'",
                    issues,
                )
            else:
        ...
        try:
            ...
            if mem0_key:
                ...
                _fail_and_issue(
                    ...
                    "(set MEM0_API_KEY in .env or run lailaba memory setup)",
        ...
        try:
            ...
            if _provider and _provider.is_available():
                ...
                check_warn(f"{_active_memory_provider} configured but not available", "run: lailaba memory status")
            else:
                check_warn(f"{_active_memory_provider} plugin not found", "run: lailaba memory setup")
        except Exception as _e:
            check_warn(f"{_active_memory_provider} check failed", str(_e))

    try:
        from lailaba_cli.profiles import list_profiles, _get_wrapper_dir, profile_exists
        import re as _re
            ...
            if wrapper_dir.is_dir():
                for wrapper in wrapper_dir.iterdir():
                    ...
                    try:
                        ...
                        if "lailaba -p" in content:
                            _m = _re.search(r"lailaba -p (\S+)", content)
    ...
    if should_fix and fixed_count > 0:
        ...
        print()
        if not should_fix:
            print(color("  Tip: run 'lailaba doctor --fix' to auto-fix what's possible.", Colors.DIM))
    else:
        print(color("─" * 60, Colors.GREEN))
