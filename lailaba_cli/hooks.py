"""lailaba hooks — inspect and manage shell-script hooks.

Usage::

    lailaba hooks list
    lailaba hooks test <event> [--for-tool X] [--payload-file F]
    lailaba hooks revoke <command>
    lailaba hooks doctor

Consent records live under ``~/.lailaba/shell-hooks-allowlist.json`` and
hook definitions come from the ``hooks:`` block in ``~/.lailaba/config.yaml``
...
def hooks_command(args) -> None:
    """Entry point for ``lailaba hooks`` — dispatches to the requested action."""
    sub = getattr(args, "hooks_action", None)

    if not sub:
        print("Usage: lailaba hooks {list|test|revoke|doctor}")
        print("Run 'lailaba hooks --help' for details.")
        return
...
def _cmd_list(_args) -> None:
    ...
    if not specs:
        print("No shell hooks configured in ~/.lailaba/config.yaml.")
        print("See `lailaba hooks --help` or")
                ...
                if entry and entry.get("approved_at"):
                    ...
                    if mtime_now and mtime_at and mtime_now > mtime_at:
                        print(
                            ...
                            f"run `lailaba hooks doctor` to re-validate"
                        )
        print()
...
# stdin a script sees under `lailaba hooks test` and `lailaba hooks doctor`
...
def _print_run_result(result: Dict[str, Any]) -> None:
    ...
    if parsed:
        print(f"      parsed (Lailaba wire shape): {json.dumps(parsed)}")
    else:
...
def _doctor_one(spec, shell_hooks) -> int:
    ...
    if entry and entry.get("script_mtime_at_approval"):
        ...
        if mtime_now and mtime_at and mtime_now > mtime_at:
            ...
            print(f"      ⚠ script modified since approval "
                  ...
                  f"then `lailaba hooks revoke` + re-approve to refresh")
    ...
    # is already allowlisted.  Otherwise `lailaba hooks doctor` would execute
    ...
    if not entry:
        print("      ℹ skipped JSON smoke test — not allowlisted yet. "
              ...
              "then re-run `lailaba hooks doctor`.")
