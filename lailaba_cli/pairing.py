...
Usage:
    lailaba pairing list              # Show all pending + approved users
    lailaba pairing approve <platform> <code>  # Approve a pairing code
    lailaba pairing revoke <platform> <user_id> # Revoke user access
    lailaba pairing clear-pending     # Clear all expired/pending codes
"""

def pairing_command(args):
    """Handle lailaba pairing subcommands."""
    ...
    if action == "list":
        ...
        print("Usage: lailaba pairing {list|approve|revoke|clear-pending}")
        print("Run 'lailaba pairing --help' for details.")
...
def _cmd_approve(store, platform: str, code: str):
    ...
    if result:
        ...
        print(
            ...
            "~/.lailaba/platforms/pairing/_rate_limits.json\n".format(platform)
        )
    else:
        print(f"\n  Code '{code}' not found or expired for platform '{platform}'.")
        print("  Run 'lailaba pairing list' to see pending codes.\n")
