"""MCP picker — interactive `lailaba mcp picker` (also the default `lailaba mcp`).

Lists every catalog entry plus any custom MCP servers the user has added via
``lailaba mcp add``, lets them pick one, and routes to install / enable /
disable / uninstall / configure-tools flows.

Mirrors the `lailaba plugin` picker UX: arrow keys to navigate, ENTER on a row
...
from lailaba_cli.cli_output import prompt_yes_no
from lailaba_cli.curses_ui import curses_single_select
from lailaba_cli.mcp_catalog import (
    CatalogEntry,
    CatalogError,
    catalog_diagnostics,
    install_entry,
...
def _enable_disable(name: str, *, enable: bool) -> None:
    ...
    print(color(
        ...
        "Start a new Lailaba session for changes to take effect.",
        Colors.GREEN,
    ))
...
def _configure_tools(name: str) -> None:
    ...
    """
    import argparse
    from lailaba_cli.mcp_config import cmd_mcp_configure
...
def _print_rows_text(rows: List[_Row]) -> None:
    ...
    as the default output of `lailaba mcp catalog`."""
    if not rows:
        print()
    ...
    print()
    print(color(
        "  Install: lailaba mcp install <name>    Picker: lailaba mcp",
        Colors.DIM,
    ))

    # Surface manifest-version warnings so users know when their Lailaba is
    ...
    if future:
        ...
        for name, _, msg in future:
            print(color(
                f"  ⚠ '{name}' requires a newer Lailaba — run `lailaba update` "
...
def show_catalog() -> None:
    """`lailaba mcp catalog` — print the curated list + custom servers, no interaction."""
    _print_rows_text(_build_rows())


def run_picker() -> None:
    """`lailaba mcp picker` (and default `lailaba mcp`) — interactive selector.
...
def install_by_name(identifier: str) -> int:
    """`lailaba mcp install <name>` — non-interactive entry-point.
    ...
    if entry is None:
        print(color(
            ...
            "Run `lailaba mcp catalog` to see available entries.",
            Colors.RED,
        ))
        return 1
    try:
