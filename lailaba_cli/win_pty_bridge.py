"""Windows ConPTY bridge for the `lailaba dashboard` chat tab.

Drop-in counterpart to ``lailaba_cli.pty_bridge.PtyBridge`` for native
Windows. Mirrors the exact public surface the ``/api/pty`` WebSocket
handler in ``lailaba_cli.web_server`` consumes: ``spawn``, ``read``,
