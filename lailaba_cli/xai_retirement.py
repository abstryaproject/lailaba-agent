...
Pure logic: walks a Lailaba config dict, returns issues for any reference
to a retired xAI model. No I/O, no CLI dependencies — testable in isolation
and reusable from both `lailaba doctor` and a future `lailaba migrate xai`.
"""
...
class RetirementIssue:
    """A reference to a retired xAI model found in a Lailaba config."""
...
def find_retired_xai_refs(config: Dict[str, Any]) -> List[RetirementIssue]:
    """Walk all model slots in a Lailaba config and return retirement issues.
