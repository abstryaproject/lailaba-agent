...
on Japanese locales, etc.).  Lailaba's banners, tool output feed, and slash
...
Entry points (``cli.py`` ``main``, ``lailaba_cli/main.py`` CLI dispatch,
...
def _flip_console_code_page_to_utf8() -> None:
    ...
    is silent — if there's no attached console (e.g. Lailaba is running
...
def configure_windows_stdio() -> bool:
    ...
    Set ``LAILABA_DISABLE_WINDOWS_UTF8=1`` in the environment to opt out
    ...
    if os.environ.get("LAILABA_DISABLE_WINDOWS_UTF8") in {"1", "true", "True", "yes"}:
        _CONFIGURED = True
        return False
    ...
    # Augment PATH with the Lailaba-managed Git install directories so
    ...
    # already-running shells don't see the change — which means lailaba
    ...
    # install without Lailaba-managed PortableGit).
    ...
    # stdin is re-configured for completeness; Lailaba's interactive
...
def _default_windows_editor() -> str:
    ...
    The prompt_toolkit buffer's ``open_in_editor`` and Lailaba's
    ``lailaba config edit`` both honour ``$EDITOR``.  Users who prefer a
    ...
    Set this before launching Lailaba (User env var in Windows Settings, or
    export in a PowerShell profile) and Lailaba picks it up automatically.
    """
    import shutil
...
def _augment_path_with_known_tools() -> None:
    """Prepend well-known Lailaba-managed tool directories to os.environ['PATH'].
    ...
    ``%LOCALAPPDATA%\\lailaba\\git\\bin`` to the User PATH via
    ...
    *spawned* processes only — already-running shells (including the one the
    user invokes ``lailaba`` from right after install) retain their old PATH.

    Any subprocess Lailaba spawns — bash, ``rg``, ``grep``, ``npm`` — inherits
    ...
    Patch-up strategy: add the known Lailaba-managed tool directories to our
    ...
    # see on next launch.
    candidate_dirs = [
        os.path.join(local_appdata, "lailaba", "git", "cmd"),
        os.path.join(local_appdata, "lailaba", "git", "bin"),
        os.path.join(local_appdata, "lailaba", "git", "usr", "bin"),
        # Lailaba venv Scripts directory — host of the lailaba.exe shim itself,
        # also where any pip-installed console scripts land.  Usually already
        # on PATH when the user invokes lailaba, but harmless to include.
        os.path.join(local_appdata, "lailaba", "lailaba-agent", "venv", "Scripts"),
