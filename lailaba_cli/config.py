"""
Configuration management for Lailaba AI.

Config files are stored in ~/.lailaba/ for easy access:
- ~/.lailaba/config.yaml  - All settings (model, toolsets, terminal, etc.)
- ~/.lailaba/.env         - API keys and secrets

This module provides:
- lailaba config          - Show current configuration
- lailaba config edit     - Open config in editor
- lailaba config set      - Set a specific value
- lailaba config wizard   - Re-run setup wizard
"""
...
def _backup_corrupt_config(config_path: Path) -> Optional[Path]:
    ...
    they re-run the setup wizard or ``lailaba config set`` (which rewrites
    ...
    ``config.yaml`` in place: lailaba never silently mutates the user's config,
...
def _warn_config_parse_failure(config_path: Path, exc: Exception) -> None:
    ...
    A YAML parse error in ``~/.lailaba/config.yaml`` causes ``load_config()``
    ...
    ``agent.log`` / ``errors.log`` at WARNING level so ``lailaba logs``
    ...
    logger.warning(msg)
    try:
        sys.stderr.write(f"⚠️  lailaba config: {msg}\n")
        sys.stderr.flush()
    except Exception:
        pass
...
#   the next ``subprocess.run([...])`` Lailaba makes loads attacker code
#   before main().
# * ``PYTHONPATH`` / ``PYTHONHOME`` / ``PYTHONSTARTUP`` /
#   ``PYTHONUSERBASE`` — Python interpreter init. Lailaba itself starts
#   from one of these on every restart.
# * ``NODE_OPTIONS`` / ``NODE_PATH`` — Node interpreter; affects npm,
#   ``lailaba update``, the TUI build.
...
#   on every plugin install / ``lailaba update``.
...
# * ``LAILABA_HOME`` / ``LAILABA_PROFILE`` / ``LAILABA_CONFIG`` /
#   ``LAILABA_ENV`` — Lailaba runtime location flags. Writing these into
...
#
# IMPORTANT: ``LAILABA_*`` overall is NOT blocked. Many legitimate
# integration credentials follow that prefix (LAILABA_LANGFUSE_PUBLIC_KEY,
# LAILABA_SPOTIFY_CLIENT_ID, ...). The
...
_ENV_VAR_NAME_DENYLIST: frozenset[str] = frozenset({
    ...
    # Lailaba runtime location — never via dashboard env writer.
    # NOT a LAILABA_* blanket: integration credentials (LAILABA_GEMINI_*,
    # LAILABA_LANGFUSE_*, LAILABA_SPOTIFY_*, ...) ARE allowed.
    "LAILABA_HOME", "LAILABA_PROFILE", "LAILABA_CONFIG", "LAILABA_ENV",
})
...
def _reject_denylisted_env_var(key: str) -> None:
    ...
    if key in _ENV_VAR_NAME_DENYLIST:
        raise ValueError(
            ...
            "PYTHONPATH, PATH, EDITOR, ...) or Lailaba runtime location "
            "(LAILABA_HOME, LAILABA_PROFILE, ...) cannot be persisted via "
            "the env writer. If you really need this, edit "
            "~/.lailaba/.env directly."
        )
...
_EXTRA_ENV_KEYS = frozenset({
    ...
    "LAILABA_TOOL_PROGRESS", "LAILABA_TOOL_PROGRESS_MODE",
    ...
    # Activation is via plugins.enabled (opt-in through `lailaba plugins enable
    # observability/langfuse` or `lailaba tools → Langfuse`); credentials gate
    # the plugin at runtime.
    "LAILABA_LANGFUSE_ENV",
    "LAILABA_LANGFUSE_RELEASE",
    "LAILABA_LANGFUSE_SAMPLE_RATE",
    "LAILABA_LANGFUSE_MAX_CHARS",
    "LAILABA_LANGFUSE_DEBUG",
    "LANGFUSE_PUBLIC_KEY",
    "LANGFUSE_SECRET_KEY",
    "LANGFUSE_BASE_URL",
})
...
from lailaba_cli.colors import Colors, color
from lailaba_cli.default_soul import DEFAULT_SOUL_MD, is_legacy_template_soul
...
def get_managed_system() -> Optional[str]:
    ...
    raw = os.getenv("LAILABA_MANAGED", "").strip()
    if raw:
    ...
    managed_marker = get_lailaba_home() / ".managed"
...
def is_managed() -> bool:
    """Check if Lailaba is running in package-manager-managed mode.

    Two signals: the LAILABA_MANAGED env var (set by the systemd service),
    or a .managed marker file in LAILABA_HOME (set by the NixOS activation
...
def get_managed_update_command() -> Optional[str]:
    ...
    if managed_system == "Homebrew":
        return "brew upgrade lailaba-agent"
...
def _install_method_project_root(project_root: Optional[Path] = None) -> Path:
    ...
    This is the parent of ``lailaba_cli/`` — i.e. the git checkout for source
    installs, ``/opt/lailaba`` inside the published image, the venv's
    site-packages root for pip installs. It is a property of the running
    interpreter, NOT of ``$LAILABA_HOME``, which is why a code-scoped stamp
...
def detect_install_method(project_root: Optional[Path] = None) -> str:
    """Detect how Lailaba was installed: 'docker', 'nixos', 'homebrew', 'git', or 'pip'.

    Resolution order:
    1. Code-scoped stamp ``<install tree>/.install_method`` (next to the
       running code) — the authoritative marker.
    2. Legacy home-scoped stamp ``$LAILABA_HOME/.install_method`` — read for
       backward compatibility, but a ``docker`` value is IGNORED when we are
       not actually running inside a container (see below).
    3. LAILABA_MANAGED env / .managed marker (NixOS, Homebrew)
    4. .git directory presence -> 'git'
    5. Fallback -> 'pip'

    Why the stamp is code-scoped, not home-scoped (issue: shared ``~/.lailaba``)
    --------------------------------------------------------------------------
    The install method describes *the binary that is running*, but
    ``$LAILABA_HOME`` is a shared DATA directory — the Docker docs deliberately
    bind-mount it (``~/.lailaba:/opt/data``) so config/sessions/memory persist
    and can be shared with a host-side Desktop/CLI install. When a
    containerised gateway and a host install share one ``$LAILABA_HOME``, a
    ...
    ``docker`` and ``lailaba update`` refuses to run ("doesn't apply inside the
    ...
    The supported installs self-identify via the code-scoped stamp:
      ...
      - the published ``nousresearch/lailaba-agent`` image bakes a ``docker``
        stamp into ``/opt/lailaba`` at build time.
    ...
    # 1. Code-scoped stamp — authoritative, immune to shared $LAILABA_HOME.
    try:
    ...
    #    host install whose shared $LAILABA_HOME was stamped by a co-located
    #    container, and honouring it wrongly blocks ``lailaba update``.
    try:
        method = (
            (get_lailaba_home() / ".install_method")
...
def _running_in_container() -> bool:
    """Thin wrapper around ``lailaba_constants.is_container`` (import-safe)."""
    try:
...
def stamp_install_method(method: str, project_root: Optional[Path] = None) -> None:
    ...
    not in ``$LAILABA_HOME``, so that two installs sharing one data directory
    ...
    ``/opt/lailaba`` in the published image, which instead bakes the stamp at
...
def is_uv_tool_install() -> bool:
    """Return True when the *running* Lailaba lives in a ``uv tool`` layout.

    ``uv tool install lailaba-agent`` places the install at
    ``.../uv/tools/lailaba-agent/...`` (default ``~/.local/share/uv/tools``,
    ...
    ``lailaba-agent`` happens to be uv-tool-installed on the machine while
    the *active* Lailaba is a regular pip/venv install, causing
    ``lailaba update`` to upgrade the wrong copy. It would also block on a
    ...
    def _has_uv_tool_marker(path: str) -> bool:
        ...
        return "/uv/tools/lailaba-agent/" in norm + "/"
...
def recommended_update_command_for_method(method: str) -> str:
    ...
    if method == "homebrew":
        return "brew upgrade lailaba-agent"
    if method == "docker":
        return "docker pull nousresearch/lailaba-agent:latest"
    if method == "pip":
        if is_uv_tool_install():
            return "uv tool upgrade lailaba-agent"
        import shutil
        if shutil.which("uv"):
            return "uv pip install --upgrade lailaba-agent"
        return "pip install --upgrade lailaba-agent"
    return "lailaba update"
...
# Long-form text for ``lailaba update`` / ``--check`` when running inside the
# Docker image.  Surfaced by ``cmd_update`` and ``_cmd_update_check`` in
# lailaba_cli/main.py; lives here so the wording stays consistent and we
...
#     — that script installs a *new* host-side Lailaba, it doesn't update
...
✗ ``lailaba update`` doesn't apply inside the Docker container.

Lailaba AI runs as a published image (nousresearch/lailaba-agent), not a
git checkout — the container has no working tree to pull into.  Update by
pulling a fresh image and restarting your container instead:

  docker pull nousresearch/lailaba-agent:latest
  # then restart whatever started the container, e.g.:
  docker compose up -d --force-recreate lailaba-agent
  # or, for ad-hoc runs, exit the current container and `docker run` again

Verify the new version after restart:
  docker run --rm nousresearch/lailaba-agent:latest --version

Notes:
  • If you pinned a specific tag (e.g. ``:v0.14.0``) the ``:latest`` tag
    won't move your container — pull the newer tag you actually want, or
    switch to ``:latest`` / ``:main`` for rolling updates.  See available
    tags at https://hub.docker.com/r/nousresearch/lailaba-agent/tags
  • Your config and session history live under ``$LAILABA_HOME`` (``/opt/data``
...
def format_docker_update_message() -> str:
    """Return the user-facing message for ``lailaba update`` inside Docker.
...
def format_managed_message(action: str = "modify this Lailaba installation") -> str:
    """Build a user-facing error for managed installs."""
    managed_system = get_managed_system() or "a package manager"
    raw = os.getenv("LAILABA_MANAGED", "").strip().lower()

    if managed_system == "NixOS":
        env_hint = "true" if raw in _MANAGED_TRUE_VALUES else raw or "true"
        return (
            f"Cannot {action}: this Lailaba installation is managed by NixOS "
            f"(LAILABA_MANAGED={env_hint}).\n"
            "Edit services.lailaba-agent.settings in your configuration.nix and run:\n"
    ...
    if managed_system == "Homebrew":
        ...
        return (
            f"Cannot {action}: this Lailaba installation is managed by Homebrew "
            f"(LAILABA_MANAGED={env_hint}).\n"
            "Use:\n"
            "  brew upgrade lailaba-agent"
        )

    return (
        f"Cannot {action}: this Lailaba installation is managed by {managed_system}.\n"
        "Use your package manager to upgrade or reinstall Lailaba."
    )
...
def get_container_exec_info() -> Optional[dict]:
    """Read container mode metadata from LAILABA_HOME/.container-mode.

    Returns a dict with keys: backend, container_name, exec_user, lailaba_bin
    or None if container mode is not active, we're already inside the
    container, or LAILABA_DEV=1 is set.

    The .container-mode file is written by the NixOS activation script when
    container.enable = true. It tells the host CLI to exec into the container
    instead of running locally.
    """
    if os.environ.get("LAILABA_DEV") == "1":
        return None
    ...
    container_mode_file = get_lailaba_home() / ".container-mode"
    ...
    container_name = info.get("container_name", "lailaba-agent")
    exec_user = info.get("exec_user", "lailaba")
    lailaba_bin = info.get("lailaba_bin", "/data/current-package/bin/lailaba")

    return {
        "backend": backend,
        "container_name": container_name,
        "exec_user": exec_user,
        "lailaba_bin": lailaba_bin,
    }
...
# Re-export from lailaba_constants — canonical definition lives there.
from lailaba_constants import get_lailaba_home  # noqa: F811,E402
from utils import atomic_replace

def get_config_path() -> Path:
    """Get the main config file path."""
    return get_lailaba_home() / "config.yaml"

def get_env_path() -> Path:
    """Get the .env file path (for API keys)."""
    return get_lailaba_home() / ".env"

def get_project_root() -> Path:
    """Get the project installation directory."""
    return Path(__file__).parent.parent.resolve()

def _resolve_lailaba_uid_gid() -> tuple[Optional[int], Optional[int]]:
    """Read the LAILABA_UID / LAILABA_GID env vars set by Docker deployments.

    Docker containers running Lailaba commonly set these to map the in-container
    user to a host user so volume-mounted state files end up with the right
    ownership. The entrypoint chowns the top-level LAILABA_HOME once, but
    subdirectories created at runtime by ``ensure_lailaba_home()`` (especially
    ...
    if sys.platform == "win32":
        return None, None
    uid_str = os.environ.get("LAILABA_UID", "").strip()
    gid_str = os.environ.get("LAILABA_GID", "").strip()
    try:
...
def _chown_to_lailaba_uid(path) -> None:
    """Chown ``path`` to ``LAILABA_UID:LAILABA_GID`` if those env vars are set.
    ...
    directories created by :func:`ensure_lailaba_home` on Docker deployments.
    See #34107.
    """
    uid, gid = _resolve_lailaba_uid_gid()
...
def _secure_dir(path):
    ...
    permissions (0750) so interactive users in the lailaba group can
    share state with the gateway service.

    The mode can be overridden via the LAILABA_HOME_MODE environment variable
    (e.g. LAILABA_HOME_MODE=0701) for deployments where a web server (nginx,
    caddy, etc.) needs to traverse LAILABA_HOME to reach a served subdirectory.
    The execute-only bit on a directory permits cd-through without exposing
    directory listings.

    Also applies ``LAILABA_UID``/``LAILABA_GID``-based ownership when those env
    ...
    if is_managed():
        return
    try:
        mode_str = os.environ.get("LAILABA_HOME_MODE", "").strip()
    ...
    try:
        ...
        pass
    _chown_to_lailaba_uid(path)


def _is_container() -> bool:
    """Detect if we're running inside a Docker/Podman/LXC container.

    When Lailaba runs in a container with volume-mounted config files, forcing
    ...
    """
    # Explicit opt-out
    if os.environ.get("LAILABA_CONTAINER") or os.environ.get("LAILABA_SKIP_CHMOD"):
        return True
...
def _secure_file(path):
    ...
    permissions.  Set LAILABA_SKIP_CHMOD=1 to force-skip on other systems.
    """
...
def _ensure_default_soul_md(home: Path) -> None:
    """Seed a default SOUL.md into LAILABA_HOME, upgrading legacy empty templates.
...
def ensure_lailaba_home():
    """Ensure ~/.lailaba directory structure exists with secure permissions.
    ...
    if is_managed():
        ...
        try:
            _ensure_lailaba_home_managed(home)
        finally:
            os.umask(old_umask)
    else:
...
def _ensure_lailaba_home_managed(home: Path):
    """Managed-mode variant: verify dirs exist (activation creates them), seed SOUL.md."""
    if not home.is_dir():
        raise RuntimeError(
            f"LAILABA_HOME {home} does not exist. "
...
DEFAULT_CONFIG = {
    ...
    "agent": {
        ...
        # the Lailaba-level retry loop that wraps the whole call.  Lower
        ...
        # prompt's environment-hints block. Lets a host that wraps Lailaba
        ...
        # identity slot (SOUL.md). Empty by default. The LAILABA_ENVIRONMENT_HINT
        # env var overrides this (build-time/container mechanism).
        "environment_hint": "",
        # Coding posture — on interactive coding surfaces (CLI, TUI, desktop
        # app, ACP) in a code workspace, Lailaba adds a coding operating brief
    ...
    "terminal": {
        ...
        #             LAILABA_HOME/home for persistent state (default)
        #   real    — force the real OS-user HOME
        #   profile — force LAILABA_HOME/home when it exists (old strict
        ...
        # skipped. When empty, Lailaba auto-sources ``~/.profile``,
        # ``~/.bash_profile``, and ``~/.bashrc`` (in that order) if the
        # snapshot shell is bash (this is the ``auto_source_bashrc``
        # behaviour — disable with that key if you want strict login-only
        # semantics).
        "shell_init_files": [],
        # When true (default), Lailaba sources the user's shell rc files
        ...
        # docker_env lets you specify exact key-value pairs — useful when Lailaba
        ...
        # bundled Lailaba image, which drops to the `lailaba` user via
    ...
    "browser": {
        ...
        "camofox": {
            # When true, Lailaba sends a stable profile-scoped userId to Camofox
            # so the server maps it to a persistent Firefox profile automatically.
            # When false (default), each session gets a random userId (ephemeral).
            "managed_persistence": False,
            # Optional externally managed Camofox identity. Useful when another
            # app owns the visible browser and Lailaba should operate in it.
            "user_id": "",
            "session_key": "",
    ...
    # Opt in via ``lailaba chat --checkpoints`` or set enabled=True here.
    "checkpoints": {
        "enabled": False,
        ...
        "max_snapshots": 20,
        # Hard ceiling on total ``~/.lailaba/checkpoints/`` size (MB).  When
        ...
        "max_file_size_mb": 10,
        # Auto-maintenance: lailaba sweeps the checkpoint base at startup
    ...
    # AGENTS.md, CLAUDE.md, .lailaba.md, or .cursorrules before Lailaba applies
    ...
    # single read_file page exceeds these limits, Lailaba truncates the
    ...
    # See tools/kanban_tools.py and lailaba_cli/kanban_db.py for the actual
    # implementations. Per-platform notification opt-out is handled by the
    # kanban dashboard (see ``lailaba dashboard`` -> Notifications).
    "kanban": {
    ...
    "auxiliary": {
        ...
        # Invoked by ``lailaba kanban specify`` (single id or --all). Set a
        ...
        # Invoked by ``lailaba kanban decompose`` and the kanban
        ...
        # ``lailaba profile describe <name> --auto`` and the dashboard's
        ...
        # model; override via `lailaba model` → auxiliary → Curator to route
    ...
    "display": {
        ...
        # Which interface bare `lailaba` (and `lailaba chat`) launches by default:
        ...
        # REPL and `--tui` (or LAILABA_TUI=1) forces the TUI regardless of config.
        "interface": "cli",
        # When true, `lailaba --tui` auto-resumes the most recent human-
        # facing session on launch instead of forging a fresh one.
        # Mirrors `lailaba -c` muscle memory.  Default off so existing
        # users aren't surprised.  LAILABA_TUI_RESUME=<id> always wins.
        "tui_auto_resume_recent": False,
        # When true (default), `lailaba --tui` drops a one-time hint
        ...
        # e.g. `model · 68% · ~/projects/lailaba`. Per-platform overrides go under
        ...
        # CLI, TUI, and desktop app. Manage with `lailaba pets`. Disabled until
        ...
        "pet": {
            ...
            # get_lailaba_home()/pets/. Empty → first installed pet.
            "slug": "",
    ...
    "dashboard": {
        ...
        # ``LAILABA_DASHBOARD_OAUTH_CLIENT_ID`` and
        # ``LAILABA_DASHBOARD_PORTAL_URL`` respectively — and the env var
        ...
        # (``LAILABA_DASHBOARD_BASIC_AUTH_USERNAME`` /
        ...
        # ``LAILABA_DASHBOARD_DRAIN_SECRET`` env var (the .env-is-for-secrets
        ...
        # Public URL override (env: ``LAILABA_DASHBOARD_PUBLIC_URL``).
        # When set, this is the complete authority — scheme + host +
        # optional path prefix (e.g. ``https://example.com/lailaba``) —
    ...
    "tts": {
        ...
        "gemini": {
            ...
            # transcript section; Lailaba appends the live transcript when absent.
            "persona_prompt_file": "",
        },
        "xai": {
        ...
        "piper": {
            ...
            # "voices_dir": "",        # Override voice cache dir; default = ~/.lailaba/cache/piper-voices/
    ...
    # a plugin in plugins/context_engine/<name>/ or ~/.lailaba/plugins/.
    "context": {
        "engine": "compressor",
    },
    ...
    # response. If not, Lailaba feeds a continuation prompt back into the
    ...
    "goals": {
        # Max continuation turns before Lailaba auto-pauses the goal and
    ...
    # always goes to ~/.lailaba/skills/.
    "skills": {
        "external_dirs": [],   # e.g. ["~/.agents/skills", "/shared/team-skills"]
        # Substitute ${LAILABA_SKILL_DIR} and ${LAILABA_SESSION_ID} in SKILL.md
    ...
    # cron daemon.
    #
    # See `lailaba curator status` for the last run summary.
    "curator": {
        "enabled": True,
        ...
        # class-level umbrellas. `lailaba curator run --consolidate` overrides
        ...
        # normally restored on every `lailaba update`, so pruning them only
        ...
        # skipped), snapshot ~/.lailaba/skills/ into
        # ~/.lailaba/skills/.curator_backups/<utc-iso>/skills.tar.gz so the
        # user can roll back with `lailaba curator rollback`.
        "backup": {
            "enabled": True,
    ...
    # This section is only needed for lailaba-specific overrides; everything else
    ...
    "whatsapp": {
        ...
        # Default (None) uses the built-in "⚕ *Lailaba AI*" header.
    ...
    "approvals": {
        ...
        # false.  TUI has its own modal overlay (LAILABA_TUI_NO_CONFIRM=1 to
        # opt out there).
    ...
    # replace Lailaba' built-in platform hint for a single messaging platform
    ...
    # stored approval from ~/.lailaba/shell-hooks-allowlist.json.
    # See `website/docs/user-guide/features/hooks.md` for schema + examples.
    "hooks": {},

    # Auto-accept shell-hook registrations without a TTY prompt.  Also
    # toggleable per-invocation via --accept-hooks or LAILABA_ACCEPT_HOOKS=1.
    ...
    "security": {
        ...
        # longer trigger the startup banner. Add via `lailaba doctor --ack
        # <id>`; remove by editing the list directly. See
        # ``lailaba_cli/security_advisories.py`` for the catalog.
        "acked_advisories": [],
        # Allow Lailaba to lazy-install opt-in backend packages from PyPI
    ...
    "cron": {
        ...
        # $LAILABA_HOME/plugins/<name>/) to relocate the trigger — e.g. "chronos",
        ...
        # Also overridable via LAILABA_CRON_MAX_PARALLEL env var.
    ...
    # todos to ready, and fires `lailaba -p <assignee> chat -q ...` for
    ...
    "kanban": {
        ...
        # one `lailaba` launches with no -p flag). This does not control the
        ...
        # decomposition is manual via `lailaba kanban decompose <id>` or
    ...
    "code_execution": {
        ...
        #     lailaba-agent's own python (sys.executable). Maximum isolation
    ...
    #
    # Core Lailaba tools (terminal, read_file, write_file, patch,
    ...
    # Logging — controls file logging to ~/.lailaba/logs/.
    ...
    # update model picker lists without shipping a lailaba-agent release.
    # The default URL is served by the docs site GitHub Pages deploy.
    "model_catalog": {
        "enabled": True,
        "url": "https://lailaba-agent.nousresearch.com/docs/api/model-catalog.json",
        # Disk cache TTL in hours.  Beyond this, the CLI refetches on the
        # next /model or `lailaba model` invocation; network failures
    ...
    "gateway": {
        ...
        # the LAILABA_SCALE_TO_ZERO env stamp) AND messaging is relay-only/absent
        ...
        # system-path denylist (/etc, /proc, ~/.ssh, ~/.aws, ~/.lailaba/.env,
        ...
        # behavior: files must live under the Lailaba cache, under
        ...
        # user. Bridged to LAILABA_MEDIA_DELIVERY_STRICT.
        "strict": False,
        # Extra directories from which model-emitted bare file paths may be
        # uploaded as native gateway attachments. Files inside the Lailaba
        # cache (~/.lailaba/cache/{documents,images,audio,video,screenshots})
        ...
        # to LAILABA_MEDIA_ALLOW_DIRS at gateway startup. Tilde paths are
        ...
        # LAILABA_MEDIA_TRUST_RECENT_FILES. Only consulted when ``strict``
        # is true; in default mode the denylist alone gates delivery.
        "trust_recent_files": True,
        # Recency window in seconds. 600 (10 min) comfortably covers a
        # multi-tool agent turn. Bridged to LAILABA_MEDIA_TRUST_RECENT_SECONDS.
    ...
    # Session storage — controls automatic cleanup of ~/.lailaba/state.db.
    ...
    "sessions": {
        ...
        # default of ``lailaba sessions prune``.
        "retention_days": 90,
        ...
        # rewrites ``~/.lailaba/sessions/session_{sid}.json`` on every turn
    ...
    # ``lailaba update`` behaviour.
    "updates": {
        # Run a full ``lailaba backup``-style zip of LAILABA_HOME before every
        # ``lailaba update``.  Backups land in ``<LAILABA_HOME>/backups/`` and
        # can be restored with ``lailaba import <path>``.  Off by default:
        # zipping a large LAILABA_HOME (sessions DB, caches, skills) can add
        # minutes to every update.  The #48200 incident — a ``lailaba update
        ...
        "backup_keep": 5,
        # What `lailaba update` does with uncommitted local changes to the
    ...
    "lsp": {
        ...
        #              ``<LAILABA_HOME>/lsp/bin/`` on first use.
    ...
    # `lailaba tools`. These settings tune the backing Responses API call.
    "x_search": {
    ...
    # rather than storing them in ~/.lailaba/.env.
    "secrets": {
        "bitwarden": {
            ...
            # in ~/.lailaba/.env (or your shell) and never in config.yaml.
            ...
            # ~/.lailaba/bin/ on first use.  When False you must install
            ...
            # `lailaba secrets bitwarden setup`.
            "server_url": "",
        },
    },
    ...
    "computer_use": {
        ...
        # by default upstream. Lailaba disables it for our users unless they
        # explicitly opt in here. When false (default), Lailaba sets
    ...
    # Lailaba Desktop (Electron app) launch options. These only affect
    # `lailaba desktop`; they do not touch the CLI/gateway.
    "desktop": {
        ...
        # Bridged to the LAILABA_DESKTOP_DISABLE_GPU env var the Electron app reads.
        "disable_gpu": "auto",
    },
...
OPTIONAL_ENV_VARS = {
    ...
    "AZURE_FOUNDRY_BASE_URL": {
        "description": "Azure Foundry base URL (set via 'lailaba model' for endpoint-specific config)",
    ...
    "TOOL_GATEWAY_USER_TOKEN": {
        "description": "Explicit Nous Subscriber access token for tool-gateway requests (optional; otherwise read from the Lailaba auth store)",
    ...
    "SLACK_ALLOWED_USERS": {
        "description": "Comma-separated Slack member IDs allowed to use Lailaba, e.g. U01ABC2DEF3. Without this, Slack may connect but deny messages by default.",
    ...
    "MATRIX_USER_ID": {
        "description": "Matrix user ID (e.g. @lailaba:example.org)",
    ...
    "MATRIX_DEVICE_ID": {
        "description": "Stable Matrix device ID for E2EE persistence across restarts (e.g. LAILABA_BOT)",
    ...
    "IRC_CHANNEL": {
        "description": "IRC channel to join (e.g. #lailaba)",
        "prompt": "IRC channel",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "IRC_NICKNAME": {
        "description": "Bot nickname on IRC (default: lailaba-bot)",
        "prompt": "IRC nickname",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "API_SERVER_MODEL_NAME": {
        "description": "Model name advertised on /v1/models. Defaults to the profile name (or 'lailaba-agent' for the default profile). Useful for multi-user setups with OpenWebUI.",
    ...
    "GATEWAY_PROXY_URL": {
        "description": "URL of a remote Lailaba API server to forward messages to (proxy mode). When set, the gateway handles platform I/O only — all agent work is delegated to the remote server. Use for Docker E2EE containers that relay to a host agent. Also configurable via gateway.proxy_url in config.yaml.",
        "prompt": "Remote Lailaba API server URL (e.g. http://192.168.1.100:8642)",
        "url": None,
        "password": False,
        "category": "messaging",
        "advanced": True,
    ...
    "GATEWAY_PROXY_KEY": {
        "description": "Bearer token for authenticating with the remote Lailaba API server (proxy mode). Must match the API_SERVER_KEY on the remote host.",
    ...
    # LAILABA_TOOL_PROGRESS and LAILABA_TOOL_PROGRESS_MODE are deprecated —
    ...
    "LAILABA_EPHEMERAL_SYSTEM_PROMPT": {
...
def get_missing_skill_config_vars() -> List[Dict[str, Any]]:
    ...
    Scans all enabled skills for ``metadata.lailaba.config`` entries, then checks
    ...
    try:
        ...
        # should never break `lailaba update`.  Skill-config prompting is a
...
def _normalize_custom_provider_entry(
    ...
    *,
    ...
    if isinstance(models, dict) and models:
        ...
        # Hand-edited configs (and older Lailaba versions) write ``models`` as
...
def get_custom_provider_context_length(
    ...
    used by:
      ...
      * ``lailaba_cli.model_switch.resolve_display_context_length`` (``/model`` confirmation display)
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    if config is None:
        try:
            ...
            return [ConfigIssue("error", "Could not load config.yaml", "Run 'lailaba setup' to create a valid config")]
    ...
    if cp and not model_cfg:
        issues.append(ConfigIssue(
            "warning",
            "custom_providers defined but no 'model' section — Lailaba won't know which provider to use",
            "Add a model section:\n"
            "  model:\n"
            "    provider: custom\n"
...
def print_config_warnings(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    lines.append("  \033[2mRun 'lailaba doctor' for fix suggestions.\033[0m")
    sys.stderr.write("\n".join(lines) + "\n\n")
...
def warn_deprecated_cwd_env_vars(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    if lines:
        hint_path = os.environ.get("LAILABA_HOME", "~/.lailaba")
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    if current_ver < 4:
        ...
        if "tool_progress" not in display:
            old_enabled = get_env_value("LAILABA_TOOL_PROGRESS")
            old_mode = get_env_value("LAILABA_TOOL_PROGRESS_MODE")
            if old_enabled and old_enabled.lower() in {"false", "0", "no"}:
                display["tool_progress"] = "off"
                results["config_added"].append("display.tool_progress=off (from LAILABA_TOOL_PROGRESS=false)")
            elif old_mode and old_mode.lower() in {"new", "all"}:
                display["tool_progress"] = old_mode.lower()
                results["config_added"].append(f"display.tool_progress={old_mode.lower()} (from LAILABA_TOOL_PROGRESS_MODE)")
            else:
    ...
    if current_ver < 5:
        ...
        if "timezone" not in config:
            old_tz = os.getenv("LAILABA_TIMEZONE", "")
            if old_tz and old_tz.strip():
                config["timezone"] = old_tz.strip()
                results["config_added"].append(f"timezone={old_tz.strip()} (from LAILABA_TIMEZONE)")
            else:
                config["timezone"] = ""
    ...
    if current_ver < 21:
        ...
        if "enabled" not in plugins_cfg:
            ...
            # Scan ``$LAILABA_HOME/plugins/`` for currently installed user plugins.
            grandfathered: List[str] = []
            try:
                user_plugins_dir = get_lailaba_home() / "plugins"
            ...
            if not quiet:
                if grandfathered:
                    ...
                    print(
                        ...
                        "Use `lailaba plugins enable <name>` to activate."
                    )
    ...
    # settings in their `config.yaml`, and `lailaba curator status` has no
    ...
    #   3. Creates `~/.lailaba/logs/curator/` if missing (belt-and-suspenders
    #      on top of ensure_lailaba_home() — old profiles that predate this
    #      migration still benefit).
    if current_ver < 23:
        try:
            curator_dir = get_lailaba_home() / "logs" / "curator"
    ...
    if isinstance(raw_mcp_servers, dict):
        try:
            from lailaba_cli.mcp_security import validate_mcp_server_entry as _validate_mcp_server_entry
        except Exception:
    ...
    if new_var_names and interactive and not quiet:
        ...
        if new_and_unset:
            ...
            if answer in {"y", "yes"}:
                ...
                print("  Set later with: lailaba config set <key> <value>")
    ...
    # metadata.lailaba.config in their SKILL.md frontmatter.
...
def _normalize_root_model_keys(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    intuitive name OpenAI-SDK / LiteLLM users reach for, and ``lailaba config set``
    ...
    backends) — while display paths (``lailaba status``/``dump``) read ``name``
...
def cfg_get(cfg: Optional[Dict[str, Any]], *keys: str, default: Any = None) -> Any:
    ...
    ubiquitous ``cfg_path = _lailaba_home / "config.yaml"`` local variable
...
def read_raw_config() -> Dict[str, Any]:
    """Read ~/.lailaba/config.yaml as-is, without merging defaults or migrating.
...
def load_config() -> Dict[str, Any]:
    """Load configuration from ~/.lailaba/config.yaml.
    ...
    (which change ``LAILABA_HOME`` and therefore ``get_config_path()``)
    don't collide.
...
def _load_config_impl(*, want_deepcopy: bool) -> Dict[str, Any]:
    with _CONFIG_LOCK:
        ...
        # cache signature so editing /etc/lailaba/config.yaml invalidates the
...
#   openai-codex (OAuth — lailaba auth) — OpenAI Codex
#   nous         (OAuth — lailaba auth) — Nous Portal
...
def save_config(
    ...
    *,
    ...
    """Save configuration to ~/.lailaba/config.yaml.\n
...
def _parse_env_value(raw_value: str) -> str:
    """Parse the small .env value subset Lailaba writes itself."""
...
def load_env() -> Dict[str, str]:
    """Load environment variables from ~/.lailaba/.env.
    ...
    interactive menu render (`lailaba tools`, `lailaba setup`, status
    panels). Sanitisation is O(lines × known-keys), so re-parsing the
    same file on every call was burning ~300ms of CPU per `lailaba tools`
...
def _sanitize_env_lines(lines: list) -> list:
    ...
    split on real Lailaba env var names, avoiding false positives from values
...
def sanitize_env_file() -> int:
    """Read, sanitize, and rewrite ~/.lailaba/.env in place.
...
def save_env_value(key: str, value: str):
    """Save or update a value in ~/.lailaba/.env."""
    if is_managed():
...
def remove_env_value(key: str) -> bool:
    """Remove a key from ~/.lailaba/.env and os.environ.
...
def reload_env() -> int:
    """Re-read ~/.lailaba/.env into os.environ. Returns count of vars updated.

    Adds/updates vars that changed and removes vars that were deleted from
    the .env file (but only vars known to Lailaba — OPTIONAL_ENV_VARS and
    ...
    for key, value in env_vars.items():
        if os.environ.get(key) != value:
            os.environ[key] = value
            count += 1
    # Remove known Lailaba vars that are no longer in .env
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ~/.lailaba/.env or environment."""
...
def show_config():
    ...
    print(color("│              ⚕ Lailaba Configuration                    │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    from lailaba_cli.auth import get_anthropic_key
    ...
    # Warn on stale LAILABA_MAX_ITERATIONS ghost in .env that disagrees with
    ...
    try:
        _env_ghost = load_env().get("LAILABA_MAX_ITERATIONS")
        if _env_ghost is not None and str(_env_ghost).strip() != str(_cfg_max_turns).strip():
            print(color(
                f"                ⚠ .env has stale LAILABA_MAX_ITERATIONS={_env_ghost} "
                f"(run 'lailaba doctor --fix' to remove)",
                Colors.YELLOW,
            ))
    except Exception:
        pass
    ...
    print(color("  lailaba config edit     # Edit config file", Colors.DIM))
    print(color("  lailaba config set <key> <value>", Colors.DIM))
    print(color("  lailaba setup           # Run setup wizard", Colors.DIM))
    print()
...
def set_config_value(key: str, value: str):
    ...
    # so a fresh `lailaba config set model.api_base ...` lands on the canonical
    ...
    # — e.g. `lailaba config set model.api_key cfut_...` routes to config.yaml
...
def config_command(args):
    ...
    if subcmd is None or subcmd == "show":
        ...
        if not key or value is None:
            print("Usage: lailaba config set <key> <value>")
            print()
            print("Examples:")
            print("  lailaba config set model anthropic/claude-sonnet-4")
            print("  lailaba config set terminal.backend docker")
            print("  lailaba config set OPENROUTER_API_KEY sk-or-...")
            sys.exit(1)
        ...
        if missing_config:
            ...
            print("    Run 'lailaba config migrate' to add them")
        ...
        print("  lailaba config           Show current configuration")
        print("  lailaba config edit      Open config in editor")
        print("  lailaba config set <key> <value>   Set a config value")
        print("  lailaba config check     Check for missing/outdated config")
        print("  lailaba config migrate   Update config with new options")
        print("  lailaba config path      Show config file path")
        print("  lailaba config env-path  Show .env file path")
        sys.exit(1)
...
# ``_inject_profile_env_vars`` surfaces them in ``lailaba config`` UI so users
