"""``lailaba dashboard`` subcommand parser.

Extracted verbatim from ``lailaba_cli/main.py:main()`` (god-file Phase 2).
...
def build_dashboard_parser(
    ...
    dashboard_parser = subparsers.add_parser(
        ...
        description="Launch the Lailaba AI web dashboard for managing config, API keys, and sessions",
    )
    ...
    # for `lailaba dashboard` cmdlines and SIGTERM them directly — the same
    # path `lailaba update` uses to clean up stale dashboards.
    dashboard_parser.add_argument(
        "--stop",
        action="store_true",
        help="Stop all running lailaba dashboard processes and exit",
    )
    dashboard_parser.add_argument(
        "--status",
        action="store_true",
        help="List running lailaba dashboard processes and exit",
    )
    # Backward-compat shim: older Lailaba desktop app shells (<= 0.15.x) spawn the
    # backend as `lailaba dashboard --no-open --tui --host ... --port ...`. The
    ...
    # showed "Lailaba couldn't start" with no actionable cause. Accept and silently
    ...
    # `lailaba dashboard register` — register a self-hosted dashboard OAuth
    # client with Nous Portal and write the client_id into ~/.lailaba/.env.
    # Nested subparser so bare `lailaba dashboard` keeps launching the server
    ...
    dashboard_register_parser = dashboard_subparsers.add_parser(
        ...
        description=(
            ...
            "LAILABA_DASHBOARD_OAUTH_CLIENT_ID into ~/.lailaba/.env, and prints "
            "how to engage the login gate. Requires being logged in (lailaba setup)."
        ),
    )
    ...
    dashboard_register_parser.add_argument(
        ...
        help=(
            ...
            "https://lailaba.example.com/auth/callback. Omit for localhost-only use."
        ),
    )
    dashboard_register_parser.add_argument(
        "--portal-url",
        ...
        help=(
            ...
            "portal. Also settable via LAILABA_DASHBOARD_PORTAL_URL. Mainly for "
