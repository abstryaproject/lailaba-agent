"""
Profile management for multiple isolated Lailaba instances.

Each profile is a fully independent LAILABA_HOME directory with its own
config.yaml, .env, memory, sessions, skills, gateway, cron, and logs.
Profiles live under ``~/.lailaba/profiles/<name>/`` by default.

The "default" profile is ``~/.lailaba`` itself — backward compatible,
zero migration needed.

Usage::

    lailaba profile create coder          # fresh profile + bundled skills
    lailaba profile create coder --clone  # also copy config, .env, SOUL.md, skills
    lailaba profile create coder --clone-all  # full copy of source profile
    coder chat                           # use via wrapper alias
    lailaba -p coder chat                 # or via flag
    lailaba profile use coder             # set as sticky default
    lailaba profile delete coder          # remove profile + alias + service
"""
...
_PROFILE_DIRS = [
    ...
    # See lailaba_constants.get_subprocess_home().
    "home",
]
...
# default profile (``~/.lailaba``).  Named profiles never contain these
# directories at root, so the exclusion is gated to avoid silently dropping
# user data from a named-profile source.
#
# Rationale per item:
#   lailaba-agent  — git repo checkout (~84 MB source + ~3 GB venv)
...
#   backups             — `lailaba backup` archives
...
# Marker file written by `lailaba profile create --no-skills`.  When present in
# a profile's root, callers of seed_profile_skills() (fresh-create, `lailaba
...
# `lailaba skills install` or drop SKILL.md files into the profile's skills/.
...
def _clone_all_copytree_ignore(source_dir: Path):
    ...
    Three categories:
      ...
      2. Root-level entries in ``_CLONE_ALL_DEFAULT_EXCLUDE_ROOT`` — known
         Lailaba infrastructure directories that only the default profile
         (``~/.lailaba``) ever contains.  Gated on ``source_dir`` actually
    ...
    is_default_source = source_resolved == _get_default_lailaba_home().resolve()
...
# Directories/files to exclude when exporting the default (~/.lailaba) profile.
# The default profile contains infrastructure (repo checkout, worktrees, DBs,
# caches, binaries) that named profiles don't have.  We exclude those so the
# export is a portable, reasonable-size archive of actual profile data.
_DEFAULT_EXPORT_EXCLUDE_ROOT = frozenset({
    # Infrastructure
    "lailaba-agent",         # repo checkout (multi-GB)
    ".worktrees",           # git worktrees
...
_RESERVED_NAMES = frozenset({
    "lailaba", "default", "test", "tmp", "root", "sudo",
})

# Lailaba subcommands that cannot be used as profile names/aliases
_LAILABA_SUBCOMMANDS = frozenset({
...
def _get_profiles_root() -> Path:
    ...
    Anchored to the lailaba root, NOT to the current LAILABA_HOME
    (which may itself be a profile).  This ensures ``coder profile list``
    can see all profiles.

    In Docker/custom deployments where LAILABA_HOME points outside
    ``~/.lailaba``, profiles live under ``LAILABA_HOME/profiles/`` so
    they persist on the mounted volume.
    """
    return _get_default_lailaba_home() / "profiles"


def _get_default_lailaba_home() -> Path:
    """Return the default (pre-profile) LAILABA_HOME path.

    In standard deployments this is ``~/.lailaba``.
    In Docker/custom deployments where LAILABA_HOME is outside ``~/.lailaba``
    (e.g. ``/opt/data``), returns LAILABA_HOME directly.
    """
...
def _get_active_profile_path() -> Path:
    ...
    return _get_default_lailaba_home() / "active_profile"
...
def validate_profile_name(name: str) -> None:
    ...
    if name in _RESERVED_NAMES:
        raise ValueError(
            ...
            f"the Lailaba installation itself or a common system binary.  "
            f"Pick a different name."
        )


def get_profile_dir(name: str) -> Path:
    """Resolve a profile name to its LAILABA_HOME directory."""
    canon = normalize_profile_name(name)
    if canon == "default":
        return _get_default_lailaba_home()
...
def check_alias_collision(name: str) -> Optional[str]:
    ...
    Checks: reserved names, lailaba subcommands, existing binaries in PATH.
    """
    canon = normalize_profile_name(name)
    if canon in _RESERVED_NAMES:
        return f"'{canon}' is a reserved name"
    if canon in _LAILABA_SUBCOMMANDS:
        return f"'{canon}' conflicts with a lailaba subcommand"
...
def create_wrapper_script(name: str, target: Optional[str] = None) -> Optional[Path]:
    ...
    if is_windows:
        ...
        try:
            lailaba_exe = shutil.which("lailaba") or "lailaba"
            wrapper_path.write_text(f'#!/bin/sh\nexec {shlex.quote(lailaba_exe)} -p {profile} "$@"\n')
...
def _migrate_profile_config_if_outdated(profile_dir: Path) -> None:
    ...
    ``_config_version``) or that is simply older than the running Lailaba. If we
    ...
    try:
        ...
        # not be migrated. The next `lailaba doctor --fix` can still surface the
...
def find_alias_for_profile(profile_name: str) -> Optional[str]:
    ...
    alias whose body invokes ``lailaba -p <profile>``. When the alias name equals
    the profile name this is trivial, but a custom alias (``lailaba profile alias
...
def _read_distribution_meta(profile_dir: Path) -> tuple:
    ...
    never break ``lailaba profile list`` for an unrelated profile.
    """
...
def _check_gateway_running(profile_dir: Path) -> bool:
    ...
    *this* reader — which is exactly the case for a dashboard process that is
    ...
    agree.  Parameterized by ``profile_dir`` so it never mutates ``LAILABA_HOME``.
    """
    try:
...
# ``config.yaml``. ``config.yaml`` is the user-facing Lailaba config
...
def list_profiles() -> List[ProfileInfo]:
    ...
    # Default profile
    default_home = _get_default_lailaba_home()
...
def profiles_to_serve(multiplex: bool) -> List[Tuple[str, Path]]:
    """Return the ``(profile_name, lailaba_home)`` pairs a gateway should serve.
    ...
    - ``multiplex=True``: returns the default profile plus every valid named
      profile under ``profiles/``, each paired with its own LAILABA_HOME.
    ...
    The returned ``lailaba_home`` is the path to pass to
    ``set_lailaba_home_override`` when scoping a turn to that profile.
    """
    active = get_active_profile_name() or "default"
    if not multiplex:
        return [(active, get_profile_dir(active))]

    serve: List[Tuple[str, Path]] = [("default", _get_default_lailaba_home())]
...
def create_profile(
    ...
    no_skills:
        ...
        a marker file so ``lailaba update`` skips re-seeding this profile's
    ...
    if canon == "default":
        raise ValueError(
            "Cannot create a profile named 'default' — it is the built-in profile (~/.lailaba)."
        )
    ...
    if clone_all and source_dir:
        # Full copy of source profile (exclude sibling ~/.lailaba/profiles/)
        shutil.copytree(
            source_dir,
            profile_dir,
    ...
    # Keys pages, `lailaba -p <name> auth add`) had no file until first
    ...
    if not env_path.exists():
        try:
            env_path.write_text(
                "# Per-profile secrets for this Lailaba profile.\n"
    ...
    if not soul_path.exists():
        try:
            from lailaba_cli.default_soul import DEFAULT_SOUL_MD
            soul_path.write_text(DEFAULT_SOUL_MD, encoding="utf-8")
        except Exception:
            pass  # best-effort — don't fail profile creation over this

    # Write the opt-out marker so seed_profile_skills() and `lailaba update`'s
    ...
    if no_skills:
        try:
            (profile_dir / NO_BUNDLED_SKILLS_MARKER).write_text(
                ...
                "(`lailaba profile create --no-skills`).\n"
    ...
    # Cloned configs can be older than the running Lailaba (or predate schema
    ...
    if description and description.strip():
        try:
            ...
            pass  # non-fatal — user can describe later with `lailaba profile describe`

    # Phase 4: when running inside a container under s6, register the
    # new profile's gateway as a runtime s6 service so
    # `lailaba -p <profile> gateway start` can supervise it via
...
def seed_profile_skills(profile_dir: Path, quiet: bool = False) -> Optional[dict]:
    ...
    Uses subprocess because sync_skills() caches LAILABA_HOME at module level.
    Returns the sync result dict, or None on failure.

    Profiles that opted out of bundled skills (via ``lailaba profile create
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "LAILABA_HOME": str(profile_dir)},
...
def backfill_profile_envs(quiet: bool = False) -> List[str]:
    ...
    default_env = _get_default_lailaba_home() / ".env"
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    if canon == "default":
        raise ValueError(
            "Cannot delete the default profile (~/.lailaba).\n"
            "To remove everything, use: lailaba uninstall"
        )
...
def _maybe_register_gateway_service(profile_name: str) -> None:
    ...
    ``LAILABA_HOME`` and binds the port resolved by ``gateway/config.py``
    ...
    try:
        ...
        from lailaba_cli.service_manager import get_service_manager
...
def _cleanup_gateway_service(name: str, profile_dir: Path) -> None:
    ...
    # Temporarily set LAILABA_HOME so _profile_suffix resolves correctly
    old_home = os.environ.get("LAILABA_HOME")
    try:
        os.environ["LAILABA_HOME"] = str(profile_dir)
        from lailaba_cli.gateway import get_service_name, get_launchd_plist_path
        ...
        if old_home is not None:
            os.environ["LAILABA_HOME"] = old_home
        elif "LAILABA_HOME" in os.environ:
            del os.environ["LAILABA_HOME"]
...
def set_active_profile(name: str) -> None:
    ...
    Writes to ``~/.lailaba/active_profile``. Use ``"default"`` to clear.
    """
    canon = normalize_profile_name(name)
    validate_profile_name(canon)
    if canon != "default" and not profile_exists(canon):
        raise FileNotFoundError(
            ...
            f"Create it with: lailaba profile create {canon}"
        )
...
def get_active_profile_name() -> str:
    """Infer the current profile name from LAILABA_HOME.

    Returns ``"default"`` if LAILABA_HOME is not set or points to ``~/.lailaba``.
    Returns the profile name if LAILABA_HOME points into ``~/.lailaba/profiles/<name>``.
    Returns ``"custom"`` if LAILABA_HOME is set to an unrecognized path.
    """
    from lailaba_constants import get_lailaba_home
    lailaba_home = get_lailaba_home()
    resolved = lailaba_home.resolve()

    default_resolved = _get_default_lailaba_home().resolve()
...
def export_profile(name: str, output_path: str) -> Path:
    ...
    if canon == "default":
        # The default profile IS ~/.lailaba itself — its parent is ~/ and its
        # directory name is ".lailaba", not "default".  We stage a clean copy
...
def import_profile(archive_path: str, name: Optional[str] = None) -> Path:
    ...
    if not inferred_name:
        raise ValueError(
            ...
            "Specify it explicitly: lailaba profile import <archive> --name <name>"
        )
    ...
    # dir.  Importing as "default" would target ~/.lailaba itself — disallow
    # that and guide the user toward a named profile.
    canon = normalize_profile_name(inferred_name)
    validate_profile_name(canon)
    if canon == "default":
        raise ValueError(
            "Cannot import as 'default' — that is the built-in root profile (~/.lailaba). "
            "Specify a different name: lailaba profile import <archive> --name <name>"
        )
    ...
    with tempfile.TemporaryDirectory(prefix="lailaba_profile_import_") as tmpdir:
...
def _migrate_honcho_profile_host(old_name: str, new_name: str, new_dir: Path) -> None:
    ...
    old_host = f"lailaba_{old_name}"
    legacy_old_host = f"lailaba.{old_name}"
    new_host = f"lailaba_{new_name}"

    candidates = [
        new_dir / "honcho.json",
        _get_default_lailaba_home() / "honcho.json",
    ...
    for path in candidates:
        ...
        if isinstance(block, dict) and "aiPeer" not in block:
            if source_host.startswith("lailaba_"):
...
def resolve_profile_env(profile_name: str) -> str:
    """Resolve a profile name to a LAILABA_HOME path string.

    Called early in the CLI entry point, before any lailaba modules
    are imported, to set the LAILABA_HOME environment variable.
    """
    canon = normalize_profile_name(profile_name)
    validate_profile_name(canon)
    profile_dir = get_profile_dir(canon)
