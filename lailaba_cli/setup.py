"""
Interactive setup wizard for Lailaba AI.
...
_DOCS_BASE = "https://lailaba-agent.nousresearch.com/docs"
...
# display_lailaba_home imported lazily at call sites (stale-module safety during lailaba update)
...
from lailaba_cli.cli_output import (  # noqa: E402
    print_error,
    print_info,
    print_success,
    print_warning,
)
from lailaba_cli.secret_prompt import masked_secret_prompt  # noqa: E402
...
def print_noninteractive_setup_guidance(reason: str | None = None) -> None:
    ...
    print()
    print(color("⚕ Lailaba Setup — Non-interactive mode", Colors.CYAN, Colors.BOLD))
    print()
    if reason:
        print_info(reason)
    print_info("The interactive wizard cannot be used here.")
    print()
    print_info("Configure Lailaba using environment variables or config commands:")
    print_info("  lailaba config set model.provider custom")
    print_info("  lailaba config set model.base_url http://localhost:8080/v1")
    print_info("  lailaba config set model.default your-model-name")
    print()
    print_info("Or set OPENROUTER_API_KEY / OPENAI_API_KEY in your environment.")
    print_info("Run 'lailaba setup' in an interactive terminal to use the full wizard.")
    print()
...
def is_noninteractive() -> bool:
    ...
    ``LAILABA_NONINTERACTIVE=1`` (see ``lailaba_cli/web_server.py``). In that
    ...
    """
    return os.environ.get("LAILABA_NONINTERACTIVE", "").strip().lower() in {
        "1",
        "true",
        "yes",
        "on",
...
def prompt_yes_no(question: str, default: bool = True) -> bool:
    ...
    Non-interactive callers (``LAILABA_NONINTERACTIVE=1`` or a closed/redirected
...
def prompt_checklist(title: str, items: list, pre_selected: list = None) -> list:
    ...
    from lailaba_cli.curses_ui import curses_checklist
...
def _prompt_api_key(var: dict):
    ...
    if value:
        ...
        print_warning("  Skipped (configure later with 'lailaba setup')")


def _print_setup_summary(config: dict, lailaba_home):
    ...
    if _vision_backends:
        ...
        tool_status.append(("Vision (image analysis)", False, "run 'lailaba setup' to configure"))
    ...
    if subscription_features.image_gen.managed_by_nous:
        ...
        try:
            ...
            from lailaba_cli.plugins import _ensure_plugins_discovered
    ...
    # Video generation — opt-in via `lailaba tools` → Video Generation.
    ...
    if subscription_features.video_gen.managed_by_nous:
        ...
        try:
            ...
            from lailaba_cli.plugins import _ensure_plugins_discovered as _ensure_plugins
            _ensure_plugins()
            _video_backend = None
    ...
    if subscription_features.tts.managed_by_nous:
        ...
        if neutts_ok:
            ...
            tool_status.append(("Text-to-Speech (NeuTTS — not installed)", False, "run 'lailaba setup tts'"))
        ...
        if kittentts_ok:
            ...
            tool_status.append(("Text-to-Speech (KittenTTS — not installed)", False, "run 'lailaba setup tts'"))
    else:
    ...
    if subscription_features.modal.managed_by_nous:
        ...
        if subscription_features.modal.direct_override:
            ...
            tool_status.append(("Modal Execution", False, "run 'lailaba setup terminal'"))
    ...
    # Spotify (OAuth via lailaba auth spotify — check auth.json, not env vars)
    try:
    ...
    if disabled_tools:
        print_warning(
            "Some tools are disabled. Run 'lailaba setup tools' to configure them,"
        )
    ...
    print(
        f"   {color('Data:', Colors.YELLOW)}      {lailaba_home}/cron/, sessions/, logs/"
    )
    print()
    ...
    print()
    print(f"   {color('lailaba setup', Colors.GREEN)}          Re-run the full wizard")
    print(f"   {color('lailaba setup model', Colors.GREEN)}    Change model/provider")
    print(f"   {color('lailaba setup terminal', Colors.GREEN)} Change terminal backend")
    print(f"   {color('lailaba setup gateway', Colors.GREEN)}  Configure messaging")
    print(f"   {color('lailaba setup tools', Colors.GREEN)}    Configure tool providers")
    print()
    print(f"   {color('lailaba config', Colors.GREEN)}         View current settings")
    print(
        f"   {color('lailaba config edit', Colors.GREEN)}    Open config in your editor"
    )
    print(f"   {color('lailaba config set <key> <value>', Colors.GREEN)}")
    ...
    print()
    print(f"   {color('lailaba', Colors.GREEN)}              Start chatting")
    print(f"   {color('lailaba gateway', Colors.GREEN)}      Start messaging gateway")
    print(f"   {color('lailaba doctor', Colors.GREEN)}       Check for issues")
    print()
...
# between `lailaba tools` and `lailaba setup tools`).
...
def setup_model_provider(config: dict, *, quick: bool = False):
    ...
    Delegates to ``cmd_model()`` (the same flow used by ``lailaba model``)
    ...
    provider added to ``lailaba model`` is automatically available here.
    ...
    # Delegate to the shared lailaba model flow — handles provider picker,
    # credential prompting, model selection, and config persistence.
    from lailaba_cli.main import select_provider_and_model
    try:
        ...
        print_info("You can try again later with: lailaba model")
    ...
    # on demand via `lailaba auth add`, `lailaba setup` vision, and
    # `lailaba setup tts`. This keeps both quick and full setup thin.
...
def _xai_oauth_logged_in_for_setup() -> bool:
    ...
    through ``lailaba model`` -> xAI Grok OAuth (SuperGrok / Premium+).
    """
    try:
        from lailaba_cli.auth import get_xai_oauth_auth_status
...
def _setup_tts_provider(config: dict):
    ...
    if selected == "nous-openai":
        ...
        if get_env_value("VOICE_TOOLS_OPENAI_KEY") or get_env_value("OPENAI_API_KEY"):
            print_warning(
                "Direct OpenAI credentials are still configured and may take precedence until removed from ~/.lailaba/.env."
            )
    ...
    if selected == "neutts":
        ...
        # via the Lailaba auth store) > existing XAI_API_KEY > prompt the user.
            ...
            if choice_idx == 0:
                ...
                if api_key:
                    ...
                    print_warning(
                        ...
                        f"via lailaba setup model or {_dhh()}/.env to use xAI TTS. "
                        "Falling back to Edge TTS."
                    )
                    selected = "edge"
            else:
...
def setup_tts(config: dict):
    """Standalone TTS setup (for 'lailaba setup tts')."""
...
def setup_terminal_backend(config: dict):
    ...
    print_info("Choose where Lailaba runs shell commands and code.")
    ...
    if selected_backend == "local":
        ...
        # configurable later via `lailaba setup terminal` / config.yaml.
        ...
        # Image and resource limits use defaults; tune via `lailaba setup terminal`.
...
def _apply_default_agent_settings(config: dict):
    ...
    # bridges it into LAILABA_MAX_ITERATIONS at startup. We no longer write
    ...
    remove_env_value("LAILABA_MAX_ITERATIONS")
    ...
    print_info("  Run `lailaba setup agent` later to customize.")
...
def setup_agent_settings(config: dict):
    ...
    try:
        ...
        if max_iter > 0:
            ...
            # LAILABA_MAX_ITERATIONS from agent.max_turns at startup.
...
def _setup_telegram_auto_result():
    ...
    try:
        from lailaba_cli.telegram_managed_bot import auto_setup_telegram_bot_result
    except ImportError:
        return None

    profile_name: str | None = None
    try:
        profile_name = _profile_name_from_lailaba_home(Path(get_lailaba_home()))
    except Exception:
        pass
...
def _profile_name_from_lailaba_home(lailaba_home) -> str | None:
    """Return the active profile name when LAILABA_HOME is a profile dir."""
    if lailaba_home.parent.name == "profiles":
        return lailaba_home.name
    return None
...
def _setup_telegram():
    ...
    print()
    print_info("📬 Home Channel: where Lailaba delivers cron job results,")
...
def _setup_bluebubbles():
    ...
    print_info("Connects Lailaba to iMessage via BlueBubbles — a free, open-source")
...
def _setup_qqbot():
    ...
    from lailaba_cli.gateway import _setup_qqbot as _gateway_setup_qqbot
...
def _setup_webhooks():
    ...
    print()
    print_info("   Full guide: https://lailaba-agent.nousresearch.com/docs/user-guide/messaging/webhooks/")
    print()
    ...
    print_info("   https://lailaba-agent.nousresearch.com/docs/user-guide/messaging/webhooks/#configuring-routes")
    print()
    print_info("   Open config in your editor:  lailaba config edit")
    print_info("   Open config in your editor:  lailaba config edit")


def setup_gateway(config: dict):
    """Configure messaging platform integrations."""
    from lailaba_cli.gateway import _all_platforms, _platform_status, _configure_platform

    print_header("Messaging Platforms")
    print_info("Connect to messaging platforms to chat with Lailaba from anywhere.")
    ...
    if not selected:
        print_info("No platforms selected. Run 'lailaba setup gateway' later to configure.")
        return
    ...
    if any_messaging:
        ...
        if missing_home:
            ...
            for plat in missing_home:
                print_info(
                    f"     lailaba config set {plat.upper()}_HOME_CHANNEL <channel_id>"
                )
        ...
        if supports_systemd and has_legacy_lailaba_units():
            print_legacy_unit_warning()
            print()
        ...
        if service_running:
            ...
            if prompt_yes_no(
                ...
                if supports_systemd and os.geteuid() == 0:  # windows-footgun: ok — guarded by supports_systemd (Linux only)
                    print_info("  Or as a boot-time service: lailaba gateway install --system")
                print_info("  Or run in foreground:  lailaba gateway")
        else:
            from lailaba_constants import is_container
            if is_container():
                print_info("Start the gateway to bring your bots online:")
                print_info("   lailaba gateway run          # Run as container main process")
                print_info("")
                ...
                print_info("Start the gateway to bring your bots online:")
                print_info("   lailaba gateway              # Run in foreground")
...
def setup_tools(config: dict, first_install: bool = False):
    ...
    Both `lailaba setup tools` and `lailaba tools` use the same flow:
...
def _model_section_has_credentials(config: dict) -> bool:
    ...
    Sources of truth:
      * ``PROVIDER_REGISTRY`` in ``lailaba_cli.auth`` — lists every supported
    ...
    """
    try:
        from lailaba_cli.auth import get_active_provider
...
def _get_section_config_summary(config: dict, section_key: str) -> Optional[str]:
    ...
    ``get_env_value`` is the module-level import from lailaba_cli.config
    ...
    if section_key == "model":
        ...
        from lailaba_cli.gateway import _all_platforms, _platform_status
...
_OPENCLAW_SCRIPT = (
    ...
    / "openclaw-migration"
    / "scripts"
    / "openclaw_to_lailaba.py"
)


def _load_openclaw_migration_module():
    """Load the openclaw_to_lailaba migration script as a module.
    ...
    spec = importlib.util.spec_from_file_location(
        "openclaw_to_lailaba", _OPENCLAW_SCRIPT
    )
...
# Config values may have different semantics between OpenClaw and Lailaba.
# Instruction/context files (.md) can contain incompatible setup procedures.
_HIGH_IMPACT_KIND_KEYWORDS = {
    "gateway": "⚠ Gateway/messaging — this will configure Lailaba to use your OpenClaw messaging channels",
    "telegram": "⚠ Telegram — this will point Lailaba at your OpenClaw Telegram bot",
    "slack": "⚠ Slack — this will point Lailaba at your OpenClaw Slack workspace",
    "discord": "⚠ Discord — this will point Lailaba at your OpenClaw Discord bot",
    "whatsapp": "⚠ WhatsApp — this will point Lailaba at your OpenClaw WhatsApp connection",
    "config": "⚠ Config values — OpenClaw settings may not map 1:1 to Lailaba equivalents",
...
def _print_migration_preview(report: dict):
    ...
    if conflict_items:
        print(color("  Would overwrite (conflicts with existing Lailaba config):", Colors.YELLOW))
    ...
    if warnings_shown:
        ...
        print()
        print(color("  Note: OpenClaw config values may have different semantics in Lailaba.", Colors.YELLOW))
        print(color("  For example, OpenClaw's tool_call_execution: \"auto\" ≠ Lailaba's yolo mode.", Colors.YELLOW))
        print(color("  Instruction files (.md) from OpenClaw may contain incompatible procedures.", Colors.YELLOW))
        print()


def _offer_openclaw_migration(lailaba_home: Path) -> bool:
    ...
    print_info("Lailaba can preview what would be imported before making any changes.")
    print()

    if not prompt_yes_no("Would you like to see what can be imported?", default=True):
        print_info(
            "Skipping migration. You can run it later with: lailaba claw migrate --dry-run"
        )
        return False
    ...
    try:
        ...
        dry_migrator = mod.Migrator(
            ...
            target_root=lailaba_home.resolve(),
    ...
    if not prompt_yes_no("Proceed with migration?", default=False):
        print_info(
            "Migration cancelled. You can run it later with: lailaba claw migrate"
        )
        print_info(
    ...
    # Execute the migration — overwrite=False so existing Lailaba configs are
    ...
    try:
        migrator = mod.Migrator(
            ...
            execute=True,
            workspace_target=None,
            overwrite=False,  # preserve existing Lailaba config
            migrate_secrets=True,
            output_dir=None,
            selected_options=selected,
            preset_name="full",
    ...
    if conflicts:
        print_info(f"Skipped {conflicts} item(s) that already exist in Lailaba (use lailaba claw migrate --overwrite to force).")
    if skipped:
...
def _run_portal_one_shot(config: dict) -> None:
    ...
    Wired into ``lailaba setup --portal`` and ``lailaba portal``. This is the
    ...
    Lailaba session — model selected, provider set, and web/image/tts/browser
    tools routed via their Portal sub — without being told to run
    ``lailaba setup`` and hunt for the quick-setup option.
    ...
    (``_run_first_time_quick_setup``) and the same one ``lailaba model`` runs
    when you pick Nous. Routing through it (instead of hand-rolling the auth +
    provider write here) means ``lailaba portal`` always offers a model picker,
    ...
    print(color("│     ⚕ Lailaba Setup — Nous Portal (one-shot)             │", Colors.MAGENTA))
    print(
        color(
            "└─────────────────────────────────────────────────────────┘",
            Colors.MAGENTA,
    ...
    # setup calls, so `lailaba portal` == quick setup's Nous step.
    try:
        from lailaba_cli.main import _model_flow_nous
        ...
        print_info("  You can retry later with `lailaba portal`.")
        return
    except Exception as exc:
        logger.debug("_model_flow_nous error during `lailaba portal`: %s", exc)
        print()
    ...
    print_info("  Run `lailaba portal info` to inspect routing.")
    print_info("  Run `lailaba` to start chatting.")
...
def run_setup_wizard(args):
    ...
    Supports full, quick, and section-specific setup:
      lailaba setup           — full or quick (auto-detected)
      lailaba setup model     — just model/provider
      lailaba setup tts       — just text-to-speech
      lailaba setup terminal  — just terminal backend
      lailaba setup gateway   — just messaging platforms
      lailaba setup tools     — just tool configuration
      lailaba setup agent     — just agent settings
    """
    from lailaba_cli.config import is_managed, managed_error
    if is_managed():
    ...
    if section:
        for key, label, func in SETUP_SECTIONS:
            if key == section:
                ...
                print(color(f"│     ⚕ Lailaba Setup — {label:<34s} │", Colors.MAGENTA))
                print(
                    color(
                        "└─────────────────────────────────────────────────────────┘",
                        Colors.MAGENTA,
    ...
    print(
        color(
            "│             ⚕ Lailaba AI Setup Wizard                │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    print(
        color(
            "│  Let's configure your Lailaba AI installation.       │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    if is_existing:
        ...
        if quick_requested:
            _run_quick_setup(config, lailaba_home)
            return

        print()
        print_header("Reconfigure")
        print_success("You already have Lailaba configured.")
        print_info("Running the full wizard — each prompt shows your current value.")
        print_info("Press Enter to keep it, or type a new value to change it.")
        print_info("")
        print_info("Tip: jump straight to a section with 'lailaba setup model|terminal|")
        ...
        migration_ran = _offer_openclaw_migration(lailaba_home)
        if migration_ran:
            config = load_config()

        setup_mode = prompt_choice(
            "How would you like to set up Lailaba?",
            [
        ...
        if setup_mode == 0:
            _run_first_time_quick_setup(config, lailaba_home, is_existing)
            return
        if setup_mode == 2:
            _run_blank_slate_setup(config, lailaba_home, is_existing)
            return
    ...
    print_info(f"Data folder:  {lailaba_home}")
    print_info(f"Install dir:  {PROJECT_ROOT}")
    print()
    print_info("You can edit these files directly or use 'lailaba config edit'")
    ...
    # Tune later with `lailaba setup agent`.
    if not is_existing:
    ...
    _print_setup_summary(config, lailaba_home)


def _run_first_time_quick_setup(config: dict, lailaba_home, is_existing: bool):
    ...
    settings, tools); the user can customize later via ``lailaba setup <section>``
    or switch providers with ``lailaba model``.
    """
    from lailaba_cli.config import load_config
    ...
    gateway_choice = prompt_choice(
        ...
        [
            ...
            "Skip — set up later with 'lailaba setup gateway'",
        ],
        0,
    )
    ...
    print()
    print_info("  Configure all settings:    lailaba setup")
    if gateway_choice != 0:
        print_info("  Connect Telegram/Discord:  lailaba setup gateway")
    print()
...
def _blank_slate_minimal_toolsets(config: dict):
    ...
    2. ``agent.disabled_toolsets`` — a global hard-suppression list (applied last
       ...
       quirks. The user re-enables any of them later via ``lailaba tools`` (which
    ...
    try:
        ...
        from lailaba_cli.tools_config import CONFIGURABLE_TOOLSETS, _get_plugin_toolset_keys
...
def _blank_slate_minimize_config(config: dict):
    ...
    Everything here is opt-in afterwards via ``lailaba setup agent`` /
    ``lailaba config set``. We keep only what's needed to run.
    """
...
def _run_blank_slate_setup(config: dict, lailaba_home, is_existing: bool):
    ...
    if path == 0:
        ...
        # `lailaba update` runs don't re-inject them.
        try:
        ...
        print_info("  Enable tools:        lailaba tools")
        print_info("  Seed skills:         lailaba skills opt-in --sync")
        print_info("  Add MCP servers:     lailaba mcp add")
        print_info("  Enable plugins:      lailaba plugins")
        print_info("  Tune agent settings: lailaba setup agent")
        print()
        _print_setup_summary(config, lailaba_home)
        return

    # ── Walkthrough path — opt in to each capability ──
    _blank_slate_walkthrough(config, lailaba_home)


def _blank_slate_walkthrough(config: dict, lailaba_home):
    ...
    try:
        ...
        if seed_skills:
            ...
            print_info("`lailaba update` runs from re-injecting them. Opt back in any")
            print_info("time with `lailaba skills opt-in --sync`.")
    except Exception as exc:
    ...
    if prompt_yes_no("Open the tool selector to enable more tools?", default=False):
        ...
        print_info("Keeping the minimal toolset. Add tools later with `lailaba tools`.")
    ...
    if prompt_yes_no("Review and enable built-in plugins now?", default=False):
        print_info("Manage plugins with `lailaba plugins list` / `lailaba plugins install`.")
    else:
        print_info("No plugins enabled. Add later with `lailaba plugins`.")

    # ── MCP servers (off unless chosen) ──
    print()
    print_header("MCP Servers")
    if prompt_yes_no("Add an MCP server now?", default=False):
        print_info("Add servers with `lailaba mcp add <name> --url ... | --command ...`.")
    else:
        print_info("No MCP servers configured. Add later with `lailaba mcp add`.")
    ...
    print()
    print_success("Blank Slate setup complete — minimal agent ready.")
    print_info("  Enable more tools:   lailaba tools")
    print_info("  Seed skills:         lailaba skills opt-in --sync")
    print_info("  Add MCP servers:     lailaba mcp add")
    print_info("  Tune agent settings: lailaba setup agent")
    print()
...
def _run_quick_setup(config: dict, lailaba_home):
    ...
    if not has_anything_missing:
        ...
        print()
        print_info("Run 'lailaba setup' and choose 'Full Setup' to reconfigure,")
    ...
    if missing_messaging:
        print()
        print_header("Messaging Platforms")
        print_info("Connect Lailaba to messaging apps to chat from anywhere.")
        print_info("You can configure these later with 'lailaba setup gateway'.")
