...
description: "Send text from any shell script, cron job, CI hook, or monitoring daemon to Telegram, Discord, Slack, Signal, and other platforms using `lailaba send`."
---

# Pipe Script Output to Messaging Platforms

`lailaba send` is a small, scriptable CLI that pushes a message to any
messaging platform Lailaba is already configured for. Think of it as a
...
- Piping any tool's output anywhere (`make | lailaba send --to slack:#builds`)

The command reuses the same credentials and platform adapters that `lailaba
...
lailaba send --to slack:#eng --subject "[CI] build.log" --file build.log

# Thread target (Telegram topic, Discord thread)
lailaba send --to telegram:-1001234567890:17585 "threaded reply"

# List every configured target
lailaba send --list

# Filter by platform
lailaba send --list telegram
```
...
Any platform Lailaba ships adapters for works as a target:
...
`lailaba send` resolves the message body in this order:

1. **Positional argument** — `lailaba send --to telegram "hi"`
2. **`--file PATH`** — `lailaba send --to telegram --file msg.txt`
3. **Piped stdin** — `echo hi | lailaba send --to telegram`

When stdin is a TTY (no pipe), Lailaba does **not** wait for input — you'll
...
if [ "$ram_pct" -ge 85 ]; then
  lailaba send --to telegram --subject "⚠ MEMORY WARNING" \
    "RAM ${ram_pct}% on $(hostname)"
fi
```

Because `lailaba send` reuses your Lailaba config, the same script works on
any host where Lailaba is installed — no need to export bot tokens into
...
`lailaba send`. If the Python interpreter can't load because the box is
...
if ./scripts/deploy.sh; then
  lailaba send --to slack:#deploys "✅ ${CI_COMMIT_SHA:0:7} deployed"
else
  tail -n 100 deploy.log | lailaba send \
...
0 9 * * * /usr/local/bin/generate-metrics.sh \
  | /home/me/.lailaba/bin/lailaba send \
...
```bash
./train.py --epochs 200 && \
  lailaba send --to telegram "training done" || \
  lailaba send --to telegram "training failed (exit $?)"
```
...
lailaba send --to telegram --quiet "keepalive" || {
  echo "Telegram delivery failed" >&2
  exit 1
}

# Capture the message ID for later editing / threading
msg_id=$(lailaba send --to discord:#ops --json "build started" \
  | jq -r .message_id)
```

---

## Does `lailaba send` Need the Gateway Running?

**Usually no.** For any bot-token platform — Telegram, Discord, Slack,
Signal, SMS, WhatsApp Cloud API, and most others — `lailaba send` calls
the platform's REST endpoint directly using credentials from
`~/.lailaba/.env` and `~/.lailaba/config.yaml`. It's a standalone subprocess
...
pointing at the gateway; start it with `lailaba gateway start` and retry.
...
# Machine-readable
lailaba send --list --json
```

The listing is built from `~/.lailaba/channel_directory.json`, which the
gateway refreshes every few minutes while it's running. If you see
"no channels discovered yet", start the gateway once (`lailaba gateway
...
| Approach | Multi-platform | Reuses Lailaba creds | Needs gateway | Best for |
|----------|----------------|---------------------|---------------|----------|
| `lailaba send` | ✅ | ✅ | No (bot-token) | Everything below |
...
`lailaba send` is intentionally the simplest possible surface. If you need
...
If you just need to pipe a raw string, reach for `lailaba send`.
...
- [Gateway Internals](/developer-guide/gateway-internals) —
  the delivery router that `lailaba send` shares with cron delivery.
