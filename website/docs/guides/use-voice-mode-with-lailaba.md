---
sidebar_position: 8
title: "Use Voice Mode with Lailaba"
description: "A practical guide to setting up and using Lailaba voice mode across CLI, Telegram, Discord, and Discord voice channels"
---

# Use Voice Mode with Lailaba
...
- you want Lailaba sitting in a Discord voice channel for live conversation
...
There are really three different voice experiences in Lailaba.
...
## Step 1: make sure normal Lailaba works first
...
Lailaba supports both local and cloud speech stacks.
...
### If you use `lailaba setup`

If you choose NeuTTS in the setup wizard, Lailaba checks whether `neutts` is already installed. If it is missing, the wizard tells you NeuTTS needs the Python package `neutts` and the system package `espeak-ng`, offers to install them for you, installs `espeak-ng` with your platform package manager, and then runs:
...
4. Lailaba transcribes and responds
...
- asking Lailaba to structure your thoughts in real time

#### Accessibility / low-typing sessions

If typing is inconvenient, voice mode is one of the fastest ways to stay in the full Lailaba loop.

## Tuning CLI behavior

### Silence threshold

If Lailaba starts/stops too aggressively, tune:
...
Lailaba stays a normal chat bot, but can speak replies.
...
- you want Lailaba to function like a portable research or ops assistant
...
Lailaba joins a Discord VC, listens to user speech, transcribes it, runs the normal agent pipeline, and speaks replies back into the channel.
...
- Lailaba detects speech boundaries
- transcripts are posted in the associated text channel
- Lailaba responds in text and audio
...
1. get text Lailaba working
2. install `lailaba-agent[voice]`
