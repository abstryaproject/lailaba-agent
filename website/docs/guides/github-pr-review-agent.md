...
┌───────────────────────────────────────────────────────────────────┐
│                                                                   │
│   Cron Timer  ──▶  Lailaba AI  ──▶  GitHub API  ──▶  Review     │
...
If you have a public endpoint available, check out [Automated GitHub PR Comments with Webhooks](./webhook-github-pr-review.md) — GitHub pushes events to Lailaba instantly when PRs are opened or updated.
:::
...
- **Lailaba AI installed** — see the [Installation guide](/getting-started/installation)
- **Gateway running** for cron jobs:
  ```bash
  lailaba gateway install   # Install as a service
  # or
...
Use `deliver: "local"` to save reviews to `~/.lailaba/cron/output/`. Great for testing before wiring up notifications.
:::
...
Make sure Lailaba can access GitHub. Start a chat:
...
```
Run: gh pr list --repo NousResearch/lailaba-agent --state open --limit 3
```
...
Still in the chat, ask Lailaba to review a real PR:

```
Review this pull request. Read the diff, check for bugs, security issues,
and code quality. Be specific about line numbers and quote problematic code.

Run: gh pr diff 3888 --repo NousResearch/lailaba-agent
```
...
A skill gives Lailaba consistent review guidelines that persist across sessions and cron runs. Without one, review quality varies.

```bash
mkdir -p ~/.lailaba/skills/code-review
```

Create `~/.lailaba/skills/code-review/SKILL.md`:
...
Verify it loaded — start `lailaba` and you should see `code-review` in the skills list at startup.

---

## Step 4: Teach It Your Conventions

This is what makes the reviewer actually useful. Start a session and teach Lailaba your team's standards:
...
2. Teach Lailaba your conventions via memory (Step 4)
3. The more context it has about your stack, the better the reviews

### Cron job doesn't run
```bash
lailaba gateway status    # Is the gateway running?
lailaba cron list         # Is the job enabled?
```
...
- **[Build a Plugin](/guides/build-a-lailaba-plugin)** — wrap the review logic into a shareable plugin
