...
This tutorial walks you through setting up a Telegram bot powered by Lailaba AI that multiple team members can use. By the end, your team will have a shared AI assistant they can message for help with code, research, system administration, and anything else — secured with per-user authorization.
...
- **Lailaba AI installed** on a server or VPS (not your laptop — the bot needs to stay running). Follow the [installation guide](/getting-started/installation) if you haven't yet.
- **A Telegram account** for yourself (the bot owner)
- **An LLM provider configured** — at minimum, an API key for OpenAI, Anthropic, or another supported provider in `~/.lailaba/.env`

:::tip
A $5/month VPS is plenty for running the gateway. Lailaba itself is lightweight — the LLM API calls are what cost money, and those happen remotely.
:::
...
2. **Send `/newbot`** — BotFather will ask you two things:
   - **Display name** — what users see (e.g., `Team Lailaba Assistant`)
   - **Username** — must end in `bot` (e.g., `myteam_lailaba_bot`)
...
4. **Set a description** (optional but recommended):
   ...
   ```
   Team AI assistant powered by Lailaba AI. DM me for help with code, research, debugging, and more.
   ```
...
Add these lines to `~/.lailaba/.env`:
...
```
[Gateway] Starting Lailaba Gateway...
...
# View live logs
journalctl --user -u lailaba-gateway -f
...
:::tip macOS PATH
The launchd plist captures your shell PATH at install time so gateway subprocesses can find tools like Node.js and ffmpeg. If you install new tools later, re-run `lailaba gateway install` to update the plist.
:::
...
```bash
lailaba gateway stop && lailaba gateway start
```
...
lailaba pairing revoke telegram 987654321
...
**Option 2:** Set it manually in `~/.lailaba/.env`:
...
Control how much detail the bot shows when using tools. In `~/.lailaba/config.yaml`:
...
Customize how the bot communicates by editing `~/.lailaba/SOUL.md`:

For a full guide, see [Use SOUL.md with Lailaba](/guides/use-soul-with-lailaba).
...
```bash
# From the CLI
lailaba cron list          # View all scheduled jobs
lailaba cron status        # Check if scheduler is running
...
| What | Location |
|------|----------|
| Gateway logs | `journalctl --user -u lailaba-gateway` (Linux) or `~/.lailaba/logs/gateway.log` (macOS) |
| Cron job output | `~/.lailaba/cron/output/{job_id}/{timestamp}.md` |
| Cron job definitions | `~/.lailaba/cron/jobs.json` |
| Pairing data | `~/.lailaba/pairing/` |
| Session history | `~/.lailaba/sessions/` |
