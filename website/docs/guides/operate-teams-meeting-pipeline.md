...
```bash
lailaba teams-pipeline validate
```
...
```bash
lailaba teams-pipeline token-health
lailaba teams-pipeline token-health --force-refresh
```
...
```bash
lailaba teams-pipeline subscriptions
```

### Renew near-expiry subscriptions

```bash
lailaba teams-pipeline maintain-subscriptions
lailaba teams-pipeline maintain-subscriptions --dry-run
```
...
#### Option 1: Lailaba cron (recommended if you already run the Lailaba gateway)

Lailaba ships a built-in cron scheduler. The `--no-agent` mode runs a script as the job (rather than using an LLM), and `--script` must point at a file under `~/.lailaba/scripts/`. First create the script:

```bash
mkdir -p ~/.lailaba/scripts
cat > ~/.lailaba/scripts/maintain-teams-subscriptions.sh <<'EOF'
#!/usr/bin/env bash
exec lailaba teams-pipeline maintain-subscriptions
EOF
chmod +x ~/.lailaba/scripts/maintain-teams-subscriptions.sh
```

Then register a script-only cron job that runs every 12 hours (gives 6x headroom against the 72h expiry window):

```bash
lailaba cron create "0 */12 * * *" \
...
Create `/etc/systemd/system/lailaba-teams-pipeline-maintain.service`:

```ini
[Unit]
Description=Lailaba Teams pipeline subscription maintenance
After=network-online.target

[Service]
Type=oneshot
User=lailaba
EnvironmentFile=/etc/lailaba/env
ExecStart=/usr/local/bin/lailaba teams-pipeline maintain-subscriptions
```

And `/etc/systemd/system/lailaba-teams-pipeline-maintain.timer`:

```ini
[Unit]
Description=Run Lailaba Teams pipeline subscription maintenance every 12 hours
...
sudo systemctl enable --now lailaba-teams-pipeline-maintain.timer
systemctl list-timers lailaba-teams-pipeline-maintain.timer
```

#### Option 3: Plain crontab

```cron
0 */12 * * * /usr/local/bin/lailaba teams-pipeline maintain-subscriptions >> /var/log/lailaba/teams-pipeline-maintain.log 2>&1
```

Make sure the cron environment has the `MSGRAPH_*` credentials. Simplest fix: source `~/.lailaba/.env` at the top of a wrapper script that crontab calls.
...
```bash
lailaba teams-pipeline subscriptions   # should show expirationDateTime advanced
lailaba teams-pipeline maintain-subscriptions --dry-run   # should show "0 expiring soon" most of the time
```
...
```bash
lailaba teams-pipeline list
lailaba teams-pipeline list --status failed
lailaba teams-pipeline show <job-id>
```

### Replay a stored job

```bash
lailaba teams-pipeline run <job-id>
```

### Dry-run meeting artifact fetches

```bash
lailaba teams-pipeline fetch --meeting-id <meeting-id>
lailaba teams-pipeline fetch --join-web-url "<join-url>"
```
...
- run `lailaba teams-pipeline maintain-subscriptions --dry-run`
- inspect `lailaba teams-pipeline list --status failed`
...
- run `lailaba teams-pipeline validate`
...
Check:
- whether you manually replayed a job with `lailaba teams-pipeline run`
...
- [ ] `lailaba teams-pipeline validate` returns an OK snapshot
- [ ] `lailaba teams-pipeline token-health --force-refresh` succeeds
- [ ] **`maintain-subscriptions` is scheduled** (Lailaba cron, systemd timer, or crontab — see [Automating subscription renewal](#automating-subscription-renewal-required-for-production)). Without this, Graph subscriptions silently expire within 72 hours.
