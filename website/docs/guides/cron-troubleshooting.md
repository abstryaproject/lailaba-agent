...
description: "Diagnose and fix common Lailaba cron issues — jobs not firing, delivery failures, skill loading errors, and performance problems"
---
...
If you're expecting jobs to fire automatically, you need a running gateway (`lailaba gateway` for foreground, or `lailaba gateway start` for the installed service). For one-off debugging, you can manually trigger a tick with `lailaba cron tick`.
...
```bash
date
lailaba cron list   # Compare next_run times with local time
```
...
| Target | Requires |
|--------|----------|
| `telegram` | `TELEGRAM_BOT_TOKEN` in `~/.lailaba/.env` |
| `discord` | `DISCORD_BOT_TOKEN` in `~/.lailaba/.env` |
| `slack` | `SLACK_BOT_TOKEN` in `~/.lailaba/.env` |
...
| `local` | Write access to `~/.lailaba/cron/output/` |
...
If delivery fails, the job still runs — it just won't send anywhere. Check `lailaba cron list` for updated `last_error` field (if available).
...
Skills must be installed before they can be attached to cron jobs. If a skill is missing, install it first with `lailaba skills install <skill-name>` or via `/skills` in the CLI.

### Check 2: Check skill name vs. skill folder name

Skill names are case-sensitive and must match the installed skill's folder name. If your job specifies `ai-funding-daily-report` but the skill folder is `ai-funding-daily-report`, confirm the exact name from `lailaba skills list`.
...
2. `~/.lailaba/logs/agent.log` for scheduler messages (or `errors.log` for warnings)
3. The job's `last_run` metadata via `lailaba cron list`

### Check 2: Common error patterns

**"No such file or directory" for scripts**
The `script` path must be an absolute path (or relative to the Lailaba config directory). Verify:
```bash
ls ~/.lailaba/scripts/your-script.py   # Must exist
lailaba cron edit <job_id> --script ~/.lailaba/scripts/your-script.py
```

**"Skill not found" at job execution**
The skill must be installed on the machine running the scheduler. If you move between machines, skills don't automatically sync — reinstall them with `lailaba skills install <skill-name>`.
...
Jobs are stored in `~/.lailaba/cron/jobs.json`. If this file is not readable/writable by your user, the scheduler will fail silently:

```bash
ls -la ~/.lailaba/cron/jobs.json
chmod 600 ~/.lailaba/cron/jobs.json   # Your user should own it
```
...
```bash
lailaba cron list                    # Show all jobs, states, next_run times
lailaba cron run <job_id>            # Schedule for next tick (for testing)
lailaba cron edit <job_id>           # Fix configuration issues
lailaba logs                         # View recent Lailaba logs
lailaba skills list                  # Verify installed skills
```
...
1. Run the job with `lailaba cron run <job_id>` (fires on next gateway tick) and watch for errors in the chat output
2. Check `~/.lailaba/logs/agent.log` for scheduler messages and `~/.lailaba/logs/errors.log` for warnings
3. Open an issue at [github.com/NousResearch/lailaba-agent](https://github.com/NousResearch/lailaba-agent) with:
