---
sidebar_position: 7
title: "Use SOUL.md with Lailaba"
description: "How to use SOUL.md to shape Lailaba AI's default voice, what belongs there, and how it differs from AGENTS.md and /personality"
---

# Use SOUL.md with Lailaba

`SOUL.md` is the **primary identity** for your Lailaba instance. It's the first thing in the system prompt — it defines who the agent is, how it speaks, and what it avoids.

If you want Lailaba to feel like the same assistant every time you talk to it — or if you want to replace the Lailaba persona entirely with your own — this is the file to use.
...
- personality
- communication style
- how direct or warm Lailaba should be
- what Lailaba should avoid stylistically
- how Lailaba should relate to uncertainty, disagreement, and ambiguity

In short:
- `SOUL.md` is about who Lailaba is and how Lailaba speaks
...
Lailaba now uses only the global SOUL file for the current instance:

```text
~/.lailaba/SOUL.md
```

If you run Lailaba with a custom home directory, it becomes:
...
Lailaba automatically seeds a starter `SOUL.md` for you if one does not already exist.

That means most users now begin with a real file they can read and edit immediately.

Important:
- if you already have a `SOUL.md`, Lailaba does not overwrite it
- if the file exists but is empty, Lailaba adds nothing from it to the prompt

## How Lailaba uses it

When Lailaba starts a session, it reads `SOUL.md` from `LAILABA_HOME`, scans it for prompt-injection patterns, truncates it if needed, and uses it as the **agent identity** — slot #1 in the system prompt. This means SOUL.md completely replaces the built-in default identity text.
...
That alone can noticeably change how Lailaba feels.
...
Lailaba already tries to be helpful and clear. `SOUL.md` should add real personality and style, not restate obvious defaults.
...
# Style
How Lailaba should sound.

# Avoid
What Lailaba should not do.

# Defaults
How Lailaba should behave when ambiguity appears.
```
...
Then restart Lailaba or start a new session.
...
### I edited SOUL.md but Lailaba still sounds the same

Check:
- you edited `~/.lailaba/SOUL.md` or `$LAILABA_HOME/SOUL.md`
...
### Lailaba is ignoring parts of my SOUL.md
