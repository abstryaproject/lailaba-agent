...
The ACP adapter wraps Lailaba' synchronous `AIAgent` in an async JSON-RPC stdio server.
...
```text
lailaba acp / lailaba-acp / python -m acp_adapter
...
The Zed ACP Registry path launches the same adapter through `uvx --from 'lailaba-agent[acp]==<version>' lailaba-acp`, pointed at the `lailaba-agent` PyPI release.
...
- `allow_once` -> Lailaba `once`
- `allow_always` -> Lailaba `always`
- reject options -> Lailaba `deny`

Timeouts and bridge failures deny by default.

### Tool rendering helpers

`acp_adapter/tools.py` maps Lailaba tools to ACP tool kinds and builds editor-facing content.
...
new_session(cwd)
  -> create SessionState
  -> create AIAgent(platform="acp", enabled_toolsets=["lailaba-acp"])
...
Instead it reuses Lailaba' runtime resolver:

- `acp_adapter/auth.py`
- `lailaba_cli/runtime_provider.py`

So ACP advertises and uses the currently configured Lailaba provider/credentials. It also always advertises a terminal setup auth method (`lailaba-setup`, args `--setup`) so first-run registry clients can open Lailaba' interactive model/provider configuration before starting a normal ACP session.
...
- ACP sessions are persisted to the shared `~/.lailaba/state.db` (SessionDB) and transparently restored across process restarts; they appear in `session_search`
...
- `toolsets.py` — `lailaba-acp` toolset definition
- `lailaba_cli/main.py` — `lailaba acp` CLI subcommand
- `pyproject.toml` — `[acp]` optional dependency + `lailaba-acp` script
