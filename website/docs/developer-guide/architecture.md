...
sidebar_position: 1
title: "Architecture"
description: "Lailaba AI internals — major subsystems, execution paths, data flow, and where to read next"
---

# Architecture

This page is the top-level map of Lailaba AI internals. Use it to orient yourself in the codebase, then dive into subsystem-specific docs for implementation details.
...
│ lailaba_state.py   │              │ Browser (5 backends)  │
...
├── lailaba_state.py           # SQLite session/state database with FTS5
├── lailaba_constants.py       # LAILABA_HOME, profile-aware paths
...
│
├── lailaba_cli/               # CLI subcommands and setup
│   ├── main.py               # Entry point — all `lailaba` subcommands (large file)
...
│   ├── skills_config.py      # lailaba skills — enable/disable per platform
│   ├── skills_hub.py         # /skills slash command
│   ├── tools_config.py       # lailaba tools — enable/disable per platform
│   ├── plugins.py            # PluginManager — discovery, loading, hooks
│   ├── callbacks.py          # Terminal callbacks (clarify, sudo, approval)
│   └── gateway.py            # lailaba gateway start/stop
│
...
Three discovery sources: `~/.lailaba/plugins/` (user), `.lailaba/plugins/` (project), and pip entry points. Plugins register tools, hooks, and CLI commands through a context API. Two specialized plugin types exist: memory providers (`plugins/memory/`) and context engines (`plugins/context_engine/`). Both are single-select — only one of each can be active at a time, configured via `lailaba plugins` or `config.yaml`.

→ [Plugin Guide](/guides/build-a-lailaba-plugin), [Memory Provider Plugin](./memory-provider-plugin.md)
...
Exposes Lailaba as an editor-native agent over stdio/JSON-RPC for VS Code, Zed, and JetBrains.
...
| **Profile isolation** | Each profile (`lailaba -p <name>`) gets its own LAILABA_HOME, config, memory, sessions, and gateway PID. Multiple profiles run concurrently. |
