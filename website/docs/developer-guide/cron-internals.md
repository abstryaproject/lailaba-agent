...
description: "How Lailaba stores, schedules, edits, pauses, skill-loads, and delivers cron jobs"
---
...
| `lailaba_cli/cron.py` | CLI `lailaba cron` subcommands |
...
Jobs are stored in `~/.lailaba/cron/jobs.json` with atomic write semantics (write to temp file, then rename). Each job record contains:
...
In CLI mode, cron jobs only fire when `lailaba cron` commands are run or during active CLI sessions.
...
```python
# ~/.lailaba/scripts/check_competitors.py
...
2. **Environment variable** — `LAILABA_CRON_SCRIPT_TIMEOUT`
...
| Local file | `local` | Save to `~/.lailaba/cron/output/` |
...
The scheduler uses cross-process file-based locking (`fcntl.flock` on Unix, `msvcrt.locking` on Windows) to prevent overlapping ticks from executing the same due-job batch twice — even between the gateway's in-process ticker and a standalone `lailaba cron` / manual `tick()` call. If the lock cannot be acquired, `tick()` returns 0 immediately.

## CLI Interface

The `lailaba cron` CLI provides direct job management:

```bash
lailaba cron list                    # Show all jobs
lailaba cron create                  # Interactive job creation (alias: add)
lailaba cron edit <job_id>           # Edit job configuration
lailaba cron pause <job_id>          # Pause a running job
lailaba cron resume <job_id>         # Resume a paused job
lailaba cron run <job_id>            # Trigger immediate execution
lailaba cron remove <job_id>         # Delete a job
```
