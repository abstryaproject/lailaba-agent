...
sidebar_position: 1
title: "Quickstart"
description: "Your first conversation with Lailaba AI — from install to chatting in under 5 minutes"
---

# Quickstart

This guide gets you from zero to a working Lailaba setup that survives real use. Install, choose a provider, verify a working chat, and know exactly what to do when something breaks.

## Prefer to watch?

**Onchain AI Garage** put together a Masterclass walkthrough of installation, setup, and basic commands — a good companion to this page if you'd rather follow along on video. For more, see the full [Lailaba AI Tutorials & Use Cases](https://www.youtube.com/playlist?list=PLmpUb_PWAkDxewld5ZYyKifuHxgIbiq2d) playlist.

<div style={{position: 'relative', paddingBottom: '56.25%', height: 0, overflow: 'hidden', maxWidth: '100%', marginBottom: '1.5rem'}}>
  <iframe
    style={{position: 'absolute', top: 0, left: 0, width: '100%', height: '100%'}}
    src="https://www.youtube-nocookie.com/embed/R3YOGfTBcQg"
    title="Lailaba AI Masterclass: Installation, Setup, Basic Commands"
    frameBorder="0"
...
- Setting up Lailaba for a team, bot, or always-on workflow
...
|---|---|---|
| I just want Lailaba working on my machine | `lailaba setup` | Run a real chat and verify it responds |
| I already know my provider | `lailaba model` | Save the config, then start chatting |
| I want a bot or always-on setup | `lailaba gateway setup` after CLI works | Connect Telegram, Discord, Slack, or another platform |
| I want a local or self-hosted model | `lailaba model` → custom endpoint | Verify the endpoint, model name, and context length |
| I want multi-provider fallback | `lailaba model` first | Add routing and fallback only after the base chat works |

**Rule of thumb:** if Lailaba cannot complete a normal chat, do not add more features yet. Get one clean conversation working first, then layer on gateway, cron, skills, voice, or routing.

---

## 1. Install Lailaba AI
### With the Lailaba Desktop installer on macOS or Windows (recommended)
To easily install the command-line and desktop applications, [download the Lailaba Desktop installer](https://lailaba-agent.nousresearch.com/) from our website and run it.

### Without Lailaba Desktop:
For a command-line only install without Lailaba Desktop, run:
...
The single most important setup step. Use `lailaba model` to walk through the choice interactively:
...
:::info Setup modes
On a fresh install, `lailaba setup` offers three modes:
...
Blank Slate writes an explicit `platform_toolsets.cli` list plus `agent.disabled_toolsets`, so nothing you didn't choose ever loads — not even after `lailaba update`. Re-enable anything later with `lailaba tools`, seed skills with `lailaba skills opt-in --sync`, or tune settings with `lailaba setup agent`.
:::
...
|----------|-----------|---------------|
| **Nous Portal** | Subscription-based, zero-config | OAuth login via `lailaba model` |
| **OpenAI Codex** | ChatGPT OAuth, uses Codex models | Device code auth via `lailaba model` |
| **Anthropic** | Claude models directly — Max plan + extra usage credits (OAuth), or API key for pay-per-token | `lailaba model` → OAuth login (requires Max + extra credits), or an Anthropic API key |
...
| **MiniMax (OAuth)** | MiniMax frontier model via browser OAuth — no API key needed (model name in `lailaba_cli/models.py` may change between releases) | `lailaba model` → MiniMax (OAuth) |
...
| **xAI Grok OAuth** | SuperGrok / Premium+ subscription, no API key needed | `lailaba model` → xAI Grok OAuth |
...
| **Qwen OAuth** | Qwen Portal browser OAuth — no API key needed | `lailaba model` → Qwen OAuth |
...
| **GitHub Copilot** | GitHub Copilot subscription (GPT-5.x, Claude, Gemini, etc.) | OAuth via `lailaba model`, or `COPILOT_GITHUB_TOKEN` / `GH_TOKEN` |
| **GitHub Copilot ACP** | Copilot ACP agent backend (spawns local `copilot` CLI) | `lailaba model` (requires `copilot` CLI + `copilot login`) |
...
Lailaba AI requires a model with at least **64,000 tokens** of context. Models with smaller windows cannot maintain enough working memory for multi-step tool-calling workflows and will be rejected at startup. Most hosted models (Claude, GPT, Gemini, Qwen, DeepSeek) meet this easily. If you're running a local model, set its context size to at least 64K (e.g. `--ctx-size 65536` for llama.cpp or `-c 65536` for Ollama).
:::

:::tip
You can switch providers at any time with `lailaba model` — no lock-in. For a full list of all supported providers and setup details, see [AI Providers](../integrations/providers.md).
:::

### How settings are stored

Lailaba separates secrets from normal config:

- **Secrets and tokens** → `~/.lailaba/.env`
- **Non-secret settings** → `~/.lailaba/config.yaml`

The easiest way to set values correctly is through the CLI:

```bash
lailaba config set model anthropic/claude-opus-4.6
lailaba config set terminal.backend docker
lailaba config set OPENROUTER_API_KEY sk-or-...
```
...
```bash
lailaba            # classic CLI
lailaba --tui      # modern TUI (recommended)
```

You'll see a welcome banner with your model, available tools, and skills. Use a prompt that's specific and easy to verify:

:::tip Pick your interface
Lailaba ships with two terminal interfaces: the classic `prompt_toolkit` CLI and a newer [TUI](../user-guide/tui.md) with modal overlays, mouse selection, and non-blocking input. Both share the same sessions, slash commands, and config — try each with `lailaba` vs `lailaba --tui`.
:::
...
```bash
lailaba --continue    # Resume the most recent session
lailaba -c            # Short form
```
...
```bash
lailaba gateway setup    # Interactive platform configuration
```
...
- `lailaba tools` — tune tool access per platform
- `lailaba skills` — browse and install reusable workflows
...
```bash
lailaba config set terminal.backend docker    # Docker isolation
lailaba config set terminal.backend ssh       # Remote server
```

### Voice mode

```bash
# From the Lailaba install directory (the curl installer placed it at
# ~/.lailaba/lailaba-agent on Linux/macOS or %LOCALAPPDATA%\lailaba\lailaba-agent on Windows):
cd ~/.lailaba/lailaba-agent
...
Skills are on-demand instruction documents that teach Lailaba how to do a specific task — deploy to Kubernetes, open a GitHub PR, fine-tune a model, search for GIFs. Each is a `SKILL.md` file with a name, a description, and a step-by-step procedure. The agent reads the short descriptions for free and only loads a skill's full content when a task actually calls for it, so adding skills doesn't bloat every request.

Lailaba ships with a catalog of bundled skills already installed in `~/.lailaba/skills/`. You can add more from the Skills Hub, or write your own.

**Browse and install from the hub:**

```bash
lailaba skills browse                      # list everything available
lailaba skills search kubernetes           # find skills by keyword
lailaba skills install openai/skills/k8s   # install one (runs a security scan first)
```

The install argument is a `source/path` slug from the hub — `openai/skills/k8s` means the `k8s` skill from OpenAI's catalog. `lailaba skills browse` shows the exact slugs to use.
...
/k8s                                       # load it and let Lailaba ask what you need
```
...
(If you installed without `[all]`, run `cd ~/.lailaba/lailaba-agent && uv pip install -e ".[acp]"` first.)
...
|---|---|---|
| Lailaba opens but gives empty or broken replies | Provider auth or model selection is wrong | Run `lailaba model` again and confirm provider, model, and auth |
...
| Gateway starts but nobody can message it | Bot token, allowlist, or platform setup is incomplete | Re-run `lailaba gateway setup` and check `lailaba gateway status` |
| `lailaba --continue` can't find old session | Switched profiles or session never saved | Check `lailaba sessions list` and confirm you're in the right profile |
...
| `lailaba doctor` flags config problems | Config values are missing or stale | Fix the config, retest a plain chat before adding features |
...
2. `lailaba model`
3. `lailaba setup`
4. `lailaba sessions list`
5. `lailaba --continue`
6. `lailaba gateway status`
...
| Command | Description |
|---------|-------------|
| `lailaba` | Start chatting |
| `lailaba model` | Choose your LLM provider and model |
| `lailaba tools` | Configure which tools are enabled per platform |
| `lailaba setup` | Full setup wizard (configures everything at once) |
| `lailaba doctor` | Diagnose issues |
| `lailaba update` | Update to latest version |
| `lailaba gateway` | Start the messaging gateway |
| `lailaba --continue` | Resume last session |
