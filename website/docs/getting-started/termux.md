...
description: "Run Lailaba AI directly on an Android phone with Termux"
---

# Lailaba on Android with Termux
...
Lailaba AI can run directly on an Android phone through [Termux](https://termux.dev/).
...
That does not stop Lailaba from working well as a phone-native CLI agent — it just means the recommended mobile install is intentionally narrower than the desktop/server install.

---

## Option 1: One-line installer

Lailaba now ships a Termux-aware installer path:
...
- links `lailaba` into `$PREFIX/bin` so it stays on your Termux PATH
...
### 5. Put `lailaba` on your Termux PATH

```bash
ln -sf "$PWD/venv/bin/lailaba" "$PREFIX/bin/lailaba"
```

`$PREFIX/bin` is already on PATH in Termux, so this makes the `lailaba` command persist across new shells without re-activating the venv every time.
...
Or set keys directly in `~/.lailaba/.env`.
...
### `lailaba doctor` says ripgrep or Node is missing
