...
If you only run one Lailaba agent, you don't need this page — see
[Profiles](./profiles.md) for the basics.

## When to use this

You want this setup when you have two or more Lailaba agents that should all
...
(`ai.lailaba.gateway-<name>.plist`) or systemd user service
(`lailaba-gateway-<name>.service`). This guide adds the patterns for managing
them collectively.
...
lailaba profile create coder
lailaba profile create personal-bot
lailaba profile create research
...
```bash
lailaba config set gateway.multiplex_profiles true
lailaba gateway restart
```

Equivalently, in the default profile's `~/.lailaba/config.yaml`:
...
You do **not** run `lailaba gateway start` for the secondary profiles — the
...
With a multiplexer running, a named-profile `lailaba gateway start` / `run` is a
...
home). `lailaba status` reports the multiplexer and the profiles it serves;
`lailaba status -p <name>` slices to one profile. Each profile still writes its
...
`~/.local/bin/lailaba-gateways` and `chmod +x` it:
...
usage() {
  echo "Usage: lailaba-gateways {start|stop|restart|status|list}"
}
...
run_for_profile() {
  ...
  if [ "$profile" = "default" ]; then
    lailaba gateway "$action"
  else
    lailaba -p "$profile" gateway "$action"
  fi
}
...
```bash
lailaba-gateways start      # start every configured profile
lailaba-gateways stop       # stop every configured profile
lailaba-gateways restart    # restart all
lailaba-gateways status     # status across all
lailaba-gateways list       # delegates to `lailaba gateway list`
```

:::tip
The `default` profile is targeted with `lailaba gateway <action>` (no `-p`),
not `lailaba -p default gateway <action>`. The wrapper above handles both forms.
:::
...
These are equivalent to `lailaba -p coder gateway <action>` — useful if a
...
| Platform | Path                                                              |
| -------- | ----------------------------------------------------------------- |
| macOS    | `~/Library/LaunchAgents/ai.lailaba.gateway-<profile>.plist`        |
| Linux    | `~/.config/systemd/user/lailaba-gateway-<profile>.service`         |

The default profile keeps the historical names: `ai.lailaba.gateway.plist` /
`lailaba-gateway.service`.
...
```bash
# Default profile
tail -f ~/.lailaba/logs/gateway.log
tail -f ~/.lailaba/logs/gateway.error.log

# Named profile
tail -f ~/.lailaba/profiles/<name>/logs/gateway.log
tail -f ~/.lailaba/profiles/<name>/logs/gateway.error.log
```

Stream every profile's log simultaneously:

```bash
tail -f ~/.lailaba/logs/gateway.log ~/.lailaba/profiles/*/logs/gateway.log
```

The CLI also has a structured log viewer:

```bash
lailaba logs -f                  # follow default profile
lailaba -p coder logs -f         # follow one profile
lailaba logs --help              # filters, levels, JSON output
```

## Identify what's actually running

```bash
lailaba profile list             # profiles + model + gateway state
lailaba-gateways status          # full status across every profile
launchctl list | grep lailaba    # macOS — PIDs and labels
systemctl --user list-units 'lailaba-gateway-*'   # Linux — units
```
...
```
~/.lailaba/profiles/<name>/
├── .env              # API keys, bot tokens (chmod 600)
├── config.yaml       # model, provider, toolsets, gateway settings
└── SOUL.md           # personality / system prompt
```

The default profile uses `~/.lailaba/` directly with the same three files.

Edit them with any editor or via the CLI:

```bash
lailaba config set model.model anthropic/claude-sonnet-4    # default profile
...
caffeinate -i -w $(cat ~/.lailaba/gateway.pid) &   # awake while default gateway runs
...
systemd-inhibit --what=idle:sleep --who=lailaba --why="gateways running" \
  sleep infinity &
...
`lailaba-gateway-<profile>.service`) continue running across SSH disconnects
and reboots.
...
grep -H 'TELEGRAM_BOT_TOKEN\|DISCORD_BOT_TOKEN' \
     ~/.lailaba/.env ~/.lailaba/profiles/*/.env
```

## Updating the code

`lailaba update` pulls the latest code once and syncs new bundled skills into
every profile:
...
You ran `lailaba gateway start` after a previous `lailaba gateway stop`. The
...
```bash
ps -ef | grep "lailaba_cli.*-p <profile>"
cat ~/.lailaba/profiles/<profile>/gateway.pid
...
```bash
# macOS
launchctl unload ~/Library/LaunchAgents/ai.lailaba.gateway-<profile>.plist
launchctl load   ~/Library/LaunchAgents/ai.lailaba.gateway-<profile>.plist

# Linux
systemctl --user restart lailaba-gateway-<profile>.service
```

### Health check

```bash
lailaba doctor                  # default profile
lailaba -p <profile> doctor     # one profile
```
