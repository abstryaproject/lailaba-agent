...
sidebar_position: 12
title: "Google Chat"
description: "Set up Lailaba AI as a Google Chat bot using Cloud Pub/Sub"
---

# Google Chat Setup

Connect Lailaba AI to Google Chat as a bot. The integration uses Cloud Pub/Sub
pull subscriptions for inbound events and the Chat REST API for outbound messages.
Equivalent ergonomics to Slack Socket Mode or Telegram long-polling: your Lailaba
...
> Run `lailaba gateway setup` and pick **Google Chat** for a guided walk-through.
...
download the file. Save it somewhere only Lailaba can read (e.g.,
`~/.lailaba/google-chat-sa.json`, `chmod 600`).
...
- Delivery type: **Pull**
- Message retention: **7 days** (so backlog survives a lailaba restart)
...
- Principal: `lailaba-chat-bot@<your-project>.iam.gserviceaccount.com`
- Role: `Pub/Sub Subscriber`

Also grant `Pub/Sub Viewer` on the same subscription — Lailaba calls
...
- **App name**: whatever you want users to see ("Lailaba" is reasonable).
...
- **Connection settings**: select **Cloud Pub/Sub**, enter the topic name
  `projects/<your-project>/topics/lailaba-chat-events`.
...
`ADDED_TO_SPACE` event that Lailaba uses to cache the bot's own `users/{id}` for
self-message filtering.

---

## Step 9: Configure Lailaba

Add the Google Chat section to `~/.lailaba/.env`:
...
Install the dependencies the Google Chat adapter needs (no Lailaba extra is currently published — install them directly):
...
Send "hola" in the test DM. The bot posts a "Lailaba is thinking…" marker, then
...
Thread support: when a user replies inside a thread, Lailaba detects the
`thread.name` and posts its reply in the same thread, so each thread gets a
separate Lailaba session.
...
3. Download the JSON. Move it onto the host that runs Lailaba.
4. Register the client with Lailaba (run under the profile you want it scoped to):
...
lailaba -p <profile> python -m plugins.platforms.google_chat.oauth \
    --client-secret /path/to/client_secret.json
```

That writes the client secret into the active profile's Lailaba home (e.g.
`~/.lailaba/google_chat_user_client_secret.json` for the default profile). The
...
The token lands at `~/.lailaba/google_chat_user_tokens/<sanitized_email>.json`.
...
single-user token at `~/.lailaba/google_chat_user_token.json` (if present from
...
1. Check the Pub/Sub subscription has undelivered messages in the console.
   If it does, Lailaba isn't authenticated — verify `GOOGLE_CHAT_SERVICE_ACCOUNT_JSON`
...
3. Check `lailaba gateway` logs for `[GoogleChat] Connected`. If you see
...
- **Attachment download protection**: Lailaba will only attach the SA bearer
...
- **User OAuth scope**: the per-user attachment flow requests *only*
  ...
  `~/.lailaba/google_chat_user_tokens/<sanitized_email>.json` (filesystem
