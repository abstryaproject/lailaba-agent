...
description: "Chat with Lailaba from Telegram, Discord, Slack, WhatsApp, Signal, SMS, Email, Home Assistant, Mattermost, Matrix, DingTalk, Yuanbao, Microsoft Teams, LINE, Raft, Webhooks, or any OpenAI-compatible frontend via the API server — architecture and setup overview"
---

# Messaging Gateway

Chat with Lailaba from Telegram, Discord, Slack, WhatsApp, Signal, SMS, Email, Home Assistant, Mattermost, Matrix, DingTalk, Feishu/Lark, WeCom, Weixin, BlueBubbles (iMessage), QQ, Yuanbao, Microsoft Teams, LINE, ntfy, or your browser. The gateway is a single background process that connects to all your configured platforms, handles sessions, runs cron jobs, and delivers voice messages.

For the full voice feature set — including CLI microphone mode, spoken replies in messaging, and Discord voice-channel conversations — see [Voice Mode](/user-guide/features/voice-mode) and [Use Voice Mode with Lailaba](/guides/use-voice-mode-with-lailaba).
...
```mermaid
flowchart TB
    subgraph Gateway["Lailaba Gateway"]
...
For group chats, hooks, and automation flows, Lailaba supports explicit silence tokens. If the agent's final response is exactly one supported token, the gateway suppresses outbound delivery and sends nothing to the chat.
...
Silence is a delivery decision only. Lailaba keeps the assistant silence turn in the session transcript, so the conversation still alternates normally:
...
Failed turns still surface as errors; Lailaba does not hide failures just because the text resembles a silence token.
...
```bash
lailaba gateway setup        # Interactive setup for all messaging platforms
```
...
```bash
lailaba gateway              # Run in foreground
lailaba gateway setup        # Configure messaging platforms interactively
lailaba gateway install      # Install as a user service (Linux) / launchd service (macOS)
sudo lailaba gateway install --system   # Linux only: install a boot-time system service
lailaba gateway start        # Start the default service
lailaba gateway stop         # Stop the default service
lailaba gateway status       # Check default service status
lailaba gateway status --system         # Linux only: inspect the system service explicitly
```
...
| `/update` | Update Lailaba AI to the latest version |
...
Configure per-platform overrides in `~/.lailaba/gateway.json`:
...
lailaba pairing approve telegram XKGH5N7P

# Other pairing commands:
lailaba pairing list          # View pending + approved users
lailaba pairing revoke telegram 123456789  # Remove access
```
...
The first time you message a busy agent on any platform, Lailaba appends a one-line reminder to the busy-ack explaining the knob (`"💡 First-time tip — …"`). The reminder fires once per install — a flag under `onboarding.seen.busy_input_prompt` latches it. Delete that key to see the tip again.
...
Control how much tool activity is displayed in `~/.lailaba/config.yaml`:
...
Off by default. When enabled, Lailaba prepends a human-readable timestamp
...
When the agent running a background session uses `terminal(background=true)` to start long-running processes (servers, builds, etc.), the gateway can push status updates to your chat. Control this with `display.background_process_notifications` in `~/.lailaba/config.yaml`:
...
```bash
LAILABA_BACKGROUND_NOTIFICATIONS=result
```
...
```bash
lailaba gateway install               # Install as user service
lailaba gateway start                 # Start the service
lailaba gateway stop                  # Stop the service
lailaba gateway status                # Check status
journalctl --user -u lailaba-gateway -f  # View logs

# Enable lingering (keeps running after logout)
sudo loginctl enable-linger $USER

# Or install a boot-time system service that still runs as your user
sudo lailaba gateway install --system
sudo lailaba gateway start --system
sudo lailaba gateway status --system
journalctl -u lailaba-gateway -f
```

Use the user service on laptops and dev boxes. Use the system service on VPS or headless hosts that should come back at boot without relying on systemd linger.

:::danger Don't add a custom `ExecStopPost` kill drop-in
The unit Lailaba installs already shuts the gateway down cleanly with `KillMode=mixed` + `KillSignal=SIGTERM`, and uses `Restart=always` with `RestartForceExitStatus` so updates and `/restart` respawn correctly. Do **not** add a systemd drop-in such as `ExecStopPost=/bin/kill -9 $MAINPID` — `ExecStopPost` fires on *every* stop, including clean restarts, so it `SIGKILL`s the freshly spawned instance before it stabilizes and `Restart=always` immediately respawns it. The result is an infinite restart loop (and, on Telegram, a flood of restart messages). If you've added such a drop-in, remove it: `systemctl --user edit lailaba-gateway` (or `sudo systemctl edit lailaba-gateway` for a system service) and delete the `ExecStopPost` line, then `systemctl --user daemon-reload`.
:::

:::tip Headless VMs: user service + linger avoids root prompts
A system service needs root for every restart — including the automatic gateway restart at the end of `lailaba update`. When `lailaba update` runs as a non-root user, it tries passwordless `sudo systemctl`; if that's unavailable, it skips the restart and prints the manual `sudo systemctl restart lailaba-gateway` command (it never blocks on an interactive password prompt).
...
```bash
lailaba gateway install          # user service
sudo loginctl enable-linger $USER   # one-time: start at boot, survive logout
```

After that, `lailaba update` can restart the gateway without any privileges. If you prefer to keep the system service, either run updates with `sudo lailaba update`, or grant the service account passwordless sudo for systemctl, e.g. in `sudo visudo -f /etc/sudoers.d/lailaba-gateway`:

```
lailaba ALL=(root) NOPASSWD: /usr/bin/systemctl --no-ask-password reset-failed lailaba-gateway*, /usr/bin/systemctl --no-ask-password start lailaba-gateway*, /usr/bin/systemctl --no-ask-password restart lailaba-gateway*
```
:::

Avoid keeping both the user and system gateway units installed at once unless you really mean to. Lailaba will warn if it detects both because start/stop/status behavior gets ambiguous.

:::info Multiple installations
If you run multiple Lailaba installations on the same machine (with different `LAILABA_HOME` directories), each gets its own systemd service name. The default `~/.lailaba` uses `lailaba-gateway`; other installations use `lailaba-gateway-<hash>`. The `lailaba gateway` commands automatically target the correct service for your current `LAILABA_HOME`.
:::

### macOS (launchd)

```bash
lailaba gateway install               # Install as launchd agent
lailaba gateway start                 # Start the service
lailaba gateway stop                  # Stop the service
lailaba gateway status                # Check status
tail -f ~/.lailaba/logs/gateway.log   # View logs
```

The generated plist lives at `~/Library/LaunchAgents/ai.lailaba.gateway.plist`. It includes three environment variables:
...
- **LAILABA_HOME** — scopes the gateway to your Lailaba installation.

:::tip PATH changes after install
launchd plists are static — if you install new tools (e.g. a new Node.js version via nvm, or ffmpeg via Homebrew) after setting up the gateway, run `lailaba gateway install` again to capture the updated PATH. The gateway will detect the stale plist and reload automatically.
:::

:::info Multiple installations
Like the Linux systemd service, each `LAILABA_HOME` directory gets its own launchd label. The default `~/.lailaba` uses `ai.lailaba.gateway`; other installations use `ai.lailaba.gateway-<suffix>`.
:::
...
|----------|---------|--------------|
| CLI | `lailaba-cli` | Full access |
| Telegram | `lailaba-telegram` | Full tools including terminal |
| Discord | `lailaba-discord` | Full tools including terminal |
| WhatsApp | `lailaba-whatsapp` | Full tools including terminal |
| WhatsApp Cloud API | `lailaba-whatsapp` | Full tools including terminal (shares toolset with the Baileys bridge) |
| Slack | `lailaba-slack` | Full tools including terminal |
| Google Chat | `lailaba-google_chat` | Full tools including terminal |
| Signal | `lailaba-signal` | Full tools including terminal |
| SMS | `lailaba-sms` | Full tools including terminal |
| Email | `lailaba-email` | Full tools including terminal |
| Home Assistant | `lailaba-homeassistant` | Full tools + HA device control (ha_list_entities, ha_get_state, ha_call_service, ha_list_services) |
| Mattermost | `lailaba-mattermost` | Full tools including terminal |
| Matrix | `lailaba-matrix` | Full tools including terminal |
| DingTalk | `lailaba-dingtalk` | Full tools including terminal |
| Feishu/Lark | `lailaba-feishu` | Full tools including terminal |
| WeCom | `lailaba-wecom` | Full tools including terminal |
| WeCom Callback | `lailaba-wecom-callback` | Full tools including terminal |
| Weixin | `lailaba-weixin` | Full tools including terminal |
| BlueBubbles | `lailaba-bluebubbles` | Full tools including terminal |
| QQBot | `lailaba-qqbot` | Full tools including terminal |
| Yuanbao | `lailaba-yuanbao` | Full tools including terminal |
| Microsoft Teams | `lailaba-teams` | Full tools including terminal |
| API Server | `lailaba-api-server` | Full tools (drops `clarify`, `send_message`, `text_to_speech` — programmatic access doesn't have an interactive user) |
| Webhooks | `lailaba-webhook` | Full tools including terminal |
| Raft | `lailaba-raft` | Wake-only channel; agent uses Raft CLI for message I/O |
...
1. **Gateway log** (`~/.lailaba/logs/gateway.log` or the systemd / launchd unit log). Search for the platform name and `circuit breaker`, `paused`, or `disabled`. The trip event includes the failure count and the last error.
