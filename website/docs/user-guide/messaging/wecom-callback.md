...
Connect Lailaba to WeCom (Enterprise WeChat) as a self-built enterprise application using the callback/webhook model.

:::info WeCom Bot vs WeCom Callback
Lailaba supports two WeCom integration modes:
...
> Run `lailaba gateway setup` and pick **WeCom Callback** for a guided walk-through.
...
3. Lailaba decrypts the message, queues it for the agent
...
(Use `lailaba gateway start` only after `lailaba gateway install` has registered the systemd/launchd service.)
...
console. A mismatch between the token configured in Lailaba and the token the
...
in `~/.lailaba/.env` values around `=` will also break signature checks. After
fixing, restart `lailaba gateway run`.
...
Check `lailaba gateway run` logs for the bound host/port. If the adapter bound to
