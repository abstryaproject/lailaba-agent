...
sidebar_position: 16
title: "Yuanbao"
description: "Connect Lailaba AI to the Yuanbao enterprise messaging platform via WebSocket gateway"
---

# Yuanbao

Connect Lailaba to [Yuanbao](https://yuanbao.tencent.com/), Tencent's enterprise messaging platform. The adapter uses a WebSocket gateway for real-time message delivery and supports both direct (C2C) and group conversations.
...
After initial setup, verify these variables in `~/.lailaba/.env`:
...
All standard Lailaba commands work on Yuanbao:
...
4. Review gateway logs: `tail -f ~/.lailaba/logs/gateway.log`
...
4. Consider enabling verbose logging: `LAILABA_LOG_LEVEL=debug`
...
Yuanbao has a maximum message size. Lailaba automatically chunks large responses with Markdown-aware splitting (respects code fences, tables, and paragraph boundaries).
...
```bash
LAILABA_LOG_LEVEL=debug lailaba gateway
```
...
```bash
lailaba chat -q "Send 'Hello from CLI' to yuanbao:group:group_code"
```
