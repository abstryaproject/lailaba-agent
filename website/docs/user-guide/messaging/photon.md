...
Connect Lailaba to **iMessage** through [Photon][photon], a managed
...
your first iMessage from Lailaba — just a phone number we can bind to
your account.
:::
...
both directions. Because the SDK is TypeScript-only, Lailaba runs it in a
...
lailaba photon setup --phone +15551234567
```
...
2. **Finds or creates** the `Lailaba AI` project on your account.
...
Runtime credentials are written to `~/.lailaba/.env`
...
(device token, dashboard project id) lives in `~/.lailaba/auth.json` under
`credential_pool.photon` / `credential_pool.photon_project`.

## Authorizing users

Photon uses the same authorization model as every other Lailaba
channel. Choose one approach:

**DM pairing (default).** When an unknown number messages your Photon
line, Lailaba replies with a pairing code. Approve it with:

```bash
lailaba pairing approve photon <CODE>
```

Use `lailaba pairing list` to see pending codes and approved users.

**Pre-authorize specific numbers** (in `~/.lailaba/.env`):

```bash
PHOTON_ALLOWED_USERS=+15551234567,+15559876543
```

**Open access** (dev only, in `~/.lailaba/.env`):
...
By default Lailaba responds to every authorized DM and group message.
...
they match a wake-word pattern. The defaults match `Lailaba` and
`@Lailaba agent` variants. For a custom agent name, set regex patterns:
...
Send an iMessage to your assigned number and Lailaba will reply.

## Status & troubleshooting

```bash
lailaba photon status
```

Prints saved credentials, sidecar health, your registered number, and the
assigned iMessage line Lailaba uses. When a Photon token and dashboard project
...
- **`sidecar deps : ✗ run lailaba photon install-sidecar`** — Node is
  installed but `spectrum-ts` isn't. Run the suggested command.
- **`device token : ✗ missing`** — run `lailaba photon setup` to log in.
- **`No iMessage line assigned yet`** — Spectrum is enabled but no line
  has been provisioned; re-run `lailaba photon setup` or check the
  [dashboard][app].
- **Sidecar won't start** — confirm `node --version` is 18.17+ and that
  `lailaba photon install-sidecar` completed without errors.
...
- **Outbound attachments are supported.** Lailaba sends images, voice
...
| `PHOTON_MENTION_PATTERNS` | Lailaba wake words  | JSON list / comma / newline regex patterns for group mentions |
