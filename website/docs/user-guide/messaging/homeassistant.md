...
description: Control your smart home with Lailaba AI via Home Assistant integration.
...
Lailaba AI integrates with [Home Assistant](https://www.home-assistant.io/) in two ways:
...
4. Click **Create Token**, give it a name like "Lailaba AI"
5. Copy the token
...
Lailaba AI registers four tools for smart home control:
...
Configure which events the agent sees in `~/.lailaba/config.yaml` under the Home Assistant platform's `extra` section:
...
Outbound messages from the agent are delivered as **Home Assistant persistent notifications** (via `persistent_notification.create`). These appear in the HA notification panel with the title "Lailaba AI".
...
The adapter reads credentials from `~/.lailaba/.env` (auto-merged at startup) or
from `config.yaml`. Double-check the file lives under the active Lailaba profile
...
running Lailaba — `curl -H "Authorization: Bearer <token>" <url>/api/` should
