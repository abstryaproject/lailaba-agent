...
sidebar_position: 3
title: "Desktop App"
description: "The native Lailaba desktop app — a polished experience for chatting with Lailaba, with streaming tool output, side-by-side previews, a file browser, voice, cron, profiles, skills, and settings. macOS, Windows, and Linux."
---

# Desktop App

The Lailaba desktop app is a native app built around the **same** agent you get from the CLI and the gateway — same config, same API keys, same sessions, same skills, same memory. It is not a separate product or a lightweight clone; it uses the same Lailaba AI core and settings, and drives it through a modern & thoughtfully designed UI. If you have used `lailaba` in a terminal, everything you set up there is already here, and anything you do here shows up there.

It runs on **macOS, Windows, and Linux**.

:::tip Which interface is which?
Lailaba has several front ends that all talk to the same agent:

- **Desktop App** (this page) — a native application with a purpose-built UI for chat, configuration, and management.
- **CLI** (`lailaba`) and **[TUI](./tui.md)** (`lailaba --tui`) — terminal interfaces.
- **[Web Dashboard](./features/web-dashboard.md)** (`lailaba dashboard`) — a browser admin panel; its optional **Chat** tab embeds the TUI through a pseudo-terminal.
...
Follow the [installation instructions for Lailaba Desktop](../getting-started/installation.md).

If you already have Lailaba installed, simply run
...
- **The same conversation history** as every other Lailaba surface — sessions started here resume in the CLI/TUI and vice versa.
...
Explore and preview the working directory without leaving the app — useful for following along as the agent reads, writes, and edits files. Set the initial project directory with `lailaba desktop --cwd <path>` (or the `LAILABA_DESKTOP_CWD` environment variable).

### Voice

Talk to Lailaba and hear it back, the same [voice mode](./features/voice-mode.md) available elsewhere. On macOS the OS will prompt once for microphone access.
...
- **Every provider and model in the menus** — the GUI surfaces the full provider list and every model that `lailaba model` knows about, so you pick from the same catalog the CLI sees rather than a curated subset.
...
The app also surfaces the broader Lailaba management surface so you don't have to drop to a terminal:

- **Skills** — browse, install, and manage [skills](./features/skills.md).
- **Cron** — view and manage [scheduled jobs](../reference/cli-commands.md#lailaba-cron).
- **Profiles** — switch between [Lailaba profiles](./profiles.md) (isolated config/skills/sessions).
...
The [manual update process](https://lailaba-agent.nousresearch.com/docs/getting-started/updating) also works with the GUI.

## Uninstalling

Open **Settings → About → Danger zone** and pick how much to remove:

- **Uninstall Chat GUI only** — removes the desktop app and its data; the Lailaba agent, your config, and your chats stay. (Same as `lailaba uninstall --gui`.)
- **Uninstall GUI + agent, keep my data** — removes the app and the agent but keeps config, chats, and secrets for a future reinstall. (Same as `lailaba uninstall`.)
- **Uninstall everything** — removes the app, the agent, and all user data. (Same as `lailaba uninstall --full`.)
...
You can do the same from the terminal — `lailaba uninstall --gui` for the GUI alone, or `lailaba uninstall` / `lailaba uninstall --full` for the agent too.

:::note
Running `lailaba uninstall --gui` from a **source checkout** (a `lailaba desktop` dev build) also removes the workspace `node_modules` and `apps/desktop/{dist,release}` build output, since those are GUI build artifacts. They're recoverable with `lailaba desktop` (or `npm install` + a rebuild) — but if you're actively hacking on the desktop app, expect to reinstall dependencies afterward.
:::

## CLI reference: `lailaba desktop`

To launch via the CLI, simply run `lailaba desktop`. By default it installs workspace Node dependencies, builds the current OS's unpacked Electron app, then launches that packaged artifact.
...
| `--build-only`       | Build the desktop app but do not launch it (used by `lailaba update`)                      |
| `--source`           | Launch via `electron .` against `apps/desktop/dist` instead of the packaged app           |
| `--cwd PATH`         | Initial project directory for desktop chat sessions (sets `LAILABA_DESKTOP_CWD`)           |
| `--lailaba-root PATH` | Override the Lailaba source root the app uses (sets `LAILABA_DESKTOP_LAILABA_ROOT`)          |
| `--ignore-existing`  | Force the app to ignore any `lailaba` CLI already on `PATH` during backend resolution      |
| `--fake-boot`        | Enable deterministic boot delays for validating the startup UI                            |

## How it works

The packaged app ships the Electron shell and a native React chat surface. On first launch it can install the Lailaba AI runtime into `LAILABA_HOME` (`~/.lailaba`, or `%LOCALAPPDATA%\lailaba` on Windows) — **the same layout a CLI install uses**, which is why the two are interchangeable. Backend resolution first honours `LAILABA_DESKTOP_LAILABA_ROOT`, then a completed managed install, then a probed `lailaba` on `PATH` (unless `--ignore-existing` / `LAILABA_DESKTOP_IGNORE_EXISTING=1` is set), and finally an explicit `LAILABA_DESKTOP_LAILABA` command override for packagers such as Nix. The React renderer talks to a `lailaba dashboard` backend over the `tui_gateway`/dashboard APIs and reuses the agent runtime rather than embedding `lailaba --tui`. Install, backend-resolution, and self-update logic live in the Electron main process.

## Connecting to a remote backend

By default the app starts and manages its own **local** backend. You can instead point it at a Lailaba backend running on another machine — a VPS, a home server, or a Mini behind Tailscale.

:::info The remote backend is a running `lailaba dashboard` process
"Remote backend" means a **`lailaba dashboard`** server running on the remote machine — that is the process the desktop app connects to. Nothing in this section works unless that dashboard is actually up and reachable. The desktop app does not start it for you; you (or a `systemd` service) keep `lailaba dashboard` running on the remote host, and the app attaches to it. If you also use messaging channels (Telegram, Discord, etc.), the **gateway** is a *separate* long-running process you start independently — see the note after the setup steps.
:::
...
Set a username and password, then start the dashboard bound to a reachable address. The credentials live in `~/.lailaba/.env` (the secrets file, mode 0600):
...
Keep that `lailaba dashboard` process running for as long as you want the desktop app to be able to connect — if it stops, the app can no longer reach the backend. Run it under `systemd`, `tmux`, or your process manager of choice so it survives logout and reboots.
...
Prefer not to keep a plaintext password at rest? Set `LAILABA_DASHBOARD_BASIC_AUTH_PASSWORD_HASH` to a scrypt hash instead — compute it with `python -c "from plugins.dashboard_auth.basic import hash_password; print(hash_password('PW'))"`. Full configuration surface (config.yaml keys, every env var, the rate limiter): [Web Dashboard → Username/password provider](./features/web-dashboard.md#usernamepassword-provider-no-oauth-idp).

Running the dashboard as a systemd service? Give the unit `EnvironmentFile=%h/.lailaba/.env` so the credentials are in the environment at boot.
...
1. **Remote URL** — `http://<backend-host>:9119` (path prefixes like `/lailaba` work if you front it with a reverse proxy)
...
3. **Save and reconnect** — switches the desktop shell onto the remote backend. The session refreshes automatically; you stay signed in across restarts when `LAILABA_DASHBOARD_BASIC_AUTH_SECRET` is set.

You can also set the backend URL without the UI via the `LAILABA_DESKTOP_REMOTE_URL` environment variable before launching the app (it overrides the in-app setting); you still sign in from the Gateway settings panel.
...
- **No "Sign in" button — it asks for a session token instead** — the backend's username/password provider isn't active. `/api/status` won't list `"basic"` in `auth_providers`. Make sure both the username and a password (or password hash) are set in `~/.lailaba/.env` and that the dashboard process actually loaded them.
- **Signed out on every restart** — set `LAILABA_DASHBOARD_BASIC_AUTH_SECRET` to a stable value. Without it the token-signing key is regenerated per boot, invalidating all sessions.
...
For the same setup from the web-dashboard angle, see [Web Dashboard → Connecting Lailaba Desktop to a remote backend](./features/web-dashboard.md#connecting-lailaba-desktop-to-a-remote-backend); the env vars are catalogued under [Environment Variables → Web Dashboard & Lailaba Desktop](../reference/environment-variables.md#web-dashboard--lailaba-desktop).

## Troubleshooting

Boot logs land in `LAILABA_HOME/logs/desktop.log` (it includes backend output and recent Python tracebacks) — check it first if the app reports a boot failure. You can also tail it from the CLI:
...
rm "$HOME/.lailaba/lailaba-agent/.lailaba-bootstrap-complete"

# Rebuild a broken Python venv (macOS/Linux)
rm -rf "$HOME/.lailaba/lailaba-agent/venv"

# Reset a stuck macOS microphone prompt
tccutil reset Microphone com.nousresearch.lailaba
```
...
ELECTRON_MIRROR=https://npmmirror.com/mirrors/electron/ \
  bash -c 'cd "$HOME/.lailaba/lailaba-agent/apps/desktop" && CSC_IDENTITY_AUTO_DISCOVERY=false npm run pack'
```
...
```bash
LAILABA_DESKTOP_LAILABA_ROOT=/path/to/clone npm run dev
LAILABA_HOME=/tmp/throwaway npm run dev
...
- [TUI](./tui.md) — the modern terminal UI used by `lailaba --tui` and the dashboard chat tab
