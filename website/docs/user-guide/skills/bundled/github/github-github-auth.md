...
- Give it a name like "lailaba-agent"
- Select scopes:
...
- Give it a title like "lailaba-agent-&lt;machine-name>"
...
elif _lailaba_env="${LAILABA_HOME:-$HOME/.lailaba}/.env"; [ -f "$_lailaba_env" ] && grep -q "^GITHUB_TOKEN=" "$_lailaba_env"; then
  export GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" "$_lailaba_env" | head -1 | cut -d= -f2 | tr -d '\n\r')
  echo "AUTH_METHOD=curl"
