...
if command -v gh &>/dev/null && gh auth status &>/dev/null; then
  ...
  if [ -z "$GITHUB_TOKEN" ]; then
    if _lailaba_env="${LAILABA_HOME:-$HOME/.lailaba}/.env"; [ -f "$_lailaba_env" ] && grep -q "^GITHUB_TOKEN=" "$_lailaba_env"; then
      GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" "$_lailaba_env" | head -1 | cut -d= -f2 | tr -d '\n\r')
...
curl -s -X POST \
  ...
  -d "{
    \"commit_id\": \"$HEAD_SHA\",
    \"event\": \"COMMENT\",
    \"body\": \"Code review from Lailaba AI\",
    \"comments\": [
...
```bash
source "${LAILABA_HOME:-$HOME/.lailaba}/skills/github/github-auth/scripts/gh-env.sh"
...
gh pr review $PR_NUMBER --approve --body "Reviewed by Lailaba AI. Code looks clean — good test coverage, no security concerns."
