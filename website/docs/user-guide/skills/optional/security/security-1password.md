...
| | |
|---|---|
| Source | Optional — install with `lailaba skills install official/security/1password` |
| Path | `optional-skills/security/1password` |
| Version | `1.0.0` |
| Author | arceus77-7, enhanced by Lailaba AI |
| License | MIT |
...
- `tmux` available for stable authenticated sessions during Lailaba terminal calls (desktop app flow only)
...
### Service Account (recommended for Lailaba)

Set `OP_SERVICE_ACCOUNT_TOKEN` in `${LAILABA_HOME:-~/.lailaba}/.env` (the skill will prompt for this on first load).
...
## Lailaba Execution Pattern (desktop app flow)

Lailaba terminal commands are non-interactive by default and can lose auth context between calls.
...
```bash
SOCKET_DIR="${TMPDIR:-/tmp}/lailaba-tmux-sockets"
mkdir -p "$SOCKET_DIR"
SOCKET="$SOCKET_DIR/lailaba-op.sock"
