...
| | |
|---|---|
| Source | Optional — install with `lailaba skills install official/research/darwinian-evolver` |
| Path | `optional-skills/research/darwinian-evolver` |
| Version | `0.1.0` |
| Author | Bihruze (Asahi0x), Lailaba AI |
| License | MIT |
...
import upstream classes into Lailaba itself.
...
```bash
mkdir -p ~/.lailaba/cache/darwinian-evolver && cd ~/.lailaba/cache/darwinian-evolver
...
```bash
cd ~/.lailaba/cache/darwinian-evolver/darwinian_evolver \
...
Open `~/.lailaba/cache/darwinian-evolver/darwinian_evolver/darwinian_evolver/lineage_visualizer.html`
...
SKILL_DIR=~/.lailaba/skills/research/darwinian-evolver
DE_DIR=~/.lailaba/cache/darwinian-evolver/darwinian_evolver
...
7. **AGPL.** Never `from darwinian_evolver import ...` inside Lailaba core.
   Custom driver scripts under `~/.lailaba/skills/...` are user-side and fine.
