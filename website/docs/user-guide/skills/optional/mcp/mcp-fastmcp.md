...
| | |
|---|---|
| Source | Optional — install with `lailaba skills install official/mcp/fastmcp` |
...
- smoke-test a server with the FastMCP CLI before wiring it into Lailaba or another client
...
Use `native-mcp` when the server already exists and only needs to be connected to Lailaba. Use `mcporter` when the goal is ad-hoc CLI access to an existing MCP server instead of building one.
...
```bash
python ~/.lailaba/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py \
...
```bash
python ~/.lailaba/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py --list
```
...
When the goal is Lailaba integration, either:

- configure the server in `~/.lailaba/config.yaml` using the `native-mcp` skill, or
...
### Lailaba cannot see the deployed server

The server-building part may be correct while the Lailaba config is not. Load the `native-mcp` skill and configure the server in `~/.lailaba/config.yaml`, then restart Lailaba.
