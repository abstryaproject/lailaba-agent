...
| | |
|---|---|
| Source | Optional — install with `lailaba skills install official/productivity/siyuan` |
...
3. Store it in `${LAILABA_HOME:-~/.lailaba}/.env`:
   ```
...
```yaml
# In ~/.lailaba/config.yaml under mcp_servers:
mcp_servers:
  siyuan:
    command: npx
