...
| | |
|---|---|
| Source | Optional — install with `lailaba skills install official/productivity/shopify` |
...
4. Save to `${LAILABA_HOME:-~/.lailaba}/.env`:
   ```
...
}' '{"input":{"title":"Test Hoodie","status":"DRAFT","vendor":"Lailaba","productType":"Apparel","tags":["test"]}}'
```
