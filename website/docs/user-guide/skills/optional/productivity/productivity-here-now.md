...
| | |
|---|---|
| Source | Optional — install with `lailaba skills install official/productivity/here-now` |
...
- Skill helper paths:
  - `${LAILABA_SKILL_DIR}/scripts/publish.sh` for publishing sites
  - `${LAILABA_SKILL_DIR}/scripts/drive.sh` for private Drive storage

## Create a site

```bash
PUBLISH="${LAILABA_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --client lailaba
```
...
```bash
PUBLISH="${LAILABA_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --slug {slug} --client lailaba
```
...
```bash
DRIVE="${LAILABA_SKILL_DIR}/scripts/drive.sh"
bash "$DRIVE" default
...
| `--client {name}`      | Agent name for attribution (e.g. `lailaba`)    |
