...
| | |
|---|---|
| Source | Optional — install with `lailaba skills install official/productivity/canvas` |
...
4. Name the token (e.g., "Lailaba AI"), set an optional expiry, and click **Generate Token**
5. Copy the token and add to `${LAILABA_HOME:-~/.lailaba}/.env`:
...
```bash
CANVAS="python $LAILABA_HOME/skills/productivity/canvas/scripts/canvas_api.py"
