...
description: "Configure and use Honcho memory with Lailaba -- cross-session user modeling, multi-profile peer isolation, observation config, dialectic reasoning, session su..."
---
...
Configure and use Honcho memory with Lailaba -- cross-session user modeling, multi-profile peer isolation, observation config, dialectic reasoning, session summaries, and context budget enforcement. Use when setting up Honcho, troubleshooting memory, managing profiles with Honcho peers, or tuning observation, recall, and dialectic settings.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `lailaba skills install official/autonomous-ai-agents/honcho` |
...
# Honcho Memory for Lailaba

Honcho provides AI-native cross-session user modeling. It learns who the user is across conversations and gives every Lailaba profile its own peer identity while sharing a unified view of the user.
...
See: https://docs.honcho.dev/v3/guides/integrations/lailaba#running-honcho-locally-with-lailaba

### Verify

```bash
lailaba honcho status    # shows resolved config, connection test, peer info
```
...
3. **AI peer card** -- the identity card for this Lailaba profile's AI peer
...
Honcho models conversations as interactions between **peers**. Lailaba creates two peers per session:

- **User peer** (`peerName`): represents the human. Honcho builds a user representation from observed messages.
- **AI peer** (`aiPeer`): represents this Lailaba instance. Each profile gets its own AI peer so agents develop independent views.
...
| `per-session` | New Honcho session each Lailaba run |
| `global` | Single session across all directories |

Manual override: `lailaba honcho map my-project-name`
...
Each Lailaba profile gets its own Honcho AI peer while sharing the same workspace (user context). This means:
...
# creates host block lailaba.coder, AI peer "coder", inherits config from default
```

What `--clone` does for Honcho:
1. Creates a `lailaba.coder` host block in `honcho.json`
...
```bash
lailaba honcho sync    # creates host blocks for all profiles that don't have one yet
```
...
Guidelines for Lailaba when Honcho memory is active.
...
Config file: `$LAILABA_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global).
...
Run `lailaba honcho setup`. Ensure `memory.provider: honcho` is in `~/.lailaba/config.yaml`.

### Memory not persisting across sessions
Check `lailaba honcho status` -- verify `saveMessages: true` and `writeFrequency` isn't `session` (which only writes on exit).

### Profile not getting its own peer
Use `--clone` when creating: `lailaba profile create <name> --clone`. For existing profiles: `lailaba honcho sync`.
...
| Command | Description |
|---------|-------------|
| `lailaba honcho setup` | Interactive setup wizard (cloud/local, identity, observation, recall, sessions) |
| `lailaba honcho status` | Show resolved config, connection test, peer info for active profile |
| `lailaba honcho enable` | Enable Honcho for the active profile (creates host block if needed) |
| `lailaba honcho disable` | Disable Honcho for the active profile |
| `lailaba honcho peer` | Show or update peer names (`--user <name>`, `--ai <name>`, `--reasoning <level>`) |
| `lailaba honcho peers` | Show peer identities across all profiles |
| `lailaba honcho mode` | Show or set recall mode (`hybrid`, `context`, `tools`) |
| `lailaba honcho tokens` | Show or set token budgets (`--context <N>`, `--dialectic <N>`) |
| `lailaba honcho sessions` | List known directory-to-session-name mappings |
| `lailaba honcho map <name>` | Map current working directory to a Honcho session name |
| `lailaba honcho identity` | Seed AI peer identity or show both peer representations |
| `lailaba honcho sync` | Create host blocks for all Lailaba profiles that don't have one yet |
| `lailaba honcho migrate` | Step-by-step migration guide from OpenClaw native memory to Lailaba + Honcho |
| `lailaba memory setup` | Generic memory provider picker (selecting "honcho" runs the same wizard) |
| `lailaba memory status` | Show active memory provider and config |
| `lailaba memory off` | Disable external memory provider |
