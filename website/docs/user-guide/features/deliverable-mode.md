...
When Lailaba AI runs inside a messaging gateway (Slack, Discord, Telegram,
...
persona in `~/.lailaba/SOUL.md`, or as a named preset under
`agent.personalities` in `~/.lailaba/config.yaml` (switchable per session
via `/personality`).
...
If you use Lailaba' kanban multi-agent workflow, workers can attach
...
Install MCP servers via `~/.lailaba/config.yaml` under the `mcp_servers`
...
back into the thread as a native attachment. Lailaba AI's deliverable
