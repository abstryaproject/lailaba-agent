...
Lailaba runs full language servers — pyright, gopls, rust-analyzer,
...
This is the same architecture top-tier coding agents use. Lailaba
...
1. Lailaba captures a baseline of current diagnostics for the file.
...
…). Lailaba auto-detects the binary on PATH or in
`<LAILABA_HOME>/lsp/bin/`.
...
`node_modules` tree — Lailaba installs both packages together when you
run `lailaba lsp install typescript` or auto-install fires on first
use.

## CLI

```
lailaba lsp status          # service state + per-server install status
lailaba lsp list            # registry, optionally --installed-only
lailaba lsp install <id>    # eagerly install one server
lailaba lsp install-all     # try every server with a known recipe
lailaba lsp restart         # tear down running clients
lailaba lsp which <id>      # print resolved binary path
```

`lailaba lsp status` is the best starting point — it shows which
...
lsp:
  ...
  #   auto    — install via npm/pip/go install into <LAILABA_HOME>/lsp/bin
...
When `install_strategy: auto`, Lailaba installs binaries into
`<LAILABA_HOME>/lsp/bin/`. NPM packages land in
`<LAILABA_HOME>/lsp/node_modules/` with bin symlinks one level up.
Go binaries come from `go install` with `GOBIN` pointed at the
staging dir.

Nothing is ever installed to `/usr/local/`, `~/.local/`, or any other
shared location — the staging dir is fully Lailaba-owned and is
...
Servers are kept alive for the life of the Lailaba process. There's
...
**`lailaba lsp status` shows a server as "missing"**

The binary isn't on PATH and isn't in `<LAILABA_HOME>/lsp/bin/`. Run
`lailaba lsp install <server_id>` to attempt an auto-install, or
install the binary manually through the language's normal toolchain.

**`Backend warnings` section in `lailaba lsp status`**
...
When `lailaba lsp status` shows a `Backend warnings` section, install
...
`~/.lailaba/logs/agent.log`.

**Server starts but never returns diagnostics**

Check `~/.lailaba/logs/agent.log` for `[agent.lsp.client]` entries —
...
the rest of the session. Run `lailaba lsp restart` to clear the set;
