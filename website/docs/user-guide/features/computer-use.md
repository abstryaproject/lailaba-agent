...
Lailaba AI can drive your desktop — clicking, typing, scrolling,
...
```
lailaba computer-use install
```

This fetches and runs the upstream cua-driver installer — `install.sh`
on macOS/Linux, `install.ps1` on Windows. Use `lailaba computer-use
status` to verify the install.

**Option 2: enable the toolset interactively.**

1. Run `lailaba tools`, pick `🖱️  Computer Use (macOS/Windows/Linux)`.
...
| Platform | Prereqs |
|---|---|
| **macOS** | System Settings → Privacy & Security → **Accessibility** + **Screen Recording** → allow your terminal (or Lailaba app). `lailaba computer-use doctor` will tell you which permission is missing. |
...
```
lailaba -t computer_use chat
```

or add `computer_use` to your enabled toolsets in `~/.lailaba/config.yaml`.

## `lailaba computer-use doctor` — your first triage stop

`lailaba computer-use doctor` runs cua-driver's structured
`health_report` MCP tool and prints a per-check matrix. It's the single
fastest way to find out *why* an action isn't working.

```
$ lailaba computer-use doctor
...
agent is acting here." Each Lailaba run declares its own cua-driver
**session id** (something like `lailaba-3a7b9c14d2e8`); the cursor's
...
Lailaba intentionally keeps its skill (`skills/computer-use/SKILL.md`)
focused on the Lailaba-side `computer_use` action vocabulary — the
...
These are **platform deep dives, not duplicates of the Lailaba skill** —
...
Code, Codex, OpenCode, OpenClaw, and Antigravity; **Lailaba
...
Lailaba applies multi-layer guardrails:
...
Pair with `approvals.mode: manual` in `~/.lailaba/config.yaml` if you
want every action confirmed.

## Token efficiency

Screenshots are expensive. Lailaba applies four layers of optimisation:
...
- **Windows: elevated (admin) windows can't be driven from a normal
  ...
  Lailaba agent) cannot enumerate the UIA tree of, or inject mouse input
  ...
  run the Lailaba agent itself at High integrity (launch from an
...
- **Platform-specific deployment gotchas:**
  - **macOS** uses private SkyLight SPIs. Apple can change them in any
    OS update. Lailaba warns when the installed cua-driver is older than
    the version it was tested against.
  - **Windows** SSH sessions run in **Session 0**, which has no
    interactive desktop. Drive Lailaba from inside the RDP / console
...
```
LAILABA_CUA_DRIVER_CMD=/path/to/your/cua-driver
```

Swap the backend entirely (for testing):

```
LAILABA_COMPUTER_USE_BACKEND=noop   # records calls, no side effects
```

### Telemetry

cua-driver ships with anonymous usage telemetry (PostHog) enabled by default
upstream. **Lailaba disables it for you** — on every cua-driver invocation
(the MCP backend, `status`, `doctor`, and install) Lailaba sets
...
When it's on, `lailaba computer-use doctor` reports `telemetry: enabled`;
...
unreleased fix — point Lailaba at a binary you built from source instead
of the published release. Lailaba resolves the driver with
`shutil.which("cua-driver")` and **does not enforce
`LAILABA_CUA_DRIVER_VERSION`**, so a local build (reported as
...
- `-NoAutoStart` skips registering the `cua-driver-serve` logon daemon
  — you don't need it for Lailaba testing (see notes).
...
### Option B — point Lailaba straight at the built binary (fastest loop)

Skip the install ceremony entirely: `cargo build` and set
`LAILABA_CUA_DRIVER_CMD` to the resulting binary. Best for rapid
edit/build/test.
...
```
# Windows (.env)
LAILABA_CUA_DRIVER_CMD=C:\path\to\cua\libs\cua-driver\rust\target\debug\cua-driver.exe
# macOS / Linux (.env)
LAILABA_CUA_DRIVER_CMD=/path/to/cua/libs/cua-driver/rust/target/debug/cua-driver
```

### Confirm Lailaba is using your build

- `lailaba computer-use status` prints the resolved binary path and
  version.
- `lailaba computer-use doctor` confirms the binary is reachable and
...
- **Lailaba spawns its own `cua-driver mcp` child over stdio** — it does
...
- **Rebuild loop.** After editing cua-driver source, re-run
  ...
  for Option A, or just re-`cargo build` for Option B — no Lailaba
  change needed either way.
- **Local builds skip the version check.** Lailaba warns when the
...
**First action when anything's off: run `lailaba computer-use doctor`.**
...
Run `lailaba computer-use install` to fetch the cua-driver binary, or
run `lailaba tools` and enable the Computer Use toolset.
...
Wayland without an XWayland bridge. `lailaba computer-use doctor` will
...
- **Lailaba-side skill** — `skills/computer-use/SKILL.md` — teaches the
  Lailaba `computer_use` action vocabulary; this is what the agent loads.
- **cua-driver skill pack** — for platform-specific deep dives
  ...
  install` autodetects Lailaba (planned follow-up), this happens
...
- **cua.ai/docs** — the cua-driver project's documentation:
  ...
  - [Connect your agent](https://cua.ai/docs/how-to-guides/driver/connect-your-agent) — register cua-driver with various harnesses (Lailaba among them)
