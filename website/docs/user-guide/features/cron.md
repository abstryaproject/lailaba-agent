...
Schedule tasks to run automatically with natural language or cron expressions. Lailaba exposes cron management through a single `cronjob` tool with action-style operations instead of separate schedule/list/remove tools.
...
All of this is available to Lailaba itself through the `cronjob` tool, so you can create, pause, edit, and remove jobs by asking in plain language — no CLI required.

:::tip
At creation, an unpinned job (one you don't give an explicit `provider`/`model`) follows the global default selected by `lailaba model` — and Lailaba **snapshots** that provider and model on the job. If the global default later changes, the job **fails closed**: it skips the run, makes no inference call, and sends an alert telling you to pin the provider/model explicitly (`cronjob action=update job_id=… provider=… model=…`) to proceed. This prevents an unattended job from silently inheriting a switch to a paid provider/model and spending money you didn't intend (#44585). To make a job deliberately track your global default, pin it to the new values after changing them. `lailaba setup --portal` is the lowest-friction option for unattended runs since OAuth refresh is automatic. See [Nous Portal](/integrations/nous-portal).
:::

:::warning
Cron-run sessions cannot recursively create more cron jobs. Lailaba disables cron management tools inside cron executions to prevent runaway scheduling loops.
:::
...
```bash
lailaba cron create "every 2h" "Check server status"
lailaba cron create "every 1h" "Summarize new feed items" --skill blogwatcher
lailaba cron create "every 1h" "Use both skills and combine the result" \
  --skill blogwatcher \
  --skill maps \
  --name "Skill combo"
```
...
Ask Lailaba normally:

```text
Every morning at 9am, check Hacker News for AI news and send me a summary on Telegram.
```

Lailaba will use the unified `cronjob` tool internally.
...
lailaba cron create "every 1d at 09:00" \
...
```bash
lailaba cron edit <job_id> --schedule "every 4h"
lailaba cron edit <job_id> --prompt "Use the revised task"
lailaba cron edit <job_id> --skill blogwatcher --skill maps
lailaba cron edit <job_id> --add-skill maps
lailaba cron edit <job_id> --remove-skill blogwatcher
lailaba cron edit <job_id> --clear-skills
```
...
```bash
lailaba cron list
lailaba cron pause <job_id_or_name>
lailaba cron resume <job_id_or_name>
lailaba cron run <job_id_or_name>
lailaba cron remove <job_id_or_name>
lailaba cron edit <job_id_or_name> [...flags]
lailaba cron status
lailaba cron tick
```
...
```bash
lailaba gateway install     # Install as a user service
sudo lailaba gateway install --system   # Linux: boot-time system service for servers
lailaba gateway             # Or run in foreground
...
On each tick Lailaba:

1. loads jobs from `~/.lailaba/cron/jobs.json`
...
A file lock at `~/.lailaba/cron/.tick.lock` prevents overlapping scheduler ticks from double-running the same job batch.
...
| `"local"` | Save to local files only (`~/.lailaba/cron/output/`) | Default on CLI |
...
If the agent's final response contains `[SILENT]`, delivery is suppressed entirely. The output is still saved locally for audit (in `~/.lailaba/cron/output/`), but no message is sent to the delivery target.
...
Or set the `LAILABA_CRON_SCRIPT_TIMEOUT` environment variable. The resolution order is: env var → config.yaml → 120s default.
...
`.sh` / `.bash` files run under `/bin/bash`; anything else under the current Python interpreter (`sys.executable`). Scripts must live in `~/.lailaba/scripts/` (same sandboxing rule as the pre-run script gate).

### The agent sets these up for you

The `cronjob` tool's schema exposes `no_agent` to Lailaba directly, so you can describe a watchdog in chat and let the agent wire it up:

```text
Ping me on Telegram if RAM is over 85%, every 5 minutes.
```

Lailaba will write the check script to `~/.lailaba/scripts/` via `write_file`, then call:
...
cronjob(
    action="create",
    prompt="Fetch the top 10 AI/ML stories from Hacker News. Save them to ~/.lailaba/data/briefs/raw.md in markdown format with title, URL, and score.",
    schedule="0 7 * * *",
...
cronjob(
    action="create",
    prompt="Read ~/.lailaba/data/briefs/raw.md. Score each story 1–10 for engagement potential and novelty. Output the top 5 to ~/.lailaba/data/briefs/ranked.md.",
    schedule="30 7 * * *",
...
cronjob(
    action="create",
    prompt="Read ~/.lailaba/data/briefs/ranked.md. Write 3 tweet drafts (hook + body + hashtags). Deliver to telegram:7976161601.",
    schedule="0 8 * * *",
...
- When Job 2 fires, Lailaba reads Job 1's most recent output from `~/.lailaba/cron/output/{job1_id}/*.md`
...
The agent's final response is automatically delivered — you do **not** need to include `send_message` in the cron prompt for that same destination. If a cron run calls `send_message` to the exact target the scheduler will already deliver to, Lailaba skips that duplicate send and tells the model to put the user-facing content in the final response instead. Use `send_message` only for additional or different targets.
...
Cron runs each job in a fresh agent session with no chat platform attached. By default the cron agent gets **the toolset you configured for the `cron` platform in `lailaba tools`** — not the CLI default, not everything under the sun.
...
When `enabled_toolsets` is set on a job it wins; otherwise the `lailaba tools` cron-platform config wins; otherwise Lailaba falls back to the built-in defaults. This matters for cost control: carrying `browser`, `delegation` into every tiny "fetch news" job bloats the tool-schema prompt on every LLM call.

### Skipping the agent entirely: `wakeAgent`

If your cron job attaches a pre-check script (via `script=`), the script can decide at runtime whether Lailaba should even invoke the agent. Emit a final stdout line of the form:
...
```bash
#!/bin/bash
# ~/.lailaba/scripts/feed-changed.sh
FEED="$HOME/data/feed.json"
STATE="$HOME/.lailaba/scripts/.feed-changed.last"
...
:::tip
Lailaba's own `~/.lailaba/state.db` is an internal schema that changes between releases. Don't query it from a pre-run gate — point at your own database or feed instead.
:::

Credit: this recipe set was prompted by @iankar8's exploration in [#2654](https://github.com/NousResearch/lailaba-agent/pull/2654), which proposed adding sql/file/command triggers as a parallel mechanism. The `script` + `wakeAgent` gate already covers all three cases at $0, so the work landed as documentation instead.
...
Jobs are stored in `~/.lailaba/cron/jobs.json`. Output from job runs is saved to `~/.lailaba/cron/output/{job_id}/{timestamp}.md`.

Jobs may store `model` and `provider` as `null`. When those fields are omitted, Lailaba resolves them at execution time from the global configuration. They only appear in the job record when a per-job override is set.
