...
Lailaba can show an animated **pet** — a small mascot sprite that reacts to what
...
- Pets are installed into your profile's `pets/` directory
  (`<LAILABA_HOME>/pets/<slug>/`), so each [profile](../profiles.md) keeps its
  own set.
...
In the terminal (CLI/TUI), Lailaba renders the sprite at full fidelity when your
...
lailaba pets list
lailaba pets list cat

# Install a pet and make it active in one step
lailaba pets install boba --select
...
## `lailaba pets` commands

| Goal | Command |
| --- | --- |
| Browse the gallery | `lailaba pets list [query] [--limit N]` |
| List installed pets | `lailaba pets list --installed` |
| Install a pet | `lailaba pets install <slug> [--select] [--force]` |
...
| Preview/animate | `lailaba pets show [slug] [--state <s>] [--cycle] [--once] [--mode <m>] [--scale <f>]` |
| Disable the pet | `lailaba pets off` |
| Remove an installed pet | `lailaba pets remove <slug>` |
| Diagnose setup | `lailaba pets doctor` |

`lailaba pets show` flags:
...
always-on-top desktop window. Out there it stays visible while Lailaba is
...
Run `lailaba pets doctor` — it reports:
