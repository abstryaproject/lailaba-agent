...
chat completions — use your Lailaba-managed provider subscription as their
...
| Use case | "Use Lailaba as a chat backend" | "Use my Portal sub from another app" |
...
This opens your browser for the Nous Portal OAuth flow. Lailaba stores
the refresh token in `~/.lailaba/auth.json` — the same place all Lailaba
...
Model:      Lailaba-4-70B    # or Lailaba-4.3-36B, Lailaba-4-405B
```
...
interface in `lailaba_cli/proxy/adapters/`.
...
If you see `not logged in`, run `lailaba portal`. If you see
...
`lailaba_cli/proxy/adapters/<provider>.py` and registering it in
