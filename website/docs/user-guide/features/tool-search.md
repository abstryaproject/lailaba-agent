...
**Tool Search** is Lailaba' opt-in progressive-disclosure layer for that
...
:::info Built-in Lailaba tools never defer
The tools that make up Lailaba' core capability set (`terminal`,
`read_file`, `write_file`, `patch`, `search_files`, `todo`, `memory`,
`browser_*`, `web_search`, `web_extract`, `clarify`, `execute_code`,
`delegate_task`, `session_search`, `send_message`, and the rest of
`_LAILABA_CORE_TOOLS`) are *always* loaded directly. Only MCP tools and
...
When the model invokes `tool_call`, Lailaba **unwraps the bridge** and
...
- **No JS sandbox.** Lailaba uses the simpler "structured tools" mode
