...
Lailaba AI ships with 8 external memory provider plugins that give the agent persistent, cross-session knowledge beyond the built-in MEMORY.md and USER.md. Only **one** external provider can be active at a time — the built-in memory is always active alongside it.

## Quick Start

```bash
lailaba memory setup      # interactive picker + configuration
lailaba memory status     # check what's active
lailaba memory off        # disable external provider
```

You can also select the active memory provider via `lailaba plugins` → Provider Plugins → Memory Provider.

Or set manually in `~/.lailaba/config.yaml`:
...
When a memory provider is active, Lailaba automatically:
...
**Setup Wizard:**
```bash
lailaba memory setup        # select "honcho" — runs the Honcho-specific post-setup
```

The legacy `lailaba honcho setup` command still works (it now redirects to `lailaba memory setup`), but is only registered after Honcho is selected as the active memory provider.

**Config:** `$LAILABA_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global). Resolution order: `$LAILABA_HOME/honcho.json` > `~/.lailaba/honcho.json` > `~/.honcho/config.json`. See the [config reference](https://github.com/NousResearch/lailaba-agent/blob/main/plugins/memory/honcho/README.md) and the [Honcho integration guide](https://docs.honcho.dev/v3/guides/integrations/lailaba).
...
:::tip Migrating from `lailaba honcho`
If you previously used `lailaba honcho setup`, your config and all server-side data are intact. Just re-enable through the setup wizard again or manually set `memory.provider: honcho` to reactivate via the new system.
:::

**Multi-peer setup:**

Honcho models conversations as peers exchanging messages — one user peer plus one AI peer per Lailaba profile, all sharing a workspace. The workspace is the shared environment: the user peer is global across profiles, each AI peer is its own identity. Every AI peer builds an independent representation / card from its own observations, so a `coder` profile stays code-oriented while a `writer` profile stays editorial against the same user.

The mapping:

| Concept | What it is |
|---------|-----------|
| **Workspace** | Shared environment. All Lailaba profiles under one workspace see the same user identity. |
| **User peer** (`peerName`) | The human. Shared across profiles in the workspace. |
| **AI peer** (`aiPeer`) | One per Lailaba profile. Host key `lailaba` → default; `lailaba.<profile>` for others. |
...
```bash
lailaba profile create coder --clone
```

`--clone` creates a `lailaba.coder` host block in `honcho.json` with `aiPeer: "coder"`, shared `workspace`, inherited `peerName`, `recallMode`, `writeFrequency`, `observation`, etc. The AI peer is eagerly created in Honcho so it exists before the first message.
...
Scans every Lailaba profile, creates host blocks for any profile without one, inherits settings from the default `lailaba` block, and creates the new AI peers eagerly. Idempotent — skips profiles that already have a host block.
...
Off-gateway these keys do nothing. `lailaba memory setup` only prompts for them when it detects a connected gateway platform. See the [Honcho page](./honcho.md#gateway-identity-mapping) for the resolver ladder and the setup flow.
...
See the [config reference](https://github.com/NousResearch/lailaba-agent/blob/main/plugins/memory/honcho/README.md) and [Honcho integration guide](https://docs.honcho.dev/v3/guides/integrations/lailaba).
...
lailaba memory setup    # select "openviking"
# Or manually:
lailaba config set memory.provider openviking
echo "OPENVIKING_ENDPOINT=http://localhost:1933" >> ~/.lailaba/.env
# Authenticated servers should use a user/admin API key:
echo "OPENVIKING_API_KEY=..." >> ~/.lailaba/.env
```
...
`OPENVIKING_AGENT` is Lailaba' peer ID in OpenViking for peer-scoped memories.
...
**Setup (Platform):**
```bash
lailaba memory setup    # select "mem0" → "Platform"
# Or manually:
lailaba config set memory.provider mem0
echo "MEM0_API_KEY=your-key" >> ~/.lailaba/.env
```

**Setup (OSS):**
```bash
lailaba memory setup    # select "mem0" → "Open Source (self-hosted)"
# Or via flags:
lailaba memory setup mem0 --mode oss --oss-llm openai --oss-llm-key sk-... --oss-vector qdrant
```

Preview without writing files:
```bash
lailaba memory setup mem0 --mode oss --oss-llm-key sk-... --dry-run
```

**Config:** `$LAILABA_HOME/mem0.json` (behavioral settings). Only the secret `MEM0_API_KEY` belongs in `~/.lailaba/.env`.

| Key | Default | Description |
|-----|---------|-------------|
| `mode` | `platform` | `platform` (Mem0 Cloud) or `oss` (self-hosted) |
| `user_id` | `lailaba-user` | User identifier |
| `agent_id` | `lailaba` | Agent identifier |
...
**Switching modes:** Re-run `lailaba memory setup mem0 --mode <platform|oss>` or edit `mem0.json` directly.
...
**Setup:**
```bash
lailaba memory setup    # select "hindsight"
# Or manually:
lailaba config set memory.provider hindsight
echo "HINDSIGHT_API_KEY=your-key" >> ~/.lailaba/.env
```

The setup wizard installs dependencies automatically and only installs what's needed for the selected mode (`hindsight-client` for cloud, `hindsight-all` for local). Requires `hindsight-client >= 0.4.22` (auto-upgraded on session start if outdated).

**Local mode UI:** `hindsight-embed -p lailaba ui start`

**Config:** `$LAILABA_HOME/hindsight/config.json`

| Key | Default | Description |
|-----|---------|-------------|
| `mode` | `cloud` | `cloud` or `local` |
| `bank_id` | `lailaba` | Memory bank identifier |
| `recall_budget` | `mid` | Recall thoroughness: `low` / `mid` / `high` |
| `memory_mode` | `hybrid` | `hybrid` (context + tools), `context` (auto-inject only), `tools` (tools only) |
| `auto_retain` | `true` | Automatically retain conversation turns |
| `auto_recall` | `true` | Automatically recall memories before each turn |
| `retain_async` | `true` | Process retain asynchronously on the server |
| `retain_context` | `conversation between Lailaba AI and the User` | Context label for retained memories |
...
See [plugin README](https://github.com/NousResearch/lailaba-agent/blob/main/plugins/memory/hindsight/README.md) for the full configuration reference.
...
**Setup:**
```bash
lailaba memory setup    # select "holographic"
# Or manually:
lailaba config set memory.provider holographic
```

**Config:** `config.yaml` under `plugins.lailaba-memory-store`

| Key | Default | Description |
|-----|---------|-------------|
| `db_path` | `$LAILABA_HOME/memory_store.db` | SQLite database path |
...
**Setup:**
```bash
lailaba memory setup    # select "retaindb"
# Or manually:
lailaba config set memory.provider retaindb
echo "RETAINDB_API_KEY=your-key" >> ~/.lailaba/.env
```
...
# Then configure Lailaba
lailaba memory setup    # select "byterover"
# Or manually:
lailaba config set memory.provider byterover
```

**Key features:**
- Automatic pre-compression extraction (saves insights before context compression discards them)
- Knowledge tree stored at `$LAILABA_HOME/byterover/` (profile-scoped)
...
| **Requires** | `pip install supermemory` + [API key](http://app.supermemory.ai/integrations?connect=lailaba) |
...
**Setup:**
```bash
lailaba memory setup    # select "supermemory"
# Or manually:
lailaba config set memory.provider supermemory
echo 'SUPERMEMORY_API_KEY=***' >> ~/.lailaba/.env
```

**Config:** `$LAILABA_HOME/supermemory.json`

| Key | Default | Description |
|-----|---------|-------------|
| `container_tag` | `lailaba` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags. |
...
- **Profile-scoped containers** — use `{identity}` in `container_tag` (e.g. `lailaba-{identity}` → `lailaba-coder`) to isolate memories per Lailaba profile
...
| **Requires** | `pip install lailaba-memori` + `lailaba-memori install` + [Memori API key](https://app.memorilabs.ai/signup) |
...
**Setup:**
```bash
pip install lailaba-memori
lailaba-memori install
lailaba config set memory.provider memori
lailaba memory setup
```
...
| **Memori** | Cloud | Free/Paid | 5 | `lailaba-memori` | Tool-aware memory + structured recall |

## Profile Isolation

Each provider's data is isolated per [profile](/user-guide/profiles):

- **Local storage providers** (Holographic, ByteRover) use `$LAILABA_HOME/` paths which differ per profile
- **Config file providers** (Honcho, Mem0, Hindsight, Supermemory) store config in `$LAILABA_HOME/` so each profile has its own credentials
