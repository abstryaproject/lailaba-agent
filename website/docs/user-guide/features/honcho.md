...
[Honcho](https://github.com/plastic-labs/honcho) is an AI-native memory backend that adds dialectic reasoning and deep user modeling on top of Lailaba's built-in memory system. Instead of simple key-value storage, Honcho maintains a running model of who the user is — their preferences, communication style, goals, and patterns — by reasoning about conversations after they happen.
...
**Multi-agent profiles**: When multiple Lailaba instances talk to the same user (e.g., a coding assistant and a personal assistant), Honcho maintains separate "peer" profiles. Each peer sees only its own observations and conclusions, preventing cross-contamination of context.

## Setup

```bash
lailaba memory setup    # select "honcho" from the provider list
```
...
- `per-session` — each `lailaba` run gets a fresh session. Clean starts, memory via tools. Recommended for new users.
...
Server-side toggles set via the [Honcho dashboard](https://app.honcho.dev) win over local defaults — Lailaba syncs them back at session init.
...
The `lailaba honcho` subcommand is **only registered when Honcho is the active memory provider** (`memory.provider: honcho` in `config.yaml`). On a fresh install, configure Honcho directly with `lailaba memory setup honcho` (or run `lailaba memory setup` and pick it from the list); the `lailaba honcho` subcommand then appears on the next invocation.

```bash
lailaba memory setup honcho    # Configure Honcho directly (works before activation)
lailaba honcho status          # Connection status, config, and key settings
lailaba honcho setup           # Redirects to `lailaba memory setup` (post-activation alias)
lailaba honcho strategy        # Show or set session strategy (per-session/per-directory/per-repo/global)
lailaba honcho peer            # Show or update peer names + dialectic reasoning level
lailaba honcho mode            # Show or set recall mode (hybrid/context/tools)
lailaba honcho tokens          # Show or set token budget for context and dialectic
lailaba honcho identity        # Seed or show the AI peer's Honcho identity
lailaba honcho sync            # Sync Honcho config to all existing profiles
lailaba honcho peers           # Show peer identities across all profiles
lailaba honcho sessions        # List known Honcho session mappings
lailaba honcho map             # Map current directory to a Honcho session name
lailaba honcho enable          # Enable Honcho for the active profile
lailaba honcho disable         # Disable Honcho for the active profile
lailaba honcho migrate         # Step-by-step migration guide from openclaw-honcho
```

## Migrating from `lailaba honcho`

If you previously used the standalone `lailaba honcho setup`:
...
No re-login or re-setup needed. Run `lailaba memory setup` and select "honcho" — the wizard detects your existing config.
