...
Type `@` followed by a reference to inject content directly into your message. Lailaba expands the reference inline and appends the content under an `--- Attached Context ---` section.
...
- Lailaba env: `$LAILABA_HOME/.env`

These directories are fully blocked (any file inside):
- `~/.ssh/`, `~/.aws/`, `~/.gnupg/`, `~/.kube/`, `$LAILABA_HOME/skills/.hub/`
