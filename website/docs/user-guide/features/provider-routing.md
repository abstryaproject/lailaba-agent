...
When using [OpenRouter](https://openrouter.ai) as your LLM provider, Lailaba AI supports **provider routing** — fine-grained control over which underlying AI providers handle your requests and how they're prioritized.
...
Add a `provider_routing` section to your `~/.lailaba/config.yaml`:
...
- **CLI mode** — configured in `~/.lailaba/config.yaml`, loaded at startup
