...
title: "Git Worktrees"
description: "Run multiple Lailaba agents safely on the same repository using git worktrees and isolated checkouts"
---

# Git Worktrees

Lailaba AI is often used on large, long‑lived repositories. When you want to:
...
This page shows how to combine worktrees with Lailaba so each session has a clean, isolated working directory.

## Why Use Worktrees with Lailaba?

Lailaba treats the **current working directory** as the project root:

- CLI: the directory where you run `lailaba` or `lailaba chat`
- Messaging gateways: the directory set by `terminal.cwd` in `~/.lailaba/config.yaml`
...
git worktree add ../repo-feature feature/lailaba-experiment
```

This creates:

- A new directory: `../repo-feature`
- A new branch: `feature/lailaba-experiment` checked out in that directory

Now you can `cd` into the new worktree and run Lailaba there:

```bash
cd ../repo-feature

# Start Lailaba in the worktree
lailaba
```
...
git worktree add ../repo-experiment-a feature/lailaba-a
git worktree add ../repo-experiment-b feature/lailaba-b
```
...
Each Lailaba process:

- Works on its own branch (`feature/lailaba-a` vs `feature/lailaba-b`).
...
- Lailaba checkpoint data under `~/.lailaba/checkpoints/` is not automatically pruned when you remove a worktree, but it is usually very small.

## Best Practices

- **One worktree per Lailaba experiment**
...
- **Name branches after the experiment**
  - e.g. `feature/lailaba-checkpoints-docs`, `feature/lailaba-refactor-tests`.
- **Commit frequently**
...
- **Avoid running Lailaba from the bare repo root when using worktrees**
  - Prefer the worktree directories instead, so each agent has a clear scope.

## Using `lailaba -w` (Automatic Worktree Mode)

Lailaba has a built‑in `-w` flag that **automatically creates a disposable git worktree** with its own branch. You don't need to set up worktrees manually — just `cd` into your repo and run:
...
- Check out an isolated branch (e.g. `lailaba/lailaba-<hash>`).
...
```bash
lailaba -w -z "Fix issue #123"
```

For parallel agents, open multiple terminals and run `lailaba -w` in each — every invocation gets its own worktree and branch automatically.

## Putting It All Together

- Use **git worktrees** to give each Lailaba session its own clean checkout.
