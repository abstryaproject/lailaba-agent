...
Lailaba AI automatically saves every conversation as a session. Sessions enable conversation resume, cross-session search, and full conversation history management.
...
1. **SQLite database** (`~/.lailaba/state.db`) — structured session metadata with FTS5 full-text search, plus full message history
...
Lailaba stores session history so it can resume conversations, but it does not
...
Lailaba explicitly injects for that turn.
...
For example, if a user sends an image and asks Lailaba to make a meme from it,
Lailaba may inspect that image once with vision and run an image-processing
...
`lailaba sessions prune` only when you want to delete old ended sessions from
...
| Source | Description |
|--------|-------------|
| `cli` | Interactive CLI (`lailaba` or `lailaba chat`) |
...
lailaba -c "my project"

# If there are lineage variants (my project, my project #2, my project #3),
# this automatically resumes the most recent one
lailaba -c "my project"   # → resumes "my project #3"
```
...
lailaba --resume 20250305_091523_a1b2c3d4
lailaba -r 20250305_091523_a1b2c3d4

# Resume by title
lailaba --resume "refactoring auth"

# Or with the chat subcommand
lailaba chat --resume 20250305_091523_a1b2c3d4
```

Session IDs are shown when you exit a CLI session, and can be found with `lailaba sessions list`.

### Conversation Recap on Resume

When you resume a session, Lailaba displays a compact recap of the previous conversation in a styled panel before the input prompt:

<img className="docs-terminal-figure" src={useBaseUrl('/img/docs/session-recap.svg')} alt="Stylized preview of the Previous Conversation recap panel shown when resuming a Lailaba session." />
...
To disable the recap and keep the minimal one-liner behavior, set in `~/.lailaba/config.yaml`:
...
**Resume back to CLI:** when you want to come back to a desktop, just run `/resume <title>` (or `lailaba -r "<title>"` from the shell) and pick up where the platform left off.
...
Lailaba automatically generates a short descriptive title (3–7 words) for each session after the first exchange. This runs in a background thread using a fast auxiliary model, so it adds no latency. You'll see auto-generated titles when browsing sessions with `lailaba sessions list` or `lailaba sessions browse`.
...
```bash
lailaba sessions rename 20250305_091523_a1b2c3d4 "refactoring auth module"
```
...
When a session's context is compressed (manually via `/compress` or automatically), Lailaba creates a new continuation session. If the original had a title, the new session automatically gets a numbered title:

```
"my project" → "my project #2" → "my project #3"
```

When you resume by name (`lailaba -c "my project"`), it automatically picks the most recent session in the lineage.
...
Lailaba provides a full set of session management commands via `lailaba sessions`:

### List Sessions

```bash
# List recent sessions (default: last 20)
lailaba sessions list

# Filter by platform
lailaba sessions list --source telegram

# Show more sessions
lailaba sessions list --limit 50
```
...
lailaba sessions export backup.jsonl

# Export sessions from a specific platform
lailaba sessions export telegram-history.jsonl --source telegram

# Export a single session
lailaba sessions export session.jsonl --session-id 20250305_091523_a1b2c3d4
```
...
lailaba sessions delete 20250305_091523_a1b2c3d4

# Delete without confirmation
lailaba sessions delete 20250305_091523_a1b2c3d4 --yes
```
...
lailaba sessions rename 20250305_091523_a1b2c3d4 "debugging auth flow"
...
lailaba sessions prune

# Custom age threshold
lailaba sessions prune --older-than 30

# Only prune sessions from a specific platform
lailaba sessions prune --source telegram --older-than 60

# Skip confirmation
lailaba sessions prune --older-than 30 --yes
```
...
For deeper analytics — token usage, cost estimates, tool breakdown, and activity patterns — use [`lailaba insights`](/reference/cli-commands#lailaba-insights).
...
When Lailaba cannot get a participant identifier for a shared chat, it falls back to one shared session for that room.

### Shared vs Isolated Group Sessions

By default, Lailaba uses `group_sessions_per_user: true` in `config.yaml`. That means:

- Alice and Bob can both talk to Lailaba in the same Discord channel without sharing transcript history
...
|------|------|-------------|
| SQLite database | `~/.lailaba/state.db` | All session metadata + messages with FTS5 |
| Gateway messages    | `~/.lailaba/state.db`   | SQLite — canonical store for all session messages |
| Gateway routing index | `~/.lailaba/sessions/sessions.json` | Maps session keys to active session IDs (origin metadata, expiry flags) |

The SQLite database uses WAL mode for concurrent readers and a single writer, which suits the gateway's multi-platform architecture well.

:::warning `sessions.json` is not the session list
`~/.lailaba/sessions/sessions.json` is the **gateway routing index** — it maps
...
`lailaba sessions list`, `/sessions`, and the dashboard all read `state.db`,
which holds **every** session (CLI, TUI, and gateway). The `/save` snapshots
under `~/.lailaba/sessions/saved/*.json` are convenience exports, not the index.

If CLI sessions genuinely don't appear in `lailaba sessions list`, the cause is
`state.db` not receiving them — run `lailaba sessions repair` and watch for a
...
`*.jsonl` files in `~/.lailaba/sessions/`. They are no longer written or
read by Lailaba. Safe to delete after verifying the corresponding session
exists in state.db.
:::
...
- Pruning runs at most once per `sessions.min_interval_hours` (default 24); the last-run timestamp is tracked inside `state.db` itself so it's shared across every Lailaba process in the same `LAILABA_HOME`

Default is **off** — session history is valuable for `session_search` recall, and silently deleting it could surprise users. Enable in `~/.lailaba/config.yaml`:
...
lailaba sessions delete <session_id>
