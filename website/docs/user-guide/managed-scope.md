...
`~/.lailaba/config.yaml`, `~/.lailaba/.env`, and even the shell environment — for
...
Managed scope is read from a system-level directory, default `/etc/lailaba`:

```text
/etc/lailaba/
├── config.yaml     # managed config layer (wins over ~/.lailaba/config.yaml)
└── .env            # managed env layer (wins over ~/.lailaba/.env + shell)
```
...
The location can be relocated with the `LAILABA_MANAGED_DIR` environment variable
(for containers or non-`/etc` deployments). This is a deployment/bootstrap path
knob — like `LAILABA_HOME` — set by the same administrator who owns the managed
files. It is **never persisted** to any `.env` by Lailaba.

```bash
# Point managed scope at a custom directory (set by IT / the deployment, not the user)
export LAILABA_MANAGED_DIR=/opt/org/lailaba-policy
```

:::warning
A user who can set `LAILABA_MANAGED_DIR` can repoint managed scope at a directory
...
left user-settable. `lailaba doctor` reports the *resolved* managed directory so
...
|---|---|---|
| 1 | `/etc/lailaba/config.yaml` (managed) | `/etc/lailaba/.env` (managed) |
| 2 | `~/.lailaba/config.yaml` (user) | `~/.lailaba/.env` (user) |
...
```bash
lailaba config        # shows a header naming the managed source + the pinned keys
lailaba doctor        # reports the resolved managed dir + pinned key counts
```

If you try to change a managed value, Lailaba refuses and names the source:

```bash
$ lailaba config set model.default my/model
Cannot set 'model.default': it is managed by your administrator
(/etc/lailaba/config.yaml) and cannot be changed.
```

The same applies to managed secrets — `lailaba config set` / setup will not write
a user value for an env key pinned by the managed `.env`.

## Setting up a managed scope (administrators)

```bash
sudo mkdir -p /etc/lailaba

# Pin some config values for every user on this machine
sudo tee /etc/lailaba/config.yaml >/dev/null <<'YAML'
model:
  provider: nous
security:
  redact_secrets: true
...
sudo tee /etc/lailaba/.env >/dev/null <<'ENV'
OPENAI_API_BASE=https://inference.example.com/v1
ENV

sudo chmod 0755 /etc/lailaba
sudo chmod 0644 /etc/lailaba/config.yaml /etc/lailaba/.env
```

Changes take effect on the next Lailaba start (a malformed managed file is logged
loudly and ignored — it never blocks startup, but the admin should check
`lailaba doctor` to confirm the policy is being applied).

## Security model and limitations (v1)

- **Enforcement is filesystem permissions only.** If a user has write access to
  the managed directory (or runs Lailaba as `root`), managed scope is advisory.
