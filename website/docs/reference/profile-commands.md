...
This page covers all commands related to [Lailaba profiles](../user-guide/profiles.md). For general CLI commands, see [CLI Commands Reference](./cli-commands.md).
...
Top-level command for managing profiles. Running `lailaba profile` without a subcommand shows help.
...
```bash
lailaba profile use <name>
```

Sets `<name>` as the active profile. All subsequent `lailaba` commands (without `-p`) will use this profile.
...
lailaba profile use default
```

## `lailaba profile create`

```bash
lailaba profile create <name> [options]
```
...
| `--description "<text>"` | One- or two-sentence description of what this profile is good at. Used by the kanban orchestrator to route tasks based on role instead of profile name alone. Skip and add later via `lailaba profile describe`. Persisted in `<profile_dir>/profile.yaml`. |
| `--no-skills` | Create an **empty** profile with zero bundled skills enabled. Writes a `.no-bundled-skills` marker into the profile so future `lailaba update` runs won't re-seed the bundled set, and refuses to combine with `--clone`, `--clone-from`, or `--clone-all` (which would copy skills in anyway). Useful for narrow orchestrator profiles or sandbox profiles that should not inherit the full skill catalog. To toggle this on an already-created profile (including the default `~/.lailaba`), use `lailaba skills opt-out` / `lailaba skills opt-in`. |
...
lailaba profile create work2 --clone-from work

# Clone everything from a specific profile
lailaba profile create work2-backup --clone-from work --clone-all
```

## `lailaba profile describe`

```bash
lailaba profile describe [<name>] [options]
```
...
lailaba profile describe researcher

# Set it explicitly
lailaba profile describe researcher --text "Reads source code and writes findings."

# Let the LLM generate one
lailaba profile describe researcher --auto

# Fill in descriptions for every profile that doesn't have one
lailaba profile describe --all --auto
```

## `lailaba profile delete`

```bash
lailaba profile delete <name> [options]
```
...
```bash
lailaba profile delete mybot
lailaba profile delete mybot --yes
```
...
## `lailaba profile show`

```bash
lailaba profile show <name>
```

Displays details about a profile including its home directory, configured model, gateway status, skills count, and configuration file status.

This shows the profile's Lailaba home directory, not the terminal working directory. Terminal commands start from `terminal.cwd` (or the launch directory on the local backend when `cwd: "."`).
...
```bash
$ lailaba profile show work
Profile: work
Path:    ~/.lailaba/profiles/work
...
## `lailaba profile alias`

```bash
lailaba profile alias <name> [options]
```

Regenerates the shell alias script at `~/.local/bin/<name>`. Useful if the alias was accidentally deleted or if you need to update it after moving your Lailaba installation.
...
```bash
lailaba profile alias work
# Creates/updates ~/.local/bin/work

lailaba profile alias work --name mywork
# Creates ~/.local/bin/mywork

lailaba profile alias work --remove
# Removes the wrapper script
```

## `lailaba profile rename`

```bash
lailaba profile rename <old-name> <new-name>
```
...
```bash
lailaba profile rename mybot assistant
# ~/.lailaba/profiles/mybot → ~/.lailaba/profiles/assistant
# ~/.local/bin/mybot → ~/.local/bin/assistant
```

## `lailaba profile export`

```bash
lailaba profile export <name> [options]
```
...
```bash
lailaba profile export work
# Creates work.tar.gz in the current directory

lailaba profile export work -o ./work-2026-03-29.tar.gz
```

## `lailaba profile import`

```bash
lailaba profile import <archive> [options]
```
...
```bash
lailaba profile import ./work-2026-03-29.tar.gz
# Infers profile name from the archive

lailaba profile import ./work-2026-03-29.tar.gz --name work-restored
```
...
:::info
`lailaba profile export` / `import` are still the right commands for
...
### `lailaba profile install`

```bash
lailaba profile install <source> [--name <name>] [--alias] [--force] [--yes]
```
...
| `--alias` | Also create a shell wrapper (e.g. `telemetry` → `lailaba -p telemetry`). |
...
lailaba profile install github.com/kyle/telemetry-distribution --alias

# Install from a full HTTPS git URL
lailaba profile install https://github.com/kyle/telemetry-distribution.git

# Install from SSH
lailaba profile install git@github.com:kyle/telemetry-distribution.git

# Install from a local directory during development
lailaba profile install ./telemetry/
```

### `lailaba profile update`

```bash
lailaba profile update <name> [--force-config] [--yes]
```
...
### `lailaba profile info`

```bash
lailaba profile info <name>
```

Prints the profile's distribution manifest — name, version, required
Lailaba version, author, env var requirements, the source URL/path, and
...
`lailaba profile list` also shows the distribution name and version in a
`Distribution` column, and `lailaba profile show <name>` / `delete <name>`
...
lailaba profile install git@github.com:your-org/internal-assistant.git

# Uses your git credential helper
lailaba profile install https://github.com/your-org/internal-assistant.git
```
...
`lailaba_requires` supports `>=`, `<=`, `==`, `!=`, `>`, `<`, or a bare
version (treated as `>=`). Install fails with a clear error if the current
Lailaba version doesn't satisfy the spec.
...
2. Initialize a git repo (or use an existing one) and push to GitHub /
   GitLab / any host Lailaba can clone from.
3. Tell recipients to run `lailaba profile install <your-repo-url>`.
...
## `lailaba -p` / `lailaba --profile`

```bash
lailaba -p <name> <command> [options]
lailaba --profile <name> <command> [options]
```

Global flag to run any Lailaba command under a specific profile without changing the sticky default. This overrides the active profile for the duration of the command.
...
```bash
lailaba -p work chat -q "Check the server status"
lailaba --profile dev gateway start
lailaba -p personal skills list
lailaba -p work config edit
```

## `lailaba completion`

```bash
lailaba completion <shell>
```
...
- `lailaba profile <TAB>` — subcommands (list, use, create, etc.)
- `lailaba profile use <TAB>` — profile names
- `lailaba -p <TAB>` — profile names
