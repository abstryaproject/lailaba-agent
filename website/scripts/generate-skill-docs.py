...
def rewrite_relative_links(body: str, meta: dict[str, Any]) -> str:
    ...
    base = f"https://github.com/NousResearch/lailaba-agent/blob/main/{source_dir}/{meta['rel_path']}"
...
def render_skill_page(
    ...
    lailaba_meta = (fm.get("metadata") or {}).get("lailaba") or {}
    tags = lailaba_meta.get("tags") or []
    related = lailaba_meta.get("related_skills") or []
    ...
    if meta["source_kind"] == "bundled":
        ...
        info_rows.append(
            (
                "Source",
                "Optional — install with `lailaba skills install official/"
                + meta["category"]
                + "/"
                + meta["slug"]
                + "`",
...
def build_catalog_md_bundled(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        'description: "Catalog of bundled skills that ship with Lailaba AI"',
        "---",
        "",
        "# Bundled Skills Catalog",
        "",
        "Lailaba ships with a large built-in skill library copied into `~/.lailaba/skills/` on install. Each skill below links to a dedicated page with its full definition, setup, and usage.",
        "",
        "Lailaba also syncs bundled skills on `lailaba update`, but the sync manifest respects local deletions and user edits. If a skill listed here is missing from your profile's `~/.lailaba/skills/` tree, it is still shipped with Lailaba; restore it with `lailaba skills reset <name> --restore`.",
        "",
