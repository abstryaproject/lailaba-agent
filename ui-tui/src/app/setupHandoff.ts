import type { RunExternalProcess } from '@lailaba/ink'
...
export async function runExternalSetup({ args, ctx, done, launcher, suspend }: RunExternalSetupOptions) {
  ...
  transcript.sys(`launching \`lailaba ${args.join(' ')}\`…`)
  ...
  if (result.error) {
    transcript.sys(`error launching lailaba: ${result.error}`)
  ...
  if (result.code !== 0) {
    transcript.sys(`lailaba ${args[0]} exited with code ${result.code}`)
    patchUiState({ status: 'setup required' })
