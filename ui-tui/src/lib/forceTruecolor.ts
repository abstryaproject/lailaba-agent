...
export function shouldForceTruecolor(env: NodeJS.ProcessEnv = process.env): boolean {
  const override = (env.LAILABA_TUI_TRUECOLOR ?? '').trim()
...
if (shouldForceTruecolor()) {
  ...
  // incorrectly. Keep Lailaba on the safer TERM-driven 256-color path unless
  // users explicitly opt back in via LAILABA_TUI_TRUECOLOR=1.
