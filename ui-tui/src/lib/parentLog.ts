...
const logDir = join(process.env.LAILABA_HOME?.trim() || join(homedir(), '.lailaba'), 'logs')
...
// a real ~/.lailaba (tests must stay hermetic — see AGENTS.md).
...
export function recordParentLifecycle(line: string): void {
  ...
  try {
    ...
    if (!warned) {
      warned = true
      process.stderr.write('lailaba-tui: parent lifecycle log unavailable\n')
    }
  }
}
