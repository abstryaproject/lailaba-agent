...
await build({
  ...
  // Skip the prebuilt @lailaba/ink bundle — esbuild's __esm helper doesn't
  // await nested async init, which breaks lazy-initialized exports like
  // `render`. Bundling from source sidesteps that.
  alias: { '@lailaba/ink': resolve(root, 'packages/lailaba-ink/src/entry-exports.ts') },
...
// the `node` token, leaving a broken interpreter). The lailaba_cli launcher
