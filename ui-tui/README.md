...
React + Ink terminal UI for Lailaba. TypeScript owns the screen. Python owns sessions, tools, model calls, and most command logic.
...
Interpreter resolution order is: `LAILABA_PYTHON` → `PYTHON` → `$VIRTUAL_ENV/bin/python` → `./.venv/bin/python` → `./venv/bin/python` → `python3` (or `python` on Windows).
...
- `src/app/setupHandoff.ts` — launches external `lailaba setup`, suspends Ink while it runs, opens a new session on success
...
- Input history is stored in `~/.lailaba/.lailaba_history` or under `LAILABA_HOME`.
...
### Setup (`setup.ts`)
`/setup` — launches external `lailaba setup` wizard, suspends Ink while it runs
...
```text
ui-tui/
  packages/lailaba-ink/   forked Ink renderer (local dep)
  src/
    ...
    app/
      ...
      setupHandoff.ts               launches external lailaba setup, suspends Ink while it runs
    ...
    lib/
      ...
      parentLog.ts               append-only log to ~/.lailaba/tui-parent.log
    ...
    types/
      lailaba-ink.d.ts            type declarations for @lailaba/ink
