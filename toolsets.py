...
_LAILABA_CORE_TOOLS = [
    ...
    "terminal", "process",
    # Read the desktop GUI's embedded terminal pane (gated on LAILABA_DESKTOP
    ...
    # spawned as a kanban worker (LAILABA_KANBAN_TASK env set) or the current
...
TOOLSETS = {
    ...
    "x_search": {
        "description": (
            ...
            "enable in `lailaba tools` → X (Twitter) Search."
        ),
        "tools": ["x_search"],
        "includes": []
    },
    ...
    "video_gen": {
        "description": (
            ...
            "``lailaba tools`` → Video Generation."
        ),
    ...
    "kanban": {
        "description": (
            ...
            "is spawned by the kanban dispatcher (LAILABA_KANBAN_TASK env "
    ...
    # Coding posture (base Lailaba — CLI/TUI/desktop/ACP). Auto-selected in a
    ...
    "coding": {
        ...
        # non-configurable-toolset recovery loop in lailaba_cli/tools_config.py).
        "posture": True,
    },
    
    # ==========================================================================
    # Full Lailaba toolsets (CLI + messaging platforms)
    #
    ...
    # the `lailaba send` CLI), not by the model deciding to send on its own.
    # ==========================================================================
    ...
    "lailaba-cli": {
        ...
        "tools": _LAILABA_CORE_TOOLS,
        "includes": []
    },

    "lailaba-cron": {
        # Mirrors lailaba-cli so cron's "default" toolset is the same set of
        # core tools users see interactively — then `lailaba tools` filters
        # them down per the platform config. _DEFAULT_OFF_TOOLSETS (moa,
        # homeassistant) are excluded by _get_platform_tools() unless
        # the user explicitly enables them.
        "description": "Default cron toolset - same core tools as lailaba-cli; gated by `lailaba tools`",
        "tools": _LAILABA_CORE_TOOLS,
        "includes": []
    },
    ...
    "lailaba-email": {
        "description": "Email bot toolset - interact with Lailaba via email (IMAP/SMTP)",
        "tools": _LAILABA_CORE_TOOLS,
        "includes": []
    },
    ...
    "lailaba-sms": {
        "description": "SMS bot toolset - interact with Lailaba via SMS (Twilio)",
        "tools": _LAILABA_CORE_TOOLS,
        "includes": []
    },
    ...
    "lailaba-webhook": {
        ...
        "tools": _LAILABA_WEBHOOK_SAFE_TOOLS,
        "includes": []
    },
    ...
    "lailaba-gateway": {
        ...
        "tools": [],
        "includes": ["lailaba-telegram", "lailaba-discord", "lailaba-whatsapp", "lailaba-slack", "lailaba-signal", "lailaba-bluebubbles", "lailaba-homeassistant", "lailaba-email", "lailaba-sms", "lailaba-mattermost", "lailaba-matrix", "lailaba-dingtalk", "lailaba-feishu", "lailaba-wecom", "lailaba-wecom-callback", "lailaba-weixin", "lailaba-qqbot", "lailaba-webhook", "lailaba-yuanbao"]
    }
}
...
def bundle_non_core_tools(toolset_name: str) -> Set[str]:
    """Return a ``lailaba-*`` bundle's platform-specific tools, excluding core.

    Platform bundles are defined as ``_LAILABA_CORE_TOOLS + [platform extras]``.
    ...
    Bundle nesting is one level deep in practice (only ``lailaba-gateway``
...
def resolve_toolset(name: str, visited: Set[str] = None) -> List[str]:
    ...
    if not toolset:
        # Auto-generate a toolset for plugin platforms (lailaba-<name>).
        # Gives them _LAILABA_CORE_TOOLS plus any tools the plugin registered
        # into a toolset matching the platform name.
        if name.startswith("lailaba-"):
            platform_name = name[len("lailaba-"):]
            try:
                from gateway.platform_registry import platform_registry
                if platform_registry.is_registered(platform_name):
                    plugin_tools = set(_LAILABA_CORE_TOOLS)
                    try:
                        from tools.registry import registry
                        plugin_tools.update(
