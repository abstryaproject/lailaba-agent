...
Uses a file-based lock (~/.lailaba/cron/.tick.lock) so only one tick
...
# Without this, standalone invocations (e.g. after `lailaba update` reloads
# the module) fail with ModuleNotFoundError for lailaba_time et al.
sys.path.insert(0, str(Path(__file__).parent.parent))

from lailaba_constants import get_lailaba_home
from lailaba_cli._subprocess_compat import windows_hide_flags
from lailaba_cli.config import load_config, _expand_env_vars
from lailaba_time import now as _lailaba_now
...
def _merge_mcp_into_per_job_toolsets(per_job: list[str], cfg: dict) -> list[str]:
    ...
    if "no_mcp" in per_job:
        return result
    # lazy import: avoid heavy lailaba_cli import at cron module load (matches
    # _resolve_cron_enabled_toolsets' fallback) and share one MCP-membership
    # computation with the gateway/CLI platform resolver.
    from lailaba_cli.tools_config import enabled_mcp_server_names
...
def _resolve_cron_enabled_toolsets(job: dict, cfg: dict) -> list[str] | None:
    ...
    2. Per-platform ``lailaba tools`` config for the ``cron`` platform.
    ...
    try:
        from lailaba_cli.tools_config import _get_platform_tools  # lazy: avoid heavy import at cron module load
...
_lailaba_home: Path | None = None


def _get_lailaba_home() -> Path:
    """Resolve Lailaba home dynamically while preserving test monkeypatch hooks.

    Cron is per-profile by design (#4707): the in-process ticker runs inside a
    profile-scoped gateway, so resolving the active LAILABA_HOME at call time
    ...
    """
    return _lailaba_home or get_lailaba_home()


def _get_lock_paths() -> tuple[Path, Path]:
    """Resolve cron lock paths at call time so profile/env changes are honored."""
    lailaba_home = _get_lailaba_home()
    lock_dir = lailaba_home / "cron"
...
def _open_continuable_cron_thread(
    ...
    thread_name = f"Lailaba — {task_name}"
    try:
...
def _get_script_timeout() -> int:
    ...
    env_value = os.getenv("LAILABA_CRON_SCRIPT_TIMEOUT", "").strip()
    if env_value:
        try:
            ...
            logger.warning("Invalid LAILABA_CRON_SCRIPT_TIMEOUT=%r; using config/default", env_value)
...
def _run_job_script(script_path: str) -> tuple[bool, str]:
    ...
    Scripts must reside within LAILABA_HOME/scripts/.  Both relative and
    ...
    * anything else — run with the current Python interpreter
    ...
    provider credentials and other Lailaba-managed secrets are not inherited
    (SECURITY.md §2.3), matching terminal and MCP child processes.

    Args:
        script_path: Path to the script.  Relative paths are resolved
            against LAILABA_HOME/scripts/.  Absolute and ~-prefixed paths
    ...
    """
    scripts_dir = _get_lailaba_home() / "scripts"
    ...
    # escape — scripts MUST reside within LAILABA_HOME/scripts/.
    try:
...
def run_job(job: dict) -> tuple[bool, str, str, Optional[str]]:
    ...
    if job.get("no_agent"):
        ...
        now_iso = _lailaba_now().strftime("%Y-%m-%d %H:%M:%S")
    ...
    if script_path:
        ...
        if _ran_ok and not _parse_wake_gate(_script_output):
            ...
            silent_doc = (
                f"# Cron Job: {job_name}\n\n"
                f"**Job ID:** {job_id}\n"
                f"**Run Time:** {_lailaba_now().strftime('%Y-%m-%d %H:%M:%S')}\n\n"
    ...
    origin = _resolve_origin(job)
    _cron_session_id = f"cron_{job_id}_{_lailaba_now().strftime('%Y%m%d_%H%M%S')}"
    ...
    os.environ["LAILABA_CRON_SESSION"] = "1"
    ...
    # gateway message. Do not seed LAILABA_SESSION_* contextvars from the
    ...
    #     (notify_on_complete / watch_patterns) reads LAILABA_SESSION_PLATFORM
    #     and LAILABA_SESSION_CHAT_ID to populate watcher_platform / chat_id,
    #     which would route completion notifications to the origin chat
    #     instead of via LAILABA_CRON_AUTO_DELIVER_* below.
    #   - tools/tts_tool.py: picks Opus vs MP3 based on
    #     LAILABA_SESSION_PLATFORM == "telegram".
    ...
    #     LAILABA_SESSION_PLATFORM.
    #   - tools/send_message_tool.py: mirror source labelling and the
    #     send_message gate read LAILABA_SESSION_PLATFORM.
    # Cron output delivery itself reads job["origin"] directly via
    # _resolve_origin(job) and the LAILABA_CRON_AUTO_DELIVER_* vars set
    # below, so clearing LAILABA_SESSION_* here does not affect delivery.
    ...
    _cron_delivery_vars = (
        "LAILABA_CRON_AUTO_DELIVER_PLATFORM",
        "LAILABA_CRON_AUTO_DELIVER_CHAT_ID",
        "LAILABA_CRON_AUTO_DELIVER_THREAD_ID",
    )
    ...
    try:
        ...
        try:
            load_dotenv(str(_get_lailaba_home() / ".env"), override=True, encoding="utf-8")
        except UnicodeDecodeError:
            load_dotenv(str(_get_lailaba_home() / ".env"), override=True, encoding="latin-1")

        delivery_target = _resolve_delivery_target(job)
        if delivery_target:
            _VAR_MAP["LAILABA_CRON_AUTO_DELIVER_PLATFORM"].set(delivery_target["platform"])
            _VAR_MAP["LAILABA_CRON_AUTO_DELIVER_CHAT_ID"].set(str(delivery_target["chat_id"]))
            _VAR_MAP["LAILABA_CRON_AUTO_DELIVER_THREAD_ID"].set(
                ""
                if delivery_target.get("thread_id") is None
                else str(delivery_target["thread_id"])
            )

        # Model resolution precedence: per-job override > LAILABA_MODEL env >
        ...
        model = job.get("model") or os.getenv("LAILABA_MODEL") or ""

        # Load config.yaml for model, reasoning, prefill, toolsets, provider routing
        _cfg = {}
        try:
            import yaml
            _cfg_path = str(_get_lailaba_home() / "config.yaml")
        ...
        if not (isinstance(model, str) and model.strip()):
            raise RuntimeError(
                ...
                f"LAILABA_MODEL={os.getenv('LAILABA_MODEL', '')!r}, "
                "config.yaml model.default missing or empty). "
                f"Set a per-job model via "
                f"`cronjob action=update job_id={job_id} model=<name>` or set a "
                "default with `lailaba model <name>`."
            )
        ...
        prefill_file = (
            os.getenv("LAILABA_PREFILL_MESSAGES_FILE", "")
        ...
        if prefill_file:
            ...
            if not pfpath.is_absolute():
                pfpath = _get_lailaba_home() / pfpath
            if pfpath.exists():
                try:
        ...
        try:
            # Do not inject LAILABA_INFERENCE_PROVIDER here. resolve_runtime_provider()
        ...
        agent = AIAgent(
            ...
            # LAILABA_HOME. When a workdir is configured, also inject project
        ...
        # override via LAILABA_CRON_TIMEOUT env var.  0 = unlimited.
        #
        # Uses the agent's built-in activity tracker (updated by
        # _touch_activity() on every tool call, API call, and stream delta).
        _raw_cron_timeout = os.getenv("LAILABA_CRON_TIMEOUT", "").strip()
        if _raw_cron_timeout:
            try:
                _cron_timeout = float(_raw_cron_timeout)
            except (ValueError, TypeError):
                logger.warning(
                    "Invalid LAILABA_CRON_TIMEOUT=%r; using default 600s",
                    _raw_cron_timeout,
                )
                _cron_timeout = 600.0
        else:
...
**Job ID:** {job_id}
**Run Time:** {_lailaba_now().strftime('%Y-%m-%d %H:%M:%S')}
...
"""
        ...
        if _session_db:
            ...
            try:
                ...
                _cron_title = f"{_title_base} · {_lailaba_now().strftime('%b %d %H:%M')}"
...
def tick(verbose: bool = True, adapters=None, loop=None, sync: bool = True) -> int:
    ...
    try:
        ...
        if verbose and not due_jobs:
            logger.info("%s - No jobs due", _lailaba_now().strftime('%H:%M:%S'))
            return 0

        if verbose:
            logger.info("%s - %s job(s) due", _lailaba_now().strftime('%H:%M:%S'), len(due_jobs))
        ...
        # Set LAILABA_CRON_MAX_PARALLEL=1 to restore old serial behaviour.
        _max_workers: Optional[int] = None
        try:
            _env_par = os.getenv("LAILABA_CRON_MAX_PARALLEL", "").strip()
            if _env_par:
                _max_workers = int(_env_par) or None
        except (ValueError, TypeError):
            logger.warning("Invalid LAILABA_CRON_MAX_PARALLEL value; defaulting to unbounded")
