...
renders natively:
  ...
  * Docs catalog          -> a copy-paste command + a ``lailaba://`` deep-link
...
def blueprint_deeplink(blueprint: AutomationBlueprint, values: Optional[Dict[str, Any]] = None) -> str:
    """Build the ``lailaba://blueprint/<key>?slot=val`` deep-link URL."""
    ...
    return f"lailaba://blueprint/{quote(blueprint.key)}{qs}"
