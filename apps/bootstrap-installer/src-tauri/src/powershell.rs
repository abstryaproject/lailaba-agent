...
///
/// `lailaba_home_override` propagates to the child as $LAILABA_HOME so the
...
pub async fn run_script(
    ...
    if let Some(cwd) = stable_script_cwd(script_path, lailaba_home_override) {
        cmd.current_dir(cwd);
    }

    if let Some(home) = lailaba_home_override {
        cmd.env("LAILABA_HOME", home);
    }
...
fn stable_script_cwd<'a>(script_path: &'a Path, lailaba_home_override: Option<&'a str>) -> Option<&'a Path> {
    if let Some(home) = lailaba_home_override {
...
"#;
    ...
    #[test]
    fn stable_script_cwd_prefers_existing_lailaba_home() {
