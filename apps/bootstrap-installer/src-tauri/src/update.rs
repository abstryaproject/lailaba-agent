...
//!
//! Driven when the installer is launched as `Lailaba-Setup.exe --update` (see
//! `AppMode` in lib.rs). The desktop app hands off to us — it exits, then we:
//!
//!   1. wait for the old Lailaba desktop process to fully exit (so both the
//!      venv shim and packaged app.asar are free; otherwise `lailaba update`
//!      or repair bootstrap can race locked files),
//!   2. run `lailaba update --yes --gateway` (Python/repo update; this does NOT
//!      rebuild apps/desktop by design — see cmd_update in lailaba_cli/main.py),
//!   3. run `lailaba desktop --build-only` (the rebuild step update skips),
...
//!
//! Cross-platform note: `lailaba update` already handles macOS/Linux (git/pip).
//! The only OS-specific bits here are the venv shim path (resolve_lailaba) and
...
/// `lailaba update` exit code meaning "another lailaba process is holding the
/// venv shim open / dirty precondition" — see _cmd_update_impl in
/// lailaba_cli/main.py (sys.exit(2)). We surface a targeted message for this.
const UPDATE_EXIT_CONCURRENT: i32 = 2;

/// How long to wait for the old desktop process to release files under the
/// install tree before giving up and letting `lailaba update`'s own guard decide.
...
pub async fn start_update(app: AppHandle) -> Result<(), String> {
        ...
        .is_err()
    {
        ...
        let mut stages = vec![
            stage_info("update", "Updating Lailaba"),
...
async fn run_update(app: AppHandle) -> Result<()> {
    let lailaba_home = crate::paths::lailaba_home();
    let install_root = lailaba_home.join("lailaba-agent");
    ...
    // it, that backend re-locks the venv shim, our `force_kill_other_lailaba`
    ...
    let lailaba = resolve_lailaba(&install_root).ok_or_else(|| {
        let msg = format!(
            "Could not find the lailaba CLI under {}. Is Lailaba installed? \
    ...
    // async on Windows. If it still holds the venv shim, `lailaba update`
    ...
    // ---- stage 1: lailaba update -----------------------------------------
    // Pass --branch so `lailaba update` targets the branch this installer was
    ...
    // without --branch, `lailaba update` switches the checkout to `main` (a
    ...
    // --force skips `lailaba update`'s Windows running-exe guard (which would
    ...
    // time `lailaba update` runs there is no legitimate lailaba.exe to protect,
    // and the guard would only produce a false "Lailaba is still running" stop.
    ...
    // Retry-once for the update-boundary crash. `lailaba update` lazily imports
    ...
    // second `lailaba update` runs clean because the now-current module is loaded
    ...
    // for the concurrent-instance guard (exit 2) — that's a "close Lailaba" state
    // a retry can't fix.
    ...
    match update.exit_code {
        ...
        Some(code) if code == UPDATE_EXIT_CONCURRENT => {
            let msg = "Lailaba is still running. Close all Lailaba windows and try \
                       the update again."
                .to_string();
            emit_stage(
                &app,
        ...
        other => {
            let msg = format!(
                "lailaba update failed (exit {:?}). See {} for details.",
                other,
                crate::paths::lailaba_home()
                    .join("logs")
                    .join("update.log")
                    .display()
            );
    ...
    // ---- stage 2: lailaba desktop --build-only ----------------------------
    // `lailaba update` deliberately does NOT build apps/desktop (it installs
    ...
    // retry-once `lailaba update` already does above, and `lailaba update`'s own
    ...
    if rebuild.exit_code != Some(0) {
        let msg = format!(
            "Rebuilding the desktop app failed (exit {:?}). The update was \
             applied but the app could not be rebuilt; run `lailaba desktop` \
    ...
    if let Some(target_app) = launch_target {
        if let Err(err) = launch_macos_app_and_exit(&app, &target_app).await {
            emit_log(
                ...
                None,
                LogStream::Stderr,
                &format!("[update] could not auto-launch desktop: {err}. Launch Lailaba manually."),
            );
        }
    } else if let Err(err) =
        crate::bootstrap::launch_lailaba_desktop(app.clone(), install_root.to_string_lossy().into_owned()).await
    {
...
pub(crate) async fn wait_for_install_locks_free(install_root: &Path, app: &AppHandle, stage: &str) {
    ...
    emit_log(app, Some(stage), LogStream::Stdout, "[handoff] waiting for Lailaba to exit…");
    ...
    loop {
        ...
        if Instant::now() >= deadline {
            // Last resort: a backend lailaba.exe (or the desktop Lailaba.exe
            ...
            // force-kill every Lailaba.exe except ourselves, then give the OS a
            ...
            emit_log(
                ...
                LogStream::Stdout,
                &format!(
                    "[handoff] Lailaba still holding install files ({}); force-killing stragglers…",
                    format_locked_paths(&locked)
                ),
            );
            force_kill_other_lailaba();
...
fn install_lock_probe_paths(install_root: &Path) -> Vec<PathBuf> {
    let mut paths = vec![venv_lailaba(install_root)];
...
fn desktop_app_payload_paths(install_root: &Path) -> Vec<PathBuf> {
    ...
    if cfg!(target_os = "windows") {
        ...
        vec![
            release.join("mac").join("Lailaba.app").join("Contents").join("Resources").join("app.asar"),
            release.join("mac-arm64").join("Lailaba.app").join("Contents").join("Resources").join("app.asar"),
        ]
    } else {
...
/// Force-kill any `lailaba.exe` other than this process. Windows-only; a no-op
...
/// knew its children — so we kill the whole `lailaba.exe` image tree via
/// taskkill, excluding our own PID.
///
/// Safe w.r.t. our own update child: this runs inside the install-lock wait,
/// which completes BEFORE we spawn `venv\Scripts\lailaba.exe update`. And a
...
/// lailaba.exe images here are stragglers from the old desktop — exactly what
/// we want gone. (`/FI PID ne <self>` also spares this Tauri process, though it
/// isn't named lailaba.exe.)
fn force_kill_other_lailaba() {
    if !cfg!(target_os = "windows") {
        return;
    }
...
/// Spawn `lailaba <args>` from `cwd`, stream stdout/stderr as Log events on the
...
/// Path to the venv lailaba shim under an install root, regardless of existence.
fn venv_lailaba(install_root: &Path) -> PathBuf {
    if cfg!(target_os = "windows") {
        install_root.join("venv").join("Scripts").join("lailaba.exe")
    } else {
        install_root.join("venv").join("bin").join("lailaba")
    }
}

/// Resolve the lailaba CLI to drive. Prefer the venv shim in the install we
/// just updated; fall back to `lailaba` on PATH.
fn resolve_lailaba(install_root: &Path) -> Option<PathBuf> {
    let shim = venv_lailaba(install_root);
    if shim.exists() {
        return Some(shim);
    }
    // PATH fallback. which-style probe via env, kept dependency-free.
    let exe = if cfg!(target_os = "windows") { "lailaba.exe" } else { "lailaba" };
...
fn update_child_env(install_root: &Path) -> Vec<(String, OsString)> {
    let lailaba_home = crate::paths::lailaba_home();
    let mut envs = vec![(
        "LAILABA_HOME".to_string(),
        lailaba_home.as_os_str().to_os_string(),
    )];
    if let Some(path) = path_with_prepended_entries(&[
        lailaba_home.join("node").join("bin"),
...
async fn install_macos_app_update(
    ...
    let rebuilt_app = crate::bootstrap::resolve_lailaba_desktop_app(install_root).ok_or_else(|| {
        anyhow!(
            "desktop rebuild succeeded but no Lailaba.app was found under {}",
    ...
    let tmp = PathBuf::from(format!("{}.lailaba-update-new", target_app.display()));
    let old = PathBuf::from(format!("{}.lailaba-update-old", target_app.display()));
    ...
    let ditto = Command::new("/usr/bin/ditto")
        .arg(&rebuilt_app)
        .arg(&tmp)
        .current_dir(crate::paths::lailaba_home())
        .status()
        .await
...
mod tests {
    ...
    #[test]
    fn venv_lailaba_is_under_install_root() {
        let root = Path::new("/x/lailaba-agent");
        let shim = venv_lailaba(root);
    ...
    fn lock_probe_paths_include_desktop_app_payload() {
        ...
        assert!(
            probes.iter().any(|p| p == &venv_lailaba(root)),
    ...
    fn locked_paths_ignores_missing_payloads() {
        let root = Path::new("/nonexistent/lailaba-agent");
        let probes = install_lock_probe_paths(root);
    ...
    fn update_marker_guard_writes_then_removes_on_drop() {
        ...
        let marker = dir.join(".lailaba-update-in-progress");
    ...
    fn parses_only_app_targets() {
        assert_eq!(
            target_app_from_args(["--update", "--target-app", "/Applications/Lailaba.app"]),
            Some(PathBuf::from("/Applications/Lailaba.app"))
        );
    ...
    async fn swap_installs_new_bundle_and_cleans_up() {
        ...
        let target = base.join("Lailaba.app");
        let tmp = base.join("Lailaba.app.lailaba-update-new");
        let old = base.join("Lailaba.app.lailaba-update-old");
    ...
    async fn swap_failure_never_leaves_target_missing() {
        ...
        let target = base.join("Lailaba.app");
        let tmp = base.join("Lailaba.app.lailaba-update-new"); // intentionally absent
        let old = base.join("Lailaba.app.lailaba-update-old");
        write_marker(&target, "OLD");
    ...
    async fn swap_rolls_back_when_install_step_fails() {
        ...
        let target = base.join("Lailaba.app");
        let tmp = base.join("Lailaba.app.lailaba-update-new"); // absent
        let old = base.join("Lailaba.app.lailaba-update-old");
        write_marker(&target, "OLD");
