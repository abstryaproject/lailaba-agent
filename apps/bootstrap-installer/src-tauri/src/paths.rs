...
//!
//! Mirrors `lailaba_constants.get_lailaba_home()` from the Python CLI:
//!   Windows: %LOCALAPPDATA%\lailaba
//!   macOS:   ~/.lailaba
//!   Linux:   ~/.lailaba  (override via $LAILABA_HOME)
//!
//! NOTE (macOS): Python's get_lailaba_home(), scripts/install.sh, and the
//! Electron desktop's resolveHermesHome() ALL use ~/.lailaba on macOS — there
...
/// Returns the canonical Lailaba home directory, respecting $LAILABA_HOME if set.
pub fn lailaba_home() -> PathBuf {
    if let Ok(override_path) = std::env::var("LAILABA_HOME") {
    ...
    {
        // %LOCALAPPDATA%\lailaba — matches scripts/install.ps1's $HermesHome.
        if let Some(local_app_data) = dirs::data_local_dir() {
            return local_app_data.join("lailaba");
        }
    }

    // macOS + Linux + fallback: ~/.lailaba (matches Python get_lailaba_home(),
    // install.sh, and the Electron desktop's resolveHermesHome()).
    if let Some(home) = dirs::home_dir() {
        return home.join(".lailaba");
    }

    // Last resort — current dir, almost certainly wrong but at least
    // doesn't panic.
    PathBuf::from(".lailaba")
}

pub fn log_dir() -> PathBuf {
    lailaba_home().join("logs")
}
...
pub fn bootstrap_cache_dir() -> PathBuf {
    lailaba_home().join("bootstrap-cache")
}
...
/// LAILABA_HOME so it survives repo checkout deletion (unlike anything under
/// lailaba-agent/).
///
/// On Windows this is `%LOCALAPPDATA%\lailaba\lailaba-setup.exe`; on other
...
/// mid-update re-locks the venv shim and triggers `force_kill_other_lailaba`,
/// which then kills that legitimate backend in a respawn loop (#50238).
///
/// Lives directly under LAILABA_HOME (same rationale as `installer_dest`) so the
/// Electron desktop — which resolves LAILABA_HOME identically and pins it into
/// the updater's env — agrees on the exact path.
pub fn update_in_progress_marker() -> PathBuf {
    lailaba_home().join(".lailaba-update-in-progress")
}
...
pub fn copy_self_to_lailaba_home() -> std::io::Result<()> {
    ...
    tracing::info!(?src, ?dest, "copied installer to LAILABA_HOME");
    Ok(())
}
...
/// We don't always know ACTIVE_LAILABA_ROOT until install.ps1 reports it, so
/// this is a probe helper, not a definitive path.
pub fn likely_bootstrap_marker(install_root: &Path) -> PathBuf {
    install_root.join(".lailaba-bootstrap-complete")
}

/// Initializes tracing to bootstrap-installer.log under LAILABA_HOME/logs/.
...
pub fn init_logging() -> Option<WorkerGuard> {
    ...
    if let Err(err) = std::fs::create_dir_all(&dir) {
        ...
        eprintln!("[lailaba-setup] could not create log dir {dir:?}: {err}");
        return None;
    }
    ...
    let env_filter = tracing_subscriber::EnvFilter::try_from_env("LAILABA_BOOTSTRAP_LOG")
...
#[tauri::command]
pub fn get_lailaba_home() -> String {
    lailaba_home().to_string_lossy().into_owned()
}
