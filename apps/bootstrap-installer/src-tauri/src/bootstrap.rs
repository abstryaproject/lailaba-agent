...
//! LAILABA_HOME/logs/bootstrap-<timestamp>.log.
//!
//! Lifecycle:
...
pub struct StartBootstrapArgs {
    ...
    /// Optional override for LAILABA_HOME. Tests use this; production
    /// almost always falls back to the OS default.
    pub lailaba_home: Option<String>,
}
...
/// Spawn the locally-built Lailaba desktop binary, then close the installer
...
#[tauri::command]
pub async fn launch_lailaba_desktop(
    app: AppHandle,
    install_root: String,
) -> Result<(), String> {
    let install_root = PathBuf::from(install_root);
    let exe_path = resolve_lailaba_desktop_exe(&install_root).ok_or_else(|| {
        format!(
            "Couldn't find a built Lailaba desktop at {}. The desktop build step \
             may have been skipped or failed. Run `lailaba desktop` from a \
             terminal to build and launch it.",
            install_root.join("apps").join("desktop").join("release").display()
        )
    })?;

    tracing::info!(?exe_path, "launching Lailaba desktop");

    // Detach from us — the installer is about to exit. On macOS launch the
    // bundle through LaunchServices instead of exec'ing Contents/MacOS/Lailaba
...
/// <os>-unpacked/<exe>).
pub(crate) fn resolve_lailaba_desktop_exe(install_root: &std::path::Path) -> Option<PathBuf> {
    let release_dir = install_root.join("apps").join("desktop").join("release");
    let candidates: &[(&str, &str)] = if cfg!(target_os = "windows") {
        &[
            ("win-unpacked", "Lailaba.exe"),
            ("win-arm64-unpacked", "Lailaba.exe"),
        ]
    } else if cfg!(target_os = "macos") {
        &[
            ("mac/Lailaba.app/Contents/MacOS", "Lailaba"),
            ("mac-arm64/Lailaba.app/Contents/MacOS", "Lailaba"),
        ]
    } else {
        &[("linux-unpacked", "lailaba")]
    };
...
pub(crate) fn resolve_lailaba_desktop_app(install_root: &std::path::Path) -> Option<PathBuf> {
    let exe = resolve_lailaba_desktop_exe(install_root)?;
    #[cfg(target_os = "macos")]
    {
        // .../Lailaba.app/Contents/MacOS/Lailaba -> .../Lailaba.app
...
/// path so a bare re-open just opens Lailaba instead of re-running setup.
pub(crate) fn lailaba_is_installed(install_root: &std::path::Path) -> bool {
    install_root.join(".lailaba-bootstrap-complete").exists()
        && resolve_lailaba_desktop_exe(install_root).is_some()
}
...
pub(crate) fn spawn_installed_desktop(install_root: &std::path::Path) -> std::io::Result<()> {
    let exe = resolve_lailaba_desktop_exe(install_root).ok_or_else(|| {
        std::io::Error::new(std::io::ErrorKind::NotFound, "no built Lailaba desktop app")
    })?;
    ...
    #[cfg(target_os = "windows")]
    {
        ...
        // installer exits, mirroring launch_lailaba_desktop. Kept correct here
...
pub(crate) fn open_macos_app_detached(app_bundle: &std::path::Path) -> std::io::Result<()> {
    ...
    cmd.arg(app_bundle);
    cmd.current_dir(crate::paths::lailaba_home());
    cmd.spawn().map(|_child| ())
}
...
async fn run_bootstrap(
    ...
    // the repo INTO at $HermesHome\lailaba-agent. Mirrors lailaba_constants.
    let lailaba_home = args
        .lailaba_home
        .clone()
        .unwrap_or_else(|| crate::paths::lailaba_home().to_string_lossy().into_owned());
    let install_root = PathBuf::from(&lailaba_home).join("lailaba-agent");

    // Copy ourselves to LAILABA_HOME/lailaba-setup.exe so the desktop app can
    ...
    if let Err(err) = crate::paths::copy_self_to_lailaba_home() {
        tracing::warn!(?err, "failed to copy installer into LAILABA_HOME (non-fatal)");
        emit_log(&format!(
...
async fn run_install_script(
    ...
    args: &[String],
    lailaba_home_override: Option<&str>,
    ...
    powershell::run_script(script_path, args, sink, lailaba_home_override, cancel_rx)
        .await
        .map_err(|e| {
...
mod tests {
    ...
    fn make_release_tree(install_root: &Path) -> PathBuf {
        ...
        if cfg!(target_os = "macos") {
            ...
            std::fs::write(macos_dir.join("Lailaba"), b"#!/bin/sh\n").unwrap();
            macos_dir.parent().unwrap().parent().unwrap().to_path_buf() // .../Lailaba.app
        } else if cfg!(target_os = "windows") {
            let dir = release.join("win-unpacked");
            std::fs::create_dir_all(&dir).unwrap();
            let exe = dir.join("Lailaba.exe");
            std::fs::write(&exe, b"stub").unwrap();
            exe
        } else {
            let dir = release.join("linux-unpacked");
            std::fs::create_dir_all(&dir).unwrap();
            let exe = dir.join("lailaba");
            std::fs::write(&exe, b"stub").unwrap();
            exe
        }
    }
    ...
    // what the updater ditto's over /Applications/Lailaba.app). A regression in
    // this derivation breaks the post-update auto-relaunch, so guard it.
    #[test]
    fn resolve_lailaba_desktop_app_finds_built_bundle() {
        let root = unique_tmp_dir("app-ok");
        let expected = make_release_tree(&root);

        let resolved = resolve_lailaba_desktop_app(&root)
    ...
    #[test]
    fn resolve_lailaba_desktop_app_is_none_without_a_build() {
        let root = unique_tmp_dir("app-none");
        // No release tree created.
        assert!(
            resolve_lailaba_desktop_app(&root).is_none(),
