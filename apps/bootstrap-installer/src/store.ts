...
export async function initialize(): Promise<void> {
  ...
  try {
    const [logPath, hermesHome, mode] = await Promise.all([
      ...
      invoke<string>('get_lailaba_home'),
  ...
  unlisten = await listen<BootstrapEvent>('bootstrap', (event) => {
    ...
    switch (payload.type) {
      ...
      case 'complete':
        ...
        // Install: show the "launch Lailaba" success screen. Update: this is a
...
export async function startUpdate(): Promise<void> {
  // Update is driven by the desktop handing off (Lailaba-Setup.exe --update);
...
export async function launchHermesDesktop(): Promise<void> {
  ...
  await invoke('launch_lailaba_desktop', { installRoot })
}
