 ...
 * product name (`Lailaba`). If a PREVIOUS `npm run pack` was interrupted
 ...
 *   '.../release/linux-unpacked/electron' -> '.../release/linux-unpacked/Lailaba'
 *
 * This is a hard failure with no obvious cause for the user — `lailaba desktop`
 ...
 * (the mac-unpacked Lailaba.app bundle) and Windows (win-unpacked), so we
