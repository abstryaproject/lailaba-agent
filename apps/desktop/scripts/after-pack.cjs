 ...
 *
 * Stamps the Lailaba icon + identity onto the packed Windows Lailaba.exe via
 * rcedit (delegated to set-exe-identity.cjs). This runs for EVERY packed build
 * — first install, `lailaba desktop`, the installer's --update rebuild, and a
 ...
 *   - packager.appInfo.productFilename: the exe basename (e.g. 'Lailaba')
 */
...
exports.default = async function afterPack(context) {
  ...
  const productName = context.packager?.appInfo?.productFilename || 'Lailaba'
  ...
  try {
    ...
    console.warn(`[after-pack] exe identity stamp failed (${err.message}); Lailaba.exe keeps the stock Electron icon`)
  }
}
