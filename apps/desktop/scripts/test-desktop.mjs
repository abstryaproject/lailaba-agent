...
// no longer bundles the Lailaba AI Python payload (that's fetched at first
// launch via install.ps1 / install.sh, per the Phase 1 thin-installer flow).
const APP = (() => {
  if (PLATFORM === 'darwin') {
    const appPath = path.join(RELEASE_ROOT, `mac-${ARCH}`, 'Lailaba.app')
    return {
      appPath,
      binary: path.join(appPath, 'Contents', 'MacOS', 'Lailaba'),
  ...
  if (PLATFORM === 'win32') {
    ...
    return {
      appPath: unpacked,
      binary: path.join(unpacked, 'Lailaba.exe'),
  ...
  return {
    appPath: unpacked,
    binary: path.join(unpacked, 'lailaba'),
    resourcesPath: path.join(unpacked, 'resources'),
    asarPath: path.join(unpacked, 'resources', 'app.asar'),
    unpackedDistIndex: path.join(unpacked, 'resources', 'app.asar.unpacked', 'dist', 'index.html')
  }
...
// Default LAILABA_HOME for non-sandboxed runs -- matches main.cjs's
// resolveHermesHome(). On Windows it's %LOCALAPPDATA%\lailaba; elsewhere
// it's ~/.lailaba. The fresh-install sandbox launchFresh() sets its own
// LAILABA_HOME and never touches this.
const DEFAULT_LAILABA_HOME = (() => {
  if (PLATFORM === 'win32' && process.env.LOCALAPPDATA) {
    return path.join(process.env.LOCALAPPDATA, 'lailaba')
  }
  return path.join(os.homedir(), '.lailaba')
})()
const VENV_ROOT = path.join(DEFAULT_LAILABA_HOME, 'lailaba-agent', 'venv')
const FRESH_SANDBOX_ROOT = path.join(os.tmpdir(), 'lailaba-desktop-fresh-install')
...
function ensurePackagedApp() {
  if (process.env.LAILABA_DESKTOP_SKIP_BUILD === '1' && exists(APP.binary)) {
    return
  }
...
function resolveDmgPath() {
  if (!exists(RELEASE_ROOT)) {
    return path.join(RELEASE_ROOT, `Lailaba-${PACKAGE_JSON.version}-${ARCH}.dmg`)
  }

  const prefix = `Lailaba-${PACKAGE_JSON.version}`
  const candidates = fs
  ...
  return candidates.length > 0
    ...
    : path.join(RELEASE_ROOT, `Lailaba-${PACKAGE_JSON.version}-${ARCH}.dmg`)
}

function resolveNsisPath() {
  // electron-builder NSIS artifactName template is 'Lailaba-${version}-${os}-${arch}.${ext}'
...
function ensureDmg() {
  ...
  if (process.env.LAILABA_DESKTOP_SKIP_BUILD === '1' && exists(resolveDmgPath())) {
    return
  }
...
function ensureNsis() {
  ...
  if (process.env.LAILABA_DESKTOP_SKIP_BUILD === '1' && resolveNsisPath()) {
    return
  }
...
function launchFresh() {
  ...
  const hermesHome = path.join(sandbox, 'lailaba-home')
  ...
  env.LAILABA_DESKTOP_CWD = cwd
  env.LAILABA_DESKTOP_IGNORE_EXISTING = '1'
  env.LAILABA_DESKTOP_TEST_MODE = 'fresh-install'
  env.LAILABA_DESKTOP_USER_DATA_DIR = userDataDir
  env.LAILABA_HOME = hermesHome
  delete env.LAILABA_DESKTOP_LAILABA
  delete env.LAILABA_DESKTOP_LAILABA_ROOT
  ...
  console.log(`  LAILABA_HOME: ${hermesHome}`)
  console.log(`  cwd: ${cwd}`)

  return { runtimeRoot: path.join(hermesHome, 'lailaba-agent', 'venv') }
}

// Validate the packaged bundle matches the thin-installer architecture:
//   - The Lailaba AI Python payload is NOT shipped (it's fetched at first
...
function validateBundle() {
  ...
  // present anymore. If a stray ship of lailaba_cli sneaks back in we want
  // to fail loudly rather than re-introduce the 400MB delta we just removed.
  const staleFactoryMarker = path.join(APP.resourcesPath, 'lailaba-agent', 'lailaba_cli', 'main.py')
...
function help() {
  console.log(`Usage:
  npm run test:desktop:existing  # build packaged app, launch with normal PATH/existing Lailaba
  npm run test:desktop:fresh     # build packaged app, launch with temp userData + LAILABA_HOME
...
Fast rerun (skip rebuild if the packaged app already exists):
  LAILABA_DESKTOP_SKIP_BUILD=1 npm run test:desktop:fresh
`)
}
