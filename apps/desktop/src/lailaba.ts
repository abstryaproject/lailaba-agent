import { JsonRpcGatewayClient } from '@lailaba/shared'
...
export class HermesGateway extends JsonRpcGatewayClient {
  constructor() {
    super({
      closedErrorMessage: 'Lailaba gateway connection closed',
      connectErrorMessage: 'Could not connect to Lailaba gateway',
      createRequestId: nextId => nextId,
      notConnectedErrorMessage: 'Lailaba gateway is not connected',
...
// the call; each pooled backend already has its own LAILABA_HOME, so no backend
...
// curation `lailaba model` does — for Nous it honors the free/paid tier so a
...
export function checkHermesUpdate(force = false): Promise<BackendUpdateCheckResponse> {
  return window.hermesDesktop.api<BackendUpdateCheckResponse>({
    ...profileScoped(),
    path: `/api/lailaba/update/check${force ? '?force=true' : ''}`
  })
}
