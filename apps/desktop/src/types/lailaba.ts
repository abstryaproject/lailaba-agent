...
export interface OAuthProvider {
   ...
   *  embedded terminal. Null when Lailaba doesn't know how to remove it. */
...
export interface EnvVarInfo {
  ...
  // in lailaba_cli/provider_catalog.py). When present, the Keys tab groups by
  // this provider identity — the SAME one `lailaba model` uses — instead of
...
export interface ModelOptionProvider {
   ...
   *  the `lailaba model` CLI / onboarding OAuth flow. */
  auth_type?: string
...
// folders. Mirrors lailaba_cli/projects_db.Project.to_dict().
...
export interface ToolProvider {
   ...
   *  CLI `lailaba tools` active-provider detection). */
  is_active: boolean
}
 ...
 *  not Lailaba); elsewhere, driver health from `cua-driver doctor`. `null`
...
/** Shape of `GET /api/lailaba/update/check` — the backend's own update state.
