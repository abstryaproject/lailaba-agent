...
import type { HermesConfigRecord } from '@/types/lailaba'

import { CONTROL_TEXT } from './constants'
import { getNested, setNested } from './helpers'
import { ListRow, LoadingState, Pill, SectionHeading } from './primitives'

// Lailaba' reasoning levels (VALID_REASONING_EFFORTS); `none` = thinking off.
// Empty config = Lailaba default (medium), shown as Medium.
...
// backend now surfaces the full `lailaba model` universe (every canonical
...
// Mirrors `_AUX_TASK_SLOTS` in lailaba_cli/web_server.py. Friendly labels and
...
export function ModelSettings({ onMainModelChanged }: ModelSettingsProps) {
  ...
  const activateApiKeyProvider = useCallback(async () => {
    ...
    try {
      ...
      // `lailaba model` curation). Best-effort — fall through to the refreshed
        ...
        {needsSetup && !setupIsApiKey && (
          <p className="mt-2 text-xs text-muted-foreground">
            {selectedProviderRow?.auth_type === 'api_key'
              ...
              : `${selectedProviderRow?.name} signs in through your browser — Lailaba runs the flow for you.`}
          </p>
        )}
