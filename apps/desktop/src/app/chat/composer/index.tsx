...
import { extractDroppedFiles, LAILABA_PATHS_MIME, partitionDroppedFiles } from '../hooks/use-composer-actions'
...
export function ChatBar({
  ...
  // Distinguish a cold start ("Starting Lailaba...") from a dropped connection
  ...
  const handleDragEnter = (event: ReactDragEvent<HTMLFormElement>) => {
    if (!onAttachDroppedItems || !dragHasAttachments(event.dataTransfer, LAILABA_PATHS_MIME)) {
      return
    }
  ...
  const handleInputDragOver = (event: ReactDragEvent<HTMLDivElement>) => {
    if (!dragHasAttachments(event.dataTransfer, LAILABA_PATHS_MIME)) {
      return
    }
