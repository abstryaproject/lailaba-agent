...
import type { HermesGateway } from '@/lailaba'
import { getGlobalModelOptions, getMoaModels } from '@/lailaba'
...
import type { MoaConfigResponse, ModelOptionProvider, ModelOptionsResponse } from '@/types/lailaba'
...
export function ModelMenuPanel({ gateway, onSelectModel, requestGateway }: ModelMenuPanelProps) {
  ...
  // session (effort/fast), gated by capability. Unset → Lailaba defaults.
          ...
          {groups.map(group => (
            <DropdownMenuGroup className="py-0.5" key={group.provider.slug}>
              ...
              {group.families.map(family => {
                ...
                // the active model, otherwise its remembered preset (Lailaba
