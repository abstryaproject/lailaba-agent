...
const LAILABA_PATHS_MIME = 'application/x-lailaba-paths'
...
function transferHasDropCandidates(t: DataTransfer): boolean {
  if (t.types?.includes(LAILABA_PATHS_MIME)) {
    return true
  }
...
function collectDroppedPaths(t: DataTransfer): string[] {
  ...
  try {
    const raw = t.getData(LAILABA_PATHS_MIME)
...
export function useTerminalSession({ cwd, onAddSelectionToChat }: UseTerminalSessionOptions) {
  ...
  useEffect(() => {
    ...
    const term = new Terminal({
      ...
      letterSpacing: 0,
      lineHeight: 1.12,
      // Full-screen TUIs (lailaba --tui, vim) grab the mouse, so a plain drag
    ...
    const mount = () => {
      ...
      try {
        ...
        console.warn('[lailaba-terminal] WebGL unavailable; falling back to DOM', err)
      }
