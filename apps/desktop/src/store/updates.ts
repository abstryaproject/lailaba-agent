...
import { checkHermesUpdate, getActionStatus, updateHermes } from '@/lailaba'
import { translateNow } from '@/i18n'
import { persistString, storedString } from '@/lib/storage'
import { dismissNotification, notify } from '@/store/notifications'
import { $connection } from '@/store/session'
import type { BackendUpdateCheckResponse } from '@/types/lailaba'
...
const UPDATE_TOAST_SNOOZE_KEY = 'lailaba:update-toast-snooze-until'
...
const SKEW_TOAST_SNOOZE_KEY = 'lailaba:backend-skew-toast-snooze-until'
...
export async function applyUpdates(opts: DesktopUpdateApplyOptions = {}): Promise<DesktopUpdateApplyResult> {
  ...
  try {
    ...
    // `lailaba update` themselves. Land on a dedicated manual state so the
    ...
    if (result?.manual) {
      $updateApply.set({
        ...
        applying: false,
        stage: 'manual',
        message: result.command ?? 'lailaba update',
        command: result.command ?? 'lailaba update'
      })
...
export async function applyBackendUpdate(): Promise<DesktopUpdateApplyResult> {
  ...
  try {
    ...
    if (!started.ok) {
      ...
      const command = (started as { update_command?: string }).update_command || 'lailaba update'
