...
import type { OAuthProvider } from '@/types/lailaba'
...
describe('refreshOnboarding', () => {
  ...
  it('does not downgrade configured=true on fallback-only readiness failures', async () => {
    ...
    window.localStorage.setItem('lailaba-desktop-onboarded-v1', '1')
    ...
    expect(window.localStorage.getItem('lailaba-desktop-onboarded-v1')).toBe('1')
  })
