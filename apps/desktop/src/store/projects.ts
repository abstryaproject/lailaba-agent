...
import type { ProjectInfo, ProjectsPayload } from '@/types/lailaba'
...
const PROJECT_SCOPE_KEY = 'lailaba.desktop.projectScope'
...
async function gatewayRequest<T>(method: string, params: Record<string, unknown> = {}): Promise<T> {
  ...
  if (!gateway) {
    throw new Error('Lailaba gateway is not connected')
  }
...
// repo, returning where Lailaba should start working. Git is the source of
