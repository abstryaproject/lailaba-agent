...
const LAILABA_REF_TYPES = ['file', 'folder', 'url', 'image', 'tool', 'line', 'terminal', 'session'] as const
type HermesRefType = (typeof LAILABA_REF_TYPES)[number]
...
const LAILABA_DIRECTIVE_RE = new RegExp(
...
function parseDirectiveText(text: string): Unstable_DirectiveSegment[] {
  const matches = [
    ...
    ...Array.from(text.matchAll(LAILABA_DIRECTIVE_RE)).map(match => {
...
/**
 * Renders text containing Lailaba directives (`@file:...`, `@image:...`) as
