...
function mkTmpHome() {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'lailaba-bootstrap-test-'))
}
...
test('runBootstrap bails immediately when the signal is already aborted', async () => {
  ...
  const result = await runBootstrap({
    installStamp: null,
    activeRoot: '/tmp/lailaba-runner-test',
    sourceRepoRoot: null,
    hermesHome: '/tmp/lailaba-runner-test',
    logRoot: '/tmp/lailaba-runner-test',
...
test('installedAgentInstallScript resolves the installer in the agent checkout', () => {
  ...
  try {
    ...
    const scriptsDir = path.join(home, 'lailaba-agent', 'scripts')
