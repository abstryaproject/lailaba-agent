...
const USER_DATA_OVERRIDE = process.env.LAILABA_DESKTOP_USER_DATA_DIR
...
const DEV_SERVER = process.env.LAILABA_DESKTOP_DEV_SERVER
...
// switches only apply pre-launch. Override with LAILABA_DESKTOP_DISABLE_GPU
...
if (REMOTE_DISPLAY_REASON) {
  ...
  console.log(
    `[lailaba] remote display detected (${REMOTE_DISPLAY_REASON}); disabling GPU hardware acceleration to prevent flicker`
  )
}
...
if (IS_WSL && !REMOTE_DISPLAY_REASON && fs.existsSync('/dev/dxg')) {
  ...
  console.log('[lailaba] WSL GPU passthrough (/dev/dxg) detected; enabling GPU acceleration')
}

ipcMain.handle('lailaba:get-remote-display-reason', () => REMOTE_DISPLAY_REASON)
    ...
    try {
      ...
      if (parsed && typeof parsed === 'object' && typeof parsed.commit === 'string' && parsed.commit.length >= 7) {
        if (parsed.schemaVersion !== INSTALL_STAMP_SCHEMA_VERSION) {
          console.warn(
            `[lailaba] install-stamp.json schemaVersion ${parsed.schemaVersion} != expected ${INSTALL_STAMP_SCHEMA_VERSION}; ignoring`
          )
          continue
        }
...
if (INSTALL_STAMP) {
  console.log(
    `[lailaba] install stamp: ${INSTALL_STAMP.commit.slice(0, 12)}${INSTALL_STAMP.branch ? ` (${INSTALL_STAMP.branch})` : ''}${INSTALL_STAMP.dirty ? ' [DIRTY]' : ''} from ${INSTALL_STAMP.source || 'unknown'}`
  )
} else if (IS_PACKAGED) {
  ...
  console.error(
    '[lailaba] WARNING: no install-stamp.json found in packaged build. First-launch bootstrap will not have a pinned ref to install.'
  )
}

// LAILABA_HOME — the user-facing root for everything Lailaba-related. Mirrors
// scripts/install.ps1's $HermesHome and scripts/install.sh's $LAILABA_HOME.
//
// Defaults:
//   Windows: %LOCALAPPDATA%\lailaba (matches install.ps1)
//   macOS / Linux: ~/.lailaba (matches install.sh)
//
// Special case for Windows: if the user has a legacy ~/.lailaba directory
// (e.g., from a prior pip install or a manual setup) AND no
// %LOCALAPPDATA%\lailaba yet, prefer the legacy path so we don't orphan their
// existing config / sessions / .env. New installs go to %LOCALAPPDATA%.
//
// LAILABA_DESKTOP_USER_DATA_DIR (used by test:desktop:fresh) puts the sandbox
// LAILABA_HOME beneath the throwaway userData dir so a fresh-install run never
// touches the user's real ~/.lailaba / %LOCALAPPDATA%\lailaba.
function resolveHermesHome() {
  if (process.env.LAILABA_HOME) return normalizeHermesHomeRoot(process.env.LAILABA_HOME)
  if (USER_DATA_OVERRIDE) return path.join(path.resolve(USER_DATA_OVERRIDE), 'lailaba-home')
  if (IS_WINDOWS) {
    // A GUI app launched from Explorer inherits the environment block captured
    // at login, so a LAILABA_HOME set via `setx` AFTER login is invisible in
    // process.env even though the CLI (a fresh shell) sees it. Without this the
    // backend silently falls back to %LOCALAPPDATA%\lailaba and reports "No
    // inference provider configured" despite a valid configured home (#45471).
    // Consult the live User-scoped registry value before the default below.
    const fromRegistry = readWindowsUserEnvVar('LAILABA_HOME')
    if (fromRegistry) return normalizeHermesHomeRoot(fromRegistry)
  }
  if (IS_WINDOWS && process.env.LOCALAPPDATA) {
    const localappdata = path.join(process.env.LOCALAPPDATA, 'lailaba')
    const legacy = path.join(app.getPath('home'), '.lailaba')
    // Migrate transparently to LOCALAPPDATA, but honour an existing legacy
    // ~/.lailaba setup (no LOCALAPPDATA install yet) so users don't lose state.
    if (!directoryExists(localappdata) && directoryExists(legacy)) return legacy
    return localappdata
  }
  return path.join(app.getPath('home'), '.lailaba')
}

const LAILABA_HOME = resolveHermesHome()

function hermesManagedNodePathEntries() {
  // NOTE: keep this ordering in sync with iter_lailaba_node_dirs() in
  // lailaba_constants.py — this Node main process cannot import the Python
  // module, so the platform-ordering rule is mirrored here.
  const root = path.join(LAILABA_HOME, 'node')
...
// ACTIVE_LAILABA_ROOT — the canonical mutable Lailaba install. Same path
// install.ps1 / install.sh use, so a desktop-only user and a CLI-only user end
// up with identical layouts and can share one install.
const ACTIVE_LAILABA_ROOT = path.join(LAILABA_HOME, 'lailaba-agent')
// VENV_ROOT — venv lives inside the repo, exactly like install.ps1 does it.
const VENV_ROOT = path.join(ACTIVE_LAILABA_ROOT, 'venv')
...
//
// We deliberately put the marker INSIDE ACTIVE_LAILABA_ROOT (not alongside)
...
const BOOTSTRAP_COMPLETE_MARKER = path.join(ACTIVE_LAILABA_ROOT, '.lailaba-bootstrap-complete')
const BOOTSTRAP_MARKER_SCHEMA_VERSION = 1

const DESKTOP_CONNECTION_CONFIG_PATH = path.join(app.getPath('userData'), 'connection.json')
const DESKTOP_UPDATE_CONFIG_PATH = path.join(app.getPath('userData'), 'updates.json')
const DESKTOP_WINDOW_STATE_PATH = path.join(app.getPath('userData'), 'window-state.json')
// active-profile.json records which Lailaba profile the desktop launches its
// local backend as. When set, startHermes() passes `lailaba --profile <name>
// dashboard …`, which deterministically pins LAILABA_HOME (see
// _apply_profile_override in lailaba_cli/main.py) and bypasses the sticky
// ~/.lailaba/active_profile file. Unset (null) preserves the legacy behavior:
// no --profile flag, so the backend honors active_profile / default.
const DESKTOP_PROFILE_CONFIG_PATH = path.join(app.getPath('userData'), 'active-profile.json')
// Mirrors lailaba_cli.profiles._PROFILE_ID_RE so we never hand the backend a
...
// desktop.log lives under LAILABA_HOME/logs/ so it sits next to agent.log,
// errors.log, gateway.log produced by lailaba_logging.setup_logging — one log
// directory per user, regardless of which UI surface produced the line.
const DESKTOP_LOG_PATH = path.join(LAILABA_HOME, 'logs', 'desktop.log')
...
//
// Mirror the Python logs (lailaba_logging.py RotatingFileHandler, maxBytes x
...
const BOOT_FAKE_MODE = process.env.LAILABA_DESKTOP_BOOT_FAKE === '1'
const BOOT_FAKE_STEP_MS = (() => {
  const raw = Number.parseInt(String(process.env.LAILABA_DESKTOP_BOOT_FAKE_STEP_MS || ''), 10)
  if (!Number.isFinite(raw) || raw <= 0) return 650
  return Math.max(120, raw)
})()
const APP_NAME = 'Lailaba'
...
// 'lailaba:native-theme' ('dark' | 'light' | 'system'); we pin
...
// electron-builder derives from the build `appId` (com.nousresearch.lailaba) —
// keep this string in sync with package.json `build.appId`. macOS/Linux don't
// need this, so gate it on Windows. (Fixes: desktop approval/turn notifications
// never firing on Windows.)
if (IS_WINDOWS) {
  app.setAppUserModelId('com.nousresearch.lailaba')
}
// Seed the native About panel with the live Lailaba version. This is refreshed
// on every open via the explicit "About" menu handler (refreshAboutPanel), so
// an in-place `lailaba update` mid-session is reflected without an app restart;
...
const MEDIA_PROTOCOL = 'lailaba-media'
// Only audio/video may be streamed. Without this the handler would read any
// non-blocklisted local file (no size cap) for any `fetch(lailaba-media://…)`.
...
const POOL_MAX_BACKENDS = Math.max(1, Number(process.env.LAILABA_DESKTOP_POOL_MAX) || 3)
const POOL_IDLE_MS = Math.max(60_000, Number(process.env.LAILABA_DESKTOP_POOL_IDLE_MS) || 10 * 60_000)
...
// respawning lailaba dashboard children in a tight loop while boot is broken.
...
let bootProgressState = {
  ...
  message: 'Waiting to start Lailaba backend',
  phase: 'idle',
  progress: 0,
  running: false,
  timestamp: Date.now()
...
function ensureWslWindowsFonts() {
  ...
  try {
    ...
    const confPath = path.join(confDir, '99-lailaba-wsl-windows-fonts.conf')
    let existing = ''
    try {
...
function broadcastBootProgress() {
  ...
  webContents.send('lailaba:boot-progress', bootProgressState)
}
...
//
// The snapshot is queryable via the lailaba:bootstrap:get IPC handler so a
...
function broadcastBootstrapEvent(ev) {
  ...
  const { webContents } = mainWindow
  if (!webContents || webContents.isDestroyed()) return
  webContents.send('lailaba:bootstrap:event', ev)
}
...
// The Tauri updater writes LAILABA_HOME/.lailaba-update-in-progress for the whole
...
// (`force_kill_other_lailaba`, taskkill /IM lailaba.exe) kills it, the launch
...
async function waitForUpdateToFinish() {
  let marker = readLiveUpdateMarker(LAILABA_HOME)
  ...
  while (marker && Date.now() < deadline) {
    await advanceBootProgress(
      'backend.update-wait',
      'An update is finishing — Lailaba will start automatically when it completes…',
      12
    )
    await new Promise(r => setTimeout(r, UPDATE_WAIT_POLL_MS))
    marker = readLiveUpdateMarker(LAILABA_HOME)
  }
  if (marker) {
...
function findOnPath(command) {
  ...
  // shell-script shim named `lailaba` — must not shadow `lailaba.cmd`/`lailaba.exe`.
...
function unwrapWindowsVenvHermesCommand(command, dashboardArgs) {
  ...
  if (!/^lailaba(?:\.exe)?$/i.test(path.basename(resolved))) return null
  ...
  return {
    label: `existing Lailaba no-console Python at ${python}`,
    command: python,
    args: ['-m', 'lailaba_cli.main', ...dashboardArgs],
    bootstrap: false,
    env: buildDesktopBackendEnv({
      hermesHome: LAILABA_HOME,
...
function isHermesSourceRoot(root) {
  return directoryExists(root) && fileExists(path.join(root, 'lailaba_cli', 'main.py'))
}

function findPythonForRoot(root) {
  const override = process.env.LAILABA_DESKTOP_PYTHON
...
function findSystemPython() {
  ...
  // CPython but several Lailaba deps (notably pywinpty's Rust-built
...
// findGitBash — locate bash.exe on Windows. Lailaba' terminal tool requires
...
function findGitBash() {
  ...
  // install.ps1 drops PortableGit at %LOCALAPPDATA%\lailaba\git\... — checked
  // first so users who installed via install.ps1 are detected before we
  // start probing system-wide locations.
  const localAppData = process.env.LOCALAPPDATA || ''
  const candidates = []
  if (localAppData) {
    candidates.push(path.join(localAppData, 'lailaba', 'git', 'bin', 'bash.exe'))
    candidates.push(path.join(localAppData, 'lailaba', 'git', 'usr', 'bin', 'bash.exe'))
  }
...
function getNoConsoleVenvPython(venvRoot) {
  ...
  // and breaks imports (yaml, lailaba_cli, …) even when PYTHONPATH is patched.
...
function applyWindowsNoConsoleSpawnHints(backend) {
  ...
  const usesHermesModule =
    backend.kind === 'python' ||
    (Array.isArray(backend.args) && backend.args[0] === '-m' && backend.args[1] === 'lailaba_cli.main')
...
// install only has PortableGit under %LOCALAPPDATA%\lailaba\git (never on
...
function resolveGitBinary() {
  ...
  const candidates = []
  if (localAppData) {
    candidates.push(path.join(localAppData, 'lailaba', 'git', 'cmd', 'git.exe'))
    candidates.push(path.join(localAppData, 'lailaba', 'git', 'bin', 'git.exe'))
  }
...
// ─── Self-update (git-pull against the running backend's lailaba root) ──────
...
// Dev → SOURCE_REPO_ROOT. Packaged/CLI install → ACTIVE_LAILABA_ROOT.
// LAILABA_DESKTOP_LAILABA_ROOT always wins so devs can pin a worktree.
function resolveUpdateRoot() {
  const candidates = [
    process.env.LAILABA_DESKTOP_LAILABA_ROOT && path.resolve(process.env.LAILABA_DESKTOP_LAILABA_ROOT),
    !IS_PACKAGED && isHermesSourceRoot(SOURCE_REPO_ROOT) ? SOURCE_REPO_ROOT : null,
    isHermesSourceRoot(ACTIVE_LAILABA_ROOT) ? ACTIVE_LAILABA_ROOT : null
  ].filter(Boolean)

  return candidates.find(c => directoryExists(path.join(c, '.git'))) || candidates[0] || ACTIVE_LAILABA_ROOT
}
...
function emitUpdateProgress(payload) {
  ...
  for (const window of BrowserWindow.getAllWindows()) {
    window.webContents.send('lailaba:updates:progress', merged)
  }
}
...
// LAILABA_HOME/lailaba-setup.exe on a successful install (see
// apps/bootstrap-installer paths::copy_self_to_lailaba_home). That binary owns
// ALL repo mutation — running `lailaba update` + rebuilding the desktop — so
...
function resolveUpdaterBinary() {
  const name = IS_WINDOWS ? 'lailaba-setup.exe' : 'lailaba-setup'
  const candidate = path.join(LAILABA_HOME, name)
...
// Path to the venv shim whose lock decides whether `lailaba update` can write
// fresh entry points. On Windows this is the file the running backend
// `lailaba.exe` holds open; on POSIX it's never mandatory-locked.
function venvHermesShimPath(updateRoot) {
  return IS_WINDOWS
    ? path.join(updateRoot, 'venv', 'Scripts', 'lailaba.exe')
    : path.join(updateRoot, 'venv', 'bin', 'lailaba')
}
...
// only signals the direct child, so on Windows a backend `lailaba.exe` that
// spawned its own grandchildren (a `lailaba` REPL, a pty terminal session, the
...
// teardown, so the updater raced a still-locked `lailaba.exe`, the quarantine
...
// problem: the desktop's backend (and the grandchildren IT spawned — a lailaba
// REPL, a pty terminal, the gateway) keep `lailaba.exe` and other files in the
...
// `lailaba update`). Instead we spawn the staged Lailaba-Setup binary with
// --update and quit, so it can run `lailaba update` (which refuses while we
...
async function applyUpdates(opts = {}) {
  ...
  try {
    ...
    if (!updater && !IS_WINDOWS) {
      // macOS/Linux drag-install: no staged Tauri lailaba-setup. Unlike Windows
      ...
      // whole update itself: `lailaba update` (backend) + `lailaba desktop
    ...
    if (!updater) {
      ...
      // `lailaba desktop`, never the Tauri installer that self-copies
      // lailaba-setup.exe into LAILABA_HOME). They DO have a working `lailaba`
      ...
      // checkout they're on — bare `lailaba update` defaults to main and would
      ...
      const updateRoot = resolveUpdateRoot()
      let command = 'lailaba update'
      try {
        const head = await runGit(['rev-parse', '--abbrev-ref', 'HEAD'], { cwd: updateRoot })
        const current = (head.stdout || '').trim()
        if (head.code === 0 && current && current !== 'HEAD') {
          const branch = await resolveHealedBranch(updateRoot, current)
          if (branch !== 'main') command = `lailaba update --branch ${branch}`
        }
      } catch {
        // Best-effort: fall back to bare `lailaba update` if branch detection fails.
      }
    ...
    emitUpdateProgress({
      stage: 'restart',
      message:
        'Updating Lailaba — this window will close and the updater will open. Don’t reopen Lailaba yourself; it restarts automatically when the update finishes.',
      percent: 100
    })
    ...
    // lailaba.exe (held by the backend child / its grandchildren) and the update
    // bricks. See releaseBackendLockForUpdate for the full failure analysis.
    await releaseBackendLockForUpdate(updateRoot)

    // Detached so the updater outlives this process — it needs us GONE before
    // `lailaba update` will run (the venv shim is locked while we live).
...
async function handOffWindowsBootstrapRecovery(reason) {
  ...
  const venvBin = path.join(updateRoot, 'venv', IS_WINDOWS ? 'Scripts' : 'bin')
  const venvHermes = path.join(venvBin, IS_WINDOWS ? 'lailaba.exe' : 'lailaba')
  ...
  // not just the `lailaba.exe` console-script shim. That shim is generated at the
  ...
  const haveRealInstall =
    ...
    fileExists(path.join(updateRoot, '.lailaba-bootstrap-complete'))
...
// Resolve the lailaba CLI to drive an in-app update: prefer the venv shim in
// the install we're updating, fall back to `lailaba` on PATH.
function resolveHermesCliBinary(updateRoot) {
  const venvHermes = path.join(updateRoot, 'venv', 'bin', 'lailaba')
  if (fileExists(venvHermes)) return venvHermes
  return findOnPath('lailaba') || null
}
...
// macOS/Linux in-app update: backend (`lailaba update`) + OS-aware GUI rebuild
// (`lailaba desktop --build-only`), then atomically swap the running .app bundle
// with the freshly built one and relaunch. Degrades to "backend updated,
// restart to load the new GUI" if the swap can't be performed.
async function applyUpdatesPosixInApp() {
  const updateRoot = resolveUpdateRoot()
  const lailaba = resolveHermesCliBinary(updateRoot)
  if (!lailaba) {
    emitUpdateProgress({ stage: 'manual', message: 'lailaba update', percent: null })
    return { ok: true, manual: true, command: 'lailaba update', hermesRoot: updateRoot }
  }

  // Put the Lailaba-managed Node and the venv on PATH so `lailaba desktop`'s
  // npm build can find them on a machine with no system Node. Windows portable
  // Node lives directly under %LOCALAPPDATA%\lailaba\node, not node\bin.
  const env = {
    LAILABA_HOME,
    PATH: pathWithHermesManagedNode(path.join(updateRoot, 'venv', 'bin'))
  }

  // `lailaba update` reaps stale `lailaba dashboard` backends (a code update
  ...
  // _kill_stale_dashboard_processes reads LAILABA_DESKTOP_CHILD_PID and excludes
  ...
  if (desktopChildPids.length) {
    env.LAILABA_DESKTOP_CHILD_PID = desktopChildPids.join(',')
  }
  ...
  emitUpdateProgress({ stage: 'update', message: 'Updating Lailaba (git + dependencies)…', percent: 10 })
  const updated = await runStreamedUpdate(lailaba, ['update', '--yes', ...branchArgs], {
    cwd: updateRoot,
    env,
    stage: 'update'
  })
  if (updated.code !== 0) {
    emitUpdateProgress({ stage: 'error', message: 'lailaba update failed.', error: updated.error || 'update-failed' })
    return { ok: false, error: 'lailaba update failed' }
  }
  ...
  const rebuilt = await runRebuildWithRetry(attempt => {
    ...
    return runStreamedUpdate(lailaba, ['desktop', '--build-only'], { cwd: updateRoot, env, stage: 'rebuild' })
  })
  if (rebuilt.code !== 0) {
    emitUpdateProgress({
      stage: 'error',
      message: 'Backend updated, but the desktop rebuild failed. Restart Lailaba to retry.',
      error: rebuilt.error || 'rebuild-failed'
    })
    return { ok: false, backendUpdated: true, error: 'desktop rebuild failed' }
  }

  // Linux in-app update terminal state (#45205). `lailaba desktop --build-only`
  ...
  if (!IS_MAC) {
    ...
    if (outcome === 'relaunch') {
      emitUpdateProgress({ stage: 'restart', message: 'Restarting Lailaba…', percent: 100 })
      ...
      const scriptPath = path.join(app.getPath('temp'), `lailaba-desktop-update-${Date.now()}.sh`)
      try {
        ...
        return {
          ...
          guiUpdated: false,
          manualRestart: true,
          message: 'Backend updated. Quit and reopen Lailaba to load the new version.'
        }
      }
    }
    ...
    if (outcome === 'guiSkew') {
      emitUpdateProgress({
        ...
        message:
          ...
          'Update or reinstall the Lailaba desktop app to match.',
        percent: 100
      })
      rememberLog(
    ...
    return {
      ...
      message:
        ...
        '(sandbox helper needs root). Quit and reopen Lailaba to finish.'
    }
  }

  const rebuiltApp = [
    path.join(updateRoot, 'apps', 'desktop', 'release', 'mac-arm64', 'Lailaba.app'),
    path.join(updateRoot, 'apps', 'desktop', 'release', 'mac', 'Lailaba.app')
  ...
  if (!rebuiltApp || !targetApp) {
    emitUpdateProgress({
      stage: 'done',
      message: 'Backend updated. Restart Lailaba to load the new version.',
      percent: 100
    })
...
if [ "$SRC" != "$DST" ]; then
  if /usr/bin/ditto "$SRC" "$DST.lailaba-update-new"; then
    rm -rf "$DST.lailaba-update-old" 2>/dev/null || true
    mv "$DST" "$DST.lailaba-update-old" 2>/dev/null || rm -rf "$DST"
    mv "$DST.lailaba-update-new" "$DST"
    rm -rf "$DST.lailaba-update-old" 2>/dev/null || true
  fi
fi
...
`
  ...
  try {
    ...
    emitUpdateProgress({
      stage: 'done',
      message: 'Backend + app updated. Restart Lailaba to load the new version.',
      percent: 100
    })
...
// Marker-independent: is the canonical install at ACTIVE_LAILABA_ROOT actually
...
function isActiveRuntimeUsable() {
  ...
  return (
    isHermesSourceRoot(ACTIVE_LAILABA_ROOT) &&
    fileExists(venvPython) &&
    canImportHermesCli(venvPython, {
      env: {
        PYTHONPATH: [ACTIVE_LAILABA_ROOT, process.env.PYTHONPATH].filter(Boolean).join(path.delimiter)
      }
    })
  )
}
...
function isBootstrapComplete() {
  ...
  // pinned commit -- users update via the in-app update path or `lailaba
...
function resolveWebDist() {
  const override = process.env.LAILABA_DESKTOP_WEB_DIST
  ...
  if (IS_PACKAGED && /app\.asar(?=$|[\\/])/.test(fallback) && !directoryExists(fallback)) {
    rememberLog(
      `[web-dist] dashboard frontend dir resolved to an asar-internal path that ` +
        ...
        `Ensure dist/** is unpacked (asarUnpack) or set LAILABA_DESKTOP_WEB_DIST.`
    )
  }
  return fallback
}
...
function resolveRendererIndex() {
  ...
  rememberLog(
    `[renderer] index.html not found — the desktop app was packaged without a ` +
      ...
      `Rebuild with: lailaba desktop --force-build`
  )
  return candidates[0]
}
...
function resolveHermesCwd() {
  ...
  // `…/win-unpacked` on Windows or `/Applications/Lailaba.app/Contents/...`
  ...
  const candidates = [
    ...
    process.env.LAILABA_DESKTOP_CWD,
...
// createActiveBackend — build a backend pointing at ACTIVE_LAILABA_ROOT, the
...
function createActiveBackend(dashboardArgs) {
  ...
  return applyWindowsNoConsoleSpawnHints({
    kind: 'python',
    label: `Lailaba at ${ACTIVE_LAILABA_ROOT}`,
    command,
    args: ['-m', 'lailaba_cli.main', ...dashboardArgs],
    env: buildDesktopBackendEnv({
      hermesHome: LAILABA_HOME,
      pythonPathEntries: [ACTIVE_LAILABA_ROOT],
      venvRoot: VENV_ROOT
    }),
    root: ACTIVE_LAILABA_ROOT,
    bootstrap: true,
    shell: false
  })
}
...
function resolveHermesBackend(dashboardArgs) {
  // 1. Explicit override -- LAILABA_DESKTOP_LAILABA_ROOT points at a developer
  //    checkout. Honour it as-is (no bootstrap; the user is driving).
  const overrideRoot = process.env.LAILABA_DESKTOP_LAILABA_ROOT && path.resolve(process.env.LAILABA_DESKTOP_LAILABA_ROOT)
  if (overrideRoot && isHermesSourceRoot(overrideRoot)) {
    const backend = createPythonBackend(overrideRoot, `Lailaba source at ${overrideRoot}`, dashboardArgs)
  ...
  //    installed `lailaba` on PATH so local Python edits are actually exercised.
  //    (In dev with no checkout, SOURCE_REPO_ROOT won't pass isHermesSourceRoot.)
  if (!IS_PACKAGED && isHermesSourceRoot(SOURCE_REPO_ROOT)) {
    const backend = createPythonBackend(SOURCE_REPO_ROOT, `Lailaba source at ${SOURCE_REPO_ROOT}`, dashboardArgs)
    if (backend) return backend
  }

  // 3. Bootstrap-complete ACTIVE_LAILABA_ROOT -- the canonical install at
  //    %LOCALAPPDATA%\lailaba\lailaba-agent (Windows) or ~/.lailaba/lailaba-agent.
  ...
  //    to spawning lailaba. Updates flow through the in-app update path
  //    (applyUpdates -> git pull) or `lailaba update` from the CLI.
  if (isBootstrapComplete()) {
    return createActiveBackend(dashboardArgs)
  }

  // 4. Existing `lailaba` on PATH -- installed via install.ps1 / install.sh from
  //    a previous tool-only setup, or pip-installed system-wide. Use it but
  //    do NOT write a bootstrap marker; the user did this themselves and we
  //    don't want to take ownership of an install we didn't perform.
  //    LAILABA_DESKTOP_IGNORE_EXISTING=1 forces the bootstrap path for testing.
  if (process.env.LAILABA_DESKTOP_IGNORE_EXISTING !== '1') {
    let hermesCommand = null
    const hermesOverride = process.env.LAILABA_DESKTOP_LAILABA
    ...
    if (hermesOverride) {
      ...
      if (resolvedOverride) {
        ...
        rememberLog(`Ignoring Windows Lailaba override under WSL: ${hermesOverride}`)
      }
    } else {
      hermesCommand = findOnPath('lailaba')
    }

    if (hermesCommand) {
      if (looksLikeDesktopAppBinary(hermesCommand)) {
        rememberLog(`Ignoring desktop app executable on PATH while resolving Lailaba CLI: ${hermesCommand}`)
        hermesCommand = null
      }
    }
    ...
    if (hermesCommand) {
      ...
      // Smoke-test the candidate before trusting it. A `lailaba` shim
      ...
      if (verifyHermesCli(hermesCommand, { shell: shellForProbe })) {
        return (
          unwrapWindowsVenvHermesCommand(hermesCommand, dashboardArgs) || {
            label: `existing Lailaba CLI at ${hermesCommand}`,
      ...
      rememberLog(
        `Ignoring existing Lailaba CLI at ${hermesCommand}: --version probe failed; falling through to bootstrap.`
      )
    }
  }

  // 5. Last-ditch: pip-installed lailaba_cli module via system Python.
  ...
  if (python) {
    ...
    // having lailaba_cli installed -- common on dev boxes that have
    ...
    // a uv-managed 3.11 into %LOCALAPPDATA%\lailaba\lailaba-agent\venv.
    if (canImportHermesCli(python)) {
      return applyWindowsNoConsoleSpawnHints({
        kind: 'python',
        label: `installed lailaba_cli module via ${python}`,
    ...
    rememberLog(`Ignoring system Python ${python}: lailaba_cli is not importable; falling through to bootstrap.`)
  }
  ...
  return {
    ...
    label: 'Lailaba AI not installed yet; bootstrap required',
    command: null,
    args: dashboardArgs,
    bootstrap: true,
    env: {},
    ...
    activeRoot: ACTIVE_LAILABA_ROOT,
...
async function ensureRuntime(backend) {
  ...
  if (backend.kind === 'bootstrap-needed') {
    rememberLog('[bootstrap] no Lailaba install found; starting first-launch bootstrap')

    if (await handOffWindowsBootstrapRecovery('bootstrap-needed')) {
      const handoffError = new Error(
        'Lailaba recovery was handed off to Lailaba Setup. The desktop will restart when recovery completes.'
      )
    ...
    const bootstrapResult = await runBootstrap({
      ...
      hermesHome: LAILABA_HOME,
      logRoot: path.join(LAILABA_HOME, 'logs'),
    ...
    if (bootstrapResult.cancelled) {
      const cancelledError = new Error('Lailaba install was cancelled.')
    ...
    if (!bootstrapResult.ok) {
      const bootstrapError = new Error(
        `Lailaba bootstrap failed${bootstrapResult.failedStage ? ` at stage '${bootstrapResult.failedStage}'` : ''}: ` +
          `${bootstrapResult.error || 'unknown error'}. ` +
          `Check ${path.join(LAILABA_HOME, 'logs', 'desktop.log')} for the full transcript.`
      )
      ...
      // lailaba:bootstrap:reset IPC (renderer's "Reload and retry").
  ...
  if (!isHermesSourceRoot(ACTIVE_LAILABA_ROOT)) {
    throw new Error(
      `Lailaba install at ${ACTIVE_LAILABA_ROOT} is missing or incomplete. ` +
        'Reinstall via the desktop installer or scripts/install.ps1.'
    )
  }

  // On Windows, preflight Git Bash. Lailaba' terminal tool calls bash.exe
  ...
  // %LOCALAPPDATA%\lailaba\git\, which findGitBash() picks up, so for any
  // user who completed the bootstrap this is a no-op. For users who got
  // here via an external `lailaba` on PATH, this check still helps.
  if (IS_WINDOWS && !findGitBash()) {
    throw new Error(
      'Git for Windows is required for Lailaba on Windows (provides Git Bash, ' +
        "which the agent's terminal tool uses). Install it from " +
        'https://git-scm.com/download/win or run `winget install -e --id Git.Git`, ' +
        'then relaunch Lailaba.'
    )
  }
  ...
  if (!fileExists(venvPython)) {
    ...
    throw new Error(
      `Lailaba venv missing at ${VENV_ROOT}. Re-run the desktop installer or ` + '`scripts/install.ps1` to rebuild it.'
    )
  }

  backend.command = getNoConsoleVenvPython(VENV_ROOT)
  backend.label = `Lailaba at ${ACTIVE_LAILABA_ROOT} (venv: ${VENV_ROOT})`
  updateBootProgress({
    phase: 'runtime.ready',
    message: 'Lailaba runtime is ready',
    progress: 82,
    running: true,
    error: null
  })
...
function fetchJson(url, token, options = {}) {
  return new Promise((resolve, reject) => {
    ...
    if (parsed.protocol !== 'http:' && parsed.protocol !== 'https:') {
      reject(new Error(`Unsupported Lailaba backend URL protocol: ${parsed.protocol}`))
      return
    }
            ...
            reject(
              new Error(
                `Expected JSON from ${url} but got HTML (status ${res.statusCode}). ` +
                  'The endpoint is likely missing on the Lailaba backend.'
              )
            )
            return
          }
    ...
    req.setTimeout(timeoutMs, () => {
      req.destroy(new Error(`Timed out connecting to Lailaba backend after ${timeoutMs}ms`))
    })
...
function fetchPublicJson(url, options = {}) {
  ...
  // NO ``X-Lailaba-Session-Token`` header — used by the auth-mode probe before
...
function getLinkTitleSession() {
  ...
  linkTitleSession = session.fromPartition('lailaba:link-titles', { cache: false })
...
function sendPreviewFileChanged(payload) {
  ...
  const { webContents } = mainWindow
  if (!webContents || webContents.isDestroyed()) return
  webContents.send('lailaba:preview-file-changed', payload)
}
...
async function waitForHermes(baseUrl, token) {
  ...
  throw new Error(`Lailaba backend did not become ready: ${lastError?.message || 'timeout'}`)
}
...
function sendBackendExit(payload) {
  ...
  const { webContents } = mainWindow
  if (!webContents || webContents.isDestroyed()) return
  webContents.send('lailaba:backend-exit', payload)
}
...
function sendClosePreviewRequested() {
  ...
  const { webContents } = mainWindow
  if (!webContents || webContents.isDestroyed()) return
  webContents.send('lailaba:close-preview-requested')
}

// Tell the renderer the machine just woke. Sleep silently drops the
// renderer's WebSocket to the local backend; the renderer reconnects on this
// signal so the chat composer doesn't stay stuck on "Starting Lailaba...".
function sendPowerResume() {
  if (!mainWindow || mainWindow.isDestroyed()) return
  const { webContents } = mainWindow
  if (!webContents || webContents.isDestroyed()) return
  webContents.send('lailaba:power-resume')
}
...
function sendOpenUpdatesRequested() {
  ...
  const { webContents } = mainWindow
  if (!webContents || webContents.isDestroyed()) return
  webContents.send('lailaba:open-updates')
...
function sendWindowStateChanged(nextIsFullscreen) {
  ...
  webContents.send('lailaba:window-state-changed', state)
}
...
const ZOOM_STORAGE_KEY = 'lailaba:desktop:zoomLevel'
...
//
// Hosted Lailaba gateways gate the dashboard behind an OAuth provider (e.g.
// Nous Research) instead of a static session token. The auth model is
// fundamentally different from the token path:
//
//   * REST is authed by HttpOnly session cookies (``lailaba_session_at``),
...
//     alongside the ~15-min access token (Portal NAS #293 / lailaba #37247).
//     Both are set as HttpOnly cookies (``lailaba_session_at`` ~15 min,
//     ``lailaba_session_rt`` 24h). When the AT cookie lapses but the RT cookie
...
const OAUTH_SESSION_PARTITION = 'persist:lailaba-remote-oauth'
  ...
  return new Promise((resolve, reject) => {
    ...
    try {
      win = new BrowserWindow({
        width: 520,
        height: 720,
        title: 'Sign in to Lailaba gateway',
...
function fetchJsonViaOauthSession(url, options = {}) {
  return new Promise((resolve, reject) => {
    ...
    const timer = setTimeout(() => {
      ...
      reject(new Error(`Timed out connecting to Lailaba backend after ${timeoutMs}ms`))
    }, timeoutMs)
...
// a valid stored value (pins the root LAILABA_HOME explicitly); null means "no
...
async function sanitizeDesktopConnectionConfig(config = readDesktopConnectionConfig(), profile = null) {
  ...
  const envOverride = key ? false : Boolean(process.env.LAILABA_DESKTOP_REMOTE_URL)

  const remoteToken = decryptDesktopSecret(block.token)
  const authMode = normAuthMode(block.authMode)
  const remoteUrl = envOverride ? String(process.env.LAILABA_DESKTOP_REMOTE_URL || '') : String(block.url || '')
  ...
  return {
    ...
    // scope is never overridden by LAILABA_DESKTOP_REMOTE_URL.
    envOverride
  }
}
...
async function buildRemoteConnection(rawUrl, authMode, token, source) {
  ...
  if (authMode === 'oauth') {
    ...
    // Portal issues a 24h rotating refresh token (lailaba #37247), and the
    ...
    if (!(await hasLiveOauthSession(baseUrl))) {
      const err = new Error(
        'Remote Lailaba gateway uses OAuth, but you are not signed in. ' +
  ...
  if (!token) {
    throw new Error(
      'Remote Lailaba gateway is selected, but no session token is saved. ' +
...
//   2. env override (LAILABA_DESKTOP_REMOTE_URL/_TOKEN) — applies app-wide
...
async function resolveRemoteBackend(profile) {
  ...
  const rawEnvUrl = process.env.LAILABA_DESKTOP_REMOTE_URL
  const rawEnvToken = process.env.LAILABA_DESKTOP_REMOTE_TOKEN
  if (rawEnvUrl) {
    if (!rawEnvToken) {
      throw new Error(
        'LAILABA_DESKTOP_REMOTE_URL is set but LAILABA_DESKTOP_REMOTE_TOKEN is not. ' +
          'Both must be provided to connect to a remote Lailaba backend.'
      )
    }
...
function globalRemoteActive() {
  if (process.env.LAILABA_DESKTOP_REMOTE_URL) {
    return true
  }
...
async function probeRemoteAuthMode(rawUrl) {
  ...
  // sending any credentials. ``/api/status`` is public on every Lailaba
...
async function testDesktopConnectionConfig(input = {}) {
  ...
  // connect to Lailaba gateway". Mirror the renderer's connect here so the test
...
async function spawnPoolBackend(profile, entry) {
  ...
  // --profile wins over the inherited LAILABA_HOME env (see _apply_profile_override
  // step 3 in lailaba_cli/main.py), so the child re-homes to this profile.
  ...
  rememberLog(`Starting Lailaba backend for profile "${profile}" via ${backend.label}`)
  ...
  const child = spawn(
    ...
    hiddenWindowsChildOptions({
      ...
      env: {
        ...
        LAILABA_DASHBOARD_SESSION_TOKEN: token,
        // Marks this dashboard backend as desktop-spawned so it runs the cron
        // scheduler tick loop (the gateway isn't running under the app).
        LAILABA_DESKTOP: '1',
        LAILABA_WEB_DIST: webDist,
        ...(readyFile ? { LAILABA_DESKTOP_READY_FILE: readyFile } : {})
      },
      shell: backend.shell,
      stdio: ['ignore', 'pipe', 'pipe']
    })
  ...
  child.once('error', error => {
    rememberLog(`Lailaba backend for profile "${profile}" failed to start: ${error.message}`)
    backendPool.delete(profile)
    rejectStart?.(error)
  })
  child.once('exit', (code, signal) => {
    rememberLog(`Lailaba backend for profile "${profile}" exited (${signal || code})`)
    backendPool.delete(profile)
    if (!ready) {
      rejectStart?.(
        new Error(`Lailaba backend for profile "${profile}" exited before it became ready (${signal || code}).`)
      )
    }
  })
  ...
  const authToken = await adoptServedDashboardToken(baseUrl, token, {
    ...
    label: `Lailaba backend for profile "${profile}"`,
    rememberLog
  })
...
async function startHermes() {
  ...
  connectionPromise = (async () => {
    await advanceBootProgress('backend.resolve', 'Resolving Lailaba backend', 8)
    // Resolve for the desktop's primary profile so a per-profile remote
    // override on the active profile is honored (falls back to env / global).
    const remote = await resolveRemoteBackend(primaryProfileKey())
    if (remote) {
      await advanceBootProgress('backend.remote', `Connecting to remote Lailaba backend at ${remote.baseUrl}`, 24)
      await waitForHermes(remote.baseUrl, remote.token)
      updateBootProgress({
        phase: 'backend.ready',
        message: 'Remote Lailaba backend is ready',
        progress: 94,
        running: true,
        error: null
      })
    ...
    // deterministic (it wins over the sticky ~/.lailaba/active_profile file) and
    // resolves LAILABA_HOME the same way `lailaba -p <name>` does on the CLI. An
    ...
    await advanceBootProgress('backend.runtime', 'Resolving Lailaba runtime', 28)
    const backend = await ensureRuntime(resolveHermesBackend(dashboardArgs))
    const hermesCwd = resolveHermesCwd()
    const webDist = resolveWebDist()
    const readyFile = backend.readyFile ? makeDashboardReadyFile() : null

    await advanceBootProgress('backend.spawn', `Starting Lailaba backend via ${backend.label}`, 84)
    rememberLog(`Starting Lailaba backend via ${backend.label}`)
    ...
    hermesProcess = spawn(
      ...
      hiddenWindowsChildOptions({
        ...
        env: {
          ...process.env,
          // Explicitly pin LAILABA_HOME for the child so Python's get_lailaba_home()
          // resolves to the SAME location our resolveHermesHome() picked. Without
          // this pin, Python falls back to ~/.lailaba on every platform — fine on
          // mac/linux (where our default matches), but on Windows our default is
          // %LOCALAPPDATA%\lailaba, which differs from C:\Users\<u>\.lailaba.
          // Mismatch would split config / sessions / .env / logs across two
          // directories. install.ps1 sets LAILABA_HOME via setx; the desktop
    ...
    hermesProcess.once('error', error => {
      rememberLog(`Lailaba backend failed to start: ${error.message}`)
      updateBootProgress(
        {
          error: error.message,
          message: `Lailaba backend failed to start: ${error.message}`,
          phase: 'backend.error',
          running: false
        },
        { allowDecrease: true }
    ...
    hermesProcess.once('exit', (code, signal) => {
      rememberLog(`Lailaba backend exited (${signal || code})`)
      hermesProcess = null
      connectionPromise = null
      sendBackendExit({ code, signal })
      if (!backendReady) {
        const message = `Lailaba backend exited before it became ready (${signal || code}).`
        updateBootProgress(
          {
            error: message,
            message,
        ...
        rejectBackendStart?.(
          new Error(
            `Lailaba backend exited before it became ready (${signal || code}). Log: ${DESKTOP_LOG_PATH}\n${recentHermesLog()}`
          )
        )
      }
    })

    await advanceBootProgress('backend.port', 'Waiting for Lailaba backend to launch', 86)
    // Discover the ephemeral port the child bound to
    ...
    const baseUrl = `http://127.0.0.1:${port}`
    await advanceBootProgress('backend.wait', 'Waiting for Lailaba backend to become ready', 90)
    ...
    updateBootProgress({
      phase: 'backend.ready',
      message: 'Lailaba backend is ready. Finalizing desktop startup',
      progress: 94,
      running: true,
      error: null
    })
...
// here so it can leave the app's bounds and stay visible while Lailaba is
...
// pushes pet state over IPC (lailaba:pet-overlay:state); the overlay just renders
// it. Control flows back (pop-in, composer submit) via lailaba:pet-overlay:control.
...
function spawnPetOverlayWindow(bounds) {
  const win = new BrowserWindow({
    ...
    // instead, leaving the main Lailaba app as the Dock/cmd-tab anchor.
    skipTaskbar: !IS_MAC,
    hasShadow: false,
    alwaysOnTop: true,
    ...
    // anchor and the Lailaba icon drops out of cmd/alt-tab — especially when the
    // main window is minimized. We flip this on only while the composer needs
    // the keyboard (see lailaba:pet-overlay:set-focusable).
    focusable: false,
    show: false,
  ...
  try {
    ...
    // which briefly hides the Dock/cmd-tab presence. Keep Lailaba in the normal
  ...
  win.on('closed', () => {
    ...
    if (mainWindow && !mainWindow.isDestroyed()) {
      mainWindow.webContents.send('lailaba:pet-overlay:control', { type: 'pop-in' })
    }
  })
...
ipcMain.handle('lailaba:connection', async (_event, profile) => ensureBackend(profile))
...
// "Starting Lailaba…". Before the renderer's backoff loop reconnects, it asks us
// to confirm the cached PRIMARY backend is still reachable; if a remote one is
// not, we drop the cache so the next getConnection() rebuilds it. Local backends
// self-heal via their child 'exit' handler, so we never touch them here.
ipcMain.handle('lailaba:connection:revalidate', async () => {
  ...
  try {
    ...
    rememberLog('Cached remote Lailaba backend failed liveness probe; dropping stale connection.')
    resetHermesConnection()
    return { ok: true, rebuilt: true }
  }
})
ipcMain.handle('lailaba:backend:touch', async (_event, profile) => {
  touchPoolBackend(profile)
  return { ok: true }
})
ipcMain.handle('lailaba:gateway:ws-url', async (_event, profile) => freshGatewayWsUrl(profile))
ipcMain.handle('lailaba:window:openSession', async (_event, sessionId, opts) => {
...
ipcMain.handle('lailaba:window:openNewSession', async () => {
...
ipcMain.handle('lailaba:pet-overlay:open', async (_event, request) => {
...
ipcMain.handle('lailaba:pet-overlay:close', async () => {
  closePetOverlay()
...
ipcMain.on('lailaba:pet-overlay:set-bounds', (_event, bounds) => {
...
ipcMain.on('lailaba:pet-overlay:ignore-mouse', (_event, ignore) => {
  if (petOverlayWindow && !petOverlayWindow.isDestroyed()) {
...
ipcMain.on('lailaba:pet-overlay:set-focusable', (_event, focusable) => {
  if (!petOverlayWindow || petOverlayWindow.isDestroyed()) {
    return
  }
...
ipcMain.on('lailaba:pet-overlay:state', (_event, payload) => {
  if (petOverlayWindow && !petOverlayWindow.isDestroyed()) {
    petOverlayWindow.webContents.send('lailaba:pet-overlay:state', payload)
  }
})
// Overlay → main renderer: control messages (pop back in, composer submit).
ipcMain.on('lailaba:pet-overlay:control', (_event, payload) => {
  if (!mainWindow || mainWindow.isDestroyed()) {
    return
  }
  ...
  mainWindow.webContents.send('lailaba:pet-overlay:control', payload)
})
ipcMain.handle('lailaba:bootstrap:reset', async () => {
...
ipcMain.handle('lailaba:bootstrap:repair', async () => {
...
ipcMain.handle('lailaba:bootstrap:cancel', async () => {
...
ipcMain.handle('lailaba:boot-progress:get', async () => bootProgressState)
ipcMain.handle('lailaba:bootstrap:get', async () => getBootstrapState())
ipcMain.handle('lailaba:connection-config:get', async (_event, profile) =>
  sanitizeDesktopConnectionConfig(readDesktopConnectionConfig(), profile)
)
ipcMain.handle('lailaba:connection-config:test', async (_event, payload) => testDesktopConnectionConfig(payload))
ipcMain.handle('lailaba:connection-config:probe', async (_event, rawUrl) => probeRemoteAuthMode(rawUrl))
ipcMain.handle('lailaba:connection-config:oauth-login', async (_event, rawUrl) => {
...
ipcMain.handle('lailaba:connection-config:oauth-logout', async (_event, rawUrl) => {
...
ipcMain.handle('lailaba:connection-config:save', async (_event, payload) => {
  const config = coerceDesktopConnectionConfig(payload)
  writeDesktopConnectionConfig(config)

  return sanitizeDesktopConnectionConfig(config, payload?.profile)
})
ipcMain.handle('lailaba:connection-config:apply', async (_event, payload) => {
  const config = coerceDesktopConnectionConfig(payload)
  writeDesktopConnectionConfig(config)
...
ipcMain.handle('lailaba:profile:get', async () => ({ profile: readActiveDesktopProfile() }))
ipcMain.handle('lailaba:profile:set', async (_event, name) => {
  const next = writeActiveDesktopProfile(name)

  // Switching profiles is a backend re-home: relaunch the dashboard under the
  // new LAILABA_HOME. Pool backends keep their own homes, so only the primary
  // is torn down.
  await teardownPrimaryBackendAndWait()
  mainWindow?.reload()
...
ipcMain.on('lailaba:previewShortcutActive', (_event, active) => {
  previewShortcutActive = Boolean(active)
})

ipcMain.handle('lailaba:requestMicrophoneAccess', async () => {
...
ipcMain.handle('lailaba:api', async (_event, request) => {
...
ipcMain.handle('lailaba:notify', (_event, payload) => {
  ...
  notification.on('click', () => {
    ...
    if (payload?.sessionId) {
      mainWindow.webContents.send('lailaba:focus-session', payload.sessionId)
    }
  })
  notification.on('action', (_actionEvent, index) => {
    ...
    if (action?.id) {
      mainWindow.webContents.send('lailaba:notification-action', { sessionId: payload?.sessionId, actionId: action.id })
    }
  })
  notification.show()
  return true
...
ipcMain.handle('lailaba:readFileDataUrl', async (_event, filePath) => {
...
ipcMain.handle('lailaba:readFileText', async (_event, filePath) => {
...
ipcMain.handle('lailaba:selectPaths', async (_event, options = {}) => {
...
ipcMain.handle('lailaba:writeClipboard', (_event, text) => {
  clipboard.writeText(String(text || ''))
  return true
})

ipcMain.handle('lailaba:saveImageFromUrl', (_event, url) => saveImageFromUrl(String(url || '')))

ipcMain.handle('lailaba:saveImageBuffer', async (_event, payload) => {
...
ipcMain.handle('lailaba:saveClipboardImage', async () => {
...
ipcMain.handle('lailaba:normalizePreviewTarget', (_event, target, baseDir) =>
  normalizePreviewTarget(String(target || ''), baseDir ? String(baseDir) : '')
)

ipcMain.handle('lailaba:watchPreviewFile', (_event, url) => watchPreviewFile(String(url || '')))

ipcMain.handle('lailaba:stopPreviewFileWatch', (_event, id) => stopPreviewFileWatch(String(id || '')))

ipcMain.on('lailaba:titlebar-theme', (_event, payload) => {
...
ipcMain.on('lailaba:native-theme', (_event, mode) => {
...
ipcMain.on('lailaba:translucency', (_event, payload) => {
...
ipcMain.handle('lailaba:openExternal', (_event, url) => {
  if (!openExternalUrl(url)) {
    throw new Error('Invalid external URL')
  }
})

ipcMain.handle('lailaba:openPreviewInBrowser', async (_event, url) => {
...
ipcMain.handle('lailaba:setting:defaultProjectDir:get', async () => ({
  dir: readDefaultProjectDir(),
  defaultLabel: app.getPath('home'),
  resolvedCwd: resolveHermesCwd()
}))

ipcMain.handle('lailaba:workspace:sanitize', async (_event, cwd) => sanitizeWorkspaceCwd(cwd))

ipcMain.handle('lailaba:setting:defaultProjectDir:set', async (_event, dir) => {
...
ipcMain.handle('lailaba:setting:defaultProjectDir:pick', async () => {
...
ipcMain.handle('lailaba:fetchLinkTitle', (_event, url) => fetchLinkTitle(url))

ipcMain.handle('lailaba:logs:reveal', async () => {
  try {
    await fs.promises.mkdir(path.dirname(DESKTOP_LOG_PATH), { recursive: true })
...
ipcMain.handle('lailaba:logs:recent', async () => ({ path: DESKTOP_LOG_PATH, lines: hermesLog.slice(-200) }))
...
function terminalShellCommand() {
  // LAILABA_DESKTOP_SHELL is the cross-platform escape hatch (a path or a bare
  ...
  const override = (process.env.LAILABA_DESKTOP_SHELL || (IS_WINDOWS ? '' : process.env.SHELL) || '').trim()
...
function terminalShellEnv() {
  ...
  // /TERM=dumb; some terminals set COLORFGBG which would flip Lailaba' TUI into
  ...
  env.TERM_PROGRAM = 'Lailaba'
  env.TERM_PROGRAM_VERSION = app.getVersion()

  // Let a lailaba/--tui launched in this pane know it's embedded in the desktop
  // GUI (build_environment_hints surfaces this). Distinct from LAILABA_DESKTOP,
  // which marks the agent *backend* and gates cron/gateway behavior.
  env.LAILABA_DESKTOP_TERMINAL = '1'

  return env
}

function terminalChannel(id, suffix) {
  return `lailaba:terminal:${id}:${suffix}`
}
...
ipcMain.handle('lailaba:fs:readDir', async (_event, dirPath) => readDirForIpc(dirPath))

ipcMain.handle('lailaba:fs:gitRoot', async (_event, startPath) => gitRootForIpc(startPath))

// Reveal a path in the OS file manager (Finder / Explorer / Files).
ipcMain.handle('lailaba:fs:reveal', async (_event, targetPath) => {
...
ipcMain.handle('lailaba:fs:rename', async (_event, targetPath, newName) => {
...
ipcMain.handle('lailaba:fs:writeText', async (_event, filePath, content) => {
...
ipcMain.handle('lailaba:fs:trash', async (_event, targetPath) => {
  const target = String(targetPath || '').trim()
...
ipcMain.handle('lailaba:git:worktreeList', async (_event, repoPath) => listWorktrees(repoPath, resolveGitBinary()))

ipcMain.handle('lailaba:git:worktreeAdd', async (_event, repoPath, options) =>
  addWorktree(repoPath, options || {}, resolveGitBinary())
)

ipcMain.handle('lailaba:git:worktreeRemove', async (_event, repoPath, worktreePath, options) =>
  removeWorktree(repoPath, worktreePath, options || {}, resolveGitBinary())
)

ipcMain.handle('lailaba:git:branchSwitch', async (_event, repoPath, branch) =>
  switchBranch(repoPath, branch, resolveGitBinary())
)

ipcMain.handle('lailaba:git:branchList', async (_event, repoPath) => listBranches(repoPath, resolveGitBinary()))
...
ipcMain.handle('lailaba:git:repoStatus', async (_event, repoPath) => repoStatus(repoPath, resolveGitBinary()))
...
ipcMain.handle('lailaba:git:review:list', async (_event, repoPath, scope, baseRef) =>
  reviewList(repoPath, scope, baseRef, resolveGitBinary())
)
ipcMain.handle('lailaba:git:review:diff', async (_event, repoPath, filePath, scope, baseRef, staged) =>
...
ipcMain.handle('lailaba:git:fileDiff', async (_event, repoPath, filePath) =>
  fileDiffVsHead(repoPath, filePath, resolveGitBinary())
)
ipcMain.handle('lailaba:git:review:stage', async (_event, repoPath, filePath) =>
  reviewStage(repoPath, filePath ?? null, resolveGitBinary())
)
ipcMain.handle('lailaba:git:review:unstage', async (_event, repoPath, filePath) =>
  reviewUnstage(repoPath, filePath ?? null, resolveGitBinary())
)
ipcMain.handle('lailaba:git:review:revert', async (_event, repoPath, filePath) =>
  reviewRevert(repoPath, filePath ?? null, resolveGitBinary())
)
ipcMain.handle('lailaba:git:review:revParse', async (_event, repoPath, ref) =>
  reviewRevParse(repoPath, ref, resolveGitBinary())
)
ipcMain.handle('lailaba:git:review:commit', async (_event, repoPath, message, push) =>
  reviewCommit(repoPath, message, Boolean(push), resolveGitBinary())
)
ipcMain.handle('lailaba:git:review:commitContext', async (_event, repoPath) =>
  reviewCommitContext(repoPath, resolveGitBinary())
)
ipcMain.handle('lailaba:git:review:push', async (_event, repoPath) => reviewPush(repoPath, resolveGitBinary()))
ipcMain.handle('lailaba:git:review:shipInfo', async (_event, repoPath) => reviewShipInfo(repoPath, resolveGhBinary()))
ipcMain.handle('lailaba:git:review:createPr', async (_event, repoPath) =>
...
ipcMain.handle('lailaba:git:scanRepos', async (_event, roots, options) => {
  try {
...
ipcMain.handle('lailaba:terminal:start', async (event, payload = {}) => {
  if (!nodePty) {
    throw new Error('PTY support is unavailable. Reinstall desktop dependencies and restart Lailaba.')
  }
...
ipcMain.handle('lailaba:terminal:write', (_event, id, data) => {
...
ipcMain.handle('lailaba:terminal:resize', (_event, id, size = {}) => {
  const sessionInfo = terminalSessions.get(String(id || ''))
...
ipcMain.handle('lailaba:terminal:dispose', (_event, id) => disposeTerminalSession(String(id || '')))

ipcMain.handle('lailaba:updates:check', async () =>
...
ipcMain.handle('lailaba:updates:apply', async (_event, payload) =>
...
ipcMain.handle('lailaba:updates:branch:get', async () => readDesktopUpdateConfig())

ipcMain.handle('lailaba:updates:branch:set', async (_event, name) => {
  const branch = typeof name === 'string' && name.trim() ? name.trim() : DEFAULT_UPDATE_BRANCH
  writeDesktopUpdateConfig({ branch })
  return { branch }
})

// Resolve the canonical Lailaba version (the one `release.py` bumps in
// lailaba_cli/__init__.py + pyproject.toml) so the desktop About panel shows the
// real Lailaba version instead of the Electron app's own package.json version,
// which historically drifted (stuck at 0.0.2). Falls back to app.getVersion()
// when the source tree can't be read (e.g. a packaged build without the repo).
function resolveHermesVersion() {
  try {
    const root = resolveUpdateRoot()
    const initPath = path.join(root, 'lailaba_cli', '__init__.py')
...
// Re-resolve the live Lailaba version and push it into the native About panel
// just before showing it, so an in-place `lailaba update` is reflected without
...
ipcMain.handle('lailaba:version', async () => ({
...
// the actual removal via `lailaba uninstall …` so the cross-platform PATH /
// registry / service / node-symlink cleanup all lives in one place
// (lailaba_cli/uninstall.py + lailaba_cli/gui_uninstall.py).
//
...
async function getUninstallSummary() {
  ...
  const agentRoot = ACTIVE_LAILABA_ROOT
  ...
  return new Promise(resolve => {
    ...
    try {
      const child = spawn(
        py,
        ['-m', 'lailaba_cli.main', 'uninstall', '--gui-summary'],
        hiddenWindowsChildOptions({
          cwd: agentRoot,
          env: { ...process.env, LAILABA_HOME, NO_COLOR: '1' },
          stdio: ['ignore', 'pipe', 'ignore']
        })
      )
      child.stdout.on('data', chunk => {
...
async function runDesktopUninstall(mode) {
  ...
  if (!fileExists(venvPy)) {
    return {
      ok: false,
      error: 'agent-missing',
      message: `Can't run the uninstaller: no Lailaba agent venv at ${VENV_ROOT}.`
    }
  }
  ...
  // Python with PYTHONPATH=<agentRoot> so `import lailaba_cli` resolves from
  ...
  if (modeRemovesAgent(mode)) {
    ...
    if (sysPy) {
      py = sysPy
      pythonPath = ACTIVE_LAILABA_ROOT
    } else if (IS_WINDOWS) {
      rememberLog(
  ...
  try {
    await releaseBackendLock(ACTIVE_LAILABA_ROOT, 'uninstall')
  } catch (error) {
  ...
  try {
    if (IS_WINDOWS) {
      scriptPath = path.join(app.getPath('temp'), `lailaba-uninstall-${Date.now()}.cmd`)
      ...
      scriptPath = path.join(app.getPath('temp'), `lailaba-uninstall-${Date.now()}.sh`)
...
ipcMain.handle('lailaba:uninstall:summary', async () => getUninstallSummary())
ipcMain.handle('lailaba:uninstall:run', async (_event, payload) => {
...
ipcMain.handle('lailaba:vscode-theme:fetch', async (_event, id) => fetchMarketplaceThemes(String(id || '')))

// Search the Marketplace for color-theme extensions (empty query = top installs).
ipcMain.handle('lailaba:vscode-theme:search', async (_event, query) => searchMarketplaceThemes(String(query || ''), 20))

// ---------------------------------------------------------------------------
// lailaba:// deep links (e.g. lailaba://blueprint/morning-brief?time=08:00).
// A docs/dashboard "Send to App" button opens this URL; we route it into the
// running app's chat composer. Three delivery paths: macOS 'open-url',
// Win/Linux running-app 'second-instance' (argv), Win/Linux cold-start argv.
// ---------------------------------------------------------------------------
const LAILABA_PROTOCOL = 'lailaba'
let _pendingDeepLink = null
let _rendererReadyForDeepLink = false

function _extractDeepLink(argv) {
  if (!Array.isArray(argv)) return null
  return argv.find(a => typeof a === 'string' && a.startsWith(`${LAILABA_PROTOCOL}://`)) || null
}
...
function handleDeepLink(url) {
  ...
  // lailaba://blueprint/<key>?slot=val  -> host="blueprint", path="/<key>"
  ...
  try {
    ...
    mainWindow.focus()
    mainWindow.webContents.send('lailaba:deep-link', payload)
...
ipcMain.handle('lailaba:deep-link-ready', () => {
  _rendererReadyForDeepLink = true
  if (_pendingDeepLink) {
    const queued = _pendingDeepLink
    _pendingDeepLink = null
    handleDeepLink(
      `${LAILABA_PROTOCOL}://${queued.kind}/${encodeURIComponent(queued.name)}` +
...
function registerDeepLinkProtocol() {
  try {
    if (process.defaultApp && process.argv.length >= 2) {
      ...
      app.setAsDefaultProtocolClient(LAILABA_PROTOCOL, process.execPath, [path.resolve(process.argv[1])])
    } else {
      app.setAsDefaultProtocolClient(LAILABA_PROTOCOL)
    }
  } catch (err) {
...
// second-instance argv. Without the lock a second `lailaba://` launch spawns a
...
app.whenReady().then(() => {
  ...
  // Win/Linux cold start: the launching lailaba:// URL is in our own argv.
