 ...
 * the three user-facing uninstall modes to the `lailaba uninstall` CLI flags,
...
/**
 * Map an uninstall mode to the `python -m lailaba_cli.uninstall` argv (after the
 * python executable). Uses the dedicated lightweight module entrypoint (not
 * `lailaba_cli.main`) so it can run under a system Python OUTSIDE the venv that
...
function uninstallArgsForMode(mode) {
  ...
  return ['-m', 'lailaba_cli.uninstall', '--mode', mode]
}
 ...
 *
 *   macOS:   …/Lailaba.app/Contents/MacOS/Lailaba  → …/Lailaba.app
 *   Windows: …\Lailaba\Lailaba.exe                 → …\Lailaba  (install dir)
...
function resolveRemovableAppPath(execPath, platform, env = {}) {
  ...
  if (platform === 'darwin') {
    // …/Lailaba.app/Contents/MacOS/Lailaba → strip 3 segments to the .app
    const macOsDir = p.dirname(exe) // …/Contents/MacOS
    const contents = p.dirname(macOsDir) // …/Contents
    const appBundle = p.dirname(contents) // …/Lailaba.app
    if (appBundle.endsWith('.app')) return appBundle
    return null
  }

  if (platform === 'win32') {
    // NSIS per-user installs Lailaba.exe directly in the install dir.
    const dir = p.dirname(exe)
    if (/[\\/]Lailaba$/i.test(dir) || /[\\/]lailaba-desktop$/i.test(dir)) return dir
    return null
  }

  // Linux: an AppImage exposes its own path via the APPIMAGE env var.
  if (env.APPIMAGE) return env.APPIMAGE
  // Unpacked electron-builder tree: …/linux-unpacked/lailaba
  const dir = p.dirname(exe)
 ...
 * being deleted); `pythonPath` is prepended to PYTHONPATH so `import lailaba_cli`
...
function buildPosixCleanupScript({ desktopPid, pythonExe, pythonPath, agentRoot, uninstallArgs, appPath, hermesHome }) {
  ...
  const lines = [
    ...
    '  done',
    'fi',
    `export LAILABA_HOME=${q(hermesHome)}`
  ]
  if (pythonPath) {
 ...
 * modes + `pythonPath`=agentRoot so `import lailaba_cli` resolves from source
...
function buildWindowsCleanupScript({
  ...
  // a problem, but Lailaba install paths don't use them.
  const q = s => `"${String(s).replace(/"/g, '')}"`
  const lines = [
    '@echo off',
    'setlocal enableextensions',
    `set "LAILABA_HOME=${String(hermesHome).replace(/"/g, '')}"`,
    `set "PID=${pid}"`
  ]
  if (pythonPath) {
