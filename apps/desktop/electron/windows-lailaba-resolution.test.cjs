...
// Regression guards for Windows `lailaba` resolution in main.cjs.
//
...
//      Git-Bash `lailaba` shim shadowed the real lailaba.cmd/lailaba.exe; the
...
//      --repair by checking ONLY venv\Scripts\lailaba.exe (the console-script
...
test('findOnPath tries PATHEXT extensions before the bare (empty) name on Windows', () => {
  ...
  assert.doesNotMatch(
    source,
    /\['', \.\.\.\(process\.env\.PATHEXT/,
    'empty-extension-first order regressed: an extensionless shim can shadow lailaba.cmd/.exe'
  )
})
...
test('Windows bootstrap recovery chooses --update when any real-install signal is present', () => {
  ...
  // The old too-narrow check (only venv\Scripts\lailaba.exe) must not return.
  assert.doesNotMatch(
    source,
    /updaterArgs = fileExists\(venvHermes\) \?/,
    'recovery regressed to gating only on the lailaba.exe shim, which forces destructive --repair'
  )
})
