...
// even though a fresh shell — and the Lailaba CLI — sees it immediately. The
// desktop's LAILABA_HOME resolution relies on process.env, so that stale-snapshot
// gap silently sends the backend to the default %LOCALAPPDATA%\lailaba. Reading
...
//       LAILABA_HOME    REG_SZ    F:\Lailaba\data
//
