 ...
 * Background
 * ----------
 * After `lailaba update` + `lailaba desktop --build-only`, the freshly-rebuilt
...
// backend/profile/root it talks to). Anything LAILABA_DESKTOP_* is preserved
// plus LAILABA_HOME. We snapshot the values, not the live env, so the new
...
const PRESERVED_ENV_KEYS = ['LAILABA_HOME', 'ELECTRON_DISABLE_SANDBOX']
const PRESERVED_ENV_PREFIXES = ['LAILABA_DESKTOP_']
