...
// have lailaba_cli" (since `node -c "import lailaba_cli"` will exit
...
test('canImportHermesCli returns false when interpreter cannot run -c', () => {
  // node IS an interpreter, but `node -c "import lailaba_cli"` is a
...
test('canImportHermesCli returns false when binary does not exist', () => {
  const ghost = path.join(os.tmpdir(), 'lailaba-probes-ghost-' + Date.now() + '.exe')
  assert.equal(canImportHermesCli(ghost), false)
})

test('lailaba runtime import probe checks config dependencies', () => {
  const probe = hermesRuntimeImportProbe()
  assert.match(probe, /\bimport yaml\b/)
  assert.match(probe, /\bimport lailaba_cli\.config\b/)
})
...
test('verifyHermesCli returns true when --version exits 0', () => {
  ...
  // it through node. This stands in for a working lailaba binary --
  // verifyHermesCli only cares about the exit code.
  const scriptPath = path.join(os.tmpdir(), `lailaba-probes-ok-${Date.now()}-${process.pid}.cjs`)
