 ...
 *     `X-Lailaba-Session-Token` header; WS uses `?token=`.
...
// lailaba_cli/dashboard_auth/cookies.py.
//
// Two cookies are in play (see that module):
//   - lailaba_session_at: the OAuth access token. Short-lived (~15 min); its
//     Max-Age tracks the access-token TTL, so the cookie jar drops it the
//     instant the AT expires.
//   - lailaba_session_rt: the OAuth refresh token. Long-lived (24h rotating,
//     reuse-detected — Portal NAS #293 / lailaba #37247). When the AT cookie
...
const AT_COOKIE_VARIANTS = ['__Host-lailaba_session_at', '__Secure-lailaba_session_at', 'lailaba_session_at']
const RT_COOKIE_VARIANTS = ['__Host-lailaba_session_rt', '__Secure-lailaba_session_rt', 'lailaba_session_rt']
...
function pathWithGlobalRemoteProfile(path, profile, opts = {}) {
  ...
  let parsed
  try {
    parsed = new URL(rawPath, 'http://lailaba.local')
  } catch {
    return path
  }
...
/**
 * True if any cookie in `cookies` is a lailaba session ACCESS-token cookie
