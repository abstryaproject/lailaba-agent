# nix/desktop.nix — Lailaba Desktop (Electron) app build + wrapper
#
# `hermesAgent` is the fully-built `.#default` package — it ships the
# `lailaba` binary with the venv, runtime PATH, bundled skills/plugins, etc.
# already wired up.  We point the desktop at it via the existing
# `LAILABA_DESKTOP_LAILABA` override env var, so the desktop's resolver
# uses our fully wrapped binary at step 4 ("existing Lailaba CLI").
...
let
  ...
  renderer = pkgs.buildNpmPackage (
    npm
    // {
      pname = "lailaba-desktop-renderer";
      inherit version;
      doCheck = true;
...
stdenv.mkDerivation {
  ...
  installPhase = ''
    ...
    mkdir -p $out/share/lailaba-desktop $out/bin
    cp -r ${renderer}/* $out/share/lailaba-desktop/
    ...
    substituteInPlace $out/share/lailaba-desktop/electron/main.cjs \
      --replace-fail "process.resourcesPath" "'$out/share/lailaba-desktop'"
    ...
    substituteInPlace $out/share/lailaba-desktop/electron/git-review-ops.cjs \
      --replace-fail "process.resourcesPath" "'$out/share/lailaba-desktop'"

    # Wrap the nixpkgs electron binary to launch our app.  Set
    # LAILABA_DESKTOP_LAILABA to the absolute path of the nix-built `lailaba`
    # binary so the desktop's resolver step 4 ("existing Lailaba CLI on
    # PATH") uses our fully wrapped binary — venv with all deps,
    # bundled skills/plugins, runtime PATH (ripgrep/git/ffmpeg/etc).
    # No reimplementation of the agent resolver in the wrapper.
    makeWrapper ${lib.getExe electron} $out/bin/lailaba-desktop \
      --add-flags "$out/share/lailaba-desktop" \
      --set LAILABA_DESKTOP_LAILABA "${lib.getExe hermesAgent}" \
  ...
  meta = with lib; {
    description = "Native Electron desktop shell for Lailaba AI";
    homepage = "https://github.com/NousResearch/lailaba-agent";
    license = licenses.mit;
    platforms = platforms.unix;
    mainProgram = "lailaba-desktop";
  };
}
