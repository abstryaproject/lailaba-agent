...
{ inputs, ... }: {
  perSystem = { pkgs, lib, self', ... }:
    let
      lailaba-agent = self'.packages.default;
      hermesVenv = lailaba-agent.hermesVenv;

      configMergeScript = pkgs.callPackage ./configMergeScript.nix { };

      # Auto-generated config key reference — always in sync with Python
      configKeys = pkgs.runCommand "lailaba-config-keys" {} ''
        set -euo pipefail
        export HOME=$TMPDIR
...
' > $out
    ...
    in {
      ...
      checks = {
        ...
        in pkgs.runCommand "lailaba-cross-eval" { } (
          if failures != [] then
        ...
        build-package = pkgs.runCommand "lailaba-build-package" { } ''
          echo "PASS: package built at ${lailaba-agent}"
          mkdir -p $out
          echo "ok" > $out/result
        '';

        # Verify the devShell builds successfully (cross-platform).
        build-devshell = pkgs.runCommand "lailaba-build-devshell" { } ''
        ...
        package-contents = pkgs.runCommand "lailaba-package-contents" { } ''
          set -e
          echo "=== Checking binaries ==="
          test -x ${lailaba-agent}/bin/lailaba || (echo "FAIL: lailaba binary missing"; exit 1)
          test -x ${lailaba-agent}/bin/lailaba-agent || (echo "FAIL: lailaba-agent binary missing"; exit 1)
          echo "PASS: All binaries present"

          echo "=== Checking version ==="
          ${lailaba-agent}/bin/lailaba version 2>&1 | grep -qi "lailaba" || (echo "FAIL: version check"; exit 1)
        ...
        entry-points-sync = pkgs.runCommand "lailaba-entry-points-sync" { } ''
          set -e
          echo "=== Checking entry points match pyproject.toml [project.scripts] ==="
          for bin in lailaba lailaba-agent lailaba-acp; do
            test -x ${lailaba-agent}/bin/$bin || (echo "FAIL: $bin binary missing from Nix package"; exit 1)
        ...
        cli-commands = pkgs.runCommand "lailaba-cli-commands" { } ''
          set -e
          export HOME=$(mktemp -d)

          echo "=== Checking lailaba --help ==="
          ${lailaba-agent}/bin/lailaba --help 2>&1 | grep -q "gateway" || (echo "FAIL: gateway subcommand missing"; exit 1)
          ${lailaba-agent}/bin/lailaba --help 2>&1 | grep -q "config" || (echo "FAIL: config subcommand missing"; exit 1)
        ...
        bundled-skills = pkgs.runCommand "lailaba-bundled-skills" { } ''
          set -e
          echo "=== Checking bundled skills ==="
          test -d ${lailaba-agent}/share/lailaba-agent/skills || (echo "FAIL: skills directory missing"; exit 1)
          echo "PASS: skills directory exists"

          SKILL_COUNT=$(find ${lailaba-agent}/share/lailaba-agent/skills -name "SKILL.md" | wc -l)
          test "$SKILL_COUNT" -gt 0 || (echo "FAIL: no SKILL.md files found in skills directory"; exit 1)
          echo "PASS: $SKILL_COUNT bundled skills found"

          grep -q "LAILABA_BUNDLED_SKILLS" ${lailaba-agent}/bin/lailaba || \
            (echo "FAIL: LAILABA_BUNDLED_SKILLS not in wrapper"; exit 1)
          echo "PASS: LAILABA_BUNDLED_SKILLS set in wrapper"
        ...
        bundled-plugins = pkgs.runCommand "lailaba-bundled-plugins" { } ''
          set -e
          echo "=== Checking bundled plugins ==="
          test -d ${lailaba-agent}/share/lailaba-agent/plugins || (echo "FAIL: plugins directory missing"; exit 1)
          echo "PASS: plugins directory exists"

          test -f ${lailaba-agent}/share/lailaba-agent/plugins/platforms/irc/plugin.yaml || \
            (echo "FAIL: irc plugin manifest missing"; exit 1)
          echo "PASS: irc plugin manifest present"

          grep -q "LAILABA_BUNDLED_PLUGINS" ${lailaba-agent}/bin/lailaba || \
            (echo "FAIL: LAILABA_BUNDLED_PLUGINS not in wrapper"; exit 1)
          echo "PASS: LAILABA_BUNDLED_PLUGINS set in wrapper"
        ...
        bundled-locales = pkgs.runCommand "lailaba-bundled-locales" { } ''
          set -e
          echo "=== Checking bundled locales ==="
          test -d ${lailaba-agent}/share/lailaba-agent/locales || (echo "FAIL: locales directory missing"; exit 1)
          echo "PASS: locales directory exists"

          LOC_COUNT=$(find ${lailaba-agent}/share/lailaba-agent/locales -name "*.yaml" | wc -l)
          test "$LOC_COUNT" -ge 16 || (echo "FAIL: expected >=16 catalogs, found $LOC_COUNT"; exit 1)
          echo "PASS: $LOC_COUNT locale catalogs found"

          test -f ${lailaba-agent}/share/lailaba-agent/locales/en.yaml || (echo "FAIL: en.yaml missing"; exit 1)
          echo "PASS: en.yaml present"

          grep -q "LAILABA_BUNDLED_LOCALES" ${lailaba-agent}/bin/lailaba || \
            (echo "FAIL: LAILABA_BUNDLED_LOCALES not in wrapper"; exit 1)
          echo "PASS: LAILABA_BUNDLED_LOCALES set in wrapper"

          echo "=== Rendering via the wrapper override (LAILABA_BUNDLED_LOCALES) ==="
          export HOME=$(mktemp -d)
          RENDERED=$(cd "$HOME" && LAILABA_BUNDLED_LOCALES=${lailaba-agent}/share/lailaba-agent/locales \
            ${hermesVenv}/bin/python3 -c "from agent import i18n; print(i18n.t('gateway.reset.header_default', lang='en'))")
          echo "rendered: $RENDERED"
          test "$RENDERED" != "gateway.reset.header_default" || (echo "FAIL: i18n returned the raw key with LAILABA_BUNDLED_LOCALES set"; exit 1)
          ...
          test "$BARE" != "gateway.reset.header_default" || \
            (echo "FAIL: sealed venv could not resolve locales without LAILABA_BUNDLED_LOCALES — data-files materialization regressed"; exit 1)
        ...
        bundled-tui = pkgs.runCommand "lailaba-bundled-tui" { } ''
          set -e
          echo "=== Checking bundled TUI ==="
          test -d ${lailaba-agent}/ui-tui || (echo "FAIL: ui-tui directory missing"; exit 1)
          echo "PASS: ui-tui directory exists"

          test -f ${lailaba-agent}/ui-tui/dist/entry.js || (echo "FAIL: compiled entry.js missing"; exit 1)
          echo "PASS: compiled entry.js present"

          # self-contained bundle; no runtime node_modules expected

          grep -q "LAILABA_TUI_DIR" ${lailaba-agent}/bin/lailaba || \
            (echo "FAIL: LAILABA_TUI_DIR not in wrapper"; exit 1)
          echo "PASS: LAILABA_TUI_DIR set in wrapper"
        ...
        # Verify LAILABA_NODE is set in wrapper and points to Node 20+
        # (string-width uses the /v regex flag which requires Node 20+)
        lailaba-node = pkgs.runCommand "lailaba-node-version" { } ''
          set -e
          echo "=== Checking LAILABA_NODE in wrapper ==="
          grep -q "LAILABA_NODE" ${lailaba-agent}/bin/lailaba || \
            (echo "FAIL: LAILABA_NODE not set in wrapper"; exit 1)
          echo "PASS: LAILABA_NODE present in wrapper"

          LAILABA_NODE=$(sed -n "s/^export LAILABA_NODE='\(.*\)'/\1/p" ${lailaba-agent}/bin/lailaba)
          test -x "$LAILABA_NODE" || (echo "FAIL: LAILABA_NODE=$LAILABA_NODE not executable"; exit 1)
          echo "PASS: LAILABA_NODE executable at $LAILABA_NODE"

          NODE_MAJOR=$("$LAILABA_NODE" --version | sed 's/^v//' | cut -d. -f1)
          test "$NODE_MAJOR" -ge 20 || \
            (echo "FAIL: Node v$NODE_MAJOR < 20, TUI needs /v regex flag support"; exit 1)
          echo "PASS: Node v$NODE_MAJOR >= 20"

          echo "=== All LAILABA_NODE checks passed ==="
          mkdir -p $out
          echo "ok" > $out/result
        '';

        # Verify LAILABA_MANAGED guard works on all mutation commands
        managed-guard = pkgs.runCommand "lailaba-managed-guard" { } ''
          set -e
          export HOME=$(mktemp -d)
          ...
          check_blocked() {
            local label="$1"
            shift
            OUTPUT=$(LAILABA_MANAGED=true "$@" 2>&1 || true)
          ...
          echo "=== Checking LAILABA_MANAGED guards ==="
          check_blocked "config set" ${lailaba-agent}/bin/lailaba config set model foo
          check_blocked "config edit" ${lailaba-agent}/bin/lailaba config edit
        ...
        extra-python-packages = let
          ...
          hermesWithExtra = lailaba-agent.override {
            extraPythonPackages = [ testPkg ];
          };
        in pkgs.runCommand "lailaba-extra-python-packages" { } ''
          set -e
          echo "=== Checking extraPythonPackages PYTHONPATH injection ==="

          grep -q "PYTHONPATH" ${hermesWithExtra}/bin/lailaba || \
            (echo "FAIL: PYTHONPATH not in wrapper"; exit 1)
          echo "PASS: PYTHONPATH present in wrapper"

          grep -q "${testPkg}" ${hermesWithExtra}/bin/lailaba || \
          ...
          if grep -q "PYTHONPATH" ${lailaba-agent}/bin/lailaba; then
        ...
        extra-dependency-groups = let
          hermesWithGroups = lailaba-agent.override {
            extraDependencyGroups = [ "honcho" ];
          };
        in pkgs.runCommand "lailaba-extra-dependency-groups" { } ''
          set -e
        ...
        messaging-variant = pkgs.runCommand "lailaba-messaging-variant" { } ''
          set -e
        ...
        in pkgs.runCommand "lailaba-config-roundtrip" {
          ...
          merge_and_load() {
            local lailaba_home="$1"
            export LAILABA_HOME="$lailaba_home"
            ${configMergeScript} ${nixSettings} "$lailaba_home/config.yaml"
            ${hermesVenv}/bin/python3 -c '
import json, sys
