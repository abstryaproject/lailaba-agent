# nix/nixosModules.nix — NixOS module for lailaba-agent
#
# Two modes:
#   container.enable = false (default) → native systemd service
#   container.enable = true            → OCI container (persistent writable layer)
#
# Container mode: lailaba runs from /nix/store bind-mounted read-only into a
...
# container recreation. Environment variables are written to $LAILABA_HOME/.env
# and read by lailaba at startup — no container recreation needed for env changes.
#
# Tool resolution: the lailaba wrapper uses --suffix PATH for nix store tools,
...
{ inputs, ... }: {
  ...
  let
    cfg = config.services.lailaba-agent;
    effectivePackage =
    ...
    lailaba-agent = inputs.self.packages.${pkgs.stdenv.hostPlatform.system}.default;

    # Deep-merge config type (from 0xrsydn/nix-lailaba-agent)
    deepConfigType = lib.types.mkOptionType {
      name = "lailaba-config-attrs";
      description = "Lailaba YAML config (attrset), merged deeply via lib.recursiveUpdate.";
    ...
    generatedConfigFile = pkgs.writeText "lailaba-config.yaml" configJson;
    configFile = if cfg.configFile != null then cfg.configFile else generatedConfigFile;

    configMergeScript = pkgs.callPackage ./configMergeScript.nix { };

    # config.yaml mode: group-writable (0660) when interactive users share this
    # LAILABA_HOME via addToSystemPackages, so they can save settings through the
    ...
    documentDerivation = pkgs.runCommand "lailaba-documents" { } (
      ''
        mkdir -p $out
      '' + lib.concatStringsSep "\n" (
        lib.mapAttrsToList (name: value:
          ...
          else "cat > $out/${name} <<'LAILABA_DOC_EOF'\n${value}\nHERMES_DOC_EOF"
        ) cfg.documents
      )
    );

    containerName = "lailaba-agent";
    containerDataDir = "/data";     # stateDir mount point inside container
    containerHomeDir = "/home/lailaba";
    ...
    # lailaba user + sudo on first boot (writable layer persists), then
    # drops privileges. Supports arbitrary base images (Debian, Alpine, etc).
    containerEntrypoint = pkgs.writeShellScript "lailaba-container-entrypoint" ''
      set -eu

      LAILABA_UID="''${LAILABA_UID:?LAILABA_UID must be set}"
      LAILABA_GID="''${LAILABA_GID:?LAILABA_GID must be set}"

      # ── Group: ensure a group with GID=$LAILABA_GID exists ──
      # Check by GID (not name) to avoid collisions with pre-existing groups
      # (e.g. GID 100 = "users" on Ubuntu)
      EXISTING_GROUP=$(getent group "$LAILABA_GID" 2>/dev/null | cut -d: -f1 || true)
      if [ -n "$EXISTING_GROUP" ]; then
        GROUP_NAME="$EXISTING_GROUP"
      else
        GROUP_NAME="lailaba"
        if command -v groupadd >/dev/null 2>&1; then
          groupadd -g "$LAILABA_GID" "$GROUP_NAME"
        elif command -v addgroup >/dev/null 2>&1; then
          addgroup -g "$LAILABA_GID" "$GROUP_NAME" 2>/dev/null || true
        fi
      fi

      # ── User: ensure a user with UID=$LAILABA_UID exists ──
      PASSWD_ENTRY=$(getent passwd "$LAILABA_UID" 2>/dev/null || true)
      if [ -n "$PASSWD_ENTRY" ]; then
        TARGET_USER=$(echo "$PASSWD_ENTRY" | cut -d: -f1)
        TARGET_HOME=$(echo "$PASSWD_ENTRY" | cut -d: -f6)
      else
        TARGET_USER="lailaba"
        TARGET_HOME="/home/lailaba"
        if command -v useradd >/dev/null 2>&1; then
          useradd -u "$LAILABA_UID" -g "$LAILABA_GID" -m -d "$TARGET_HOME" -s /bin/bash "$TARGET_USER"
        elif command -v adduser >/dev/null 2>&1; then
          adduser -u "$LAILABA_UID" -D -h "$TARGET_HOME" -s /bin/sh -G "$GROUP_NAME" "$TARGET_USER" 2>/dev/null || true
        fi
      fi
      mkdir -p "$TARGET_HOME"
      chown "$LAILABA_UID:$LAILABA_GID" "$TARGET_HOME"
      chmod 0750 "$TARGET_HOME"

      # Ensure LAILABA_HOME is owned by the target user.
      ...
      if [ -n "''${LAILABA_HOME:-}" ] && [ -d "$LAILABA_HOME" ]; then
        find "$LAILABA_HOME" \! -user "$LAILABA_UID" -exec chown "$LAILABA_UID:$LAILABA_GID" {} +
      fi
      ...
      if [ ! -f /var/lib/lailaba-tools-provisioned ] && command -v apt-get >/dev/null 2>&1; then
        ...
        touch /var/lib/lailaba-tools-provisioned
      fi

      if command -v sudo >/dev/null 2>&1 && [ ! -f /etc/sudoers.d/lailaba ]; then
        mkdir -p /etc/sudoers.d
        echo "$TARGET_USER ALL=(ALL) NOPASSWD:ALL" > /etc/sudoers.d/lailaba
        chmod 0440 /etc/sudoers.d/lailaba
      fi
      ...
      if command -v setpriv >/dev/null 2>&1; then
        exec setpriv --reuid="$LAILABA_UID" --regid="$LAILABA_GID" --init-groups "$@"
    ...
    # so they can update without recreation. Env vars go through $LAILABA_HOME/.env.
    ...
    # Default: /var/lib/lailaba/workspace → /data/workspace.
  ...
  in {
    options.services.lailaba-agent = with lib; {
      enable = mkEnableOption "Lailaba AI gateway service";

      # ── Package ──────────────────────────────────────────────────────────
      package = mkOption {
        type = types.package;
        default = lailaba-agent;
        description = "The lailaba-agent package to use.";
      };
      ...
      stateDir = mkOption {
        type = types.str;
        default = "/var/lib/lailaba";
        description = "State directory. Contains .lailaba/ subdir (LAILABA_HOME).";
      };
      ...
      settings = mkOption {
        ...
        default = { };
        description = ''
          Declarative Lailaba config (attrset). Deep-merged across module
      ...
      environmentFiles = mkOption {
        ...
        description = ''
          ...
          Contents are merged into $LAILABA_HOME/.env at activation time.
          Lailaba reads this file on every startup via load_lailaba_dotenv().
        '';
      };
      ...
      environment = mkOption {
        ...
        default = { };
        description = ''
          Non-secret environment variables. Merged into $LAILABA_HOME/.env
          ...
          options = {
            ...
            auth = mkOption {
              ...
              description = ''
                ...
                (remote MCP servers). Tokens are stored in $LAILABA_HOME/mcp-tokens/.
              '';
            };
      ...
      extraArgs = mkOption {
        type = types.listOf types.str;
        default = [ ];
        description = "Extra command-line arguments for `lailaba gateway`.";
      };
      ...
      extraPackages = mkOption {
        ...
        description = ''
          ...
          Implemented via the lailaba user's per-user profile
      ...
      extraPlugins = mkOption {
        ...
        default = [ ];
        description = ''
          Directory-based plugin packages to symlink into the lailaba plugins
          directory. Each package should contain a plugin.yaml and __init__.py
          at its root. Lailaba discovers these automatically on startup.
        '';
        example = literalExpression ''
          [
      ...
      extraPythonPackages = mkOption {
        ...
        description = ''
          ...
          lailaba_agent.plugins entry-point group. Each package must be built
          with the same Python interpreter as lailaba (python312).
        '';
        example = literalExpression ''
          [
      ...
      extraDependencyGroups = mkOption {
        ...
        description = ''
          ...
          Use this for optional extras already declared in lailaba-agent's
      ...
      addToSystemPackages = mkOption {
        ...
        default = false;
        description = ''
          Add the lailaba CLI to environment.systemPackages and export
          LAILABA_HOME system-wide (via environment.variables) so interactive
      ...
      container = {
        ...
        hostUsers = mkOption {
          ...
          default = [ ];
          description = ''
            Interactive users who get a ~/.lailaba symlink to the service
            stateDir. These users are automatically added to the lailaba group.
          '';
          example = [ "sidbin" ];
        };
      };
    ...
    config = lib.mkIf cfg.enable (lib.mkMerge [
      ...
      (lib.mkIf (cfg.mcpServers != { }) {
        services.lailaba-agent.settings.mcp_servers = lib.mapAttrs (_name: srv:
          # Stdio transport
      ...
      # ── Host CLI ──────────────────────────────────────────────────────
      # Add the lailaba CLI to system PATH and export LAILABA_HOME system-wide
      # so interactive shells share state (sessions, skills, cron) with the
      # gateway service instead of creating a separate ~/.lailaba/.
      (lib.mkIf cfg.addToSystemPackages {
        environment.systemPackages = [ effectivePackage ];
        environment.variables.LAILABA_HOME = "${cfg.stateDir}/.lailaba";
      })
      ...
      # ── Assertions ─────────────────────────────────────────────────────
      {
        ...
        in [{
          ...
          message = "services.lailaba-agent.extraPlugins: duplicate plugin names detected: ${toString names}. If using fetchFromGitHub, set name = \"plugin-name\" to disambiguate.";
        }];
      }
      ...
      # Wire extraPackages into the lailaba user's per-user profile so the
      ...
      (lib.mkIf (cfg.container.enable && !cfg.addToSystemPackages && cfg.container.hostUsers != []) {
        warnings = [
          ''
            services.lailaba-agent: container.enable is true and container.hostUsers
            is set, but addToSystemPackages is false. Without a host-installed lailaba
            binary, container routing will not work for interactive users.
            Set addToSystemPackages = true or ensure lailaba is on PATH.
          ''
        ];
      })
      ...
      # ── Directories ───────────────────────────────────────────────────
      {
        systemd.tmpfiles.rules = [
          ...
          "d ${cfg.stateDir}/.lailaba        2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.lailaba/cron   2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.lailaba/sessions 2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.lailaba/logs   2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.lailaba/memories 2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.lailaba/plugins 2770 ${cfg.user} ${cfg.group} - -"
      ...
      {
        system.activationScripts."lailaba-agent-setup" = lib.stringAfter ([ "users" ] ++ lib.optional (config.system.activationScripts ? setupSecrets) "setupSecrets") ''
          # Ensure directories exist (activation runs before tmpfiles)
          mkdir -p ${cfg.stateDir}/.lailaba
          mkdir -p ${cfg.stateDir}/home
          mkdir -p ${cfg.workingDirectory}
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir} ${cfg.stateDir}/.lailaba ${cfg.stateDir}/home ${cfg.workingDirectory}
          chmod 2770 ${cfg.stateDir} ${cfg.stateDir}/.lailaba ${cfg.workingDirectory}
          ...
          find ${cfg.stateDir}/.lailaba -maxdepth 1 \
            \( -name "*.db" -o -name "*.db-wal" -o -name "*.db-shm" -o -name "SOUL.md" \) \
            -exec chmod g+rw {} + 2>/dev/null || true
          for _subdir in cron sessions logs memories plugins; do
            mkdir -p "${cfg.stateDir}/.lailaba/$_subdir"
            chown ${cfg.user}:${cfg.group} "${cfg.stateDir}/.lailaba/$_subdir"
            chmod 2770 "${cfg.stateDir}/.lailaba/$_subdir"
            find "${cfg.stateDir}/.lailaba/$_subdir" -type f \
              -exec chmod g+rw {} + 2>/dev/null || true
          done
          ...
          # lailaba-group users can save settings via the CLI/TUI, else 0640).
          ${if cfg.configFile != null then ''
            install -o ${cfg.user} -g ${cfg.group} -m ${configYamlMode} -D ${configFile} ${cfg.stateDir}/.lailaba/config.yaml
          '' else ''
            ${configMergeScript} ${generatedConfigFile} ${cfg.stateDir}/.lailaba/config.yaml
            chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.lailaba/config.yaml
            chmod ${configYamlMode} ${cfg.stateDir}/.lailaba/config.yaml
          ''}

          # Managed mode marker (so interactive shells also detect NixOS management)
          touch ${cfg.stateDir}/.lailaba/.managed
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.lailaba/.managed
          chmod 0644 ${cfg.stateDir}/.lailaba/.managed
          ...
          ${if cfg.container.enable then ''
            cat > ${cfg.stateDir}/.lailaba/.container-mode <<'LAILABA_CONTAINER_MODE_EOF'
    # Written by NixOS activation script. Do not edit manually.
    backend=${cfg.container.backend}
    container_name=${containerName}
    exec_user=${cfg.user}
    lailaba_bin=${containerDataDir}/current-package/bin/lailaba
    LAILABA_CONTAINER_MODE_EOF
            chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.lailaba/.container-mode
            chmod 0644 ${cfg.stateDir}/.lailaba/.container-mode
          '' else ''
            rm -f ${cfg.stateDir}/.lailaba/.container-mode

            # Remove symlink bridge for hostUsers
            ${lib.concatStringsSep "\n" (map (user:
              let
                userHome = config.users.users.${user}.home;
                symlinkPath = "${userHome}/.lailaba";
              in ''
                if [ -L "${symlinkPath}" ] && [ "$(readlink "${symlinkPath}")" = "${cfg.stateDir}/.lailaba" ]; then
                  rm -f "${symlinkPath}"
                  echo "lailaba-agent: removed symlink ${symlinkPath}"
                fi
              '') cfg.container.hostUsers)}
          ''}

          # ── Symlink bridge for interactive users ───────────────────────
          # Create ~/.lailaba -> stateDir/.lailaba for each hostUser so the
          ...
          ${lib.optionalString cfg.container.enable
            (lib.concatStringsSep "\n" (map (user:
              let
                userHome = config.users.users.${user}.home;
                symlinkPath = "${userHome}/.lailaba";
                target = "${cfg.stateDir}/.lailaba";
              in ''
                if [ -d "${symlinkPath}" ] && [ ! -L "${symlinkPath}" ]; then
                  # Real directory — back it up, then create symlink.
                  # (ln -sfn cannot atomically replace a directory.)
                  _backup="${symlinkPath}.bak.$(date +%s)"
                  echo "lailaba-agent: backing up existing ${symlinkPath} to $_backup"
          ...
          ${lib.optionalString (cfg.authFile != null) ''
            ${if cfg.authFileForceOverwrite then ''
              install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.lailaba/auth.json
            '' else ''
              if [ ! -f ${cfg.stateDir}/.lailaba/auth.json ]; then
                install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.lailaba/auth.json
              fi
            ''}
          ''}
          ...
          # Lailaba reads $LAILABA_HOME/.env at startup via load_lailaba_dotenv(),
          # so this is the single source of truth for both native and container mode.
          ${lib.optionalString (cfg.environment != {} || cfg.environmentFiles != []) ''
            ENV_FILE="${cfg.stateDir}/.lailaba/.env"
            install -o ${cfg.user} -g ${cfg.group} -m 0640 /dev/null "$ENV_FILE"
            cat > "$ENV_FILE" <<'LAILABA_NIX_ENV_EOF'
    ${envFileContent}
    LAILABA_NIX_ENV_EOF
            ${lib.concatStringsSep "\n" (map (f: ''
              if [ -f "${f}" ]; then
        ...
        find ${cfg.stateDir}/.lailaba/plugins -maxdepth 1 -type l -name 'nix-managed-*' -delete 2>/dev/null || true
        ...
        ${lib.concatStringsSep "\n" (map (plugin:
          ...
          in ''
            ...
            ln -sfn ${plugin} ${cfg.stateDir}/.lailaba/plugins/nix-managed-${name}
            chown -h ${cfg.user}:${cfg.group} ${cfg.stateDir}/.lailaba/plugins/nix-managed-${name}
          '') cfg.extraPlugins)}
        '';
      }
      ...
      (lib.mkIf (!cfg.container.enable) {
        systemd.services.lailaba-agent = {
          description = "Lailaba AI Gateway";
          ...
          environment = {
            HOME = cfg.stateDir;
            LAILABA_HOME = "${cfg.stateDir}/.lailaba";
            LAILABA_MANAGED = "true";
          ...
          serviceConfig = {
            ...
            # $LAILABA_HOME/.env by the activation script. load_lailaba_dotenv()
            # reads them at Python startup — no systemd EnvironmentFile needed.

            ExecStart = lib.concatStringsSep " " ([
              "${effectivePackage}/bin/lailaba"
              "gateway"
            ] ++ cfg.extraArgs);
            ...
            # so interactive users in the lailaba group can read/write them.
            UMask = "0007";
      ...
      (lib.mkIf cfg.container.enable {
        ...
        systemd.services.lailaba-agent = {
          description = "Lailaba AI Gateway (container)";
          wantedBy = [ "multi-user.target" ];
          after = [ "network-online.target" ]
          ...
          preStart = ''
            ...
            if [ "$NEED_CREATE" = "true" ]; then
              ...
              LAILABA_UID=$(${pkgs.coreutils}/bin/id -u ${cfg.user})
              LAILABA_GID=$(${pkgs.coreutils}/bin/id -g ${cfg.user})
              ...
              ${containerBin} create \
                ...
                --env LAILABA_UID="$LAILABA_UID" \
                --env LAILABA_GID="$LAILABA_GID" \
                --env LAILABA_HOME=${containerDataDir}/.lailaba \
                --env LAILABA_MANAGED=true \
                --env HOME=${containerHomeDir} \
                --env MESSAGING_CWD=${containerWorkDir} \
                ${lib.concatStringsSep " " cfg.container.extraOptions} \
                ${cfg.container.image} \
                ${containerDataDir}/current-package/bin/lailaba gateway run --replace ${lib.concatStringsSep " " cfg.extraArgs}
