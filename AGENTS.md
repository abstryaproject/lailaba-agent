# Lailaba AI - Development Guide

Instructions for AI coding assistants and developers working on the lailaba-agent codebase.

**Never give up on the right solution.**

## What Lailaba Is

Lailaba is a personal AI agent that runs the same agent core across a CLI, a
...
Read the balance right: Lailaba ships a **lot** — most merges are bug fixes to
...
- **Expand reach at the edges.** New platform adapters, channels, providers,
  ...
  (`lailaba tools`, `lailaba setup`, auto-install) rather than bolting on a raw
  env var.
...
- **E2E validation, not just green unit mocks.** For anything touching
  ...
  against a temp `LAILABA_HOME`. Mocks hide integration bugs.
...
- **New `LAILABA_*` env vars for non-secret config.** `.env` is for secrets
...
- **Outbound telemetry / usage attribution without opt-in gating.** No new
  ...
  generic user-facing opt-in (config gate + setup prompt + `lailaba tools`
...
- **Third-party products / other people's projects integrated into the core
  ...
  **standalone plugin repo** users install into `~/.lailaba/plugins/` (or via a
...
2. **CLI command + skill** — manages config/state/infra expressible as shell
   commands. The agent runs `lailaba <subcommand>` guided by a skill. Zero
   model-tool footprint. Default choice for subscriptions, scheduled tasks,
   service setup. Examples: `lailaba webhook`, `lailaba cron`, `lailaba tools`.
...
4. **Plugin** — third-party/niche/user-specific capability that doesn't ship in
   core. Lives in `~/.lailaba/plugins/` or a pip package, discovered at runtime.
...
`$HOME/.lailaba/lailaba-agent/venv` (for worktrees that share a venv with the
main checkout).
...
├── toolsets.py           # Toolset definitions, _LAILABA_CORE_TOOLS list
├── cli.py                # HermesCLI class — interactive CLI orchestrator (~11k LOC)
├── lailaba_state.py       # SessionDB — SQLite session store (FTS5 search)
├── lailaba_constants.py   # get_lailaba_home(), display_lailaba_home() — profile-aware paths
├── lailaba_logging.py     # setup_logging() — agent.log / errors.log / gateway.log (profile-aware)
├── batch_runner.py       # Parallel batch processing
├── agent/                # Agent internals (provider adapters, memory, caching, compression, etc.)
├── lailaba_cli/           # CLI subcommands, setup wizard, plugins loader, skin engine
...
│   ├── lailaba-achievements/  # Gamified achievement tracking
...
├── ui-tui/               # Ink (React) terminal UI — `lailaba --tui`
...
**User config:** `~/.lailaba/config.yaml` (settings), `~/.lailaba/.env` (API keys only).
**Logs:** `~/.lailaba/logs/` — `agent.log` (INFO+), `errors.log` (WARNING+),
`gateway.log` when running the gateway. Profile-aware via `get_lailaba_home()`.
Browse with `lailaba logs [--follow] [--level ...] [--session ...]`.

## TypeScript Style

Applies to TypeScript across Lailaba: desktop, TUI, website, and future TS packages.
...
- **Skin engine** (`lailaba_cli/skin_engine.py`) — data-driven CLI theming; initialized from `display.skin` config key at startup; skins customize banner colors, spinner faces/verbs/wings, tool prefix, response box, branding text
- `process_command()` is a method on `HermesCLI` — dispatches on canonical command name resolved via `resolve_command()` from the central registry
- Skill slash commands: `agent/skill_commands.py` scans `~/.lailaba/skills/`, injects as **user message** (not system prompt) to preserve prompt caching

### Slash Command Registry (`lailaba_cli/commands.py`)
...
- **Slack** — `slack_subcommand_map()` generates `/lailaba` subcommand routing
- **Autocomplete** — `COMMANDS` flat dict feeds `SlashCommandCompleter`
- **CLI help** — `COMMANDS_BY_CATEGORY` dict feeds `show_help()`

### Adding a Slash Command

1. Add a `CommandDef` entry to `COMMAND_REGISTRY` in `lailaba_cli/commands.py`:
```python
...
The TUI is a full replacement for the classic (prompt_toolkit) CLI, activated via `lailaba --tui` or `LAILABA_TUI=1`.
...
npm run dev       # watch mode (rebuilds lailaba-ink + tsx --watch)
npm start         # production
npm run build     # full build (lailaba-ink + tsc)
...
### TUI in the Dashboard (`lailaba dashboard` → `/chat`)

The dashboard embeds the real `lailaba --tui` — **not** a rewrite.  See `lailaba_cli/pty_bridge.py` + the `@app.websocket("/api/pty")` endpoint in `lailaba_cli/web_server.py`.

- Browser loads `web/src/pages/ChatPage.tsx`, which mounts xterm.js's `Terminal` with the WebGL renderer, `@xterm/addon-fit` for container-driven resize, and `@xterm/addon-unicode11` for modern wide-character widths.
- `/api/pty?token=…` upgrades to a WebSocket; auth uses the same ephemeral `_SESSION_TOKEN` as REST, via query param (browsers can't set `Authorization` on WS upgrade).
- The server spawns whatever `lailaba --tui` would spawn, through `ptyprocess` (POSIX PTY — WSL works, native Windows does not).
- Frames: raw PTY bytes each direction; resize via `\x1b[RESIZE:<cols>;<rows>]` intercepted on the server and applied with `TIOCSWINSZ`.

**Do not re-implement the primary chat experience in React.** The main transcript, composer/input flow (including slash-command behavior), and PTY-backed terminal belong to the embedded `lailaba --tui` — anything new you add to Ink shows up in the dashboard automatically. If you find yourself rebuilding the transcript or composer for the dashboard, stop and extend Ink instead.

**Structured React UI around the TUI is allowed when it is not a second chat surface.** Sidebar widgets, inspectors, summaries, status panels, and similar supporting views (e.g. `ChatSidebar`, `ModelPickerDialog`, `ToolCall`) are fine when they complement the embedded TUI rather than replacing the transcript / composer / terminal. Keep their state independent of the PTY child's session and surface their failures non-destructively so the terminal pane keeps working unimpaired.

### Electron Desktop Chat App (`apps/desktop/`)

A **separate** chat surface from both the classic CLI and the dashboard's embedded TUI. It is an Electron + React + nanostore renderer (`@assistant-ui/react`) that talks to a `tui_gateway` backend over JSON-RPC (`requestGateway(method, params)`). It does NOT embed `lailaba --tui` — it has its own composer, transcript, and slash-command pipeline. Route desktop bugs to the `lailaba-desktop-app-work` skill, not `lailaba-dashboard-work`.
...
- **The renderer curates via `apps/desktop/src/lib/desktop-slash-commands.ts`.** This is the load-bearing file. It holds `DESKTOP_COMMANDS` (the ~19 built-ins shown in the palette) plus block-lists for terminal-only / messaging-only / picker-owned / settings-owned / advanced commands that should NOT clutter the desktop popover.
  ...
  - `isDesktopSlashExtensionCommand(name)` — true when the command is NOT a known Lailaba built-in (i.e. a skill or user quick command). Both suggestion and catalog-filter paths allow extensions through so skill commands surface in the palette. (Added when fixing "skill commands missing from the desktop slash palette" — the curated allow-list was silently dropping every skill/quick command from completions even though they executed fine when typed.)
...
be core tools. For custom or local-only tools, do **not** edit Lailaba core.
Use the plugin route instead: create `~/.lailaba/plugins/<name>/plugin.yaml`
and `~/.lailaba/plugins/<name>/__init__.py`, then register tools with
`ctx.register_tool(...)`. Plugin toolsets are discovered automatically and can be
enabled or disabled without touching `tools/` or `toolsets.py`.

Use the built-in route below only when the user is explicitly contributing a new
core Lailaba tool that should ship in the base system.
...
**2. Add to `toolsets.py`** — either `_LAILABA_CORE_TOOLS` (all platforms) or a new toolset. **This step is required:** auto-discovery imports the tool and registers its schema, but the tool is only *exposed to an agent* if its name appears in a toolset. `_LAILABA_CORE_TOOLS` is not dead code — it's the default bundle every platform's base toolset inherits from.

Auto-discovery: any `tools/*.py` file with a top-level `registry.register()` call is imported automatically — no manual import list to maintain. Wiring into a toolset is still a deliberate, manual step.

The registry handles schema collection, dispatch, availability checking, and error wrapping. All handlers MUST return a JSON string.

**Path references in tool schemas**: If the schema description mentions file paths (e.g. default output directories), use `display_lailaba_home()` to make them profile-aware. The schema is generated at import time, which is after `_apply_profile_override()` sets `LAILABA_HOME`.

**State files**: If a tool stores persistent state (caches, logs, checkpoints), use `get_lailaba_home()` for the base directory — never `Path.home() / ".lailaba"`. This ensures each profile gets its own state.
...
### config.yaml options:
1. Add to `DEFAULT_CONFIG` in `lailaba_cli/config.py`
...
1. Add to `OPTIONAL_ENV_VARS` in `lailaba_cli/config.py` with metadata:
```python
"NEW_API_KEY": {
...
| `load_config()` | `lailaba tools`, `lailaba setup`, most CLI subcommands | `lailaba_cli/config.py` — merges `DEFAULT_CONFIG` + user YAML |
...
The skin engine (`lailaba_cli/skin_engine.py`) provides data-driven CLI visual customization. Skins are **pure data** — no code changes needed to add a new skin.

### Architecture

```
lailaba_cli/skin_engine.py    # SkinConfig dataclass, built-in skins, YAML loader
~/.lailaba/skins/*.yaml       # User-installed custom skins (drop-in)
```
...
- `default` — Classic Lailaba gold/kawaii (the current look)
...
Add to `_BUILTIN_SKINS` dict in `lailaba_cli/skin_engine.py`:
...
Users create `~/.lailaba/skins/<name>.yaml`:
...
Lailaba has two plugin surfaces. Both live under `plugins/` in the repo so
repo-shipped plugins can be discovered alongside user-installed ones in
`~/.lailaba/plugins/` and pip-installed entry points.

### General plugins (`lailaba_cli/plugins.py` + `plugins/<name>/`)

`PluginManager` discovers plugins from `~/.lailaba/plugins/`, `./.lailaba/plugins/`,
...
- Register CLI subcommands via `ctx.register_cli_command(...)` — the
  plugin's argparse tree is wired into `lailaba` at startup so
  `lailaba <pluginname> <subcmd>` works with no change to `main.py`
...
`post_setup(lailaba_home, config)` for setup-wizard integration.

**CLI commands via `plugins/memory/<name>/cli.py`:** if a memory plugin
defines `register_cli(subparser)`, `discover_plugin_cli_commands()` finds
it at argparse setup time and wires it into `lailaba <plugin>`. The
framework only exposes CLI commands for the **currently active** memory
provider (read from `memory.provider` in config.yaml), so disabled
providers don't clutter `lailaba --help`.

**Rule (Teknium, May 2026):** plugins MUST NOT modify core files
(`run_agent.py`, `cli.py`, `gateway/run.py`, `lailaba_cli/main.py`, etc.).
...
into `~/.lailaba/plugins/` (or via pip entry points) — they implement
the same `MemoryProvider` ABC, register through the same discovery
path, and integrate via `lailaba memory setup` / `post_setup()` without
...
`~/.lailaba/plugins/` (or via pip entry points). They register through
...
2. User: `$LAILABA_HOME/plugins/model-providers/<name>/`
...
[`lailaba-example-plugins`](https://github.com/NousResearch/lailaba-example-plugins)
...
- **`optional-skills/`** — heavier or niche skills shipped with the repo but
  ...
  `lailaba skills install official/<category>/<skill>`. Adapter lives in
...
`metadata.lailaba.tags`, `metadata.lailaba.category`,
`metadata.lailaba.related_skills`, `metadata.lailaba.config` (config.yaml
settings the skill needs — stored under `skills.config.<key>`, prompted
during setup, injected at load time).

Top-level `tags:` and `category:` are also accepted and mirrored from
`metadata.lailaba.*` by the loader.
...
2. **Tools referenced in SKILL.md prose must be native Lailaba tools or
...
4. **`author` credits the human contributor first.** For external
   ...
   first; "Lailaba AI" is the secondary collaborator. If the
   contributor's commit shows "Lailaba AI" as author (because they
   used Lailaba to draft the skill), replace it with their actual name
...
lives in the `lailaba-agent-dev` skill at
...
`"messaging"`); `_LAILABA_CORE_TOOLS` is the default bundle most
...
Enable/disable per platform via `lailaba tools` (the curses UI) or the
...
Lailaba returns a delegation id immediately and the result re-enters the
...
go to `~/.lailaba/skills/.archive/` and are restorable.

- **Core:** `agent/curator.py` (review loop, auto-transitions, LLM review
  prompt) + `agent/curator_backup.py` (pre-run tar.gz snapshots).
- **CLI:** `lailaba_cli/curator.py` wires `lailaba curator <verb>` where
  verbs are: `status`, `run`, `pause`, `resume`, `pin`, `unpin`,
  `archive`, `restore`, `prune`, `backup`, `rollback`.
- **Telemetry:** `tools/skill_usage.py` owns the sidecar
  `~/.lailaba/skills/.usage.json` — per-skill `use_count`, `view_count`,
...
schedule jobs via the `cronjob` tool; users via `lailaba cron <verb>`
...
- File lock at `~/.lailaba/cron/.tick.lock` prevents duplicate ticks
  across processes.
...
collaborate on shared tasks. Users drive it via `lailaba kanban <verb>`;
workers spawned by the dispatcher drive it via a dedicated `kanban_*`
toolset so their schema footprint is zero when they're not inside a
kanban task.

- **CLI:** `lailaba_cli/kanban.py` wires `lailaba kanban` with verbs
...
- **Plugin assets:** `plugins/kanban/dashboard/` (web UI) +
  `plugins/kanban/systemd/` (`lailaba-kanban-dispatcher.service` for
  standalone dispatcher deployment).

Isolation model:
- **Board** is the hard boundary — workers are spawned with
  `LAILABA_KANBAN_BOARD` pinned in their env so they can't see other
  boards.
...
Lailaba-Agent ensures caching remains valid throughout a conversation. **Do NOT implement changes that would:**
...
in config.yaml (or `LAILABA_BACKGROUND_NOTIFICATIONS` env var):
...
Lailaba supports **profiles** — multiple fully isolated instances, each with its own
`LAILABA_HOME` directory (config, API keys, memory, sessions, skills, gateway, etc.).

The core mechanism: `_apply_profile_override()` in `lailaba_cli/main.py` sets
`LAILABA_HOME` before any module imports. All `get_lailaba_home()` references
automatically scope to the active profile.

### Rules for profile-safe code

1. **Use `get_lailaba_home()` for all LAILABA_HOME paths.** Import from `lailaba_constants`.
   NEVER hardcode `~/.lailaba` or `Path.home() / ".lailaba"` in code that reads/writes state.
   ```python
   # GOOD
   from lailaba_constants import get_lailaba_home
   config_path = get_lailaba_home() / "config.yaml"

   # BAD — breaks profiles
   config_path = Path.home() / ".lailaba" / "config.yaml"
   ```

2. **Use `display_lailaba_home()` for user-facing messages.** Import from `lailaba_constants`.
   This returns `~/.lailaba` for default or `~/.lailaba/profiles/<name>` for profiles.
   ```python
   # GOOD
   from lailaba_constants import display_lailaba_home
   print(f"Config saved to {display_lailaba_home()}/config.yaml")

   # BAD — shows wrong path for profiles
   print("Config saved to ~/.lailaba/config.yaml")
   ```

3. **Module-level constants are fine** — they cache `get_lailaba_home()` at import time,
   which is AFTER `_apply_profile_override()` sets the env var. Just use `get_lailaba_home()`,
   not `Path.home() / ".lailaba"`.

4. **Tests that mock `Path.home()` must also set `LAILABA_HOME`** — since code now uses
   `get_lailaba_home()` (reads env var), not `Path.home() / ".lailaba"`:
   ```python
   with patch.object(Path, "home", return_value=tmp_path), \
        patch.dict(os.environ, {"LAILABA_HOME": str(tmp_path / ".lailaba")}):
       ...
   ```
...
6. **Profile operations are HOME-anchored, not LAILABA_HOME-anchored** — `_get_profiles_root()`
   returns `Path.home() / ".lailaba" / "profiles"`, NOT `get_lailaba_home() / "profiles"`.
   This is intentional — it lets `lailaba -p coder profile list` see all profiles regardless
   of which one is active.

## Known Pitfalls

### DO NOT hardcode `~/.lailaba` paths
Use `get_lailaba_home()` from `lailaba_constants` for code paths. Use `display_lailaba_home()`
for user-facing print/log messages. Hardcoding `~/.lailaba` breaks profiles — each profile
has its own `LAILABA_HOME` directory. This was the source of 5 bugs fixed in PR #3575.

### DO NOT introduce new `simple_term_menu` usage
Existing call sites in `lailaba_cli/main.py` remain for legacy fallback only;
the preferred UI is curses (stdlib) because `simple_term_menu` has
ghost-duplication rendering bugs in tmux/iTerm2 with arrow keys. New
interactive menus must use `lailaba_cli/curses_ui.py` — see
`lailaba_cli/tools_config.py` for the canonical pattern.
...
with actual imports (not mocks) against a temp `LAILABA_HOME`.

### Tests must not write to `~/.lailaba/`
The `_isolate_lailaba_home` autouse fixture in `tests/conftest.py` redirects `LAILABA_HOME` to a temp dir. Never hardcode `~/.lailaba/` paths in tests.

**Profile tests**: When testing profile features, also mock `Path.home()` so that
`_get_profiles_root()` and `_get_default_lailaba_home()` resolve within the temp dir.
Use the pattern from `tests/lailaba_cli/test_profiles.py`:
```python
@pytest.fixture
def profile_env(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("LAILABA_HOME", str(home))
    return home
```
...
- The plugin disables itself in child processes (sentinel envvar
  `LAILABA_ISOLATE_CHILD=1`), so there's no fork-bomb risk.
...
| HOME / `~/.lailaba/` | Your real config+auth.json | Temp dir per test |
