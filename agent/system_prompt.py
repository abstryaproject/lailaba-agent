...
upstream prefix cache warm.  See ``lailaba-agent-dev``'s
...
def build_system_prompt_parts(agent: Any, system_message: Optional[str] = None) -> Dict[str, str]:
    ...
    AIAgent.  Lailaba never re-renders parts of this string mid-
    ...
    # Some execution modes (cron) still want LAILABA_HOME persona while keeping
    ...
    # Pointer to the lailaba-agent skill + docs for user questions about Lailaba itself.
    stable_parts.append(LAILABA_AGENT_HELP_GUIDANCE)
    ...
    # LAILABA_KANBAN_TASK env var). Normal chat sessions never see
    ...
    # Coding posture (base Lailaba, any interactive coding surface in a code
    ...
    # Active-profile hint — names the Lailaba profile the agent is running
    # under so it doesn't conflate ~/.lailaba/skills/ (default profile) with
    # ~/.lailaba/profiles/<active>/skills/ (this profile's). Deterministic
    ...
    if active_profile == "default":
        stable_parts.append(
            "Active Lailaba profile: default. Other profiles (if any) live "
            "under ~/.lailaba/profiles/<name>/. Each profile has its own "
        ...
        stable_parts.append(
            f"Active Lailaba profile: {active_profile}. This session reads "
            f"and writes ~/.lailaba/profiles/{active_profile}/. The default "
            f"profile's data lives at ~/.lailaba/skills/, ~/.lailaba/plugins/, "
            f"~/.lailaba/cron/, ~/.lailaba/memories/ — those belong to a "
...
def build_system_prompt(agent: Any, system_message: Optional[str] = None) -> str:
    ...
    one cached block — Lailaba never rebuilds or reinjects parts of it
