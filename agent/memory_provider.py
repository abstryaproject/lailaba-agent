...
Optional hooks (override to opt in):
  ...
  backup_paths() -> list[str]            — extra on-disk paths to include in `lailaba backup`
"""
...
class MemoryProvider(ABC):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        kwargs always include:
          - lailaba_home (str): The active LAILABA_HOME directory path. Use this
            for profile-scoped storage instead of hardcoding ``~/.lailaba``.
        ...
        kwargs may also include:
          ...
          - agent_workspace (str): Shared workspace name (e.g. "lailaba").
    ...
    def get_config_schema(self) -> List[Dict[str, Any]]:
        ...
        Used by 'lailaba memory setup' to walk the user through configuration.
    ...
    def save_config(self, values: Dict[str, Any], lailaba_home: str) -> None:
        """Write non-secret config to the provider's native location.

        Called by 'lailaba memory setup' after collecting user inputs.
        ``values`` contains only non-secret fields (secrets go to .env).
        ``lailaba_home`` is the active LAILABA_HOME directory path.
    ...
    def backup_paths(self) -> List[str]:
        """Return extra on-disk paths this provider stores OUTSIDE LAILABA_HOME.

        ``lailaba backup`` only walks LAILABA_HOME, so any provider state kept
        ...
        of the archive, and ``lailaba import`` restores them to their original
