...
Plugins built on Lailaba AI often need to make their own LLM calls
...
Today the only stable plugin surfaces extend an existing Lailaba
...
:class:`~lailaba_cli.plugins.PluginContext`:
...
embedded slugs, no shorthands. This mirrors Lailaba' main config
...
handles every provider, fallback chain, and per-task override Lailaba
supports.
"""
...
class PluginLlm:
    ...
    Instances are constructed by :class:`lailaba_cli.plugins.PluginContext`
