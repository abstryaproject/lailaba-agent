...
``auth.json``.  They intentionally have no dependency on ``lailaba_cli.auth`` so
...
# Sources Lailaba owns and can intentionally persist in auth.json.  Everything
...
def sanitize_borrowed_credential_payload(
    ...
    Owned sources (manual entries and Lailaba-owned OAuth/device-code state)
