...
from lailaba_constants import get_lailaba_home, get_skills_dir, is_wsl
...
_LAILABA_MD_NAMES = (".lailaba.md", "LAILABA.md")


def _find_lailaba_md(cwd: Path) -> Optional[Path]:
    """Discover the nearest ``.lailaba.md`` or ``LAILABA.md``.
...
DEFAULT_AGENT_IDENTITY = (
    "You are Lailaba AI, an intelligent AI assistant created by Nous Research. "
...
LAILABA_AGENT_HELP_GUIDANCE = (
    "You run on Lailaba AI (by Nous Research). When the user needs help with "
    "Lailaba itself — configuring, setting up, using, extending, or troubleshooting "
    "it — or when you need to understand your own features, tools, or capabilities, "
    "the documentation at https://lailaba-agent.nousresearch.com/docs is your "
    "authoritative reference and always holds the latest, most up-to-date "
    "information. Load the `lailaba-agent` skill with skill_view(name='lailaba-agent') "
...
KANBAN_GUIDANCE = (
    ...
    "the shared board at `~/.lailaba/kanban.db`. Your task id is in "
    "`$LAILABA_KANBAN_TASK`; your workspace is `$LAILABA_KANBAN_WORKSPACE`. "
    ...
    "ground truth.\n"
    "2. **Work inside the workspace.** `cd $LAILABA_KANBAN_WORKSPACE` before "
    ...
    "\n"
    "- **Workspace.** `cd $LAILABA_KANBAN_WORKSPACE` first. For a `worktree` kind "
    "with no `.git`, `git worktree add <path> "
    "${LAILABA_KANBAN_BRANCH:-wt/$LAILABA_KANBAN_TASK}` from the main repo, then "
    "cd there. For a project-linked task the workspace is a fresh "
    "`<repo>/.worktrees/<task-id>` and `$LAILABA_KANBAN_BRANCH` a deterministic "
    ...
    "every assignee in a real profile (`lailaba profile list`, or ask the user), "
    "and express dependencies via `parents=[...]` on `kanban_create`, not prose.\n"
    "\n"
    "## Do NOT\n"
    "\n"
    "- Do not shell out to `lailaba kanban <verb>` for board operations. Use "
...
#
# The lailaba-agent runtime already executes a batch of tool calls
...
# from Cline's TypeScript tool-surface guidance to lailaba-agent's Python
...
def computer_use_guidance(platform_name: Optional[str] = None) -> str:
    ...
    return (
        ...
        "run `lailaba computer-use doctor` and share the output. That command "
...
STEER_CHANNEL_NOTE = (
    ...
    "While you work, the user can send an out-of-band message that Lailaba "
...
PLATFORM_HINTS = {
    ...
    "tui": (
        "You are running in the Lailaba terminal UI (TUI). "
        "Cron jobs scheduled from this session are LOCAL-ONLY: their output is "
        "saved (viewable via cronjob action='list') but is NOT delivered back "
    ...
    "webui": (
        "You are in the Lailaba WebUI, a browser-based chat interface. "
...
# container / remote host rather than on the machine where Lailaba itself
...
def _probe_remote_backend(env_type: str) -> str | None:
    ...
    operate on a different machine than the host Lailaba runs on.
    """
...
def build_environment_hints() -> str:
    ...
    if not is_remote_backend:
        ...
        if probe:
            hints.append(
                ...
                f"where Lailaba itself is running. The host OS, home, and cwd "
                f"of the Lailaba process are irrelevant; only the following "
            ...
            hints.append(
                f"Terminal backend: {backend}. Your `terminal`, `read_file`, "
                f"`write_file`, `patch`, and `search_files` tools all operate "
                f"inside {description} — NOT on the machine where Lailaba "
    ...
    # Lailaba desktop GUI — any agent running under the desktop app should know
    # it. LAILABA_DESKTOP marks the backend powering the chat; LAILABA_DESKTOP_TERMINAL
    # marks a lailaba launched in the embedded terminal pane. Both set by main.cjs.
    _truthy = ("1", "true", "yes")
    _in_desktop = (os.getenv("LAILABA_DESKTOP") or "").strip().lower() in _truthy
    _in_desktop_term = (os.getenv("LAILABA_DESKTOP_TERMINAL") or "").strip().lower() in _truthy
    if _in_desktop or _in_desktop_term:
        _desktop_hint = "Runtime surface: you're running inside the Lailaba desktop GUI app."
        if _in_desktop_term:
            _desktop_hint += (
    ...
    # Embedder-supplied environment description. Lets a host that wraps Lailaba
    ...
    extra = (os.getenv("LAILABA_ENVIRONMENT_HINT") or "").strip()
    if not extra:
        try:
...
def _skills_prompt_snapshot_path() -> Path:
    return get_lailaba_home() / ".skills_prompt_snapshot.json"
...
def build_skills_system_prompt(
    ...
    scanned alongside the local ``~/.lailaba/skills/`` directory.  External dirs
    ...
    _platform_hint = (
        os.environ.get("LAILABA_PLATFORM")
        or get_session_env("LAILABA_SESSION_PLATFORM")
        or ""
    )
    ...
    if not skills_by_category:
        ...
        result = (
            ...
            "or troubleshoot Lailaba AI itself — its CLI, config, models, providers, tools, "
            "skills, voice, gateway, plugins, or any feature — load the `lailaba-agent` skill "
            "first. It has the actual commands (e.g. `lailaba config set …`, `lailaba tools`, "
            "`lailaba setup`) so you don't have to guess or invent workarounds.\n"
...
def build_nous_subscription_prompt(valid_tool_names: "set[str] | None" = None) -> str:
    ...
    lines.extend(
        [
            ...
            "Useful commands: lailaba setup, lailaba setup tools, lailaba setup terminal, lailaba status.",
        ]
    )
    return "\n".join(lines)
...
def load_soul_md(context_length: Optional[int] = None) -> Optional[str]:
    """Load SOUL.md from LAILABA_HOME and return its content, or None.
    ...
    """
    try:
        from lailaba_cli.config import ensure_lailaba_home
        ensure_lailaba_home()
    except Exception as e:
        logger.debug("Could not ensure LAILABA_HOME before loading SOUL.md: %s", e)

    soul_path = get_lailaba_home() / "SOUL.md"
...
def _load_lailaba_md(cwd_path: Path, context_length: Optional[int] = None) -> str:
    """.lailaba.md / LAILABA.md — walk to git root."""
    lailaba_md_path = _find_lailaba_md(cwd_path)
    if not lailaba_md_path:
        return ""
    try:
        content = lailaba_md_path.read_text(encoding="utf-8").strip()
        if not content:
            return ""
        content = _strip_yaml_frontmatter(content)
        rel = lailaba_md_path.name
        try:
            rel = str(lailaba_md_path.relative_to(cwd_path))
        except ValueError:
            pass
        content = _scan_context_content(content, rel)
        result = f"## {rel}\n\n{content}"
        return _truncate_content(
            result, ".lailaba.md", context_length=context_length,
            read_path=str(lailaba_md_path),
        )
    except Exception as e:
        logger.debug("Could not read %s: %s", lailaba_md_path, e)
        return ""
...
def build_context_files_prompt(
    ...
    Priority (first found wins — only ONE project context type is loaded):
      1. .lailaba.md / LAILABA.md  (walk to git root)
      2. AGENTS.md / agents.md   (cwd only)
      3. CLAUDE.md / claude.md   (cwd only)
      4. .cursorrules / .cursor/rules/*.mdc  (cwd only)

    SOUL.md from LAILABA_HOME is independent and always included when present.
    ...
    project_context = (
        _load_lailaba_md(cwd_path, context_length)
    ...
    # SOUL.md from LAILABA_HOME only — skip when already loaded as identity
    if not skip_soul:
