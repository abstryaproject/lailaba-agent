"""Projects codex app-server events into Lailaba' messages list.

The translator that lets Lailaba' memory/skill review keep working under the
...
Lailaba' message-alternation invariants (system → user → assistant → user/tool
→ assistant → ...). Multiple Codex tool calls within one Codex turn produce
multiple consecutive (assistant, tool) pairs, which is the same shape Lailaba
...
def _format_tool_args(d: dict) -> str:
    """Format a dict as JSON the way Lailaba' existing tool_calls path does."""
...
class CodexEventProjector:
    ...
    items but Lailaba stashes it on the next assistant message)."""
    ...
    def project(self, notification: dict) -> ProjectionResult:
        ...
        # don't enter the messages list — same way Lailaba already only writes
    ...
    def _project_user_message(self, item: dict) -> ProjectionResult:
        ...
        # non-text parts (images, etc.) — Lailaba' messages store text only.
        text_parts: list[str] = []
