...
Owns one Codex thread per Lailaba session. Drives `turn/start`, consumes
...
# Lailaba' tools.terminal.security_mode → Codex's permissions profile id.
# Defaults if config is missing → workspace-write (matches Codex's own default).
_LAILABA_TO_CODEX_PERMISSION_PROFILE = {
...
def _coerce_turn_input_text(user_input: Any) -> str:
    """Collapse Lailaba/OpenAI rich content into app-server text input.
...
class CodexAppServerSession:
    """One Codex thread per Lailaba session, lifetime owned by AIAgent.
    ...
    def __init__(
        ...
        *,
        ...
        self._permission_profile = (
            permission_profile or _LAILABA_TO_CODEX_PERMISSION_PROFILE.get(
                os.environ.get("LAILABA_TERMINAL_SECURITY_MODE", "auto"),
                "workspace-write",
            )
        )
    ...
    def ensure_started(self) -> str:
        ...
        self._client.initialize(
            client_name="lailaba",
            client_title="Lailaba AI",
            client_version=_get_lailaba_version(),
        )
    ...
    def run_turn(
        ...
        *,
        ...
        # supports rich content but Lailaba' text path is the common case).
        try:
    ...
    def _handle_server_request(self, req: dict) -> None:
        """Translate a codex server request (approval) into Lailaba' approval
        ...
        if method == "item/commandExecution/requestApproval":
            ...
            # OAuth, form data). For our own lailaba-tools callback we
            # auto-accept — the user already approved Lailaba' tools
...
def _approval_choice_to_codex_decision(choice: str) -> str:
    """Map Lailaba approval choices onto codex's CommandExecutionApprovalDecision
    / FileChangeApprovalDecision wire values.

    Lailaba returns 'once', 'session', 'always', or 'deny'.
...
def _get_lailaba_version() -> str:
    """Best-effort Lailaba version string for codex's userAgent line."""
    try:
