...
def openclaw_residue_hint_cli() -> str:
    """Banner shown the first time Lailaba starts and finds ``~/.openclaw/``.

    Points users at ``lailaba claw migrate`` (non-destructive port of config,
    memory, and skills) first. ``lailaba claw cleanup`` is mentioned as the
    ...
    return (
        ...
        "To port your config, memory, and skills over to Lailaba, run "
        "`lailaba claw migrate`.\n"
        "If you've already migrated and want to archive the old directory, "
        "run `lailaba claw cleanup` (renames it to ~/.openclaw.pre-migration — "
