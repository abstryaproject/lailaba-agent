...
- ``set_secret_scope(mapping)`` installs the active profile's secrets for the
  ...
  via ``copy_context()`` exactly like the LAILABA_HOME override).
...
_GLOBAL_ENV_EXACT = frozenset({
    # Lailaba runtime / deployment
    "LAILABA_HOME", "LAILABA_PROFILE", "LAILABA_GATEWAY_LOCK_DIR",
    "LAILABA_MAX_ITERATIONS", "LAILABA_MAX_TOKENS", "LAILABA_API_TIMEOUT",
    "LAILABA_REDACT_SECRETS", "LAILABA_NOUS_TIMEOUT_SECONDS",
    "_LAILABA_GATEWAY",
    # OS / interpreter
    "PATH", "HOME", "USER", "LANG", "LC_ALL", "TZ", "PWD", "SHELL", "TMPDIR",
    "VIRTUAL_ENV", "PYTHONPATH", "SSL_CERT_FILE",
    # Kanban paths (per-board, not per-profile-secret)
    "LAILABA_KANBAN_DB", "LAILABA_KANBAN_WORKSPACES_ROOT", "LAILABA_KANBAN_BOARD",
})
_GLOBAL_ENV_PREFIXES = (
    "LAILABA_KANBAN_",
    "LAILABA_TELEGRAM_",   # tuning knobs (batch delays, fallback toggles) — NOT the token
...
def build_profile_secret_scope(lailaba_home: Path) -> Dict[str, str]:
    ...
    """
    return load_env_file(Path(lailaba_home) / ".env")
