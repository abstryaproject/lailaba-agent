...
row taxonomy is inferred from the atlas shape so Lailaba can render both legacy
...
# rows (1536x1872), while older Lailaba/petdex sheets used 9 columns x 8 rows
...
class PetState(str, Enum):
    ...
    These are Lailaba' activity state names. They are not always identical to the
...
# Legacy Lailaba/petdex row order (top -> bottom) used by the older 8-row,
...
# Canonical Lailaba activity names -> accepted row-name aliases in descending
