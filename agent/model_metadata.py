...
def _resolve_requests_verify() -> bool | str:
    ...
    by default. Lailaba also honours LAILABA_CA_BUNDLE (its own convention)
    ...
    """
    for env_var in ("LAILABA_CA_BUNDLE", "REQUESTS_CA_BUNDLE", "SSL_CERT_FILE"):
...
def _get_model_metadata_cache_path() -> Path:
    ...
    return get_lailaba_home() / "cache" / "openrouter_model_metadata.json"
...
# Minimum context length required to run Lailaba AI.  Models with fewer
...
DEFAULT_CONTEXT_LENGTHS = {
    ...
    # so these hardcoded fallbacks prevent Lailaba from probing-down to
...
_URL_TO_PROVIDER: Dict[str, str] = {
    ...
    # (often 8K) makes it unusable for Lailaba' system prompt, but mapping
...
def _get_context_cache_path() -> Path:
    ...
    from lailaba_constants import get_lailaba_home
    return get_lailaba_home() / "context_length_cache.yaml"
        ...
        with httpx.Client(timeout=3.0, headers=headers) as client:
            ...
            if server_type == "ollama":
                ...
                if resp.status_code == 200:
                    ...
                    # Lailaba grow conversations past the runtime limit and Ollama
...
def get_model_context_length(
    ...
    if effective_provider in {"copilot", "copilot-acp", "github-copilot"}:
        try:
            from lailaba_cli.models import get_copilot_model_context
...
def estimate_request_tokens_rough(
    ...
    *,
    ...
    Includes the major payload buckets Lailaba sends to providers:
