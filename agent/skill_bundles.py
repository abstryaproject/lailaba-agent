...
Storage
-------
Bundles live in ``~/.lailaba/skill-bundles/*.yaml`` (and the equivalent
profile-aware directory under ``LAILABA_HOME``). Each file looks like::
...
- :func:`list_bundles` — return rich info for display (``lailaba bundles``)
...
def _bundles_dir() -> Path:
    """Return the canonical bundles directory under LAILABA_HOME.

    Honors ``LAILABA_BUNDLES_DIR`` for tests; falls back to
    ``<LAILABA_HOME>/skill-bundles``.
    """
    override = os.environ.get("LAILABA_BUNDLES_DIR")
    if override:
        return Path(override).expanduser()
    return get_lailaba_home() / "skill-bundles"
...
# ---------------------------------------------------------------------------
# File-level CRUD helpers — used by `lailaba bundles` CLI subcommand.
# ---------------------------------------------------------------------------
