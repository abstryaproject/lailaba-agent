...
See the ``lailaba-agent-dev`` skill (``references/self-improvement-loop.md``)
...
_SKILL_REVIEW_PROMPT = (
    ...
    "  • Bundled skills (shipped with Lailaba, e.g. 'lailaba-agent').\n"
    "  • Hub-installed skills (installed via 'lailaba skills install').\n"
    "Pinned skills (marked via 'lailaba curator pin') CAN be improved — "
...
def build_memory_write_metadata(
    ...
    *,
    ...
    metadata: Dict[str, Any] = {
        ...
        "platform": agent.platform or os.environ.get("LAILABA_SESSION_SOURCE", "cli"),
        "tool_name": "memory",
    }
    if task_id:
...
def _run_review_in_thread(
    ...
    try:
        with open(os.devnull, "w", encoding="utf-8") as _devnull, \
            ...
            # scratch (fresh _lailaba_now() timestamp, fresh
