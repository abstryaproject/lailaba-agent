...
def _session_is_messaging_surface() -> bool:
    ...
    ``LAILABA_SESSION_PLATFORM``; the CLI and TUI set ``LAILABA_SESSION_SOURCE``
    (e.g. ``cli``, ``tui``) instead. Both are consulted via the session-context
    helper (with an ``os.environ`` fallback), alongside the ``LAILABA_PLATFORM``
    ...
    try:
        ...
        platform = os.getenv("LAILABA_PLATFORM", "") or os.environ.get(
...
def verify_on_stop_enabled(config: dict[str, Any] | None = None) -> bool:
    ...
    Precedence: an explicit ``LAILABA_VERIFY_ON_STOP`` env var wins, then an
    ...
    falls back to OFF.
    """
    env = os.environ.get("LAILABA_VERIFY_ON_STOP")
    if env is not None:
    ...
    *,
    ...
    if verify_commands:
        ...
        command_instruction = (
            ...
            "`tempfile` path with a `lailaba-verify-` filename prefix, run it "
