...
Every Lailaba request that hits the Nous Portal — main agent loop, auxiliary
client (compression / titles / vision / web_extract / session_search / etc.),
and any future code path — must carry the same product-attribution tags so
Nous can attribute usage to Lailaba AI and bucket it by client release.

Tag shape (sent in OpenAI-compatible ``extra_body['tags']``):

    [
        "product=lailaba-agent",
        "client=lailaba-client-v<__version__>",
    ]

The version is sourced live from ``lailaba_cli.__version__`` so it auto-aligns
...
``lailaba_cli.__version__`` is the canonical source of truth.
"""
...
def _lailaba_version() -> str:
    """Return the current Lailaba release version, e.g. ``"0.13.0"``.

    Falls back to ``"unknown"`` if ``lailaba_cli`` cannot be imported (should
...
def lailaba_client_tag() -> str:
    """Return the ``client=...`` tag for Nous Portal requests.

    Format: ``client=lailaba-client-v<MAJOR>.<MINOR>.<PATCH>``.
    """
    return f"client=lailaba-client-v{_lailaba_version()}"
...
def nous_portal_tags() -> List[str]:
    ...
    """
    return ["product=lailaba-agent", lailaba_client_tag()]
