...
def _lailaba_home_path() -> Path:
    """Resolve the active LAILABA_HOME (profile-aware) without circular imports."""
    try:
        from lailaba_constants import get_lailaba_home  # local import to avoid cycles
        return get_lailaba_home()
    except Exception:
        return Path(os.path.expanduser("~/.lailaba"))


def _lailaba_root_path() -> Path:
    """Resolve the Lailaba root dir (always the parent of any profile, never per-profile)."""
    try:
        from lailaba_constants import get_default_lailaba_root  # local import to avoid cycles
...
def build_write_denied_paths(home: str) -> set[str]:
    ...
    lailaba_home = _lailaba_home_path()
    lailaba_root = _lailaba_root_path()
    return {
        os.path.realpath(p)
        for p in [
            ...
            str(lailaba_home / ".env"),
            # Top-level .env, even when running under a profile — overwriting it
            # leaks credentials across every profile that inherits from root (#15981).
            str(lailaba_root / ".env"),
            # Active profile Anthropic PKCE credential store.
            str(lailaba_home / ".anthropic_oauth.json"),
            # Top-level Anthropic PKCE credential store remains sensitive even
            # when a profile is active; default/non-profile sessions still read it.
            str(lailaba_root / ".anthropic_oauth.json"),
...
def get_safe_write_roots() -> set[str]:
    """Return resolved LAILABA_WRITE_SAFE_ROOT paths. Supports multiple directories
    separated by ``os.pathsep`` (``:`` on Unix, ``;`` on Windows).
    E.g., ``/opt/data:/var/www/html`` on Unix, ``C:\\data;D:\\www`` on Windows."""
    env = os.getenv("LAILABA_WRITE_SAFE_ROOT", "")
    if not env:
        return set()
    roots: set[str] = set()
...
def is_write_denied(path: str) -> bool:
    ...
    lailaba_dirs = []
    for base in (_lailaba_home_path(), _lailaba_root_path()):
        try:
            real = os.path.realpath(base)
            if real not in lailaba_dirs:
                lailaba_dirs.append(real)
        except Exception:
            continue

    for base_real in lailaba_dirs:
        try:
...
def get_read_block_error(path: str) -> Optional[str]:
    """Return an error message when a read targets a denied Lailaba path.

    Three categories are blocked:

      * Internal Lailaba cache files under ``LAILABA_HOME/skills/.hub`` —
        readable metadata that an attacker could use as a prompt-injection
        carrier.
      * Credential / secret stores under LAILABA_HOME and the global Lailaba
    ...
    **This is NOT a security boundary.** The terminal tool runs as the
    ...
    or ``cat ~/.lailaba/.env`` and exfiltrate the file. The read-deny exists
    ...
    # Resolve BOTH the active LAILABA_HOME (profile-aware) AND the global
    # Lailaba root so credential stores at <root>/auth.json etc. are also
    # blocked when running under a profile (LAILABA_HOME points at
    # <root>/profiles/<name> in profile mode). Same shape as the write
    # deny widening (#15981, #14157).
    lailaba_dirs: list[Path] = []
    for base in (_lailaba_home_path(), _lailaba_root_path()):
        try:
            real = base.resolve()
            if real not in lailaba_dirs:
    ...
    for hd in lailaba_dirs:
        for name in credential_file_names:
            ...
            if resolved == blocked:
                return (
                    f"Access denied: {path} is a Lailaba credential store "
    ...
    for hd in lailaba_dirs:
        ...
        if resolved == mcp_tokens:
            return (
                f"Access denied: {path} is the Lailaba MCP token directory "
        ...
        try:
            ...
            continue
        return (
            f"Access denied: {path} is a Lailaba MCP token file "
            "and cannot be read directly. (Defense-in-depth — not a "
            "security boundary; the terminal tool can still bypass.)"
        )
...
#
# Lailaba profiles are separate LAILABA_HOME dirs under
...
#
# Reference: May 2026 incident where a lailaba-security profile session
# edited skills under both ``~/.lailaba/profiles/lailaba-security/skills/``
# AND ``~/.lailaba/skills/`` (the default profile's skills) without realizing
# the second path belonged to a different profile.
# ---------------------------------------------------------------------------

# Profile-scoped directories under LAILABA_HOME / <root> / <root>/profiles/<X>/
...
def _resolve_active_profile_name() -> str:
    """Return the active profile name derived from LAILABA_HOME.

    ``~/.lailaba``              -> ``"default"``
    ``~/.lailaba/profiles/X``  -> ``"X"``

    Falls back to ``"default"`` on any resolution failure so the guard
    never raises into the tool path.
    """
    try:
        home_real = _lailaba_home_path().resolve()
        root_real = _lailaba_root_path().resolve()
...
def classify_cross_profile_target(path: str) -> Optional[dict]:
    ...
    Returns ``None`` when the target is outside Lailaba scope, or is inside
...
def get_cross_profile_warning(path: str) -> Optional[str]:
    ...
    Lailaba entirely. Caller is expected to surface the warning to the
    ...
    return (
        ...
        f"belongs to Lailaba profile {info['target_profile']!r}, but the "
...
#
#   <LAILABA_HOME>/profiles/<name>/sandboxes/<backend>/<task>/home/.lailaba/...
#
...
# ``…/sandboxes/<backend>/<task>/home/.lailaba/…`` segment and warns
# regardless of which Lailaba profile is active. It does NOT cover the
# inner-container case where the bind mount strips the ``sandboxes/`` prefix
# (the agent's view inside the container is plain ``/root/.lailaba/...``);
...
def _find_sandbox_mirror_segments(parts: tuple) -> Optional[int]:
    """Return the index of the inner ``.lailaba`` part in a sandbox-mirror path.

    Matches ``…/sandboxes/<backend>/<task>/home/.lailaba/…`` and returns the
    index where the inner Lailaba-state portion starts. Returns ``None`` for
    paths that do not contain the sandbox-mirror shape.
    """
    for i, part in enumerate(parts):
        if part != "sandboxes":
            continue
        # Need at least: sandboxes / <backend> / <task> / home / .lailaba / <thing>
        if i + 5 >= len(parts):
            continue
        if parts[i + 3] == "home" and parts[i + 4] == ".lailaba":
            return i + 4
    return None


def classify_sandbox_mirror_target(path: str) -> Optional[dict]:
    """Classify a write target as a sandbox-mirror of authoritative Lailaba state.

    Returns ``None`` when the path does not match the sandbox-mirror shape.
    Otherwise returns a dict with:

      * ``target_path``: the resolved path string
      * ``mirror_root``: the ``…/sandboxes/<backend>/<task>/home/.lailaba``
        prefix (so callers can show users which sandbox owns the mirror)
      * ``inner_path``: the portion under the mirror's ``.lailaba`` (what the
        agent likely meant to address on the host)

    Detection is path-shape-only — does not require any Lailaba resolver to
    succeed, so it works correctly even when called from contexts where
    LAILABA_HOME resolution would be ambiguous.
    """
    try:
        target = Path(os.path.expanduser(str(path))).resolve()
    except (OSError, RuntimeError):
...
def get_sandbox_mirror_warning(path: str) -> Optional[str]:
    ...
    return (
        ...
        f"Writes here land on a copy that the host Lailaba process never "
        f"reads — the authoritative file is likely {info['inner_path']!r} "
        f"under the real LAILABA_HOME. Use the host-side tool for "
...
# full ``…/sandboxes/<backend>/<task>/home/.lailaba/…`` prefix on the host.
# But when file tools execute *inside* the container the bind-mount strips
# that prefix: the agent sees plain ``/root/.lailaba/…``.  The root:root
...
def classify_container_mirror_target(
    ...
    under the mirror prefix. Otherwise returns:
      ...
      * ``inner_path``: portion under the mirror root (what the agent
        likely meant to address in the host LAILABA_HOME)
    """
...
def get_container_mirror_warning(
    ...
    sandbox mirror of authoritative Lailaba state.
    ...
    return (
        ...
        f"bind-mounted home — a per-task mirror that the host Lailaba "
        f"process never reads. The authoritative file is "
        f"{info['inner_path']!r} under the real LAILABA_HOME. Use the "
