...
class MemoryManager:
    ...
    def add_provider(self, provider: MemoryProvider) -> None:
        ...
        _core_tool_names = set(_LAILABA_CORE_TOOLS)
    ...
    def _strip_skill_scaffolding(text: str) -> Optional[str]:
        ...
        When a user invokes a /skill or /bundle, Lailaba expands the turn into
    ...
    def initialize_all(self, session_id: str, **kwargs) -> None:
        ...
        Automatically injects ``lailaba_home`` into *kwargs* so that every
        provider can resolve profile-scoped storage paths without importing
        ``get_lailaba_home()`` themselves.
        """
        if "lailaba_home" not in kwargs:
            from lailaba_constants import get_lailaba_home
            kwargs["lailaba_home"] = str(get_lailaba_home())
        for provider in self._providers:
            try:
