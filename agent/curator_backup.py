...
A pre-run snapshot of ``~/.lailaba/skills/`` (excluding ``.curator_backups/``
itself) is taken before any mutating curator pass. Snapshots are tar.gz
files under ``~/.lailaba/skills/.curator_backups/<utc-iso>/`` with a
...
``~/.lailaba/cron/jobs.json`` as ``cron-jobs.json`` when it exists. Cron
...
def _backups_dir() -> Path:
    return get_lailaba_home() / "skills" / ".curator_backups"
...
def _cron_jobs_file() -> Path:
    """Source path for the live cron jobs store (``~/.lailaba/cron/jobs.json``)."""
    return get_lailaba_home() / "cron" / "jobs.json"
...
def snapshot_skills(reason: str = "manual", *, protect_ids: Optional[Set[str]] = None) -> Optional[Path]:
    """Create a tar.gz snapshot of ``~/.lailaba/skills/`` and prune old ones.
    ...
    if not skills.exists():
        logger.debug("No ~/.lailaba/skills/ directory — nothing to back up")
        return None
...
def rollback(backup_id: Optional[str] = None) -> Tuple[bool, str, Optional[Path]]:
    """Restore ``~/.lailaba/skills/`` from a snapshot.
    ...
    Strategy:
      ...
      4. Extract the chosen snapshot into ``~/.lailaba/skills/``.
    ...
    if target is None:
        return (
            ...
            + " (use `lailaba curator rollback --list` to see available snapshots)",
            None,
        )
