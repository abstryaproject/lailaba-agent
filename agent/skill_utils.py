...
from lailaba_constants import get_config_path, get_skills_dir, is_termux
...
def _detect_environment(env: str) -> bool:
    ...
    if env == "kanban":
        ...
        # dispatcher sets ``LAILABA_KANBAN_TASK`` / ``LAILABA_KANBAN_BOARD`` in the
        ...
        # tool availability.
        if os.getenv("LAILABA_KANBAN_TASK") or os.getenv("LAILABA_KANBAN_BOARD"):
            result = True
        else:
            try:
        ...
        # The Lailaba Docker image runs s6-overlay as PID 1 (/init). s6 plants
...
def _load_raw_config() -> Dict[str, Any]:
    ...
    This module intentionally avoids importing ``lailaba_cli.config`` on the
...
def get_disabled_skill_names(platform: str | None = None) -> Set[str]:
    ...
    Args:
        platform: Explicit platform name (e.g. ``"telegram"``).  When
            *None*, resolves from ``LAILABA_PLATFORM`` or
            ``LAILABA_SESSION_PLATFORM`` env vars.  Returns the global
...
# which becomes the dominant cost of ``lailaba`` startup when ~120 skills
...
def get_external_skills_dirs() -> List[Path]:
    ...
    paths that resolve to the local ``~/.lailaba/skills/`` are silently skipped.

    Cached in-process, keyed on ``config.yaml`` mtime — the function is
    called once per skill during banner / tool-registry scans, and YAML
    parsing a non-trivial config dominates ``lailaba`` cold-start time
    ...
    for entry in raw_dirs:
        ...
        p = Path(expanded)
        # Resolve relative paths against LAILABA_HOME, not cwd
        if not p.is_absolute():
            p = (lailaba_home / p).resolve()
        else:
            p = p.resolve()
        if p == local_skills:
            continue
...
def get_all_skills_dirs() -> List[Path]:
    """Return all skill directories: local ``~/.lailaba/skills/`` first, then external.
...
def is_external_skill_path(path) -> bool:
    ...
    ``skills.external_dirs`` are externally owned: Lailaba can discover and view
...
def extract_skill_conditions(frontmatter: Dict[str, Any]) -> Dict[str, List]:
    ...
    if not isinstance(metadata, dict):
        metadata = {}
    lailaba = metadata.get("lailaba") or {}
    if not isinstance(lailaba, dict):
        lailaba = {}
    return {
        "fallback_for_toolsets": lailaba.get("fallback_for_toolsets", []),
        "requires_toolsets": lailaba.get("requires_toolsets", []),
        "fallback_for_tools": lailaba.get("fallback_for_tools", []),
        "requires_tools": lailaba.get("requires_tools", []),
    }
...
def extract_skill_config_vars(frontmatter: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    if not isinstance(metadata, dict):
        return []
    lailaba = metadata.get("lailaba")
    if not isinstance(lailaba, dict):
        return []
    raw = lailaba.get("config")
    if not raw:
        return []
...
def iter_skill_index_files(skills_dir: Path, filename: str):
    ...
    Excludes Lailaba metadata, VCS, virtualenv/dependency, cache, and skill
