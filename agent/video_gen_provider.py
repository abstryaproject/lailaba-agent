...
as ``kind: backend``) or ``~/.lailaba/plugins/video_gen/<name>/`` (user, opt-in
via ``plugins.enabled``).
...
class VideoGenProvider(abc.ABC):
    ...
    def capabilities(self) -> Dict[str, Any]:
        ...
        Used by the tool layer for soft validation and by ``lailaba tools``
...
def _videos_cache_dir() -> Path:
    """Return ``$LAILABA_HOME/cache/videos/``, creating parents as needed."""
    from lailaba_constants import get_lailaba_home

    path = get_lailaba_home() / "cache" / "videos"
...
def save_b64_video(
    ...
    *,
    ...
    """Decode base64 video data and write under ``$LAILABA_HOME/cache/videos/``.
