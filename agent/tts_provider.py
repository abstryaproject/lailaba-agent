...
2. **Command-type providers** declared under ``tts.providers.<name>:
   ...
   CLI into Lailaba with shell-template placeholders. **Wins over a
...
shipped today) or ``~/.lailaba/plugins/tts/<name>/`` (user-installed).
...
``{success: False, error: …}`` JSON envelope the rest of Lailaba
expects.
"""
...
class TTSProvider(abc.ABC):
    ...
    def display_name(self) -> str:
        """Human-readable label shown in ``lailaba tools``.
    ...
    def is_available(self) -> bool:
        ...
        Must NOT raise — used by the picker and ``lailaba setup`` for
