"""Centralized logging setup for Lailaba AI.

Provides a single ``setup_logging()`` entry point that both the CLI and
gateway call early in their startup path.  All log files live under
``~/.lailaba/logs/`` (profile-aware via ``get_lailaba_home()``).
...
Component separation:
    ...
    gui.log receives dashboard-side records from ``lailaba_cli.web_server``,
    ``lailaba_cli.pty_bridge``, ``tui_gateway.*``, and ``uvicorn.*``.
...
# essentially always in Lailaba (TUI, gateway, ``hy_memory`` server, MCP
...
from lailaba_constants import get_config_path, get_lailaba_home
...
def _install_session_record_factory() -> None:
    ...
    if getattr(current_factory, "_lailaba_session_injector", False):
    ...
    _session_record_factory._lailaba_session_injector = True  # type: ignore[attr-defined]
...
# Used by _ComponentFilter and exposed for ``lailaba logs --component``.
COMPONENT_PREFIXES = {
    ...
    # ``lailaba logs --component gateway``.
    "gateway": ("gateway", "lailaba_plugins", "plugins.platforms"),
    "agent": ("agent", "run_agent", "model_tools", "batch_runner"),
    "tools": ("tools",),
    "cli": ("lailaba_cli", "cli"),
    "cron": ("cron",),
    "gui": (
        "lailaba_cli.web_server",
        "lailaba_cli.pty_bridge",
        "tui_gateway",
        "uvicorn",
    ),
}
...
def setup_logging(
    *,
    lailaba_home: Optional[Path] = None,
    ...
    """Configure the Lailaba logging subsystem.
    ...
    *force* is ``True``.
    ...
    ----------
    lailaba_home
        Override for the Lailaba home directory.  Falls back to
        ``get_lailaba_home()`` (profile-aware).
    log_level
    ...
    home = lailaba_home or get_lailaba_home()
    log_dir = home / "logs"
...
def setup_verbose_logging() -> None:
    ...
    for h in root.handlers:
        if isinstance(h, logging.StreamHandler) and not isinstance(h, RotatingFileHandler):
            if getattr(h, "_lailaba_verbose", False):
                return

    handler = logging.StreamHandler(_safe_stderr())
    handler.setLevel(logging.DEBUG)
    handler.setFormatter(RedactingFormatter(_LOG_FORMAT_VERBOSE, datefmt="%H:%M:%S"))
    handler._lailaba_verbose = True  # type: ignore[attr-defined]
...
class _ManagedRotatingFileHandler(RotatingFileHandler):
    ...
    1.  In managed mode (NixOS), the stateDir uses setgid (2770) so new files
        inherit the lailaba group. However, both ``_open()`` (initial creation)
    ...
    def __init__(self, *args, **kwargs):
        from lailaba_cli.config import is_managed
