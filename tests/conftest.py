"""Shared fixtures for the lailaba-agent test suite.
...
2. **Isolated LAILABA_HOME.** LAILABA_HOME points to a per-test tempdir so
   code reading ``~/.lailaba/*`` via ``get_lailaba_home()`` can't see the
   real one. (We do NOT also redirect HOME — that broke subprocesses in
   CI. Code using ``Path.home() / ".lailaba"`` instead of the canonical
   ``get_lailaba_home()`` is a bug to fix at the callsite.)
3. **Deterministic runtime.** TZ=UTC, LANG=C.UTF-8, PYTHONHASHSEED=0.
4. **No LAILABA_SESSION_* inheritance** — the agent's current gateway
...
# LAILABA_* vars that change test behavior by being set. Unset all of these
# unconditionally — individual tests that need them set do so explicitly.
_LAILABA_BEHAVIORAL_VARS = frozenset({
    "LAILABA_YOLO_MODE",
    "LAILABA_INTERACTIVE",
    "LAILABA_QUIET",
    "LAILABA_TOOL_PROGRESS",
    "LAILABA_TOOL_PROGRESS_MODE",
    "LAILABA_MAX_ITERATIONS",
    "LAILABA_SESSION_PLATFORM",
    "LAILABA_SESSION_CHAT_ID",
    "LAILABA_SESSION_CHAT_NAME",
    "LAILABA_SESSION_THREAD_ID",
    "LAILABA_SESSION_SOURCE",
    "LAILABA_SESSION_KEY",
    "LAILABA_GATEWAY_SESSION",
    "LAILABA_CRON_SESSION",
    "_LAILABA_GATEWAY",
    "LAILABA_PLATFORM",
    "LAILABA_MODEL",
    "LAILABA_INFERENCE_MODEL",
    "LAILABA_INFERENCE_PROVIDER",
    "LAILABA_TUI_PROVIDER",
    "LAILABA_MANAGED",
    "LAILABA_MANAGED_DIR",
    "LAILABA_DEV",
    "LAILABA_CONTAINER",
    ...
    "LAILABA_TIMEZONE",
    "LAILABA_REDACT_SECRETS",
    "LAILABA_BACKGROUND_NOTIFICATIONS",
    "LAILABA_EXEC_ASK",
    "LAILABA_HOME_MODE",
    "LAILABA_AGENT_USE_LEGACY_SESSION_KEYS",
    # Kanban path/board pins must never leak from a developer shell or
    # dispatched worker into tests; otherwise tests can write fake tasks to
    # the real ~/.lailaba/kanban.db instead of the per-test LAILABA_HOME.
    "LAILABA_KANBAN_DB",
    "LAILABA_KANBAN_BOARD",
    "LAILABA_KANBAN_HOME",
    "LAILABA_KANBAN_WORKSPACES_ROOT",
    "LAILABA_KANBAN_LOGS_ROOT",
    "LAILABA_KANBAN_TASK",
    "LAILABA_KANBAN_WORKSPACE",
    "LAILABA_KANBAN_RUN_ID",
    "LAILABA_KANBAN_CLAIM_LOCK",
    "LAILABA_KANBAN_DISPATCH_IN_GATEWAY",
    "LAILABA_TENANT",
    ...
    "LAILABA_DASHBOARD_OAUTH_CLIENT_ID",
    "LAILABA_DASHBOARD_PORTAL_URL",
    "TERMINAL_CWD",
    "TERMINAL_ENV",
...
def _hermetic_environment(tmp_path, monkeypatch):
    ...
    Also redirects HOME and LAILABA_HOME to per-test tempdirs so code that
    reads ``~/.lailaba/*`` can't touch the real one, and pins TZ/LANG so
    ...
    # 2. Blank behavioral LAILABA_* vars that could change test semantics.
    for name in _LAILABA_BEHAVIORAL_VARS:
        monkeypatch.delenv(name, raising=False)

    # 3. Redirect LAILABA_HOME to a per-test tempdir. Code that reads
    #    ``~/.lailaba/*`` via ``get_lailaba_home()`` now gets the tempdir.
    #
    #    NOTE: We do NOT also redirect HOME. Doing so broke CI because
    #    some tests (and their transitive deps) spawn subprocesses that
    #    inherit HOME and expect it to be stable. If a test genuinely
    #    needs HOME isolated, it should set it explicitly in its own
    #    fixture. Any code in the codebase reading ``~/.lailaba/*`` via
    #    ``Path.home() / ".lailaba"`` instead of ``get_lailaba_home()``
    #    is a bug to fix at the callsite.
    fake_lailaba_home = tmp_path / "lailaba_test"
    fake_lailaba_home.mkdir()
    (fake_lailaba_home / "sessions").mkdir()
    (fake_lailaba_home / "cron").mkdir()
    (fake_lailaba_home / "memories").mkdir()
    (fake_lailaba_home / "skills").mkdir()
    monkeypatch.setenv("LAILABA_HOME", str(fake_lailaba_home))
    ...
    #    ~/.lailaba/plugins/ (which, per step 3, is now empty — but the
    #    singleton might still be cached from a previous test).
    try:
        import lailaba_cli.plugins as _plugins_mod
...
@pytest.fixture(autouse=True)
def _isolate_lailaba_home(_hermetic_environment):
...
@pytest.fixture()
def mock_config():
    """Return a minimal lailaba config dict suitable for unit tests."""
    return {
...
# environment and finds the developer's live ``lailaba-gateway`` process
...
# correlated with a ``tests/lailaba_cli/`` pytest run starting up.
#
...
#    ``check_output`` reject any ``systemctl ... <verb> lailaba-gateway``
...
def _live_system_guard(request, monkeypatch):
    ...
    tokens[0]), so ``bash -c "systemctl restart lailaba-gateway"``,
    ``sudo systemctl ...``, ``env systemctl ...``, ``setsid systemctl ...``
    are all caught. ``pkill``/``killall``/``taskkill`` invocations
    targeting lailaba/python patterns are also blocked.
    """
    ...
    def _matches_lailaba_gateway(cmd_str: str) -> bool:
        low = cmd_str.lower()
        return any(tok in low for tok in _LAILABA_TOKENS)

    def _is_blocked_systemctl(cmd) -> bool:
        cmd_str = _cmd_to_string(cmd)
        if "systemctl" not in cmd_str:
            return False
        if not _matches_lailaba_gateway(cmd_str):
            return False
        try:
    ...
    def _is_process_killer(cmd) -> bool:
        ...
        for tok in tokens:
            ...
            if head in _PROCESS_KILLERS:
                low = cmd_str.lower()
                # pkill -f pattern: catch lailaba-themed patterns + a
                # plain "python" -f which would catch the live gateway
                # whose cmdline contains "python -m lailaba_cli.main".
                if (
                    "lailaba" in low
                    or "gateway" in low
    ...
    def _check_subprocess_cmd(name, cmd):
        if _is_blocked_systemctl(cmd):
            raise RuntimeError(
                ...
                "live lailaba-gateway systemd unit. Mock "
        ...
        if _is_process_killer(cmd):
            raise RuntimeError(
                ...
                "targeting lailaba/python could hit the live gateway. "
                "Mark with @pytest.mark.live_system_guard_bypass if "
                "intentional."
            )
        # Block any subprocess that would run `lailaba update` (or the
        # equivalent `python -m lailaba_cli.main update`).  These commands
        ...
        if "update" in low and (
            # lailaba update / lailaba update --gateway / setsid bash -c ... lailaba update
            ("lailaba" in low and "update" in low.split())
            or
            # python -m lailaba_cli.main update --gateway
            ("lailaba_cli" in low and "update" in low.split())
            or
            # venv/bin/lailaba update  (absolute path variant used in tests)
            (".venv/bin/lailaba" in low and "update" in low)
        ):
            raise RuntimeError(
                f"tests/conftest.py live-system guard: blocked "
                f"subprocess.{name}({cmd!r}) — this command would run "
                "`lailaba update` against the real checkout, fetching "
