...
The ``lailaba update`` Python path already clears the conflict with ``git reset``
...
def test_install_ps1_stops_venv_resident_processes_before_removing_venv() -> None:
    ...
    ``venv\\Scripts\\pythonw.exe -m lailaba_cli.main gateway run`` — image name
    ``pythonw``, not ``lailaba.exe`` — so the ``taskkill /IM lailaba.exe`` guard
    ...
    # The lailaba.exe tree-kill is preserved (kills spawned child processes too).
    assert 'taskkill /F /T /IM lailaba.exe' in text
