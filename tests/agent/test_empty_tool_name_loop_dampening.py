...
@pytest.fixture()
def agent_env():
    """Spin up the mock provider + an isolated LAILABA_HOME, yield (agent, helpers)."""
    ...
    test_home = tempfile.mkdtemp(prefix="lailaba_e2e_47967_")
    os.makedirs(os.path.join(test_home, ".lailaba"))
    prev_home = os.environ.get("LAILABA_HOME")
    os.environ["LAILABA_HOME"] = os.path.join(test_home, ".lailaba")
    ...
    for mod in list(sys.modules):
        if mod == "run_agent" or mod.startswith("agent.") or mod.startswith("tools.") or mod.startswith("lailaba_"):
            del sys.modules[mod]
    ...
    try:
        ...
        if prev_home is None:
            ...
            os.environ["LAILABA_HOME"] = prev_home
