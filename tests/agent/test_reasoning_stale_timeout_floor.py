...
thinking phase.  Lailaba's default cloud-stream stale detector
(``LAILABA_STREAM_STALE_TIMEOUT`` = 180s) and non-stream detector
(``LAILABA_API_CALL_STALE_TIMEOUT`` = 90s) both fire before the
...
def test_reasoning_floor_applies_to_nemotron_3_ultra(monkeypatch, tmp_path):
    ...
    import importlib
    from lailaba_cli import config as cfg_mod, timeouts as to_mod
