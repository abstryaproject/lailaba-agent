...
def http_server(tmp_path, monkeypatch):
    """Spin up a localhost HTTP server and isolate LAILABA_HOME under tmp_path."""
    monkeypatch.setenv("LAILABA_HOME", str(tmp_path / ".lailaba"))
    (tmp_path / ".lailaba").mkdir()

    # Force the constants/image cache helpers to re-read LAILABA_HOME.
    import sys
    for mod in list(sys.modules):
        if mod.startswith("lailaba_constants") or mod.startswith("agent.image_gen_provider"):
...
class TestSaveUrlImage:
    def test_writes_real_bytes_to_lailaba_home_cache(self, http_server):
        base, _ = http_server
        ...
        # The cache directory must be under LAILABA_HOME — gateway cleanup
