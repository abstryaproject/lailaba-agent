...
class TestEnvFileReadBlocking:
    ...
    def test_allowed_lailaba_env(self):
        """Lailaba' own .env inside LAILABA_HOME is NOT blocked by this rule
        (it's handled by other mechanisms). Only project-local .env is blocked."""
        # Note: lailaba internal .env is in ~/.lailaba/.env which is NOT a project-local
        # path, but the basename check applies to ANY .env. This is intentional —
        # even ~/.lailaba/.env should not be readable via read_file.
        error = get_read_block_error(os.path.expanduser("~/.lailaba/.env"))
        assert error is not None
...
class TestCacheFileReadBlocking:
    """Internal Lailaba cache files must remain blocked."""

    def test_hub_index_cache_blocked(self, tmp_path):
        """Hub index-cache reads are blocked."""
        lailaba_home = tmp_path / ".lailaba"
        cache = lailaba_home / "skills" / ".hub" / "index-cache" / "data.json"
        cache.parent.mkdir(parents=True)
        cache.write_text("{}")

        with patch("agent.file_safety._lailaba_home_path", return_value=lailaba_home):
            error = get_read_block_error(str(cache))
            assert error is not None
            assert "internal Lailaba cache" in error

    def test_hub_directory_blocked(self, tmp_path):
        """Hub directory reads are blocked."""
        lailaba_home = tmp_path / ".lailaba"
        hub = lailaba_home / "skills" / ".hub" / "metadata.json"
...
class TestCombinedGuards:
    ...
    def test_env_guard_works_regardless_of_lailaba_home(self, tmp_path):
        """The env basename guard does not depend on LAILABA_HOME resolution."""
        lailaba_home = tmp_path / ".lailaba"
        lailaba_home.mkdir()
    ...
    def test_cache_guard_still_works_with_env_guard(self, tmp_path):
        ...
        lailaba_home = tmp_path / ".lailaba"
        cache = lailaba_home / "skills" / ".hub" / "index-cache" / "x"
        cache.parent.mkdir(parents=True)
        cache.write_text("")
