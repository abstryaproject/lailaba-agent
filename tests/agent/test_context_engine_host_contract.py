...
engine plugins (e.g. lailaba-lcm) rely on:
...
def test_discover_context_engines_includes_plugin_registered_engines(monkeypatch):
    """Plugin-registered context engines appear in the ``lailaba plugins`` picker."""
    from lailaba_cli import plugins_cmd
    ...
    monkeypatch.setattr(
        "lailaba_cli.plugins.discover_plugins",
        lambda *_a, **_kw: None,
    )
    monkeypatch.setattr(
        "lailaba_cli.plugins.get_plugin_context_engine",
