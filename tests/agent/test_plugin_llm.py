...
""",
        ...
        from lailaba_cli import config as _config_mod
    ...
    def test_missing_plugin_entry_yields_default_deny(self, tmp_path, monkeypatch):
        ...
        lailaba_home = tmp_path / ".lailaba"
        lailaba_home.mkdir()
        (lailaba_home / "config.yaml").write_text("plugins: {}\n", encoding="utf-8")
        monkeypatch.setenv("LAILABA_HOME", str(lailaba_home))
        from lailaba_cli import config as _config_mod
        _config_mod._config_cache = None  # type: ignore[attr-defined]
...
class TestPluginContextIntegration:
    def test_ctx_llm_is_lazy_singleton(self):
        from lailaba_cli.plugins import PluginContext, PluginManifest, PluginManager
