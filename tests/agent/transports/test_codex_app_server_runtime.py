...
class TestSpawnEnvIsolation:
    ...
    def test_kanban_worker_adds_only_kanban_writable_root(self, monkeypatch):
        ...
        danger-full-access. Lailaba passes a narrow app-server config override
        ...
        monkeypatch.setattr(subprocess, "Popen", FakePopen)
        monkeypatch.setenv("HOME", "/users/alice")
        monkeypatch.setenv("LAILABA_HOME", "/users/alice/.lailaba/profiles/backend-worker")
        monkeypatch.setenv("LAILABA_KANBAN_TASK", "t_smoke")
        monkeypatch.setenv(
            "LAILABA_KANBAN_DB",
            "/users/alice/.lailaba/kanban/boards/smoke/kanban.db",
        )
        ...
        assert (
            'sandbox_workspace_write.writable_roots=["/users/alice/.lailaba/kanban/boards/smoke"]'
            in cmd
        )
