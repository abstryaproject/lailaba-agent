...
def test_lailaba_client_tag_includes_current_version():
    """The client tag must reflect lailaba_cli.__version__ verbatim."""
    from lailaba_cli import __version__
    from agent.portal_tags import lailaba_client_tag

    assert lailaba_client_tag() == f"client=lailaba-client-v{__version__}"


def test_lailaba_client_tag_format():
    """The client tag has the exact shape Nous Portal expects."""
    from agent.portal_tags import lailaba_client_tag

    tag = lailaba_client_tag()
    assert tag.startswith("client=lailaba-client-v")
...
def test_nous_portal_tags_contains_product_and_client():
    ...
    from agent.portal_tags import lailaba_client_tag, nous_portal_tags

    tags = nous_portal_tags()
    assert "product=lailaba-agent" in tags
    assert lailaba_client_tag() in tags
    assert len(tags) == 2
