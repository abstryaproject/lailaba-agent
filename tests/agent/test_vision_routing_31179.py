...
def isolated_home(monkeypatch):
    """Temp LAILABA_HOME with config + clean credential env vars."""
    test_home = tempfile.mkdtemp(prefix="lailaba_test_31179_")
    lailaba_home = os.path.join(test_home, ".lailaba")
    os.makedirs(lailaba_home)
...
def _fresh_modules():
    """Drop cached lailaba modules so each test reloads against current env."""
