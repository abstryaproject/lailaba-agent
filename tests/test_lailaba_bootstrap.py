"""Tests for lailaba_bootstrap — Windows UTF-8 stdio shim.

The bootstrap module is imported at the top of every Lailaba entry point
(lailaba, lailaba-agent, lailaba-acp, gateway, batch_runner, cli.py).  It
...
Key invariants covered by these tests:
  ...
  5. Load order: every Lailaba entry point imports lailaba_bootstrap as its
...
def _fresh_import():
    """Return a freshly-imported lailaba_bootstrap module.

    Drops any cached copy from sys.modules first so module-level code
    runs again and the platform check re-evaluates.
    """
    sys.modules.pop("lailaba_bootstrap", None)
    import lailaba_bootstrap  # noqa: WPS433
    return lailaba_bootstrap
...
class TestWindowsBehavior:
    ...
    def test_stdout_reconfigured_to_utf8_on_windows(self):
        # The live process's stdout should now be UTF-8 (the Lailaba CLI
...
class TestEntryPointsImportBootstrap:
    """Every Lailaba entry point must import lailaba_bootstrap as its
    ...
    # Entry points that invoke Lailaba as a process.  Each one must
    # import lailaba_bootstrap before doing any file I/O or stdout writes.
    ENTRY_POINTS = [
        "lailaba_cli/main.py",   # lailaba CLI (console_script)
        "run_agent.py",          # lailaba-agent (console_script)
        "acp_adapter/entry.py",  # lailaba-acp (console_script)
    ...
    def test_entry_point_imports_bootstrap(self, path):
        """The file must contain 'import lailaba_bootstrap' and that
        ...
        half-finished ``lailaba update`` (git-reset landed new code but
        ``uv pip install -e .`` didn't finish re-registering
        ``lailaba_bootstrap`` as a top-level module) leaves lailaba
        ...
        """
        # Resolve relative to the lailaba-agent repo root.  Tests live
        # at tests/test_lailaba_bootstrap.py, so go up one dir.
        import pathlib
        ...
        for node in ast.iter_child_nodes(tree):
            ...
            # lailaba start even when lailaba_bootstrap hasn't been
        ...
        assert first_import_name == "lailaba_bootstrap", (
            f"{path}: first top-level import is {first_import_name!r}, "
            f"but it must be 'lailaba_bootstrap' so UTF-8 stdio is "
            f"configured before anything else initializes.  Move the "
            f"'import lailaba_bootstrap' line to be the first import."
        )


class TestHardenImportPath:
    """harden_import_path() must keep a same-named package in the launch
    directory from shadowing Lailaba's own top-level modules — covering both
    the relative ('' / '.') and absolute-path forms the cwd can take on
    sys.path (issue #51286)."""

    def _run(self, hb, path_seed, env=None):
        original = sys.path[:]
        original_env = os.environ.get("LAILABA_PYTHON_SRC_ROOT")
        try:
            sys.path[:] = path_seed
            if env is not None:
                os.environ["LAILABA_PYTHON_SRC_ROOT"] = env
            elif "LAILABA_PYTHON_SRC_ROOT" in os.environ:
                del os.environ["LAILABA_PYTHON_SRC_ROOT"]
            hb.harden_import_path(src_root="/opt/lailaba")
            return sys.path[:]
        finally:
            sys.path[:] = original
            if original_env is None:
                os.environ.pop("LAILABA_PYTHON_SRC_ROOT", None)
            else:
                os.environ["LAILABA_PYTHON_SRC_ROOT"] = original_env

    def test_relative_cwd_forms_removed(self):
        hb = _fresh_import()
        result = self._run(hb, ["", ".", "/opt/lailaba", "/usr/lib/python"])
    ...
    def test_absolute_cwd_path_loses_to_src_root(self):
        ...
        # Lailaba root.  The guard must relocate Lailaba to the front.
        hb = _fresh_import()
        result = self._run(hb, ["/home/user/tg-ws-proxy", "/opt/lailaba"])
        assert result[0] == "/opt/lailaba"
        # The cwd absolute path may still appear (it can hold legit deps),
        # but only AFTER the Lailaba root.
        assert result.index("/opt/lailaba") < result.index("/home/user/tg-ws-proxy")

    def test_src_root_not_duplicated(self):
        hb = _fresh_import()
        result = self._run(hb, ["/opt/lailaba", "/opt/lailaba", ""])
        assert result.count("/opt/lailaba") == 1
    ...
    def test_env_var_used_when_no_arg(self):
        ...
        try:
            ...
            os.environ["LAILABA_PYTHON_SRC_ROOT"] = "/env/lailaba"
            hb.harden_import_path()
            assert sys.path[0] == "/env/lailaba"
        finally:
            sys.path[:] = original
            if original_env is None:
                os.environ.pop("LAILABA_PYTHON_SRC_ROOT", None)
