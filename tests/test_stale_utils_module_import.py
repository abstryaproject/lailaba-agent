...
Real incident (gateway session 1518671026962174144)::
    ...
    cannot import name 'env_float' from 'utils' (~/.lailaba/lailaba-agent/utils.py)
...
2. ``lailaba update`` ran ``git pull``, updating ``utils.py`` (now defining
...
``lailaba_cli/main.py`` (the ``lailaba update`` flow, ~line 9326) already
acknowledges this exact hazard -- "source files on disk are newer than cached
Python modules in this process" -- and reloads ``lailaba_constants`` after the
