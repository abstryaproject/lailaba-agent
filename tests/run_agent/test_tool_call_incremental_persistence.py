...
def _make_agent():
    lailaba_home = Path(tempfile.mkdtemp(prefix="lailaba-test-home-"))
    (lailaba_home / "logs").mkdir(parents=True, exist_ok=True)
    with (
        patch(
        ...
        patch("run_agent._lailaba_home", lailaba_home),
