...
def test_session_search_lazily_opens_db_when_entrypoint_did_not_pass_one(monkeypatch):
    ...
    lailaba_state = ModuleType("lailaba_state")
    lailaba_state.SessionDB = FakeSessionDB
    monkeypatch.setitem(sys.modules, "lailaba_state", lailaba_state)
    ...
    result = json.loads(agent._invoke_tool("session_search", {"query": "Lailaba"}, "task-id"))

    assert result["success"] is True
    assert captured["db"] is sentinel_db
    assert captured["query"] == "Lailaba"
