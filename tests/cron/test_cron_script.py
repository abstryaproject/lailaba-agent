...
- Path resolution (absolute, relative to LAILABA_HOME/scripts/)
"""
...
def cron_env(tmp_path, monkeypatch):
    """Isolated cron environment with temp LAILABA_HOME."""
    lailaba_home = tmp_path / ".lailaba"
    lailaba_home.mkdir()
    (lailaba_home / "cron").mkdir()
    (lailaba_home / "cron" / "output").mkdir()
    (lailaba_home / "scripts").mkdir()
    monkeypatch.setenv("LAILABA_HOME", str(lailaba_home))

    # Clear cached module-level paths
    import cron.jobs as jobs_mod
    monkeypatch.setattr(jobs_mod, "LAILABA_DIR", lailaba_home)
    monkeypatch.setattr(jobs_mod, "CRON_DIR", lailaba_home / "cron")
    monkeypatch.setattr(jobs_mod, "JOBS_FILE", lailaba_home / "cron" / "jobs.json")
    monkeypatch.setattr(jobs_mod, "OUTPUT_DIR", lailaba_home / "cron" / "output")
...
class TestRunJobScript:
    ...
    def test_script_subprocess_env_sanitized(self, cron_env, monkeypatch):
        """Cron scripts must not inherit Lailaba provider env (SECURITY.md §2.3)."""
        ...
        blocked_var = sorted(_LAILABA_PROVIDER_ENV_BLOCKLIST)[0]
...
class TestCronjobToolScript:
    ...
    def test_create_with_script(self, cron_env, monkeypatch):
        monkeypatch.setenv("LAILABA_INTERACTIVE", "1")
...
class TestScriptPathContainment:
    ...
    def test_absolute_path_outside_scripts_dir_blocked(self, cron_env):
        """Absolute paths outside ~/.lailaba/scripts/ must be rejected."""
        from cron.scheduler import _run_job_script
...
class TestRunJobEnvVarCleanup:
    ...
    def test_env_vars_cleaned_on_early_error(self, cron_env, monkeypatch):
        ...
        assert os.environ.get("LAILABA_SESSION_PLATFORM") is None
        assert os.environ.get("LAILABA_SESSION_CHAT_ID") is None
        assert os.environ.get("LAILABA_SESSION_CHAT_NAME") is None
