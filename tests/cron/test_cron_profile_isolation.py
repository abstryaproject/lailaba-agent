...
profile's LAILABA_HOME and EXECUTE under it.

- Storage: a job created under profile ``coder`` writes to
  ``~/.lailaba/profiles/coder/cron/jobs.json`` — NOT the shared default root.
- Execution: the profile-scoped gateway's in-process ticker resolves the
  active LAILABA_HOME (profile home) at call time, so jobs run with that
...
one store and runs them under whatever LAILABA_HOME the ticker happens to have —
...
def _set_profile_env(monkeypatch, root: Path, profile_home: Path) -> None:
    ...
    LAILABA_HOME is a profile under it (``<root>/profiles/<name>``)."""
    import lailaba_constants

    monkeypatch.setattr(
        lailaba_constants, "_get_platform_default_lailaba_home", lambda: root
    )
    monkeypatch.setenv("LAILABA_HOME", str(profile_home))


def test_cron_storage_anchors_at_profile_home(tmp_path, monkeypatch):
    """Under a profile LAILABA_HOME (<root>/profiles/<name>), the cron store
    resolves to <profile>/cron, NOT the shared <root>/cron."""
    root = tmp_path / "lailaba_home"
    ...
    assert lailaba_constants.get_lailaba_home().resolve() == profile_home.resolve()
    assert lailaba_constants.get_default_lailaba_root().resolve() == root.resolve()

    # cron/jobs.py computes LAILABA_DIR from get_lailaba_home() at import, so a
    # fresh import under this env anchors the store at <profile>/cron.
    import cron.jobs as jobs

    importlib.reload(jobs)
    try:
        assert jobs.LAILABA_DIR.resolve() == profile_home.resolve()
        assert (
...
def test_cron_execution_home_follows_active_profile(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(scheduler, "_lailaba_home", None, raising=False)
    assert scheduler._get_lailaba_home().resolve() == profile_home.resolve()
    assert scheduler._get_lailaba_home().resolve() != root.resolve()


def test_cron_storage_unaffected_when_no_profile(tmp_path, monkeypatch):
    """With no profile (LAILABA_HOME == root), the store is the root's cron dir
    ...
    importlib.reload(jobs)
    try:
        assert jobs.LAILABA_DIR.resolve() == root.resolve()
