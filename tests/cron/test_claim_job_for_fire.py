...
These exercise the real store against a temp LAILABA_HOME (no mocks) per the
...
def temp_home(tmp_path, monkeypatch):
    """Isolated LAILABA_HOME so jobs.json doesn't touch the real store."""
    monkeypatch.setenv("LAILABA_HOME", str(tmp_path))
    # cron.jobs caches no home at import; get_lailaba_home() reads the env live.
    yield tmp_path
