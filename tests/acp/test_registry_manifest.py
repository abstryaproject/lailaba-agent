"""Tests for ACP Registry metadata shipped with Lailaba."""
...
def test_agent_json_matches_official_registry_required_fields():
    ...
    assert data["id"] == "lailaba-agent"
    assert re.fullmatch(r"[a-z][a-z0-9-]*", data["id"])
    assert data["name"] == "Lailaba AI"
    assert data["description"]
    assert data["repository"] == "https://github.com/NousResearch/lailaba-agent"
    assert data["website"].startswith("https://lailaba-agent.nousresearch.com/")
...
def test_agent_json_uses_uvx_distribution_without_local_command_fields():
    ...
    assert uvx["package"] == f"lailaba-agent[acp]=={data['version']}"
    assert uvx["args"] == ["lailaba-acp"]
...
def test_agent_json_pins_uvx_package_to_pyproject_version():
    ...
    assert _manifest()["distribution"]["uvx"]["package"] == (
        f"lailaba-agent[acp]=={_pyproject_version()}"
    )
