...
def test_compression_split_continuation(db):
    ...
    prov = build_session_provenance(
        db, "acp-1", "new", previous_lailaba_session_id="old"
    )
...
def test_no_false_rotation_when_head_unchanged(db):
    ...
    prov = build_session_provenance(
        db, "acp-1", "s", previous_lailaba_session_id="s"
    )
...
def test_meta_wrapper_shape(db):
    ...
    assert set(meta.keys()) == {"lailaba"}
    assert "sessionProvenance" in meta["lailaba"]
    assert meta["lailaba"]["sessionProvenance"]["currentHermesSessionId"] == "root1"
