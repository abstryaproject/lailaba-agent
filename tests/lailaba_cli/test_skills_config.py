"""Tests for lailaba_cli/skills_config.py and skills_tool disabled filtering."""
...
class TestSaveDisabledSkills:
    @patch("lailaba_cli.skills_config.save_config")
    def test_saves_global_sorted(self, mock_save):
        from lailaba_cli.skills_config import save_disabled_skills
        config = {}
...
class TestIsSkillDisabled:
    @patch("lailaba_cli.config.load_config")
    ...
    @patch("lailaba_cli.config.load_config")
    @patch.dict("os.environ", {"LAILABA_PLATFORM": "discord"})
...
class TestGetDisabledSkillNames:
    ...
    def test_explicit_platform_param(self, tmp_path, monkeypatch):
        ...
        monkeypatch.delenv("LAILABA_PLATFORM", raising=False)
    ...
    def test_session_platform_env_var(self, tmp_path, monkeypatch):
        """LAILABA_SESSION_PLATFORM should be used when LAILABA_PLATFORM is unset."""
        config = tmp_path / "config.yaml"
        config.write_text(
            "skills:\n"
            "  disabled:\n"
    ...
    def test_lailaba_platform_takes_precedence(self, tmp_path, monkeypatch):
        """LAILABA_PLATFORM should win over LAILABA_SESSION_PLATFORM."""
        config = tmp_path / "config.yaml"
        config.write_text(
            "skills:\n"
            "  platform_disabled:\n"
        ...
        monkeypatch.setenv("LAILABA_HOME", str(tmp_path))
        monkeypatch.setenv("LAILABA_PLATFORM", "telegram")
        monkeypatch.setenv("LAILABA_SESSION_PLATFORM", "discord")
...
class TestGetCategories:
    def test_extracts_unique_categories(self):
        from lailaba_cli.skills_config import _get_categories
        skills = [
