"""Regression test: ``lailaba mcp add --command`` must not clobber the
top-level ``args.command`` subparser dest.

The top-level argparse parser uses ``dest="command"`` for its subparsers
(``lailaba_cli/_parser.py``).  The dispatcher in ``lailaba_cli/main.py``
...
result, ``lailaba mcp add foo --url ...`` silently launches chat instead
...
def _build_parser():
    """Minimal replica of the slice of the lailaba parser that exhibits
...
class TestMcpAddCommandDest:
    def test_url_invocation_preserves_top_level_command(self):
        """`lailaba mcp add foo --url ...` must keep args.command == "mcp".
