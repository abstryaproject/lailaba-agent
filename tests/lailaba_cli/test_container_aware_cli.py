...
def container_env(tmp_path, monkeypatch):
    """Set up a fake LAILABA_HOME with .container-mode file."""
    lailaba_home = tmp_path / ".lailaba"
    lailaba_home.mkdir()
    monkeypatch.setenv("LAILABA_HOME", str(lailaba_home))
    monkeypatch.delenv("LAILABA_DEV", raising=False)

    container_mode = lailaba_home / ".container-mode"
    container_mode.write_text(
        "# Written by NixOS activation script. Do not edit manually.\n"
        "backend=podman\n"
        "container_name=lailaba-agent\n"
        "exec_user=lailaba\n"
        "lailaba_bin=/data/current-package/bin/lailaba\n"
    )
    return lailaba_home
...
def test_get_container_exec_info_returns_metadata(container_env):
    ...
    with patch("lailaba_constants.is_container", return_value=False):
        info = get_container_exec_info()

    assert info is not None
    assert info["backend"] == "podman"
    assert info["container_name"] == "lailaba-agent"
    assert info["exec_user"] == "lailaba"
    assert info["lailaba_bin"] == "/data/current-package/bin/lailaba"


def test_get_container_exec_info_none_inside_container(container_env):
    """Returns None when we're already inside a container."""
    with patch("lailaba_constants.is_container", return_value=True):
        info = get_container_exec_info()
...
def test_get_container_exec_info_skipped_when_lailaba_dev(container_env, monkeypatch):
    """Returns None when LAILABA_DEV=1 is set (dev mode bypass)."""
    monkeypatch.setenv("LAILABA_DEV", "1")
...
def test_get_container_exec_info_not_skipped_when_lailaba_dev_zero(container_env, monkeypatch):
    """LAILABA_DEV=0 does NOT trigger bypass — only '1' does."""
    monkeypatch.setenv("LAILABA_DEV", "0")
...
def test_get_container_exec_info_defaults():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        lailaba_home = Path(tmpdir) / ".lailaba"
        lailaba_home.mkdir()
        (lailaba_home / ".container-mode").write_text(
            "# minimal file with no keys\n"
        )

        with patch("lailaba_constants.is_container", return_value=False), \
             patch.dict(get_container_exec_info.__globals__, {"get_lailaba_home": lambda: lailaba_home}), \
             patch.dict(os.environ, {}, clear=False):
            os.environ.pop("LAILABA_DEV", None)
            info = get_container_exec_info()
...
def test_get_container_exec_info_docker_backend(container_env):
    ...
    (container_env / ".container-mode").write_text(
        "backend=docker\n"
        "container_name=lailaba-custom\n"
        "exec_user=myuser\n"
        "lailaba_bin=/opt/lailaba/bin/lailaba\n"
    )
    ...
    assert info["backend"] == "docker"
    assert info["container_name"] == "lailaba-custom"
    assert info["exec_user"] == "myuser"
    assert info["lailaba_bin"] == "/opt/lailaba/bin/lailaba"
...
def test_exec_in_container_calls_execvp(docker_container_info):
    ...
    from lailaba_cli.main import _exec_in_container
    ...
    assert "-it" in cmd
    idx_u = cmd.index("-u")
    assert cmd[idx_u + 1] == "lailaba"
    e_indices = [i for i, v in enumerate(cmd) if v == "-e"]
    e_values = [cmd[i + 1] for i in e_indices]
    assert "TERM=xterm-256color" in e_values
    assert "LANG=en_US.UTF-8" in e_values
    assert "lailaba-agent" in cmd
    assert "/data/current-package/bin/lailaba" in cmd
    assert "chat" in cmd
