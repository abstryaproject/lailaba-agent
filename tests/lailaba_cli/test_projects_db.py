"""Tests for the per-profile Projects store (lailaba_cli/projects_db)."""
...
def test_create_get_list(conn):
    pid = pdb.create_project(conn, name="Lailaba AI", folders=["/tmp/lailaba"])
    proj = pdb.get_project(conn, pid)

    assert proj is not None
    assert proj.slug == "lailaba-agent"
    assert proj.name == "Lailaba AI"
    # First folder becomes primary.
    assert proj.primary_path == "/tmp/lailaba"
    assert [f.path for f in proj.folders] == ["/tmp/lailaba"]
    assert proj.folders[0].is_primary is True

    # Lookup by slug too.
    assert pdb.get_project(conn, "lailaba-agent").id == pid
    assert len(pdb.list_projects(conn)) == 1


def test_slug_collision_disambiguates(conn):
    pdb.create_project(conn, name="Lailaba AI")
    pdb.create_project(conn, name="Lailaba AI")
    slugs = sorted(p.slug for p in pdb.list_projects(conn))

    assert slugs == ["lailaba-agent", "lailaba-agent-2"]
...
def test_per_profile_isolation(tmp_path):
    # Two distinct DB paths stand in for two profiles' LAILABA_HOME.
...
def test_db_path_under_lailaba_home():
    # Resolves under LAILABA_HOME (set by the autouse isolation fixture).
