...
def test_single_handler_builders(name, builder, kw, argv):
    parser = argparse.ArgumentParser(prog="lailaba")
...
# ── deprecated `lailaba login` fails gracefully, not with argparse error ────
#
# `lailaba login` is a removed command; its handler (`login_command` in
# `lailaba_cli/auth.py`) prints a deprecation notice pointing at `lailaba auth` /
# `lailaba model` and exits 0.  Two behavior contracts guard the UX:
...
def test_login_subparser_help_is_suppressed():
    """The deprecated `login` row must not appear in `lailaba --help`.
