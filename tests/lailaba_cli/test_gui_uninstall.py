"""Tests for lailaba_cli.gui_uninstall — GUI-only uninstall + install discovery.
...
import lailaba_cli.gui_uninstall as gu


def _make_agent(lailaba_home: Path) -> Path:
    """Create a fake agent install: source package + venv."""
    agent_root = lailaba_home / "lailaba-agent"
    (agent_root / "lailaba_cli").mkdir(parents=True)
    (agent_root / "lailaba_cli" / "__init__.py").write_text("")
    (agent_root / "venv" / "bin").mkdir(parents=True)
    return agent_root


def _make_gui_build(lailaba_home: Path) -> None:
    """Create the source-built GUI artifacts a `lailaba desktop` run produces."""
    desktop = lailaba_home / "lailaba-agent" / "apps" / "desktop"
    ...
    (lailaba_home / "lailaba-agent" / "node_modules").mkdir(parents=True)
    (lailaba_home / "desktop-build-stamp.json").write_text("{}")


def _make_user_data(lailaba_home: Path) -> None:
    (lailaba_home / "config.yaml").write_text("x: 1\n")
    (lailaba_home / ".env").write_text("KEY=secret\n")
    (lailaba_home / "sessions").mkdir()


def test_agent_is_installed_detects_source_and_venv(tmp_path):
    lailaba_home = tmp_path / ".lailaba"
    lailaba_home.mkdir()
    assert gu.agent_is_installed(lailaba_home) is False
    _make_agent(lailaba_home)
    assert gu.agent_is_installed(lailaba_home) is True


def test_agent_is_installed_venv_only(tmp_path):
    """A checkout with only a venv (no package dir yet) still counts."""
    lailaba_home = tmp_path / ".lailaba"
    (lailaba_home / "lailaba-agent" / "venv").mkdir(parents=True)
    assert gu.agent_is_installed(lailaba_home) is True


def test_source_built_artifacts_lists_known_paths(tmp_path):
    lailaba_home = tmp_path / ".lailaba"
    _make_gui_build(lailaba_home)
    artifacts = gu.source_built_gui_artifacts(lailaba_home)
...
def test_gui_is_installed_true_when_built(tmp_path, monkeypatch):
    ...
    assert gu.gui_is_installed(lailaba_home) is True
...
def test_gui_is_installed_false_when_nothing(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(gu, "packaged_gui_app_paths", lambda: [])
    monkeypatch.setattr(gu, "desktop_userdata_dir", lambda: tmp_path / "nope")
    assert gu.gui_is_installed(lailaba_home) is False
...
def test_uninstall_gui_removes_only_gui_artifacts(tmp_path, monkeypatch):
    ...
    lailaba_home = tmp_path / ".lailaba"
    agent_root = _make_agent(lailaba_home)
    _make_gui_build(lailaba_home)
    _make_user_data(lailaba_home)
    ...
    removed = gu.uninstall_gui(lailaba_home)
    ...
    assert not (lailaba_home / "desktop-build-stamp.json").exists()
    assert "dist" in removed_names

    # Agent + user data preserved.
    assert (agent_root / "lailaba_cli" / "__init__.py").exists()
    assert (agent_root / "venv").exists()
    assert (lailaba_home / "config.yaml").exists()
    assert (lailaba_home / ".env").exists()
    assert (lailaba_home / "sessions").exists()
...
def test_uninstall_gui_removes_userdata(tmp_path, monkeypatch):
    lailaba_home = tmp_path / ".lailaba"
    _make_agent(lailaba_home)
    userdata = tmp_path / "Lailaba-userdata"
    userdata.mkdir()
...
def test_uninstall_gui_keeps_userdata_when_requested(tmp_path, monkeypatch):
    ...
    gu.uninstall_gui(lailaba_home, remove_userdata=False)
...
def test_gui_install_summary_shape(tmp_path, monkeypatch):
    ...
    summary = gu.gui_install_summary(lailaba_home)
    ...
    assert summary["lailaba_home"] == str(lailaba_home)
    assert summary["platform"] == sys.platform


def test_userdata_dir_per_platform(monkeypatch):
    """userData path matches Electron's app.getPath('userData') for "Lailaba"."""
    home = Path("/home/tester")
    monkeypatch.setattr(Path, "home", classmethod(lambda cls: home))

    monkeypatch.setattr(gu.sys, "platform", "darwin")
    assert gu.desktop_userdata_dir() == home / "Library" / "Application Support" / "Lailaba"

    monkeypatch.setattr(gu.sys, "platform", "linux")
    monkeypatch.delenv("XDG_CONFIG_HOME", raising=False)
    assert gu.desktop_userdata_dir() == home / ".config" / "Lailaba"
...
def test_userdata_dir_windows(monkeypatch):
    ...
    assert gu.desktop_userdata_dir() == Path(r"C:\Users\tester\AppData\Roaming") / "Lailaba"
...
def test_run_uninstall_yes_keep_data_is_non_interactive(tmp_path, monkeypatch):
    ...
    LAILABA_HOME with all the destructive externals stubbed out.
    """
    import lailaba_cli.uninstall as uninstall
    ...
    monkeypatch.setattr(uninstall, "get_lailaba_home", lambda: lailaba_home)
    ...
    from lailaba_cli import gui_uninstall as gu_mod
    ...
    assert not (desktop / "release").exists()
    assert (lailaba_home / "config.yaml").exists()
    assert lailaba_home.exists()


def test_run_uninstall_yes_full_wipes_home(tmp_path, monkeypatch):
    """``--yes --full`` removes the whole LAILABA_HOME non-interactively."""
    import lailaba_cli.uninstall as uninstall

    lailaba_home = tmp_path / ".lailaba"
    (lailaba_home / "lailaba-agent" / "lailaba_cli").mkdir(parents=True)
    (lailaba_home / "config.yaml").write_text("x: 1\n")
    fake_code = tmp_path / "checkout"
    fake_code.mkdir()
    ...
    assert not lailaba_home.exists()


def test_uninstall_module_main_gui_mode(tmp_path, monkeypatch):
    """`python -m lailaba_cli.uninstall --mode gui` runs the GUI-only path.
    ...
    monkeypatch.setattr(gu_mod, "packaged_gui_app_paths", lambda: [])
    monkeypatch.setattr(gu_mod, "desktop_userdata_dir", lambda: tmp_path / "none")
    monkeypatch.setattr(gu_mod, "get_lailaba_home", lambda: lailaba_home)
    ...
    assert not (desktop / "release").exists()
    assert not (lailaba_home / "desktop-build-stamp.json").exists()
    assert (agent_root / "lailaba_cli").exists()
    assert (lailaba_home / "config.yaml").exists()
