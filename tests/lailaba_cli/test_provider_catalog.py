"""Tests for the unified provider catalog (lailaba_cli.provider_catalog).

These are invariant tests, not snapshots: they assert the parity *contract*
between what ``lailaba model`` shows (``CANONICAL_PROVIDERS``) and what the
...
from lailaba_cli.provider_catalog import (
    ProviderDescriptor,
    provider_catalog,
...
def test_catalog_covers_every_lailaba_model_provider():
    """PARITY CONTRACT: the catalog == the `lailaba model` universe."""
    slugs = {d.slug for d in provider_catalog()}
    for entry in CANONICAL_PROVIDERS:
        assert entry.slug in slugs, (
            f"{entry.slug} is shown in `lailaba model` but missing from provider_catalog()"
        )


def test_catalog_has_no_providers_outside_lailaba_model():
    """The catalog must not invent providers `lailaba model` doesn't show."""
