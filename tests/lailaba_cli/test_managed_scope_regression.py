...
def lailaba_home(tmp_path, monkeypatch):
    ...
    # /etc/lailaba on the dev/CI box can't influence the test.
    monkeypatch.setenv("LAILABA_MANAGED_DIR", str(tmp_path / "no_such_managed_dir"))
...
def test_user_config_overrides_default(lailaba_home, monkeypatch):
    from lailaba_cli.config import load_config, cfg_get
...
def test_env_expansion_in_user_config(lailaba_home, monkeypatch):
    from lailaba_cli.config import load_config, cfg_get
...
def test_no_managed_dir_means_user_value_wins(lailaba_home):
...
def test_user_env_overrides_shell(tmp_path, monkeypatch):
    ...
    load_lailaba_dotenv(lailaba_home=str(home))
