...
``lailaba_cli.main`` skips eager plugin discovery at argparse-setup time
when the invocation is clearly targeting a known built-in subcommand.
This saves 500-650ms on ``lailaba --help``, ``lailaba version``,
``lailaba logs``, etc., by not importing ``google.cloud.pubsub_v1``,
...
def _live_subcommand_names() -> set[str]:
    """Run ``lailaba --help`` in-process and parse the subcommand block.
    ...
    built-in-only set.
    """
    from lailaba_cli import main as _main

    argv_backup = sys.argv[:]
    sys.argv = ["lailaba", "--help"]
    buf = io.StringIO()
    try:
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (["lailaba", "-w", "--tui"], None),
        (["lailaba", "version"], "version"),
        (["lailaba", "--tui", "chat"], "chat"),
        (["lailaba", "-w", "logs"], "logs"),
        (["lailaba", "chat", "hello world"], "chat"),
        (["lailaba", "gateway", "run"], "gateway"),
        # Top-level value-taking flags: the value should be skipped.
        (["lailaba", "-m", "gpt5", "chat"], "chat"),
        (["lailaba", "--model", "gpt5", "chat", "hi"], "chat"),
        (["lailaba", "-m", "gpt5", "--provider", "openai", "chat"], "chat"),
        (["lailaba", "-z", "hello world"], None),
        (["lailaba", "-z", "hello", "chat"], "chat"),
        (["lailaba", "--model=gpt5", "chat"], "chat"),     # inline form
        (["lailaba", "--", "chat"], "chat"),               # -- terminator
        (["lailaba", "-w", "--"], None),
        # Unknown positional after skipped flags → plugin-cmd candidate.
        (["lailaba", "some-plugin-cmd"], "some-plugin-cmd"),
        (["lailaba", "-m", "gpt5", "some-plugin-cmd"], "some-plugin-cmd"),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ["lailaba"],                          # bare → chat
        ["lailaba", "--help"],                # top-level help
        ["lailaba", "-h"],
        ["lailaba", "version"],               # known built-in
        ["lailaba", "logs"],
        ["lailaba", "gateway", "run"],
        ["lailaba", "--tui"],
        ["lailaba", "-w", "--tui"],
        ["lailaba", "chat", "hi"],
        ["lailaba", "help"],                  # accepted built-in-ish
        ["lailaba", "-m", "gpt5", "chat"],    # flag-value-skipping
    ],
)
...
@pytest.mark.parametrize(
    "argv",
    [
        ["lailaba", "meet", "join"],          # potential google_meet plugin
        ["lailaba", "honcho", "status"],      # potential memory plugin
        ["lailaba", "unknown-subcmd"],
    ],
)
...
def test_builtin_set_covers_every_registered_subcommand():
    ...
    # in the set so ``lailaba help <cmd>`` skips discovery; it won't show
    ...
    assert not missing_from_declaration, (
        ...
        f"lailaba_cli/main.py::_BUILTIN_SUBCOMMANDS so plugin discovery "
