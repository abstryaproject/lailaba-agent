...
Covers the three paths changed by fix/bedrock-provider-model-ids-live-discovery:
  ...
  2. list_authenticated_providers() Section 2 (LAILABA_OVERLAYS) — bedrock
...
# ---------------------------------------------------------------------------
# 2. list_authenticated_providers() — bedrock via LAILABA_OVERLAYS (Section 2)
# ---------------------------------------------------------------------------
...
class TestBedrockOverlayRegistration:
    """bedrock entry in LAILABA_OVERLAYS is correctly configured."""

    def test_bedrock_overlay_exists(self):
        from lailaba_cli.providers import LAILABA_OVERLAYS
        assert "bedrock" in LAILABA_OVERLAYS

    def test_bedrock_overlay_transport(self):
        from lailaba_cli.providers import LAILABA_OVERLAYS
        assert LAILABA_OVERLAYS["bedrock"].transport == "bedrock_converse"

    def test_bedrock_overlay_auth_type(self):
        from lailaba_cli.providers import LAILABA_OVERLAYS
        assert LAILABA_OVERLAYS["bedrock"].auth_type == "aws_sdk"
