"""Tests for the `lailaba memory reset` CLI command.
...
- Profile-scoped reset (uses LAILABA_HOME)
"""
...
def memory_env(tmp_path, monkeypatch):
    """Set up a fake LAILABA_HOME with memory files."""
    lailaba_home = tmp_path / ".lailaba"
    memories = lailaba_home / "memories"
    memories.mkdir(parents=True)
    monkeypatch.setenv("LAILABA_HOME", str(lailaba_home))

    # Create sample memory files
    (memories / "MEMORY.md").write_text(
        "§\nHermes repo is at ~/.lailaba/lailaba-agent\n§\nUser prefers dark themes",
        encoding="utf-8",
    )
...
def _run_memory_reset(target="all", yes=False, monkeypatch=None, confirm_input="no"):
    ...
    Simulates what happens when `lailaba memory reset` is run.
    """
...
class TestMemoryReset:
    ...
    def test_reset_no_files_exist(self, tmp_path, monkeypatch):
        ...
        lailaba_home = tmp_path / ".lailaba"
        (lailaba_home / "memories").mkdir(parents=True)
        monkeypatch.setenv("LAILABA_HOME", str(lailaba_home))
    ...
    def test_reset_profile_scoped(self, tmp_path, monkeypatch):
        """Reset should work on the active profile's LAILABA_HOME."""
    ...
    def test_reset_empty_memories_dir(self, tmp_path, monkeypatch):
        ...
        # The memories dir won't exist; get_lailaba_home() / "memories" won't have files
        result = _run_memory_reset(target="all", yes=True)
        assert result == "nothing"
