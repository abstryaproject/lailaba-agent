...
Covers:
- Defense 1: gateway stop/restart refuse when _LAILABA_GATEWAY=1
...
class TestGatewayLifecyclePattern:
    ...
    @pytest.mark.parametrize("text", [
        ...
        "lailaba  gateway  restart",         # double spaces
        "Hermez Gateway Restart".lower().replace("z", "s"),  # case handled
        "LAILABA GATEWAY RESTART",           # uppercase
    ])
    def test_lailaba_gateway_commands(self, text):
        assert _contains_gateway_lifecycle_command(text), f"Should match: {text!r}"

    @pytest.mark.parametrize("text", [
        "launchctl kickstart gui/501/ai.lailaba.gateway",
        "launchctl unload ~/Library/LaunchAgents/ai.lailaba.gateway.plist",
        "launchctl stop ai.lailaba.gateway",
        "systemctl restart lailaba-gateway",
        "systemctl stop lailaba-gateway.service",
        "systemctl start lailaba-gateway",
    ])
    def test_service_manager_commands(self, text):
        assert _contains_gateway_lifecycle_command(text), f"Should match: {text!r}"

    @pytest.mark.parametrize("text", [
        "kill lailaba gateway process",
        "pkill -f lailaba.*gateway",
    ])
    ...
    @pytest.mark.parametrize("text", [
        ...
        "lailaba cron list",
        "lailaba update",
        "lailaba config set model claude",
...
class TestCronCreateLifecycleBlock:
    ...
    def test_block_lailaba_gateway_restart(self, capsys):
        args = Namespace(
            cron_command="create",
            schedule="30m",
            prompt="Upgrade lailaba then run lailaba gateway restart",
            name=None,
            deliver=None,
            repeat=None,
            skill=None,
    ...
    def test_block_launchctl_kickstart(self, capsys):
        args = Namespace(
            cron_command="create",
            schedule="0 9 * * *",
            prompt="Run launchctl kickstart -k gui/501/ai.lailaba.gateway",
            name=None,
            deliver=None,
            repeat=None,
            skill=None,
...
class TestGatewaySelfTargetingGuard:
    """Verify lailaba gateway stop/restart refuse when _LAILABA_GATEWAY=1."""

    def test_stop_refuses_inside_gateway(self, monkeypatch):
        monkeypatch.setenv("_LAILABA_GATEWAY", "1")
    ...
    def test_stop_allows_outside_gateway(self, monkeypatch):
        ...
        monkeypatch.delenv("_LAILABA_GATEWAY", raising=False)
...
class TestTerminalToolGatewayLifecycleGuard:
    """terminal_tool must refuse gateway lifecycle commands when _LAILABA_GATEWAY=1.

    Issue #37453: systemctl --user restart lailaba-gateway runs as a child of the
    ...
    def test_force_true_cannot_bypass_block(self, monkeypatch):
        ...
        result = json.loads(tt.terminal_tool(
            command="systemctl restart lailaba-gateway", force=True
        ))
    ...
    def test_safe_systemctl_commands_pass_through(self, monkeypatch):
        """Non-lailaba systemctl commands must not be blocked by this guard."""
        import tools.terminal_tool as tt
    ...
    def test_guard_inactive_outside_gateway(self, monkeypatch):
        """Without _LAILABA_GATEWAY=1 the lifecycle guard must not fire."""
        import tools.terminal_tool as tt
        ...
        result = json.loads(tt.terminal_tool(command="systemctl restart lailaba-gateway"))

        # Outside the gateway the lifecycle guard doesn't block — the normal
        # approval flow handles it (here mocked as approved).
        assert result["exit_code"] == 0
        assert calls == ["systemctl restart lailaba-gateway"]
