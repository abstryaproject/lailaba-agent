...
Before the fix, ``lailaba whatsapp`` wrote ``WHATSAPP_ENABLED=true`` at
...
``lailaba gateway`` then paid a 30s bridge-bootstrap timeout and queued
...
def isolated_home(tmp_path, monkeypatch):
    home = tmp_path / "home"
    lailaba = home / ".lailaba"
    lailaba.mkdir(parents=True)
    monkeypatch.setattr(Path, "home", lambda: home)
    monkeypatch.setenv("LAILABA_HOME", str(lailaba))
...
def _env_value(lailaba_home: Path, key: str) -> str | None:
...
def test_aborted_setup_does_not_enable_whatsapp(isolated_home, monkeypatch):
    ...
    """
    from lailaba_cli.main import cmd_whatsapp
    ...
    monkeypatch.setattr("lailaba_cli.main._require_tty", lambda *_a, **_kw: None)
...
def test_existing_pairing_skip_branch_enables_whatsapp(isolated_home, monkeypatch):
    """User runs ``lailaba whatsapp`` with an existing paired session and
