...
the SAME provider universe as ``lailaba model`` (the CLI/TUI picker).
...
def test_every_lailaba_model_provider_is_configurable_in_desktop():
    """PARITY CONTRACT: GUI (keys ∪ accounts) ⊇ `lailaba model` universe."""
    ...
    assert not missing, (
        "providers shown in `lailaba model` but not configurable in the desktop "
