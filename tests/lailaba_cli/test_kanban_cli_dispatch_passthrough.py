...
def isolated_kanban_home(monkeypatch):
    """Spin up a fresh LAILABA_HOME with a clean kanban DB."""
    test_home = tempfile.mkdtemp(prefix="kanban_cli_passthrough_")
    os.makedirs(os.path.join(test_home, "profiles", "default"), exist_ok=True)
    monkeypatch.setenv("LAILABA_HOME", test_home)
    for mod in list(sys.modules.keys()):
        if mod.startswith("lailaba_cli") or mod.startswith("lailaba_state") or mod == "lailaba_constants":
            del sys.modules[mod]
    yield test_home


def test_cli_dispatch_passes_max_in_progress_from_config(isolated_kanban_home, monkeypatch):
    """#33488: lailaba kanban dispatch must pass kanban.max_in_progress from
    ...
    monkeypatch.setattr(
        "lailaba_cli.config.load_config", lambda: fake_config
    )
...
def test_cli_max_flag_overrides_config_max_spawn(isolated_kanban_home, monkeypatch):
    ...
    monkeypatch.setattr("lailaba_cli.config.load_config", lambda: fake_config)
