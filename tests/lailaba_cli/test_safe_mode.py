"""Tests for `lailaba chat --safe-mode` — pristine troubleshooting runs.
...
_VARS = ("LAILABA_SAFE_MODE", "LAILABA_IGNORE_USER_CONFIG", "LAILABA_IGNORE_RULES")
...
class TestSafeModeEnvWiring:
    ...
    def test_safe_mode_sets_all_gates(self):
        # Mirrors the cmd_chat logic in lailaba_cli/main.py.
        class Args:
            safe_mode = True
        ...
        assert os.environ.get("LAILABA_SAFE_MODE") == "1"
...
class TestSafeModePluginDiscovery:
    """Plugin discovery must be a no-op under LAILABA_SAFE_MODE=1."""

    def test_discovery_skipped(self, monkeypatch):
        monkeypatch.setenv("LAILABA_SAFE_MODE", "1")
    ...
    def test_discovery_runs_without_safe_mode(self, monkeypatch):
        monkeypatch.delenv("LAILABA_SAFE_MODE", raising=False)
        from lailaba_cli.plugins import PluginManager
...
class TestSafeModeMCP:
    """_load_mcp_config must return no servers under LAILABA_SAFE_MODE=1."""
...
class TestSafeModeParser:
    """--safe-mode must parse on both the root parser and `lailaba chat`."""
