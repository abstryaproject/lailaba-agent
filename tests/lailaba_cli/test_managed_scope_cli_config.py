...
lailaba_cli.config._load_config_impl, so the managed-scope merge has to be
applied in BOTH places or the interactive CLI/TUI surface (skin, display prefs)
silently ignores administrator-pinned values while `lailaba config`/`doctor`
...
def _load_cli_config(home):
    ...
    cli.py binds ``_lailaba_home = get_lailaba_home()`` at import time (module
    singleton), so monkeypatching LAILABA_HOME after import doesn't move it.
    ...
    this only matters for tests that swap LAILABA_HOME.)
    """
    import cli

    cli._lailaba_home = home
