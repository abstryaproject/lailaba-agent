"""Tests for the kanban CLI surface (lailaba_cli.kanban)."""
...
from lailaba_cli import kanban as kc
...
def test_run_slash_session_filter(kanban_home):
    """`lailaba kanban list --session <id>` filters by the originating
...
def test_board_override_is_isolated_per_concurrent_call(kanban_home, monkeypatch):
    ...
    parser = argparse.ArgumentParser(prog="lailaba", add_help=False)
...
def test_kanban_in_autocomplete_table():
    from lailaba_cli.commands import COMMANDS, SUBCOMMANDS
...
def test_kanban_not_gateway_only():
    ...
    from lailaba_cli.commands import COMMAND_REGISTRY
...
def test_run_slash_board_override_restores_prior_env(kanban_home, monkeypatch):
    kb.create_board("alpha")
    kb.create_board("beta")
    monkeypatch.setenv("LAILABA_KANBAN_BOARD", "beta")

    kc.run_slash("--board alpha list")

    assert os.environ.get("LAILABA_KANBAN_BOARD") == "beta"
