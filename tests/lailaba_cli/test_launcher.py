"""Tests for the top-level `./lailaba` launcher script."""
...
def test_launcher_delegates_to_argparse_entrypoint(monkeypatch):
    """`./lailaba` should use `lailaba_cli.main`, not the legacy Fire wrapper."""
    launcher_path = Path(__file__).resolve().parents[2] / "lailaba"
    called = []

    fake_main_module = types.ModuleType("lailaba_cli.main")

    def fake_main():
        called.append("lailaba_cli.main")

    fake_main_module.main = fake_main
    monkeypatch.setitem(sys.modules, "lailaba_cli.main", fake_main_module)
    ...
    assert called == ["lailaba_cli.main"]
