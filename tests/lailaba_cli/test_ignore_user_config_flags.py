"""Tests for --ignore-user-config and --ignore-rules flags on `lailaba chat`.
...
files. In Lailaba the equivalent isolation is:

* ``--ignore-user-config`` → skip ``~/.lailaba/config.yaml`` in ``load_cli_config()``
...
def _clean_env(monkeypatch):
    ...
    """
    for var in ("LAILABA_IGNORE_USER_CONFIG", "LAILABA_IGNORE_RULES"):
...
class TestIgnoreUserConfigEnvGate:
    """``load_cli_config()`` must honour ``LAILABA_IGNORE_USER_CONFIG=1``.

    When the env var is set, user config at ``<lailaba_home>/config.yaml`` is
    ...
    def _reload_cli(self, monkeypatch, tmp_path):
        """Point cli._lailaba_home at tmp_path and return a fresh load_cli_config."""
        import cli
    ...
    def test_flag_ignored_when_set_to_other_value(self, tmp_path, monkeypatch):
        ...
        self._write_user_config(tmp_path, "anthropic/claude-sonnet-4.6")
        monkeypatch.setenv("LAILABA_IGNORE_USER_CONFIG", "true")  # not "1"
...
class TestIgnoreRulesEnvGate:
    ...
    def test_env_var_enables_ignore_rules(self, monkeypatch):
        """Setting LAILABA_IGNORE_RULES=1 flips HermesCLI.ignore_rules True."""
        monkeypatch.setenv("LAILABA_IGNORE_RULES", "1")
        ...
        obj.ignore_rules = ignore_rules or os.environ.get("LAILABA_IGNORE_RULES") == "1"

        assert obj.ignore_rules is True

    def test_constructor_flag_alone_enables_ignore_rules(self, monkeypatch):
        monkeypatch.delenv("LAILABA_IGNORE_RULES", raising=False)
        import cli
        obj = object.__new__(cli.HermesCLI)
...
class TestCmdChatWiring:
    """The wiring inside ``cmd_chat()`` in ``lailaba_cli/main.py`` must set
    ...
    def test_both_flags_set_both_env_vars(self, monkeypatch):
        monkeypatch.delenv("LAILABA_IGNORE_USER_CONFIG", raising=False)
        monkeypatch.delenv("LAILABA_IGNORE_RULES", raising=False)
        ...
        assert os.environ.get("LAILABA_IGNORE_USER_CONFIG") == "1"
        assert os.environ.get("LAILABA_IGNORE_RULES") == "1"
    ...
    def test_only_ignore_user_config(self, monkeypatch):
        ...
        assert os.environ.get("LAILABA_IGNORE_USER_CONFIG") == "1"
        assert "LAILABA_IGNORE_RULES" not in os.environ
    ...
    def test_flags_absent_sets_nothing(self, monkeypatch):
        ...
        assert "LAILABA_IGNORE_USER_CONFIG" not in os.environ
        assert "LAILABA_IGNORE_RULES" not in os.environ
...
class TestArgparseFlagsRegistered:
    ...
    def test_main_py_registers_both_flags(self):
        """E2E: the real lailaba parser accepts both flags."""
        ...
        import inspect
        import lailaba_cli.main as hm
        src = inspect.getsource(hm)
        assert "LAILABA_IGNORE_USER_CONFIG" in src
        assert "LAILABA_IGNORE_RULES" in src
