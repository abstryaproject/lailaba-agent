...
Covers:
 - Normal path (tokens already in Lailaba auth store)
 - Claude Code fallback (tokens only in ~/.claude/.credentials.json)
 - Negative case (no credentials anywhere)

Note: auto-import from ~/.codex/auth.json was removed in #12360 — Lailaba
now owns its own openai-codex auth state, and users explicitly adopt
existing Codex CLI tokens via `lailaba auth openai-codex`. The old
...
@pytest.fixture()
def lailaba_auth_only_env(tmp_path, monkeypatch):
    """Tokens already in Lailaba auth store (no Codex CLI needed)."""
...
def test_normal_path_still_works(lailaba_auth_only_env):
    """openai-codex appears when tokens are already in Lailaba auth store."""
...
def test_codex_picker_uses_live_codex_catalog(lailaba_auth_only_env, tmp_path, monkeypatch):
...
def claude_code_only_env(tmp_path, monkeypatch):
    ...
    ~/.claude/.credentials.json (Claude Code) — not in env vars or Lailaba
    auth store."""
    lailaba_home = tmp_path / ".lailaba"
    lailaba_home.mkdir()
