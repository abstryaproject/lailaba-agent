"""Tests for IRC gateway configuration via `lailaba setup gateway` UI.

Covers the full plugin-platform discovery → status → configure flow so that
a fresh Lailaba install (no state, no env vars) can set up IRC through the
...
class TestIRCFreshInstallDiscovery:
    """IRC appears in the setup menu on a brand-new Lailaba install."""
    ...
    def test_irc_status_configured_when_env_set(self, monkeypatch):
        ...
        try:
            ...
            monkeypatch.setenv("IRC_CHANNEL", "#lailaba")
            monkeypatch.setenv("IRC_NICKNAME", "lailaba-bot")
...
class TestIRCGatewaySetupFreshInstall:
    """Simulate the full `lailaba setup gateway` experience with IRC present."""
