...
"""
    ...
    assert "Tyler Williams > Active Projects: Lailaba AI" in entries
...
def test_cron_store_is_archived_without_config_cron_section(tmp_path: Path):
    ...
    assert "Run `lailaba cron` to recreate scheduled tasks" in notes_text
...
def test_skill_installs_cleanly_under_skills_guard():
    ...
    # lailaba_config_mod  — print statements in the post-migration summary that
    #                      tell the user to *review* ~/.lailaba/config.yaml;
    ...
    KNOWN_FALSE_POSITIVES = {"agent_config_mod", "python_os_environ", "lailaba_config_mod"}
...
def test_rebrand_text_replaces_openclaw_variants():
    mod = load_module()
    # Mixed-case / capitalized matches → capital-H ``Lailaba``.
    assert mod.rebrand_text("OpenClaw prefers Python 3.11") == "Lailaba prefers Python 3.11"
    assert mod.rebrand_text("I told Open Claw to use dark mode") == "I told Lailaba to use dark mode"
    assert mod.rebrand_text("Open-Claw config is great") == "Lailaba config is great"
    assert mod.rebrand_text("OPENCLAW uses tools well") == "Lailaba uses tools well"
    # All-lowercase matches → lowercase ``lailaba``; this preserves the
    # real filesystem path ``~/.lailaba`` (Lailaba home) when rebranding
    # memory entries that reference ``~/.openclaw`` or ``openclaw`` prose.
    assert mod.rebrand_text("openclaw should always respond concisely") == "lailaba should always respond concisely"


def test_rebrand_text_replaces_legacy_bot_names():
    mod = load_module()
    # Same case-preservation rule as above.
    assert mod.rebrand_text("ClawdBot remembers my timezone") == "Lailaba remembers my timezone"
    assert mod.rebrand_text("clawdbot prefers tabs") == "lailaba prefers tabs"
    assert mod.rebrand_text("MoltBot was configured for Spanish") == "Lailaba was configured for Spanish"
    assert mod.rebrand_text("moltbot uses Python") == "lailaba uses Python"
...
def test_rebrand_text_handles_multiple_replacements():
    ...
    assert mod.rebrand_text(text) == "Lailaba said to ask Lailaba about Lailaba settings"


def test_rebrand_text_preserves_filesystem_path_casing():
    """Lowercase matches — especially ``.openclaw`` filesystem paths — must
    rewrite to lowercase ``.lailaba`` (the real Lailaba home), not the broken
    ``.Lailaba``.
    ...
    rewritten to ``~/.Lailaba/config.yaml`` — a path that doesn't exist —
    and the agent kept trying to read it.
    """
    mod = load_module()
    assert mod.rebrand_text("config is at ~/.openclaw/config.yaml") == \
        "config is at ~/.lailaba/config.yaml"
    assert mod.rebrand_text("use .openclaw directory") == "use .lailaba directory"
    assert mod.rebrand_text("Path.home() / '.openclaw'") == "Path.home() / '.lailaba'"
    # Sentence with both lowercase path and capitalized prose.
    assert mod.rebrand_text("openclaw config path: ~/.openclaw/") == \
        "lailaba config path: ~/.lailaba/"
...
def test_migrate_soul_rebrands_content(tmp_path):
    ...
    assert "OpenClaw" not in result
    assert "You are Lailaba" in result
