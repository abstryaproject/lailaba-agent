...
binary download are exercised manually by `lailaba secrets bitwarden
...
def lailaba_home(tmp_path, monkeypatch):
    """Point Lailaba at an isolated home directory."""
    home = tmp_path / ".lailaba"
    home.mkdir()
    monkeypatch.setenv("LAILABA_HOME", str(home))
    # Some modules cache get_lailaba_home; clear if needed.
    import lailaba_constants
    if hasattr(lailaba_constants, "_LAILABA_HOME_CACHE"):
        lailaba_constants._LAILABA_HOME_CACHE = None  # type: ignore[attr-defined]
    return home
...
def test_install_bws_rejects_malicious_member(lailaba_home, monkeypatch):
...
def test_install_bws_happy_path(lailaba_home, monkeypatch):
...
def test_install_bws_checksum_mismatch(lailaba_home, monkeypatch):
...
def test_install_bws_missing_checksum_entry(lailaba_home, monkeypatch):
...
def test_env_loader_skips_when_disabled(tmp_path, monkeypatch):
    ...
    from lailaba_cli.env_loader import _apply_external_secret_sources
...
def test_disk_cache_written_after_first_fetch(monkeypatch, tmp_path):
    """First fetch hits bws AND writes a 0600 file under lailaba_home/cache/."""
    home = tmp_path / ".lailaba"
    home.mkdir()
    fake_binary = tmp_path / "bws"
    fake_binary.write_text("")
