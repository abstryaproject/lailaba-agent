...
Run from the PR worktree:
    ...
    cd ~/.lailaba/lailaba-agent/.worktrees/browser-providers-plugin
...
# checkout lives two levels up at ``~/.lailaba/lailaba-agent``.
MAIN_DIR = REPO_ROOT.parent.parent  # ~/.lailaba/lailaba-agent
PR_DIR = REPO_ROOT  # the worktree we're in
assert (MAIN_DIR / "tools" / "browser_tool.py").exists(), (
    f"MAIN_DIR={MAIN_DIR} doesn't look like a lailaba-agent checkout"
)
...
# Isolated LAILABA_HOME for the config write.
