...
class TestScanCronSkillAssembled:
    ...
    def test_descriptive_attack_command_prose_allowed(self):
        ...
        Real example: the `lailaba-agent-dev` skill contains a postmortem
        section saying 'the attacker could just cat ~/.lailaba/.env'.
        """
        assert _scan_cron_skill_assembled(
            "the attacker could just cat ~/.lailaba/.env to steal credentials"
        )[1] == ""
        assert _scan_cron_skill_assembled(
...
class TestCronjobRequirements:
    def test_requires_no_crontab_binary(self, monkeypatch):
        ...
        # because lailaba uses an internal scheduler, not system crontab.
    ...
    def test_accepts_exec_ask(self, monkeypatch):
        monkeypatch.delenv("LAILABA_INTERACTIVE", raising=False)
        monkeypatch.delenv("LAILABA_GATEWAY_SESSION", raising=False)
        monkeypatch.setenv("LAILABA_EXEC_ASK", "1")
    ...
    def test_rejects_false_like_interactive_env(self, monkeypatch, false_like_value):
        monkeypatch.setenv("LAILABA_INTERACTIVE", false_like_value)
        monkeypatch.delenv("LAILABA_GATEWAY_SESSION", raising=False)
        monkeypatch.delenv("LAILABA_EXEC_ASK", raising=False)
        assert check_cronjob_requirements() is False
    ...
    @pytest.mark.parametrize(
        "var_name",
        ["LAILABA_INTERACTIVE", "LAILABA_GATEWAY_SESSION", "LAILABA_EXEC_ASK"],
    )
    @pytest.mark.parametrize("false_like_value", ["0", "false", "no", "off"])
    def test_rejects_false_like_any_session_env(
        ...
        for v in ("LAILABA_INTERACTIVE", "LAILABA_GATEWAY_SESSION", "LAILABA_EXEC_ASK"):
...
class TestResolveModelOverride:
    ...
    def test_keeps_bare_custom_when_a_named_entry_exists(self, monkeypatch):
        import lailaba_cli.runtime_provider as rp_mod
    ...
    def test_pins_main_provider_when_bare_custom_unresolvable(self, monkeypatch):
        import lailaba_cli.config as cfg_mod
        import lailaba_cli.runtime_provider as rp_mod
