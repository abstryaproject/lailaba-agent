"""Contract tests for the Docker image's immutable /opt/lailaba install tree."""
...
def test_dockerfile_makes_opt_lailaba_readonly_for_lailaba_user() -> None:
    ...
    assert "chown -R root:root /opt/lailaba" not in text
    assert "chmod -R a+rX /opt/lailaba" not in text
    assert "chmod -R a-w /opt/lailaba" not in text


def test_dockerfile_keeps_mutable_state_under_opt_data() -> None:
    text = _dockerfile_text()

    assert "ENV LAILABA_HOME=/opt/data" in text
    assert "ENV LAILABA_WRITE_SAFE_ROOT=/opt/data" in text
...
def test_dockerfile_disables_runtime_install_mutations() -> None:
    ...
    assert "ENV LAILABA_DISABLE_LAZY_INSTALLS=1" in text
    assert "LAILABA_TUI_DIR=/opt/lailaba/ui-tui" in text


def test_dockerfile_does_not_chown_install_trees_to_lailaba() -> None:
    text = _dockerfile_text()
    forbidden_patterns = (
        r"chown\s+-R\s+lailaba:lailaba\s+/opt/lailaba/\.venv",
        r"chown\s+-R\s+lailaba:lailaba\s+/opt/lailaba/ui-tui",
        r"chown\s+-R\s+lailaba:lailaba\s+/opt/lailaba/gateway",
        r"chown\s+-R\s+lailaba:lailaba\s+/opt/lailaba/node_modules",
    )
    for pattern in forbidden_patterns:
        assert not re.search(pattern, text), (
            "runtime install trees under /opt/lailaba must stay immutable; "
...
def test_dockerfile_bakes_code_scoped_install_method_stamp() -> None:
    ...
    (/opt/lailaba/.install_method) first; baking it at build time keeps the
    published image self-identifying as 'docker' WITHOUT writing into the
    shared $LAILABA_HOME data volume (which a host install may also use).
    The stamp is created by root in the shim-wiring RUN block; the lailaba
    user can't modify it (go-w from the --chmod on the source COPY).
    """
    text = _dockerfile_text()
    assert "printf 'docker\\n' > /opt/lailaba/.install_method" in text

    # The stamp must be in the RUN block that wires the exec shim.
    shim_block = re.search(
        r"RUN mkdir -p /opt/lailaba/bin && \\\n"
        r"(?:.*\\\n)+?"
...
def test_dockerfile_redirects_lazy_installs_to_durable_target() -> None:
    ...
    # never under the immutable /opt/lailaba tree.
    assert f"ENV LAILABA_LAZY_INSTALL_TARGET={target}" in text
    assert target.startswith("/opt/data/"), "target must be on the durable volume"
    assert "ENV LAILABA_LAZY_INSTALL_TARGET=/opt/lailaba" not in text
    ...
    # succeed as the unprivileged lailaba runtime user.
    stage2 = (REPO_ROOT / "docker" / "stage2-hook.sh").read_text()
    assert '"$LAILABA_HOME/lazy-packages"' in stage2, (
