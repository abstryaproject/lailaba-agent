...
def _skill_dir(tmp_path):
    ...
    only the temp directory — not the real ~/.lailaba/skills/."""
...
class TestSecurityScanGate:
    ...
    def test_guard_flag_reads_config_default_false(self):
        ...
        with patch("lailaba_cli.config.load_config", return_value={"skills": {}}):
    ...
    def test_guard_flag_reads_config_when_set(self):
        ...
        with patch("lailaba_cli.config.load_config",
...
class TestExternalSkillMutations:
    ...
    caused agents to create duplicate copies in ~/.lailaba/skills/ as a
    workaround.
    """
...
# come up. The user unpins via `lailaba curator unpin <name>` to delete.
# ---------------------------------------------------------------------------
...
class TestPinnedGuard:
    ...
    def test_delete_refuses_pinned(self, tmp_path):
        ...
        assert "lailaba curator unpin my-skill" in result["error"]
        # Skill still exists
        assert (tmp_path / "my-skill" / "SKILL.md").exists()
...
def _curator_pass(tmp_path, *, monkeypatch):
    ...
    Points LAILABA_HOME at ``tmp_path/.lailaba`` so skill_usage's archive path
    (``get_lailaba_home()``) resolves into the same tree the skill manager
    searches, and flips ``is_background_review()`` → True so the consolidation
    guard fires.
    """
    lailaba_home = tmp_path / ".lailaba"
    skills_root = lailaba_home / "skills"
...
class TestCuratorConsolidationDeleteGuard:
    ...
    def test_dispatcher_preserves_usage_record_on_curator_archive(self, tmp_path, monkeypatch):
        ...
        # `lailaba curator restore` can bring it back.
