...
class TestFirecrawlClientConfig:
    ...
    def test_nous_auth_token_respects_lailaba_home_override(self, tmp_path):
        """Auth lookup should read from LAILABA_HOME/auth.json, not ~/.lailaba/auth.json."""
        real_home = tmp_path / "real-home"
        (real_home / ".lailaba").mkdir(parents=True)
...
class TestBackendSelection:
    ...
    ``lailaba tools``.  Falls back to key-based detection for legacy/manual
    setups.
    """
...
class TestCheckWebApiKey:
    ...
    def test_tool_gateway_availability_skips_refresh_for_expired_cached_token(
        ...
        monkeypatch.setattr(
            "lailaba_cli.auth.resolve_nous_access_token",
            _record_refresh,
        )
