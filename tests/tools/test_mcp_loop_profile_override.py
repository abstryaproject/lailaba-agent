"""Regression tests for LAILABA_HOME override propagation onto the MCP loop.
...
get_lailaba_home() inside the coroutine, most visibly OAuth token-store
...
def test_override_propagates_to_mcp_loop(tmp_path, monkeypatch, mcp_loop):
    ...
    process_home.mkdir()
    profile_home.mkdir()
    monkeypatch.setenv("LAILABA_HOME", str(process_home))
    ...
    token = set_lailaba_home_override(str(profile_home))
    try:
