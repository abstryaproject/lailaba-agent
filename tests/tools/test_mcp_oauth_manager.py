...
def test_lailaba_provider_subclass_exists():
    """HermesMCPOAuthProvider is defined and subclasses OAuthClientProvider."""
    from tools.mcp_oauth_manager import _LAILABA_PROVIDER_CLS
    from mcp.client.auth.oauth2 import OAuthClientProvider

    assert _LAILABA_PROVIDER_CLS is not None
    assert issubclass(_LAILABA_PROVIDER_CLS, OAuthClientProvider)
...
def test_manager_builds_lailaba_provider_subclass(tmp_path, monkeypatch):
    """get_or_build_provider returns HermesMCPOAuthProvider, not plain OAuthClientProvider."""
    from tools.mcp_oauth_manager import (
        MCPOAuthManager, _LAILABA_PROVIDER_CLS, reset_manager_for_tests,
    )
    reset_manager_for_tests()
    monkeypatch.setenv("LAILABA_HOME", str(tmp_path))
    _set_interactive_stdin(monkeypatch)
    ...
    assert _LAILABA_PROVIDER_CLS is not None
    assert isinstance(provider, _LAILABA_PROVIDER_CLS)
    assert provider._lailaba_server_name == "srv"
