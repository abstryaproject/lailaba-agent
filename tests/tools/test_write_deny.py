...
class TestWriteDenyExactPaths:
    ...
    def test_lailaba_env(self):
        # ``.env`` under the active LAILABA_HOME (profile-aware, not just
        # ``~/.lailaba``) must be write-denied. The hermetic test conftest
        # points LAILABA_HOME at a tempdir — resolve via get_lailaba_home()
        # to match the denylist.
        from lailaba_constants import get_lailaba_home
        path = str(get_lailaba_home() / ".env")
        assert _is_write_denied(path) is True

    def test_lailaba_root_env_when_running_under_profile(self, tmp_path, monkeypatch):
        ...
        ``~/.lailaba/.env`` (whose credentials are inherited by every profile)
        could be silently overwritten by ``write_file`` while a profile was
        active.
        """
        root = tmp_path / "lailaba_root"
        ...
        # Sanity check: LAILABA_HOME does point to the profile dir, not the root.
        from lailaba_constants import get_lailaba_home, get_default_lailaba_root
        assert get_lailaba_home() == profile_home
        assert get_default_lailaba_root() == root
...
class TestWriteAllowed:
    ...
    def test_lailaba_control_files_requested_writable(self):
        from lailaba_constants import get_lailaba_home
