...
Older images made /opt/lailaba/ui-tui and /opt/lailaba/node_modules writable so a
...
def test_dockerfile_uses_prebuilt_tui_instead_of_writable_node_modules() -> None:
    ...
    assert "ENV LAILABA_TUI_DIR=/opt/lailaba/ui-tui" in text
    assert "cd ../ui-tui && npm run build" in text
    assert "chown -R lailaba:lailaba /opt/lailaba/ui-tui" not in text
    assert "chown -R lailaba:lailaba /opt/lailaba/node_modules" not in text
