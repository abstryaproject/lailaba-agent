...
class TestEnsureInstalled:
    ...
    def test_not_found_returns_none(self, mock_cfg):
        ...
        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._lailaba_bin_dir", return_value="/nonexistent"), \
...
class TestInstallArchiveMemberValidation:
    ...
    def test_install_extracts_regular_tirith_member(self, mock_target, mock_which,
        ...
        assert reason == ""
        assert path == str(lailaba_home / "bin" / "tirith")
    ...
    def test_install_rejects_non_regular_tirith_member(self, mock_target, mock_which,
        ...
        assert not os.path.lexists(lailaba_home / "bin" / "tirith")
...
class TestDiskFailureMarker:
    ...
    def test_install_failed_recovers_from_lailaba_bin(self):
        """After _INSTALL_FAILED, manual install in LAILABA_HOME/bin is picked up."""
        from tools.tirith_security import _resolve_tirith_path, _INSTALL_FAILED
        import tempfile
        tmpdir = tempfile.mkdtemp()
        lailaba_bin = os.path.join(tmpdir, "tirith")
        # Create a fake executable
        with open(lailaba_bin, "w") as f:
            f.write("#!/bin/sh\n")
        os.chmod(lailaba_bin, 0o755)

        _tirith_mod._resolved_path = _INSTALL_FAILED

        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._lailaba_bin_dir", return_value=tmpdir), \
             patch("tools.tirith_security._clear_install_failed") as mock_clear:
            result = _resolve_tirith_path("tirith")
            assert result == lailaba_bin
            assert _tirith_mod._resolved_path == lailaba_bin
            mock_clear.assert_called_once()
...
# ---------------------------------------------------------------------------
# LAILABA_HOME isolation
# ---------------------------------------------------------------------------

class TestHermesHomeIsolation:
    def test_lailaba_bin_dir_respects_lailaba_home(self):
        """_lailaba_bin_dir must use LAILABA_HOME, not hardcoded ~/.lailaba."""
        from tools.tirith_security import _lailaba_bin_dir
        import tempfile
        tmpdir = tempfile.mkdtemp()
        with patch.dict(os.environ, {"LAILABA_HOME": tmpdir}):
            result = _lailaba_bin_dir()
        assert result == os.path.join(tmpdir, "bin")
        assert os.path.isdir(result)

    def test_failure_marker_respects_lailaba_home(self):
        """_failure_marker_path must use LAILABA_HOME, not hardcoded ~/.lailaba."""
        from tools.tirith_security import _failure_marker_path
        with patch.dict(os.environ, {"LAILABA_HOME": "/custom/lailaba"}):
            result = _failure_marker_path()
        assert result == "/custom/lailaba/.tirith-install-failed"

    def test_conftest_isolation_prevents_real_home_writes(self):
        """The conftest autouse fixture sets LAILABA_HOME; verify it's active."""
        lailaba_home = os.getenv("LAILABA_HOME")
        assert lailaba_home is not None, "LAILABA_HOME should be set by conftest"
        assert "lailaba_test" in lailaba_home, "Should point to test temp dir"

    def test_get_lailaba_home_fallback(self):
        """Without LAILABA_HOME set, falls back to the active OS home."""
        from tools.tirith_security import _get_lailaba_home
        with patch.dict(os.environ, {}, clear=True):
            # Remove LAILABA_HOME entirely. With HOME also absent, expanduser
            ...
            expected = os.path.join(os.path.expanduser("~"), ".lailaba")
            result = _get_lailaba_home()
    ...
    def test_mkdtemp_oserror_propagates_to_ensure_installed(self):
        ...
        with patch("tools.tirith_security.tempfile.mkdtemp",
             ...
             patch("tools.tirith_security.shutil.which",
                   return_value=None), \
             patch("tools.tirith_security._lailaba_bin_dir",
