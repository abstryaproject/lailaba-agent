...
def _chown_lailaba_tree_function(text: str) -> str:
...
def _run_helper(
    ...
    log_path: Path,
    *,
    lailaba_home: Path | None = None,
) -> subprocess.CompletedProcess[str]:
    shell = shutil.which("sh")
    if shell is None:
        pytest.skip("sh not available")
    lailaba_home = target if lailaba_home is None else lailaba_home
    script = (
        "set -eu\n"
        f'LAILABA_HOME="{lailaba_home}"\n'
        f"{_chown_lailaba_tree_function(text)}\n"
        f'chown() {{ printf "%s\\n" "$*" >> "{log_path}"; }}\n'
        f'chown_lailaba_tree "{target}"\n'
    )
...
def test_chown_helper_refuses_symlinked_directories(stage2_text: str, tmp_path: Path) -> None:
    ...
    real_home.mkdir()
    symlinked_home = tmp_path / "lailaba-home"
    try:
...
def test_chown_helper_refuses_target_under_symlinked_home(
    ...
    proc = _run_helper(
        ...
        linked_home / "cron",
        log_path,
        lailaba_home=linked_home,
    )

    assert proc.returncode == 0, proc.stderr
    assert not log_path.exists(), "must not chown through a symlinked LAILABA_HOME"
    assert "refusing recursive chown through symlinked path" in proc.stdout
...
def test_stage2_uses_symlink_safe_helper_for_lailaba_home_trees(stage2_text: str) -> None:
    assert 'chown_lailaba_tree "$LAILABA_HOME/$sub"' in stage2_text
    assert 'chown_lailaba_tree "$LAILABA_HOME/profiles"' in stage2_text
    assert 'chown_lailaba_tree "$LAILABA_HOME/cron"' in stage2_text
    assert 'chown -R lailaba:lailaba "$LAILABA_HOME/$sub"' not in stage2_text
    assert 'chown -R lailaba:lailaba "$LAILABA_HOME/profiles"' not in stage2_text
    assert 'chown -R lailaba:lailaba "$LAILABA_HOME/cron"' not in stage2_text


def test_stage2_skips_top_level_chown_for_symlinked_lailaba_home(
    stage2_text: str,
) -> None:
    assert 'refuse_symlinked_path "chown" "$LAILABA_HOME"' in stage2_text
