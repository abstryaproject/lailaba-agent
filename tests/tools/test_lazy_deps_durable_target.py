...
class TestGatingWithTarget:
    """``LAILABA_DISABLE_LAZY_INSTALLS=1`` must STOP blocking once a durable
    ...
    def test_disable_env_blocks_without_target(self, monkeypatch):
        monkeypatch.setenv("LAILABA_DISABLE_LAZY_INSTALLS", "1")
        monkeypatch.delenv(ld._LAZY_TARGET_ENV, raising=False)
        # config unreadable → fails open on the config check, but the sealed
        # env var with no target still blocks.
        monkeypatch.setattr(
            "lailaba_cli.config.load_config", lambda: {}, raising=False
        )
    ...
    def test_normal_mode_unaffected(self, monkeypatch):
        ...
        monkeypatch.delenv("LAILABA_DISABLE_LAZY_INSTALLS", raising=False)
        monkeypatch.delenv(ld._LAZY_TARGET_ENV, raising=False)
        monkeypatch.setattr(
            "lailaba_cli.config.load_config", lambda: {}, raising=False
        )
...
@pytest.mark.skipif(
    os.environ.get("LAILABA_RUN_NETWORK_TESTS") != "1",
    reason="opt-in real-install test (set LAILABA_RUN_NETWORK_TESTS=1); CI runs "
