...
def _restore_tool_modules():
    original_lailaba_home = os.environ.get("LAILABA_HOME")
    original_modules = {
        name: module
        ...
        or name == "lailaba_cli"
        or name.startswith("lailaba_cli.")
        or name == "modal"
    ...
    try:
        ...
        if original_lailaba_home is None:
            os.environ.pop("LAILABA_HOME", None)
        else:
            os.environ["LAILABA_HOME"] = original_lailaba_home
        _reset_modules(("tools", "lailaba_cli", "modal"))
...
def _install_modal_test_modules(
    ...
    *,
    ...
    lailaba_cli = types.ModuleType("lailaba_cli")
    lailaba_cli.__path__ = []  # type: ignore[attr-defined]
    sys.modules["lailaba_cli"] = lailaba_cli
    lailaba_home = tmp_path / "lailaba-home"
    os.environ["LAILABA_HOME"] = str(lailaba_home)
    sys.modules["lailaba_cli.config"] = types.SimpleNamespace(
        get_lailaba_home=lambda: lailaba_home,
    )
    ...
    async def _lookup_aio(_name: str, create_if_missing: bool = False):
        return types.SimpleNamespace(name="lailaba-agent", create_if_missing=create_if_missing)
    ...
    return {
        "snapshot_store": lailaba_home / "modal_snapshots.json",
