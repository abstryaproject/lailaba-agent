...
Covers the canonical fix for issues #4146, #27303, #30882, #33057:
  ...
  4. tools.code_execution_tool._scrub_child_env — broad LAILABA_ prefix dropped,
...
def gw_session(monkeypatch):
    """A clean gateway session: LAILABA_GATEWAY_SESSION set, a bound session
    ...
    monkeypatch.delenv("LAILABA_CRON_SESSION", raising=False)
...
def test_guard_cron_deny_blocks(monkeypatch):
    monkeypatch.setenv("LAILABA_CRON_SESSION", "1")
    monkeypatch.delenv("LAILABA_GATEWAY_SESSION", raising=False)
    monkeypatch.setattr(A, "_get_approval_mode", lambda: "manual")
...
def test_env_scrub_lailaba_allowlist_and_secret_blocks():
    from tools.code_execution_tool import _scrub_child_env

    env = {
        # operational allowlist → kept
        "LAILABA_HOME": "/h", "LAILABA_PROFILE": "p",
        "LAILABA_CONFIG": "/c.yaml", "LAILABA_ENV": "/e",
        # other LAILABA_* → dropped (broad prefix removed)
        "LAILABA_BASE_URL": "https://x", "LAILABA_INTERACTIVE": "1",
        "LAILABA_KANBAN_DB": "postgres://u:p@h/db",
    ...
    for kept in ("LAILABA_HOME", "LAILABA_PROFILE", "LAILABA_CONFIG", "LAILABA_ENV", "PATH"):
        assert kept in out, f"{kept} should be kept"
    for dropped in (
        "LAILABA_BASE_URL", "LAILABA_INTERACTIVE", "LAILABA_KANBAN_DB",
...
def test_env_scrub_logs_dropped_lailaba_vars(caplog):
    """Dropping a non-allowlisted, non-secret LAILABA_* var must be diagnosable:
    ...
    leave users guessing why a sandbox script sees an unset LAILABA_* var."""
    import logging

    from tools.code_execution_tool import _scrub_child_env

    env = {
        "LAILABA_HOME": "/h",          # allowlisted → kept, not logged
        "LAILABA_BASE_URL": "https://x",   # dropped → logged
        "LAILABA_KANBAN_DB": "postgres://u:p@h/db",  # dropped → logged
        "LAILABA_API_KEY": "sk",       # secret → dropped silently (not logged)
        "PATH": "/usr/bin",           # safe prefix → kept
    }
    with caplog.at_level(logging.DEBUG, logger="tools.code_execution_tool"):
        out = _scrub_child_env(env, is_passthrough=lambda _: False, is_windows=False)

    assert "LAILABA_HOME" in out and "PATH" in out
    assert "LAILABA_BASE_URL" not in out and "LAILABA_KANBAN_DB" not in out

    msgs = "\n".join(r.getMessage() for r in caplog.records)
    assert "LAILABA_BASE_URL" in msgs and "LAILABA_KANBAN_DB" in msgs
    assert "env_passthrough" in msgs
    # Secret vars are dropped but must NOT be named in the diagnostic log.
    assert "LAILABA_API_KEY" not in msgs


def test_env_scrub_no_log_when_nothing_dropped(caplog):
    """No diagnostic noise when there are no dropped LAILABA_* vars."""
    import logging
