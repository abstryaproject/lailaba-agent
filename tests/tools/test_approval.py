...
class TestApprovalModeParsing:
    def test_unquoted_yaml_off_boolean_false_maps_to_off(self):
        with mock_patch("lailaba_cli.config.load_config", return_value={"approvals": {"mode": False}}):
            assert _get_approval_mode() == "off"

    def test_string_off_still_maps_to_off(self):
        with mock_patch("lailaba_cli.config.load_config", return_value={"approvals": {"mode": "off"}}):
            assert _get_approval_mode() == "off"
...
class TestSessionKeyContext:
    def test_context_session_key_overrides_process_env(self):
        ...
        try:
            with mock_patch.dict("os.environ", {"LAILABA_SESSION_KEY": "bob"}, clear=False):
...
class TestTeePattern:
    ...
    def test_tee_lailaba_env(self):
        dangerous, key, desc = detect_dangerous_command("echo x | tee ~/.lailaba/.env")
        assert dangerous is True
        assert key is not None
    ...
    def test_tee_custom_lailaba_home_env(self):
        dangerous, key, desc = detect_dangerous_command("echo x | tee $LAILABA_HOME/.env")
        assert dangerous is True
        assert key is not None

    def test_tee_quoted_custom_lailaba_home_env(self):
        dangerous, key, desc = detect_dangerous_command('echo x | tee "$LAILABA_HOME/.env"')
        assert dangerous is True
        assert key is not None
...
class TestHermesConfigWriteProtection:
    ...
    ~/.lailaba/config.yaml (#14639). config.yaml IS the security policy
    ...
    def test_redirect_overwrite(self):
        dangerous, key, desc = detect_dangerous_command("echo 'approvals:' > ~/.lailaba/config.yaml")
        assert dangerous is True
        assert key is not None

    def test_append(self):
        dangerous, key, desc = detect_dangerous_command("echo '  mode: off' >> ~/.lailaba/config.yaml")
        assert dangerous is True

    def test_tee(self):
        dangerous, key, desc = detect_dangerous_command("echo x | tee ~/.lailaba/config.yaml")
        assert dangerous is True

    def test_cp_over_config(self):
        dangerous, key, desc = detect_dangerous_command("cp /tmp/evil.yaml ~/.lailaba/config.yaml")
        assert dangerous is True
    ...
    def test_sed_in_place(self):
        ...
        dangerous, key, desc = detect_dangerous_command("sed -i 's/manual/off/' ~/.lailaba/config.yaml")
        assert dangerous is True
        assert "lailaba config" in desc.lower() or "in-place" in desc.lower()

    def test_sed_in_place_long_flag(self):
        dangerous, key, desc = detect_dangerous_command("sed --in-place 's/manual/off/' ~/.lailaba/config.yaml")
        assert dangerous is True

    def test_sed_in_place_absolute_lailaba_home_config(self):
    ...
    def test_sed_in_place_absolute_lailaba_home_env(self):
        env_path = get_lailaba_home() / ".env"
        dangerous, key, desc = detect_dangerous_command(
    ...
    def test_custom_lailaba_home(self):
        dangerous, key, desc = detect_dangerous_command("echo x | tee $LAILABA_HOME/config.yaml")
        assert dangerous is True
    ...
    def test_perl_in_place_config(self):
        ...
        dangerous, key, desc = detect_dangerous_command(
            "perl -i -pe 's/approvals.mode: on/approvals.mode: off/' ~/.lailaba/config.yaml"
        )
        assert dangerous is True
        assert "in-place" in desc.lower() or "perl" in desc.lower()

    def test_perl_in_place_absolute_lailaba_home_config(self):
        config_path = get_lailaba_home() / "config.yaml"
        dangerous, key, desc = detect_dangerous_command(
    ...
    def test_ruby_in_place_config(self):
        dangerous, key, desc = detect_dangerous_command(
            "ruby -i -pe 'gsub(/manual/, \"off\")' ~/.lailaba/config.yaml"
        )
        assert dangerous is True

    def test_ruby_in_place_absolute_lailaba_home_env(self):
        env_path = get_lailaba_home() / ".env"
        dangerous, key, desc = detect_dangerous_command(
    ...
    def test_perl_in_place_env(self):
        dangerous, key, desc = detect_dangerous_command(
            "perl -i -pe 's/SECRET=old/SECRET=new/' ~/.lailaba/.env"
        )
        assert dangerous is True
    ...
    def test_perl_in_place_separate_flag_token(self):
        ...
        dangerous, key, desc = detect_dangerous_command(
            "perl -p -i -e 's/approvals.mode: on/approvals.mode: off/' ~/.lailaba/config.yaml"
        )
        assert dangerous is True
    ...
    def test_perl_in_place_backup_suffix(self):
        ...
        dangerous, key, desc = detect_dangerous_command(
            "perl -i.bak -pe 's/x/y/' ~/.lailaba/config.yaml"
        )
        assert dangerous is True
    ...
    def test_perl_eval_no_inplace_safe(self):
        ...
        dangerous, key, desc = detect_dangerous_command(
            "perl -wne 'print' ~/.lailaba/config.yaml"
        )
        assert dangerous is False

    def test_read_is_safe(self):
        # Reading config is not a write — must not trip.
        dangerous, key, desc = detect_dangerous_command("cat ~/.lailaba/config.yaml")
        assert dangerous is False

    def test_normal_yaml_write_safe(self):
        # A non-Lailaba config.yaml in a project dir is handled by the project
...
class TestSensitiveRedirectPattern:
    ...
    def test_redirect_to_custom_lailaba_home_env(self):
        dangerous, key, desc = detect_dangerous_command("echo x > $LAILABA_HOME/.env")
        assert dangerous is True
        assert key is not None
...
class TestSensitiveCopyMovePattern:
    ...
    shell rc files, or ~/.lailaba/config.yaml/.env must require approval — the
    ...
    def test_cp_to_lailaba_config(self):
        dangerous, key, desc = detect_dangerous_command("cp /tmp/evil.yaml ~/.lailaba/config.yaml")
        assert dangerous is True
...
class TestWindowsAbsolutePathFolding:
    """Windows absolute home / Lailaba-home prefixes must fold to ~/ and
    ~/.lailaba/ in dangerous-command detection.
    ...
    SSH, and Lailaba config/env files returned "safe" without an approval prompt.
    The OS-specific ``Path.home()`` / ``get_lailaba_home()`` tests above only
    exercise this branch on a Windows host; these monkeypatch a Windows-style
    HOME/LAILABA_HOME so the fold is verified on the POSIX CI runner too."""
    ...
    def test_windows_lailaba_home_config_folds(self, monkeypatch):
        # Lailaba home nests under the user home on Windows; it must fold before
        # the user-home rewrite eats its prefix.
        monkeypatch.setenv("HOME", r"C:\Users\tester")
        monkeypatch.setenv("LAILABA_HOME", r"C:\Users\tester\.lailaba")
        dangerous, key, _ = detect_dangerous_command(
            r"sed -i 's/manual/off/' C:\Users\tester\.lailaba\config.yaml"
        )
        assert dangerous is True
        assert key is not None
...
class TestGatewayProtection:
    ...
    def test_gateway_run_with_disown_detected(self):
        cmd = "kill 1605 && cd ~/.lailaba/lailaba-agent && source venv/bin/activate && python -m lailaba_cli.main gateway run --replace &disown; echo done"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
        assert "systemctl" in desc

    def test_gateway_run_with_ampersand_detected(self):
        cmd = "python -m lailaba_cli.main gateway run --replace &"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True

    def test_gateway_run_with_nohup_detected(self):
        cmd = "nohup python -m lailaba_cli.main gateway run --replace"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True

    def test_gateway_run_with_setsid_detected(self):
        cmd = "lailaba_cli.main gateway run --replace &disown"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
    ...
    def test_systemctl_restart_flagged(self):
        ...
        cmd = "systemctl --user restart lailaba-gateway"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
        assert "stop/restart" in desc

    def test_pkill_lailaba_detected(self):
        """pkill targeting lailaba/gateway processes must be caught."""
        cmd = 'pkill -f "cli.py --gateway"'
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
        assert "self-termination" in desc

    def test_killall_lailaba_detected(self):
        cmd = "killall lailaba"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
        assert "self-termination" in desc
...
class TestPgrepKillExpansion:
    """kill -9 $(pgrep lailaba) bypasses the pkill/killall name-matching
    ...
    def test_pkill_lailaba_still_detected(self):
    ...
    def test_kill_dollar_pidof_detected(self):
        """`kill $(pidof lailaba)` is the BSD/Linux equivalent of the
        pgrep expansion and bypasses the pkill/killall name pattern
        in the same way. See issue #33071."""
        cmd = "kill -TERM $(pidof lailaba_cli.main)"
        dangerous, _, desc = detect_dangerous_command(cmd)
        assert dangerous is True
        assert "pidof" in desc.lower() or "pgrep" in desc.lower()

    def test_kill_backtick_pidof_detected(self):
        cmd = "kill -9 `pidof lailaba`"
        dangerous, _, _ = detect_dangerous_command(cmd)
        assert dangerous is True


class TestLaunchctlGatewayLifecycle:
    """launchctl stop/kickstart/bootout/unload against the Lailaba service
    label achieves the same effect as `lailaba gateway stop|restart` and
    must require the same approval. See issue #33071.
    """

    def test_launchctl_stop_lailaba_detected(self):
        cmd = "launchctl stop ai.lailaba.gateway"
        dangerous, _, desc = detect_dangerous_command(cmd)
        assert dangerous is True
        assert "launchd" in desc.lower() or "lailaba" in desc.lower()

    def test_launchctl_kickstart_lailaba_detected(self):
        cmd = "launchctl kickstart -k system/ai.lailaba.gateway"
        dangerous, _, _ = detect_dangerous_command(cmd)
        assert dangerous is True

    def test_launchctl_bootout_lailaba_detected(self):
        cmd = "launchctl bootout system/ai.lailaba.gateway"
        dangerous, _, _ = detect_dangerous_command(cmd)
        assert dangerous is True

    def test_launchctl_unload_lailaba_detected(self):
        cmd = "launchctl unload ~/Library/LaunchAgents/ai.lailaba.gateway.plist"
        dangerous, _, _ = detect_dangerous_command(cmd)
        assert dangerous is True
    ...
    def test_launchctl_stop_unrelated_not_flagged(self):
        """`launchctl stop` on a non-Lailaba label is out of scope for the
...
class TestApprovalTimeoutIsNotConsent:
    ...
    def setup_method(self):
        ...
        self._saved_env = {
            k: os.environ.get(k)
            for k in ("LAILABA_GATEWAY_SESSION", "LAILABA_CRON_SESSION",
                      "LAILABA_YOLO_MODE",
                      "LAILABA_SESSION_KEY", "LAILABA_INTERACTIVE")
        }
        os.environ.pop("LAILABA_YOLO_MODE", None)
        os.environ.pop("LAILABA_INTERACTIVE", None)
        # LAILABA_CRON_SESSION takes priority over LAILABA_GATEWAY_SESSION in
        ...
        os.environ.pop("LAILABA_CRON_SESSION", None)
        os.environ["LAILABA_GATEWAY_SESSION"] = "1"
        os.environ["LAILABA_SESSION_KEY"] = self.SESSION_KEY
    ...
    * ``tirith_fail_open: false`` → surface a Tirith-style warning through
    ...
    def test_fail_open_true_allows_silently_on_import_error(self):
        ...
        with _patch("builtins.__import__", side_effect=self._make_failing_import(real_import)):
            with _patch("lailaba_cli.config.load_config", return_value=cfg):
                with _patch("tools.approval.detect_dangerous_command", return_value=(False, None, None)):
                    with mock_patch.dict("os.environ", {"LAILABA_INTERACTIVE": "1"}, clear=False):
