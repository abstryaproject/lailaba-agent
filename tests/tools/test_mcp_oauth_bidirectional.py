...
@pytest.mark.asyncio
async def test_lailaba_provider_forwards_asend_values(tmp_path, monkeypatch):
    ...
    from tools.mcp_oauth_manager import _LAILABA_PROVIDER_CLS, reset_manager_for_tests

    assert _LAILABA_PROVIDER_CLS is not None, "SDK OAuth types must be available"
    ...
    metadata = OAuthClientMetadata(
        redirect_uris=[AnyUrl("http://127.0.0.1:12345/callback")],
        client_name="Lailaba AI",
    )
    provider = _LAILABA_PROVIDER_CLS(
        server_name="srv",
...
@pytest.mark.asyncio
async def test_lailaba_provider_forwards_401_triggers_refresh(tmp_path, monkeypatch):
