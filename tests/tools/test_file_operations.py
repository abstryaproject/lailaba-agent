...
class TestIsWriteDenied:
    ...
    def test_oauth_mcp_tokens_and_pairing_denied(self, path):
        ...
        full_path = str(lailaba_home / path)
    ...
    def test_lailaba_control_files_requested_writable(self, path):
        from lailaba_constants import get_lailaba_home

        assert _is_write_denied(str(get_lailaba_home() / path)) is False
    ...
    def test_pairing_dir_denied(self, tmp_path, monkeypatch):
        ...
        PR #30383 introduced ~/.lailaba/pairing/{platform}-approved.json as the
...
class TestShellFileOpsHelpers:
    ...
    def test_read_file_strips_leaked_terminal_fence_markers(self, mock_env):
        leaked = (
            "'\x07__LAILABA_FENCE_a9f7b3__\x1b]0;cat "
            "'/tmp/test/a.py' 2> /dev/null\x07\n"
            "print('ok')\n"
            "__LAILABA_FENCE_a9f7b3__\x07'\n"
        )
        ...
        assert "LAILABA_FENCE" not in result.content
    ...
    def test_read_file_raw_strips_leaked_terminal_fence_markers(self, mock_env):
        leaked = (
            "__LAILABA_FENCE_a9f7b3__\x07'\n"
            "alpha\n"
            "\x1b]0;cat '/tmp/test/a.txt'\x07__LAILABA_FENCE_a9f7b3__\n"
        )
