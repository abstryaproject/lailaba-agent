...
def _clean_state():
    ...
    saved = {}
    for k in ("LAILABA_INTERACTIVE", "LAILABA_GATEWAY_SESSION", "LAILABA_EXEC_ASK", "LAILABA_YOLO_MODE"):
        if k in os.environ:
...
class TestTirithWarnSafe:
    ...
    def test_warn_session_approved(self, mock_tirith):
        os.environ["LAILABA_INTERACTIVE"] = "1"
        session_key = os.getenv("LAILABA_SESSION_KEY", "default")
    ...
    def test_warn_non_interactive_auto_allow(self, mock_tirith):
        # No LAILABA_INTERACTIVE or LAILABA_GATEWAY_SESSION set
        result = check_all_command_guards("curl https://bit.ly/abc", "local")
        assert result["approved"] is True
