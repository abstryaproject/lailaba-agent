...
def _manager_provider_with_context(storage: HermesTokenStorage, **context_attrs):
    ...
    provider = _LAILABA_PROVIDER_CLS.__new__(_LAILABA_PROVIDER_CLS)
    provider._lailaba_server_name = context_attrs.get("server_name", "srv")
    context = MagicMock()
...
class TestManagerOAuthProviderMetadata:
    def test_initialize_restores_metadata_from_disk(self, tmp_path, monkeypatch):
        ...
        with patch.object(
            _LAILABA_PROVIDER_CLS.__bases__[0], "_initialize", new=AsyncMock()
        ):
    ...
    def test_async_auth_flow_persists_on_completion(self, tmp_path, monkeypatch):
        ...
        with patch.object(
            _LAILABA_PROVIDER_CLS.__bases__[0],
            "async_auth_flow",
