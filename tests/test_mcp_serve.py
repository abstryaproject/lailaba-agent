"""
Tests for mcp_serve — Lailaba MCP server.
...
def _isolate_lailaba_home(tmp_path, monkeypatch):
    """Redirect LAILABA_HOME to a temp directory."""
    monkeypatch.setenv("LAILABA_HOME", str(tmp_path))
    try:
        import lailaba_constants
        monkeypatch.setattr(lailaba_constants, "get_lailaba_home", lambda: tmp_path)
...
def _create_test_db(db_path, session_id, messages):
    """Create a minimal SQLite DB mimicking lailaba_state schema."""
