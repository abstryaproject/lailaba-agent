...
def server():
    with patch.dict(
        "sys.modules",
        {
            "lailaba_constants": MagicMock(
                get_lailaba_home=MagicMock(return_value="/tmp/lailaba_test_review_summary")
            ),
...
def test_init_session_attaches_background_review_callback(server, monkeypatch):
    ...
    cb("💾 Self-improvement review: Skill 'lailaba-release' patched")
    ...
    assert sid == "sid-abc"
    assert payload == {
        "text": "💾 Self-improvement review: Skill 'lailaba-release' patched"
    }
