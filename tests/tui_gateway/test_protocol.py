...
def server():
    with patch.dict("sys.modules", {
        "lailaba_constants": MagicMock(get_lailaba_home=MagicMock(return_value="/tmp/lailaba_test")),
        "lailaba_cli.env_loader": MagicMock(),
        "lailaba_cli.banner": MagicMock(),
        "lailaba_state": MagicMock(),
    }):
        import importlib
...
def test_disable_flush_env_var_actually_wires_to_module_constant(monkeypatch):
    """End-to-end: setting `LAILABA_TUI_GATEWAY_NO_FLUSH=1` and importing
    ...
    monkeypatch.setenv("LAILABA_TUI_GATEWAY_NO_FLUSH", "1")
    ...
    try:
        ...
        monkeypatch.delenv("LAILABA_TUI_GATEWAY_NO_FLUSH", raising=False)
...
def test_sync_session_key_after_compress_reanchors_active_session_lease(
    ...
    from lailaba_cli.active_sessions import (
...
def test_slash_exec_rejects_skill_commands(server):
    ...
    fake_skills = {"/lailaba-agent-dev": {"name": "lailaba-agent-dev", "description": "Dev workflow"}}

    with patch("agent.skill_commands.get_skill_commands", return_value=fake_skills):
        resp = server.handle_request({
            "id": "r1",
            "method": "slash.exec",
            "params": {"command": "lailaba-agent-dev", "session_id": sid},
        })
...
def test_slash_exec_handles_plugin_commands_in_live_gateway(server):
    ...
    with patch(
        "lailaba_cli.plugins.get_plugin_command_handler",
...
def test_command_dispatch_returns_skill_payload(server):
    ...
    with patch("agent.skill_commands.scan_skill_commands", return_value=fake_skills), \
        ...
        resp = server.handle_request({
            "id": "r2",
            "method": "command.dispatch",
            "params": {"name": "lailaba-agent-dev", "session_id": sid},
        })
    ...
    assert result["name"] == "lailaba-agent-dev"
