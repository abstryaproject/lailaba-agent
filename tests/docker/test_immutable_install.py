...
Build the real image and verify at runtime:
  ...
  1. /opt/lailaba is not writable by the lailaba user (immutable install tree)
  2. PYTHONDONTWRITEBYTECODE and LAILABA_DISABLE_LAZY_INSTALLS are set
  3. /opt/lailaba/.install_method contains "docker" (code-scoped stamp)
  4. $LAILABA_HOME/.install_method is NOT stamped as "docker" by stage2
  5. A stale "docker" stamp in $LAILABA_HOME is healed (removed) on boot
"""
...
def test_install_tree_not_writable_by_lailaba(
    built_image: str, container_name: str,
) -> None:
    """The lailaba user must not be able to modify /opt/lailaba.
    ...
    r = docker_exec_sh(
        container_name,
        # Try to create a file under /opt/lailaba as the lailaba user
        "touch /opt/lailaba/test_write 2>&1 && "
        "echo WRITE_SUCCEEDED || echo WRITE_FAILED",
        timeout=10,
    )
    assert "WRITE_FAILED" in r.stdout, (
        f"lailaba user can write to /opt/lailaba (install tree not immutable): "
        f"{r.stdout}"
    )
    ...
    r = docker_exec_sh(
        container_name,
        "touch /opt/lailaba/.venv/test_write 2>&1 && "
        "echo WRITE_SUCCEEDED || echo WRITE_FAILED",
        timeout=10,
    )
    assert "WRITE_FAILED" in r.stdout, (
        f"lailaba user can write to /opt/lailaba/.venv: {r.stdout}"
    )


def test_lailaba_disable_lazy_installs_and_dont_write_bytecode(
    built_image: str, container_name: str,
) -> None:
    """The container must set PYTHONDONTWRITEBYTECODE and
    LAILABA_DISABLE_LAZY_INSTALLS=1 so no .pyc files are written to the
    ...
    r = docker_exec_sh(
        ...
        'test "$LAILABA_DISABLE_LAZY_INSTALLS" = "1" && '
        'echo ENV_OK || echo ENV_MISSING',
        timeout=10,
    )
    assert "ENV_OK" in r.stdout, (
        f"expected PYTHONDONTWRITEBYTECODE=1 and "
        f"LAILABA_DISABLE_LAZY_INSTALLS=1, got: {r.stdout} stderr={r.stderr}"
    )
...
def test_install_method_stamp_is_code_scoped(
    ...
    /opt/lailaba/.install_method (code-scoped), NOT in $LAILABA_HOME."""
    start_container(built_image, container_name)

    # Code-scoped stamp must exist and say "docker"
    r = docker_exec_sh(
        container_name,
        "cat /opt/lailaba/.install_method",
        timeout=10,
    )
    assert r.returncode == 0, (
        f"/opt/lailaba/.install_method not found: {r.stderr}"
    )
    assert r.stdout.strip() == "docker", (
        f"expected 'docker' stamp, got: {r.stdout.strip()!r}"
    )

    # $LAILABA_HOME must NOT have a 'docker' stamp
    r = docker_exec_sh(
        container_name,
    ...
    assert r.stdout.strip() != "docker", (
        f"$LAILABA_HOME/.install_method is stamped 'docker' - stage2 must "
...
def test_stale_docker_stamp_in_home_is_healed_on_boot(
    ...
    """A stale 'docker' stamp left in $LAILABA_HOME by an older image
    ...
    assert "HEALED" in r.stdout or r.stdout.strip() != "docker", (
        f"stale 'docker' stamp in $LAILABA_HOME was not healed on boot: "
        f"{r.stdout}"
    )
