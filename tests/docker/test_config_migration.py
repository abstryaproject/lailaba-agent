...
Build the real image and verify: a config.yaml present in $LAILABA_HOME
is migrated by docker_config_migrate.py on boot, running as the lailaba
user.
"""
...
def test_config_migration_runs_on_boot(
    ...
    """A config.yaml in $LAILABA_HOME must be migrated on boot by
    docker_config_migrate.py, running as the lailaba user."""
    # Start container
    ...
    assert "EXISTS" in r.stdout, (
        f"config.yaml not found in $LAILABA_HOME: {r.stdout}"
    )

    # Verify the migration script exists in the image
    r = docker_exec_sh(
        container_name,
        "test -f /opt/lailaba/scripts/docker_config_migrate.py && "
    ...
    # Verify config.yaml is owned by lailaba (migration ran as lailaba)
    r = docker_exec_sh(
        container_name,
        'stat -c "%U" /opt/data/config.yaml',
        timeout=10,
    )
    assert r.stdout.strip() == "lailaba", (
        f"config.yaml not owned by lailaba (migration may have run as root): "
        f"{r.stdout.strip()}"
    )
...
def test_config_migration_opt_out_env_var_respected(
    ...
    """LAILABA_SKIP_CONFIG_MIGRATION=1 must skip the migration."""
    start_container(
        built_image, container_name, "LAILABA_SKIP_CONFIG_MIGRATION=1",
    )
