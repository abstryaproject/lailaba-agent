"""Harness: dashboard opt-in via LAILABA_DASHBOARD.

Today (tini): dashboard starts once when LAILABA_DASHBOARD=1; if it crashes
...
Every ``docker exec`` here runs as the unprivileged ``lailaba`` user
...
def test_dashboard_not_running_by_default(
    ...
    """Without LAILABA_DASHBOARD, no dashboard process should be running."""
    start_container(built_image, container_name, cmd="sleep 60")
    r = docker_exec(container_name, "pgrep", "-f", "lailaba dashboard")
    # pgrep exits non-zero when no match found
    assert r.returncode != 0, (
        "Dashboard should not be running without LAILABA_DASHBOARD"
    )
...
def test_dashboard_slot_reports_down_when_disabled(
    ...
    """Without LAILABA_DASHBOARD, s6-svstat should report the dashboard
    slot as DOWN (not up-with-sleep-infinity, which would
    false-positive `lailaba doctor` and any other health check).
    ...
    LAILABA_DASHBOARD is unset, so the slot reflects reality.
    """
    start_container(built_image, container_name, cmd="sleep 60")
    ...
    assert "down" in r.stdout, (
        f"Dashboard slot should be 'down' without LAILABA_DASHBOARD; "
...
def test_dashboard_slot_reports_up_when_enabled(
    ...
    """Symmetry: with LAILABA_DASHBOARD=1, s6-svstat reports the slot as up."""
    # The default dashboard host is 0.0.0.0, which now engages the
    # OAuth auth gate. Without a provider registered (no
    # LAILABA_DASHBOARD_OAUTH_CLIENT_ID in this test env), start_server
    ...
    start_container(
        ...
        "LAILABA_DASHBOARD_BASIC_AUTH_PASSWORD=test-dashboard-pw",
        cmd="sleep 120",
    )
...
def test_dashboard_opt_in_starts(
    ...
    """With LAILABA_DASHBOARD=1, a dashboard process should be visible."""
    ...
    ok, _ = poll_container(
        container_name, "pgrep -f 'lailaba dashboard'", deadline_s=30.0,
    )
    assert ok, "Dashboard should be running with LAILABA_DASHBOARD=1"


def test_dashboard_port_override(
    built_image: str, container_name: str,
) -> None:
    """LAILABA_DASHBOARD_PORT changes the dashboard's listen port."""
...
def test_dashboard_restarts_after_crash(
    ...
    for _attempt in range(10):
        first_pid_result = docker_exec(
            container_name, "pgrep", "-f", "lailaba dashboard",
        )
    ...
    # Kill the dashboard. The dashboard process runs as lailaba, so the
    # lailaba user can kill it (same UID).
...
# whenever `LAILABA_DASHBOARD_HOST` was non-loopback, silently disabling
...
"""
    ...
    probe = (
        "/opt/lailaba/.venv/bin/python - <<'PY'\n"
        f"{py_program}"
        "PY"
    )
...
def test_dashboard_oauth_gate_engages_on_non_loopback_bind(
    ...
    provider activates whenever ``LAILABA_DASHBOARD_OAUTH_CLIENT_ID`` is
    set).
    ...
    start_container(
        built_image, container_name,
        "LAILABA_DASHBOARD=1",
        "LAILABA_DASHBOARD_HOST=0.0.0.0",
        "LAILABA_DASHBOARD_OAUTH_CLIENT_ID=agent:test-instance",
        cmd="sleep 120",
    )
    ...
    assert "nous" in provider_names, (
        ...
        f"LAILABA_DASHBOARD_OAUTH_CLIENT_ID is set. Got: {payload!r}"
    )
    ...
    assert status_code == 401, (
        ...
        "when a provider is registered and LAILABA_DASHBOARD_INSECURE "
...
def test_dashboard_insecure_env_var_no_longer_bypasses_gate(
    ...
    """``LAILABA_DASHBOARD_INSECURE=1`` NO LONGER disables the auth gate
