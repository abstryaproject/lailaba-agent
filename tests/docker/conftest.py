...
Override the image with ``LAILABA_TEST_IMAGE`` env var to point at a pre-built
...
IMAGE_TAG = os.environ.get("LAILABA_TEST_IMAGE", "lailaba-agent-harness:latest")
...
def built_image() -> str:
    ...
    Override with ``LAILABA_TEST_IMAGE`` env var to point at a pre-built
    image (faster local iteration).
    """
    if os.environ.get("LAILABA_TEST_IMAGE"):
        return IMAGE_TAG
...
# ---------------------------------------------------------------------------
# docker_exec — default to the unprivileged lailaba user
# ---------------------------------------------------------------------------
#
# Background: every Lailaba runtime path inside the container drops to UID
# 10000 (the ``lailaba`` user) via ``s6-setuidgid lailaba``. ``docker exec``
...
# fails (PermissionError swallowed) for lailaba, so a test that ran as root
...
# helpers below run every probe under ``-u lailaba`` unless a specific
...
def docker_exec(
    ...
    *args: str,
    ...
    """Run a command inside ``container`` as ``user`` (default: lailaba).
