"""Runtime smoke test for Docker $LAILABA_HOME/logs/gateways seeding.

Build the real image and verify logs/ and logs/gateways/ exist and are
owned by the lailaba user after container boot.
...
registered later runs its log service as the dropped lailaba user and
...
def test_logs_gateways_seeded_and_lailaba_owned(
    built_image: str, container_name: str,
) -> None:
    """logs/ and logs/gateways/ must exist and be owned by lailaba after boot."""
    ...
    assert "logs=lailaba" in r.stdout, (
        f"logs/ not owned by lailaba: {r.stdout}"
    )
    assert "gateways=lailaba" in r.stdout, (
        f"logs/gateways/ not owned by lailaba: {r.stdout}"
    )
