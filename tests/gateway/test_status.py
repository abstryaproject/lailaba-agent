...
class TestGatewayPidState:
    def test_write_pid_file_records_gateway_metadata(self, tmp_path, monkeypatch):
        ...
        assert payload["kind"] == "lailaba-gateway"
    ...
    def test_get_running_pid_cleans_stale_record_from_dead_process(self, tmp_path, monkeypatch):
        ...
        pid_path.write_text(json.dumps({
            ...
            "argv": ["python", "-m", "lailaba_cli.main", "gateway", "run"],
            "start_time": 111,
        }))
    ...
    def test_get_running_pid_accepts_gateway_metadata_when_cmdline_unavailable(self, tmp_path, monkeypatch):
        ...
        pid_path.write_text(json.dumps({
            "pid": os.getpid(),
            "kind": "lailaba-gateway",
            "argv": ["python", "-m", "lailaba_cli.main", "gateway"],
            "start_time": 123,
        }))
    ...
    def test_get_running_pid_accepts_script_style_gateway_cmdline(self, tmp_path, monkeypatch):
        ...
        pid_path.write_text(json.dumps({
            "pid": os.getpid(),
            "kind": "lailaba-gateway",
            "argv": ["/venv/bin/python", "/repo/lailaba_cli/main.py", "gateway", "run", "--replace"],
            "start_time": 123,
        }))
        ...
        monkeypatch.setattr(
            ...
            lambda pid: "/venv/bin/python /repo/lailaba_cli/main.py gateway run --replace",
        )
    ...
    def test_get_running_pid_accepts_no_supervisor_restart_runtime(self, tmp_path, monkeypatch):
        ...
        record = {
            "pid": os.getpid(),
            "kind": "lailaba-gateway",
            "argv": ["python", "-m", "lailaba_cli.main", "gateway", "restart"],
            "start_time": 123,
        }
        ...
        monkeypatch.setattr(
            status,
            "_read_process_cmdline",
            lambda pid: "python -m lailaba_cli.main gateway restart",
        )
...
class TestGatewayRuntimeStatus:
    ...
    def test_write_runtime_status_overwrites_stale_argv_on_restart(self, tmp_path, monkeypatch):
        ...
        state_path.write_text(json.dumps({
            ...
            "start_time": 1000.0,
            "kind": "lailaba-gateway",
            "argv": ["/old/path/lailaba", "gateway", "run"],
            "platforms": {},
            "updated_at": "2025-01-01T00:00:00Z",
        }))

        monkeypatch.setattr(status.sys, "argv", ["/new/path/lailaba", "gateway", "run"])
        monkeypatch.setattr(status, "_get_process_start_time", lambda pid: 2000)

        status.write_runtime_status(gateway_state="running")

        payload = status.read_runtime_status()
        assert payload["argv"] == ["/new/path/lailaba", "gateway", "run"]
        assert payload["pid"] == os.getpid()
    ...
    def test_runtime_status_running_pid_rejects_stale_record_for_supervisor_pid(self, monkeypatch):
        ...
        payload = {
            ...
            "gateway_state": "running",
            "kind": "lailaba-gateway",
            "argv": ["/opt/lailaba/.venv/bin/lailaba", "gateway", "run", "--replace"],
        }
    ...
    def test_runtime_status_running_pid_rejects_pid_reused_by_other_profile(self, monkeypatch):
        ...
        onto the live *default* gateway (``lailaba gateway run``).  The recorded
        ...
        monkeypatch.setattr(
            status, "_read_process_cmdline", lambda pid: "lailaba gateway run --replace"
        )
    ...
    def test_runtime_status_running_pid_accepts_matching_profile_cmdline(self, monkeypatch):
        ...
        for cmdline in (
            "lailaba -p coder gateway run --replace",
            "/opt/lailaba/.venv/bin/lailaba --profile coder gateway run --replace",
            "lailaba_home=/opt/data/profiles/coder lailaba gateway run --replace",
        ):
    ...
    def test_runtime_status_running_pid_default_profile_rejects_named_cmdline(self, monkeypatch):
        ...
        monkeypatch.setattr(status, "_get_process_start_time", lambda pid: None)
        monkeypatch.setattr(
            status, "_read_process_cmdline", lambda pid: "lailaba -p coder gateway run --replace"
        )
    ...
    def test_runtime_status_running_pid_default_profile_accepts_bare_cmdline(self, monkeypatch):
        """The default/root gateway (bare ``lailaba gateway run``) is reported
        running for the default profile."""
        payload = {
            "pid": 139,
            "gateway_state": "running",
...
class TestScopedLocks:
    ...
    def test_acquire_scoped_lock_rejects_live_other_process(self, tmp_path, monkeypatch):
        monkeypatch.setenv("LAILABA_GATEWAY_LOCK_DIR", str(tmp_path / "locks"))
    ...
    def test_acquire_scoped_lock_replaces_pid_reused_by_unrelated_process(self, tmp_path, monkeypatch):
        ...
        lock_path.write_text(json.dumps({
            ...
            "start_time": None,
            "kind": "lailaba-gateway",
            "argv": ["/Users/user/.lailaba/lailaba-agent/lailaba_cli/main.py", "gateway", "run", "--replace"],
        }))
    ...
    def test_acquire_scoped_lock_keeps_lock_when_cmdline_unreadable_but_record_is_gateway(self, tmp_path, monkeypatch):
        ...
        lock_path.write_text(json.dumps({
            ...
            "start_time": None,
            "kind": "lailaba-gateway",
            "argv": ["lailaba_cli/main.py", "gateway", "run"],
        }))
    ...
    def test_acquire_scoped_lock_replaces_reused_pid_even_with_matching_start_time(self, tmp_path, monkeypatch):
        ...
        lock_path.write_text(json.dumps({
            ...
            "start_time": 123,
            "kind": "lailaba-gateway",
            "argv": ["/usr/bin/python", "-m", "lailaba_cli.main", "gateway", "run"],
        }))
...
class TestTakeoverMarker:
    ...
    def test_write_marker_records_replacer_lailaba_home(self, tmp_path, monkeypatch):
        """The marker stamps the replacer's LAILABA_HOME for cross-profile guard (#29092)."""
        monkeypatch.setenv("LAILABA_HOME", str(tmp_path))
        monkeypatch.setattr(status, "_get_process_start_time", lambda pid: 42)
        ...
        payload = json.loads((tmp_path / ".gateway-takeover.json").read_text())
        assert payload["replacer_lailaba_home"] == str(tmp_path)
    ...
    def test_consume_rejects_marker_from_different_profile(self, tmp_path, monkeypatch):
        ...
        LAILABA_HOME must be rejected even when PID + start_time coincidentally
        match — otherwise two profile services sharing a default ~/.lailaba flap
        ...
        marker_path.write_text(json.dumps({
            ...
            "replacer_pid": 99999,
            "replacer_lailaba_home": str(tmp_path / "profiles" / "other"),
            "written_at": datetime.now(timezone.utc).isoformat(),
        }))
    ...
    def test_consume_accepts_legacy_marker_without_lailaba_home(self, tmp_path, monkeypatch):
        """Back-compat (#29092): markers written by older Lailaba versions have no
        ``replacer_lailaba_home`` field; an absent field is treated as same-home so
...
class TestPlannedStopMarker:
    ...
    def test_consume_returns_true_on_windows_when_start_time_unavailable(
        ...
        it would always return False — and ``lailaba gateway stop`` would be
...
class TestReadProcessCmdlinePsFallback:
    ...
    def test_proc_cmdline_takes_priority_over_ps(self, monkeypatch):
        ...
        def fake_read_bytes(self):
            calls.append("proc")
            return b"python\x00lailaba_cli/main.py\x00gateway\x00"

        monkeypatch.setattr(status.Path, "read_bytes", fake_read_bytes)
        result = status._read_process_cmdline(12345)
        assert "lailaba_cli/main.py" in result
        assert calls == ["proc"]
    ...
    def test_ps_fallback_used_when_proc_returns_empty(self, monkeypatch):
        ...
        monkeypatch.setattr(
            status.subprocess, "run",
            lambda args, **kwargs: SimpleNamespace(returncode=0, stdout="python lailaba_cli/main.py gateway run\n"),
        )
        result = status._read_process_cmdline(12345)
        assert "lailaba_cli/main.py" in result
