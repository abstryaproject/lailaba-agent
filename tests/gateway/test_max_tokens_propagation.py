...
Precedence verified here:
    LAILABA_MAX_TOKENS env  >  model.max_tokens  >  per-provider
...
def isolated_home(tmp_path, monkeypatch):
    """Isolated LAILABA_HOME with a writable config.yaml and a clean module cache.

    These tests deliberately re-import ``lailaba_cli`` / ``gateway`` so each
    ...
    monkeypatch.delenv("LAILABA_MAX_TOKENS", raising=False)

    _saved = {
        k: v
        for k, v in sys.modules.items()
        if k.startswith(("lailaba_cli", "gateway"))
    }

    def write_cfg(body: str) -> None:
        (lailaba_home / "config.yaml").write_text(textwrap.dedent(body))

    def fresh_gateway():
        for mod in list(sys.modules.keys()):
            if mod.startswith(("lailaba_cli", "gateway")):
                del sys.modules[mod]
...
def test_env_override_beats_everything(isolated_home, monkeypatch):
    """LAILABA_MAX_TOKENS is the internal override mechanism (highest priority)."""
    write_cfg, fresh_gateway = isolated_home
    monkeypatch.setenv("LAILABA_MAX_TOKENS", "2048")
    write_cfg(
        """
        model:
          default: glm-5.1
...
def test_lift_helper_accepts_alias_and_rejects_garbage(isolated_home):
    ...
    write_cfg("model:\n  provider: openrouter\n")
    for mod in list(sys.modules.keys()):
        if mod.startswith("lailaba_cli"):
            del sys.modules[mod]
    rp = importlib.import_module("lailaba_cli.runtime_provider")
