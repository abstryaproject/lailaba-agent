...
class TestConfigEnvOverrides(unittest.TestCase):
    ...
    @patch.dict(os.environ, {
        "EMAIL_ADDRESS": "lailaba@test.com",
    ...
    def test_email_config_loaded_from_env(self):
        ...
        self.assertEqual(config.platforms[Platform.EMAIL].extra["address"], "lailaba@test.com")
...
class TestDispatchMessage(unittest.TestCase):
    ...
    def test_non_allowlisted_sender_dropped(self):
        ...
        import asyncio
        with patch.dict(os.environ, {
            "EMAIL_ALLOWED_USERS": "lailaba@test.com,admin@test.com",
        }):
            adapter = self._make_adapter()
            adapter._message_handler = MagicMock()
...
class TestThreadContext(unittest.TestCase):
    ...
    def test_no_thread_context_uses_default_subject(self):
        """Without thread context, subject should be 'Re: Lailaba AI'."""
        adapter = self._make_adapter()
        ...
        with patch("smtplib.SMTP") as mock_smtp:
            ...
            send_call = mock_server.send_message.call_args[0][0]
            self.assertEqual(send_call["Subject"], "Re: Lailaba AI")
            self.assertIn("Date", send_call)
...
class TestSendMethods(unittest.TestCase):
    ...
    def test_send_calls_smtp(self):
        ...
        with patch("smtplib.SMTP") as mock_smtp:
            ...
            result = asyncio.run(
                adapter.send("user@test.com", "Hello from Lailaba!")
            )

            self.assertTrue(result.success)
            mock_server.starttls.assert_called_once()
            mock_server.login.assert_called_once_with("lailaba@test.com", "secret")
...
class TestSendEmailStandalone(unittest.TestCase):
    ...
    def test_send_email_tool_success(self):
        ...
        with patch("smtplib.SMTP") as mock_smtp:
            ...
            result = asyncio.run(
                _send_email({"address": "lailaba@test.com", "smtp_host": "smtp.test.com"}, "user@test.com", "Hello")
            )
            ...
            send_call = mock_server.send_message.call_args[0][0]
            self.assertEqual(send_call["Subject"], "Lailaba AI")
            self.assertIn("Date", send_call)
            self.assertEqual(send_call["To"], "user@test.com")
            self.assertEqual(send_call["From"], "lailaba@test.com")
...
class TestImapIdExtensionForNetEase(unittest.TestCase):
    ...
    def test_connect_sends_imap_id_after_login(self):
        ...
        self.assertIn("lailaba-agent", payload)
...
class TestConnectionConfigResolution(unittest.TestCase):
    ...
    def test_host_and_address_whitespace_stripped(self):
        ...
        self.assertEqual(adapter._address, "lailaba@test.com")

    def test_falls_back_to_platform_config_extra(self):
        """When env vars are absent, settings come from PlatformConfig.extra —
        the same dict gateway.config populates and `lailaba config show` reads."""
        from gateway.config import PlatformConfig
        from plugins.platforms.email.adapter import EmailAdapter
        cfg = PlatformConfig(enabled=True)
        cfg.extra.update({
            "address": "lailaba@test.com",
    ...
    def test_all_settings_present_satisfies_requirements(self):
        ...
        from plugins.platforms.email.adapter import check_email_requirements
        with patch.dict(os.environ, {
            "EMAIL_ADDRESS": "lailaba@test.com", "EMAIL_PASSWORD": "secret",
