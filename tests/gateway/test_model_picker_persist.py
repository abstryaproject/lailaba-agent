...
closure the PR changed, against a real temp ``LAILABA_HOME``.
"""
...
def _fake_switch_result():
    ...
    from lailaba_cli.model_switch import ModelSwitchResult
...
def _setup_isolated_home(tmp_path, monkeypatch, model_yaml_value):
    ...
    monkeypatch.setattr(
        "lailaba_cli.model_switch.resolve_display_context_length",
        lambda *a, **k: 272000,
    )
    # save_config writes to ``get_lailaba_home() / config.yaml`` — point it here.
    monkeypatch.setattr("lailaba_constants.get_lailaba_home", lambda: lailaba_home)
    monkeypatch.setattr("lailaba_cli.config.get_lailaba_home", lambda: lailaba_home)
    return cfg_path
