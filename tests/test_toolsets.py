...
class TestToolsetConsistency:
    ...
    def test_lailaba_platforms_share_core_tools(self):
        """All lailaba-* platform toolsets share the same core tools.

        Platform-specific additions (e.g. ``discord`` / ``discord_admin``
        on lailaba-discord, gated on DISCORD_BOT_TOKEN) are allowed on top —
        the invariant is that the core set is identical across platforms.
        """
        platforms = ["lailaba-cli", "lailaba-telegram", "lailaba-discord", "lailaba-whatsapp", "lailaba-slack", "lailaba-signal", "lailaba-homeassistant"]
...
class TestDefaultPlatformWebSearchCoverage:
    def test_lailaba_whatsapp_toolset_includes_web_search(self):
        assert "web_search" in resolve_toolset("lailaba-whatsapp")

    def test_lailaba_api_server_toolset_includes_web_search(self):
        assert "web_search" in resolve_toolset("lailaba-api-server")
