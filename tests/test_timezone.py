"""
Tests for timezone support (lailaba_time module + integration points).
...
def _reset_lailaba_time_cache():
    """Reset the lailaba_time module cache (replacement for removed reset_cache)."""
    lailaba_time._cached_tz = None
    lailaba_time._cached_tz_name = None
    lailaba_time._cache_resolved = False


# =========================================================================
# lailaba_time.now() — core helper
# =========================================================================
...
class TestHermesTimeNow:
    ...
    def test_valid_timezone_applies(self):
        ...
        os.environ["LAILABA_TIMEZONE"] = "Asia/Kolkata"
        result = lailaba_time.now()
    ...
    def test_utc_timezone(self):
        """UTC timezone works."""
        os.environ["LAILABA_TIMEZONE"] = "UTC"
        result = lailaba_time.now()
    ...
    def test_invalid_timezone_falls_back(self, caplog):
        ...
        os.environ["LAILABA_TIMEZONE"] = "Mars/Olympus_Mons"
        with caplog.at_level(logging.WARNING, logger="lailaba_time"):
            result = lailaba_time.now()
...
class TestGetTimezone:
    ...
    def test_returns_zoneinfo_for_valid(self):
        os.environ["LAILABA_TIMEZONE"] = "Europe/London"
        tz = lailaba_time.get_timezone()
    ...
    def test_returns_none_for_invalid(self):
        os.environ["LAILABA_TIMEZONE"] = "Not/A/Timezone"
        tz = lailaba_time.get_timezone()
        assert tz is None
...
class TestCodeExecutionTZ:
    ...
    def test_tz_injected_when_configured(self):
        """When LAILABA_TIMEZONE is set, child process sees TZ env var.
        ...
        #   2) LAILABA_TIMEZONE itself does NOT leak into the child env
        probe = (
            'import os; '
            'print("TZ=" + os.environ.get("TZ", "NOT_SET")); '
            'print("LAILABA_TIMEZONE=" + os.environ.get("LAILABA_TIMEZONE", "NOT_SET"))'
        )
        ...
        assert "LAILABA_TIMEZONE=NOT_SET" in result["output"], (
            "LAILABA_TIMEZONE should not leak into child env (only TZ)"
        )

    def test_tz_not_injected_when_empty(self):
        """When LAILABA_TIMEZONE is not set, child process has no TZ."""
        import json as _json
        os.environ.pop("LAILABA_TIMEZONE", None)
...
class TestCronTimezone:
    ...
    def test_ensure_aware_naive_preserves_absolute_time(self):
        ...
        Regression: the old code used replace(tzinfo=lailaba_tz) which shifted
        absolute time when system-local tz != Lailaba tz.  The fix interprets
    ...
    def test_ensure_aware_normalizes_aware_to_lailaba_tz(self):
        """Already-aware datetimes should be normalized to Lailaba tz."""
        from cron.jobs import _ensure_aware
        ...
        # Must be in Lailaba tz (Kolkata) but same absolute instant
    ...
    def test_ensure_aware_due_job_not_skipped_when_system_ahead(self, tmp_path, monkeypatch):
        """Reproduce the actual bug: system tz ahead of Lailaba tz caused
        overdue jobs to appear as not-yet-due.

        Scenario: system is Asia/Kolkata (UTC+5:30), Lailaba is UTC.
    ...
    def test_get_due_jobs_naive_cross_timezone(self, tmp_path, monkeypatch):
        """Naive past timestamps must be detected as due even when Lailaba tz
        ...
        # Use a Lailaba timezone far behind UTC so that the numeric wall time
        # of the naive timestamp exceeds _lailaba_now's wall time — this would
        # have caused a false "not due" with the old replace(tzinfo=...) approach.
        os.environ["LAILABA_TIMEZONE"] = "Pacific/Midway"  # UTC-11
        _reset_lailaba_time_cache()
        ...
        due = get_due_jobs()
        assert len(due) == 1, (
            "Naive past timestamp should be due regardless of Lailaba timezone"
        )
    ...
    def test_create_job_stores_tz_aware_timestamps(self, tmp_path, monkeypatch):
        ...
        os.environ["LAILABA_TIMEZONE"] = "US/Eastern"
        _reset_lailaba_time_cache()
