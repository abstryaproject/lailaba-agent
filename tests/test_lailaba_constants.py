"""Tests for lailaba_constants module."""
...
from lailaba_constants import (
    ...
    find_lailaba_node_executable,
    find_node_executable,
    find_node_executable_on_path,
    get_default_lailaba_root,
    get_lailaba_dir,
    get_lailaba_home,
    heal_lailaba_managed_node,
    lailaba_managed_node_tree_present,
    iter_lailaba_node_dirs,
    is_container,
    node_tool_runnable,
    parse_reasoning_effort,
    secure_parent_dir,
    with_lailaba_node_path,
)


class TestGetDefaultHermesRoot:
    """Tests for get_default_lailaba_root() — Docker/custom deployment awareness."""

    def test_no_lailaba_home_returns_native(self, tmp_path, monkeypatch):
        """When LAILABA_HOME is not set, returns ~/.lailaba."""
        monkeypatch.delenv("LAILABA_HOME", raising=False)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)

        assert get_default_lailaba_root() == tmp_path / ".lailaba"

    def test_lailaba_home_is_native(self, tmp_path, monkeypatch):
        """When LAILABA_HOME = ~/.lailaba, returns ~/.lailaba."""
        native = tmp_path / ".lailaba"
        native.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("LAILABA_HOME", str(native))
        assert get_default_lailaba_root() == native

    def test_lailaba_home_is_profile(self, tmp_path, monkeypatch):
        """When LAILABA_HOME is a profile under ~/.lailaba, returns ~/.lailaba."""
        native = tmp_path / ".lailaba"
    ...
    def test_lailaba_home_is_docker(self, tmp_path, monkeypatch):
        """When LAILABA_HOME points outside ~/.lailaba (Docker), returns LAILABA_HOME."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("LAILABA_HOME", str(docker_home))
        assert get_default_lailaba_root() == docker_home

    def test_lailaba_home_is_custom_path(self, tmp_path, monkeypatch):
        """Any LAILABA_HOME outside ~/.lailaba is treated as the root."""
        custom = tmp_path / "my-lailaba-data"
        custom.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("LAILABA_HOME", str(custom))
        assert get_default_lailaba_root() == custom

    def test_docker_profile_active(self, tmp_path, monkeypatch):
        """When a Docker profile is active (LAILABA_HOME=<root>/profiles/<name>),
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("LAILABA_HOME", str(profile))
        assert get_default_lailaba_root() == docker_root

    def test_no_lailaba_home_returns_localappdata_root_on_windows(self, tmp_path, monkeypatch):
        """Native Windows falls back to %LOCALAPPDATA%\\lailaba, not ~/.lailaba."""
        local_appdata = tmp_path / "LocalAppData"
        monkeypatch.delenv("LAILABA_HOME", raising=False)
        monkeypatch.setenv("LOCALAPPDATA", str(local_appdata))
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "Home")
        monkeypatch.setattr(lailaba_constants.sys, "platform", "win32")

        assert get_default_lailaba_root() == local_appdata / "lailaba"

    def test_no_lailaba_home_uses_windows_path_when_localappdata_missing(self, tmp_path, monkeypatch):
        """Windows fallback still uses AppData/Local/lailaba without LOCALAPPDATA."""
        home = tmp_path / "Home"
        monkeypatch.delenv("LAILABA_HOME", raising=False)
        ...
        assert get_default_lailaba_root() == home / "AppData" / "Local" / "lailaba"


class TestGetHermesHome:
    """Tests for get_lailaba_home() platform-aware fallback."""

    def test_windows_fallback_uses_localappdata(self, tmp_path, monkeypatch):
        """When LAILABA_HOME is unset on Windows, use %LOCALAPPDATA%\\lailaba."""
        local_appdata = tmp_path / "LocalAppData"
        monkeypatch.delenv("LAILABA_HOME", raising=False)
        monkeypatch.setenv("LOCALAPPDATA", str(local_appdata))
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "Home")
        monkeypatch.setattr(lailaba_constants.sys, "platform", "win32")
        monkeypatch.setattr(lailaba_constants, "_profile_fallback_warned", False)

        assert get_lailaba_home() == local_appdata / "lailaba"
...
class TestHermesManagedNode:
    def test_windows_node_dir_prefers_portable_root(self, tmp_path, monkeypatch):
        ...
        assert iter_lailaba_node_dirs() == [node_dir, bin_dir]
    ...
    def test_windows_finds_npm_cmd_before_path(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(lailaba_constants.sys, "platform", "win32")
        monkeypatch.setenv("LAILABA_HOME", str(home))
        monkeypatch.setattr(lailaba_constants, "node_tool_runnable", lambda path: True)

        assert find_lailaba_node_executable("npm") == str(npm_cmd)
    ...
    def test_windows_skips_broken_managed_npm_without_path_fallback(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("LAILABA_HOME", str(home))
        monkeypatch.setenv("PATH", str(bin_dir))
        monkeypatch.setattr(lailaba_constants, "_managed_node_heal_attempted", False)
        monkeypatch.setattr(lailaba_constants, "heal_lailaba_managed_node", lambda: False)
        monkeypatch.setattr(
            lailaba_constants,
            "node_tool_runnable",
            lambda path: False,
        ...
        assert lailaba_managed_node_tree_present() is True
        assert find_node_executable("npm") is None
        assert find_node_executable("npm") != str(path_npm)

    def test_with_lailaba_node_path_prepends_existing_managed_dirs(self, tmp_path, monkeypatch):
        home = tmp_path / "lailaba"
        node_dir = home / "node"
        bin_dir = node_dir / "bin"
        node_dir.mkdir(parents=True)
        ...
        env = with_lailaba_node_path({"PATH": "system-node"})
...
class TestNodeToolRunnable:
    """node_tool_runnable() rejects broken Lailaba-managed npm/node wrappers."""
    ...
    def test_broken_managed_npm_heals_when_node_still_runs(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(lailaba_constants, "heal_lailaba_managed_node", _heal)
    ...
    def test_broken_managed_npm_heals_instead_of_path_fallback(self, tmp_path, monkeypatch):
        ...
        assert find_lailaba_node_executable("npm") == str(healed_npm)
...
class TestIsContainer:
    ...
    def _reset_cache(self, monkeypatch):
        ...
        monkeypatch.setattr(lailaba_constants, "_container_detected", None)
    ...
    def test_caches_result(self, monkeypatch):
        ...
        monkeypatch.setattr(lailaba_constants, "_container_detected", True)
        assert is_container() is True
...
class TestSecureParentDir:
    ...
    def test_safe_path_calls_chmod(self, tmp_path, monkeypatch):
        ...
        safe_dir = tmp_path / "home" / "user" / ".lailaba"
...
class TestGetHermesDir:
    """Tests for ``get_lailaba_dir(new_subpath, old_name)``.
    ...
    def test_neither_exists_returns_new(self, tmp_path, monkeypatch):
        ...
        result = get_lailaba_dir("platforms/pairing", "pairing")
    ...
    def test_legacy_populated_returns_legacy(self, tmp_path, monkeypatch):
        ...
        result = get_lailaba_dir("cache/images", "image_cache")
    ...
    def test_legacy_populated_with_subdir_returns_legacy(self, tmp_path, monkeypatch):
        ...
        result = get_lailaba_dir("platforms/matrix/store", "matrix/store")
        assert result == legacy
    ...
    def test_legacy_empty_and_new_missing_returns_new(self, tmp_path, monkeypatch):
        ...
        legacy.mkdir()
        result = get_lailaba_dir("cache/audio", "audio_cache")
