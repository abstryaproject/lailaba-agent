...
class TestGatewayPersonalityNone:
    ...
    async def test_empty_personality_list_uses_profile_display_path(self, tmp_path):
        ...
        with patch("gateway.run._lailaba_home", tmp_path), \
             patch("lailaba_constants.display_lailaba_home", return_value="~/.lailaba/profiles/coder"):
            event = self._make_event("")
            result = await runner._handle_personality_command(event)

        assert result == "No personalities configured in `~/.lailaba/profiles/coder/config.yaml`"
