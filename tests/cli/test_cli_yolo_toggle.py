...
``os.environ["LAILABA_YOLO_MODE"]``. That env var is captured once at
...
prompt. Only ``lailaba --yolo`` (process-start env), ``LAILABA_YOLO_MODE=1``,
and ``lailaba config set approvals.mode off`` actually bypassed.
...
class TestToggleYoloIsSessionScoped:
    ...
    def test_toggle_yolo_does_not_mutate_env_var(self):
        """Toggling /yolo must not write ``LAILABA_YOLO_MODE`` — that path is
...
class TestIsSessionYoloActiveHelper:
    ...
    def test_helper_honors_frozen_yolo_mode(self):
        """``lailaba --yolo`` sets ``LAILABA_YOLO_MODE`` before tool imports, so
