...
class TestWorktreeCreation:
    ...
    def test_creates_worktree(self, git_repo):
        ...
        assert info["branch"].startswith("lailaba/lailaba-")
...
class TestOrphanedBranchPruning:
    """Test cleanup of orphaned lailaba/* and pr-* branches."""

    def test_prunes_orphaned_lailaba_branch(self, git_repo):
        """lailaba/lailaba-* branches with no worktree should be deleted."""
        # Create a branch that looks like a worktree branch but has no worktree
        subprocess.run(
            ["git", "branch", "lailaba/lailaba-deadbeef", "HEAD"],
            cwd=str(git_repo), capture_output=True,
        )

        # Verify it exists
        result = subprocess.run(
            ["git", "branch", "--list", "lailaba/lailaba-deadbeef"],
            capture_output=True, text=True, cwd=str(git_repo),
        )
        assert "lailaba/lailaba-deadbeef" in result.stdout
        ...
        assert "lailaba/lailaba-deadbeef" in orphaned
        ...
        assert "lailaba/lailaba-deadbeef" not in result.stdout
