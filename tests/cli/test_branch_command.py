...
def session_db(tmp_path):
    ...
    os.environ["LAILABA_HOME"] = str(tmp_path / ".lailaba")
    os.makedirs(tmp_path / ".lailaba", exist_ok=True)
    from lailaba_state import SessionDB
    db = SessionDB(db_path=tmp_path / ".lailaba" / "test_sessions.db")
    yield db
    db.close()
...
class TestBranchCommandCLI:
    ...
    def test_branch_rotates_lailaba_session_id_env_and_context(self, cli_instance, session_db):
        """Branching must update process-local session-id readers too."""
        from cli import HermesCLI
        from gateway.session_context import _UNSET, _VAR_MAP, get_session_env

        old_session_id = cli_instance.session_id
        os.environ["LAILABA_SESSION_ID"] = old_session_id
        _VAR_MAP["LAILABA_SESSION_ID"].set(old_session_id)

        try:
            HermesCLI._handle_branch_command(cli_instance, "/branch")

            assert cli_instance.session_id != old_session_id
            assert os.environ["LAILABA_SESSION_ID"] == cli_instance.session_id
            assert get_session_env("LAILABA_SESSION_ID") == cli_instance.session_id
        finally:
