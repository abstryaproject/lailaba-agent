...
#
# Wired into the image as /etc/cont-init.d/01-lailaba-setup by the
...
LAILABA_HOME="${LAILABA_HOME:-/opt/data}"
INSTALL_DIR="/opt/lailaba"

# Drop to lailaba via s6-setuidgid, but skip it when already non-root.
as_lailaba() { [ "$(id -u)" = 0 ] || { "$@"; return; }; s6-setuidgid lailaba "$@"; }
...
# starts non-root. The baked install tree under /opt/lailaba is intentionally
# root-owned and non-writable; mutable runtime state must live under
# $LAILABA_HOME. An arbitrary `--user` UID therefore cannot repair or populate
...
# default) and pass LAILABA_UID/LAILABA_GID — or the PUID/PGID aliases — which the
...
# `id -u lailaba` is the unremapped build UID (10000) because no root-only remap
# could run. root starts (id -u = 0) and the normal supervised drop to the
# lailaba UID are both unaffected.
cur_uid="$(id -u)"
if [ "$cur_uid" != 0 ] && [ "$cur_uid" != "$(id -u lailaba)" ]; then
    cat >&2 <<EOF
[stage2] ERROR: container started with --user $cur_uid (an arbitrary, non-lailaba UID).
...
and the baked /opt/lailaba install tree is intentionally root-owned and
...
Start the container as root (the default) and pass your host UID/GID instead:
    ...
    docker run -e LAILABA_UID=\$(id -u) -e LAILABA_GID=\$(id -g) ...

NAS users (Synology / unRAID / UGOS) can use the PUID/PGID aliases:

    docker run -e PUID=\$(id -u) -e PGID=\$(id -g) ...

The image remaps the lailaba user to that UID/GID at boot and chowns the data
...
# --- Bootstrap LAILABA_HOME as root ---
# Create the directory (and any missing parents) while we still have root
# privileges so the chown checks below see real metadata and the later
# `s6-setuidgid lailaba mkdir -p` block doesn't EACCES on root-owned
# ancestors. Without this, custom LAILABA_HOME paths whose parents only
# root can create (e.g. `LAILABA_HOME=/home/lailaba/.lailaba` in a Compose
...
# --- UID/GID remap ---
# Accept PUID/PGID as aliases for LAILABA_UID/LAILABA_GID.  NAS users (UGOS,
...
# UID 10000 leaves the runtime unable to read the volume.  LAILABA_UID/
# LAILABA_GID still win when both are set.  See #15290, salvages #25872.
LAILABA_UID="${LAILABA_UID:-${PUID:-}}"
LAILABA_GID="${LAILABA_GID:-${PGID:-}}"

if [ -n "${LAILABA_UID:-}" ] && validate_uid_gid "$LAILABA_UID" && [ "$LAILABA_UID" != "$(id -u lailaba)" ]; then
    echo "[stage2] Changing lailaba UID to $LAILABA_UID"
    usermod -u "$LAILABA_UID" lailaba
fi
if [ -n "${LAILABA_GID:-}" ] && validate_uid_gid "$LAILABA_GID" && [ "$LAILABA_GID" != "$(id -g lailaba)" ]; then
    echo "[stage2] Changing lailaba GID to $LAILABA_GID"
    # -o allows non-unique GID (e.g. macOS GID 20 "staff" may already
    # exist as "dialout" in the Debian-based container image).
    groupmod -o -g "$LAILABA_GID" lailaba 2>/dev/null || true
fi
...
# host's `docker` group (or root). The supervised lailaba user (UID 10000)
...
# lailaba process with `Groups: 10000` (998 gone); after this hook adds
...
# matching entry that includes lailaba. Idempotent across container
...
#   - lailaba is already a member of the right group (idempotent restart)
...
for sock in /var/run/docker.sock /run/docker.sock; do
    ...
    if id -G lailaba 2>/dev/null | tr ' ' '\n' | grep -qx "$sock_gid"; then
        echo "[stage2] lailaba already in group $sock_gid for $sock"
        break
    fi
    ...
    if usermod -aG "$sock_group" lailaba 2>/dev/null; then
        echo "[stage2] Added lailaba to group $sock_group (GID $sock_gid) for $sock"
    else
        echo "[stage2] Warning: usermod -aG $sock_group lailaba failed; docker backend may fail with EACCES"
    fi
    break
done

# --- Fix ownership of data volume ---
# When LAILABA_UID is remapped or the top-level $LAILABA_HOME isn't owned by
# the runtime lailaba UID, restore ownership to lailaba — but ONLY for the
# directories lailaba actually writes to. The full $LAILABA_HOME may be a
# host-mounted bind containing unrelated user files; `chown -R` would
# silently destroy host ownership of those (see issue #19788).
#
# The canonical list of lailaba-owned subdirs is the same one the s6-setuidgid
# mkdir -p block below seeds. Keep them in sync if the seed list changes.
actual_lailaba_uid=$(id -u lailaba)
...
chown_lailaba_tree() {
    ...
    chown -R lailaba:lailaba "$target" 2>/dev/null || \
        echo "[stage2] Warning: chown $target failed (rootless container?) — continuing"
}

needs_chown=false
if [ "$(stat -c %u "$LAILABA_HOME" 2>/dev/null)" != "$actual_lailaba_uid" ]; then
    needs_chown=true
fi
if [ "$needs_chown" = true ]; then
    echo "[stage2] Fixing ownership of $LAILABA_HOME (targeted) to lailaba ($actual_lailaba_uid)"
    # In rootless Podman the container's "root" is mapped to an
    # unprivileged host UID — chown will fail. That's fine: the volume
    # is already owned by the mapped user on the host side.
    #
    # Top-level $LAILABA_HOME: chown the directory itself (not its contents)
    # so lailaba can mkdir new subdirs but bind-mounted host files keep
    # their existing ownership.
    if refuse_symlinked_path "chown" "$LAILABA_HOME"; then
        :
    else
        chown lailaba:lailaba "$LAILABA_HOME" 2>/dev/null || \
            echo "[stage2] Warning: chown $LAILABA_HOME failed (rootless container?) — continuing"
    fi
    # Lailaba-owned subdirs: recursive chown is safe here because these are
    # created and managed exclusively by lailaba (see the s6-setuidgid mkdir
    ...
    for sub in cron sessions logs hooks memories skills skins plans workspace home profiles pairing platforms/pairing lazy-packages; do
        if [ -e "$LAILABA_HOME/$sub" ]; then
            chown_lailaba_tree "$LAILABA_HOME/$sub"
        fi
    done
fi
...
# lailaba user. Hosted/container instances keep mutable state under
# $LAILABA_HOME (/opt/data) and run with PYTHONDONTWRITEBYTECODE plus
# LAILABA_DISABLE_LAZY_INSTALLS=1. Keeping /opt/lailaba root-owned and
...
# $LAILABA_HOME/lazy-packages dir on the data volume (Dockerfile sets
# LAILABA_LAZY_INSTALL_TARGET). That dir is appended to the END of sys.path,
...
# even though installs are re-enabled. The dir is seeded + chowned to lailaba
...
# Always reset ownership of $LAILABA_HOME/profiles to lailaba on every
# boot. Profile dirs and files can land owned by root when commands
# are invoked via `docker exec <container> lailaba …` (which defaults
# to root unless `-u` is passed), and that breaks the cont-init
# reconciler (02-reconcile-profiles) which runs as lailaba and walks
# the profiles dir. Idempotent; skipped on rootless containers where
# chown would fail.
if [ -d "$LAILABA_HOME/profiles" ]; then
    chown_lailaba_tree "$LAILABA_HOME/profiles"
fi

# Always reset ownership of $LAILABA_HOME/cron on every boot for the same
# docker-exec/root-write reason as profiles/. The cron scheduler state
# (jobs.json) must stay readable by the unprivileged lailaba runtime even
# after root-context maintenance commands or scheduler writes.
if [ -d "$LAILABA_HOME/cron" ]; then
    chown_lailaba_tree "$LAILABA_HOME/cron"
fi

# Reset ownership of lailaba-owned top-level state files on every boot.
# The targeted data-volume chown above only covers lailaba-owned
# *subdirectories*; loose state files living directly under $LAILABA_HOME
# are missed. When those files are created or rewritten by
# `docker exec <container> lailaba …` (root unless `-u` is passed) they
# land root-owned, and the unprivileged lailaba runtime then hits
...
# sweep so host-owned files in a bind-mounted $LAILABA_HOME are never
...
# entries of lailaba_cli.profile_distribution.USER_OWNED_EXCLUDE plus the
...
for f in \
    ...
    active_profile; do
    if [ -e "$LAILABA_HOME/$f" ]; then
        if refuse_symlinked_path "chown" "$LAILABA_HOME/$f"; then
            :
        else
            chown lailaba:lailaba "$LAILABA_HOME/$f" 2>/dev/null || true
        fi
    fi
done

# --- config.yaml permissions ---
# Ensure config.yaml is readable by the lailaba runtime user even if it
# was edited on the host after initial ownership setup.
if [ -f "$LAILABA_HOME/config.yaml" ]; then
    if refuse_symlinked_path "chown/chmod" "$LAILABA_HOME/config.yaml"; then
        :
    else
        chown lailaba:lailaba "$LAILABA_HOME/config.yaml" 2>/dev/null || true
        chmod 640 "$LAILABA_HOME/config.yaml" 2>/dev/null || true
    fi
fi

# --- Seed directory structure as lailaba user ---
# Run as lailaba via s6-setuidgid so dirs end up owned correctly (matters
# under rootless Podman where chown back to root would fail).
#
# Use direct `mkdir -p` invocation (no `sh -c "..."` wrapper) so the
# shell isn't a second interpreter — defends against $LAILABA_HOME values
# containing shell metacharacters. PR #30136 review item O2.
as_lailaba mkdir -p \
    "$LAILABA_HOME/cron" \
    "$LAILABA_HOME/sessions" \
    "$LAILABA_HOME/logs" \
    "$LAILABA_HOME/logs/gateways" \
    "$LAILABA_HOME/hooks" \
    "$LAILABA_HOME/memories" \
    "$LAILABA_HOME/skills" \
    "$LAILABA_HOME/skins" \
    "$LAILABA_HOME/plans" \
    "$LAILABA_HOME/workspace" \
    "$LAILABA_HOME/home" \
    "$LAILABA_HOME/pairing" \
    "$LAILABA_HOME/platforms/pairing" \
    "$LAILABA_HOME/lazy-packages"

# --- Install-method stamp ---
# The 'docker' stamp is baked into the immutable install tree at
# /opt/lailaba/.install_method (see Dockerfile), NOT written here into
# $LAILABA_HOME. detect_install_method() reads the code-scoped stamp first.
#
# Why we no longer stamp $LAILABA_HOME: it is a shared DATA volume, commonly
# bind-mounted from the host (~/.lailaba:/opt/data) and sometimes shared with a
...
# 'lailaba update'. To heal homes already poisoned by older images, remove a
# stale 'docker' stamp from $LAILABA_HOME if one is present (the host install's
# own installer re-creates its code-scoped stamp; a genuine container relies on
# the baked /opt/lailaba stamp, so deleting the data-dir copy is safe).
if [ -f "$LAILABA_HOME/.install_method" ]; then
    stamped="$(tr -d '[:space:]' < "$LAILABA_HOME/.install_method" 2>/dev/null || true)"
    if [ "$stamped" = "docker" ]; then
        rm -f "$LAILABA_HOME/.install_method" 2>/dev/null || true
    fi
fi
...
seed_one() {
    dest=$1
    src=$2
    if [ ! -f "$LAILABA_HOME/$dest" ] && [ -f "$INSTALL_DIR/$src" ]; then
        if refuse_symlinked_path "seed" "$LAILABA_HOME/$dest"; then
            :
        else
            as_lailaba cp "$INSTALL_DIR/$src" "$LAILABA_HOME/$dest"
        fi
    fi
}
...
if [ -f "$LAILABA_HOME/.env" ]; then
    if refuse_symlinked_path "chown/chmod" "$LAILABA_HOME/.env"; then
        :
    else
        chown lailaba:lailaba "$LAILABA_HOME/.env" 2>/dev/null || true
        chmod 600 "$LAILABA_HOME/.env" 2>/dev/null || true
    fi
fi

# --- Migrate persisted config schema ---
# Docker image upgrades replace the code under $INSTALL_DIR but preserve
# $LAILABA_HOME on the mounted volume. Run the same safe, non-interactive
# config-schema migrations that `lailaba update` runs for non-Docker installs,
# after first-boot seeding and before supervised gateway services start.
# Set LAILABA_SKIP_CONFIG_MIGRATION=1 for controlled/manual migrations.
if [ -f "$LAILABA_HOME/config.yaml" ]; then
    s6-setuidgid lailaba "$INSTALL_DIR/.venv/bin/python" "$INSTALL_DIR/scripts/docker_config_migrate.py" \
...
if [ ! -f "$LAILABA_HOME/auth.json" ] && [ -n "${LAILABA_AUTH_JSON_BOOTSTRAP:-}" ]; then
    if refuse_symlinked_path "seed" "$LAILABA_HOME/auth.json"; then
        :
    else
        printf '%s' "$LAILABA_AUTH_JSON_BOOTSTRAP" > "$LAILABA_HOME/auth.json"
        chown lailaba:lailaba "$LAILABA_HOME/auth.json" 2>/dev/null || true
        chmod 600 "$LAILABA_HOME/auth.json"
    fi
fi
...
# can set LAILABA_GATEWAY_BOOTSTRAP_STATE=running; we seed the state file
...
# LAILABA_AUTH_JSON_BOOTSTRAP seeds auth.json. The [ ! -f ] guard is the
...
if [ ! -f "$LAILABA_HOME/gateway_state.json" ] && \
        [ "${LAILABA_GATEWAY_BOOTSTRAP_STATE:-}" = "running" ]; then
    if refuse_symlinked_path "seed" "$LAILABA_HOME/gateway_state.json"; then
        :
    else
        printf '{"gateway_state":"running"}\n' > "$LAILABA_HOME/gateway_state.json"
        chown lailaba:lailaba "$LAILABA_HOME/gateway_state.json" 2>/dev/null || true
        chmod 644 "$LAILABA_HOME/gateway_state.json"
    fi
fi
...
if [ -d "$INSTALL_DIR/skills" ]; then
    as_lailaba "$INSTALL_DIR/.venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" \
        || echo "[stage2] Warning: skills_sync.py failed; continuing"
fi

# --- Discover agent-browser's Chromium binary ---
# The image's Dockerfile runs `npx playwright install chromium`, which
# populates ``$PLAYWRIGHT_BROWSERS_PATH`` (=/opt/lailaba/.playwright) with
# a ``chromium_headless_shell-<build>/chrome-headless-shell-linux64/``
# directory. agent-browser (the runtime CLI Lailaba spawns for the
...
# main-wrapper.sh propagates it into the supervised ``lailaba`` process
...
if [ -z "${AGENT_BROWSER_EXECUTABLE_PATH:-}" ] && \
    ...
    if [ -n "$browser_bin" ]; then
        ...
        # up for all supervised services (main-lailaba, dashboard, etc.).
