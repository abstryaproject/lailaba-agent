...
This document is the formal interface between the **Lailaba gateway** (Python,
...
- **Registration.** The gateway registers a **wake URL** at enroll/provision —
  ...
  a dashboard host). Self-hosted: `lailaba gateway enroll --wake-url <url>` (or
...
**per-gateway secret** (`lailaba gateway enroll` → connector `/relay/enroll`, or
...
driven by the managed Portal / `lailaba` CLI.
