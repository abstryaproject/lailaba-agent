...
Lailaba supports multiple gateway processes running concurrently — one per profile
...
needed. On every other profile gateway, add to `~/.lailaba/config.yaml`:
...
Or set the env var: `LAILABA_KANBAN_DISPATCH_IN_GATEWAY=false`
