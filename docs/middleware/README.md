...
Lailaba middleware is the behavior-changing companion to observer hooks.
...
selection, or handoff to runtimes such as NeMo Relay without changing Lailaba'
planner, model provider adapters, tool registry, memory, or CLI UX.

With middleware enabled, plugins can:

- Rewrite LLM provider request kwargs before Lailaba calls the provider.
- Rewrite tool arguments before guardrails, approval checks, hooks, and tool
  execution see them.
- Wrap the actual LLM execution callback while preserving Lailaba retry,
  streaming, interrupt, and hook behavior.
- Wrap the actual tool execution callback while preserving Lailaba guardrails,
...
- `telemetry_schema_version`: currently `lailaba.observer.v1`
- `middleware_schema_version`: currently `lailaba.middleware.v1`
...
Lailaba stores those trace entries in later observer hook payloads as
`middleware_trace`.
...
If multiple plugins register the same execution middleware kind, Lailaba runs
them as a nested chain in registration order. Middleware failures are fail-open:
Lailaba logs a warning and continues with the next middleware or the base
runtime path.
...
For each provider request, Lailaba applies middleware in this order:
...
For each tool call, Lailaba applies middleware in this order:

1. Parse and coerce model-provided tool arguments.
2. Apply `tool_request` middleware.
3. Run the normal Lailaba pre-execution path against the effective arguments:
...
```bash
lailaba plugins enable <plugin-name>
```

For isolated local testing, use one `LAILABA_HOME` for plugin enablement and the
agent run:

```bash
export LAILABA_HOME=/tmp/lailaba-middleware-test
mkdir -p "$LAILABA_HOME"
lailaba plugins enable <plugin-name>
lailaba chat --query 'Reply exactly ok'
```
...
```bash
uv sync
uv run lailaba plugins enable <plugin-name>
uv run lailaba chat --query 'Reply exactly ok'
```
...
def tag_llm_request(**kwargs):
    ...
    extra_body.setdefault("metadata", {})["lailaba_middleware_demo"] = True
...
def normalize_terminal_workdir(**kwargs):
    ...
    args.setdefault("workdir", "/tmp/lailaba-middleware-demo")
    return {
        "args": args,
        "source": "middleware-demo",
...
Return the same response shape Lailaba expects from the provider adapter. Do not
...
- If execution middleware raises before calling `next_call(...)`, Lailaba treats
...
- If execution middleware calls `next_call(...)` successfully and then raises
  during post-processing, Lailaba preserves the downstream result and does not
  run the provider or tool a second time.
- If downstream provider or tool execution fails, middleware may let that error
  propagate or translate it deliberately. Lailaba does not convert downstream
