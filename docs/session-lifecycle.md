...
The canonical transcript store is SQLite via `SessionDB` (from `lailaba_state`). The
...
Counts consecutive restarts via a JSON file (`{LAILABA_HOME}/restart_counts.json`). If a
...
This prevents unwanted auto-resets after `lailaba update`, `lailaba gateway restart`,
or `/restart`.
