# Lailaba Observer Hooks

Lailaba observer hooks are the read-only telemetry contract for plugins that
...
They do not replace Lailaba' planner, model providers, memory, tool registry,
...
Hook callbacks are fail-open. Lailaba catches callback exceptions, logs a
...
payload construction is gated behind `has_hook(...)`, so Lailaba only builds
