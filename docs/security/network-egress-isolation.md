...
When running Lailaba inside Docker, the default `network_mode: host` gives the
...
The Lailaba [SECURITY.md](../../SECURITY.md) §2 defines the trust model. The
...
│                                             │
│   ┌──────────────┐   ┌──────────────────┐   │
│   │ lailaba-agent │   │ lailaba-dashboard │   │
│   └──────┬───────┘   └────────┬─────────┘   │
│          │                    │              │
│          ▼                    │              │
│   ┌──────────────┐            │              │
...
#
# Usage:
#   LAILABA_UID=$(id -u) LAILABA_GID=$(id -g) docker compose up -d
#
...
services:
  gateway:
    ...
    environment:
      ...
      - NO_PROXY=lailaba,lailaba-dashboard,localhost
...
docker compose exec gateway \
  curl -sf --max-time 5 http://lailaba-dashboard:9119/health && echo "OK: internal reachable" || echo "FAIL"
...
- [SECURITY.md](../../SECURITY.md) — Lailaba trust model and vulnerability reporting
