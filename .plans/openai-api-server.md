# OpenAI-Compatible API Server for Lailaba AI
...
SSE streaming. By exposing this endpoint, lailaba-agent becomes instantly
...
│  AnythingLLM     │  │    {"messages": [...]}             │  lailaba-agent   │
│  NextChat        │  │                                    │  gateway        │
...
A user would:
1. Set `API_SERVER_ENABLED=true` in `~/.lailaba/.env`
2. Run `lailaba gateway` (API server starts alongside Telegram/Discord/etc.)
3. Point Open WebUI (or any frontend) at `http://localhost:8642/v1`
4. Chat with lailaba-agent through any OpenAI-compatible UI
...
| GET | `/v1/models` | List available "models" (returns lailaba-agent as a model) |
...
Authorization: Bearer lailaba-api-key-here
...
But lailaba-agent sessions have persistent state (memory, skills, tool context).
...
Frontends send `"model": "lailaba-agent"` (or whatever). The actual LLM model
used is configured server-side in config.yaml. The API server maps any
requested model name to the configured lailaba-agent model.
...
1. `gateway/platforms/api_server.py` — new adapter
   ...
   - Frontend system prompt layered on top of lailaba-agent's core prompt
...
Once implemented, lailaba-agent works as a drop-in backend for:
