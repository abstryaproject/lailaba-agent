"""Windows UTF-8 bootstrap for Lailaba entry points.
...
should be imported at the very top of every Lailaba entry point
(``lailaba``, ``lailaba-agent``, ``lailaba-acp``, ``python -m gateway.run``,
...
def harden_import_path(src_root: str | None = None) -> None:
    """Stop a package in the current directory from shadowing Lailaba modules.

    Lailaba ships top-level modules with common names (``utils``, ``proxy``,
    ...
    We drop the relative forms outright, then force the real Lailaba source root
    ...
    """
    root = src_root or os.environ.get("LAILABA_PYTHON_SRC_ROOT") or os.path.dirname(
...
def activate_durable_lazy_target() -> None:
    ...
    (``LAILABA_LAZY_INSTALL_TARGET``, e.g. ``/opt/data/lazy-packages``).
    ...
    """
    if not os.environ.get("LAILABA_LAZY_INSTALL_TARGET", "").strip():
        return
    try:
...
# Apply on import — entry points just need ``import lailaba_bootstrap``
# (or ``from lailaba_bootstrap import apply_windows_utf8_bootstrap``) at
