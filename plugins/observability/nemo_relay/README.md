...
Optional Lailaba observability plugin that maps Lailaba observer hooks to
...
not replace Lailaba AI's planner, tools, memory, model provider routing, or
CLI UX. Instead, this plugin lets Lailaba emit NeMo Relay lifecycle events for
the work Lailaba already owns: sessions, turns, provider/API calls, tool calls,
approval prompts, and delegated subagents.

With this plugin enabled, Lailaba AI can:

- Preserve Lailaba execution as NeMo Relay scopes, LLM spans, tool spans, and
  mark events.
...
```bash
lailaba plugins enable observability/nemo_relay
```

The `LAILABA_NEMO_RELAY_*` environment variables below only configure an
already-enabled plugin. They do not enable plugin discovery by themselves.

For isolated test homes, enable the plugin in the same `LAILABA_HOME` that the
agent run will use:

```bash
env LAILABA_HOME=/tmp/lailaba-nemo-relay-test \
  lailaba plugins enable observability/nemo_relay
```

Runs started with `--ignore_user_config` skip the enabled-plugin state from
`LAILABA_HOME`, so local E2E tests should omit that flag unless the test harness
loads `observability/nemo_relay` explicitly another way.

`LAILABA_HOME` is the Lailaba profile/config home used by both
`lailaba plugins enable ...` and the later `lailaba chat ...` run. If unset,
Lailaba uses the user's default home, usually `~/.lailaba`. For isolated smoke
tests, choose any writable temporary directory and use the same value for every
command in that test:

```bash
export LAILABA_HOME=/tmp/lailaba-nemo-relay-test
lailaba plugins enable observability/nemo_relay
lailaba chat --query 'Reply exactly ok' --provider custom --model qwen3.6:35b
```

For source checkouts, make sure the `lailaba` command you run is built from the
...
uv run lailaba plugins enable observability/nemo_relay
uv run lailaba chat --query 'Reply exactly ok' --provider custom --model qwen3.6:35b
```
...
```bash
uv build --wheel
python -m pip install --force-reinstall dist/lailaba_agent-*.whl
...
The plugin can configure exporters directly from `LAILABA_NEMO_RELAY_*`
...
```bash
export LAILABA_NEMO_RELAY_ATOF_ENABLED=1
export LAILABA_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY=.nemo-relay/atof
export LAILABA_NEMO_RELAY_ATIF_ENABLED=1
export LAILABA_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY=.nemo-relay/atif
```

Optional overrides:

- `LAILABA_NEMO_RELAY_ATOF_FILENAME`
- `LAILABA_NEMO_RELAY_ATOF_MODE` (`append` or `overwrite`)
- `LAILABA_NEMO_RELAY_ATIF_FILENAME_TEMPLATE`
- `LAILABA_NEMO_RELAY_ATIF_AGENT_NAME`
- `LAILABA_NEMO_RELAY_ATIF_AGENT_VERSION`
- `LAILABA_NEMO_RELAY_ATIF_MODEL_NAME`
- `LAILABA_NEMO_RELAY_ATIF_SUBAGENT_EXPORT_MODE` (`embedded` by default; set `all` to also write standalone child files)

### NeMo Relay Component Config

To initialize NeMo Relay from a component config, create a `plugins.toml` file
and point Lailaba at it:

```bash
export LAILABA_NEMO_RELAY_PLUGINS_TOML=.nemo-relay/plugins.toml
```
...
agent_name = "Lailaba AI"
agent_version = "local"
```

When `LAILABA_NEMO_RELAY_PLUGINS_TOML` is set and initializes successfully, NeMo
Relay owns exporter lifecycle through that config. The direct
`LAILABA_NEMO_RELAY_ATOF_*` fallback setup is skipped. If the same
`plugins.toml` observability config enables `atif`, the direct
`LAILABA_NEMO_RELAY_ATIF_*` fallback setup is also skipped so Lailaba does not
double-export trajectories on teardown. If `plugins.toml` initialization fails,
Lailaba keeps the direct env-var fallbacks active for that run.
...
exposes `llm.execute(...)` / `tools.execute(...)`, Lailaba routes LLM and tool
...
For the full generic Lailaba middleware contract, see
...
export LAILABA_HOME=/tmp/lailaba-nemo-relay-docs/lailaba-home
mkdir -p "$LAILABA_HOME"

cat > "$LAILABA_HOME/config.yaml" <<'YAML'
model:
  provider: custom
  default: qwen3.6:35b
...
This run starts a parent Lailaba session, delegates to a child subagent, has the
child call `terminal`, and writes both ATOF and ATIF.

```bash
export LAILABA_NEMO_RELAY_ATOF_ENABLED=1
export LAILABA_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY=/tmp/lailaba-nemo-relay-docs/subagent/atof
export LAILABA_NEMO_RELAY_ATOF_FILENAME=nested-subagent-atof.jsonl
export LAILABA_NEMO_RELAY_ATOF_MODE=overwrite
export LAILABA_NEMO_RELAY_ATIF_ENABLED=1
export LAILABA_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY=/tmp/lailaba-nemo-relay-docs/subagent/atif
export LAILABA_NEMO_RELAY_ATIF_FILENAME_TEMPLATE='nested-subagent-atif-{session_id}.json'
export LAILABA_NEMO_RELAY_ATIF_AGENT_NAME='Lailaba AI E2E'
export LAILABA_NEMO_RELAY_ATIF_AGENT_VERSION=docs-example
export LAILABA_NEMO_RELAY_ATIF_SUBAGENT_EXPORT_MODE=all
...
{"kind":"mark","name":"lailaba.subagent.start","metadata":{"parent_session_id":"docs-parent-session","session_id":"docs-child-session","subagent_id":"sa-0-docs","child_role":"leaf"}}
...
```json
{
  ...
  "session_id": "docs-parent-session",
  "agent": {"name": "Lailaba AI E2E", "version": "docs-example", "model_name": "qwen3.6:35b"},
  "steps": [
    {
      "source": "agent",
...
message. Lailaba dispatches the read-only tools as one batch, and NeMo Relay
records both tool invocations.

```bash
mkdir -p /tmp/lailaba-nemo-relay-docs/workdir
printf 'docs_parallel_alpha_function\n' > /tmp/lailaba-nemo-relay-docs/workdir/alpha.txt
printf 'docs_parallel_beta_function\n' > /tmp/lailaba-nemo-relay-docs/workdir/beta.txt
cd /tmp/lailaba-nemo-relay-docs/workdir

export LAILABA_NEMO_RELAY_ATOF_ENABLED=1
export LAILABA_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY=/tmp/lailaba-nemo-relay-docs/parallel/atof
export LAILABA_NEMO_RELAY_ATOF_FILENAME=parallel-tools-atof.jsonl
export LAILABA_NEMO_RELAY_ATOF_MODE=overwrite
export LAILABA_NEMO_RELAY_ATIF_ENABLED=1
export LAILABA_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY=/tmp/lailaba-nemo-relay-docs/parallel/atif
export LAILABA_NEMO_RELAY_ATIF_FILENAME_TEMPLATE='parallel-tools-atif-{session_id}.json'
export LAILABA_NEMO_RELAY_ATIF_AGENT_NAME='Lailaba AI E2E'
export LAILABA_NEMO_RELAY_ATIF_AGENT_VERSION=docs-example
...
- Lailaba sessions map to `agent` scopes.
- Lailaba API request hooks map to `llm` scope start/end events.
- Lailaba tool hooks map to `tool` scope start/end events.
...
The `observability/nemo_relay` plugin uses Lailaba execution middleware to hand
...
Enable it for Lailaba:

```bash
export LAILABA_NEMO_RELAY_PLUGINS_TOML=/tmp/lailaba-middleware-test/plugins.toml
```

When the adaptive component is enabled and the installed NeMo Relay runtime
exposes `llm.execute(...)` and `tools.execute(...)`, Lailaba routes execution
through these boundaries:

```text
Lailaba provider call
  -> llm_execution middleware
    -> nemo_relay.llm.execute(...)
      -> Lailaba provider adapter next_call(...)

Lailaba tool call
  -> tool_execution middleware
    -> nemo_relay.tools.execute(...)
      -> Lailaba tool dispatcher next_call(...)
```
...
middleware for a local Lailaba run. This path requires a NeMo Relay runtime that
...
```bash
export LAILABA_HOME=/tmp/lailaba-middleware-test/lailaba-home
mkdir -p "$LAILABA_HOME" /tmp/lailaba-middleware-test/nemo-relay
...
cat > /tmp/lailaba-middleware-test/nemo-relay/plugins.toml <<'TOML'
version = 1
...
[components.config.atof]
enabled = true
output_directory = "/tmp/lailaba-middleware-test/atof"
filename = "middleware-events.jsonl"
mode = "overwrite"

[components.config.atif]
enabled = true
output_directory = "/tmp/lailaba-middleware-test/atif"
filename_template = "middleware-trajectory-{session_id}.json"
agent_name = "Lailaba Middleware E2E"
agent_version = "local"
...
export LAILABA_NEMO_RELAY_PLUGINS_TOML=/tmp/lailaba-middleware-test/nemo-relay/plugins.toml
