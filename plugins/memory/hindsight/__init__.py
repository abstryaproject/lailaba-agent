...
Config via environment variables:
  ...
  HINDSIGHT_BANK_ID                — memory bank identifier (default: lailaba)
...
Or via $LAILABA_HOME/hindsight/config.json (profile-scoped), falling back to
...
def _check_local_runtime() -> tuple[bool, str | None]:
    ...
    so Lailaba can degrade gracefully instead of repeatedly trying to start
...
def _load_config() -> dict:
    ...
    Resolution order:
      1. $LAILABA_HOME/hindsight/config.json  (profile-scoped)
    ...
    profile_path = get_lailaba_home() / "hindsight" / "config.json"
    ...
    return {
        ...
        "banks": {
            "lailaba": {
                "bankId": os.environ.get("HINDSIGHT_BANK_ID", "lailaba"),
...
def _embedded_profile_name(config: dict[str, Any]) -> str:
    """Return the Hindsight embedded profile name for this Lailaba config."""
    profile = config.get("profile", "lailaba")
    return str(profile or "lailaba")
...
def _resolve_bank_id_template(template: str, fallback: str, **placeholders: str) -> str:
    ...
    Supported placeholders (each is sanitized before substitution):
      {profile}   — active Lailaba profile name (from agent_identity)
      {workspace} — Lailaba workspace name (from agent_workspace)
      {platform}  — "cli", "telegram", "discord", etc.
      {user}      — platform user id (gateway sessions)
      {session}   — current session id

    Missing/empty placeholders are rendered as the empty string and then
    collapsed — e.g. ``lailaba-{user}`` with no user becomes ``lailaba``.
...
class HindsightMemoryProvider(MemoryProvider):
    ...
    def __init__(self):
        ...
        self._retain_context = "conversation between Lailaba AI and the User"
    ...
    def save_config(self, values, lailaba_home):
        """Write config to $LAILABA_HOME/hindsight/config.json."""
        import json
        from pathlib import Path
        config_dir = Path(lailaba_home) / "hindsight"
    ...
    def post_setup(self, lailaba_home: str, config: dict) -> None:
        ...
        from lailaba_cli.memory_setup import _CANCELLED, _curses_select, _print_cancelled_setup
        ...
        if mode == "cloud":
            ...
            if llm_key:
                ...
                env_path = Path(lailaba_home) / ".env"
                existing_llm_key = ""
                if env_path.exists():
        ...
        provider_config.setdefault("bank_id", "lailaba")
        ...
        self.save_config(provider_config, lailaba_home)
        ...
        if mode == "local_embedded":
            ...
            config_path = Path(lailaba_home) / "hindsight" / "config.json"
            try:
            ...
            if not llm_api_key:
                llm_api_key = _load_simple_env(Path(lailaba_home) / ".env").get("HINDSIGHT_LLM_API_KEY", "")
            if not llm_api_key:
    ...
    def get_config_schema(self):
        return [
            ...
            {"key": "bank_id", "description": "Memory bank name (static fallback when bank_id_template is unset)", "default": "lailaba"},
            {"key": "bank_id_template", "description": "Optional template to derive bank_id dynamically. Placeholders: {profile}, {workspace}, {platform}, {user}, {session}. Example: lailaba-{profile}", "default": ""},
            ...
            {"key": "retain_context", "description": "Context label for retained memories", "default": "conversation between Lailaba AI and the User"},
        ...
        if self._client is None:
            if self._mode == "local_embedded":
                ...
                logger.debug("Creating HindsightEmbedded client (profile=%s, provider=%s)",
                             self._config.get("profile", "lailaba"), llm_provider)
                kwargs = dict(
                    profile=self._config.get("profile", "lailaba"),
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        banks = cfg_get(self._config, "banks", "lailaba", default={})
        static_bank_id = self._config.get("bank_id") or banks.get("bankId", "lailaba")
        ...
        self._retain_context = self._config.get("retain_context", "conversation between Lailaba AI and the User")
        ...
        if self._mode == "local_embedded":
            ...
            if hasattr(os, "geteuid") and os.geteuid() == 0:
                msg = (
                    ...
                    "memory daemon. Run Lailaba as a non-root user, or switch "
                    "to cloud / local_external mode via 'lailaba memory setup'."
                )
                logger.warning(msg)
                # Surface to the terminal too — a daemon that never starts
                # would otherwise fail silently and the user would only see
                # Lailaba get sluggish. (issue #13125)
                try:
    ...
    def on_session_switch(
        ...
        **kwargs,
        ...
        document. See lailaba-agent#6672.
        ...
        if self._client is not None:
            try:
                if self._mode == "local_embedded":
                    ...
                    # When Lailaba created/used that client on the shared async loop,
