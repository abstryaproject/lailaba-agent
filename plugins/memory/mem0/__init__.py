...
Configuration
-------------
Secret (lives in $LAILABA_HOME/.env or the environment):
  MEM0_API_KEY       — Mem0 Platform API key (required for platform mode)

Behavioral settings (live in $LAILABA_HOME/mem0.json, set via `lailaba memory
setup`):
  ...
  agent_id           — Agent identifier (default: lailaba)
...
def _load_config() -> dict:
    """Load config from env vars, with $LAILABA_HOME/mem0.json overrides.
    ...
    config = {
        ...
        "agent_id": os.environ.get("MEM0_AGENT_ID", "lailaba"),
        "oss": {},
    }
    ...
    config_path = get_lailaba_home() / "mem0.json"
...
class Mem0MemoryProvider(MemoryProvider):
    ...
    def save_config(self, values, lailaba_home):
        """Write config to $LAILABA_HOME/mem0.json."""
        import json
    ...
    def post_setup(self, lailaba_home: str, config: dict) -> None:
        ...
        post_setup(lailaba_home, config)
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        #   1. Operator-configured MEM0_USER_ID (env or $LAILABA_HOME/mem0.json) —
        ...
        self._agent_id = self._config.get("agent_id", "lailaba")
