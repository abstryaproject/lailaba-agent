...
def _save_mem0_json(lailaba_home: str, data: dict) -> None:
    """Merge-write to mem0.json."""
    config_path = Path(lailaba_home) / "mem0.json"
    existing = {}
...
def _setup_platform(lailaba_home: str, config: dict, flags: dict[str, str]) -> None:
    ...
    schema = [
        ...
        {"key": "user_id", "description": "User identifier", "default": "lailaba-user"},
        {"key": "agent_id", "description": "Agent identifier", "default": "lailaba"},
    ...
    provider.save_config(provider_config, lailaba_home)

    if env_writes:
        _write_env(Path(lailaba_home) / ".env", env_writes)
...
def _setup_oss(lailaba_home: str, config: dict, flags: dict[str, str]) -> None:
    ...
    if not flags.get("_mode_from_flag"):
        _setup_oss_interactive(lailaba_home, config)
        return
    ...
    user_id = flags.get("user_id") or os.getenv("USER", "lailaba-user")
    ...
    if env_writes:
        _write_env(Path(lailaba_home) / ".env", env_writes)
    _save_mem0_json(lailaba_home, {"mode": "oss", "user_id": user_id, "agent_id": "lailaba", "oss": oss_config})
...
def _prompt_api_key(label: str, env_var: str, lailaba_home: str) -> str:
...
_PGVECTOR_CONTAINER = "lailaba-pgvector"
_PGVECTOR_IMAGE = "pgvector/pgvector:pg17"
_PGVECTOR_PASSWORD = "lailaba"
...
def _setup_oss_interactive(lailaba_home: str, config: dict) -> None:
    ...
    if llm_def["needs_key"]:
        key = _prompt_api_key(llm_def["label"], llm_def["env_var"], lailaba_home)
        if key:
    ...
    if embedder_def["needs_key"] and embedder_id != llm_id:
        key = _prompt_api_key(f"{embedder_def['label']} embedder", embedder_def["env_var"], lailaba_home)
        if key:
    ...
    user_id = input(f"  User ID [{os.getenv('USER', 'lailaba-user')}]: ").strip()
    user_id = user_id or os.getenv("USER", "lailaba-user")

    agent_id = input("  Agent ID [lailaba]: ").strip()
    agent_id = agent_id or "lailaba"
    ...
    if env_writes:
        _write_env(Path(lailaba_home) / ".env", env_writes)
    _save_mem0_json(lailaba_home, {"mode": "oss", "user_id": user_id, "agent_id": agent_id, "oss": oss_config})
...
def post_setup(lailaba_home: str, config: dict) -> None:
    """Entry point called by lailaba memory setup framework.
    ...
    if flags["mode"] == "oss":
        ...
        _setup_oss(lailaba_home, config, flags)
        return

    if flags["mode"] == "platform":
        _setup_platform(lailaba_home, config, flags)
        return
