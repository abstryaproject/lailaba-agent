...
```bash
lailaba memory setup    # select "mem0"
```
...
Behavioral settings live in `$LAILABA_HOME/mem0.json` (set them via `lailaba memory setup`). Only the secret `MEM0_API_KEY` belongs in `~/.lailaba/.env`.

| Key | Default | Description |
|-----|---------|-------------|
| `mode` | `platform` | `platform` (Mem0 Cloud) or `oss` (self-hosted) |
| `user_id` | `lailaba-user` | User identifier on Mem0 |
...
```bash
lailaba memory setup mem0 --mode oss \
...
```bash
lailaba memory setup mem0 --mode oss --oss-llm-key sk-...
```

Or edit `$LAILABA_HOME/mem0.json` directly:
```json
{
  "mode": "oss",
  "oss": {
    ...
    "vector_store": {"provider": "qdrant", "config": {"path": "~/.lailaba/mem0_qdrant"}}
  }
}
```
...
```bash
lailaba memory setup mem0 --mode platform --api-key sk-...
```
...
- Check `user_id` matches between sessions (`$LAILABA_HOME/mem0.json`).
