...
``lailaba://`` handler. Endpoints are env-overridable with local-dev defaults
...
# The loopback redirect registered for the Lailaba OAuth client. IP-literal so
...
_DEFAULT_CLIENT_ID = "lailaba-agent"
...
def begin_authorization(
    ...
    *,
    ...
    (``lailaba-desktop`` / ``lailaba-cli``) so the consent side can attribute
...
_CALLBACK_HTML = (
    ...
    b"<div>Connected to Honcho. You can close this tab and return to Lailaba.</div>"
)
...
def start_loopback_flow_background(
    *,
    ...
    # the request, where a context-local LAILABA_HOME override can't reach.
