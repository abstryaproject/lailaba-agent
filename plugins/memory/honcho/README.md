...
> **Honcho docs:** <https://docs.honcho.dev/v3/guides/integrations/lailaba>
...
```bash
lailaba memory setup honcho   # configure Honcho directly (works on a fresh install)
lailaba memory setup          # generic picker, choose Honcho from the list
```
...
Or manually:
```bash
lailaba config set memory.provider honcho
echo "HONCHO_API_KEY=***" >> ~/.lailaba/.env
```

> `lailaba honcho setup` also works, but only **after** Honcho is the active
> memory provider — the `honcho` subcommand is registered for the active
> provider only. On a fresh install, use `lailaba memory setup honcho`.
...
| Priority | Path | Scope |
|----------|------|-------|
| 1 | `$LAILABA_HOME/honcho.json` | Profile-local (isolated Lailaba instances) |
| 2 | `~/.lailaba/honcho.json` | Default profile (shared host blocks) |
| 3 | `~/.honcho/config.json` | Global (cross-app interop) |

Host key is derived from the active Lailaba profile: `lailaba` (default) or `lailaba_<profile>`.
...
> **Deprecated:** `pinPeerName` is a legacy alias for `pinUserPeer`, still read for back-compat (`pinUserPeer` wins where both are set). `lailaba honcho setup` migrates it onto `pinUserPeer` on touch and never writes it.
...
| 4 | `per-session` strategy | Lailaba session ID (`20260415_a3f2b1`) |
| 5 | `per-repo` strategy | Git root directory name (`lailaba-agent`) |
| 6 | `per-directory` strategy | Current directory basename (`src`) |
| 7 | `global` strategy | Workspace name (`lailaba`) |

Gateway platforms always resolve via priority 3 (per-chat isolation) regardless of `sessionStrategy`. The strategy setting only affects CLI sessions.

If `sessionPeerPrefix` is `true`, the peer name is prepended: `alice-lailaba-agent`.

#### What each strategy produces

- **`per-directory`** — basename of `$PWD`. Opening lailaba in `~/code/myapp` and `~/code/other` gives two separate sessions. Same directory = same session across runs.
...
- **`per-session`** — Lailaba session ID (timestamp + hex). Every `lailaba` invocation starts a fresh Honcho session. Falls back to `per-directory` if no session ID is available.
- **`global`** — workspace name. One session for everything. Memory accumulates across all directories and runs.

### Multi-Profile Pattern

Multiple Lailaba profiles can share one workspace while maintaining separate AI identities. Config resolution is **host block > root > env var > default** — host blocks inherit from root, so shared settings only need to be declared once:
...
Both profiles see the same user (`yourname`) in the same shared environment (`lailaba`), but each AI peer builds its own observations, conclusions, and behavior patterns. The coder's memory stays code-oriented; the main agent's stays broad.

Host key is derived from the active Lailaba profile: `lailaba` (default) or `lailaba_<profile>` (e.g. `lailaba -p coder` -> host key `lailaba_coder`). Older `lailaba.<profile>` host blocks are still read for compatibility and are migrated when the CLI writes profile-scoped Honcho config.
...
| `LAILABA_HONCHO_HOST` | Host key override |
...
| `HONCHO_OAUTH_CLIENT_ID` | OAuth client (default `lailaba-agent`) |
...
|---------|-------------|
| `lailaba memory setup honcho` | Configure Honcho directly — works on a fresh install |
| `lailaba honcho setup` | Interactive setup wizard (only registered once Honcho is the active provider; redirects to `lailaba memory setup`) |
| `lailaba honcho status` | Show resolved config for active profile |
| `lailaba honcho enable` / `disable` | Toggle Honcho for active profile |
| `lailaba honcho mode <mode>` | Change recall or observation mode |
| `lailaba honcho peer --user <name>` | Update user peer name |
| `lailaba honcho peer --ai <name>` | Update AI peer name |
| `lailaba honcho tokens --context <N>` | Set context token budget |
| `lailaba honcho tokens --dialectic <N>` | Set dialectic max chars |
| `lailaba honcho map <name>` | Map current directory to a session name |
| `lailaba honcho sync` | Create host blocks for all Lailaba profiles |
