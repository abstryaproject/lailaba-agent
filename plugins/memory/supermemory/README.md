...
- Supermemory API key from [app.supermemory.ai/integrations?connect=lailaba](http://app.supermemory.ai/integrations?connect=lailaba)
...
Config file: `$LAILABA_HOME/supermemory.json`

| Key | Default | Description |
|-----|---------|-------------|
| `container_tag` | `lailaba` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags (e.g. `lailaba-{identity}` → `lailaba-coder`). |
...
All Supermemory API calls send `x-sm-source: lailaba`, and document writes stamp
`metadata.sm_source: lailaba`. This is a **functional routing key, not telemetry**:
it groups Lailaba-written memories into a dedicated "Lailaba" Space in the
...
Use `{identity}` in the `container_tag` to scope memories per Lailaba profile:

```json
{
  "container_tag": "lailaba-{identity}"
}
```

For a profile named `coder`, this resolves to `lailaba-coder`. The default profile resolves to `lailaba-default`. Without `{identity}`, all profiles share the same container.
