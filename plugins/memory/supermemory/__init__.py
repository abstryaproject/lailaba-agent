...
_DEFAULT_CONTAINER_TAG = "lailaba"
...
_API_KEY_URL = "http://app.supermemory.ai/integrations?connect=lailaba"
...
def _load_supermemory_config(lailaba_home: str) -> dict:
    config = _default_config()
    config_path = Path(lailaba_home) / "supermemory.json"
...
def _save_supermemory_config(values: dict, lailaba_home: str) -> None:
    config_path = Path(lailaba_home) / "supermemory.json"
    existing = {}
    if config_path.exists():
        try:
...
class _SupermemoryClient:
    def __init__(self, api_key: str, timeout: float, container_tag: str, search_mode: str = "hybrid"):
        ...
        self._client = Supermemory(
            ...
            timeout=timeout,
            max_retries=0,
            default_headers={"x-sm-source": "lailaba"},
        )

    def _merge_metadata(self, metadata: Optional[dict]) -> dict:
        # sm_source routes Lailaba writes into the "Lailaba" Space in the Supermemory
        ...
        merged = {"sm_source": "lailaba", **(metadata or {})}
...
def _resolve_container_tag_for_setup(lailaba_home: str, *, identity: str = "default") -> str:
    config = _load_supermemory_config(lailaba_home)
...
def _probe_supermemory_connection(api_key: str, lailaba_home: str, *, identity: str = "default") -> dict:
    config = _load_supermemory_config(lailaba_home)
    status = {
        "ok": False,
        "error": "",
        "container_tag": _resolve_container_tag_for_setup(lailaba_home, identity=identity),
        "profile_facts": 0,
...
class SupermemoryMemoryProvider(MemoryProvider):
    ...
    def get_config_schema(self):
        # Only prompt for the API key during `lailaba memory setup`.
        # All other options are documented for $LAILABA_HOME/supermemory.json
    ...
    def save_config(self, values, lailaba_home):
        ...
        _save_supermemory_config(sanitized, lailaba_home)
    ...
    def get_status_config(self, provider_config: dict) -> dict:
        ...
        api_key = os.environ.get("SUPERMEMORY_API_KEY", "")
        status = _probe_supermemory_connection(api_key, lailaba_home)
    ...
    def post_setup(self, lailaba_home: str, config: dict) -> None:
        ...
        from lailaba_cli.memory_setup import _prompt, _write_env_vars
        ...
        if env_writes:
            _write_env_vars(Path(lailaba_home) / ".env", env_writes)
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        from lailaba_constants import get_lailaba_home
        self._lailaba_home = kwargs.get("lailaba_home") or str(get_lailaba_home())
        self._session_id = session_id
        self._turn_count = 0
        self._config = _load_supermemory_config(self._lailaba_home)
