...
or a linked OpenViking CLI config:
  ...
  OPENVIKING_AGENT     — Lailaba peer ID in OpenViking (default: lailaba)
...
_DEFAULT_AGENT = "lailaba"
_AGENT_PROMPT_LABEL = "Lailaba peer ID in OpenViking"
...
_SYNC_TRACE_ENV = "LAILABA_OPENVIKING_SYNC_TRACE"
...
def _derive_openviking_user_text(content: Any) -> str:
    """Strip Lailaba slash-skill scaffolding before sending content to OpenViking.
...
def _load_lailaba_openviking_config() -> dict:
    try:
...
def _openviking_server_log_path() -> Path:
    try:
        ...
        home = Path(os.environ.get("LAILABA_HOME", "")).expanduser() if os.environ.get("LAILABA_HOME") else Path.home() / ".lailaba"
...
def _print_openviking_ready(message: str, path: Optional[Path] = None) -> None:
    ...
    print("  Start a new Lailaba session to activate.\n")
...
def _run_create_profile_setup(
    *,
    ...
    save_choice = select(
        ...
        [
            ("Keep in Lailaba only", "write values only to Lailaba .env"),
    ...
    _save_lailaba_only_config(
        config=config,
        provider_config=provider_config,
        env_path=env_path,
        values=values,
    )
    _print_openviking_ready("Connection saved to Lailaba .env.")
    return True
...
class OpenVikingMemoryProvider(MemoryProvider):
    ...
    def __init__(self):
        ...
        # against a concurrent increment. See lailaba-agent#28296 review.
    ...
    def is_available(self) -> bool:
        ...
        if os.environ.get("OPENVIKING_ENDPOINT"):
            return True
        provider_config = _load_lailaba_openviking_config()
    ...
    def get_config_schema(self):
        return [
            ...
            },
            {
                "key": "agent",
                "description": (
                    "Lailaba peer ID in OpenViking, sent as the actor peer and "
    ...
    def post_setup(self, lailaba_home: str, config: dict) -> None:
        ...
        from lailaba_cli.memory_setup import _CANCELLED, _curses_select, _print_cancelled_setup, _prompt

        lailaba_home_path = Path(lailaba_home)
        env_path = lailaba_home_path / ".env"
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        settings = _resolve_connection_settings(_load_lailaba_openviking_config())
        ...
        try:
            ...
            if health_state == "unreachable":
                ...
                _emit_runtime_warning(
                    f"{health_message} OpenViking memory disabled for this Lailaba run.",
                    warning_callback,
                )
                self._client = None
        except ImportError:
    ...
    def _extract_current_turn_messages(
        ...
        """Slice the completed turn out of Lailaba' full canonical transcript."""
        if not messages:
            return []
    ...
    def _messages_to_openviking_batch(
        ...
        *,
        ...
        """Convert Lailaba canonical messages into OpenViking batch payloads."""
    ...
    def on_session_switch(
        ...
        **kwargs,
        ...
        and memory extraction never fires for it. See lailaba-agent#28296.
    ...
    def _read_uri_payload(
        ...
        *,
        ...
        # (e.g. viking://user/lailaba/.overview.md).
