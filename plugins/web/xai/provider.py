...
``{title, url, description, position}`` rows every other Lailaba web
provider produces.
...
prefers Lailaba-managed xAI Grok OAuth (via ``lailaba auth``) and falls back
to ``XAI_API_KEY`` (resolved through ``~/.lailaba/.env``, then
``os.environ``).
"""
...
class XAIWebSearchProvider(WebSearchProvider):
    ...
    def is_available(self) -> bool:
        ...
        every ``lailaba tools`` repaint and at tool-registration time.
    ...
    def search(self, query: str, limit: int = 5) -> Dict[str, Any]:
        ...
        if not api_key:
            return {
                "success": False,
                "error": (
                    "No xAI credentials found. Run `lailaba auth` to sign in with "
