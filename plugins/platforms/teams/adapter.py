"""
Microsoft Teams platform adapter for Lailaba AI.
...
class TeamsAdapter(BasePlatformAdapter):
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        try:
            ...
            self._app = App(
                ...
                client=ClientOptions(headers={"User-Agent": "Lailaba"}),
            )
    ...
    async def _on_card_action(
        ...
        data = action.data or {}
        lailaba_action = data.get("lailaba_action", "")
        session_key = data.get("session_key", "")

        if not lailaba_action or not session_key:
        ...
        choice = choice_map.get(lailaba_action)
        if not choice:
            return InvokeResponse(
                status=200,
                body=AdaptiveCardActionMessageResponse(value="Unknown action."),
    ...
    async def send_exec_approval(
        ...
        card = (
            ...
            .with_actions([
                ExecuteAction(
                    title="Allow Once",
                    verb="lailaba_approve",
                    data={**btn_data_base, "lailaba_action": "approve_once"},
                    style="positive",
                ),
                ExecuteAction(
                    title="Allow Session",
                    verb="lailaba_approve",
                    data={**btn_data_base, "lailaba_action": "approve_session"},
                ),
                ExecuteAction(
                    title="Always Allow",
                    verb="lailaba_approve",
                    data={**btn_data_base, "lailaba_action": "approve_always"},
                ),
                ExecuteAction(
                    title="Deny",
                    verb="lailaba_approve",
                    data={**btn_data_base, "lailaba_action": "deny"},
                    style="destructive",
                ),
            ])
        )
...
def interactive_setup() -> None:
    ...
    print_info("  teams app create --name \"Lailaba\" --endpoint \"https://<tunnel>/api/messages\"")
    print()
    ...
    print()
    print_success("Teams configuration saved to ~/.lailaba/.env")
