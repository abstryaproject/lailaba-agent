"""SimpleX Chat platform adapter (Lailaba plugin).
...
This adapter ships as a Lailaba platform plugin under
``plugins/platforms/simplex/``. The Lailaba plugin loader scans the
...
Optional environment variables:
    ...
    LAILABA_SIMPLEX_TEXT_BATCH_DELAY
...
discoverable and ``lailaba setup`` can describe it even when websockets is
...
# Imported at module top because they're stdlib-only inside Lailaba — no
...
class SimplexAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig, **kwargs):
        ...
        self._text_batch_delay = float(
            os.getenv("LAILABA_SIMPLEX_TEXT_BATCH_DELAY", "0.8")
        )
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``lailaba setup gateway`` → SimpleX.

    Prompts for the WebSocket URL and the optional allowlist / groups /
    auto-accept / home channel. Writes to ``~/.lailaba/.env`` via
    ``lailaba_cli.config``.
    """
    print()
    ...
    try:
        ...
        print(
            "lailaba_cli.config not available; set SIMPLEX_* vars manually in "
            "~/.lailaba/.env"
        )
        return
