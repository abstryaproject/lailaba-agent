...
# ``model_tools`` import time, so ``lailaba status``, ``lailaba chat``, etc.
...
# ``lailaba_plugins.platforms__google_chat.adapter`` once the plugin
...
#     fresh "Lailaba is thinking…" card during the small window between
...
def _mime_for_message_type(mime: str) -> MessageType:
    """Map a MIME string to a lailaba MessageType.
...
class GoogleChatAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # Persisted to ${LAILABA_HOME}/google_chat_thread_counts.json so
        ...
        try:
            ...
            _lailaba_home = _get_lailaba_home()
        except (ModuleNotFoundError, ImportError):
            _lailaba_home = _Path.home() / ".lailaba"
        self._thread_count_store = _ThreadCountStore(
            _lailaba_home / "google_chat_thread_counts.json"
        )
        ...
        # they don't sit in the chat forever as "Lailaba is thinking…".
    ...
    def _bot_id_cache_path(self) -> _Path:
        ...
        base = os.getenv("LAILABA_HOME", str(_Path.home() / ".lailaba"))
    ...
    async def _dispatch_message(self, msg: Dict[str, Any], envelope: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if text.startswith("/setup-files") and event.source is not None:
                ...
                # ${LAILABA_HOME}/google_chat_user_tokens/<sanitized>.json
    ...
    async def _build_message_event(
        ...
        """Parse a Chat API message into a lailaba MessageEvent."""
        space = envelope.get("space") or msg.get("space") or {}
    ...
    async def send_typing(self, chat_id: str, metadata: Any = None) -> None:
        """Post a visible 'Lailaba is thinking…' marker message.
        ...
        "Lailaba is thinking…" stuck in chat forever, plus one card that
        ...
        body: Dict[str, Any] = {"text": "Lailaba is thinking…"}
        if thread_id:
            body["thread"] = {"name": thread_id}
    ...
    async def stop_typing(self, chat_id: str) -> None:
        ...
        Google Chat has no separate typing API: the "Lailaba is thinking…"
    ...
    async def on_processing_complete(
        ...
        see "Lailaba is thinking…" stuck forever.
        """
    ...
    async def _consume_typing_card_with_text(
        ...
        creating a fresh "Lailaba is thinking…" card during any subsequent
...
def interactive_setup() -> None:
    """Walk the user through Google Chat configuration via ``lailaba setup``.

    The setup wizard at ``lailaba_cli/gateway.py`` calls this for plugin
    ...
    prompt for env vars, persist them to ``~/.lailaba/.env`` so the next
    ...
    print_info("  3. Create a Pub/Sub topic (e.g. lailaba-chat-events) and a Pull subscription.")
    ...
    print()
    print_success("Google Chat configuration saved to ~/.lailaba/.env")
    print_info("Restart the gateway: lailaba gateway restart")
...
async def _standalone_send(
    ...
    *,
    ...
    runner is not in this process (e.g. ``lailaba cron`` running as a
    separate process from ``lailaba gateway``).  Without this hook,
...
def register(ctx) -> None:
    """Plugin entry point — called by the Lailaba plugin system at startup.
    ...
    ctx.register_platform(
        ...
        install_hint="pip install 'lailaba-agent[google_chat]'",
        ...
        # the "Lailaba is thinking..." marker patches and edit overhead.
        ...
        platform_hint=(
            ...
            "are generating a response, a 'Lailaba is thinking…' marker message "
