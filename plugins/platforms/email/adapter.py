"""
Email platform adapter for the Lailaba gateway.

Allows users to interact with Lailaba by sending emails.
...
def _send_imap_id(imap: "imaplib.IMAP4") -> None:
    ...
    try:
        ...
        imap.xatom(
            "ID",
            f'("name" "lailaba-agent" "version" "{_lailaba_version}" '
            '"vendor" "NousResearch" '
...
class EmailAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # send-helper, and `lailaba config show` already read. Without the
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        if missing:
            message = (
                ...
                + ", ".join(missing)
                + ". Set it via `lailaba gateway setup` (env) or platforms.email "
                "in config.yaml."
            )
    ...
    def _send_email(
        ...
        subject = ctx.get("subject", "Lailaba AI")
        ...
        msg_id = f"<lailaba-{uuid.uuid4().hex[:12]}@{self._address.split('@')[1]}>"
...
# lailaba_cli/gateway.py, and the _send_email dispatch in
