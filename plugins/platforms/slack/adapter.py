...
class SlackAdapter(BasePlatformAdapter):
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        tokens_file = get_lailaba_home() / "slack_tokens.json"
        ...
        try:
            ...
            # Lailaba does not currently need to route them into the agent loop.
            ...
            # /model work directly without /lailaba prefix). A single regex
            ...
            # manifest (see `lailaba slack manifest`). In Socket Mode, Slack
            ...
            from lailaba_cli.commands import slack_native_slashes
            import re as _re
            ...
            if _slash_names:
                ...
                _slash_pattern = _re.compile(r"^/lailaba$")

            @self._app.command(_slash_pattern)
            async def handle_lailaba_command(ack, command):
            ...
            for _action_id in (
                "lailaba_approve_once",
                "lailaba_approve_session",
                "lailaba_approve_always",
                "lailaba_deny",
            ):
            ...
            for _action_id in (
                "lailaba_confirm_once",
                "lailaba_confirm_always",
                "lailaba_confirm_cancel",
            ):
    ...
    async def create_handoff_thread(
        ...
        try:
            ...
            if client is None:
                return None
            seed_text = (
                f":thread: Lailaba handoff — *{(name or 'session').strip()[:80]}*"
            )
    ...
    def _dm_top_level_threads_as_sessions(self) -> bool:
        ...
        own Lailaba session — matching the per-thread behavior channels already
    ...
    async def _handle_slack_message(self, event: dict) -> None:
        ...
        if original_text.startswith("!"):
            try:
                from lailaba_cli.commands import is_gateway_known_command

                first_token = original_text[1:].split(maxsplit=1)[0]
                # Strip "@suffix" the same way get_command() does, so
                # forms like ``!stop@lailaba`` still resolve.
                    ...
                    "elements": [
                        {
                            ...
                            "style": "primary",
                            "action_id": "lailaba_approve_once",
                            "value": session_key,
                        },
                        {
                            "type": "button",
                            "text": {"type": "plain_text", "text": "Allow Session"},
                            "action_id": "lailaba_approve_session",
                            "value": session_key,
                        },
                        {
                            "type": "button",
                            "text": {"type": "plain_text", "text": "Always Allow"},
                            "action_id": "lailaba_approve_always",
                            "value": session_key,
                        },
                        {
                            "type": "button",
                    ...
                    "elements": [
                        {
                            ...
                            "style": "primary",
                            "action_id": "lailaba_confirm_once",
                            "value": value,
                        },
                        {
                            "type": "button",
                            "text": {"type": "plain_text", "text": "Always Approve"},
                            "action_id": "lailaba_confirm_always",
                            "value": value,
                        },
                        {
                            "type": "button",
                            ...
                            "style": "danger",
                            "action_id": "lailaba_confirm_cancel",
                            "value": value,
                        },
                    ],
                },
    ...
    async def _handle_slash_confirm_action(self, ack, body, action) -> None:
        ...
        choice_map = {
            "lailaba_confirm_once": "once",
            "lailaba_confirm_always": "always",
            "lailaba_confirm_cancel": "cancel",
        }
    ...
    async def _handle_approval_action(self, ack, body, action) -> None:
        ...
        choice_map = {
            "lailaba_approve_once": "once",
            "lailaba_approve_session": "session",
            "lailaba_approve_always": "always",
            "lailaba_deny": "deny",
        }
    ...
    async def _handle_slash_command(self, command: dict) -> None:
        ...
        The legacy ``/lailaba <subcommand> [args]`` form is preserved for
        backward compatibility with older workspace manifests and for users
        who want a single entry point for free-form questions (``/lailaba
        ...
        if slash_name in {"lailaba", ""}:
            # Legacy /lailaba <subcommand> [args] routing + free-form questions.
            # Empty slash_name falls into this branch for backward compat
            # with any caller that didn't populate command["command"].
            from lailaba_cli.commands import slack_subcommand_map

            subcommand_map = slack_subcommand_map()
            subcommand_map["compact"] = "/compress"
            # Guard against whitespace-only text where ``text`` is truthy but
            # ``text.split()`` returns ``[]`` (e.g. user sends ``/lailaba   ``).
        ...
        # "Running /cmd…" ack shown by handle_lailaba_command).
        # Only stash for COMMAND events (text starts with "/") — free-form
        # questions via "/lailaba <question>" must produce public replies so
...
# ``lailaba_cli/{setup,gateway}.py``, and the ``_send_slack`` dispatch in
...
def interactive_setup() -> None:
    ...
    offers to set a home channel. Replaces ``lailaba_cli/setup.py::_setup_slack``.
    """
    ...
    def _write_slack_manifest_and_instruct() -> None:
        """Generate the Slack manifest, write it under LAILABA_HOME, and print
        paste-into-Slack instructions. Failures are non-fatal."""
        try:
            from lailaba_cli.slack_cli import _build_full_manifest
            from lailaba_constants import get_lailaba_home
            import json as _json

            manifest = _build_full_manifest(
                bot_name="Lailaba",
                bot_description="Your Lailaba agent on Slack",
            )
            target = Path(get_lailaba_home()) / "slack-manifest.json"
            ...
            print_info(
                "   Re-run `lailaba slack manifest --write` anytime to refresh after "
                "Lailaba adds new commands."
            )
        except Exception as e:
    ...
    if existing:
        ...
        if not prompt_yes_no("Reconfigure Slack?", False):
            ...
            if prompt_yes_no(
                ...
                "list? (recommended after `lailaba update`)",
                True,
            ):
    ...
    print()
    print_info("   Full guide: https://lailaba-agent.nousresearch.com/docs/user-guide/messaging/slack/")
    print()
...
def _is_connected(config) -> bool:
    ...
    Looks up via ``lailaba_cli.gateway.get_env_value`` at call time (not via the
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="pip install 'lailaba-agent[slack]'",
        # Interactive setup wizard — replaces lailaba_cli/setup.py::_setup_slack
        # and the static _PLATFORMS["slack"] dict in lailaba_cli/gateway.py.
