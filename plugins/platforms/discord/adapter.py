...
def _read_dm_role_auth_guild() -> Optional[int]:
    ...
    policy, ``~/.lailaba/.env`` is for secrets only, and this is a
...
class DiscordAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._text_batch_delay_seconds = env_float("LAILABA_DISCORD_TEXT_BATCH_DELAY_SECONDS", 0.6)
        self._text_batch_split_delay_seconds = env_float("LAILABA_DISCORD_TEXT_BATCH_SPLIT_DELAY_SECONDS", 2.0)
        ...
        self._liveness_interval_seconds = env_float(
            "LAILABA_DISCORD_LIVENESS_INTERVAL_SECONDS", 60.0
        )
        self._liveness_failure_threshold = env_int(
            "LAILABA_DISCORD_LIVENESS_FAILURE_THRESHOLD", 3
        )
    ...
    def _handle_bot_task_done(self, task: asyncio.Task) -> None:
        ...
        marked running, the Discord websocket is dead while the Lailaba gateway
    ...
    def _command_sync_state_path(self) -> _Path:
        ...
        directory = get_lailaba_home() / _DISCORD_COMMAND_SYNC_STATE_SUBDIR
        try:
    ...
    def _canonicalize_app_command_payload(self, payload: Dict[str, Any]) -> Dict[str, Any]:
        """Reduce command payloads to the semantic fields Lailaba manages."""
    ...
    def _register_slash_commands(self) -> None:
        ...
        @tree.command(name="reset", description="Reset your Lailaba session")
        ...
        @tree.command(name="status", description="Show Lailaba session status")
        ...
        @tree.command(name="stop", description="Stop the running Lailaba agent")
        ...
        @tree.command(name="reload-skills", description="Re-scan ~/.lailaba/skills/ for new or removed skills")
        ...
        @tree.command(name="update", description="Update Lailaba AI to the latest version")
        async def slash_update(interaction: discord.Interaction):
            await self._run_simple_slash(interaction, "/update", "Update initiated~")

        @tree.command(name="restart", description="Gracefully restart the Lailaba gateway")
        ...
        @tree.command(name="thread", description="Create a new thread and start a Lailaba session in it")
        @discord.app_commands.describe(
            name="Thread name",
            message="Optional first message to send to Lailaba in the thread",
        ...
        # lailaba_cli/commands.py automatically appear as Discord slash
        ...
        dropped_over_cap = 0
        try:
            from lailaba_cli.commands import COMMAND_REGISTRY, _is_gateway_available, _resolve_config_gates
        ...
        try:
            from lailaba_cli.commands import _iter_plugin_command_entries
    ...
    def _register_skill_group(self, tree) -> None:
        ...
        try:
            ...
            cmd = discord.app_commands.Command(
                name="skill",
                description="Run a Lailaba skill",
    ...
    def _refresh_skill_catalog_state(self) -> None:
        ...
        """
        from lailaba_cli.commands import discord_skill_commands_by_category
    ...
    async def _handle_thread_create_slash(
        ...
        # If a message was provided, kick off a new Lailaba session in the thread
    ...
    async def _create_thread(
        ...
        *,
        ...
        try:
            ...
            try:
                seed_content = starter_message or f"\U0001f9f5 Thread created by Lailaba: **{name}**"
    ...
    async def _auto_create_thread(self, message: 'DiscordMessage') -> Optional[Any]:
        ...
        thread_name = content[:80] if content else "Lailaba"
        if len(content) > 80:
        ...
        try:
            ...
            try:
                seed_msg = await message.channel.send(f"\U0001f9f5 Thread created by Lailaba: **{thread_name}**")
                thread = await seed_msg.create_thread(
                    name=thread_name,
    ...
    async def create_handoff_thread(
        ...
        reason = "Lailaba session handoff"
        ...
        try:
            ...
            if send is None:
                return None
            seed_msg = await send(f"\U0001f9f5 Lailaba handoff: **{thread_name}**")
            thread = await seed_msg.create_thread(
                name=thread_name,
                auto_archive_duration=1440,
                reason=reason,
    ...
    async def send_clarify(
        ...
        try:
            ...
            embed = discord.Embed(
                title="❓ Lailaba needs your input",
                description=body,
                color=discord.Color.orange(),
            )
    ...
    async def send_update_prompt(
        ...
        Used by the gateway ``/update`` watcher when ``lailaba update --gateway``
...
def _component_check_auth(
    ...
    the pairing store (``lailaba pairing approve``).
    ...
    # so users approved via ``lailaba pairing approve`` can interact with
...
def _define_discord_view_classes() -> None:
    ...
    class UpdatePromptView(discord.ui.View):
        """Interactive Yes/No buttons for ``lailaba update`` prompts.
...
# is not in this process (e.g. ``lailaba cron`` running standalone) and no live
...
def interactive_setup() -> None:
    ...
    from lailaba_cli.cli_output import (
        prompt,
        prompt_yes_no,
        print_header,
        print_info,
...
def _apply_yaml_config(yaml_cfg: dict, discord_cfg: dict) -> dict | None:
    ...
    if lis is not None and not os.getenv("LAILABA_DISCORD_LIVENESS_INTERVAL_SECONDS"):
        os.environ["LAILABA_DISCORD_LIVENESS_INTERVAL_SECONDS"] = str(lis)
    lft = discord_cfg.get("liveness_failure_threshold")
    if lft is not None and not os.getenv("LAILABA_DISCORD_LIVENESS_FAILURE_THRESHOLD"):
        os.environ["LAILABA_DISCORD_LIVENESS_FAILURE_THRESHOLD"] = str(lft)
...
def _is_connected(config) -> bool:
    ...
    Looks up via ``lailaba_cli.gateway.get_env_value`` at call time (not via
    ...
    """
    import lailaba_cli.gateway as gateway_mod
...
def register(ctx) -> None:
    """Plugin entry point — called by the Lailaba plugin system."""
    ctx.register_platform(
        ...
        install_hint="pip install 'lailaba-agent[messaging]'",
        # Interactive setup wizard — replaces the central
        # lailaba_cli/setup.py::_setup_discord function.  Same shape as Teams.
