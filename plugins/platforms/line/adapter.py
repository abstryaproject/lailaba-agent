"""
LINE Messaging API platform adapter for Lailaba AI.
...
to Lailaba AI. It deliberately ports the *strongest* idea from each into
...
class LineAdapter(BasePlatformAdapter):
    ...
    async def _handle_media(self, request) -> Any:
        ...
        ``/private/tmp`` on macOS), and ``LAILABA_HOME``. PR #8398.
        """
        from aiohttp import web
        ...
        try:
            ...
            lailaba_home = Path(get_lailaba_home()).resolve()
        except Exception:
            lailaba_home = Path.home().joinpath(".lailaba").resolve()
...
def is_connected(config) -> bool:
    """Surface in ``lailaba status`` even before the adapter is instantiated."""
...
def _env_enablement() -> Optional[Dict[str, Any]]:
    ...
    Lets ``lailaba status`` reflect a LINE configuration that lives entirely
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``lailaba setup line``.

    Mirrors the irc/teams style: prompts for the two required vars, plus
    one optional public URL. Writes to ``~/.lailaba/.env`` via ``lailaba_cli.config``.
    """
    print()
    ...
    try:
        from lailaba_cli.config import get_env_var, set_env_var
    except ImportError:
        print("lailaba_cli.config not available; set LINE_* vars manually in ~/.lailaba/.env")
        return
