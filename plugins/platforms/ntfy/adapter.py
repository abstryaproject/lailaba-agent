"""ntfy platform adapter (Lailaba plugin).
...
a Lailaba dependency.

This adapter ships as a Lailaba platform plugin under
``plugins/platforms/ntfy/``. The Lailaba plugin loader scans the
    ...
    platforms:
      ntfy:
        ...
        extra:
          ...
          topic: "lailaba-in"              # subscribe topic (incoming)
          publish_topic: "lailaba-out"     # optional — defaults to topic
...
_ECHO_TAG = "lailaba-agent"  # tag added to outgoing messages for echo-loop prevention
...
async def _standalone_send(
    ...
    *,
    ...
    ``lailaba cron`` running standalone). Without this hook,
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="pip install httpx   # already a Lailaba dependency",
        # Env-driven auto-configuration: seeds PlatformConfig.extra so
        # env-only setups show up in `lailaba gateway status` without
