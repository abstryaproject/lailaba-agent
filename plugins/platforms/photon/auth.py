...
Credential storage mirrors every other Lailaba channel:

    * runtime SDK creds  -> ``~/.lailaba/.env``  (``PHOTON_PROJECT_ID`` =
      project id, ``PHOTON_PROJECT_SECRET``) via ``save_env_value``
    * management metadata -> ``~/.lailaba/auth.json`` under
...
try:
    import httpx
except ImportError:  # pragma: no cover - httpx is a lailaba dependency
...
# registers Lailaba as its own client_id.
...
# Default name of the project Lailaba provisions for the operator.
DEFAULT_PROJECT_NAME = "Lailaba AI"
...
# ---------------------------------------------------------------------------
# auth.json helpers — share the file with the rest of lailaba-agent.

def _auth_json_path() -> Path:
    """Resolve ``~/.lailaba/auth.json`` honouring the active Lailaba profile."""
    try:
        from lailaba_constants import get_lailaba_home
        return Path(get_lailaba_home()) / "auth.json"
    except Exception:
        return Path(os.path.expanduser("~/.lailaba")) / "auth.json"
...
def load_project_credentials() -> Tuple[Optional[str], Optional[str]]:
    ...
    Precedence: process env (``~/.lailaba/.env`` is loaded into the gateway's
...
def store_project_credentials(
    *,
    ...
    The runtime SDK creds land in ``~/.lailaba/.env`` via the same
...
def _persist_runtime_env(spectrum_project_id: str, project_secret: str) -> None:
    """Write the SDK creds to ``~/.lailaba/.env`` (canonical runtime store).
    ...
    try:
        ...
        logger.warning("photon: lailaba_cli.config unavailable — skipping .env write")
        return
    try:
...
def print_credential_summary(emit: Any = print) -> None:
    ...
    labels["device_token"] = (
        ...
        else "✗ missing (run `lailaba photon setup`)"
    )
    ...
    labels["phone_number"] = (
        phone if phone else "✗ missing (run `lailaba photon setup --phone ...`)"
    )
    labels["assigned_phone_number"] = (
        assigned if assigned else "✗ missing (run `lailaba photon setup`)"
    )
...
def credential_summary() -> Dict[str, str]:
    ...
    def _present_phone() -> str:
        ...
        return phone or "✗ missing (run `lailaba photon setup --phone ...`)"

    def _present_assigned_phone() -> str:
        _phone, assigned = load_user_numbers()
        return assigned or "✗ missing (run `lailaba photon setup`)"
