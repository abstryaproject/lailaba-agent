...
Small Node helper that bridges Lailaba AI to Photon's Spectrum SDK
(`spectrum-ts`).  Lailaba is Python; Photon has no public HTTP
...
- exposes a loopback-only HTTP control channel for the Python adapter
  to push send/typing requests (auth via `X-Lailaba-Sidecar-Token`)
...
The Lailaba plugin's `lailaba photon setup` command runs `npm install`
here automatically.
