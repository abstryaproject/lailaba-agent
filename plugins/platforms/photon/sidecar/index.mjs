// Lailaba AI — Photon Spectrum sidecar
//
...
//
// Inbound  (gRPC -> Lailaba): the SDK's `app.messages` async iterator is a
...
//   gateway connects.
// Outbound (Lailaba -> gRPC): `/send` drives `space.send(...)`; `/typing`
//   sends the documented `typing("start" | "stop")` content builder.
//
// Protocol (all requests require `X-Lailaba-Sidecar-Token: ${TOKEN}`):
...
//                          default off — toggle with `lailaba photon telemetry`)
  ...
  streamRestartTimer = setTimeout(() => {
    ...
    console.error(
      `photon-sidecar: upstream stream degraded for ${degradedForMs}ms; ` +
        "exiting so Lailaba can restart the Photon adapter"
    );
    process.exit(75);
...
// instead of a cryptic module-resolution error during import. Apply Lailaba'
...
function inboundStreamErrorMessage(e) {
  ...
  // iMessage internal error. Local Lailaba allowlists cannot cause or fix this:
  ...
  if (
    ...
    out +=
      ...
      "this is upstream of Lailaba, so inbound iMessages may not be delivered " +
...
const server = http.createServer(async (req, res) => {
  if (!tokenOk(req.headers["x-lailaba-sidecar-token"])) {
  ...
  try {
    ...
    if (req.url === "/send-attachment") {
      ...
      // overrides only when Lailaba supplied them so a known-good
