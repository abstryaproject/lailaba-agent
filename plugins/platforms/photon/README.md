...
This plugin connects Lailaba AI to iMessage (and other Spectrum
...
TypeScript-only, Lailaba runs it inside a small supervised Node sidecar and
...
- **Outbound**: `send` / `send_typing` / reaction tapbacks are loopback POSTs
  ...
  `/unreact`), authenticated with a shared `X-Lailaba-Sidecar-Token`.
...
There is no separate `login` command; like every other Lailaba channel,
...
Run `lailaba photon status` to see what's configured.

## Credentials

Runtime SDK credentials live in `~/.lailaba/.env` (the same place every other
...
Management metadata lives in `~/.lailaba/auth.json` under `credential_pool`:
...
| `PHOTON_TELEMETRY`        | false                      | Spectrum SDK telemetry — toggle with `lailaba photon telemetry on\|off` (restart the gateway to apply) |
...
6. Verify end-to-end: `lailaba photon status`, a DM and a group roundtrip,
