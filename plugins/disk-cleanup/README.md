...
Auto-tracks and cleans up ephemeral files created during Lailaba AI
sessions — test scripts, temp outputs, cron logs, stale chrome profiles.
Scoped strictly to `$LAILABA_HOME` and `/tmp/lailaba-*`.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `LAILABA_HOME`, track it silently as `test` / `temp` / `cron-output`. |
...
- `is_safe_path()` rejects anything outside `LAILABA_HOME` or `/tmp/lailaba-*`
- Windows mounts (`/mnt/c` etc.) are rejected
- The state directory `$LAILABA_HOME/disk-cleanup/` is itself excluded
- `$LAILABA_HOME/logs/`, `memories/`, `sessions/`, `skills/`, `plugins/`,
