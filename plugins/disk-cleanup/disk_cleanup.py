"""disk_cleanup — ephemeral file cleanup for Lailaba AI.
...
Rules:
  ...
  - empty dirs    → always delete (under LAILABA_HOME)
  - research      → keep 10 newest, prompt for older (deep only)
  - chrome-profile→ prompt after 14 days (deep only)
  - >500 MB files → prompt always (deep only)

Scope: strictly LAILABA_HOME and /tmp/lailaba-*
Never touches: ~/.lailaba/logs/ or any system directory.
"""
...
try:
    ...
    def get_lailaba_home() -> Path:  # type: ignore[no-redef]
        val = (os.environ.get("LAILABA_HOME") or "").strip()
        return Path(val).resolve() if val else (Path.home() / ".lailaba").resolve()
...
def get_state_dir() -> Path:
    """State dir — separate from ``$LAILABA_HOME/logs/``."""
    return get_lailaba_home() / "disk-cleanup"
...
def get_log_file() -> Path:
    """Audit log — intentionally NOT under ``$LAILABA_HOME/logs/``."""
...
def is_safe_path(path: Path) -> bool:
    """Accept only paths under LAILABA_HOME or ``/tmp/lailaba-*``.

    Rejects Windows mounts (``/mnt/c`` etc.) and any system directory.
    """
    lailaba_home = get_lailaba_home()
    try:
        path.resolve().relative_to(lailaba_home)
        return True
    except (ValueError, OSError):
        pass
    # Allow /tmp/lailaba-* explicitly
    parts = path.parts
    if len(parts) >= 3 and parts[1] == "tmp" and parts[2].startswith("lailaba-"):
        return True
    return False
...
_EMPTY_DIR_PROTECTED_TOP_LEVEL = frozenset({
    ...
    "lailaba-agent", "backups", "profiles", ".worktrees",
})
...
# Paths under $LAILABA_HOME that must NEVER be deleted by quick(),
...
def _is_protected_cron_path(p: Path) -> bool:
    ...
    """
    # Lazily build the set once per process so LAILABA_HOME is resolved
    # exactly once.
    if not _PROTECTED_CRON_PATHS:
        lailaba_home = get_lailaba_home()
        for parent in ("cron", "cronjobs"):
            base = lailaba_home / parent
...
def track(path_str: str, category: str, silent: bool = False) -> bool:
    ...
    if not is_safe_path(path):
        _log(f"REJECT: {path} (outside LAILABA_HOME)")
        return False
...
def quick() -> Dict[str, Any]:
    ...
    # Remove empty dirs under LAILABA_HOME, but never recurse into known
    # durable state trees.  Some installs place the Lailaba checkout, venv,
    # and desktop build under LAILABA_HOME; a full rglob over that tree can
    # stall the gateway event loop for minutes.
    lailaba_home = get_lailaba_home()
    empty_removed = 0
    sweep_stack: List[Tuple[Path, bool]] = []
    try:
        for top in lailaba_home.iterdir():
            if (
                top.is_dir()
...
def guess_category(path: Path) -> Optional[str]:
    ...
    lailaba_home = get_lailaba_home()
    try:
        rel = path.resolve().relative_to(lailaba_home)
        top = rel.parts[0] if rel.parts else ""
        if top in {
            "disk-cleanup", "logs", "memories", "sessions", "config.yaml",
            "skills", "plugins", ".env", "USER.md", "MEMORY.md", "SOUL.md",
            "auth.json", "lailaba-agent",
        }:
            return None
        ...
        # Path isn't under LAILABA_HOME (e.g. /tmp/lailaba-*) — fall through.
        pass
