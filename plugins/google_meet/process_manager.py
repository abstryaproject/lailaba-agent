...
session-scoped state file under ``$LAILABA_HOME/workspace/meetings/.active.json``
...
# File + directory layout (under $LAILABA_HOME):
#
#   workspace/meetings/
...
def _root() -> Path:
    return Path(get_lailaba_home()) / "workspace" / "meetings"
...
def start(
    ...
    *,
    ...
    If a bot is already running for this lailaba install, leave it first —
    ...
    env = os.environ.copy()
    env["LAILABA_MEET_URL"] = url
    env["LAILABA_MEET_OUT_DIR"] = str(out)
    env["LAILABA_MEET_GUEST_NAME"] = guest_name
    if headed:
        env["LAILABA_MEET_HEADED"] = "1"
    if auth_state:
        env["LAILABA_MEET_AUTH_STATE"] = auth_state
    if duration:
        env["LAILABA_MEET_DURATION"] = duration
    # v2: realtime mode + passthroughs. The bot defaults to transcribe
    # mode if LAILABA_MEET_MODE isn't set, matching v1 behavior.
    if mode:
        env["LAILABA_MEET_MODE"] = mode
    if realtime_model:
        env["LAILABA_MEET_REALTIME_MODEL"] = realtime_model
    if realtime_voice:
        env["LAILABA_MEET_REALTIME_VOICE"] = realtime_voice
    if realtime_instructions:
        env["LAILABA_MEET_REALTIME_INSTRUCTIONS"] = realtime_instructions
    if realtime_api_key:
        env["LAILABA_MEET_REALTIME_KEY"] = realtime_api_key
