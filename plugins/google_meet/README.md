...
Let the lailaba ai join a Google Meet call, transcribe it, optionally speak
...
```
┌─ gateway (Linux box, where lailaba runs) ────────────────────────────┐
│                                                                      │
...
│                                                                      │
│   NodeServer (from `lailaba meet node run`)                           │
│     │                                                                │
...
| `__init__.py` | `register(ctx)` — registers 5 tools + `on_session_end` hook + `lailaba meet` CLI |
...
| `cli.py` | `lailaba meet setup / auth / join / status / transcript / say / stop / node ...` |
...
| `node/registry.py` | v3: `$LAILABA_HOME/workspace/meetings/nodes.json` |
| `node/server.py` | v3: `NodeServer` (runs on host machine) |
| `node/client.py` | v3: `NodeClient` (used by tool handlers + CLI on gateway) |
| `node/cli.py` | v3: `lailaba meet node {run,list,approve,remove,status,ping}` |
...
lailaba meet install                                      # pip + Chromium
lailaba meet setup                                        # preflight
lailaba meet auth                                         # optional
lailaba meet join https://meet.google.com/abc-defg-hij    # transcribe
```
...
```bash
lailaba meet install --realtime                     # installs pulseaudio-utils
echo 'OPENAI_API_KEY=sk-...' >> ~/.lailaba/.env
lailaba meet join https://meet.google.com/abc-defg-hij --mode realtime
# then from the agent or CLI:
lailaba meet say "Good morning everyone, I'm the note-taker bot."
```

macOS:
```bash
lailaba meet install --realtime     # runs: brew install blackhole-2ch ffmpeg
# then — manually! — open System Settings → Sound → Input → BlackHole 2ch
echo 'OPENAI_API_KEY=sk-...' >> ~/.lailaba/.env
lailaba meet join https://meet.google.com/abc-defg-hij --mode realtime
```

On macOS, lailaba will **not** switch your system audio input automatically — the
...
lailaba plugins enable google_meet
lailaba meet node run --display-name my-mac --host 0.0.0.0 --port 18789
