"""`lailaba meet node ...` subcommand tree.

Wired into the existing ``lailaba meet`` parser by the plugin's top-level
...
def register_cli(subparser: argparse.ArgumentParser) -> None:
    ...
    *subparser* is the ``lailaba meet node`` argparse object — typically
    ...
    run.add_argument("--display-name", default="lailaba-meet-node")
...
def node_command(args: argparse.Namespace) -> int:
    """Dispatch for ``lailaba meet node ...``.
    ...
    if cmd == "run":
        ...
        print(f"             lailaba meet node approve <name> ws://<host>:{args.port} {token}")
        try:
