...
``_check_spotify_available()`` — when the user has not run ``lailaba auth
spotify``, the tools remain registered (so they appear in ``lailaba
...
The Spotify auth flow (``lailaba auth spotify``), CLI plumbing, and docs
