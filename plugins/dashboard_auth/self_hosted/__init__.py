...
A standards-compliant OpenID Connect Relying Party for the ``lailaba dashboard``
...
:class:`~lailaba_cli.dashboard_auth.DashboardAuthProvider` methods and touches
...
``lailaba_cli/dashboard_auth/routes.py``; this provider only:
  ...
  4. verifies the **ID token** (RS256/ES256) against the discovered
     ...
     onto a :class:`~lailaba_cli.dashboard_auth.Session`.
...
same precedence convention as the ``nous`` plugin)::
    ...
    LAILABA_DASHBOARD_OIDC_SCOPES        # optional; defaults to "openid profile email"
...
class SelfHostedOIDCProvider(DashboardAuthProvider):
    ...
    def _session_from_tokens(
        ...
        *,
        ...
        OAuth access token is intentionally NOT stored — Lailaba does not call
...
def register(ctx) -> None:
    ...
    client_id are configured (via ``LAILABA_DASHBOARD_OIDC_*`` env vars or the
    ...
    issuer = _resolve_setting(
        "LAILABA_DASHBOARD_OIDC_ISSUER", oidc_cfg.get("issuer")
    )
    client_id = _resolve_setting(
        "LAILABA_DASHBOARD_OIDC_CLIENT_ID", oidc_cfg.get("client_id")
    )
    scopes = (
        _resolve_setting("LAILABA_DASHBOARD_OIDC_SCOPES", oidc_cfg.get("scopes"))
        or _DEFAULT_SCOPES
    )
    ...
    if not issuer or not client_id:
        LAST_SKIP_REASON = (
            ...
            "(LAILABA_DASHBOARD_OIDC_ISSUER + LAILABA_DASHBOARD_OIDC_CLIENT_ID) "
