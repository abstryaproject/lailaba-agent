# Lailaba Achievements Performance Spec (Post-Hackathon)

Status: Draft (no code changes yet)
Owner: lailaba-achievements plugin
...
### `GET /api/plugins/lailaba-achievements/achievements`
...
### `POST /api/plugins/lailaba-achievements/rescan` (optional)
Manual refresh trigger.
Prefer async trigger + immediate status response.

### `GET /api/plugins/lailaba-achievements/scan-status` (optional new)
Reports scan state for UX/ops.

### Removed
- `GET /api/plugins/lailaba-achievements/overview`
...
- Persist plugin data under:
  - `~/.lailaba/plugins/lailaba-achievements/scan_snapshot.json`

### Phase 2
- Incremental scan checkpoints with per-session fingerprints.
- Persist checkpoint data under:
  - `~/.lailaba/plugins/lailaba-achievements/scan_checkpoint.json`
...
- `~/.lailaba/plugins/lailaba-achievements/`
