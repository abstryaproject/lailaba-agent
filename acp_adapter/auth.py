"""ACP auth helpers — detect and advertise Lailaba authentication methods."""
...
TERMINAL_SETUP_AUTH_METHOD_ID = "lailaba-setup"


def detect_provider() -> Optional[str]:
    """Resolve the active Lailaba runtime provider, or None if unavailable.
...
def has_provider() -> bool:
    """Return True if Lailaba can resolve any runtime provider credentials."""
    return detect_provider() is not None


def build_auth_methods() -> list[Any]:
    """Return registry-compatible ACP auth methods for Lailaba.
    ...
    not have Lailaba provider credentials configured yet, so Lailaba always
    ...
    if provider:
        methods.append(
            AuthMethodAgent(
                ...
                description=(
                    "Authenticate Lailaba using the currently configured "
