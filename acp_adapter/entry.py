"""CLI entry point for the lailaba-agent ACP adapter.

Loads environment variables from ``~/.lailaba/.env``, configures logging
...
try:
    ...
    # shadowing Lailaba's own modules — ``lailaba acp`` can be started from any
    # cwd, including a project that has same-named packages on its path.
    lailaba_bootstrap.harden_import_path()
...
def _load_env() -> None:
    """Load .env from LAILABA_HOME (default ``~/.lailaba``)."""
    from lailaba_cli.env_loader import load_lailaba_dotenv

    lailaba_home = get_lailaba_home()
    loaded = load_lailaba_dotenv(lailaba_home=lailaba_home)
    if loaded:
        for env_file in loaded:
            logging.getLogger(__name__).info("Loaded env from %s", env_file)
    else:
        logging.getLogger(__name__).info(
            "No .env found at %s, using system env", lailaba_home / ".env"
        )
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        prog="lailaba-acp",
        description="Run Lailaba AI as an ACP stdio server.",
    )
    parser.add_argument("--version", action="store_true", help="Print Lailaba version and exit")
    parser.add_argument(
        "--check",
        action="store_true",
    ...
    parser.add_argument(
        "--setup",
        action="store_true",
        help="Run interactive Lailaba provider/model setup for ACP terminal auth",
    )
    parser.add_argument(
        "--setup-browser",
        action="store_true",
        help="Install agent-browser + Playwright Chromium into ~/.lailaba/node/ "
...
def _run_check() -> None:
    ...
    print("Lailaba ACP check OK")


def _run_setup() -> None:
    from lailaba_cli.main import main as lailaba_main

    old_argv = sys.argv[:]
    try:
        sys.argv = [old_argv[0] if old_argv else "lailaba", "model"]
        lailaba_main()
    finally:
        sys.argv = old_argv
...
def _run_setup_browser(assume_yes: bool = False) -> int:
    ...
    with ``lailaba postinstall`` and the runtime lazy installer.
...
def main(argv: list[str] | None = None) -> None:
    ...
    logger.info("Starting lailaba-agent ACP adapter")
