# Lailaba AI Security Policy

This document describes Lailaba AI's trust model, names the one
security boundary the project treats as load-bearing, and defines the
scope for vulnerability reports.

## 1. Reporting a Vulnerability

Report privately via [GitHub Security Advisories](https://github.com/NousResearch/lailaba-agent/security/advisories/new)
or **security@nousresearch.com**. Do not open public issues for
security vulnerabilities. **Lailaba AI does not operate a bug
bounty program.**
...
- Environment details (`lailaba version`, commit SHA, OS, Python
  version).
...
- **Trust envelope.** The set of resources an operator has implicitly
  granted Lailaba AI access to by running it — typically, whatever
  the operator's own user account can reach on the host.
- **Stance.** An explicit statement in Lailaba AI's documentation
...
Lailaba AI supports two OS-level isolation postures. They address
...
Lailaba AI supports this in two ways:

- **Lailaba AI's own Docker image and Compose setup.** Lighter-
...
Under a whole-process wrapper, Lailaba AI's in-process heuristics
...
Lailaba AI filters the environment it passes to its lower-trust
...
A malicious or buggy plugin is not a vulnerability in Lailaba AI
itself. Bugs in Lailaba AI's plugin-install or plugin-discovery
...
4. **Within the authorized set, all callers are equally trusted.**
   Lailaba AI does not model per-caller capabilities inside a
...
- Trust-model documentation violations: code behaving contrary to
  what this policy, Lailaba AI's own documentation, or reasonable
  operator expectations would predict — including cases where
  Lailaba AI has documented a stance about how its output should
...
- **Documented break-glass settings.** Operator-selected trade-offs
  ...
  lailaba-home security, and similar. Reports against those
...
- **Community-contributed skills and plugins.** Third-party skills
  ...
  plugins are in the operator's review surface, not Lailaba AI's
  trust surface (§2.4, §2.5). A skill or plugin doing something
  malicious is the expected failure mode of one that wasn't
  reviewed, not a vulnerability in Lailaba AI. Bugs in Lailaba
...
- Lailaba AI includes supply-chain guards for MCP server
