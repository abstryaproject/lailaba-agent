...
$LAILABA_OSINT_CACHE/icij/ (default: ~/.cache/lailaba-osint/icij/) and search
...
def _cache_dir() -> Path:
    base = os.environ.get("LAILABA_OSINT_CACHE")
    if base:
        return Path(base) / "icij"
    return Path.home() / ".cache" / "lailaba-osint" / "icij"
...
def _download(dest: Path, force: bool = False) -> Path:
    ...
    req = urllib.request.Request(
        BULK_URL,
        headers={"User-Agent": "lailaba-agent osint-investigation skill"},
    )
...
def main() -> int:
    ...
    p.add_argument(
        ...
        type=Path,
        default=None,
        help="Override cache directory (default: $LAILABA_OSINT_CACHE/icij or ~/.cache/lailaba-osint/icij)",
    )
    p.add_argument(
        "--force-refresh",
        action="store_true",
