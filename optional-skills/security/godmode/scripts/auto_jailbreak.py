...
Usage in execute_code:
    exec(open(os.path.expanduser(
        os.path.join(os.environ.get("LAILABA_HOME", os.path.expanduser("~/.lailaba")), "skills/red-teaming/godmode/scripts/auto_jailbreak.py")
    )).read())
...
try:
    ...
    _SKILL_DIR = Path(os.getenv("LAILABA_HOME", Path.home() / ".lailaba")) / "skills" / "red-teaming" / "godmode"
...
# ═══════════════════════════════════════════════════════════════════
# Lailaba config paths
# ═══════════════════════════════════════════════════════════════════

LAILABA_HOME = Path(os.getenv("LAILABA_HOME", Path.home() / ".lailaba"))
CONFIG_PATH = LAILABA_HOME / "config.yaml"
PREFILL_PATH = LAILABA_HOME / "prefill.json"
...
MODEL_STRATEGIES = {
    ...
    # Nous/Lailaba models — already uncensored, just needs clean prompt
    "lailaba": {
        "order": ["prefill_only"],
        "system_templates": {},
    },
...
def _detect_model_family(model: str) -> str:
    ...
    if "grok" in model_lower or "x-ai" in model_lower:
        return "grok"
    if "lailaba" in model_lower or "nous" in model_lower:
        return "lailaba"
...
def _get_current_model() -> tuple:
    """Read current model and provider from Lailaba config.yaml.
...
def _write_prefill(prefill_messages: list):
    """Write prefill messages to ~/.lailaba/prefill.json."""
    ...
    if winning_strategy:
        ...
        if not dry_run:
            ...
            if verbose:
                ...
                print()
                print("[DONE] Jailbreak locked in. Restart Lailaba for changes to take effect.")
        else:
            if verbose:
...
def undo_jailbreak(verbose=True):
    ...
    if verbose:
        print("[UNDO] Jailbreak removed. Restart Lailaba for changes to take effect.")
