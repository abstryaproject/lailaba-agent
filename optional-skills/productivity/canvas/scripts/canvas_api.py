#!/usr/bin/env python3
"""Canvas LMS API CLI for Lailaba AI.
...
def _check_config():
    ...
    if missing:
        lailaba_env = os.path.join(
            os.environ.get("LAILABA_HOME", os.path.expanduser("~/.lailaba")), ".env"
        )
        print(
            f"Missing required environment variables: {', '.join(missing)}\n"
            f"Set them in {lailaba_env} or export them in your shell.\n"
...
def main():
    parser = argparse.ArgumentParser(
        description="Canvas LMS API CLI for Lailaba AI"
    )
