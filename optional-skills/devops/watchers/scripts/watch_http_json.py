...
Usage (via cron with --no-agent):
    ...
    lailaba cron create api-events \\
      --schedule "*/1 * * * *" --no-agent \\
      --script "$LAILABA_HOME/skills/devops/watchers/scripts/watch_http_json.py" \\
...
def main() -> int:
    ...
    req = urllib.request.Request(args.url, headers={"User-Agent": "Lailaba-Watcher/1.0"})
