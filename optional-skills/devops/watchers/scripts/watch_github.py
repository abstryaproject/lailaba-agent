...
Usage (via cron with --no-agent):
    ...
    lailaba cron create lailaba-issues \\
      --schedule "*/5 * * * *" --no-agent \\
      --script "$LAILABA_HOME/skills/devops/watchers/scripts/watch_github.py" \\
      --script-args "--name lailaba-issues --repo NousResearch/lailaba-agent --scope issues"

Set GITHUB_TOKEN (or GH_TOKEN) in the Lailaba .env file
(``${LAILABA_HOME:-~/.lailaba}/.env``) to avoid the 60 req/hr
...
def main() -> int:
    ...
    headers = {
        ...
        "User-Agent": "Lailaba-Watcher/1.0",
    }
