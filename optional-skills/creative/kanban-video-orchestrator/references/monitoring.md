...
lailaba kanban watch --tenant <project-slug>

# Snapshot of the board
lailaba kanban list --tenant <project-slug>
lailaba kanban list --tenant <project-slug> --json     # machine-readable

# Per-status counts + oldest-ready age
lailaba kanban stats --tenant <project-slug>

# Visual dashboard (browser)
lailaba dashboard

# Inspect a specific task (includes comments + events)
lailaba kanban show <task-id>

# Follow a single task's event stream
lailaba kanban tail <task-id>
```

Verify available subcommands with `lailaba kanban --help` — the kanban CLI
...
|---------|--------------|--------|
| Task RUNNING but no heartbeat in 2+ min | Worker stuck, infinite loop, blocked on input | `lailaba kanban show <id>` — read the worker's last events. The dispatcher SIGTERMs tasks that exceed their `max-runtime`; if you need to stop one earlier, `lailaba kanban block <id>` then `lailaba kanban archive <id>`, and create a re-run task. |
| Same task retried 2+ times | Reproducible failure (missing key, bad spec, broken tool) | `lailaba kanban show <id>` to read failure events. Fix root cause before re-running. |
| RUNNING longer than max_runtime | Task is slow but progressing OR genuinely stuck | Check heartbeats with `lailaba kanban tail <id>`. If progressing, the dispatcher will SIGTERM eventually anyway — raise `max-runtime` on a re-created task. |
...
| Tasks created but never picked up | Profile not running, or tenant mismatch, or dispatcher not running | Check `lailaba profile list` (profile exists?), `lailaba status` (gateway/dispatcher up?), and verify tenant. |
...
lailaba kanban comment <renderer-task-id> "Scene 3 looks too sparse \
— increase visual density. Tighten color palette to brand spec."

# 2. Create a re-render task with the original as parent
lailaba kanban create "Scene 3 — re-render with feedback" \
...
NEW_TASK_ID=$(lailaba kanban create "Generate SRT captions from voiceover" \
    --assignee captioner \
    --workspace dir:"$HOME/projects/video-pipeline/<slug>" \
    --tenant <slug> \
...
lailaba kanban link "$NEW_TASK_ID" <editor-task-id>
```
...
lailaba kanban block <task-id>
lailaba kanban archive <task-id>

# Diagnose what happened
lailaba kanban show <task-id>      # task body, comments, recent events
lailaba kanban tail <task-id>      # follow the live event stream
lailaba kanban log <task-id>       # worker process log
```
...
3. Re-fire the initial `lailaba kanban create` for the director
...
while true; do
    clear
    lailaba kanban list --tenant <slug>
    echo "---"
    lailaba kanban stats --tenant <slug>
    sleep 30
done
```

For a live event feed, run `lailaba kanban watch --tenant <slug>` in a
...
- **Profile process not running.** Tasks queue indefinitely in READY if no
  worker for that profile is online. Check `lailaba profile list` and start
