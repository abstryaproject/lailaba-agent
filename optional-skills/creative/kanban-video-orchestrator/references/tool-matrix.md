...
Maps each role archetype to the Lailaba skills it should `always_load` and the
toolsets it needs. Only references skills that ship in the public lailaba-agent
...
## Lailaba skills relevant to video production

### Visual / rendering skills (`lailaba-agent/skills/creative/`)
...
### Design / pre-production skills (`lailaba-agent/skills/creative/`)
...
### Audio / media skills (`lailaba-agent/skills/creative/` + `skills/media/`)
...
These are **not** Lailaba skills but external CLIs / APIs that profiles invoke.
...
## Built-in Lailaba tools for media review

These are native Lailaba tools — not invoked via terminal but through their own
...
key is present in `${LAILABA_HOME:-~/.lailaba}/.env` (or macOS Keychain) before firing the kanban.
...
| Anthropic | `ANTHROPIC_API_KEY` | every Lailaba profile (Claude) |

If a key is missing, prompt the user to add it. Storage methods, in order of
preference: macOS Keychain → `${LAILABA_HOME:-~/.lailaba}/.env` → environment variable.
...
When a new Lailaba-public video skill ships:
