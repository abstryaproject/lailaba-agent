...
the actual `setup.sh` that creates the project workspace, configures Lailaba
...
2. **Create profiles** — `lailaba profile create <name> --clone`
3. **Configure profiles** — patch each profile's
   `~/.lailaba/profiles/<name>/config.yaml` to set toolsets, always_load skills,
   and `cwd`
4. **Write SOUL.md per profile** — the personality + role definition
5. **Copy any provided assets + write `brief.md`, `TEAM.md`, and `taste/`**
6. **Fire the initial kanban task** — `lailaba kanban create` assigned to the director
...
```bash
lailaba profile create director --clone 2>/dev/null || true
```
...
Each profile has a YAML config at `~/.lailaba/profiles/<name>/config.yaml`. The
...
p = os.path.expanduser(f"~/.lailaba/profiles/{profile}/config.yaml")
with open(p) as f:
...
PyYAML must be installed in the user's Python (it ships with most Lailaba
...
Each profile gets a `SOUL.md` at `~/.lailaba/profiles/<name>/SOUL.md` that
...
```bash
lailaba kanban create "Direct production of <video title>" \
    --assignee director \
...
the Lailaba `.env` (`${LAILABA_HOME:-$HOME/.lailaba}/.env`) and macOS Keychain
(if on macOS):
...
check_key() {
    ...
    local kc_account="$2"
    local kc_service="$3"
    local _lailaba_env="${LAILABA_HOME:-$HOME/.lailaba}/.env"
    if grep -q "^${var}=" "$_lailaba_env" 2>/dev/null && \
       [ -n "$(grep "^${var}=" "$_lailaba_env" | cut -d= -f2-)" ]; then
        return 0
    fi
    ...
    echo "ERROR: ${var} not set in ${_lailaba_env} or Keychain (${kc_account}/${kc_service})"
    return 1
}

check_key ELEVENLABS_API_KEY lailaba ELEVENLABS_API_KEY || exit 1
check_key OPENROUTER_API_KEY lailaba OPENROUTER_API_KEY || exit 1
# ...
```
