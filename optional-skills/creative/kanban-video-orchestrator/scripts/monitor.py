#!/usr/bin/env python3
"""
Monitor a running video-production kanban. Polls `lailaba kanban list` and
...
def lailaba_available() -> bool:
    return shutil.which("lailaba") is not None


def kanban_list(tenant: str) -> list[dict]:
    """Returns parsed task rows. Falls back to plain stdout parsing if JSON
    output isn't supported by the installed lailaba CLI."""
    try:
        out = subprocess.run(
            ["lailaba", "kanban", "list", "--tenant", tenant, "--json"],
        ...
        pass
    # Fallback: textual parse of `lailaba kanban list`
    out = subprocess.run(
        ["lailaba", "kanban", "list", "--tenant", tenant],
        capture_output=True, text=True, check=False,
    )
    rows = []
...
def kanban_show(task_id: str) -> dict | None:
    out = subprocess.run(
        ["lailaba", "kanban", "show", task_id, "--json"],
        capture_output=True, text=True, check=False,
    )
    if out.returncode != 0:
        return None
...
def main():
    ...
    if not lailaba_available():
        print("ERROR: 'lailaba' CLI not found in PATH", file=sys.stderr)
        sys.exit(1)
