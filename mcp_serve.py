"""
Lailaba MCP Server — expose messaging conversations as MCP tools.
...
Plus: channels_list (Lailaba-specific extra)

Usage:
    lailaba mcp serve
    lailaba mcp serve --verbose
...
logger = logging.getLogger("lailaba.mcp_serve")
...
def _get_sessions_dir() -> Path:
    """Return the sessions directory using LAILABA_HOME."""
    try:
        from lailaba_constants import get_lailaba_home
        return get_lailaba_home() / "sessions"
    except ImportError:
        return Path(os.environ.get("LAILABA_HOME", Path.home() / ".lailaba")) / "sessions"
...
def _load_channel_directory() -> dict:
    ...
    try:
        from lailaba_constants import get_lailaba_home
        directory_file = get_lailaba_home() / "channel_directory.json"
    except ImportError:
        directory_file = Path(
            os.environ.get("LAILABA_HOME", Path.home() / ".lailaba")
...
class EventBridge:
    ...
    This is the Lailaba equivalent of OpenClaw's WebSocket gateway bridge.
    ...
    def _poll_once(self, db):
        ...
        try:
            from lailaba_constants import get_lailaba_home
            db_file = get_lailaba_home() / "state.db"
        except ImportError:
            db_file = Path(os.environ.get("LAILABA_HOME", Path.home() / ".lailaba")) / "state.db"
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "FastMCP":
    """Create and return the Lailaba MCP server with all tools registered."""
    ...
    mcp = FastMCP(
        "lailaba",
        instructions=(
            "Lailaba AI messaging bridge. Use these tools to interact with "
...
def run_mcp_server(verbose: bool = False) -> None:
    """Start the Lailaba MCP server on stdio."""
    if not _MCP_SERVER_AVAILABLE:
        print(
