...
// (e.g. https://mission-control.tilos.com/lailaba/). The Python backend
// injects ``window.__LAILABA_BASE_PATH__`` into index.html based on the
// incoming ``X-Forwarded-Prefix`` header so the SPA can address its own
// ``/api/...`` and ``/dashboard-plugins/...`` URLs correctly without a
// rebuild. Empty string means "served at root".
function readBasePath(): string {
  if (typeof window === "undefined") return "";
  const raw = window.__LAILABA_BASE_PATH__ ?? "";
  if (!raw) return "";
...
export const LAILABA_BASE_PATH = readBasePath();
const BASE = LAILABA_BASE_PATH;
...
declare global {
  interface Window {
    __LAILABA_SESSION_TOKEN__?: string;
    __LAILABA_BASE_PATH__?: string;
    ...
    __LAILABA_AUTH_REQUIRED__?: boolean;
  }
}
let _sessionToken: string | null = null;
const SESSION_HEADER = "X-Lailaba-Session-Token";
...
export async function fetchJSON<T>(
  ...
  const token = window.__LAILABA_SESSION_TOKEN__;
  if (token) {
  ...
  if (res.status === 401) {
    ...
    // (``lailaba update``, ``lailaba gateway restart``, etc.). A tab kept
    // open across the restart holds the OLD token in
    // ``window.__LAILABA_SESSION_TOKEN__`` from the previous HTML render,
    ...
    if (!window.__LAILABA_AUTH_REQUIRED__ && !options?.allowUnauthorized) {
      let alreadyReloaded = false;
      try {
        alreadyReloaded =
          sessionStorage.getItem("lailaba.tokenReloadAttempted") === "1";
      } catch {
        /* SSR / privacy mode — fall through to throw */
      }
      if (!alreadyReloaded) {
        try {
          sessionStorage.setItem("lailaba.tokenReloadAttempted", "1");
        } catch {
  ...
  if (res.ok) {
    ...
    // current ``window.__LAILABA_SESSION_TOKEN__`` is valid, so the next
    // 401 — if any — should be allowed to trigger its own reload cycle.
    try {
      sessionStorage.removeItem("lailaba.tokenReloadAttempted");
    } catch {
      /* SSR / privacy mode — ignore */
    }
  }
...
async function getSessionToken(): Promise<string> {
  ...
  const injected = window.__LAILABA_SESSION_TOKEN__;
  if (injected) {
    _sessionToken = injected;
    return _sessionToken;
  }
  throw new Error("Session token not available — page must be served by the Lailaba dashboard server");
}

/**
 * Fetch a single-use ticket for a WebSocket upgrade in gated mode.
 *
 * The dashboard's gated-mode WS auth (``lailaba_cli.web_server._ws_auth_ok``)
...
export async function buildWsAuthParam(): Promise<[string, string]> {
  if (window.__LAILABA_AUTH_REQUIRED__) {
 ...
 *  - loopback / ``--insecure``: attach the ``X-Lailaba-Session-Token`` header.
 *  - gated OAuth: no token header (it's absent by design); the
 *    ``lailaba_session_at`` cookie rides along via ``credentials: 'include'``.
 *
 ...
 * ``window.__LAILABA_SESSION_TOKEN__`` directly, so the gated-mode ticket
...
export const api = {
  ...
  updateHermes: () =>
    fetchJSON<ActionResponse>("/api/lailaba/update", { method: "POST" }),
  checkHermesUpdate: (force = false) =>
    fetchJSON<UpdateCheckResponse>(
      `/api/lailaba/update/check${force ? "?force=true" : ""}`,
    ),
...
export interface SkillHubSource {
  ...
  /** lailaba-index only: whether the centralized index loaded. */
  available?: boolean;
}
...
export interface StatusResponse {
   ...
   * updates are handled by the outer launcher instead of ``lailaba update``. */
  can_update_lailaba?: boolean;
  config_path: string;
