# Lailaba AI Has Had "Routines" Since March
...
Lailaba equivalent — works today:
```bash
...
Lailaba equivalent — works today:
```bash
lailaba webhook subscribe auth-watch \
  --events "pull_request" \
...
Every use case in their blog post — backlog triage, docs drift, deploy verification, alert correlation, library porting, bespoke PR review — has a working Lailaba implementation. No new features needed. It's been shipping since March 2026.

---

## What's Different

| | Claude Code Routines | Lailaba AI |
|---|---|---|
...
## Things Lailaba Does That Routines Can't
...
Lailaba has no daily limit. Run 500 automations a day if you want. The only constraint is your API budget, and you choose which models to use for which tasks.
...
Lailaba AI is open source and free. The automation infrastructure — cron scheduler, webhook platform, skill system, multi-platform delivery — is built in.
...
```bash
lailaba gateway setup    # enable webhooks
lailaba webhook subscribe pr-review \
  --events "pull_request" \
...
Full automation blueprints gallery: [lailaba-agent.nousresearch.com/docs/reference/automation-blueprints-catalog](https://lailaba-agent.nousresearch.com/docs/reference/automation-blueprints-catalog)

Documentation: [lailaba-agent.nousresearch.com](https://lailaba-agent.nousresearch.com)

GitHub: [github.com/NousResearch/lailaba-agent](https://github.com/NousResearch/lailaba-agent)

---

*Lailaba AI is built by [Nous Research](https://nousresearch.com). Open source, model-agnostic, runs on your infrastructure.*
