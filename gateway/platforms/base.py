...
# Audio file extensions Lailaba recognizes for native audio delivery.
...
def _thread_metadata_for_source(source, reply_to_message_id: str | None = None) -> dict | None:
    ...
    value. Telegram private-chat topics created through Lailaba' DM-topic helper
...
def _reply_anchor_for_event(event) -> str | None:
    ...
    replying to the triggering message. Lailaba-created Telegram private-chat
...
from lailaba_constants import get_default_lailaba_root, get_lailaba_dir, get_lailaba_home


GATEWAY_SECRET_CAPTURE_UNSUPPORTED_MESSAGE = (
    "Secure secret entry is not supported over messaging. "
    "Load this skill in the local CLI to be prompted, or add the key to ~/.lailaba/.env manually."
)
...
# Default location: {LAILABA_HOME}/cache/images/ (legacy: image_cache/)
IMAGE_CACHE_DIR = get_lailaba_dir("cache/images", "image_cache")
...
AUDIO_CACHE_DIR = get_lailaba_dir("cache/audio", "audio_cache")
...
VIDEO_CACHE_DIR = get_lailaba_dir("cache/videos", "video_cache")
...
DOCUMENT_CACHE_DIR = get_lailaba_dir("cache/documents", "document_cache")
SCREENSHOT_CACHE_DIR = get_lailaba_dir("cache/screenshots", "browser_screenshots")
_LAILABA_HOME = get_lailaba_home()
_LAILABA_ROOT = get_default_lailaba_root()
MEDIA_DELIVERY_ALLOW_DIRS_ENV = "LAILABA_MEDIA_ALLOW_DIRS"
MEDIA_DELIVERY_TRUST_RECENT_ENV = "LAILABA_MEDIA_TRUST_RECENT_FILES"
MEDIA_DELIVERY_TRUST_RECENT_SECONDS_ENV = "LAILABA_MEDIA_TRUST_RECENT_SECONDS"
...
MEDIA_DELIVERY_STRICT_ENV = "LAILABA_MEDIA_DELIVERY_STRICT"
MEDIA_DELIVERY_SAFE_ROOTS = (
    ...
    _LAILABA_HOME / "image_cache",
    _LAILABA_HOME / "audio_cache",
    _LAILABA_HOME / "video_cache",
    _LAILABA_HOME / "document_cache",
    _LAILABA_HOME / "browser_screenshots",
    ...
    _LAILABA_HOME / "cache" / "images",
    _LAILABA_HOME / "cache" / "audio",
    _LAILABA_HOME / "cache" / "videos",
    _LAILABA_HOME / "cache" / "documents",
    _LAILABA_HOME / "cache" / "screenshots",
)
...
def _profile_cache_roots() -> List[Path]:
    """Return per-profile canonical cache roots under the shared Lailaba root.

    Profile gateways write generated artifacts to
    ``<root>/profiles/<name>/cache/{images,audio,...}``. The static safe-roots
    list only covers the *active* LAILABA_HOME's cache, so a gateway running at
    the root (e.g. ``LAILABA_HOME=/opt/data``) while the model emits a
    ...
    is consulted (which otherwise wins when LAILABA_HOME is symlinked under a
    denied prefix and $HOME is not that prefix). See issue #31733.
    """
    roots: List[Path] = []
    profiles_dir = _LAILABA_ROOT / "profiles"
    try:
...
def _media_delivery_denied_paths() -> List[Path]:
    ...
    # The active Lailaba profile and shared Lailaba root both contain control
    ...
    # LAILABA_HOME root. The set mirrors the canonical read guard in
    ...
    # tree, so skills/, logs/, and ad-hoc agent-written files under ~/.lailaba
    ...
    for lailaba_root in (_LAILABA_HOME, _LAILABA_ROOT):
        for rel in _ROOT_CREDENTIAL_FILES:
            denied.append(lailaba_root / rel)
...
def _path_under_denied_prefix(resolved: Path) -> bool:
    ...
    sub-directories inside home (``~/.ssh``, ``~/.aws``, ...) and Lailaba
    secrets (``~/.lailaba/.env``, ``auth.json``) are *separate, more-specific*
...
def validate_media_delivery_path(path: str) -> Optional[str]:
    ...
    ``LAILABA_MEDIA_DELIVERY_STRICT=1``): the file MUST live under a
    Lailaba-managed cache, under an operator-allowlisted root
    (``LAILABA_MEDIA_ALLOW_DIRS``), or be freshly produced inside the
    ...
    # credential/secret stores under the Lailaba root (~/.lailaba/.env,
    ...
    # ``MEDIA:~/.lailaba/google_token.json``) remain rejected.
...
_PLAINTEXT_GATEWAY_RESTART_PATTERNS: tuple[re.Pattern[str], ...] = (
    ...
    re.compile(r"^(?:please\s+)?restart\s+(?:the\s+)?lailaba\s+gateway[.!?\s]*$", re.IGNORECASE),
    re.compile(r"^(?:please\s+)?restart\s+lailaba[.!?\s]*$", re.IGNORECASE),
)
...
class BasePlatformAdapter(ABC):
    ...
    # into the ``LAILABA_SESSION_ASYNC_DELIVERY`` contextvar at session-bind
    ...
    # Lailaba commands.  Default "/" (most platforms deliver "/approve" etc.
    ...
    def __init__(self, config: PlatformConfig, platform: Platform):
        ...
        self._busy_text_mode: str = (
            os.environ.get("LAILABA_GATEWAY_BUSY_TEXT_MODE", "interrupt").strip().lower()
            or "interrupt"
        )
        self._busy_text_debounce_seconds: float = _float_env(
            "LAILABA_GATEWAY_BUSY_TEXT_DEBOUNCE_SECONDS", 0.35
        )
        self._busy_text_hard_cap_seconds: float = _float_env(
            "LAILABA_GATEWAY_BUSY_TEXT_HARD_CAP_SECONDS", 1.0
        )
        ...
        # Chats in ``voice_only`` mode reply with voice audio and NO readable
        # text (caption or separate text message). Distinct from ``all`` mode,
        # which sends voice + text. Populated by the gateway runner's
        # ``_sync_voice_mode_state_to_adapter`` from persisted ``/voice`` state.
        self._voice_only_chats: set = set()
    ...
    def prefers_fresh_final_streaming(
        ...
        implementation supports. Telegram is the motivating case: Lailaba sends
    ...
    def _mask_json_string_media(content: str) -> str:
        ...
        Serialized tool results frequently embed a previous reply's text, e.g.::
            ...
            {"result": "MEDIA:/Users/x/.lailaba/media/generated/stale.png"}
    ...
    async def handle_message(self, event: MessageEvent) -> None:
        ...
        if session_key in self._active_sessions:
            ...
            from lailaba_cli.commands import should_bypass_active_session
    ...
    def _get_human_delay() -> float:
        ...
        Reads from env vars:
          LAILABA_HUMAN_DELAY_MODE: "off" (default) | "natural" | "custom"
          LAILABA_HUMAN_DELAY_MIN_MS: minimum delay in ms (default 800, custom mode)
          LAILABA_HUMAN_DELAY_MAX_MS: maximum delay in ms (default 2500, custom mode)
        """
        mode = os.getenv("LAILABA_HUMAN_DELAY_MODE", "off").lower()
        if mode == "off":
            return 0.0
        if mode == "natural":
        ...
        try:
            min_ms = int(os.getenv("LAILABA_HUMAN_DELAY_MIN_MS", "800"))
        except (TypeError, ValueError):
            min_ms = 800
        try:
            max_ms = int(os.getenv("LAILABA_HUMAN_DELAY_MAX_MS", "2500"))
        except (TypeError, ValueError):
            max_ms = 2500
        return random.uniform(min_ms / 1000.0, max_ms / 1000.0)
    ...
    async def _process_message_background(self, event: MessageEvent, session_key: str) -> None:
        ...
        try:
            ...
            if response:
                ...
                # True voice-only mode: send voice audio with NO readable text
                # caption (chat is explicitly in ``voice_only`` mode via /voice on).
                _voice_only = bool(getattr(self, "_voice_only_chats", set())) and (
                    event.source.chat_id in getattr(self, "_voice_only_chats", set())
                )
                # Tracks whether the voice audio was actually delivered. Used by
                # the text-suppression guard below (the file is deleted right
                # after play_tts, so we can't rely on _tts_path existing).
                ...
                if _tts_path and Path(_tts_path).exists():
                    try:
                        ...
                        _tts_audio_delivered = bool(getattr(tts_result, "success", False))
                    except Exception as _ptts_exc:
                        logger.warning("[%s] play_tts failed: %s", self.name, _ptts_exc)
                    finally:
                        try:
                            os.remove(_tts_path)
                        except OSError:
                ...
                # In true voice-only mode, suppress the readable text entirely
                # when voice audio was delivered — either via the auto-TTS block
                # (_tts_audio_delivered) or because the agent emitted a TTS MEDIA
                # attachment (media_files contains the generated .mp3/.ogg/...).
                # This is what makes the reply "voice only" with no transcript leak.
                #
                # CRITICAL: only applies when the INBOUND message is a voice NOTE
                # (MessageType.VOICE). A plain AUDIO file attachment (MessageType.AUDIO)
                # must NOT trigger voice-only — it gets a normal text reply, per the
                # "voice only on voice chat, not audio" rule.
            ...
            # not at the top of this task.  _lailaba_run_generation is set on
