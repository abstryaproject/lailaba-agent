...
class WhatsAppBehaviorMixin:
    ...
    DEFAULT_REPLY_PREFIX: str = "⚕ *Lailaba AI*\n────────────\n"
...
def resolve_whatsapp_bridge_dir() -> Path:
    """Resolve the WhatsApp bridge directory, mirroring to LAILABA_HOME if needed.

    When the install tree is read-only (e.g., Docker /opt/lailaba), this function
    mirrors the bridge source to a writable LAILABA_HOME location and returns that
    ...
    # Try LAILABA_HOME location first
    lailaba_home = get_lailaba_home()
    lailaba_home_bridge = lailaba_home / "scripts" / "whatsapp-bridge"
    ...
    # Install dir is read-only, mirror to LAILABA_HOME if needed
    if lailaba_home_bridge.exists():
        return lailaba_home_bridge

    # Mirror the bridge source to LAILABA_HOME
    try:
        lailaba_home_bridge.parent.mkdir(parents=True, exist_ok=True)
        shutil.copytree(
            install_bridge,
            lailaba_home_bridge,
            dirs_exist_ok=False,
