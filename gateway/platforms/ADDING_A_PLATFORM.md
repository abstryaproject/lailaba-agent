...
There are two ways to add a platform to the Lailaba gateway:

## Plugin Path (Recommended for Community/Third-Party)

Create a plugin directory in `~/.lailaba/plugins/` (or under `plugins/platforms/`
...
**zero changes to core Lailaba code**.
...
- `env_enablement_fn: () -> Optional[dict]` — seeds `PlatformConfig.extra`
  ...
  `lailaba gateway status` or `get_connected_platforms()` until the SDK
  instantiates.
...
- `plugin.yaml` `requires_env` / `optional_env` rich-dict entries —
  auto-populate `OPTIONAL_ENV_VARS` in `lailaba_cli/config.py` so the setup
...
Checklist for integrating a platform directly into the Lailaba core.
...
And add it to the `lailaba-gateway` composite:

```python
"lailaba-gateway": {
    "includes": [..., "lailaba-your-platform"]
}
```
...
## 12. Status Display (`lailaba_cli/status.py`)
...
## 13. Gateway Setup Wizard (`lailaba_cli/gateway.py`)
...
grep -r "telegram\|discord\|whatsapp\|slack" gateway/ tools/ agent/ cron/ lailaba_cli/ toolsets.py \
  --include="*.py" -l | sort -u
