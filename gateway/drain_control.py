...
Contract (presence-based, mirroring ``.restart_notify.json``):
  ...
  * begin-drain  → write ``{LAILABA_HOME}/.drain_request.json`` with
...
Why the epoch (NS-570). ``LAILABA_HOME`` is a **durable** store — on Lailaba
...
def current_instantiation_epoch() -> str:
    ...
    Together they discriminate every restart mode that matters:
      ...
      | host ``lailaba gateway restart``| same    | same(init) | SAME   | honour |
...
def drain_request_path(home: Optional[Path] = None) -> Path:
    """Absolute path to the drain-request marker, respecting LAILABA_HOME."""
    base = home if home is not None else get_lailaba_home()
...
def write_drain_request(
    *, principal: str = "drain-control", home: Optional[Path] = None
    ...
    later survives a machine restart on the durable LAILABA_HOME volume can be
...
def drain_requested(*, home: Optional[Path] = None) -> bool:
    ...
    treated as absent: it survived a container/VM restart (LAILABA_HOME is a
    durable Fly volume on Lailaba Cloud) and the lifecycle action that triggered
