...
class GatewayConfig:
    ...
    # Storage paths
    sessions_dir: Path = field(default_factory=lambda: get_lailaba_home() / "sessions")
    ...
    # gateway behaves exactly as before — single LAILABA_HOME, no profile stamping.
...
def load_gateway_config() -> GatewayConfig:
    ...
    2. ~/.lailaba/config.yaml (primary user-facing config)
    3. ~/.lailaba/gateway.json (legacy — provides defaults under config.yaml)
    4. Built-in defaults
    """
    _home = get_lailaba_home()
    gw_data: dict = {}
    ...
    try:
        ...
        if config_yaml_path.exists():
            ...
            # through lailaba_cli.config.load_config, so without this a managed
            ...
            if not isinstance(streaming_cfg, dict):
                ...
                # ``lailaba config set gateway.streaming.*``
