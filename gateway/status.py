...
The PID file lives at ``{LAILABA_HOME}/gateway.pid``.  LAILABA_HOME defaults to
``~/.lailaba`` but can be overridden via the environment variable.  This means
separate LAILABA_HOME directories naturally get separate PID files — a property
...
def _get_pid_path() -> Path:
    """Return the path to the gateway PID file, respecting LAILABA_HOME."""
...
def _get_lock_dir() -> Path:
    ...
    override = os.getenv("LAILABA_GATEWAY_LOCK_DIR")
    if override:
        return Path(override)
    state_home = Path(os.getenv("XDG_STATE_HOME", Path.home() / ".local" / "state"))
    return state_home / "lailaba" / _LOCKS_DIRNAME
...
def _gateway_command_subcommand(command: str | None) -> str | None:
    """Return the Lailaba gateway lifecycle subcommand from a command line.
    ...
    test also matched ``lailaba_cli.main gateway status`` and even unrelated
    ...
    (``"C:\\Program Files\\...\\lailaba-gateway.exe"``) survive, and strips
    ``--profile``/``-p`` selectors from anywhere in argv -- Lailaba's
    ...
    for token in tokens:
        ...
        if basename in ("lailaba-gateway", "lailaba-gateway.exe"):
            return "run"

    joined = " ".join(tokens)
    has_gateway_entry = (
        "lailaba_cli.main" in joined
        or "lailaba_cli/main.py" in joined
        or any(t.rsplit("/", 1)[-1] in ("lailaba", "lailaba.exe") for t in tokens)
    )
    ...
    for i, token in enumerate(filtered):
        if token != "gateway":
            continue
        if i + 1 >= len(filtered):
            return "run"  # bare `lailaba gateway` defaults to `run`
        return filtered[i + 1]
    return None
...
def looks_like_gateway_runtime_command_line(command: str | None) -> bool:
    ...
    use this broader matcher only when validating Lailaba-owned runtime records
...
def _looks_like_gateway_process(pid: int) -> bool:
    """Return True when the live PID still looks like the Lailaba gateway."""
...
def _profile_name_for_home(profile_home: Path) -> Optional[str]:
    """Return the profile id a LAILABA_HOME directory represents, or None.

    A named profile's home is ``<root>/profiles/<name>`` (immediate parent is
    ``profiles``).  The root/default home (``~/.lailaba`` or ``$LAILABA_HOME``)
...
def _command_line_belongs_to_profile(command: str, profile_home: Path) -> bool:
    ...
    Mirrors ``lailaba_cli.gateway._matches_current_profile`` so the dashboard's
    ...
    explicit ``LAILABA_HOME=<path>``) on its argv; the default/root gateway runs
    ...
    if profile_name is not None and profile_name != "default":
        ...
        return (
            ...
            or f"lailaba_home={home_lc}" in command_lc
        )
    ...
    # a non-matching explicit LAILABA_HOME= on the argv. LAILABA_HOME is usually
    ...
    if "--profile " in command_lc or " -p " in command_lc:
        return False
    if "lailaba_home=" in command_lc and f"lailaba_home={home_lc}" not in command_lc:
        return False
    return True
...
def write_runtime_status(
    *,
    ...
    if served_profiles is not _UNSET:
        ...
        # for a single-profile gateway. Lets `lailaba status` show per-profile
...
def read_runtime_status(path: Optional[Path] = None) -> Optional[dict[str, Any]]:
    ...
    can do so without mutating ``LAILABA_HOME`` in-process.  Defaults to
...
def get_runtime_status_running_pid(
    ...
    *,
    ...
    LAILABA_HOME.  Pass it when validating *another* profile's state file (the
...
def acquire_scoped_lock(scope: str, identity: str, metadata: Optional[dict[str, Any]] = None) -> tuple[bool, Optional[dict[str, Any]]]:
    ...
    at once (e.g. the same Telegram bot token across different LAILABA_HOME dirs).
    """
...
# enabled in the user's systemd (e.g. ``lailaba.service`` + ``lailaba-
# gateway.service``).
#
...
def _consume_pid_marker_for_self(
    ...
    *,
    ...
    # running under a different LAILABA_HOME. When two profile gateway
    # services share the same default ~/.lailaba (LAILABA_HOME not set
    ...
    # field is absent in markers written by older Lailaba versions; treat
    ...
    replacer_home = record.get("replacer_lailaba_home")
    if replacer_home is not None and replacer_home != str(get_lailaba_home()):
        return False
    ...
    # ``lailaba gateway stop`` on Windows would be misclassified as an
...
def write_takeover_marker(target_pid: int) -> bool:
    ...
    try:
        ...
        record = {
            ...
            "replacer_lailaba_home": str(get_lailaba_home()),
